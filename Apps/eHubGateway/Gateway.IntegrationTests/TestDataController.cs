...
namespace CargoWise.eHub.Gateway.IntegrationTests
{
	public class TestDataController
	{
		...
		public void LoadTestData()
		{
			...
			if (testDataResourceNames.Count() > 0)
			{
				foreach (var testDataName in testDataResourceNames)
				{
					...
					string dbName;
					var database = new SqlDbUnitTest(SqlServerHelper.GetAdminConnectionString(DatabaseNameSchemaMapping.TryGetValue(name, out dbName) ? dbName : name.Replace("_", ".")));
					using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream(AssembleyName + SchemaLocation + name + ".xsd"))
					{
		...
		Dictionary<string, string> DatabaseNameSchemaMapping = new Dictionary<string, string>()
		{
			{ "CargoWise_eServices_Billing", DatabaseSetup.BillingDatabaseName }
		};
	}
}
