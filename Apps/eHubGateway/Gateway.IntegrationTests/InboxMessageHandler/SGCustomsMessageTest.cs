...
{
	[TestFixture]
	public class SGCustomsMessageTest : GatewayIntegrationTestBase
	{
		[Test]
		public void TestSGCustomsMessageTest_SenderHasTestLicense_MessageInsertedIntoInbox()
		{
		...
		[Test]
		public void TestSGCustomsMessageTest_SenderHasProductionLicense_MessageInsertedIntoInbox()
		{
			using (var stream = new MemoryStream(Encoding.ASCII.GetBytes(SampleMessage)))
			{
		...
		[Test]
		public void TestSGCustomsMessageTest_SenderHasInvalidLicense_MessageNotInsertedIntoInbox()
		{
			using (var stream = new MemoryStream(Encoding.ASCII.GetBytes(SampleMessage)))
			{
		...
		[Test]
		public void TestSGCustomsMessageTest_MonitoringSender_MessageInsertedIntoInbox()
		{
			using (var stream = new MemoryStream(Encoding.ASCII.GetBytes(SampleMessage)))
			{
				var message = new eHubMessage(Guid.NewGuid(), SGCustomsMonitoringClientID, RecipientIDForProduction, MessageSchemaType.Xml, ApplicationCode, SchemaName, stream);
				...
				Assert.IsFalse(FindInboxMessage(message.TrackingID, SGCustomsMonitoringClientPK, ApplicationCode, RecipientPKForTest, SchemaName, "", "", content, 0));
				Assert.IsTrue(FindInboxMessage(message.TrackingID, SGCustomsMonitoringClientPK, ApplicationCode, RecipientPKForProduction, SchemaName, "", "", content, 0));
			}
		}

		private const string ApplicationCode = "UDM";

		private readonly Guid RecipientPKForTest = Guid.Parse("38602D21-8720-435D-B7F5-F414DA5A43BA"); // ID = SGCustomsTest

		private const string RecipientIDForProduction = "SGCustoms";
		...
		private const string SGCustomsMonitoringClientID = "T_____SGC";
		private static readonly Guid SGCustomsMonitoringClientPK = new Guid("82E68FED-2226-4BAC-9A81-B6F5DFFB1B8E");
