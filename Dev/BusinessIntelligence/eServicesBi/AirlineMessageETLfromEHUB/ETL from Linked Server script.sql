...
DECLARE @dt2 DATE = DATEADD(DAY, + 32, @dt0);
  ...
  eHubTransactions.dbo.DecodeAndDecompress( AM_SenderMessageRaw ) AS [Raw Message],
  CAST(m.AM_RecipientMessageXML as nvarchar(max)) AS AM_RecipientMessageXML , m.AM_ReceivedFromSenderUTC,
  ...
  m.AM_ErrorMessage
   FROM eHubArchiveMessage m WITH (NOLOCK)
       LEFT JOIN [eHubTransactions]..eHubClient so WITH (NOLOCK) ON so.CC_PK = m.AM_CC_SenderOutbox
       LEFT JOIN [eHubTransactions]..eHubClient ro WITH (NOLOCK) ON ro.CC_PK = m.AM_CC_RecipientOutbox
       WHERE (
