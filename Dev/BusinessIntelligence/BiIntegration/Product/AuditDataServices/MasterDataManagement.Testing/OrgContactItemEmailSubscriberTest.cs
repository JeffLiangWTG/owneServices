...
namespace Enterprise.AuditDataServices.MDM.Testing
{
	...
	class OrgContactItemEmailSubscriberTest : PatternMatchingBaseSubscriberTest<OrgContactItem, OrgContactItemSubscriber>
	{
		...
		{
			AssertPatternMatchingRecords(factory, bizO, "aabbcc@aabbcc.com");
		}

		void AssertPatternMatchingRecords(BusinessObjectFactory factory, OrgContactItem bizO, string email)
		{
			...
			AssertEquals(bizO.Contact.Person.PK, firstPatternMatchingEmail.PME_PER);
			AssertEquals(OrgContactItemSchema.Constants.Prefix, firstPatternMatchingEmail.PME_ParentTableCode);
			AssertEquals(bizO.PK, firstPatternMatchingEmail.PME_ParentId);
			AssertEquals(bizO.Contact.Person.PER_RN_NKCountry, firstPatternMatchingEmail.PME_RN_NKCountryCode);
		}

		protected override void AssertPatternMatchingForNewRecords(BusinessObjectFactory factory, OrgContactItem bizO)
		{
			AssertPatternMatchingRecords(factory, bizO, "qwert@asd.com");
		}

		protected override void AssertPatternMatchingForNonSubscribedColumns(BusinessObjectFactory factory, OrgContactItem bizO)
		{
			var patternMatchingEmailFirst = factory.Load<PatternMatchingEmail>(new ZQuery(PatternMatchingEmailSchema.PME_ParentId, bizO.PK)).ToList().Single();
			AssertEquals(GetUpperValueToHash(bizO.OI_Address), patternMatchingEmailFirst.PME_HashedValue);
		}
		...
		protected override OrgContactItem CreateParentRecords(BusinessObjectFactory factory)
		{
			...
			contactItem.OI_Address = "qwert@asd.com";
			contactItem.OI_ContactItemType = OrgContactItemTypes.Codes.Email;
		...
		{
			SubscriberTestUtilities.CreatePatternMatchingBusinessObjectForPerson<PatternMatchingEmail>(factory, bizO.Contact.Person.PK, GetUpperValueToHash(bizO.OI_Address), OrgContactItemSchema.Constants.Prefix, bizO.PK, bizO.Contact.Person.PER_RN_NKCountry);
		}
		...
		protected override OrgContactItem CreatePlaceholderRecords(BusinessObjectFactory factory)
		{
			...
			var contactItem = orgContact.ContactItems.AddNew();
			contactItem.OI_Address = "test@test.com";
			contactItem.OI_ContactItemType = OrgContactItemTypes.Codes.Email;
		...
		{
			return new BetterLogForTest(LogType.Information, string.Format(CultureInfo.InvariantCulture, GetLog, GetPatternMasters(bizO), "Modified", bizO.PK, OrgContactItemTypes.Codes.Email, "test@test.com"));
		}

		protected override BetterLogForTest GetLogForExistingRecords(OrgContactItem bizO)
		{
			return new BetterLogForTest(LogType.Information, string.Format(CultureInfo.InvariantCulture, GetLog, GetPatternMasters(bizO), "Modified", bizO.PK, OrgContactItemTypes.Codes.Email, "aabbcc@aabbcc.com"));
		}

		protected override BetterLogForTest GetLogForNewRecords(OrgContactItem bizO)
		{
			return new BetterLogForTest(LogType.Information, string.Format(CultureInfo.InvariantCulture, GetLog, GetPatternMasters(bizO), "Added", bizO.PK, OrgContactItemTypes.Codes.Email, bizO.OI_Address));
		}
		...
		{
			return string.Format(CultureInfo.InvariantCulture, GetGenericQuery, "2", bizO.PK, bizO.Contact.PK, OrgContactItemTypes.Codes.Email, bizO.OI_Address, bizO.OI_Description);
		}

		protected override string GetNonSubscribedColumnsCDCInsertQuery(OrgContactItem bizO)
		{
			return string.Format(CultureInfo.InvariantCulture, GetGenericQuery, "4", bizO.PK, bizO.Contact.PK, OrgContactItemTypes.Codes.Email, bizO.OI_Address, "ABC");
		}

		protected override string GetOriginalRecordCDCInsertQuery(OrgContactItem bizO)
		{
			return string.Format(CultureInfo.InvariantCulture, GetGenericQuery, "3", bizO.PK, bizO.Contact.PK, OrgContactItemTypes.Codes.Email, bizO.OI_Address, bizO.OI_Description);
		}

		protected override string GetUpdateRecordCDCInsertQuery(OrgContactItem bizO)
		{
			return string.Format(CultureInfo.InvariantCulture, GetGenericQuery, "4", bizO.PK, bizO.Contact.PK, OrgContactItemTypes.Codes.Email, "aabbcc@aabbcc.com", bizO.OI_Description);
		}

		protected override string GetUpdateRecordToPlaceHolderCDCInsertQuery(OrgContactItem bizO)
		{
			return string.Format(CultureInfo.InvariantCulture, GetGenericQuery, "4", bizO.PK, bizO.Contact.PK, OrgContactItemTypes.Codes.Email, "test@test.com", bizO.OI_Description);
		}
