...
internal abstract partial class ButtonBaseAdapter
{
	internal partial class LayoutOptions
	{
		...
		static readonly int s_combineImageText = BitVector32.CreateMask(s_combineCheck);

		bool disableWordWrapping;
		...
		public int CheckAndTextSpace { get; set; }
		...
		int FullBorderSize => OnePixExtraBorder ? ++BorderSize : BorderSize;
		bool OnePixExtraBorder => GrowBorderBy1PxWhenDefault && IsDefault;
		...
		int FullCheckSize => CheckSize + CheckPaddingSize;
		...
		Size Compose(Size checkSize, Size imageSize, Size textSize)
		{
			// Copied From Winforms
		...
		Size Decompose(Size checkSize, Size imageSize, Size proposedSize)
		{
			// Copied From Winforms
			Composition hComposition = GetHorizontalComposition();
			Composition vComposition = GetVerticalComposition();
		...
		Composition GetHorizontalComposition()
		{
			// Copied From Winforms
		...
		Composition GetVerticalComposition()
		{
			// Copied From Winforms
			BitVector32 action = new BitVector32();
		...
		void CalcCheckmarkRectangle(LayoutData layout)
		{
			...
			switch (align)
			{
				...
				case ContentAlignment.BottomLeft:
					...
					layout.CheckArea.Width = checkSizeFull + CheckAndTextSpace + 1;

					layout.CheckArea.Y = field.Y;
					layout.CheckArea.Height = field.Height;

					layout.Field.X += checkSizeFull + CheckAndTextSpace + 1;
					layout.Field.Width -= checkSizeFull + CheckAndTextSpace + 1;
					break;
		...
		static readonly TextImageRelation[] _imageAlignToRelation = new TextImageRelation[]
		{
		...
		static TextImageRelation ImageAlignToRelation(ContentAlignment alignment)
			=> _imageAlignToRelation[LayoutUtils.ContentAlignmentToIndex(alignment)];

		static TextImageRelation TextAlignToRelation(ContentAlignment alignment)
