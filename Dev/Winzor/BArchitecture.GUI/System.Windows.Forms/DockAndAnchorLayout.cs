...
internal class DefaultLayout : LayoutEngine
{
	...
	static readonly int s_cachedBoundsProperty = PropertyStore.CreateKey();
	...
	/// </summary>
	static void LayoutAutoSizedControls(IArrangedElement container)
	{
	...
	/// </summary>
	static Rectangle GetGrowthBounds(IArrangedElement element, Size newSize)
	{
	...
	/// </summary>
	static GrowthDirection GetGrowthDirection(IArrangedElement element)
	{
		AnchorStyles anchor = GetAnchor(element);
	...
	/// </summary>
	static Rectangle GetAnchorDestination(IArrangedElement element, Rectangle displayRect, bool measureOnly)
	{
	...
	static void LayoutAnchoredControls(IArrangedElement container)
	{
	...
	static Size LayoutDockedControls(IArrangedElement container, bool measureOnly)
	{
		...
		for (int i = children.Count - 1; i >= 0; i--)
		{
			...
			if (CommonProperties.GetNeedsDockLayout(element))
			{
				...
				switch (GetDock(element))
				{
					...
					{
						// *********************************************************************
						// The Winforms source code includes support for MDIClients.
						// Removed here to avoid compiling in additional unneeded dependencies
						// *********************************************************************
	...
	/// </summary>
	static void TryCalculatePreferredSizeDockedControl(IArrangedElement element, Rectangle newElementBounds, bool measureOnly, ref Size preferredSize, ref Rectangle remainingBounds)
	{
		if (measureOnly)
		{
	...
	static Size GetVerticalDockedSize(IArrangedElement element, Size remainingSize, bool measureOnly)
	{
	...
	static Size GetHorizontalDockedSize(IArrangedElement element, Size remainingSize, bool measureOnly)
	{
	...
	static Size xGetDockedSize(IArrangedElement element, Size remainingSize, Size constraints, bool measureOnly)
	{
		Size desiredSize;
	...
	/// </remarks>
	static bool TryCalculatePreferredSize(IArrangedElement container, bool measureOnly, out Size preferredSize)
	{
		var children = container.Children;
	...
	/// </summary>
	static void UpdateAnchorInfo(IArrangedElement element)
	{
		...
		{
			Rectangle bounds = GetCachedBounds(element);
			AnchorInfo oldAnchorInfo = new AnchorInfo { Left = anchorInfo.Left, Top = anchorInfo.Top, Right = anchorInfo.Right, Bottom = anchorInfo.Bottom };
	...
	static Rectangle GetCachedBounds(IArrangedElement element)
	{
		if (element.Container != null)
		{
	...
	static bool HasCachedBounds(IArrangedElement container)
	{
		return container != null && container.Properties.GetObject(s_cachedBoundsProperty) != null;
	}

	static void ApplyCachedBounds(IArrangedElement container)
	{
	...
	static void ClearCachedBounds(IArrangedElement container)
	{
		container.Properties.SetObject(s_cachedBoundsProperty, null);
	}

	static void SetCachedBounds(IArrangedElement element, Rectangle bounds)
	{
	...
	static AnchorInfo GetAnchorInfo(IArrangedElement element)
	{
		return (AnchorInfo)element.Properties.GetObject(s_layoutInfoProperty);
	}

	static void SetAnchorInfo(IArrangedElement element, AnchorInfo value)
	{
	...
	static Size GetAnchorPreferredSize(IArrangedElement container)
	{
