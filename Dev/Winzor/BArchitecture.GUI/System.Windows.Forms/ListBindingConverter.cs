...
// Disabling nullable checks for this file as it is a direct port of the source code.
#nullable disable
...
{
	static Type[] ctorTypes;  // the list of type of our ctor parameters.
	static string[] ctorParamProps; // the name of each property to check to see if we need to init with a ctor.

	/// <summary>
	/// Cache to store ConstructorInfo objects for Binding constructors, keyed by a string representation of parameter types.
	/// </summary>
	static readonly Lazy<Dictionary<string, ConstructorInfo>> _ctorInfoCache = new Lazy<Dictionary<string, ConstructorInfo>>(() =>
		new Dictionary<string, ConstructorInfo>());
	/// <summary>
	/// Represents the default constructor for Binding that takes three parameters: propertyName (string), dataSource (object), and dataMember (string).
	/// This is used as a fallback when a specific constructor cannot be found.
	/// </summary>
	static readonly Lazy<ConstructorInfo> _defaultCtorInfo = new Lazy<ConstructorInfo>(() =>
		typeof(Binding).GetConstructor(new Type[] { typeof(string), typeof(object), typeof(string) }));

	/// <summary>
	///  Creates our array of types on demand.
	/// </summary>
	static Type[] ConstructorParamaterTypes
	{
		get
		{
			if (ctorTypes == null)
	...
	/// </summary>
	static string[] ConstructorParameterProperties
	{
		get
		{
	...
	/// </summary>
	InstanceDescriptor GetInstanceDescriptorFromValues(Binding b)
	{
		...
		// Get the ctor info.
		//
		ConstructorInfo ctor = GetBindingConstructorInfo(ctorParams, ref isComplete);
	...
	/// <summary>
	/// Retrieves the ConstructorInfo for a Binding constructor with the specified parameter types.
	/// If the ConstructorInfo is not in the cache, it's retrieved using reflection and then cached for future use.
	/// </summary>
	ConstructorInfo GetBindingConstructorInfo(Type[] ctorParams, ref bool isComplete)
	{
		string cacheKey = string.Join(",", ctorParams.Select(p => p.FullName));
		if (!_ctorInfoCache.Value.TryGetValue(cacheKey, out ConstructorInfo ctorInfo))
		{
			ctorInfo = typeof(Binding).GetConstructor(ctorParams);
			if (ctorInfo == null)
			{
				Debug.Fail("Failed to find Binding ctor for types!");
				isComplete = false;
				return _defaultCtorInfo.Value;
			}
			else
			{
				_ctorInfoCache.Value[cacheKey] = ctorInfo;
			}
		}
		return ctorInfo;
	}
