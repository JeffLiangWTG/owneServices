...
namespace System.Windows.Forms
{
    ...
    public class CurrencyManager : BindingManagerBase
    {
        ...
        {
			using var activity = TelemetryService.ActivitySource.StartActivity($"{GetType().Name}.Set{nameof(CancelCurrentEdit)}");
			if (Count > 0)
            {
        ...
        {
			using var activity = TelemetryService.ActivitySource.StartActivity($"{GetType().Name}.Set{nameof(EndCurrentEdit)}");
			if (Count > 0)
            {
