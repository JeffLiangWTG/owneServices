...
{
	class OAuth2AuthorizationTest : OAuth2AuthorizationMockOpenIDIdentityServerTestCase
	{
		...
		{
			var httpActionContext = AuthenticationTestHelper.Authenticate(null, CancellationToken.None, () => new EDIClientOAuth2AuthorizationAttribute(eAdaptorNextApplicationDescriptor.ApplicationCode));
		...
		public void TestHttp()
		{
			...
			var result = AuthenticationTestHelper.Authenticate(authorization, CancellationToken.None, () => new EDIClientOAuth2AuthorizationAttribute(eAdaptorNextApplicationDescriptor.ApplicationCode), "http://www.uhoh.com");
		...
		public void TestInvalidAccessToken()
		{
			...
			SetRegistryItems(new[] { authorityUrl }, new[] { authServer.ClientIdentifier });
		...
		public void TestValidAccessToken()
		{
			...
			authServer.Claims = new List<Claim> { new Claim("user", "CWService") };
			var authorityUrl = $"https://{MockIdentityServerBase.HostName}:{authServer.Port}";
			SetRegistryItems(new[] { authorityUrl.ToUpper() }, new[] { authServer.ClientIdentifier, "SomeOtherClientID", "AnotherClientID" });
		...
		public void TestInvalidClientID()
		{
			...
			authServer.Claims = new List<Claim> { new Claim("user", "CWService") };
			var authorityUrl = $"https://{MockIdentityServerBase.HostName}:{authServer.Port}";
			SetRegistryItems(new[] { authorityUrl }, new[] { "CWService" });
			...
			SetRegistryItems(new[] { authorityUrl }, new[] { authServer.ClientIdentifier.ToLower() });
		...
		[TestRequiresAdministrativePrivileges("Adding Trusted Self Signed Certificate")]
		public void TestNoClientIDsInRegistryItem()
		{
			authServer.ClientIdentifier = "UnregisteredClientID";
			authServer.Claims = new List<Claim> { new Claim("user", "CWService") };
			var authorityUrl = $"https://{MockIdentityServerBase.HostName}:{authServer.Port}";
			SetRegistryItems(new[] { authorityUrl }, Array.Empty<string>());
			AssertEquals("Preconditions: No client ids in registry", 0, eAdaptorRegistry.Instance.eAdaptorInboundOAuthClientIDs.GetValueWithoutFallback(Guid.Empty, Guid.Empty, Guid.Empty).Length);
		...
		public void TestInvalidAuthorityURL()
		{
			...
			authServer.Claims = new List<Claim> { new Claim("user", "CWService") };
			var authorityUrl = $"https://{MockIdentityServerBase.HostName}:{authServer.Port}";
			SetRegistryItems(new[] { "https://www.noplace.com" }, new[] { "CWService" });
			Task.Run(() =>
			{
				using (Db.DisposableActionForDbConnection())
				{
		...
		void AssertAuthServerReturnsBadResponse(HttpStatusCode code)
		{
			using (var authServer = new SimpleMockServer(code))
			{
				...
				eAdaptorRegistry.Instance.eAdaptorInboundOAuthAuthorityUrls.SetValue(Guid.Empty, Guid.Empty, Guid.Empty, new[] { authorityUrl });
		...
		public void TestOAuthAttributeOneAdaptorNextController()
		{
			AssertEquals(2, Attribute.GetCustomAttributes(typeof(eAdaptorNextController)).Length);
			AssertEquals(true, Attribute.IsDefined(typeof(eAdaptorNextController), typeof(EDIClientAuthenticationDeciderAttribute)));
		}
