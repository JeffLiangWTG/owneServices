...
{
	[TestedType(typeof(ProfileMemoryModel))]
	sealed class ProfileMemoryModelTest : ProfileModelTest
	{
		protected override string RunnerName => "dotMemory";
		...
		public void TestStart_PrerequisitesInstalledAndSnapshotsFolderSelected_StartProfiling()
		{
			...
			dialogServiceMock.Verify(d => d.ProfileMemory(It.Is<ProfileMemoryModel>(m => m.UserSnapshotsPath == "C:\\McLaren")), Times.Once);
		...
		protected override string PrerequisitesUrl => "https://myaccount-portal.cargowise.com/my-account/public/downloads/JetBrains.dotMemory.Console.windows-x64.2022.2.2.zip";
		...
		protected override string PrerequisitesDestinationPath => Path.Combine(Path.Combine(Path.GetTempPath(), BrandingFactory.Instance.CompanyName), "JetBrains.dotMemory.Console.windows-x64.2022.2.2");

		class ModelForTest : ProfileMemoryModel
		{
			public ModelForTest(IDialogService dialogService, string prerequisiteCheckSum) : base(dialogService)
			{
