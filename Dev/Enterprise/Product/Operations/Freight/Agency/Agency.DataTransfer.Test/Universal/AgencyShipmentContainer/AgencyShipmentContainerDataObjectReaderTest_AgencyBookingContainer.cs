...
{
	sealed class AgencyShipmentContainerDataObjectReaderTest_AgencyBookingContainer : UniversalShipmentDataObjectReaderTest
	{
		protected override string CreateFieldMap(BusinessObject businessObject)
		{
			var container = (AgencyBookingContainer)businessObject;
			var mapper = new AgencyBookingFieldMapper(container.Booking);
			return mapper.Map();
		}
		...
		protected override UniversalShipment GetDataObject()
		{
			...
			booking.JS_CFSReference = "booking ref";
			factory.Save();
		...
		protected override string GetExpectedShipmentMap()
		{
			...
			{
				return retriever.GetString("Enterprise.Freight.Agency.DataTransfer.Test.Universal.AgencyShipmentContainer.TestFiles.AgencyBookingContainer_FieldMap.txt");
			}
		}
