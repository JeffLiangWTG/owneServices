...
namespace Enterprise.Freight.Agency.Documents.DocDataObjects
{
	public static class AddressBuilder
	{
		...
		public static Address Create(IContext context, IAddress address, bool includeContactDetail = true)
		{
			...
			result.RegistrationNumbers = address.RegistrationNumbers.Select(x => new RegistrationNumber()
			{
		...
		{
			if (jobDocAddress == null || jobDocAddress.IsEmpty)
			{
				return CreateEmptyAddress(context);
			}
		...
		public static Address Create(IContext context, ZAddressWithContact zAddressWithContact, bool includeContactInfo = true)
		{
			...
			if (includeContactInfo && zAddressWithContact.OrgContact != null)
			{
		...
		{
			var result = header?.CustomsCodes.OfType<OrgCusCode>()
				.Select(code => new MasterFiles.Business.Macros.RegistrationNumber(code)).ToArray();
		...
		static Address CreateAddressForCurrentUser(IContext context)
		{
			...
			if (GlbBranch.CurrentBranch?.OrgProxy?.MainAddress != null && !GlbBranch.CurrentBranch.OrgProxy.MainAddress.Address1.IsEmpty)
			{
