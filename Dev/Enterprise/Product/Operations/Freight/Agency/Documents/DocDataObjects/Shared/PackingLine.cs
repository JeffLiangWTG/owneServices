...
{
	public class PackingLine : DocDataObject, IPackingLine
	{
		...
		public ZString GroupPOFNumber
		{
			...
			set
			{
				var formattedValue = string.Join(", ", value.ToString().Split(new string[] { ", " }, StringSplitOptions.RemoveEmptyEntries).Where(x => !string.IsNullOrEmpty(x.Trim())).Distinct(StringComparer.OrdinalIgnoreCase));

				if (SetNonPersistentPropertyValue(GroupPOFNumberInfo, ref groupPOFNumber, formattedValue))
				{
