namespace Enterprise.Freight.Agency.Module
{
	...
	internal sealed class ReleaseOrderWithdrawActionMethod : ReleaseOrderActionMethod
	{
		public ReleaseOrderWithdrawActionMethod()
			: base(new ZGuid("a6a0b96e-9619-11e4-9ef6-902b34dc814a")) { }

		public override string Name
		{
			get { return Res.GetString("a6a0b96e-9619-11e4-9ef6-902b34dc814a", "Withdraw Import Release Order Message"); }
		}
		...
		public override string Description
		{
			get
			{
				return Res.GetString(
					"9d3c50f4-9619-11e4-a88b-902b34dc814a",
					@"Sends a withdraw release order message for all containers attached to the bills on which the action is run.
...
Skip - if an error is encountered that only affects a limited number of bills, then the affected bills are skipped allowing the action to continue with the remaining bills. This is the recommended behavior if sending release order original messages is the only thing the action does."
		...
		{
			return new WithdrawNZReleaseOrderActionMethodApplicator((ReleaseImportOrderSettings)settings);
		}
	}
}
