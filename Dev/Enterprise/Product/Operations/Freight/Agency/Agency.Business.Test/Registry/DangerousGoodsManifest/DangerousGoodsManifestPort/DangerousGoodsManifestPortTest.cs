...
{
	[TestedType(typeof(DangerousGoodsManifestPort))]
	internal sealed class DangerousGoodsManifestPortTest : RegistryBusinessObjectTemplateTestCase<DangerousGoodsManifestPort>
	{
		...
		public void TestDefaultValue()
		{
			...
			{
				var port = new DangerousGoodsManifestPort();
		...
		DangerousGoodsManifestPortCollection Collection
		{
			get
			{
				return collection ?? (collection = new DangerousGoodsManifestPortCollection());
			}
		}
		DangerousGoodsManifestPortCollection collection;
		DangerousGoodsManifestPort Port1
		{
			get
			{
				if (port1 == null)
				{
					...
					port1.CurrentFallbackLevel = new FallbackLevel(CompanyAU.PK.ToGuid(), Guid.Empty, Guid.Empty);
				}
				return port1;
			}
		}
		...
		DangerousGoodsManifestPort port1;
		DangerousGoodsManifestPort Port2
		{
			get
			{
				if (port2 == null)
				{
					...
					port2.CurrentFallbackLevel = new FallbackLevel(companyAU.PK.ToGuid(), Guid.Empty, Guid.Empty);
				}
				return port2;
			}
		}
		...
		{
			get { return companyAU ?? (companyAU = GetTestCompany("AUSYD")); }
		}
		GlbCompany companyAU;
		...
		protected override DangerousGoodsManifestPort GetBusinessObjectToClone()
		{
			return GetNewPopulatedBusinessObject();
		}

		protected override DangerousGoodsManifestPort GetBusinessObjectToSerialise()
		{
			return GetNewPopulatedBusinessObject();
		}

		protected override void CheckAllPropertiesAreEqual(DangerousGoodsManifestPort originalBusinessObject, DangerousGoodsManifestPort newBusinessObject, bool isClone)
		{
		...
		DangerousGoodsManifestPort GetNewPopulatedBusinessObject()
		{
			DangerousGoodsManifestPort port = new DangerousGoodsManifestPort();
			port.Port = "AUMEL";
