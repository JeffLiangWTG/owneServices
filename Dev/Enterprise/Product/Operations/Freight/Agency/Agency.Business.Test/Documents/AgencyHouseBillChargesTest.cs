...
public class AgencyHouseBillChargesTest : TestCaseWithFactory
{
	...
	public void TestGetChargesAtOrigin_ShipmentWithoutOrigin_NoExceptionThrown()
	{
		...
		{
			var houseBillCharges = new AgencyHouseBillCharges(shipment);
	...
	public void TestGetChargesAtOrigin_NoCompanyWorkInOrigin_ReturnEmpty()
	{
		...
		Assert("because no company in Ukraine", actualCharges.Length == 0);
		Assert(true);
	}
	...
	public void TestGetChargesAtOrigin_CommunityRegionCompanyHasChargesAtOrigin_ReturnChargesFromCommunityRegionCompany()
	{
		...
		using (FreightDataRegistry.Instance.CommunityRegionsForDirectionCalculation.SetTemporaryValue(nzCompany.PK.ToGuid(), Guid.Empty, Guid.Empty, new[] { au.PK.ToGuid() }))
		{
			var houseBillCharges = new AgencyHouseBillCharges(shipment);
			var actualCharges = houseBillCharges.GetChargesAtOrigin();
			Assert("because neither australian company nor nz one has job header", actualCharges.Length == 0);

			// NZ company has created job header with charges for the shipment
			var newFactory = new BusinessObjectFactory();
			var shipmentInANewFactory = newFactory.Load<AgencyShipment>(shipment.PK);
			...
			houseBillCharges = new AgencyHouseBillCharges(shipmentInANewFactory);
			actualCharges = houseBillCharges.GetChargesAtOrigin();
			Assert("because NZ company has charges for the shipment", actualCharges.Length == 1);
			AssertEquals("because NZ company has charges for the shipment", actualCharges.FirstOrDefault().JR_OSSellAmt, 10m);
		}

		Assert(true);
	}

	public void TestGetChargesAtOrigin_MultipleCompaniesWorkInOrigin()
	{
		...
		TestCase(() =>
		{
			...
			using (Env.SetTemporaryUserContext(Env.CurrentUserPK, auBranch1.PK.ToGuid(), Env.CurrentDepartmentPK))
			{
				...
				Assert("because none of the companies meet the criteria of an Export Company", actualCharges.Length == 0);
			}
		});
		...
		TestCase(() =>
		{
			...
			using (Env.SetTemporaryUserContext(Env.CurrentUserPK, auBranch1.PK.ToGuid(), Env.CurrentDepartmentPK))
			{
				...
				{
					AssertNotNull(actualCharges.FirstOrDefault(c => c.SellAmount == 120));
					AssertNotNull(actualCharges.FirstOrDefault(c => c.SellAmount == 50));
				});
			}
		});
		...
		TestCase(() =>
		{
			AssertEquals(RelatedPartyDirectionList.Codes.Pickup, consignorIFTrelatedParty.PR_FreightDirection);
			var shipment = GetNewShipment();
		...
		TestCase(() =>
		{
			...
			using (Env.SetTemporaryUserContext(Env.CurrentUserPK, auBranch1.PK.ToGuid(), Env.CurrentDepartmentPK))
			{
				...
				{
					AssertNotNull(actualCharges.FirstOrDefault(c => c.SellAmount == 100));
					AssertNotNull(actualCharges.FirstOrDefault(c => c.SellAmount == 50));
				});
			}
		});
		...
		TestCase(() =>
		{
			...
			using (Env.SetTemporaryUserContext(Env.CurrentUserPK, auBranch1.PK.ToGuid(), Env.CurrentDepartmentPK))
			{
				...
				{
					AssertNotNull(actualCharges.FirstOrDefault(c => c.SellAmount == 310));
					AssertNotNull(actualCharges.FirstOrDefault(c => c.SellAmount == 330));
				});
			}
		});
		...
		TestCase(() =>
		{
			...
			SetupJobHeaderInBranch(auBranch3, shipment, header =>
			{
				...
				Assert(header.Branch.GB_RL_NKHomePort != shipment.JS_RL_NKOrigin);
	...
	Shipping Booking V00001000 : Autorating cannot be run because there are errors on this job. Please correct these errors before Autorating.";

		AssertStartsWith("WhenAutoratingForDestination Message Should Start With", expectedMessage, messageSeen);
	...
	public void TestGetCharges_Performance()
	{
		...
		using (Env.SetTemporaryUserContext(Env.CurrentUserPK, auBranch.PK.ToGuid(), Env.CurrentDepartmentPK))
		{
			...
			CombineAssertions(() =>
			{
				AssertRunJobChargesTimes(jobTimes, 90d, $"Called AgencyHouseBillCharges.JobHeaderAtOrigin for {shipments.Count} shipments");
				AssertRunJobChargesTimes(chargeTimes, 60d, $"Called AgencyHouseBillCharges.GetCharges for {shipments.Count} shipments");
			});
		}
	}
	...
	public void TestGetCharges_DBHits()
	{
		...
		{
			var newFactory = new BusinessObjectFactory();
			var shipmentInANewFactory = newFactory.Load<AgencyShipment>(shipments[0].PK);
			var houseBillCharges = new AgencyHouseBillCharges(shipmentInANewFactory);
			newFactory.ResetDatabaseLoadCount();
			var job = houseBillCharges.JobHeaderAtOrigin;
			AssertNotNull(job);
			var charges = houseBillCharges.GetCharges();
			Assert("Count should be greater than 0", charges.Length > 0);
	...
	List<AgencyShipment> SetupPerformanceTestData(int numberOfShipments = 1)
	{
		var (auCompany1, auBranch1) = CreateGlobalCompanyAndBranch("AU", "AU Branch 1");
		var (auCompany2, auBranch2) = CreateGlobalCompanyAndBranch("AU", "AU Branch 2");
		var (auCompany3, auBranch3) = CreateGlobalCompanyAndBranch("AU", "AU Branch 3");
...
Note: This is DEVELOPER ONLY test and won't fail on DAT.";
	...
	(TimeSpan[] JobTimes, TimeSpan[] ChargeTimes) RunJobCharges(List<AgencyShipment> shipments)
	{
		...
		foreach (var shipment in shipments)
		{
			...
			var actualCharges = GetCharges(shipment.PK);
			Assert("Count should be greater than 0", actualCharges.Length > 0);
			stopwatch.Stop();
	...
	void SetupJobHeaderInBranch(GlbBranch branch, AgencyShipment shipment, Action<Job> configureJobHeader)
	{
	...
	{
		var newFactory = new BusinessObjectFactory();
		var shipmentInANewFactory = newFactory.Load<AgencyShipment>(shipmentPK);
		var houseBillCharges = new AgencyHouseBillCharges(shipmentInANewFactory);
