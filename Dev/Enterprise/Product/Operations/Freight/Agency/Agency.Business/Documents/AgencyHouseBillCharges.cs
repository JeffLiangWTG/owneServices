...
public class AgencyHouseBillCharges
{
	public AgencyHouseBillCharges(AgencyShipment shipment)
	{
	...
	bool IsIFTToRelatedParty(OrgHeader debtor, OrgHeader orgHeader)
	{
		...
		var invoiceFrtToOrgs = new[]
		{
			orgHeader?.GetRelatedParty(RelatedPartyTypeList.Codes.InvoiceFreightJobsTo, RelatedPartyDirectionList.Codes.Pickup),
			orgHeader?.GetRelatedParty(RelatedPartyTypeList.Codes.InvoiceFreightJobsTo, RelatedPartyDirectionList.Codes.Delivery)
		}.WhereNotNull();

		isRelated = invoiceFrtToOrgs.Any(invoiceFrtToOrg => debtor.PK == invoiceFrtToOrg.PK);
	...
	static ZQuery GetCountryCompaniesQuery(AgencyShipment shipment, bool isOrigin)
	{
	...
	internal JobCharge[] AutoRateOverseasJob(GlbCompany overseasAgentCompany) =>
		shipment.Factory.GetCachedValue(DestinationChargesCacheKey(shipment.PK), () =>
		{
	...
	public static string DestinationChargesCacheKey(ZGuid pk) => $"DestinationCharges_{pk}";

	void ShowAutoratingLog(GlbCompany overseasAgentCompany, JobCharge[] charges, string ratingLog)
	{
		var title = ResString.GetMultilingualString("3cf7367f-bfbd-4941-a5f7-3a9a8ff30e52", "Autorating Log");
		var message = ResString.GetMultilingualString("b8656323-5acd-4afb-b2cd-819493ff3ce1",
