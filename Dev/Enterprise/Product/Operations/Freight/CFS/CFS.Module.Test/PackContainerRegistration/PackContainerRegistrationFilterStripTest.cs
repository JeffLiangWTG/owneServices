...
{
	sealed class PackContainerRegistrationFilterStripTest : TestCaseWithFactory
	{
		...
		public void TestMilestoneCompletedFilter()
		{
			...
			var shipment1 = GetNewCFSContainer("CONT0000001");
			var shipment2 = GetNewCFSContainer("CONT0000002");
			...
			var filterBO = new PackContainerRegistrationFilterBusinessObject();
			var filter = (ModuleTextFilter)filterBO.ModuleFilters["Milestone Completed"];

			filter.Property = "Completed";
			filter.IsActive = true;
			Asserter.AssertMatches("Should contains container with completed workflow", filter, shipment1);

			filter.Property = "Not Completed";
			filter.IsActive = true;
			Asserter.AssertMatches("Should contains container with incomplete workflow", filter, shipment2);
		}
		...
		public void TestMilestoneLastCompletedFilter()
		{
			...
			filter.IsActive = true;
			Asserter.AssertMatches("Should contains container within date range", filter, shipment1);
		}
		...
		CFSContainer GetNewCFSContainer(string containerNumber)
		{
			CFSContainer shipment = Factory.NewWithValidTestData<CFSContainer>();
			shipment.JC_ContainerNum = containerNumber;
			Factory.Save();
			return shipment;
		}

		void CreateMilestone(CFSContainer parent, ZString status, ZDateTimeOffset scheduledDate, ZDateTimeOffset actualDate, ZString triggerEventCode)
		{
		...
		FilterStripAsserter<CFSContainer> Asserter
		{
			get { return asserter ?? (asserter = new FilterStripAsserter<CFSContainer>(Factory, x => x.JC_ContainerNum)); }
		}
		FilterStripAsserter<CFSContainer> asserter;
