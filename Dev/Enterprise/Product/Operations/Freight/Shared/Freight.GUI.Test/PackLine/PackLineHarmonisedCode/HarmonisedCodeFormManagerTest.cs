...
{
	sealed class HarmonisedCodeFormManagerTest : TestCaseWithFactory
	{
		public void TestDoubleBindingShouldNotAddDuplicateColumnStyles()
		{
			...
			new HarmonisedCodeFormManager(grid).Initialize();

			using (var form = new ZForm())
			{
				form.Controls.Add(grid);
				grid.ReOrderColumns(grid.Columns.Select(column => column.ColumnName).ToArray());
				new HarmonisedCodeFormManager(grid).Initialize(); //double initialization from a rare UI scenario
