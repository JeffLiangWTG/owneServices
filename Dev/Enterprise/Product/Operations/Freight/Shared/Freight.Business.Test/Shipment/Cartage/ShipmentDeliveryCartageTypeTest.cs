...
{
	sealed class ShipmentDeliveryCartageTypeTest : TestCaseWithFactory
	{
		public void TestDeliveryCompleted_Loose()
		{
			...
			ShipmentDeliveryCartageType ct = new ShipmentDeliveryCartageType(shp);
			AssertEquals(0, shp.DeliveryConfirms.Count);

			ct.DeliveryCompleted(DocAddressType.LocalCartageImporter, now);
			Factory.Save();
			AssertEquals(now, shp.DocsAndCartage.JP_DeliveryCartageCompleted);
			AssertEquals(1, shp.DeliveryConfirms.Count);
			AssertEquals(30, shp.DeliveryConfirms[0].TotalBookedPackages);
			AssertEquals(30, shp.DeliveryConfirms[0].TotalDeliveredPackages);
			AssertEquals(now, shp.DeliveryConfirms[0].EU_PickupDeliveryTime);
		}

		public void TestDeliveryCompleted_Containerised()
		{
			...
			ShipmentDeliveryCartageType ct = new ShipmentDeliveryCartageType(shp);
			AssertEquals(0, shp.DeliveryConfirms.Count);
			AssertEquals(ZDateTime.Empty, cnt1.DestinationConfirm.EU_PickupDeliveryTime);
			AssertEquals(ZDateTime.Empty, cnt2.DestinationConfirm.EU_PickupDeliveryTime);

			ct.DeliveryCompleted(cnt1, DocAddressType.LocalCartageImporter, now);
			AssertEquals(0, shp.DeliveryConfirms.Count);
			AssertEquals(now, cnt1.DestinationConfirm.EU_PickupDeliveryTime);
			AssertEquals(ZDateTime.Empty, cnt2.DestinationConfirm.EU_PickupDeliveryTime);

			ct.DeliveryCompleted(cnt2, DocAddressType.LocalCartageImporter, now.AddDays(1));
			AssertEquals(0, shp.DeliveryConfirms.Count);
			AssertEquals(now, cnt1.DestinationConfirm.EU_PickupDeliveryTime);
			AssertEquals(now.AddDays(1), cnt2.DestinationConfirm.EU_PickupDeliveryTime);
		}
		...
		/// </summary>
		public void TestDeliveryCompleted_AssemblyMasterShipment()
		{
			#region setup
			...
			AssertEquals("Precondition - No Delivery Date on ASM Master", ZDateTime.Empty, asmShipment.DocsAndCartage.JP_DeliveryCartageCompleted);
			AssertEquals("Precondition - No Delivery Date on Sub Shipment 1", ZDateTime.Empty, subShipment1.DocsAndCartage.JP_DeliveryCartageCompleted);
			AssertEquals("Precondition - No Delivery Date on Sub Shipment 2", ZDateTime.Empty, subShipment2.DocsAndCartage.JP_DeliveryCartageCompleted);
			AssertEquals("Precondition - No DCF event on ASM Master", false, HasDCFEvent(asmShipment));
			AssertEquals("Precondition - No DCF event on Sub Shipment 1", false, HasDCFEvent(subShipment1));
			AssertEquals("Precondition - No DCF event on Sub Shipment 2", false, HasDCFEvent(subShipment2));

			ZDateTime now = ZDateTime.Now;
			var cartageJob = new ShipmentDeliveryCartageType(asmShipment);
			cartageJob.DeliveryCompleted(DocAddressType.LocalCartageImporter, new ZDateTime(2012, 01, 01)); // Force multiple calls to make sure latest is taken
			cartageJob.DeliveryCompleted(DocAddressType.LocalCartageImporter, now);
			Factory.Save();

			AssertEquals("Delivery Date on ASM Master", now, asmShipment.DocsAndCartage.JP_DeliveryCartageCompleted);
			AssertEquals("DCF event on ASM Master", true, HasDCFEvent(asmShipment));
			AssertEquals("No Delivery Date on Sub Shipment 1", ZDateTime.Empty, subShipment1.DocsAndCartage.JP_DeliveryCartageCompleted);
			AssertEquals("No Delivery Date on Sub Shipment 2", ZDateTime.Empty, subShipment2.DocsAndCartage.JP_DeliveryCartageCompleted);
			AssertEquals("No DCF event on Sub Shipment 1", false, HasDCFEvent(subShipment1));
			AssertEquals("No DCF event on Sub Shipment 2", false, HasDCFEvent(subShipment2));
		}

		bool HasDCFEvent(CommonShipment shipment)
		{
			return shipment.Logs.HasLogWith(StmALogSchema.SL_SE_NKEvent, Events.DeliveryCartageCompleteFinalised.Code);
		}
		...
		public void TestEstimatedCartagePickup()
		{
			...
			ZDateTime now = ZDateTime.Now;
			ShipmentDeliveryCartageType ct = new ShipmentDeliveryCartageType(shp);
			shp.DocsAndCartage.JP_EstimatedDelivery = now;
		...
		public void TestEstimatedCartageDelivery()
		{
			...
			ZDateTime now = ZDateTime.Now;
			ShipmentDeliveryCartageType ct = new ShipmentDeliveryCartageType(shp);
			shp.DocsAndCartage.JP_DeliveryRequiredBy = now;
		...
		public void TestSetTotalDemurrage()
		{
			...
			var ct = new ShipmentDeliveryCartageType(shp);
			AssertEquals(true, shp.DocsAndCartage.JP_DeliveryTruckWaitTime.IsEmpty);

			ct.SetTotalDemurrage(new TimeSpan(2, 3, 4));
			AssertEquals((ZDateTime)new TimeSpan(2, 3, 4), shp.DocsAndCartage.JP_DeliveryTruckWaitTime);
		}
		...
		public void TestCartageJobType()
		{
			...
			ShipmentDeliveryCartageType ct = new ShipmentDeliveryCartageType(shp);
			AssertEquals(Constants.CartageJobType.NEW_LCLImport, ct.CartageJobType);

			shp.JS_RL_NKOrigin = "AUSYD";
			shp.JS_RL_NKDestination = "AUBNE";

			ct = new ShipmentDeliveryCartageType(shp);
			AssertEquals(Constants.CartageJobType.NEW_DomesticLooseDelivery, ct.CartageJobType);
			...
			AssertEquals(Constants.CartageJobType.NEW_FCLCTOtoCNE, ct.CartageJobType);

			consol.JK_OA_ContainerYardEmptyReturnAddress = Factory.NewWithValidTestData<OrgAddress>().PK;
			AssertEquals(Constants.CartageJobType.NEW_FCLImportToCNE, ct.CartageJobType);

			consol.JK_OA_UnpackDepotAddress = Factory.NewWithValidTestData<OrgAddress>().PK;
			AssertEquals(Constants.CartageJobType.NEW_FCLImportUnpack, ct.CartageJobType);

			consol.JK_OA_ContainerYardEmptyReturnAddress = ZGuid.Empty;
			AssertEquals(Constants.CartageJobType.NEW_FCLCTOtoCFS, ct.CartageJobType);
		}

		public void TestDropMode()
		{
			CommonShipment shp = GetNewShipment();
			shp.DocsAndCartage.JP_FCLDeliveryEquipmentNeeded = Constants.FCLEquipmentNeeded.SideLoader;
			ShipmentDeliveryCartageType ct = new ShipmentDeliveryCartageType(shp);
		...
		{
			CommonShipment shp = GetNewShipment();
			var cartageType = new ShipmentDeliveryCartageType(shp);
		...
		public void TestShipmentParentWithoutSailingPopulatesScheduleFromMostInterestingTransportSailing()
		{
			...
			var cartageType = new ShipmentDeliveryCartageType(shipment);
