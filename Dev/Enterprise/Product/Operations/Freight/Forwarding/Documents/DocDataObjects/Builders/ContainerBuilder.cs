...
namespace Enterprise.Freight.Forwarding.Documents.DocDataObjects
{
	...
	{
		public Container Build<T>(CommonContainer containerBO, IContext context, IReadOnlyCollection<T> packlineBOs = null) where T : PackLine
		{
			if (containerBO == null)
			{
				return null;
		...
		void PopulateMeasures(Container container, CommonContainer containerBO, IContext context)
		{
			...
			{
				Value = containerBO.JC_Calc_OverhangLength,
				Unit = new CodeDescription(overhangUnits)
				{
					Code = Constants.Length.Feet
				}
			...
			{
				Value = containerBO.JC_Calc_OverhangWidth,
				Unit = new CodeDescription(overhangUnits)
				{
					Code = Constants.Length.Feet
				}
		...
		void PopulateCollections(Container container, CommonContainer containerBO, IContext context)
		{
			...
			container.CustomerLoadReference = string.Join(", ", containerBO.AdditionalReferenceNumbers.Cast<Customs.Common.CusEntryNumber>()
				.Where(x => x.CE_EntryType == Enterprise.Registry.Business.CustomsReferenceNumberType.CustomsAdditionalReferenceNumbersCodes.CustomerLoadReference).Select(x => x.CE_EntryNum).Distinct());
		}
	}
}
