...
namespace Enterprise.Freight.Forwarding.Documents.DocDataObjects.CN
{
	...
	{
		public BookingContainer(object identifier) : base(identifier)
		{
		}
		...
		public ZInt PackCount => PackingLines.Sum(p => p.Quantity);

		public ICodeDescription PackType => packType ?? (packType = new CodeDescription(new RefPackTypeCollection(new BusinessObjectFactory()))
		{
		...
		{
			get => milestone;
			set => milestone = SetChildCollection(milestone, value);
		}

		IReadOnlyCollection<IMilestone> milestone;
