...
{
	sealed class DGNDangerousGood : DocDataObject, IDangerousGood
	{
		public DGNDangerousGood(object identifier)
			: base(identifier)
		{
		}
		...
		#region RegulationStandard

		public ZString RegulationStandard
		{
			get => regulationStandard;
			set
			{
				if (SetNonPersistentPropertyValue(RegulationStandardInfo, ref regulationStandard, value))
				{
				}
			}
		}

		ZString regulationStandard;

		public ZPropertyInfo RegulationStandardInfo => GetZPropertyInfo(nameof(RegulationStandard));
		...
		public ZBool PackedInLimitedQuantity
		{
			...
			set
			{
				...
				{
					Validate(PackedInExceptedQuantityInfo);
		...
		#region RequiredTemperatureMaximum
		...
		#region RadioactiveMaximumActivity
		...
		#region MedicalFirstAidGuide
		public ZString MedicalFirstAidGuide
		{
			get => medicalFirstAidGuide;
			set
			{
				if (SetNonPersistentPropertyValue(MedicalFirstAidGuideInfo, ref medicalFirstAidGuide, value))
				{
					Validate(MedicalFirstAidGuideInfo);
				}
			}
		}

		ZString medicalFirstAidGuide;

		public ZPropertyInfo MedicalFirstAidGuideInfo => GetZPropertyInfo(nameof(MedicalFirstAidGuide));
		#endregion
		...
		public IMeasurement NetExplosiveWeight
		{
			get => netExplosiveWeight;
			set => netExplosiveWeight = SetChild(netExplosiveWeight, value);
		}

		IMeasurement netExplosiveWeight;
		#endregion

		#region Radioactivity
		public IMeasurement Radioactivity
		{
			get => radioactivity;
			set => radioactivity = SetChild(radioactivity, value);
		}

		IMeasurement radioactivity;
		#endregion

		#region PackedInExceptedQuantity
		public ZBool PackedInExceptedQuantity
		{
			get => packedInExceptedQuantity;
			set
			{
				if (SetNonPersistentPropertyValue(PackedInExceptedQuantityInfo, ref packedInExceptedQuantity, value))
				{
					Validate(PackedInExceptedQuantityInfo);
					Validate(PackedInLimitedQuantityInfo);
				}
		...
		ZBool packedInExceptedQuantity;

		public ZPropertyInfo PackedInExceptedQuantityInfo => GetZPropertyInfo(nameof(PackedInExceptedQuantity));

		#endregion

		#region RadioactiveCriticalitySafetyIndex
		public ZDecimal RadioactiveCriticalitySafetyIndex
		{
			get => radioactiveCriticalitySafetyIndex;
			set
			{
				if (SetNonPersistentPropertyValue(RadioactiveCriticalitySafetyIndexInfo, ref radioactiveCriticalitySafetyIndex, value))
				{
					Validate(RadioactiveCriticalitySafetyIndexInfo);
				}
			}
		}

		ZDecimal radioactiveCriticalitySafetyIndex;

		public ZPropertyInfo RadioactiveCriticalitySafetyIndexInfo => GetZPropertyInfo(nameof(RadioactiveCriticalitySafetyIndex));
		#endregion
		...
		#region ContainerDGNIndex
		public ZInt ContainerDGNIndex
		{
			get => containerDGNIndex;
			set
			{
				if (SetNonPersistentPropertyValue(ContainerDGNIndexInfo, ref containerDGNIndex, value))
				{
					Validate(ContainerDGNIndexInfo);
				}
			}
		}

		ZInt containerDGNIndex;

		public ZPropertyInfo ContainerDGNIndexInfo => GetZPropertyInfo(nameof(ContainerDGNIndex));

		#endregion ContainerDGNIndex
