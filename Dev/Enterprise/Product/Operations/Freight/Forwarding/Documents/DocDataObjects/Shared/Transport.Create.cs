	...
	public sealed partial class Transport
	{
		public static Transport Create(IContext context, Freight.Business.Transport transport)
		{
			...
			return new Transport(transport.PK)
			{
				...
				DepartureReference = transport.Sailing?.Origin?.JA_DepartReference ?? ZString.Empty,
				ArrivalReference = transport.Sailing?.Destination?.JB_ArrivalReference ?? ZString.Empty,
				...
				DepartureFrom = AddressBuilder.Create(context, transport.DepartureLocation),
				ArrivalAt = AddressBuilder.Create(context, transport.ArrivalLocation),
			};
		}
		...
		public static Transport Create(IContext context, TransportLeg transportLeg)
		{
			...
			return new Transport()
			{
				...
				VGMCutOff = transportLeg.VGMCutOff.GetValueOrDefault(),
				DocumentCutOff = transportLeg.DocumentCutOff.GetValueOrDefault(),
				FCLCutOff = transportLeg.FCLCutOff.GetValueOrDefault(),
				DepartureReference = transportLeg.DepartureReference.GetValueOrDefault(),
				ArrivalReference = transportLeg.ArrivalReference.GetValueOrDefault(),
				...
				DepartureFrom = AddressBuilder.Create(context, transportLeg.DepartureFrom),
				ArrivalAt = AddressBuilder.Create(context, transportLeg.ArrivalAt)
			};
		}
