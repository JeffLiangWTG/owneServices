...
{
	public sealed class ChargesCollection : IChargesCollection
	{
		public ChargesCollection(ForwardingShipment shipment, HouseBillLookups lookups, bool isOriginal, bool isHBL = true)
		{
		...
		IMoney CalculateLumpSum()
		{
			...
			{
				currency.Code = Constants.CurrencyCodes.UnitedStates;
		...
		{
			return shipment?.JobHeader?.CurrencyConverter
		...
		ChargeLine[] CreateCharges()
		{
			...
			var revenueCharges = GetRevenueChargesFromCache();
		...
		JobCharge[] GetRevenueChargesFromCache() => shipment.Factory.GetCachedValue(shipment.GetChargesCacheKey(), () => houseBillCharges.GetCharges(isHBL));
