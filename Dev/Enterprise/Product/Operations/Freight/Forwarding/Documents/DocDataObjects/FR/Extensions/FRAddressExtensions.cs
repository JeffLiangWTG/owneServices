...
{
	static class FRAddressExtensions
	{
		#region Ci5

		public static RegistrationNumber GetCi5Number(this OrgAddress address, IContext context)
		{
			return new RegistrationNumber()
			{
				Value = address.GetRegistrationNumberWithFallbackToOrgHeader(OrgCusCode.FranceCodeTypes.CI5),
		...
		public static RegistrationNumber GetSONNumber(this OrgAddress address, IContext context)
		{
			return new RegistrationNumber()
			{
				Value = address.GetRegistrationNumberWithFallbackToOrgHeader(OrgCusCode.FranceCodeTypes.SON),
				Type = new CodeDescription(new OrgCodeLists().CustomsCodes_List(Core.Constants.CountryCodes.France))
				{
		...
		public static RegistrationNumber GetSiretNumber(this OrgAddress address, IContext context)
		{
			return new RegistrationNumber()
			{
				Value = address.GetRegistrationNumberWithFallbackToOrgHeader(OrgCusCode.FranceCodeTypes.Siret),
				Type = new CodeDescription(new OrgCodeLists().CustomsCodes_List(Core.Constants.CountryCodes.France))
				{
		...
		public static RegistrationNumber GetRegistrationNumberFromRegistry(this OrgAddress address, IContext context, string type)
		{
			return address.GetRegistrationNumberFromRegistry(context, type, true, null);
		}

		public static RegistrationNumber GetRegistrationNumberFromRegistry(this OrgAddress address, IContext context, string type, bool isForwarderCode, IUnloco operationalPort)
		{
			var registrationNumber = CreateRegistrationNumber(context, type);
			registrationNumber.Value = GetOrgCusCode(type, isForwarderCode, operationalPort);
