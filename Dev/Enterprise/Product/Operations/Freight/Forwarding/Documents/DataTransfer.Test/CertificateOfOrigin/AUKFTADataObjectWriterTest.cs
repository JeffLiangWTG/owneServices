...
{
	sealed class AUKFTADataObjectWriterTest : CertificateOfOriginDataObjectWriterBaseTest<AUKFTA, AUKFTALineItem, OriginCriterionListAUKFTA>
	{
		protected override string ShipmentDocumentName => ShipmentDocumentNames.AUKFTACertificateOfOrigin;

		protected override AUKFTA CreateCertificate(ZString sourceType, ZString sourceId)
		{
			return new AUKFTA(sourceType, sourceId);
		}

		protected override AUKFTALineItem CreateLineItem(object lineItemId)
		{
			return new AUKFTALineItem(lineItemId);
		}

		protected override void PopulateCertificate(AUKFTA aukfta)
		{
			aukfta.DeclarationCompletedBy = new DeclarationCompletionFlag() { };
		}

		protected override Documents.DataTransfer.CertificateOfOrigin.Base.CertificateOfOriginDataObjectWriter<AUKFTA, AUKFTALineItem> CreateWriter(IDataWritingManager writeManager, IDocument document)
		{
			return new AUKFTADataObjectWriter(writeManager, document);
		}

		public void Test_Correct_DeclarationCompletedByAddInfoIsPresent()
		{
			var aukfta = CreateCertificateCore();
			var writer = new AUKFTADataObjectWriter(new DataWritingManager(DefaultDataObjectWriterStrategy.Instance), null);
			...
			using (DocumentsDataRegistry.Instance.EnableSubmissionToCustomsAuthority.SetTemporaryValue(Guid.Empty, Guid.Empty, Guid.Empty, false))
			{
				...
				var dataObject = writer.GetDataObject(aukfta);
				AssertUXml(dataObject, CreateExpectedXML(nameof(aukfta.DeclarationCompletedBy.IsExporter)));

				aukfta.DeclarationCompletedBy.IsProducer = true;
				dataObject = writer.GetDataObject(aukfta);
				AssertUXml(dataObject, CreateExpectedXML(nameof(aukfta.DeclarationCompletedBy.IsProducer)));

				aukfta.DeclarationCompletedBy.IsAuthorizedOnBehalfExporter = true;
				dataObject = writer.GetDataObject(aukfta);
				AssertUXml(dataObject, CreateExpectedXML(nameof(aukfta.DeclarationCompletedBy.IsAuthorizedOnBehalfExporter)));

				aukfta.DeclarationCompletedBy.IsAuthorizedOnBehalfProducer = true;
				dataObject = writer.GetDataObject(aukfta);
				AssertUXml(dataObject, CreateExpectedXML(nameof(aukfta.DeclarationCompletedBy.IsAuthorizedOnBehalfProducer)));
			}
		}

		protected override string FirstLineItemOriginCode => Core.Constants.CountryCodes.Australia;
		protected override string FirstLineItemOrigin => nameof(Core.Constants.CountryCodes.Australia);
		protected override string SecondLineItemOriginCode => Core.Constants.CountryCodes.UnitedKingdom;
		protected override string SecondLineItemOrigin => nameof(Core.Constants.CountryCodes.UnitedKingdom);
...
";
		...
		protected override string ExpectedPackingLineXml => CreateExpectedXML();

		string CreateExpectedXML(string declarationCompletedBy = "")
		{
			var declarationCompletedByAddInfo = !string.IsNullOrEmpty(declarationCompletedBy) ? $@"
      <AddInfo>
        <Key>DeclarationCompletedBy</Key>
        <Value>{declarationCompletedBy}</Value>
      </AddInfo>"
				: string.Empty;
  ...
  <Shipment>
    ...
    <AddInfoCollection>
      ...
      <AddInfo>
        <Key>IndemnityTermsVersion</Key>
        <Value>2</Value>
      </AddInfo>{declarationCompletedByAddInfo}
    </AddInfoCollection>
    <AttachedDocumentCollection>
      <AttachedDocument>
        <FileName>INV0001.pdf</FileName>
