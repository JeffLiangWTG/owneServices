...
{
	sealed class KAFTADataObjectWriterTest : CertificateOfOriginDataObjectWriterBaseTest<KAFTA, KAFTALineItem, OriginCriterionListKAFTA>
	{
		protected override string ShipmentDocumentName => ShipmentDocumentNames.KAFTACertificateOfOrigin;

		protected override KAFTA CreateCertificate(ZString sourceType, ZString sourceId)
		{
			return new KAFTA(sourceType, sourceId);
		}

		protected override KAFTALineItem CreateLineItem(object packlineId)
		{
			return new KAFTALineItem(packlineId);
		}

		protected override DataTransfer.CertificateOfOrigin.Base.CertificateOfOriginDataObjectWriter<KAFTA, KAFTALineItem> CreateWriter(IDataWritingManager writeManager, DocumentVisualizer.Core.IDocument document)
		{
			return new KAFTADataObjectWriter(writeManager, document);
		}
		...
		protected override string SecondLineItemOriginCode => Core.Constants.CountryCodes.KoreaSouth;
		protected override string SecondLineItemOrigin => nameof(Core.Constants.CountryCodes.KoreaSouth);
