...
{
	sealed class IAECTADataObjectWriterTest : CertificateOfOriginDataObjectWriterBaseTest<IAECTA, IAECTALineItem, OriginCriterionListIAECTA>
	{
		protected override IAECTA CreateCertificate(ZString sourceType, ZString sourceId)
		{
			return new IAECTA(sourceType, sourceId);
		}

		protected override IAECTALineItem CreateLineItem(object lineItemId) => new IAECTALineItem(lineItemId);

		protected override Documents.DataTransfer.CertificateOfOrigin.Base.CertificateOfOriginDataObjectWriter<IAECTA, IAECTALineItem> CreateWriter(IDataWritingManager writeManager, IDocument document)
		{
			return new IAECTADataObjectWriter(writeManager, document);
		}

		protected override string ShipmentDocumentName => ShipmentDocumentNames.IAECTACertificateOfOrigin;
		...
		protected override string SecondLineItemOriginCode => Core.Constants.CountryCodes.India;
		protected override string SecondLineItemOrigin => nameof(Core.Constants.CountryCodes.India);

		protected override void PopulateCertificate(IAECTA certificate)
		{
			certificate.ExportDocumentNumber = "XYZ";
		}

		public void Test_ExportDocumentNumberAddInfoIsNotPresent_When_ExportDocumentNumberIsNull()
		{
			var iaecta = CreateCertificateCore();
			var writer = new IAECTADataObjectWriter(new DataWritingManager(DefaultDataObjectWriterStrategy.Instance), null);
			...
			{
				iaecta.ExportDocumentNumber = null;
				var dataObjectWithoutExportDocumentNumber = writer.GetDataObject(iaecta);
				AssertUXml(dataObjectWithoutExportDocumentNumber, CreateExpectedPackingLineXml(false));
			}
		}
...
";
		...
		string CreateExpectedPackingLineXml(bool isAddExportDocumentNumber = true)
		{
			var exportDocumentNumberAddInfo = isAddExportDocumentNumber ? @"
      <AddInfo>
        <Key>ExportDocumentNumber</Key>
        <Value>XYZ</Value>
      </AddInfo>"
  ...
  <Shipment>
    ...
    <AddInfoCollection>
      ...
      <AddInfo>
        <Key>IndemnityTermsVersion</Key>
        <Value>2</Value>
      </AddInfo>{exportDocumentNumberAddInfo}
    </AddInfoCollection>
    <AttachedDocumentCollection>
      <AttachedDocument>
        <FileName>INV0001.pdf</FileName>
