...
namespace Enterprise.Freight.Forwarding.Documents.DataTransfer.Testing.CertificateOfOrigin
{
	sealed class AUCONPDataObjectWriterTest : CertificateOfOriginDataObjectWriterBaseTest<AUCONP, AUCONPLineItem, OriginCriterionListEmpty>
	{
		protected override string ShipmentDocumentName => ShipmentDocumentNames.AUCONPCertificateOfOrigin;

		protected override AUCONP CreateCertificate(ZString sourceType, ZString sourceId)
		{
			return new AUCONP(sourceType, sourceId);
		}

		protected override AUCONPLineItem CreateLineItem(object lineItemId) => new AUCONPLineItem(lineItemId);

		protected override DataTransfer.CertificateOfOrigin.Base.CertificateOfOriginDataObjectWriter<AUCONP, AUCONPLineItem> CreateWriter(IDataWritingManager writeManager, IDocument document)
		{
			return new AUCONPDataObjectWriter(writeManager, document);
		}
		...
		protected override void PopulateCertificate(AUCONP certificate)
		{
			certificate.ExporterReference = "EXPREF";
		}

		public void Test_ExporterReferenceAddInfoIsNotPresent_When_ExporterReferenceIsNull()
		{
			var auconp = CreateCertificateCore();
			var writer = new AUCONPDataObjectWriter(new DataWritingManager(DefaultDataObjectWriterStrategy.Instance), null);
			...
			{
				auconp.ExporterReference = null;
				var dataObjectWithoutThirdPartyInvoice = writer.GetDataObject(auconp);
				AssertUXml(dataObjectWithoutThirdPartyInvoice, CreateExpectedPackingLineXml(false));
			}
		}
...
";
		...
		string CreateExpectedPackingLineXml(bool isAddExporterReference = true)
		{
			var exporterReferenceAddInfo = isAddExporterReference ? @"
      <AddInfo>
        <Key>ExporterReference</Key>
        <Value>EXPREF</Value>
      </AddInfo>"
  ...
  <Shipment>
    ...
    <AddInfoCollection>
      ...
      <AddInfo>
        <Key>IndemnityTermsVersion</Key>
        <Value>2</Value>
      </AddInfo>{exporterReferenceAddInfo}
    </AddInfoCollection>
    <AttachedDocumentCollection>
      <AttachedDocument>
        <FileName>INV0001.pdf</FileName>
