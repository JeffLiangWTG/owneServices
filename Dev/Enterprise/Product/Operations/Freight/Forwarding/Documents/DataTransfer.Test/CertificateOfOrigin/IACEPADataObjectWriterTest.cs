...
{
	sealed class IACEPADataObjectWriterTest : CertificateOfOriginDataObjectWriterBaseTest<IACEPA, IACEPALineItem, OriginCriterionListIACEPA>
	{
		protected override string ShipmentDocumentName => ShipmentDocumentNames.IACEPACertificateOfOrigin;

		#region TestPopulateDataObject

		protected override IACEPA CreateCertificate(ZString sourceType, ZString sourceId)
		{
			return new IACEPA(sourceType, sourceId);
		}

		protected override IACEPALineItem CreateLineItem(object packlineId)
		{
			return new IACEPALineItem(packlineId);
		}

		protected override void PopulateCertificate(IACEPA iacepa)
		{
			if (iacepa.IsExhibition)
			{
				iacepa.ExhibitionDetail = "EXPO 2024";
			}
			else
			{
				iacepa.ExhibitionDetail = string.Empty;
			}
		}

		public void Test_ExhibitionDetails_Depending_On_IsExhibition()
		{
			var iacepa = CreateCertificateCore();
			var writer = new IACEPADataObjectWriter(new DataWritingManager(DefaultDataObjectWriterStrategy.Instance), null);
			...
			{
				iacepa.IsExhibition = true;
				iacepa.ExhibitionDetail = "EXPO 2024";

				var dataObjectWithExhibitionDetail = writer.GetDataObject(iacepa);
				AssertUXml(dataObjectWithExhibitionDetail, CreateExpectedPackingLineXml(isExhibition: true));

				iacepa.IsExhibition = false;

				var dataObjectWithoutExhibitionDetail = writer.GetDataObject(iacepa);
				AssertUXml(dataObjectWithoutExhibitionDetail, CreateExpectedPackingLineXml());
			}
		}

		protected override Documents.DataTransfer.CertificateOfOrigin.Base.CertificateOfOriginDataObjectWriter<IACEPA, IACEPALineItem> CreateWriter(IDataWritingManager writeManager, IDocument document)
		{
			return new IACEPADataObjectWriter(writeManager, document);
		}
		...
		protected override string SecondLineItemOriginCode => Core.Constants.CountryCodes.Indonesia;
		protected override string SecondLineItemOrigin => nameof(Core.Constants.CountryCodes.Indonesia);
...
";
		protected override string ExpectedPackingLineXml => CreateExpectedPackingLineXml();

		string CreateExpectedPackingLineXml(bool isExhibition = false)
		{
			var exhibitionDetails = string.Empty;

			if (isExhibition)
			{
				exhibitionDetails = @"
      <AddInfo>
        <Key>ExhibitionDetail</Key>
        <Value>EXPO 2024</Value>
      </AddInfo>";
		}
