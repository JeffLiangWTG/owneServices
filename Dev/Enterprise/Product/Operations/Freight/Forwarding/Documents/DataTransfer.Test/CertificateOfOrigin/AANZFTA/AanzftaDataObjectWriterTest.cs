...
{
	sealed class AanzftaDataObjectWriterTest : CertificateOfOriginDataObjectWriterBaseTest<Aanzfta, AanzftaLineItem, OriginCriterionListAanzfta>
	{
		protected override string ShipmentDocumentName => ShipmentDocumentNames.AANZFTACertificateOfOrigin;

		protected override Documents.DataTransfer.CertificateOfOrigin.Base.CertificateOfOriginDataObjectWriter<Aanzfta, AanzftaLineItem> CreateWriter(IDataWritingManager writeManager, IDocument document)
			=> new AanzftaDataObjectWriter(writeManager, document);

		protected override Aanzfta CreateCertificate(ZString sourceType, ZString sourceId)
			=> new Aanzfta(sourceType, sourceId);

		protected override AanzftaLineItem CreateLineItem(object lineItemId) => new AanzftaLineItem(lineItemId);

		protected override void PopulateCertificate(Aanzfta certificate)
		{
			certificate.ArrivalDate = ZDateTime.Empty;
		}

		public void Test_ThirdPartyInvoiceIssuerAddInfoIsPresent_When_IsSubjectToThirdPartyInvoiceIsTrue()
		{
			var aanzfta = CreateCertificateCore();
			var writer = new AanzftaDataObjectWriter(new DataWritingManager(DefaultDataObjectWriterStrategy.Instance), null);
			...
			{
				aanzfta.IsSubjectOfThirdPartyInvoice = true;
				aanzfta.ThirdPartyInvoiceIssuer = "ABC Company";

				var dataObjectWithThirdPartyInvoice = writer.GetDataObject(aanzfta);
				AssertUXml(dataObjectWithThirdPartyInvoice, CreateExpectedPackLineXML(isSubjectToThirdPartyInvoice: true));

				aanzfta.IsSubjectOfThirdPartyInvoice = false;

				var dataObjectWithoutThirdPartyInvoice = writer.GetDataObject(aanzfta);
				AssertUXml(dataObjectWithoutThirdPartyInvoice, CreateExpectedPackLineXML());
			}
		}
		...
		protected override string ExpectedCommercialInvoiceLineXml => CreateExpectedInvoiceLineXML();
		protected override string ExpectedPackingLineXml => CreateExpectedPackLineXML();

		string CreateExpectedPackLineXML(bool isSubjectToThirdPartyInvoice = false)
		{
			var thirdPartyInvoiceIssuerAddInfo = isSubjectToThirdPartyInvoice ? @"
      <AddInfo>
        <Key>ThirdPartyInvoiceIssuer</Key>
  ...
  <Shipment>
    ...
    <AddInfoCollection>
      ...
      <AddInfo>
        ...
        <Value>{isSubjectToThirdPartyInvoice.ToString().ToLower()}</Value>
      </AddInfo>
      <AddInfo>
      ...
      <AddInfo>
        ...
        <Value>2</Value>
      </AddInfo>{thirdPartyInvoiceIssuerAddInfo}
    </AddInfoCollection>
    ...
    <PackingLineCollection>
      ...
      <PackingLine>
		...
		string CreateExpectedInvoiceLineXML(bool isSubjectToThirdPartyInvoice = false)
		{
			var thirdPartyInvoiceIssuerAddInfo = isSubjectToThirdPartyInvoice ? @"
      <AddInfo>
        <Key>ThirdPartyInvoiceIssuer</Key>
