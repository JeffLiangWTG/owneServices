...
{
	sealed class ExportPreAdviceNotificationMessageEventProcessorTest : TestCaseWithFactory
	{
		public void TestOnMessageSent()
		{
			...
			AddMessageSentEvent(consol);

			var messageEventsProcessor = CreateMessageEventsProcessor(consol, containers);
			messageEventsProcessor.OnMessageSent();

			foreach (var containerBizObj in consol.Containers.OfType<ForwardingContainer>())
			{
				var stmALog = containerBizObj.Logs.MostRecentLogByEventTime(Events.MessageSent);
				AssertNotNull(stmALog);
				AssertEquals("|DEP=Terminal|MST=Export Pre-Advice Notification", stmALog.SL_Reference);
				AssertEquals("Container event - MSN", "MSN", stmALog.SL_SE_NKEvent);
			}
		}
		...
		public void TestOnMessageWithdrawalSent()
		{
			...
			AddMessageWithdrawCancelRequestEvent(consol);

			var messageEventsProcessor = CreateMessageEventsProcessor(consol, containers);
			messageEventsProcessor.OnMessageWithdrawalSent();

			foreach (var containerBizObj in consol.Containers.OfType<ForwardingContainer>())
			{
				var stmALog = containerBizObj.Logs.MostRecentLogByEventTime(Events.MessageWithdrawCancelRequest);
				AssertNotNull(stmALog);
				AssertEquals("|DEP=Terminal|LOC=NZAKL|MST=Export Pre-Advice Notification|RES=Reason of withdraw accepted", stmALog.SL_Reference);
				AssertEquals("Container event - MWR", "MWR", stmALog.SL_SE_NKEvent);
			}
		}
		...
		IMessageEventsProcessor CreateMessageEventsProcessor(ForwardingConsol consol, ICollection<ForwardingContainer> containers)
		{
			...
			document.SetupGet(d => d.DataContext).Returns(DataContext.ExportPreAdviceNotification);

			var supporter = new ForwardingConsolVisualizableDocumentSupporter(consol);
			return supporter.GetMessageEventsProcessor(document.Object);
		}
