...
{
	sealed class CresaWithoutUIMessageSenderTransitReceiveConsignmentTest : DocDataObjectWithoutUIMessageSenderTest
	{
		#region TestStmNote
		...
		{
			var bizObj = CresaTestHelper.CreateReceiveConsignment(Factory);
			var notificationsHandler = new NotificationsHandler();
		...
		public void TestSendMessage_InvalidTransitWarehouseJob()
		{
			...
			using (Factory.AddDisposableService())
			{
				...
				CombineAssertions("Cannot send CRESA Message from invalid RCN", () =>
				{
					...
					AssertContains("We cannot send CRESA message from Receive Consignment RC1", errorMessage);
				});
			}
		}
		...
		protected override BusinessObject CreateBusinessObject() => CresaTestHelper.CreateReceiveConsignment(Factory).PopulateRequiredOrgCodes();
  ...
  <Shipment>
    ...
    <BookingConfirmationReference>RC1</BookingConfirmationReference>
    ...
    <AdditionalReferenceCollection>
      <AdditionalReference>
        ...
        <ReferenceNumber>RC1</ReferenceNumber>
      </AdditionalReference>
    </AdditionalReferenceCollection>
...
Message Error - PortServiceCodeReference: Port Service Reference is missing from RCN > Booking Party > Organization > Config > Registration Numbers/Codes - type PSR.
Message Error - PortArea: Port Area is missing from organization RCN > Booking Party > Organization > Config > Registration Numbers/Codes - type PSN (code before '\')
Message Error - PortLocation: Port Location is missing from organization RCN > Booking Party > Organization > Config > Registration Numbers/Codes - type PSN (code after '\')
CRESA Message Details:
Operational Port    PCS            Transport Mode    Transhipment Port    Port of Arrival    Port Area      Port Service Reference    Port Location    Cargo Receipt Date    ETA at Port of Arrival
