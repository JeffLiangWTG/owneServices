...
{
	sealed class ILDLOResetToOriginalMessageCommandTest : ILResetToOriginalMessageCommandBaseTest
	{
...
Before using this option, you should always check with Customs to make sure they have not already processed the message.";
			...
			try
			{
				...
				CombineAssertions("When Reset To Original invoke", () =>
				{
					...
					AssertNullOrEmpty("JS_DLO is reset", shipment.JS_DLO);

					var newFactory = new BusinessObjectFactory();
					var zQuery = new ZQuery(CusEntryNumSchema.CE_ParentID, SQLComparisonOperator.Equal, shipment.PK);
					zQuery.AddToFilter(JoinCondition.And, CusEntryNumSchema.CE_EntryType, SQLComparisonOperator.Equal, "DON");
					zQuery.AddToFilter(JoinCondition.And, CusEntryNumSchema.CE_RN_NKCountryCode, SQLComparisonOperator.Equal, "IL");

					var cusEntryNumberDON = newFactory.LoadTop1<CusEntryNumber>(zQuery);
					AssertNullOrEmpty("DON is reset", cusEntryNumberDON.CE_EntryNum);

					AssertEquals("shipment data logs", "STU Propagated: All Document Data|DEP=CargoWise Support|MST=Delivery Order|TYP=Reset To Original",
					shipmentSTULog);
				});
			}
			finally
		...
		protected override ILCustomCommandBase CreateCommand(ForwardingShipment shipment)
			=> new ILDLOResetToOriginalMessageCommand(shipment?.DeliveryOrderProvider);
