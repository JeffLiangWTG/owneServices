...
{
	public partial class CFSContainerLoadList : CommonContainerLoadList
		, ICFSContainerLoadList
		, IWorkflowProvider
	...
	{
		public CFSContainerLoadList(BusinessObjectFactory factory, DataRow row)
			: base(factory, row)
		{
			if (this.CLH_LoadMode == ZString.Empty)
			{
				this.CLH_LoadMode = Constants.ContainerLoadListHeaderLoadMode.ContainerFreightStation;
			}
		}

		#region IDocManagerSupport

		public DocManagerInfo DocManagerInfo => docManagerInfo ?? (docManagerInfo = new DocManagerInfo(this, Constants.DocManagerCodes.ContainerLoadPlan));
		...
		public DocumentSupporter DocumentSupporter => new CFSContainerLoadListDocumentSupporter(this);
		...
		public IColumnValueRanker GetTemplateSelectionCriteria()
		{
			...
			result.Add(ProcessTaskTemplateSchema.P0_OH_Client, ControllingCustomerAddress.Organisation?.PK, ZGuid.Empty);
			result.Add(ProcessTaskTemplateSchema.P0_SubType1, CLH_PlannedTransportMode, ZString.Empty);
		...
		ZString IWorkflowProviderCore.WorkflowType => WorkflowDescriptors.ContainerLoadPlanWorkflowDescriptorCode;

		[ChildEditable(true)]
		public ContainerLoadPlanProcessTaskCollection WorkflowItems
		{
			get
			{
				if (workflowItems == null)
				{
					workflowItems = this.GetOrCreateProcessTaskCollection(() => new ContainerLoadPlanProcessTaskCollection(this));
		...
		ContainerLoadPlanProcessTaskCollection workflowItems;
		...
		public ZString GetRequestTypeCode() => ExternalRequestTypes.Codes.ContainerLoadPlan;
		...
		public (ZGuid OrginzationPK, ZGuid ContactPK) GetRequestSupportedAddressInfo(ZString addressType) => addressType.ToString() switch
		{
			...
			DocAddressTypes.Codes.SupplierDocumentaryAddress => RequestGenerationProviderHelper.GetRequestSupportedAddressInfo(Factory, string.Format(ScripForGetRequestSupplierAddressInfo, PK.ToString()), OrgAddressSchema.Constants.OA_OH, JobDocAddressSchema.Constants.E2_Contact),
		...
		string ScripForGetRequestSupplierAddressInfo => 
			"""
				...
				INNER JOIN dbo.JobDocAddress ON JSB_PK = E2_ParentID AND E2_AddressOverride = 0 AND E2_AddressSequence = 0 AND E2_OA_Address IS NOT NULL AND E2_AddressType = 'SUD'
		...
		string ScripForGetRequestManufacturerAddressInfo =>
			"""
				...
				FROM dbo.ContainerLoadListLine
				INNER JOIN dbo.JobSupplierBookingLine ON JSL_PK = CLL_JSL_BookingLine
				INNER JOIN dbo.JobDocAddress
				ON JSL_PK = E2_ParentID AND E2_AddressOverride = 0 AND E2_AddressSequence = 0 AND E2_OA_Address IS NOT NULL AND E2_AddressType = 'MAN'
				INNER JOIN dbo.OrgAddress ON OA_PK = E2_OA_Address
				WHERE CLL_CLH_LoadListHeader = '{0}'
			""";
		...
		protected override void FillWithValidTestDataCore(TestBusinessObjectKind kind, System.ComponentModel.PropertyDescriptor[] propertyPath)
		{
			...
			CLH_LoadMode = CommonContainerLoadListLoadModeList.Codes.CFS;
