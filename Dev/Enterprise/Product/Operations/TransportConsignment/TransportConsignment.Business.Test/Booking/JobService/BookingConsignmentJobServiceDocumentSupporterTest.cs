...
{
	[TestedType(typeof(BookingConsignmentJobServiceDocumentSupporter))]
	class BookingConsignmentJobServiceDocumentSupporterTest : DocumentSupporterTest
	{
		public void TestDocumentSupporter()
		{
			AssertEquals("Document Supporter should be of type", typeof(BookingConsignmentJobServiceDocumentSupporter), DocumentSupporter.GetType());
		}

		public void TestService()
		{
			BookingConsignmentJobServiceDocumentSupporter docSupporter = DocumentSupporter;
		...
		{
			AssertEquals("Core.Constants.DataContext.RequestForService is Supported", true, DocumentSupporter.IsDataContextSupported(new DataContextValueForTesting(Core.Constants.DataContext.ConsignmentJobService)));
		...
		{
			AssertEquals(BusinessContext.CnsgmentJobService, DocumentSupporter.BusinessContext);
		}
		...
		List<Tuple<Constants.DataContext, string>> SupportedDataContextAndNotFoundMessageReasonPairs
		{
			get
			{
				...
				dataContextAndMessagePairs.Add(new Tuple<Constants.DataContext, string>(Constants.DataContext.ConsignmentJobService, "Cannot find Booking Consignment Services."));
				dataContextAndMessagePairs.Add(new Tuple<Constants.DataContext, string>(Constants.DataContext.GenericFreightJobServices, "Cannot find Booking Consignment Services."));
				dataContextAndMessagePairs.Add(new Tuple<Constants.DataContext, string>(Constants.DataContext.GenericFreightJob, "Cannot find Booking Consignment."));
		...
		BookingConsignmentJobServiceDocumentSupporter documentSupporter;

		public BookingConsignmentJobServiceDocumentSupporter DocumentSupporter
		{
			get { return documentSupporter ?? (documentSupporter = new BookingConsignmentJobServiceDocumentSupporter(Factory.New<BookingConsignmentJobService>())); }
		}
		#endregion
	}
}
