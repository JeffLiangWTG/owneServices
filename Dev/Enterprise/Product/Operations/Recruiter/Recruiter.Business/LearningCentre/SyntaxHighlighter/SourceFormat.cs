...
namespace CodeFormatter
{
	...
	public abstract class SourceFormat
	{
		...
		string FormatCode(string source, bool lineNumbers, bool alternate, bool subCode)
		{
			if (!subCode)
			{
			...
			source = ((CodeRegex == null)
						? Parser.Evaluate(source)
						: CodeRegex.Replace(source, MatchEval));

			StringBuilder sb = new StringBuilder();
			if (!subCode)
			{
				sb.Append((NoResString)"<pre class=\"codeblock\">"); // Html syntax for code block highlighting.
			}
			...
			if (lineNumbers || alternate) //we have to process the code line by line
			{
				using (StringReader reader = new StringReader(source))
				{
					...
					while ((line = reader.ReadLine()) != null)
					{
						...
						if (alternate && ((i % 2) == 1))
						{
							sb.Append((NoResString)"<pre class=\"alt\">"); // Html syntax for code block highlighting.
						}
						else
						{
							sb.Append((NoResString)"<pre>"); // Html syntax for code block highlighting.
						}

						if (lineNumbers)
						{
							int order = (int)Math.Log10(i);
							sb.Append((NoResString)"<font color=\"#000000\">" // Html syntax for code block highlighting.
								+ spaces.Substring(0, 3 - order) + i
								+ (NoResString)" </font>"); // Html syntax for code block highlighting.
						}

						if (line.Length == 0)
						{
							sb.Append("&nbsp;"); // Html syntax for code block highlighting.
						}
						else
						{
							sb.Append(line);
						...
						sb.Append((NoResString)"</pre>\n"); // Html syntax for code block highlighting.
					}
					reader.Close();
				}
			}
