...
namespace Enterprise.Tracking.Web.Testing
{
	...
	class TrackingOrderColumnProviderTest : GridColumnProviderTest
	{
		...
		protected override DataGridColumn[] GetColumnsForLayoutFixNoDynamicColumns()
		{
			...
			{
				TestProvider[WebTracker.Grids.TrackingOrders.MainVoyage],
				TestProvider[WebTracker.Grids.TrackingOrders.OrderNumber],
				TestProvider[WebTracker.Grids.TrackingOrders.PickupAddress],
				TestProvider[WebTracker.Grids.TrackingOrders.Packs]
			};
		}
		...
		protected override DataGridColumn[] GetColumnsForLayoutFixAllDynamicColumns()
		{
			return new DataGridColumn[]
			{
				...
				TestProvider[WebTracker.Grids.TrackingOrders.Origin],
				TestProvider[WebTracker.Grids.Milestones.LastMilestoneDescription],
				TestProvider[WebTracker.Grids.TrackingOrders.RequiredExWorks],
		...
		protected override DataGridColumn[] GetColumnsForLayoutFixFewDynamicColumns()
		{
			return new DataGridColumn[]
			{
				TestProvider[WebTracker.Grids.TrackingOrders.SendingAgent],
				TestProvider[WebTracker.Grids.TrackingOrders.OrderDate],
				TestProvider[WebTracker.Grids.Milestones.LastMilestoneDate],
				TestProvider[WebTracker.Grids.TrackingOrders.OrderNumber],
				TestProvider[WebTracker.Grids.Milestones.LastMilestoneDescription],
				TestProvider[WebTracker.Grids.TrackingOrders.MasterBill]
			};
		}
		...
		protected override void SetupColumnsCore()
		{
			...
			AddRequiredColumn(new ZHyperLinkColumn("Order #", TrackingOrder.Schema.JD_OrderNumberAndSplit)
			{
			...
			AddDefaultsColumn(new ZTextEditColumn("Split Number", TrackingOrder.Schema.JD_OrderNumberSplit) { ColumnKey = WebTracker.Grids.TrackingOrders.SplitNumber });
			...
			AddDefaultsColumn(new ZTextEditColumn("Supplier", TrackingOrder.Schema.SupplierName) { ColumnKey = WebTracker.Grids.TrackingOrders.Supplier });
			AddDefaultsColumn(new ZTextEditColumn("Buyer", TrackingOrder.Schema.BuyerName) { ColumnKey = WebTracker.Grids.TrackingOrders.Buyer });

			AddDefaultsColumn(new ZTextEditColumn("Controlling Customer", "ControllingCustomerDocAddress+E2_CompanyName") { ColumnKey = WebTracker.Grids.TrackingOrders.ControllingCustomer });
			...
			AddDefaultsColumn(new ZCodeFindBoxColumn("Origin", TrackingOrder.Schema.JD_RL_NKGoodsAvailableAt, "Lookups+GoodsAvailableAts", typeof(TrackingOrder))
			{
				ColumnKey = WebTracker.Grids.TrackingOrders.Origin,
				DisplayStyle = OComboBoxDropDownStyle.DescriptionOnly
			});

			AddDefaultsColumn(new ZCodeFindBoxColumn("Destination", TrackingOrder.Schema.JD_RL_NKGoodsDeliveredTo, "Lookups+GoodsDeliveredTos", typeof(TrackingOrder))
			{
				ColumnKey = WebTracker.Grids.TrackingOrders.Destination,
				DisplayStyle = OComboBoxDropDownStyle.DescriptionOnly
			});
			...
			AddDefaultsColumn(new ZTextEditColumn("Packs", TrackingOrder.Schema.JD_PacksWithUnits) { ColumnKey = WebTracker.Grids.TrackingOrders.Packs });
			AddDefaultsColumn(new ZTextEditColumn("Volume", TrackingOrder.Schema.JD_ActualVolumeWithUnits) { ColumnKey = WebTracker.Grids.TrackingOrders.Volume });
			AddDefaultsColumn(new ZTextEditColumn("Weight", TrackingOrder.Schema.JD_ActualWeightWithUnits) { ColumnKey = WebTracker.Grids.TrackingOrders.Weight });
			...
			if (WebEnv.AppInstance != null)
			{
				...
				if (siteUser != null)
				{
					...
					foreach (CustomLabelInfo field in new TrackingOrder.CustomLabelsProvider(null, true).GetCustomFields(siteUser.LoggedInOrganisation, Factory))
					{
