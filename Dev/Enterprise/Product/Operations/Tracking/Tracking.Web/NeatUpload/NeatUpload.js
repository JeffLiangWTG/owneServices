...
//For File Extension Limit
var dangerousExtensionList = [
"??_","{??","001","002","386","3GR","ACE","ACM","ADE","ADP","ADT","AP?","ARC","ARJ","ASA","ASD","ASP","AX?","B64","BA?","BIN","BO?","BZ?",
"CAB","CC?","CDR","CDX","CEO","CGI","CHM","CLA","CMD","CNV","CO?","COM","CPL","CPT","CPY","CRT","CSC","CSS","D?B","DAT","DEV","DIF","DL?",
"DOT","DQY","DRV","EE?","EFV","EX?","EXE","FDF","FMT","FO?","FPH","FPW","GF?","GIM","GIX","GMS","GNA","GW?","GWI","GZ?","HDI","HHT","HLP",
"HWD","ICE","ICS","IM?","IN?","IQY","ISP","ITS","IZE","JAR","JS?","LGP","LIB","LNK","LWP","LZH","M3U","MB0","MB1","MB2","MBR","MD?","MHT",
"MOD","MPD","MPT","MRC","MSO","NAP","NEW","NWS","OB?","OC?","OL?","OLE","OTM","OV?","PCD","PCI","PF?","PHP","PI?","PIF","PLG","POT","PPZ",
"PRC","PWZ","QLB","QPW","QQY","QTC","QTC","RAR","REG","RMF","RQY","SCR","SCT","SH?","SIS","SKV","SLK","SMM","SPL","SWF","SYS","TAR","TAZ",
"TBZ","ID0","TFT","TGZ","TLB","TSP","UNP","URL","VB?","VBS","VS?","VWP","VXD","WBK","WIZ","WMV","WP?","WRI","WRL","WRZ","WS?","X32","X32",
"WRF", "XTP", "XX?"
];

var FileMatchesDangerousExtension = function (ext)
{
    var extArray = ext.split('');
    var result = false;
    for(var i=0;i<dangerousExtensionList.length;i++)
    {
        var nextExt = dangerousExtensionList[i].split('');
        var Match = nextExt.Length === extArray.Length;
        if (Match)
        {
            for (var j = 0; j < nextExt.length; j++)
            {
                if (nextExt[j] != '?' && nextExt[j] != extArray[j])
                {
                    Match = false;
                }
            }
...
var OnChangeInputFile = function (e)
{
	...
	{
		ext = filename.toUpperCase().split('.').pop();
	}
	if (FileMatchesDangerousExtension(ext))
	{
		document.getElementById('OK').disabled = true;
		alert(document.getElementById('dangerousFileMessageHidden').value);
	}
	var size = this.files[0].size;
	var maximumFileSize = document.getElementById('maximumFileSizeHidden').value;
	if (size == 0)
	{
		document.getElementById('OK').disabled = true;
		alert(document.getElementById('zeroSizeFileMessageHidden').value);        
	}
	else if (size > maximumFileSize)
	{
		document.getElementById('OK').disabled = true;
		alert(document.getElementById('maximumFileSizeMessageHidden').value);
	}
};
