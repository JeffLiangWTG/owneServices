...
{
	public partial class TrackingLinerAndAgencyBooking : AgencyBooking,
	IMilestonesProvider,
	...
	IEventReferenceProvider
	{
		...
		public TrackingLinerAndAgencyBooking(BusinessObjectFactory factory, DataRow row)
			: base(factory, row)
		{
		...
		public static TrackingLinerAndAgencyBooking FromPKBySiteUser(BusinessObjectFactory factory, ZGuid agencyBookingPK, TrackingSiteUser siteUser)
		{
			TrackingLinerAndAgencyBooking result = null;
			if (siteUser != null && siteUser.LoggedInOrganisation != null && !agencyBookingPK.IsEmpty)
			{
				var filter = new ZDBOnlyQuery(typeof(TrackingLinerAndAgencyBooking));
				filter.AddToFilter(JobShipmentSchema.PK, agencyBookingPK);
		...
		public List<string> UpdatableMilestoneEventCodes
		{
			get
			{
				...
				{
					return (new UpdateableMilestoneEventsHelper(SiteUser)).GetUpdateableMilestoneEvents(WebDataRegistry.Instance.LinerAndAgencyBookingMilestoneEventUpdates.Value, WebParties);
				}
		...
		public WebUserEditableNote UserEditableNoteHelper
		{
			get
			{
				if (userEditableNoteHelper == null)
				{
					...
					RegisterEditableChildObject(userEditableNoteHelper);
				}
