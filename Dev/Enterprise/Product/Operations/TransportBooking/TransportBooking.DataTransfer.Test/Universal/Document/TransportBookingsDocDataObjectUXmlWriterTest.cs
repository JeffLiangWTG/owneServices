...
{
	[TestedType(typeof(TransportBookingsDocDataObjectUXmlWriter))]
	class TransportBookingsDocDataObjectUXmlWriterTest : TestCaseWithFactory
	{
		...
		public void TestGetDataObjectOnDocumentWithFFMMessageRequest()
		{
			var writer = new TransportBookingsDocDataObjectUXmlWriter();
			var mockDocumentData = new Mock<IDynamicData>();
			mockDocumentData.SetupGet(dd => dd.Value).Returns(mockFFMMessage);
			var mockDocument = new Mock<IDocument>();
			mockDocument.SetupGet(d => d.Data).Returns(mockDocumentData.Object);
			mockDocument.SetupGet(d => d.DataContext).Returns("FFMMessageRequest");
			var dataObject = writer.GetDataObject(DefaultDataObjectWriterStrategy.Instance, mockDocument.Object);
			var dataObjectAsUniversalShipment = dataObject as UniversalShipment;
			AssertNotNull("dataObject returned should be a non-null UniversalShipment", dataObjectAsUniversalShipment);
			CombineAssertions("Main shipment details invalid", () =>
			{
				AssertEquals("Voyage Number is incorrect.", voyageNumber, dataObjectAsUniversalShipment.VoyageFlightNo);

				var universalShipmentLeaveDate = dataObjectAsUniversalShipment.DateCollection.Find(d => d.Type == UniversalDataBuss.DataObjects.Universal.DateType.Departure && d.Value == leaveDate);
				...
				AssertEquals("Airport Code is incorrect.", expectedPortOfLoading, dataObjectAsUniversalShipment.PortOfLoading?.Code);
				AssertEquals("Port of Destination is incorrect.", expectedPortOfDestination, dataObjectAsUniversalShipment.PortOfDestination?.Code);

				AssertEquals("Regulated Agent ID is incorrect.", regulatedAgentID, dataObjectAsUniversalShipment.CarrierDocumentsOverride.AWBHeader.AgentName);
				AssertEquals("Country of Regulated Agent is incorrect.", approvalCountryCode, dataObjectAsUniversalShipment.CarrierDocumentsOverride.AWBHeader.AgentPlace);
				AssertEquals("Security status code is incorrect.", consolSecurityStatus, dataObjectAsUniversalShipment.CarrierDocumentsOverride.AWBHeader.SpecialHandlingCode);

				var documentID = dataObjectAsUniversalShipment.InstructionCollection?.FirstOrDefault()?
				...
				AssertEquals("There should be exactly 3 packages lines.", 3, dataObjectAsUniversalShipment.SubShipmentCollection.Count);
			});

			var expectedContainer1SubPackage1 = dataObjectAsUniversalShipment.SubShipmentCollection.FirstOrDefault(s => s.ContainerCollection.Any(c => (c.ContainerNumber ?? ZString.Empty) == containerID1));
			...
			var expectedContainer2SubPackage1 = dataObjectAsUniversalShipment.SubShipmentCollection.ToList<UniversalShipment>()
			...
			var expectedContainer2SubPackage2 = dataObjectAsUniversalShipment.SubShipmentCollection.ToList<UniversalShipment>()
				.FindAll(s => s.ContainerCollection.Any(c => (c.ContainerNumber ?? ZString.Empty) == containerID2))
		...
		public void TestGetDataObjectOnDocumentWithOtherDataContext()
		{
			var writer = new TransportBookingsDocDataObjectUXmlWriter();
			var mockDocumentData = new Mock<IDynamicData>();
			mockDocumentData.SetupGet(dd => dd.Value).Returns(mockFFMMessage);
			var mockDocument = new Mock<IDocument>();
			mockDocument.SetupGet(d => d.Data).Returns(mockDocumentData.Object);
			mockDocument.SetupGet(d => d.DataContext).Returns("SomethingOtherThanFFMMessageRequest");
			var dataObject = writer.GetDataObject(DefaultDataObjectWriterStrategy.Instance, mockDocument.Object);
			AssertNull("Should return null for unknown DataContext", dataObject);
		}
	}
}
