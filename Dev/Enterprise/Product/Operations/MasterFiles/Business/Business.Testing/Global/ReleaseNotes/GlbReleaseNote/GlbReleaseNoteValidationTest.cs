...
{
	public class GlbReleaseNoteValidationTest : BusinessObjectValidationTestCase
	{
		public void TestCheckGF_Category()
		{
			...
			Assert("Precondition: Categories[1].Code should not be empty.", !string.IsNullOrEmpty(Note.Lookups.Categories[1].Code));
		...
		public virtual void TestCheckGF_Section()
		{
			Note.GF_Section = "";
			...
			foreach (var section in NewsAnnouncementSectionListRetriever.GetList(NewsAnnouncementSectionListMode.WiseTechOnly).GetAllCodes())
			{
				...
				AssertNoErrors($"GlbReleaseNote.GF_Section should not have an error for: {section}", Note.GF_SectionInfo);
			}
			...
			{
				Note.GF_Section = section;
				AssertHasErrors($"GlbReleaseNote.GF_Section should have an error for: {section}", Note.GF_SectionInfo);
			}
		}
		...
		public void TestCheckGF_RN_NKCountryForReleaseNote()
		{
			...
			Assert("Precondition: Countries[1].Code should not be empty.", !string.IsNullOrEmpty(Note.Lookups.Countries[1].Code));
		...
		protected virtual GlbReleaseNote GetNewNote() => Factory.New<GlbReleaseNote>();

		protected GlbReleaseNote Note { get; set; }
