...
{
	public class SendACASAmendmentMenuItemTest : SendACASMessageMenuItemTest<SendACASAmendmentMenuItem>
	{
		public void TestSendACASAmendmentVisibility_NoAmendmentRequired()
		{
			AssertMenuItemVisibilityGivenMessageStatus("Hide sendACASAmendmentMenuItem if no amendment required", HVLVACASMessageStatusList.Codes.OriginalSent, false);
		}

		public void TestSendACASAmendmentVisibility_AmendmentRequired()
		{
			AssertMenuItemVisibilityGivenMessageStatus("Show sendACASAmendmentMenuItem if amendment required", HVLVACASMessageStatusList.Codes.AmendmentRequired, true);
		}

		public void TestSendACASmendment_FiltersConsignments()
		{
			using (GlbCompany.CurrentCompany.TemporarilySetCountry(CountryCodes.UnitedStates))
			{
				...
				using (var form = new ZForm(shipment))
				{
					...
					sendACASMenuGroup.PerformSelect();
					var sendACASAmendmentMenuItem = sendACASMenuGroup.MenuItems.OfType<SendACASAmendmentMenuItem>().Single();

					sendACASAmendmentMenuItem.PerformClick();
