...
{
	[TestedType(typeof(HVLVItemDensity))]
	public class HVLVItemDensityTest : BaseDensityTest
	{
		public void TestIsRefreshAllowed()
		{
			var item = Factory.NewWithValidTestData<HVLVItem>();
			var density = new HVLVItemDensity(item);
		...
		public void TestIsChargeableByWeight()
		{
			...
			var chargeableByWeightProperty = density.GetType().GetProperty("IsChargeableByWeight", BindingFlags.Instance | BindingFlags.NonPublic);
			AssertEquals("IsChargebleByWeight should be false when item is not allocate to shipment", false, chargeableByWeightProperty.GetValue(density));
			...
			AssertEquals("IsChargeableByWeight should be true when item is allocate to shipment", true, chargeableByWeightProperty.GetValue(density));
		}
		...
		public void TestDensityCalculation_ShouldIncludeUQConversion()
		{
			...
			AssertEquals("Density should be 10 for 100g/0.006m3(Total Weight = 0.1kg, Volume Weight = 1kg)", 10m, item.DensityFactor);

			consignment.HVC_WeightUQ = Weight.Kilograms;
			AssertEquals("Density should be updated to 0.01 when changing weight UQ to kg (100kg/0.006m3)(Total Weight = 100kg, Volume Weight = 1kg)", 0.01m, item.DensityFactor);

			item.HVI_ActualWeight = 0.1;
			AssertEquals("Density should be updated to 10 when changing weight value to 0.1 (0.1kg/0.006m3)(Total Weight = 0.1kg, Volume Weight = 1kg)", 10m, item.DensityFactor);

			shipment.JS_TransportMode = TransportModes.Sea;
			item.HVI_ActualWeight = 1;
			AssertEquals("Density should be 6 for 1kg/0.006m3(Total Volume = 0.006m3, Volume Weight = 0.001m3)", 6m, item.DensityFactor);

			item.HVI_ActualVolume = 6;
			AssertEquals("Density should be updated to 6000 when changing volume value to 6 (1kg/6m3)(Total Volume = 6m3, Volume Weight = 0.001m3)", 6000m, item.DensityFactor);

			consignment.HVC_VolumeUQ = Volume.CubicDecimetres;
			AssertEquals("Density should be updated to 6 when changing volume UQ to dm3 (100kg/6dm3)(Total Volume = 0.006m3, Volume Weight = 0.001m3)", 6m, item.DensityFactor);
		}

		public void TestDensityCalculation_MultipleItemOnConsignment()
		{
			var shipment = Factory.NewWithValidTestData<ForwardingShipment>();
			shipment.JS_TransportMode = TransportModes.Sea;
			shipment.JS_RL_NKOrigin = "AUSYD";
			...
			item1.HVI_ManifestedVolume = 0.006;
			item1.HVI_ActualVolume = 0.006;

			var item2 = consignment.Items.AddNew();
			item2.HVI_ManifestedWeight = 1;
			item2.HVI_ActualWeight = 1;
			item2.HVI_ManifestedVolume = 0.006;
			item2.HVI_ActualVolume = 0.006;

			CombineAssertions("Item density should be calculated correctly when there are multiple items on a single consignment.", () =>
			{
				AssertEquals(6m, item1.DensityFactor);
				AssertEquals(6m, item2.DensityFactor);
			});
		}

		protected override BusinessObject GetNewBusinessObject()
		{
			return new HVLVItemDensity(Factory.New<HVLVItem>());
		}
	}
}
