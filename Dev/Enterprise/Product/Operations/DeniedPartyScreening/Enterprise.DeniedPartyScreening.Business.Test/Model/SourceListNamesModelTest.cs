...
namespace Enterprise.DeniedPartyScreening.Business.Test
{
	public class SourceListNamesModelTest : TestCaseWithFactory
	{
		public void TestConstructor()
		{
			...
			model = new SourceListNamesModel(Factory, new[] { "AAA", "BBB", "CCC" }, true);
			AssertEquals("Included list contains 3 entry", 3, model.IncludedLists.Length);
			AssertContainsExactElementsInAnyOrder(new[] { ("AAA Name", "AAA Description"), ("BBB Name", "BBB Description"), ("CCC Name", "CCC Description") }, model.IncludedLists.Select(u => (u.Name.ToString(), u.Description.ToString())));

			AssertEquals("Excluded list contains 0 entry", 0, model.ExcludedLists.Length);
		}
