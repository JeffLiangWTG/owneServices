...
namespace Enterprise.Warehouse.Web.WebService
{
	partial class WhsSecureService
	{
		...
		[WebMethod(Description = "Suspend Task")]
		[SoapHeader("SecurityHeader", Direction = SoapHeaderDirection.In)]
		public WebServiceResponse SuspendTask(Guid taskPK)
		{
			return HandleWebServiceRequest((WebServiceResponse r) => SuspendTaskCore(r, taskPK));
		}

		void SuspendTaskCore(WebServiceResponse response, Guid taskPK)
		{
			var task = Factory.Load<ProcessTask>(taskPK);
			var staff = WebServiceHelper.GetStaff(Factory, SecurityHeader.UserName);
			var shouldSave = BeginRFTaskHelper.SuspendRFTask(response, task, staff);
			if (shouldSave)
			{
