...
{
	class UpdateWeightAndDimsForHandlingUnitTest : WhsSecureServiceTestCase
	{
		...
		public void TestInvalidArguments_PackageNotFound()
		{
			...
			var response = webService.UpdateWeightAndDimsForHandlingUnit(Guid.NewGuid(), new PackageDimensionsInfo());
		...
		public void TestInvalidArguments_PackTypeNotFound()
		{
			using (WarehouseDataRegistry.Instance.CheckAndroidDeviceVersion.SetTemporaryValue(Guid.Empty, Guid.Empty, Guid.Empty, temporaryValue: false))
			{
				...
				var response = webService.UpdateWeightAndDimsForHandlingUnit(handlingUnitPackage.PK.ToGuid(), null);
				AssertBusinessValidationError(webService, "Package Dimensions were empty.", response);
			}
		}

		#endregion

		#region TestUpdateWeightAndDimsForHandlingUnit

		public void TestUpdateWeightAndDimsForHandlingUnit()
		{
			using (WarehouseDataRegistry.Instance.CheckAndroidDeviceVersion.SetTemporaryValue(Guid.Empty, Guid.Empty, Guid.Empty, temporaryValue: false))
			{
				var data = new TestDataSimpleEnvironment(Helper.Factory, 2, 1);
				...
				handlingUnitPackage.KP_F3_NKPackType = "PLT";
				handlingUnitPackage.KP_WeightUQ = Constants.Weight.Grams;
				handlingUnitPackage.KP_TareWeight = 1m;
				handlingUnitPackage.KP_Weight = 1m;
				handlingUnitPackage.KP_DimensionUQ = Constants.Length.Centimetres;
				handlingUnitPackage.KP_Length = 1m;
				handlingUnitPackage.KP_Width = 1m;
				handlingUnitPackage.KP_Height = 1m;
				...
				var newDimensions = new PackageDimensionsInfo
				{
					PackType = "PAC",
					EmptyWeight = 3m,
					Weight = 10m,
				...
				var webService = GetNewWebService(data.Whs1);
				var response = webService.UpdateWeightAndDimsForHandlingUnit(handlingUnitPackage.PK.ToGuid(), newDimensions);
				AssertSuccessfulResponseWithNoErrors(response, webService);

				AssertEquals("PAC", handlingUnitPackage.KP_F3_NKPackType);
				AssertEquals("KG", handlingUnitPackage.KP_WeightUQ);
				AssertEquals(3m, handlingUnitPackage.KP_TareWeight);
				AssertEquals(10m, handlingUnitPackage.KP_Weight);
				AssertEquals("M", handlingUnitPackage.KP_DimensionUQ);
				AssertEquals(2m, handlingUnitPackage.KP_Height);
				AssertEquals(4m, handlingUnitPackage.KP_Length);
				AssertEquals(8m, handlingUnitPackage.KP_Width);
			}
		}

		public void TestUpdateWeightAndDimsForHandlingUnit_DBHits()
		{
			var data = new TestDataSimpleEnvironment(Factory, 2, 1);
