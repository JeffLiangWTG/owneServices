...
{
	class CompleteTaskTest : WhsSecureServiceTestCase
	{
		public void TestCompleteTask_BadTaskPK()
		{
			...
			var response = webService.CompleteTask(Guid.NewGuid());

			AssertBusinessValidationError(webService, "Task could not be found.", response);
		}

		public void TestCompleteTask_InvalidTaskFormFlowType()
		{
			var data = new TestDataSimpleEnvironment(Helper.Factory, 2, 1);
		...
		public void TestCompleteTask_SuccessfulIfServiceSuccess()
		{
			var data = new TestDataSimpleEnvironment(Helper.Factory, 2, 1);
			var receive = Helper.CreateWhsReceive(data.Org1, data.Whs1, "R1");
			var staff = Helper.CreateGlbStaff("US1", "User");
			...
			taskManagementServiceMock.Setup(t => t.SetTaskToCompletedIfValid(It.IsAny<IProcessTask>(), staff.GS_Code)).Returns(UpdateTaskStatusResult.Success);
			...
			using (ObjectFactory.Substitute(taskManagementServiceMock.Object))
			{
				...
				AssertSuccessfulResponseWithNoErrors(response, webService);
				taskManagementServiceMock.Verify(t => t.SetTaskToCompletedIfValid(It.Is<IProcessTask>(tk => tk.PK == task.PK), staff.GS_Code));
				taskManagementServiceMock.VerifyNoOtherCalls();
			}
		}

		public void TestCompleteTask_AlreadyClosed()
		{
			var data = new TestDataSimpleEnvironment(Helper.Factory, 2, 1);
			var receive = Helper.CreateWhsReceive(data.Org1, data.Whs1, "R1");
			var staff = Helper.CreateGlbStaff("US1", "User");
			...
			var taskManagementServiceMock = new Mock<IWhsTaskManagementService>();
			taskManagementServiceMock.Setup(t => t.SetTaskToCompletedIfValid(It.IsAny<IProcessTask>(), staff.GS_Code)).Returns(UpdateTaskStatusResult.SuccessWithNoChanges);
