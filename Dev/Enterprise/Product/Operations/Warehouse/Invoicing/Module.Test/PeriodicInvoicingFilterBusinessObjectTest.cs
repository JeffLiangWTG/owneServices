...
{
	[TestedType(typeof(PeriodicInvoicingFilterBusinessObject))]
	public abstract class PeriodicInvoicingFilterBusinessObjectTest : FilterStripBusinessObjectTestCase
	{
		#region Filters
		...
		{
			FilterBizObj = GetPeriodicInvoicingFilterBusinessObject();
			Factory.NewWithValidTestData<PeriodicInvoicing>();
		...
		public virtual void TestFilterWarehouseVisibility()
		{
			...
			try
			{
				var invoiceFilter1 = GetPeriodicInvoicingFilterBusinessObject();
				...
				try
				{
					var invoiceFilter2 = GetPeriodicInvoicingFilterBusinessObject();
		...
		{
			var filterBizObj = GetPeriodicInvoicingFilterBusinessObject();
			var invoiceCollection = GetPeriodicInvoicingCollection();
		...
		PeriodicInvoicing CreatePeriodicInvoice(PeriodicInvoicingCollection invoiceCollection, GlbCompany company, OrgHeader client, WhsWarehouse warehouse, ZDateTime fromDate, bool addJobHeader)
		{
		...
		void SetupData()
		{
			var year = ZDateTime.Now.Year;
			FilterBizObj = GetPeriodicInvoicingFilterBusinessObject();
			InvoiceCollection = GetPeriodicInvoicingCollection();
		...
		protected abstract PeriodicInvoicingCollection GetPeriodicInvoicingCollection();

		PeriodicInvoicingFilterBusinessObject GetPeriodicInvoicingFilterBusinessObject()
		{
			return (PeriodicInvoicingFilterBusinessObject)GetNewFilterStripBusinessObject();
		}

		FilterStripAsserter<PeriodicInvoicing> Asserter
		{
			get { return asserter ?? (asserter = new FilterStripAsserter<PeriodicInvoicing>(Factory, i => i.ET_StorageJobNumber)); }
		}
		...
		{
			return GetPeriodicInvoicingFilterBusinessObject();
		}

		protected PeriodicInvoicingTestHelperFunctions Helper
		{
			get { return helper ??= new PeriodicInvoicingTestHelperFunctions(Factory); }
		}

		PeriodicInvoicingFilterBusinessObject FilterBizObj;
		PeriodicInvoicingCollection InvoiceCollection;

		OrgHeader Client1;
		OrgHeader Client2;
		WhsWarehouse Warehouse1;
		WhsWarehouse Warehouse2;
		PeriodicInvoicing Invoice1;
		PeriodicInvoicing Invoice2;
		PeriodicInvoicing Invoice3;
		PeriodicInvoicing Invoice4;

		PeriodicInvoicingTestHelperFunctions helper;
		FilterStripAsserter<PeriodicInvoicing> asserter;
