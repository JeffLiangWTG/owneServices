...
{
	[TestedType(typeof(GteBookingForm))]
	public class GteBookingFormTest : ZFormBasherTest
	{
		public void TestOpenInBrowser()
		{
			using (GlowRegistry.Instance.GlowPortalsUri.SetTemporaryValue(Guid.Empty, Guid.Empty, Guid.Empty, "https://address/"))
			{
				...
				using (var form = new GteBookingForm(booking))
				{
					form.Show();
		...
		static void AssertOpenedOnTheWeb(GteBooking booking, string glowPortalsUri)
		{
			...
			AssertEquals("Expected URL to include the PK of booking so the GLOW edit form for booking is opened", booking.PK.ToString(), entityPK);
		}
		...
		{
			var booking = Factory.New<GteBooking>();
			return new GteBookingForm(booking);
		}
