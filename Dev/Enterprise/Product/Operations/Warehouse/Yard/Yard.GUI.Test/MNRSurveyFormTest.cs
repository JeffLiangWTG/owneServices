...
{
	[TestedType(typeof(MNRSurveyForm))]
	public class MNRSurveyFormTest : ZFormBasherTest
	{
		...
		protected override Form GetFormToBashCore()
		{
			...
			var form = new MNRSurveyForm(survey);
			form.ControllerID = ControllerIDs.MNRSurvey;
		...
		public void TestOpenInBrowser()
		{
			...
			using (var form = new MNRSurveyForm(survey))
			{
				form.Show();
			...
			AssertOpenedOnTheWeb(survey, glowPortalsUri: "address");
		}
		...
		public void TestGlowLinkCaption()
		{
			...
			using (var form = new MNRSurveyForm(survey))
			{
				...
				AssertEquals("Survey can only be accessed via the Container Yard GLOW Portal.", form.GlowLinkLabel.CaptionResourceString.Caption);
			}
		}

		static void AssertOpenedOnTheWeb(MNRSurvey survey, string glowPortalsUri)
		{
			...
			AssertEquals("Expected URI to go to this GLOW alias URL", "/Goto/MNRSurvey", uri.AbsolutePath);
			AssertEquals("Expected URL to include the PK of survey so the GLOW edit form for receive advice is opened", survey.PK.ToString(), entityPK);
		}
		...
		{
			using (var form = (MNRSurveyForm)GetFormToBash())
			{
