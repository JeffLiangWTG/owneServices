...
{
	[TestedType(typeof(WorkOrderLineCollectionDataObjectReader))]
	public class WorkOrderLineCollectionDataObjectReaderTest : DataObjectCollectionReaderTest
	{
		...
		public override void TestReadIntoCollection()
		{
			...
			var workOrderLines = workOrders.First().WorkOrderLines;
			AssertEquals("1 WorkOrderLine should be populated.", 1, workOrderLines.Count);

			var workOrderLine = workOrderLines.First();
			AssertEquals("RCC001", workOrderLine.ComponentCode.RCC_Code);
			AssertEquals("RRC001", workOrderLine.RepairCode.RRC_Code);
			AssertEquals("RFM001", workOrderLine.Damage.RFM_Code);
			AssertEquals("RUS001", workOrderLine.UnitSection.RUS_Code);
			AssertEquals("RMC001", workOrderLine.Material.RMC_Code);
			AssertEquals("CM", workOrderLine.MWL_UnitOfDimension);
			AssertEquals("OWN", workOrderLine.MWL_ResponsibleParty);
			AssertEquals("TEST Description", workOrderLine.MWL_Description);
		}
