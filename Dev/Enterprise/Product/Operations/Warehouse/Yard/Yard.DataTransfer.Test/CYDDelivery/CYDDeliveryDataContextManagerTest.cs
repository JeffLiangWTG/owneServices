...
[TestedType(typeof(CYDDeliveryDataContextManager))]
class CYDDeliveryDataContextManagerTest : DataContextManagerTestCase<CYDDeliveryDataContextManager, CYDDelivery>
{
	public void TestExportBookingConfirmationEventWithDataContextKey()
	{
		var delivery = Factory.NewWithValidTestData<CYDDelivery>();
		delivery.YDL_DeliveryID = "YDL000000000001";

		var logger = new DummyLogger();
		var dataContext = DataContextFactory.New();
		dataContext.AddDataSource(DataContextType.GateMovementBooking, "GBM00000001");
		var linkCreator = new UniversalJobLinkCreator(delivery.Factory, delivery, null, dataContext, logger, true);
		linkCreator.TryCreateJobLink(DataContextType.GateMovementBooking);

		Factory.SaveForTesting();

		var log = delivery.Logs.AddNew(AutoEvents.BookingConfirmed);
  ...
  <Event>
    <DataContext>
      <DataSourceCollection>
        <DataSource>
          <Type>CYDDelivery</Type>
          <Key>YDL000000000001</Key>
        </DataSource>
      </DataSourceCollection>
...
""";
		...
		var manager = (IEventDataContextManager)delivery.GetUniversalDataContextManager();
		var writer = manager.GetEventDataObjectWriter(new DataWritingManager(new ActionInfo(RecipientRoleType.GDM, delivery)));
