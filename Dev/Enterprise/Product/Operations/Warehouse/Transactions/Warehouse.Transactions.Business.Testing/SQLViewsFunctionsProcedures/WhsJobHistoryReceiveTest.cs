...
{
	internal class WhsJobHistoryReceiveTest : WhsTestCaseWithFactory
	{
		...
		public void TestView_WithProductCategoryFilter_NonInventory()
		{
			var categoryBeverages = Helper.CreateProductCategory("BEVERAGE", "All Beverages");
			...
			Helper.CreateWhsOrderWithOrderLine(client, warehouse, "O1", product, 10m);
			Factory.Save();

			var result = LoadView_WithProductCategory(categoryBeverages.PK);
			AssertEquals("ResultSet Count", 0, result.Count);
		}
		...
		DynamicBusinessObjectCollection LoadView_WithProductCategory(ZGuid categoryPK)
		{
			...
			var sql = categoryPK.IsEmpty
				? @"select * from WhsJobHistoryReceiveReport(null)"
				: @"select * from WhsJobHistoryReceiveReport(@ProductCategoryPK)";
		...
		public void TestView()
		{
			...
			var tempInventory = new WhsInventoryViewCollection(Factory);

			tempInventory.AddRange(receives[0].Inventory);
			tempInventory.AddRange(receives[1].Inventory);
			tempInventory.AddRange(receives[2].Inventory);
			tempInventory.AddRange(receives[3].Inventory);
			tempInventory.AddRange(receives[4].Inventory);

			foreach (WhsInventoryView inventoryObj in tempInventory)
			{
				if (inventoryObj.SupplierPart.PK == part.PK && inventoryObj.Warehouse.PK == whs.PK &&
					inventoryObj.Client.PK == client.PK)
				{
			...
			inventory.Sort("WI_InDocketLineUnits", System.ComponentModel.ListSortDirection.Ascending);

			AssertResult(inventory[0], result[0], commodity);
			AssertResult(inventory[1], result[1], commodity);
			AssertResult(inventory[2], result[2], commodity);
		}
		...
		public void TestView_ProductCategory()
		{
			...
			AssertResult_ProductCategory(receive.Inventory[0], result[0]);
			AssertResult_ProductCategory(receive.Inventory[1], result[0]);
		}
		...
		void AssertResult_ProductCategory(WhsInventoryView inventory, DynamicBusinessObject dynamicObject)
		{
			var partRelation =
				inventory.SupplierPart.RelatedOrganisations.FindByOrganisationAndRelationship(inventory.Docket.Client,
			...
			AssertEquals("Receive Category", inventory.Docket.WD_ReceiveCategory, dynamicObject["ReceiveCategory"]);
		}

		void AssertResult(WhsInventoryView inventory, DynamicBusinessObject businessObject,
			RefCommodityCode[] commodity)
		{
			...
			if (receive.IsFinalised)
			{
				...
				AssertEquals("DateTimeOffsetField", receive.WD_FinalisedDate, ((ZDateTimeOffset)businessObject["DateTimeOffsetField"]));
			}
			else
			{
				...
				AssertEquals("DateTimeOffsetField", expectedDateField, ((ZDateTimeOffset)businessObject["DateTimeOffsetField"]));
			}

			AssertEquals("WarehousePK", receive.Warehouse.PK, businessObject["WarehousePK"]);
			AssertEquals("DocketPK", receive.PK, businessObject["DocketPK"]);
			AssertEquals("WarehouseName", receive.Warehouse.WW_WarehouseName, businessObject["WarehouseName"]);
			AssertEquals("ClientPK", receive.Client.PK, businessObject["ClientPK"]);
			AssertEquals("ClientFullName", receive.Client.OH_FullName, businessObject["ClientFullName"]);
			AssertEquals("ClientCode", receive.Client.OH_Code, businessObject["ClientCode"]);
			AssertEquals("Reference", receive.WD_ExternalReference, businessObject["Reference"]);
			AssertEquals("FinalisedDate", receive.WD_FinalisedDate.Date,
				((ZDateTime)businessObject["FinalisedDate"]).Date);
			AssertEquals("BookingDate", receive.WD_BookingDate.Date, ((ZDateTimeOffset)businessObject["BookingDate"]).Date);
			AssertEquals("ArrivalDate", receive.WD_ArrivalDate.Date, ((ZDateTime)businessObject["ArrivalDate"]).Date);
			AssertEquals("DocketStatus", receive.WD_DocketStatus, businessObject["DocketStatus"]);
			AssertEquals("CustomerReference", receive.WD_CustomerReference, businessObject["CustomerReference"]);
			AssertEquals("TotalUnits", receive.WD_TotalUnits, businessObject["TotalUnits"]);
			AssertEquals("ProductPK", inventory.SupplierPart.PK, businessObject["ProductPK"]);
			...
			AssertEquals("PartAttrib1", inventory.WI_PartAttrib1, businessObject["PartAttrib1"]);
			AssertEquals("PartAttrib2", inventory.WI_PartAttrib2, businessObject["PartAttrib2"]);
			AssertEquals("PartAttrib3", inventory.WI_PartAttrib3, businessObject["PartAttrib3"]);
			AssertEquals("SerialNumber", inventory.WI_SerialNumber, businessObject["SerialNumber"]);
			AssertEquals("ExpiryDate", inventory.WI_ExpiryDate, ((ZDateTime)businessObject["ExpiryDate"]).Date);
			AssertEquals("PackingDate", inventory.WI_PackingDate, ((ZDateTime)businessObject["PackingDate"]).Date);
			AssertEquals("ConsigneePK", ZGuid.Empty, businessObject["ConsigneePK"]);
			AssertEquals("ConsigneeName", ZString.Empty, businessObject["ConsigneeName"]);
			AssertEquals("DistributionCentrePK", ZGuid.Empty, businessObject["DistributionCentrePK"]);
			AssertEquals("DistributionCentreCoName", ZString.Empty, businessObject["DistributionCentreCoName"]);
			AssertEquals("Service Level", inventory.Docket.WD_RS_NKServiceLevel, businessObject["ServiceLevel"]);
			AssertEquals("Receive Category", inventory.Docket.WD_ReceiveCategory, businessObject["ReceiveCategory"]);

			if (receive.Notes
					.FindByDescription(PredefinedNoteTypes.Instance.ReceiveConfirmationInstructionsNote.Description)
					.Length > 0)
			{
				AssertEquals("ConfirmationNotes",
					receive.Notes.FindByDescription(PredefinedNoteTypes.Instance.ReceiveConfirmationInstructionsNote
						.Description)[0].ST_NoteDataAsText, businessObject["ConfirmationNotes"]);
			}
			else
			{
				AssertEquals("ConfirmationNotes", "", businessObject["ConfirmationNotes"]);
			}
		}
		...
		public void TestView_SerialNumber()
		{
			...
			var inv1 = CreateReceiveLine("SN1").Inventory[0];
			var inv2 = CreateReceiveLine("SN2").Inventory[0];
			var inv3 = CreateReceiveLine("SN3").Inventory[0];
...
select
	*
from
	WhsJobHistoryReceiveReport(@ProductCategoryPK)
where
...
select
	*
from
	WhsJobHistoryReceiveReport(null)
where
	WarehousePK = @WarehousePK and
	ClientPK = @ClientPK and
	...
	SerialNumber";
		...
		WhsReceive[] SetupData(WhsWarehouse whs, OrgHeader client, OrgSupplierPart part, RefCommodityCode[] commodity)
		{
			...
			var receives = new[]
			{
				...
				SetupReceive(client, whs, "12", part1, 121, part, 122, false, arriveDate, emptyDateOffset, receiveCategory: "RC2"),
				SetupReceive(client, whs, "13", part1, 121, part, 123, false, arriveDate, arriveDate),
				SetupReceive(client2, whs, "21", part2, 211, part2, 212, true, arriveDate, emptyDateOffset, receiveCategory: "RC1"),
				SetupReceive(client2, whs2, "22", part2, 221, part2, 222, false, arriveDate, emptyDateOffset, receiveCategory: "RC2")
			};
		...
		WhsReceive SetupDataForProductCategory(WhsWarehouse whs, OrgHeader client, OrgSupplierPart part)
		{
			...
			var receive = SetupReceive(client, whs, "21", part, 211, part, 212, true, arriveDate, emptyDateOffset, receiveCategory: "RC1");
		...
		WhsReceive SetupReceive(OrgHeader client, WhsWarehouse warehouse, ZString reference,
			OrgSupplierPart warehousePart1, ZDecimal units1,
			OrgSupplierPart warehousePart2, ZDecimal units2, ZBool finalized, ZDateTimeOffset arrivalDate, ZDateTimeOffset etaDate,
			string serviceLevel = "", string receiveCategory = "")
		{
			var receive = Helper.CreateWhsReceive(client, warehouse, reference, Notify);
			...
			receive.Notes.AddNew(false, PredefinedNoteTypes.Instance.ReceiveConfirmationInstructionsNote.Description,
				"Confirmation Note" + reference);
