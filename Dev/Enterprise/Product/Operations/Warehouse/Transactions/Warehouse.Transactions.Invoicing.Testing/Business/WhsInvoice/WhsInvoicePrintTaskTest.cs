...
{
	public class WhsInvoicePrintTaskTest : TestCaseWithFactory
	{
		...
		void TestPrintTaskIncludesAllDocumentsCore(bool setFactory)
		{
			...
			whsInvoice.ET_WW = warehouse.PK;
			whsInvoice.ET_OH_Client = org.PK;
			whsInvoice.AutoRateJobHeader(null);
			whsInvoice.PostInvoice();

			var invoice = Factory.NewWithValidTestData<ARInvoice>();
			invoice.AH_GB = GlbBranch.CurrentBranch.PK;
			invoice.AH_JH = whsInvoice.JobHeader.PK;
			...
			using (var task = new WhsInvoicePrintTaskForTest(new InvoicingBase[] { invoice }, factory))
			{
			...
			DeleteJobHeadersWithoutAnyCharges(whsInvoice);
		}
	...
	public class WhsInvoicePrintTaskForTest : WhsInvoicePrintTask
	{
		public WhsInvoicePrintTaskForTest(InvoicingBase[] invoices, BusinessObjectFactory factory)
