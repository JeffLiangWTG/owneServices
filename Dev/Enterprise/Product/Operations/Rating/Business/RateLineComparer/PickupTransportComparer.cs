...
{
	class PickupTransportComparer : BaseRateLineComparer
	{
		public PickupTransportComparer(RatingCriteria criteria)
		{
		...
		public override int Compare(FastLine line1, FastLine line2)
		{
			...
			{
				var shipmentPickupTransport = commonShipment.DocsAndCartage?.JP_OA_PickupCartageCoAddr_ZAddress?.OrgHeader?.PK ?? ZGuid.Empty;

				if (!shipmentPickupTransport.IsValid)
				{
					return 0;
				}
				...
				if (org1.PK == shipmentPickupTransport &&
				...
				if (org2.PK == shipmentPickupTransport &&
		...
		{
			return (NoResString)"Pickup Transport"; // log message, subject to change, more for support people as of now
		}
	}
}
