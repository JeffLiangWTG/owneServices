...
namespace Enterprise.Rating.Web
{
	...
	{
		[System.Diagnostics.CodeAnalysis.SuppressMessage("CargoWiseOne", "CW1161:ResGetStringAnalyzer", Justification = "Swagger settings")]
		public static void ConfigureSwagger(this HttpConfiguration config)
		{
			...
			config
				.EnableSwagger(c =>
				{
					...
					//
					c.SingleApiVersion("v1", "Rating APIs");
					...
					//
					c.DocumentFilter<IncludeRatingAPIsOnly>();
		...
		class IncludeRatingAPIsOnly : IDocumentFilter
		{
			public void Apply(SwaggerDocument swaggerDoc, SchemaRegistry schemaRegistry, IApiExplorer apiExplorer)
			{
				...
				{
					string assemblyName = item.ActionDescriptor?.ControllerDescriptor?.ControllerType?.Assembly?.FullName?.ToUpperInvariant();

					if (assemblyName != null && !this.GetType().Assembly.FullName.ToUpperInvariant().Equals(assemblyName))
					{
						removals.Add(item);
					}
				}
