...
{
	public class QuickBookingJobFact : CustomFieldJobFact, IQuickBookingJobFact
	{
		public QuickBookingJobFact(IJobInvoicingPlugIn jobPlugin,
			...
			: base(jobPlugin?.InvoicingSupporter?.Job, environmentFact, localClientFact, salesRepFact)
		{
			...
			BookingPK = jobPlugin.PK.IsValid ? jobPlugin.PK.ToGuid() : Guid.Empty;
			...
			SalesRepresentitive = new FactLeftJoin<IStaffFact>(salesRepFact);

			if (jobPlugin is QuotedBooking booking)
			{
				Mode = booking.Mode;
				OuterPackUnit = booking.Booking.JS_F3_NKPackType;
			}
			else if (jobPlugin is ForwardingShipment shipment)
			{
				OuterPackUnit = shipment.JS_F3_NKPackType;
				Mode = RateOneOffShipment.FreightModeConverter.GetMode(shipment.JS_TransportMode, shipment.JS_PackingMode);
			}
		}

		public Guid BookingPK { get; }
		...
		public FactLeftJoin<IStaffFact> SalesRepresentitive { get; }
