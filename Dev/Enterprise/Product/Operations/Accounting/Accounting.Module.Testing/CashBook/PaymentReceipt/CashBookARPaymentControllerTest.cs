...
{
	[TestedType(typeof(CashBookARPaymentController))]
	class CashBookARPaymentControllerTest : ARPaymentControllerTestCase
	{
		...
		{
			var controller = new CashBookARPaymentController();

			using (IZForm newForm = controller.ShowNewForm())
			{
				AssertNotNull("New Form", newForm);
				AssertEquals("New Form Type", typeof(PaymentForm), newForm.GetType());

				var newFormAsPaymentForm = (PaymentForm)newForm;
				var typeOfBusinessEntity = newFormAsPaymentForm.BusinessEntity.GetType();

				AssertEquals("Type of New BusinessEntity", typeof(ARPayment), typeOfBusinessEntity);
