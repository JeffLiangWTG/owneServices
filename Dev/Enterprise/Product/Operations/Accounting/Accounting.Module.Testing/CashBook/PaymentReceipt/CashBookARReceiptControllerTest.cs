...
{
	[TestedType(typeof(CashBookARReceiptController))]
	class CashBookARReceiptControllerTest : ARReceiptControllerTestCase
	{
		...
		{
			var controller = new CashBookARReceiptController();
			...
			using (var newForm = controller.ShowNewForm())
			{
				...
				AssertEquals("Type of New BusinessEntity", typeof(ARReceipt), typeOfBusinessEntity);
