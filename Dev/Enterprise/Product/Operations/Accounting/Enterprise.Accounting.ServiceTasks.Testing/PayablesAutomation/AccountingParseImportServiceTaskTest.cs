...
{
	[TestedType(typeof(AccountingParseImportServiceTask))]
	public class AccountingParseImportServiceTaskTest : ServiceTaskTestCase<AccountingParseImportServiceTask>
	{
		public void TestHostedServiceAttribute()
		{
			...
			CombineAssertions(() =>
			{
				AssertEquals(nameof(HostedServiceAttribute.Code), "API", hostedServiceAttribute.Code);
				AssertEquals(nameof(HostedServiceAttribute.Description), "Accounting Parse Import Service Task", hostedServiceAttribute.Description);
		...
		public void TestRunTask()
		{
			...
			var task = new AccountingParseImportServiceTask { ServiceLogger = serviceLogger };

			task.RunTask(cancellationToken);

			AssertContains("No messages to process.", serviceLogger.ToString());
		}
