...
{
	class GetCategoryDetailsFromRegistryTest : ScriptTest
	{
		...
		void PrepareData()
		{
			string insertRegistryValues = string.Format(@"DECLARE @registryRawValue nvarchar(MAX)
...
Values
(newID(), 'COMPLIANCEREPORTSSETUPSCN', 'BIN', CONVERT(varbinary(MAX), @registryRawValue), 1, 0)");
