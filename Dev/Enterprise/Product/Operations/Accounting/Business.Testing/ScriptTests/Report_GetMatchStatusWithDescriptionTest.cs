...
{
	class Report_GetMatchStatusWithDescriptionTest : ScriptTest
	{
		public void TestReport_GetMatchStatusWithDescription()
		{
			var dt = RunScript(GlbCompany.CurrentCompany.PK);
			AssertEquals("default value one line", 1, dt.Rows.Count);
			AssertEquals(1, dt.Select("Code = 'UAC' and Description = 'Unallocated'").Length);
			...
			AccountingConfigurationRegistry.Instance.MatchStatus.SetValue(Guid.Empty, Guid.Empty, Guid.Empty, newCollection);
			newCollection.Add("TS2", (NoResString)"test description2", false);
			AccountingConfigurationRegistry.Instance.MatchStatus.SetValue(GlbCompany.CurrentCompany.PK.ToGuid(), Guid.Empty, Guid.Empty, newCollection);
			Factory.Save();
		...
		{
			var sql = $"SELECT Code, Description FROM [dbo].[Report_GetMatchStatusWithDescription]('{companyPK}')";
