...
{
	public class UruguayEInvoicingPivotsToRequeueInfoTest : TestCaseWithFactory
	{
...
- 'DLV' - Delivered (when you have appropriate security rights).", GetInstance().GetPivotsToRequeueMessage());
		...
		IEInvoicingPivotsToRequeueFilterProvider GetInstance() => (ObjectFactory.Get<IGlobalAccountingCountryFactory>().GetCountryFactory(Constants.CountryCodes.Uruguay) as IInstanceProvider<IEInvoicingPivotsToRequeueFilterProvider>).Get();
	}
}
