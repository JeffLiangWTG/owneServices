...
{
	public class PanamaEInvoicingPivotsToRequeueInfoTest : TestCaseWithFactory
	{
...
- 'DLV' - Delivered (when you have appropriate security rights).", GetInstance().GetPivotsToRequeueMessage());
		...
		IEInvoicingPivotsToRequeueFilterProvider GetInstance() => (ObjectFactory.Get<IGlobalAccountingCountryFactory>().GetCountryFactory(Constants.CountryCodes.Panama) as IInstanceProvider<IEInvoicingPivotsToRequeueFilterProvider>).Get();
	}
}
