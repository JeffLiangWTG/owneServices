...
namespace Enterprise.Accounting.Business.PayableOrder.Testing
{
	...
	[TestedType(typeof(AccPayableOrderLinesTotalByProductCollection))]
	internal sealed class AccPayableOrderLinesTotalByProductCollectionTest : NonPersistentBusinessObjectCollectionTestCase<AccPayableOrderLinesTotalByProductCollection>
	{
		public void TestPopulation()
		{
			...
			orderLine1.APL_Quantity = 3.00M;
			orderLine1.APL_QtyInvoiced = 2.00M;
			orderLine1.APL_QtyReceived = 1.00M;
			orderLine1.APL_PartNo = part1.OP_PartNum;
			var orderLine2 = order.OrderLines.AddNew();
			orderLine2.APL_Quantity = 4.50M;
			orderLine2.APL_QtyInvoiced = 3.50M;
			orderLine2.APL_QtyReceived = 1.50M;
			orderLine2.APL_PartNo = part1.OP_PartNum;
			var orderLine3 = order.OrderLines.AddNew();
			orderLine3.APL_Quantity = 5M;
			orderLine3.APL_QtyInvoiced = 4M;
			orderLine3.APL_QtyReceived = 3M;
			orderLine3.APL_PartNo = part2.OP_PartNum;
			var orderLine4 = order.OrderLines.AddNew();
			orderLine4.APL_Quantity = 6M;
			orderLine4.APL_QtyInvoiced = 5M;
			orderLine4.APL_QtyReceived = 4M;
			orderLine4.APL_Desc = "no product";
			var orderLine5 = order.OrderLines.AddNew();
			orderLine5.APL_Quantity = 8M;
			orderLine5.APL_QtyInvoiced = 5M;
			orderLine5.APL_QtyReceived = 4.9M;
			orderLine5.APL_PartNo = "111444";
			orderLine5.APL_Desc = "manually added part";
			var totals = new AccPayableOrderLinesTotalByProductCollection(order, Factory);
			AssertEquals(0, totals.Count);
			totals.Populate();
			AssertEquals(4, totals.Count);
			foreach (AccPayableOrderLinesTotalByProduct total in totals)
			{
				...
				else if (total.OrderLines[0].APL_PartNo == "111444")
				{
			...
			orderLine2.APL_QtyInvoiced = 2.55M;
			orderLine2.APL_QtyReceived = 1.50M;
			AssertEquals(4, totals.Count);
			foreach (AccPayableOrderLinesTotalByProduct total in totals)
			{
			...
			orderLine2.APL_PartNo = part2.OP_PartNum;
			totals.Populate();
			AssertEquals(2, totals.Count);
			foreach (AccPayableOrderLinesTotalByProduct total in totals)
			{
		...
		void AssertCorrectForOrderLine5(AccPayableOrderLinesTotalByProduct total)
		{
		...
		protected override AccPayableOrderLinesTotalByProductCollection GetCollectionToTest()
		{
			return new AccPayableOrderLinesTotalByProductCollection(Factory.New<AccPayableOrderHeader>(), Factory);
		}
