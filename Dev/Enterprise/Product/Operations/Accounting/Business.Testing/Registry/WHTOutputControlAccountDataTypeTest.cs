...
{
	[TestedType(typeof(WHTOutputControlAccountDataType))]
	class WHTOutputControlAccountDataTypeTest : GeneralLedgerRegistryDataTypeTest
	{
		#region Implementation

		public WHTOutputControlAccountDataTypeTest()
		{
			HeaderLinesCauseValidationError = new List<HeaderLine>();
			var header = new Header() { PK = Guid.NewGuid(), Ledger = "CB", TransactionType = "DRC", InvoiceNumber = "1" };
			var line = new Line() { PK = Guid.NewGuid(), AH = header.PK, IsGST = false, LineType = "REV", GSTVATBasis = "A", PostToGL = "N", IsInsertAccCashBasisVAT = false, IsDeleteAccCashBasisVATQueue = true };
			var headerLine = new HeaderLine() { Header = header, Line = line };
			HeaderLinesCauseValidationError.Add(headerLine);
			header = new Header() { PK = Guid.NewGuid(), Ledger = "AR", TransactionType = "JNL", InvoiceNumber = "2" };
			...
			header = new Header() { PK = Guid.NewGuid(), Ledger = "AP", TransactionType = "DRC", InvoiceNumber = "3" };
		...
		{
			get { return typeof(WHTOutputControlAccountDataType); }
		}
