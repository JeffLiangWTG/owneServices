...
{
	public class TaxIdAndTaxMessageMappingLimitTaxIDsValidatorTest : TaxIdAndTaxMessageMappingValidatorTest
	{
		protected override void AssertLineTypes(string[] lineTypes)
		{
			...
			AssertEquals(false, MappingValidator.IsValidateMapping(lineTypes, taxRate3, taxMsg11));
			AssertEquals(false, MappingValidator.IsValidateMapping(lineTypes, taxRate3, taxMsg2));
		}
		...
		protected override TaxIdAndTaxMessageMappingValidator CreateMappingValidator(TaxIdAndTaxMessageCombinationRulesCollection rules)
			=> new TaxIdAndTaxMessageMappingLimitTaxIDsValidator(rules);
	}
}
