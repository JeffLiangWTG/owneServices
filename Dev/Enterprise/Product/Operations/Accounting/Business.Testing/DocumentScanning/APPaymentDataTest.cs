...
{
	public  class APPaymentDataTest : TestCaseWithFactory
	{
		public void TestGetEDocViaUniversalXmlSupport()
		{
			var orgHeader = Factory.NewWithValidTestData<OrgHeader>();
			orgHeader.OH_Code = "MYTESTORG";
			var uxmlSupport = new APPaymentData().GetEDocsViaUniversalXmlSupport();
			AssertEquals(null, uxmlSupport.LoadBusinessObjectFromCode(Factory, "invalid-code"));

			var apPayment1 = Factory.NewWithValidTestData<APPayment>();
			apPayment1.AH_OH = orgHeader.PK;

			Factory.Save();
			AssertEquals("00001000", apPayment1.AH_TransactionNum);

			var apPayment2 = Factory.NewWithValidTestData<APPayment>();
			apPayment2.AH_OH = orgHeader.PK;
			Factory.Save();
			AssertEquals("00001001", apPayment2.AH_TransactionNum);
			
			AssertEquals(apPayment1.PK, uxmlSupport.LoadBusinessObjectFromCode(Factory, "MYTESTORG|00001000")?.PK);
		...
		{
			AssertEquals("ABIGAS|100001", new APPaymentData().GetEDocsViaUniversalXmlSupport().ExampleCodeFormat);
		}
	}
}
