...
namespace Enterprise.Accounting.Business.EmailNotification.Testing
{
	...
	class JobOverseasAgentChargesPosterEmailTest : MasterFiles.Business.Testing.AccountingEmailDefTest
	{
		public void TestEmailSubjectAndBody()
		{
			...
			var poster = new JobOverseasAgentChargesPoster(Shipment);
			...
			AssertEquals("Email subject", "Overseas Agent Charge posting errors for S00010001", email.Subject);
			AssertEquals("Email body", @"Overseas Agent Charges posting was run as a workflow action, and an attempt to post overseas agent charge(s) for S00010001 failed because of the following errors:
You cannot post because job S00010001 has errors. Please fix errors before posting.
 ...
 - Local Client: Please enter Local Client or Overseas Agent.", email.Body);
			...
			poster = new JobOverseasAgentChargesPoster(Shipment);
...
Please enter charges before posting.", email.Body);
		...
		protected override void SetUp()
		{
			...
			TestObjectCreator.CreateCharge(Job, TestObjectCreator.CC1, "Desc 1", TestObjectCreator.AUD, 50M, null, TestObjectCreator.AUD, 100M, TestObjectCreator.Agent);
