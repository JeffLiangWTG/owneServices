...
namespace Enterprise.Accounting.Business.EmailNotification.Testing
{
	...
	class LocalSisterCompanyChargePosterEmailTest : MasterFiles.Business.Testing.AccountingEmailDefTest
	{
		public void TestEmailSubjectAndBody()
		{
			...
			var poster = new LocalSisterCompanyChargePoster(shipment);
			...
			AssertEquals("Email subject", "Local Group Company Charge posting errors for S00010001", email.Subject);
			AssertEquals("Email body", @"Local Group Company Charge posting was run as a workflow action, and an attempt to post charge(s) for S00010001 failed because of the following errors:
You cannot post because job S00010001 has errors. Please fix errors before posting.
 ...
 - Local Client: Please enter Local Client or Overseas Agent.", email.Body);
			...
			poster = new LocalSisterCompanyChargePoster(shipment);
...
Please enter charges before posting.", email.Body);
		...
		protected override void SetUp()
		{
			...
			var orgHeader = TestObjectCreator.CreateOrgHeader("ORGAU1", false, true, "AUMEL");
