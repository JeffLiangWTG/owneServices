...
namespace Enterprise.Accounting.Business.EmailNotification.Testing
{
	...
	class SisterCompanyChargePosterEmailTest : MasterFiles.Business.Testing.AccountingEmailDefTest
	{
		public void TestEmailSubjectAndBody()
		{
			...
			var poster = new SisterCompanyChargePoster(shipment);
			...
			AssertEquals("Email subject", "Group Company Charge posting errors for S00010001", email.Subject);
			AssertEquals("Email body", @"Group Company Charge posting was run as a workflow action, and an attempt to post charge(s) for S00010001 failed because of the following errors:
You cannot post because job S00010001 has errors. Please fix errors before posting.
 ...
 - Local Client: Please enter Local Client or Overseas Agent.", email.Body);
			...
			poster = new SisterCompanyChargePoster(shipment);
...
Please enter charges before posting.", email.Body);
		...
		{
			AssertExceptionThrown(typeof(ArgumentNullException), () => new SisterCompanyChargePosterEmail(null, "some error"));
			AssertExceptionThrown(typeof(ArgumentException), () => new SisterCompanyChargePosterEmail(job, string.Empty));
			AssertExceptionThrown(typeof(ArgumentException), () => new SisterCompanyChargePosterEmail(job, null));
		}
		...
		protected override Type EmailDefType
		{
			get
			{
				return typeof(SisterCompanyChargePosterEmail);
			}
		}
		...
		{
			var orgHeader = TestObjectCreator.CreateOrgHeader("ORGNZ1", false, true, "NZAKL");
			var companyNZ = TestObjectCreator.CreateNewCompany("NZ1", "NZ");
			companyNZ.GC_OH_OrgProxy = orgHeader.PK;
			...
			TestObjectCreator.CreateCharge(job, TestObjectCreator.CC1, "Desc 1", TestObjectCreator.AUD, 50M, null, TestObjectCreator.AUD, 100M, orgHeader);
