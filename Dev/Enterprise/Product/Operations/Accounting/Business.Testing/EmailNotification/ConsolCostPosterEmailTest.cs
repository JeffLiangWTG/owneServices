...
{
	class ConsolCostPosterEmailTest : AccountingEmailDefTest
	{
		public void TestEmailSubjectAndBody_WhenAllowZeroValueInvoices()
		{
			AssertEmailSubjectAndBody(true);
		}

		public void TestEmailSubjectAndBody_WhenDisallowZeroValueInvoices()
		{
		...
		void AssertEmailSubjectAndBody(bool allowZeroValueARInvoices)
		{
			...
			var consolCostPoster = new ConsolCostPoster(consol);

			var exceptionThrown = AssertExceptionThrown<LogSubscriberToAbortLogGroupProcessingSilentlyException>(() => consolCostPoster.Process(new NotificationBuffer()));
...
You may want to check the data you entered on Job Invoicing tabs on each shipment attached to this consol, and on the Costing tab of this form. Make sure that amounts are not zero and all appropriate information is entered for AP Invoices (if applicable).";
			...
			var expectedSubject = "All Cost posting errors for Consol C0001000";
			AssertEquals("Email subject", expectedSubject, email.Subject);
			var expectedBody = @"The workflow trigger action 'Post All Costs' ran for Consol C0001000, but it failed due to the following errors:" + System.Environment.NewLine + expectedMessage;
...
Parameter name: errors", () => new JobConsolCostOnlyPosterEmail(consol, string.Empty));
			AssertExceptionThrown(typeof(ArgumentException), @"Value cannot be null.
Parameter name: errors", () => new JobConsolCostOnlyPosterEmail(consol, null));
		}
		...
		protected override Type EmailDefType
		{
			get
			{
				return typeof(ConsolCostPosterEmail);
			}
		}
