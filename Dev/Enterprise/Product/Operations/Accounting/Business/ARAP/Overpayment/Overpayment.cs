...
namespace Enterprise.Accounting.Business.ARAP.Overpayment
{
	...
	public abstract partial class Overpayment : TransactionHeader, IMatching, IReversing, IMiscellaneousTransaction
	{
		public Overpayment(BusinessObjectFactory factory, DataRow row)
			: base(factory, row)
		{
		...
		protected override void SetDefaultValues()
		{
			...
			AH_Desc = AccountingConstants.MatchingDefaultDesc.OverpaymentDesc;
			AH_AG = AccountingConfigurationRegistry.Instance.OverpaymentsAccount.Value;
		}

		protected override TransactionHeaderValidation GetNewValidationCore()
		{
			return new OverpaymentValidation(this);
		}
		...
		{
			get { return Enterprise.ZArchitecture.Core.TransactionTypes.Overpayment; }
		}
		...
		void IMatching.Unmatch(ZDecimal matchLinkAmount, ZDecimal matchLinkOSAmount)
		{
			...
			revTrans.SetDescription(string.Format(AccountingConfigurationRegistry.Instance.GetTransactionDescriptionFromCode(AccountingConstants.VoucherItemRegistryCode.UnOverpaymentRelated,
									   Res.GetString("e37a1278-f8b6-41ce-b6b7-801ce39d21eb", "OVERPAYMENT RELATING TO UN-MATCH NO")) + " {0}", originalLink.AP_MatchGroupNum));
			revTrans.SetNumberOfSupportingDocuments(AccountingConfigurationRegistry.Instance.GetVoucherNoOfAttchmentsFromCode(AccountingConstants.VoucherItemRegistryCode.UnOverpaymentRelated, 0));
		...
		SecurityCheckpoint IMiscellaneousTransaction.CheckpointForUnmatch => CheckpointToUnmatchCore;

		protected abstract SecurityCheckpoint CheckpointToUnmatchCore { get; }
