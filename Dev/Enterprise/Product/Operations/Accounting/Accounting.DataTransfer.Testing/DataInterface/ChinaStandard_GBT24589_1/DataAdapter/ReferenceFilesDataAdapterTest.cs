...
{
	[TestedType(typeof(ReferenceFilesDataAdapter))]
	sealed class ReferenceFilesDataAdapterTest : BaseAccountingDataAdapterTest<BusinessObjectThatDoesntSaveForCN, XSDs.公共档案>
	{
		[TestDate(2015, 8, 26)]
		public void TestExportReferenceFiles()
		{
			AddTranslations();
			...
			{
				serialiser.WriteToXml(writer, Adapter, new BusinessObjectThatDoesntSaveForCN(Factory) { Period = 200501 }, context);
				string expectedText = Retriever.GetString("ReferenceFiles.xml");
				this.AssertXMLEqualsByDiff("ReferenceFiles XML does not match", expectedText, writer.ToString());
			}
		}
		...
		protected override void AssertImportFromThenExportToProducesSameXml(BusinessObjectAndExpectedOutputFileName sample, BusinessObjectThatDoesntSaveForCN bizObjToImportTo, XSDs.公共档案 exportedValueObject, string exportedValueObjectXml, string bizObjToImportToDescription)
		{
			Assert(true);
		}
		...
		protected override ValueObjectDataAdapter<BusinessObjectThatDoesntSaveForCN, XSDs.公共档案> GetNewBizObjXmlDataAdapter()
		{
			return new ReferenceFilesDataAdapter();
		}
		...
		{
			return new BusinessObjectAndExpectedOutputFileName(new BusinessObjectThatDoesntSaveForCN(Factory) { Period = 200501 }, Retriever.SaveResourceToFile("EmptyReferenceFiles.xml"), ValidationKind.None, "Fully Populated Reference Files");
		}

		protected override BusinessObjectAndExpectedOutputFileName GetPopulatedBizObjWithEmptyFieldsSample()
		{
			return new BusinessObjectAndExpectedOutputFileName(new BusinessObjectThatDoesntSaveForCN(Factory) { Period = 200501 }, Retriever.SaveResourceToFile("EmptyReferenceFiles.xml"), ValidationKind.None, "Semi Populated Reference Files");
		}

		protected override BusinessObjectAndExpectedOutputFileName GetEmptyBizObjSample()
		{
			return new BusinessObjectAndExpectedOutputFileName(new BusinessObjectThatDoesntSaveForCN(Factory) { Period = 200501 }, Retriever.SaveResourceToFile("EmptyReferenceFiles.xml"), ValidationKind.None, "Empty Reference Files");
		}
		...
		ReferenceFilesDataAdapter fAdapter;
		ReferenceFilesDataAdapter Adapter
		{
			get { return fAdapter ?? (fAdapter = new ReferenceFilesDataAdapter()); }
		}
		...
		protected override string[] XmlNodesToExcludeFromCoverageTest
		{
			get
			{
				...
				return new string[] { "电子账簿/locID",
									...
									"会计期间/会计期间起始日期/locID",
									"会计期间/会计期间结束日期/locID",
									"记账凭证类型/记账凭证类型编号/locID",
									"记账凭证类型/记账凭证类型名称/locID",
									"记账凭证类型/记账凭证类型简称/locID",
									"汇率类型/汇率类型编号/locID",
									"汇率类型/汇率类型名称/locID",
									"币种/币种编码/locID",
									"币种/币种编码/Value",
									...
									"自定义档案项/档案描述/locID",
									"自定义档案项/档案描述/Value",
									"自定义档案项/是否有层级特征/locID",
									"自定义档案项/是否有层级特征/Value",
									"自定义档案项/档案编码规则/locID",
									"自定义档案项/档案编码规则/Value",
									"自定义档案项/locID",
									"自定义档案项值/档案编码/locID",
									...
									"自定义档案项值/档案值描述/locID",
									"自定义档案项值/档案值描述/Value",
									"自定义档案项值/档案值父节点/locID",
									"自定义档案项值/档案值父节点/Value",
									"自定义档案项值/档案值级次/locID",
									"自定义档案项值/档案值级次/Value",
									"自定义档案项值/locID",
									"locID" };
