...
{
	internal sealed class AllChargesStrategyTest : ChargeSpecificationStrategyTest<AllChargesStrategy>
	{
...
Warning: The Invoicing Job for company CO3 was skipped during the Import process as it has posted charges.";
		...
		public void TestMatching()
		{
			...
			((IBindingList)job.Charges).ListChanged += new ListChangedEventHandler((sender, e) => jobChargesListChangedHitCount++);
			...
			AssertEquals("ListChanged on Job.Charges should be called only once", 1, jobChargesListChangedHitCount);
		}
		...
		{
			get { return Xsd.ChargesSpecified.AllCharges; }
		}
