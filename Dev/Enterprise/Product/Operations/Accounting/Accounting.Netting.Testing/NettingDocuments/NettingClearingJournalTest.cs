...
{
	[TestedType(typeof(NettingClearingJournal))]
	public class NettingClearingJournalTest : NonPersistentBusinessObjectTestCase
	{
		public void TestNettingPeriod()
		{
			AssertNotNull(Statement);
			var clearingJournal = new NettingClearingJournal(Statement);
			Assert(!clearingJournal.NettingPeriod.IsEmpty);

			Statement = new NettingCentreStatement(Factory);
			clearingJournal = new NettingClearingJournal(Statement);
			Assert(clearingJournal.NettingPeriod.IsEmpty);

			clearingJournal = new NettingClearingJournal();
			Assert(clearingJournal.NettingPeriod.IsEmpty);
		}
		...
		{
			AssertNotNull(Statement);
			AssertNotNull(Statement.NettingClearingJournals);

			foreach (var clearingJournal in Statement.NettingClearingJournals)
			{
				AssertNotNull(clearingJournal.TransactionRef);
			}
		}
		...
		public void TestTransactionLineRef()
		{
			...
			foreach (var clearingJournal in Statement.NettingClearingJournals)
			{
				AssertNotNull(clearingJournal.TransactionLineRef);
			}
		}
