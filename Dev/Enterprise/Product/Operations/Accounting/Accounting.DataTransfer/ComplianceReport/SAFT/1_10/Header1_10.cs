...
{
	internal class Header1_10 : ComplianceReportXmlBuilder
	{
		...
		XStreamingElement BuildXmlCore(AccComplianceReport report, SAFTAdditionalDataCollector additionalData)
		{
			...
			var norwayCusCodes = reportCompany.OrgProxy.CustomsCodes.Cast<OrgCusCode>().Where(x => x.OK_RN_NKCodeCountry == Core.Constants.CountryCodes.Norway);
			var govRegistrationNumber = norwayCusCodes.FirstOrDefault(x => x.OK_CodeType == OrgCusCode.CodeTypes.GovBusinessCode)?.OK_CustomsRegNo;
			var taxRegistrationNumber = norwayCusCodes.FirstOrDefault(x => x.OK_CodeType == OrgCusCode.NorwayCodeTypes.MVA)?.OK_CustomsRegNo ?? ComplianceReportAdditionalDataProviderHelper.NoneTaxRegistrationNumber;
			var countryCode = companyOrgAddress?.Country?.Code ?? ZString.Empty;
			var contactStaff = additionalData.ContactStaff;

			var companyElement = BuildCompany(companyOrgAddress, contactStaff, countryCode, govRegistrationNumber, taxRegistrationNumber, additionalData.DefaultReceiptAccountNumber);
			var selectionCriteriaElement = BuildSelectionCriteria(additionalData.DateFrom, additionalData.DateTo);
			var header = BuildHeader(companyElement, selectionCriteriaElement, companyOrgAddress.Country.Code, reportCompany.LocalCurrency.Code);
		...
		XStreamingElement BuildHeader(XElement companyElement, XElement selectionCriteriaElement, string countryCode, string localCurrencyCode)
		{
			var header = new XStreamingElement("Header",
							new XElement("AuditFileVersion", "1.10"),
			...
			header.Add(new XElement("TaxAccountingBasis", "A"));

			return header;
		}

		[SuppressMessage("CargoWiseOne", "CW1161:ResGetStringAnalyzer", Justification = "Hard-coded xml node name")]
		XElement BuildCompany(OrganizationAddress companyOrgAddress, GlbStaff contactStaff, string countryCode, string govRegistrationNumber, string taxRegistrationNumber, string defaultReceiptAccountNumber)
		{
			...
			result.Add(BuildTaxRegistration(taxRegistrationNumber, countryCode));
		...
		[SuppressMessage("CargoWiseOne", "CW1161:ResGetStringAnalyzer", Justification = "Hard-coded xml node name")]
		XElement BuildTaxRegistration(string taxRegistrationNumber, string countryCode)
		{
			return new XElement("TaxRegistration",
							new XElement("TaxRegistrationNumber", taxRegistrationNumber),
							new XElement("TaxAuthority", countryCode == Core.Constants.CountryCodes.Norway ? "Skatteetaten" : null)
							);
		}
