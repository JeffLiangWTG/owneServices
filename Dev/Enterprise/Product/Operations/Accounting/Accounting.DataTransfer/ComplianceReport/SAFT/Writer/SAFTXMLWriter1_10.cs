...
{
	class SAFTXMLWriter1_10 : SAFTXMLWriterBase
	{
		public SAFTXMLWriter1_10(AccComplianceReport report, ReportModeAndCreditorSelector reportModeAndCreditorSelector, Action<string, int?> updateProgressStatus) : base(report, reportModeAndCreditorSelector, updateProgressStatus) { }

		public SAFTXMLWriter1_10(IEnumerable<AccComplianceReport> reports, ReportModeAndCreditorSelector reportModeAndCreditorSelector, Action<string, int?> updateProgressStatus) : base(reports, reportModeAndCreditorSelector, updateProgressStatus) { }

		protected override IEnumerable<ComplianceReportXmlBuilder> MainBodyElements => new ComplianceReportXmlBuilder[] { new Header1_10(), new MasterFiles1_10(), new GeneralLedgerEntries1_10() };

		protected override string XSDName => "Enterprise.Accounting.DataTransfer.ComplianceReport.SAFT._1_10.SAFT1.10.xsd";
		...
		(string attachFileName, int xmlFileIndex) WriteMasterFilesXMLToZipFile(int zipIndex, int xmlFileIndex, int xmlTotalCount, string fileName, string fileNameWithoutExtension)
		{
			...
			WriteReportXmlToZipFile(Reports, new ComplianceReportXmlBuilder[] { new Header1_10(), new MasterFiles1_10() }, xmlName);
		...
		IEnumerable<string> WriteGeneralLedgerEntriesXMLToZipFile(int zipIndex, int xmlFileIndex, int xmlTotalCount, string fileName, string fileNameWithoutExtension)
		{
			...
			while (zipIndex <= Reports.Count() / 2)
			{
				...
				foreach (var report in reportsInZip)
				{
					...
					WriteReportXmlToZipFile(new AccComplianceReport[] { report }, new ComplianceReportXmlBuilder[] { new Header1_10(), new GeneralLedgerEntries1_10() }, name);
				}
