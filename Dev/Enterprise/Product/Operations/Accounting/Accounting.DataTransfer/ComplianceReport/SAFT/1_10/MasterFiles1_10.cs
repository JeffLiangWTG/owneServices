...
{
	internal class MasterFiles1_10 : ComplianceReportXmlBuilder
	{
			...
			return new XStreamingElement("MasterFiles",
					...
					new XElement("TaxTable",
						new XElement("TaxTableEntry",
							new XElement("TaxType", "MVA"),
							new XElement("Description", "Merverdiavgift"),
		...
		XStreamingElement BuildCustomerSupplierXml(AccComplianceReport report, UniversalOrgAddress orgAddress, OrgContact contact, SAFTAdditionalDataCollector additionalData, string contactType)
		{
			...
			var norwayRegistrationNumbers = orgAddress.RegistrationNumberCollection?.Where(x => x.CountryOfIssue.Code.Value == Core.Constants.CountryCodes.Norway) ?? new List<UniversalRegistrationNumber>();
			var govRegistrationNumber = norwayRegistrationNumbers.FirstOrDefault(x => x.Type.Code.Value == OrgCusCode.CodeTypes.GovBusinessCode)?.Value;
			var taxRegistrationNumber = norwayRegistrationNumbers.FirstOrDefault(x => x.Type.Code.Value == OrgCusCode.NorwayCodeTypes.MVA)?.Value ?? ComplianceReportAdditionalDataProviderHelper.NoneTaxRegistrationNumber;
			...
			return new XStreamingElement(parentNodeName,
					...
					new XElement("TaxRegistration",
							...
							new XElement("TaxAuthority", GlbCompany.CurrentCompany.Country.Code == Core.Constants.CountryCodes.Norway ? "Skatteetaten" : string.Empty)
						),
		...
		XStreamingElement BuildGLLedgerAccountXml(AccComplianceReport report, SAFTAdditionalDataCollector additionalData)
		{
			...
			return new XStreamingElement("GeneralLedgerAccounts",
				validGLAccounts.OrderBy(x => x.AccountNum).Select(x => new XStreamingElement("Account",
					...
					new XElement("StandardAccountID", x.GLAccount.LocalComplianceAccountCode.HasValue
													&& x.GLAccount.LocalComplianceAccountCode.Value.Length >= 2 ? x.GLAccount.LocalComplianceAccountCode.Value.ToString().Substring(0, 2) : string.Empty),
