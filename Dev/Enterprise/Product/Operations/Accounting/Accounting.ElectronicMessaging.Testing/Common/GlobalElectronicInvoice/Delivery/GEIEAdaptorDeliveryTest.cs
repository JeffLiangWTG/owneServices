...
{
	public class GEIEAdaptorDeliveryTest : TestCaseWithFactory
	{
...
""";
				...
				using (var stream = (SubStreamableStream)new MemoryStream(Encoding.UTF8.GetBytes(messageBody)))
				{
					...
					var delivery = new GEIEAdaptorDelivery();
					...
					AssertEquals("EI_HeaderText", "<EDIDelivery><FileName></FileName><EmailSubject></EmailSubject></EDIDelivery>", result.EI_HeaderText);
