...
{
	sealed class EntityTradeDetailWrapperFetchStrategyTest : TestCaseWithFactory
	{
		#region FetchForView
		...
		public void TestFetchForView_EstimatedValues()
		{
			...
			var expectedDbHits = new Dictionary<string, int>
			{
				...
				{ OrgTradeDetailSchema.Constants.TableName, 6 },
		...
		void AssertFetchForViewDbHits(string[] viewColumnNames, Dictionary<string, int> expectedDbHits)
		{
			...
			foreach (var entityDetail in testSalesCollection.SelectMany(x => x.EntityTradeDetails))
			{
				entityDetail.FetchStrategy.FetchForView(viewColumnNames.Select(x => new TableColumn("", x)).ToArray());
			}
