...
{
	sealed class TradePeriodStateToStateGrouperTest : TradePeriodGrouperTestCase
	{
		public void TestGetGroupings()
		{
			...
			var grouper = new TradePeriodStateToStateGrouper();
			...
			using (RowFactory.SetCachedTables()) // do not cache RefCountry to test fetch hints are working
			{
				...
				AssertContainsExactElementsInAnyOrder(
					new ZString[]
				{
					"AU, South Australia -> AU, South Australia",
					"AU, South Australia -> IT, Salerno",
					"IT, Salerno -> IT, Salerno",
					"AU, South Australia -> (Unknown)",
					"(Unknown) -> AU, South Australia",
				},
				...
				AssertContainsExactElementsInAnyOrder(BusinessObjectEqualityComparer<OrgTradePeriod>.PKOnlyComparer,
					new[] { period1, period3 },
					groupings.Single(x => x.GroupKey == "AU, South Australia -> AU, South Australia").GroupedTradePeriods);

				AssertContainsExactElementsInAnyOrder(BusinessObjectEqualityComparer<OrgTradePeriod>.PKOnlyComparer,
					new[] { period2 },
					groupings.Single(x => x.GroupKey == "AU, South Australia -> IT, Salerno").GroupedTradePeriods);

				AssertContainsExactElementsInAnyOrder(BusinessObjectEqualityComparer<OrgTradePeriod>.PKOnlyComparer,
					new[] { period4 },
					groupings.Single(x => x.GroupKey == "IT, Salerno -> IT, Salerno").GroupedTradePeriods);

				AssertContainsExactElementsInAnyOrder(BusinessObjectEqualityComparer<OrgTradePeriod>.PKOnlyComparer,
					new[] { period5 },
					groupings.Single(x => x.GroupKey == "AU, South Australia -> (Unknown)").GroupedTradePeriods);

				AssertContainsExactElementsInAnyOrder(BusinessObjectEqualityComparer<OrgTradePeriod>.PKOnlyComparer,
					new[] { period6 },
					groupings.Single(x => x.GroupKey == "(Unknown) -> AU, South Australia").GroupedTradePeriods);
