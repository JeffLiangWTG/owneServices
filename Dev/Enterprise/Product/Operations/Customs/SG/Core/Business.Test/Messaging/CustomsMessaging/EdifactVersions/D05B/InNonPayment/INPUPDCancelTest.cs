...
{
	public class INPUPDCancelTest : TestCaseWithFactory
	{
		[ExpectNoExceptions()]
		...
		public void TestGenerateSegmentGroup5()
		{
			...
			DataProvider.InwardTransportCode = SGConstants.TransportCodes.Sea;
			CUSDECMessage msg = MessageBuilder.CusdecMessage;
			AssertEquals("Should have attachments only", "DOC+:::DOCUMENT ATTACHMENT+001::ATTDOC_0001'DOC+:::DOCUMENT ATTACHMENT+002::ATTDOC_0002'", msg.Group5.ToString(new UNOASGCharacterSet()));
		}
		...
		public void TestGenerateSegmentGroup6()
		{
			...
			CUSDECMessage msg = MessageBuilder.CusdecMessage;
			AssertMultilineEquals("INPUPD Cancel - SG6 only declarant should be present", "NAD+DT++DECLARER'CTA+IC+:DEC0012SA'COM+322223:TE'", msg.Group6.ToString(new UNOASGCharacterSet()), '\'');
		}
		...
		public void TestGenerateSegmentGroup30()
		{
			...
			CUSDECMessage msg = MessageBuilder.CusdecMessage;
			AssertEquals("Cancel msg should not have SG30", "", msg.Group30.ToString(new UNOASGCharacterSet()));
		}
		...
		INPUPDCancel MessageBuilder
		{
			get
			{
				if (fMessageBuilder == null)
				{
					fMessageBuilder = new INPUPDCancel(DataProvider);
				}
		...
		void InitItem(ItemsTestClass item)
		{
			...
			//item.CASCCodes1[0] = threeCASCCodes;
		}
		#endregion
	}
}
