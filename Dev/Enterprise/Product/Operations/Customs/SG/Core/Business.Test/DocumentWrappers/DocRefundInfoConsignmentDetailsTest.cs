...
{
	[TestedType(typeof(DocRefundInfoConsignmentDetails))]
	sealed class DocRefundInfoConsignmentDetailsTest : DocumentWrapperTestCase
	{
		#region IRefundInfoConsignment
		...
		DocRefundInfoConsignmentDetails docConsignmentDetails
		{
			get
			{
				if (fDocConsignmentDetails == null)
				{
					fDocConsignmentDetails = DocRefundInfoConsignmentDetails.New(ConsignmentDetails, Factory);
				}
				return fDocConsignmentDetails;
			}
		}
		DocRefundInfoConsignmentDetails fDocConsignmentDetails;

		RefundInfoConsignmentDetails ConsignmentDetails
		{
			get
			{
				if (fConsignmentDetails == null)
				{
					...
					fConsignmentDetails = new RefundInfoConsignmentDetails(iptupt.ConsignmentDetails[0], Factory);
				}
				return fConsignmentDetails;
			}
		}
		RefundInfoConsignmentDetails fConsignmentDetails;

		public override DocumentWrapper[] GetDocumentWrappers()
		{
			DocRefundInfoConsignmentDetails result = DocRefundInfoConsignmentDetails.New(ConsignmentDetails, Factory);
			return new DocumentWrapper[] { result };
		}

		protected override DocumentWrapper CreateDocumentWrapperFromStaticNewMethod()
		{
			return DocRefundInfoConsignmentDetails.New(ConsignmentDetails, Factory);
		}
