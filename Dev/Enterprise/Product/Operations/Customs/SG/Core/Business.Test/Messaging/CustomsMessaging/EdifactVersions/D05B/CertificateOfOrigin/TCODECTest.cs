...
{
	public class TCODECTest : TestCaseWithFactory
	{
		public void TestMessageConstructor()
		{
			TCODEC tCODECMessage = new TCODEC(CofODataProvider);
			AssertNotNull(tCODECMessage);
		}
		...
		{
			TCODECMessage msg = MessageBuilder.EdifactMessage;
			AssertEquals("UNH+1+TCODEC:0:1:RT:040+COODEC'", msg.UNH.ToString(new UNOASGCharacterSet()));
		}
		...
		public void TestGenerateSG3()
		{
			...
			AssertEquals("NAD+AE+AAA374M++EAGLE DATAMATION INTERNATIONAL'", msg.Group3[0].NAD.ToString(new UNOASGCharacterSet()));
			SegmentGroup3 declarantSG3 = msg.Group3[1];
			...
			AssertEquals("NAD+DT++TEST SG BROKER'", declarantSG3.NAD.ToString(new UNOASGCharacterSet()));
			AssertEquals("CTA+IC+:V13T001'", declarantSG3.CTA.ToString(new UNOASGCharacterSet()));
...
UNIVERSAL PRACTICE AND IS NOT DEPENDENT ON THE TYPE OF BUSINESS OR INDUSTRY.THIS MESSAGE INCORPORATES THE NECESSARY TRADE, TRANSPORT, STATISTICAL AND APPLICANT/DECLARANT INFORMATION ON THE APPLICATION.";
		...
		public void TestExporterName()
		{
			...
			TCODECMessage msg = MessageBuilder.EdifactMessage;
			AssertEquals("NAD+EX+200102977K++HYDRO ALUMINIUM MALAYSIA SDN BHD C/:O OIA GLOBAL LOGISTICS (S) PTE LTD'", msg.Group3[2].NAD.ToString(new UNOASGCharacterSet()));
		}
		...
		public void TestAllSegmentsGenerated()
		{
			...
			TCODECMessage msg = MessageBuilder.EdifactMessage;
			string messageTextResult = msg.ToString(new UNOASGCharacterSet());
			Assert("Message Header", messageTextResult.Contains("UNH+1+TCODEC:0:1:RT:040+COODEC'"));
			...
			Assert("Transport Segment", messageTextResult.Contains("TDT+20++1+++++18W:::PACIFIC STAR'"));
			...
			Assert("Declaring Agent", messageTextResult.Contains("NAD+AE+AAA374M++EAGLE DATAMATION INTERNATIONAL'"));
			Assert("Declarant Segments", messageTextResult.Contains("NAD+DT++CHOW LEW WEE'CTA+IC+:V13T001'COM+?+64 9760 2847:TE'"));
			Assert("Consignee", messageTextResult.Contains("NAD+CN+++JOHN SMITH+"));
			Assert("Exporter", messageTextResult.Contains("NAD+EX+12093857483R++SINGAPORE EXPORTERS PTE LTD+"));
			Assert("Carrier Agent", messageTextResult.Contains("NAD+CA+16003857483R++SINGAPORE PACIFIC LINE'"));
		...
		public void TestWith2Certificates()
		{
			...
			Assert("Declaring Agent", messageTextResult.Contains("NAD+AE+AAA374M++EAGLE DATAMATION INTERNATIONAL'"));
			Assert("Declarant Segments", messageTextResult.Contains("NAD+DT++CHOW LEW WEE'CTA+IC+:V13T001'COM+?+64 9760 2847:TE'"));
			Assert("Consignee", messageTextResult.Contains("NAD+CN+++JOHN SMITH+1556 SMITH ST LOS ANGELES'"));
			Assert("Exporter", messageTextResult.Contains("NAD+EX+12093857483R++SINGAPORE EXPORTERS PTE LTD+GWANG ZO BUILDING C'"));
			Assert("Carrier Agent", messageTextResult.Contains("NAD+CA+16003857483R++SINGAPORE PACIFIC LINE'"));
			Assert("Detail seperator", messageTextResult.Contains("UNS+D'"));
		...
		TCODEC MessageBuilder
		{
			get
			{
				if (fMessageBuilder == null)
				{
					fMessageBuilder = new TCODEC(CofODataProvider);
				}
