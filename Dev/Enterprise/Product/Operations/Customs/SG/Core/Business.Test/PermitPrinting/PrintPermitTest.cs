...
{
	[TestedType(typeof(PrintPermit))]
	public class PrintPermitTest : NonPersistentBusinessObjectTestCase
	{
		protected override BusinessObject GetNewBusinessObject()
		{
			...
			declaration.DoMerge();
			ediMessage = declaration.ActiveEntryHeaders[0].Messages.AddNew();
			...
			ediMessage.EM_MessageText = strSample;
			ediMessage.EM_MessageType = Cuspmt09bMessageProcessor.MessageType;
			ediMessage.EM_SystemCreateTimeUtc = ZDateTime.Now;
			CUSPMT cuspmt = new CUSPMT();
			cuspmt.Parse(new UNOASGCharacterSet(), ediMessage.EM_MessageText);
			return new PrintPermit(cuspmt, Factory, ediMessage.EM_LinkUniqueID);
		}
		...
		public void TestIVisualizerNoteSupporterMembers()
		{
			...
			if (supporter != null)
			{
				AssertEquals("supporter.PK", ediMessage.EM_LinkUniqueID, supporter.PK);
