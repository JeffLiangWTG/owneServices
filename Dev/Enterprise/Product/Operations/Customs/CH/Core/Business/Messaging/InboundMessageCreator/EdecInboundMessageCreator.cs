...
public class EdecInboundMessageCreator : BaseInboundMessageCreator
{
	public EdecInboundMessageCreator(LoggingInformation logger) : base(logger)
	{
	}
	...
	protected override IEnumerable<ResponseParsingResult> GetResponseParsingResults(EDIInterchange interchange)
	{
		...
		{
			return (interchange.EI_From == MessagingConstants.CustomsDestinationCodes.CustomsEdecEmail
				? MIMETypeXTParser.ParseTextMail(reader)
				: MIMETypeXTParser.ParseTextHttp(reader)).ToArray();
		}
	}

	protected override ZString GetMessageSubTypeFromResponse(string responseText) => MessageSchemaDecider.GetSpecificMessageAnalyzer<IGoodsDeclarationsResponseAnalyzer>(responseText)?.GetMessageSubType();
