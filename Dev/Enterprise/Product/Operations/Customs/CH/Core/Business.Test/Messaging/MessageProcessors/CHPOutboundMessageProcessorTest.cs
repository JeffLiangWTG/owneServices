...
class CHPOutboundMessageProcessorTest : TestCaseWithFactory
{
	public void TestGetMessageFilterQuery() => CombineAssertions(() =>
	{
		...
		var processor = new CHPOutboundMessageProcessor(logger);
		...
		message2.Reload();
		AssertNotEquals($"EM_ApplicationCode={message2.EM_ApplicationCode}", EDIMessageStatusList.Codes.Queued, message2.EM_Status);
		message3.Reload();
