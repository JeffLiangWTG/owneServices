...
{
	[TestedType(typeof(ED815HeaderProvider))]
	class ED815HeaderProviderTest : HeaderProviderAbstractTest<ED815HeaderProvider>
	{
		...
		public void TestInvoiceNumber()
		{
			CombineAssertions(() =>
			{
				AssertEquals("No ConsolidatedDocument: empty InvoiceNumber", string.Empty, HeaderProvider.InvoiceNumber);

				emcsDeclaration.InvoiceNumber = "INV1234";
				AssertEquals("No ConsolidatedDocument: InvoiceNumber not empty", "INV1234", HeaderProvider.InvoiceNumber);

				emcsDeclaration.SetConsolidatedDocument();
				AssertEquals("Is ConsolidatedDocument: InvoiceNumber not empty", "INV1234", HeaderProvider.InvoiceNumber);

				emcsDeclaration.InvoiceNumber = ZString.Empty;
				AssertEquals("Is ConsolidatedDocument: empty InvoiceNumber", Constants.ConsolidatedDocumentDefaultString, HeaderProvider.InvoiceNumber);
			});
		}
		...
		public void TestTransportDetails()
		{
			CombineAssertions(() =>
			{
				...
				AssertType<ED815TransportProvider>("Is ED815TransportProvider", transportDetails.First());
			});
		}
		...
		{
			AssertEquals(EmcsDeferredSubmissionList.Codes.Nein, HeaderProvider.DeferredSubmissionFlag);
			emcsDeclaration.ZG_DeferredSubmission = EmcsDeferredSubmissionList.Codes.Ja;
			AssertEquals(EmcsDeferredSubmissionList.Codes.Ja, HeaderProvider.DeferredSubmissionFlag);
		}
		...
		public void TestLocalReferenceNumber()
		{
			...
			AssertEquals("B123456", HeaderProvider.LocalReferenceNumber);
		}
		...
		protected override ED815HeaderProvider GetHeaderProvider() => new ED815HeaderProvider(emcsDeclaration);

		protected override ED815HeaderProvider GetProvider()
		{
			emcsDeclaration.ZG_GuarantorType = EMCSGuarantorTypeList.Codes.Transporter;
			//TransporterDocumentaryAddress is used for GuarantorTrader and FirstTransporterTrader setup for caching
			...
			emcsDeclaration.CarrierAgentDocumentaryAddress.OrganisationPK = GetPartyVatNumberOrg("VAT438").PK;
			emcsDeclaration.SpecialInstructions = "OTHER DESCRIPTION";
			return new ED815HeaderProvider(emcsDeclaration);
		}

		protected override IEnumerable<Expression<Func<ED815HeaderProvider, object>>> GetPropertiesNeedToBeCached()
		{
		...
		protected new IED815Header HeaderProvider => base.HeaderProvider;
