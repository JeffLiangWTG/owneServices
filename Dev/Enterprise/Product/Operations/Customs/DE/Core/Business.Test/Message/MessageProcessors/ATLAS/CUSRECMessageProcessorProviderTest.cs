...
{
	class CUSRECMessageProcessorProviderTest : TestCaseWithFactory
	{
		public void TestGetProcessor_TemporaryStorage()
		{
			CombineAssertions(() =>
			{
				foreach (ICodeDescription pair in new TemporaryStorageMessageSubTypeList())
				{
					...
					AssertType<TemporaryStorageCUSRECMessageProcessor>(messageSubType, processor);
				}
			});
		}
		...
		public void TestGetProcessor_MonthlyClosing()
		{
			CombineAssertions(() =>
			{
				foreach (ICodeDescription pair in new MonthlyClosingMessageSubTypeList())
				{
					...
					message.EM_MessageSubType = messageSubType;
					var processor = provider.GetProcessor(message);
					AssertType<MonthlyClosingCUSRECMessageProcessor>(messageSubType, processor);
				}
			});
		}
		...
		public void TestGetProcessor_Import()
		{
			CombineAssertions(() =>
			{
				foreach (ICodeDescription pair in new ImportMessageSubTypeList())
				{
					...
					message.EM_MessageSubType = messageSubType;
					var processor = provider.GetProcessor(message);
					AssertType<ImportCUSRECMessageProcessor>(messageSubType, processor);
				}
			});
		}
		...
		protected override void SetUp()
		{
			...
			provider = new CUSRECMessageProcessorProvider(logger);
			message = Factory.New<AtlasInboundEDIMessage<ICUSREC>>();
		}
		CUSRECMessageProcessorProvider provider;
