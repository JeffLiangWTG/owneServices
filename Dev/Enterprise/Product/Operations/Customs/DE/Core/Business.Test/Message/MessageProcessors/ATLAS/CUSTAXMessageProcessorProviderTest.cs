...
{
	class CUSTAXMessageProcessorProviderTest : TestCaseWithFactory
	{
		public void TestGetProcessor_MonthlyClosing()
		{
			CombineAssertions(() =>
			{
				foreach (ICodeDescription pair in new MonthlyClosingMessageSubTypeList())
				{
					...
					AssertType<MonthlyClosingCUSTAXMessageProcessor>(messageSubType, processor);
				}
			});
		}
		...
		public void TestGetProcessor_Import()
		{
			CombineAssertions(() =>
			{
				foreach (ICodeDescription pair in new ImportMessageSubTypeList())
				{
					...
					message.EM_MessageSubType = messageSubType;
					var processor = provider.GetProcessor(message);
					AssertType<ImportCUSTAXMessageProcessor>(messageSubType, processor);
				}
			});
		}
		...
		protected override void SetUp()
		{
			...
			provider = new CUSTAXMessageProcessorProvider(logger);
			message = Factory.New<AtlasInboundEDIMessage<ICUSTAX>>();
		}
		CUSTAXMessageProcessorProvider provider;
