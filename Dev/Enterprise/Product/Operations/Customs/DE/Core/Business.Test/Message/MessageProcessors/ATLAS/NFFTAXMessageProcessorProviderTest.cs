...
{
	class NFFTAXMessageProcessorProviderTest : TestCaseWithFactory
	{
		public void TestGetProcessor_MonthlyClosing()
		{
			CombineAssertions(() =>
			{
				foreach (ICodeDescription pair in new MonthlyClosingMessageSubTypeList())
				{
					...
					AssertType<MonthlyClosingNFFTAXMessageProcessor>(messageSubType, processor);
				}
			});
		}
		...
		public void TestGetProcessor_Import()
		{
			CombineAssertions(() =>
			{
				foreach (ICodeDescription pair in new ImportMessageSubTypeList())
				{
					...
					message.EM_MessageSubType = messageSubType;
					var processor = provider.GetProcessor(message);
					AssertType<ImportNFFTAXMessageProcessor>(messageSubType, processor);
				}
			});
		}
		...
		protected override void SetUp()
		{
			...
			provider = new NFFTAXMessageProcessorProvider(logger);
			message = Factory.New<AtlasInboundEDIMessage<INFFTAX>>();
		}
		NFFTAXMessageProcessorProvider provider;
