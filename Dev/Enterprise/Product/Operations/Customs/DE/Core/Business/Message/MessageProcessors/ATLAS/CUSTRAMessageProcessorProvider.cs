...
{
	public class CUSTRAMessageProcessorProvider : IDEBranchCustomsApplicationTypeMessageProcessorProvider
	{
		public CUSTRAMessageProcessorProvider(LoggingInformation logger)
		{
		...
		public BranchCustomsApplicationTypeMessageProcessor GetProcessor(EDIMessage message)
		{
			...
			{
				result = new MonthlyClosingCUSTRAMessageProcessor(logger);
			}
			else if (factory.GetCachedValue<ImportMessageSubTypeList>().ContainsCode(messageSubType))
			{
				result = new ImportCUSTRAMessageProcessor(logger);
			}
