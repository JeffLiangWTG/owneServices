...
{
	public class CUSTAXMessageProcessorProvider : IDEBranchCustomsApplicationTypeMessageProcessorProvider
	{
		public CUSTAXMessageProcessorProvider(LoggingInformation logger)
		{
		...
		public BranchCustomsApplicationTypeMessageProcessor GetProcessor(EDIMessage message)
		{
			...
			{
				result = new MonthlyClosingCUSTAXMessageProcessor(logger);
			}
			else if (factory.GetCachedValue<ImportMessageSubTypeList>().ContainsCode(messageSubType))
			{
				result = new ImportCUSTAXMessageProcessor(logger);
			}
