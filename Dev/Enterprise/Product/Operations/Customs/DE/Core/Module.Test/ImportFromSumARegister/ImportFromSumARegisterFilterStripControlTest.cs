...
{
	class ImportFromSumARegisterFilterStripControlTest : TestCaseWithFactory
	{
		public void TestAvailableGridColumns()
		{
			using (var userControl = new ImportFromSumARegisterFilterStripControl(new ActiveBusinessObjectCollection<CusTempStorageRegLine>(Factory), new ImportFromSumARegisterFilterStripBusinessObject()))
			{
				var expectedColumns = new[] { SRL_SystemCreateUser, SRL_SystemCreateTimeUtc, SRL_SystemLastEditUser, SRL_SystemLastEditTimeUtc, ReferenceColumnName, InternalReferenceColumnName, SRL_LineNumber, SRL_GoodsDescription,
					SRL_OwnerReferenceType, SRL_OwnerReference, SRL_PackagesRemaining, SRL_PackageType };
		...
		public void TestColumn_ReferenceColumnName()
		{
			using (var form = new ZForm())
			{
				...
				AssertEquals("Caption", "Registration Number", filteredGrid.GetColumnCaption(ReferenceColumnName));
			}
		}
		...
		public void TestColumn_InternalReferenceColumnName()
		{
			using (var form = new ZForm())
			{
				...
				AssertEquals("Caption", "Customer Reference", filteredGrid.GetColumnCaption(InternalReferenceColumnName));
			}
		}
		...
		{
			base.SetUp();
			userControl = new ImportFromSumARegisterFilterStripControl(new ActiveBusinessObjectCollection<CusTempStorageRegLine>(Factory), new ImportFromSumARegisterFilterStripBusinessObject());
			filteredGrid = userControl.FilteredGrid;
		}
		ImportFromSumARegisterFilterStripControl userControl;
		ZGrid filteredGrid;
