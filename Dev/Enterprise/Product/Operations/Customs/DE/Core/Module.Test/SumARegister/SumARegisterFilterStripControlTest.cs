...
namespace Enterprise.Customs.DE.Module.Testing
{
	sealed class SumARegisterFilterStripControlTest : TestCaseWithFactory
	{
		...
		{
			new ColumnDetails(CusTempStorageRegHeader.Schema.SRH_Reference, "Registration Number", 130),
			...
			new ColumnDetails(CusTempStorageRegHeader.Schema.SRH_InternalReference, "Customer Reference", 130)
		};
		...
		ColumnDetails[] OrderedLinesGridColumnDetails => new[]
		{
			...
			new ColumnDetails(CusTempStorageRegLine.Schema.SRL_GoodsOwnerIdentifier, "Disp. Ent. Trader EORI", 140),
			new ColumnDetails(CusTempStorageRegLine.Schema.SRL_PackagesRemaining, "Package Count", 80),
			new ColumnDetails(CusTempStorageRegLine.Schema.SRL_CustomsStatus, "Line Status", 80)
		};
