...
namespace Enterprise.Customs.DE.ExitControl.GUI
{
	public class ExitControlLayoutProvider : IExitControlLayoutProvider
	{
		...
		IEnumerable<ITabPage> IExitControlLayoutProvider.AdditionalConsignmentTabPages
		{
			get
			{
				yield return new ConsignmentAdditionalInformationTabPage();
			}
		}
		...
		IEnumerable<ITabPageWithVisibility<EU.ExitControl.Business.CusExitReport>> IExitControlLayoutProvider.AdditionalReportTabPages => Enumerable.Empty<ITabPageWithVisibility<EU.ExitControl.Business.CusExitReport>>();
