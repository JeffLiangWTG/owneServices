...
sealed class ImportCUSDECMessageSegmentBuilderTest : TestCaseWithFactory
{
	...
	public void TestPopulateCUSDECMessageBGMMessageType()
	{
		...
		void AssertGeneratedMessageBodyMessageType(string messageType, string expectedValue) => CombineAssertions($"Message type: {messageType}", () =>
		{
			...
			expectedMessageLines[index] = $"BGM+929++137:20240101:102+{expectedValue}'";
			AssertGeneratedMessageBody(testInput);
		});
	}
	...
	public void TestPopulateCUSDECMessage_WithEmptyOrInvalidMessageTypeInDataProvider() => CombineAssertions(() =>
	{
		...
		void SetMessageTypeAndAssert(ZString messageType)
		{
			...
			Assert($"When MessageType is '{messageType}', MessageFunctionCoded value is empty and is not included in the segment", generatedMessage.Contains("BGM+929++137:20240101:102'"));
		}
	});
	...
	public void TestPopulateCUSDECMessageMOAAdjustedValueIsDiscount()
	{
		const int index = 33;
		...
		AssertGeneratedMessageBodyAdjustedValueIsDiscount(true, "21");
		AssertGeneratedMessageBodyAdjustedValueIsDiscount(false, "160");
		return;

		void AssertGeneratedMessageBodyAdjustedValueIsDiscount(bool isDiscount, string expectedAmountType) => CombineAssertions($"Adjusted value is discount: {isDiscount}", () =>
		{
			itemDetailsMock.SetupGet(x => x.AdjustedValueIsDiscount).Returns(isDiscount);
			expectedMessageLines[index] = $"MOA+14+123:16188+{expectedAmountType}:1188+40:107546'";
			AssertGeneratedMessageBody(testInput);
		});
	}
	...
	public void TestPopulateCUSDECMessageMOAStatisticValueIsMandatory()
	{
		...
		expectedMessageLines[index] = "MOA+14+123:0+160:1188+40:107546'";
		AssertGeneratedMessageBody(testInput);
	}
	...
	static (TestMocks mocks, ICUSDECMessageDataProvider testInput) GetTestInput()
	{
		...
		testInputMock.SetupGet(x => x.AssociationAssignedCode).Returns("NEP-I");
		testInputMock.SetupGet(x => x.LocalReferenceNumber).Returns("NO1234");
		// BGM
		testInputMock.SetupGet(x => x.DocumentMessageName).Returns(DocumentMessageNameCodedList.GetFromString("929"));
		...
		testInputMock.SetupGet(x => x.SenderAddressType).Returns(PartyQualifierList.GetFromString("SE"));
		...
		itemDetailsMock.SetupGet(x => x.ReducedCustomsFlag).Returns("S");
		// PAC
		...
		itemDetailsMock.SetupGet(x => x.ChassisNumber).Returns("ME3DJELT5NV007653");
		itemDetailsMock.SetupGet(x => x.ContainerNumbers).Returns(new[] { (ZString)"PVDU1130919", (ZString)"PVDU1130666" });
		// LOC
		itemDetailsMock.SetupGet(x => x.CountryOfOrigin).Returns("FR");
		// MOA
		itemDetailsMock.SetupGet(x => x.AdjustedValue).Returns("1188");
		itemDetailsMock.SetupGet(x => x.AdjustedValueIsDiscount).Returns(false);
		itemDetailsMock.SetupGet(x => x.StatisticValue).Returns("16188");
		itemDetailsMock.SetupGet(x => x.AddedValueDueToProcessingAbroad).Returns("107546");
		// MEA
	...
	[
		"UNH+01011498465KLL+CUSDEC:1:902:UN:NEP-I+NO1234'",
		"BGM+929++137:20240101:102+9'",
		...
		"NAD+SE++NEXANS NETWORK SOLUTIONS:LOGISTICKA 173::35002 CHEB 2'",
		...
		"PCI+++AAQ:PVDU1130666'",
		"LOC+27:FR'",
		"MOA+14+123:16188+160:1188+40:107546'",
		"MEA+AAF+G+KGM:110'",
		"MEA+AAF+N+KGM:72'",
		"MEA+AAF+ZZ+STK:10'",
