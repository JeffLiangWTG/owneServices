...
[TestedType(typeof(CusTempStorageRegLineTransactionValidation))]
sealed class CusTempStorageRegLineTransactionValidationTest : BusinessObjectValidationTestCase
{
	public void TestCheckSRT_GrossWeight_TransactionTypeOBL()
	{
		...
		CombineAssertions(() =>
		{
			AssertEquals("TransactionType", CusTempStorageRegLineTransactionTypeList.Codes.OpeningBalance, transaction.SRT_TransactionType);
	...
	public void TestCheckSRT_GrossWeight_TransactionTypeADJ()
	{
		...
		CombineAssertions(() =>
		{
			AssertEquals("TransactionType", CusTempStorageRegLineTransactionTypeList.Codes.Adjustment, transaction2.SRT_TransactionType);
	...
	public void TestCheckSRT_PackageQty()
	{
		...
		transaction1.SRT_TransactionType = CusTempStorageRegLineTransactionTypeList.Codes.Adjustment;
		...
		var transaction2 = line.CusTempStorageRegLineTransactions.AddNew();
		transaction2.SRT_TransactionType = CusTempStorageRegLineTransactionTypeList.Codes.Adjustment;
	...
	public void TestCheckSRT_BondAmount() => CombineAssertions(() =>
	{
		var transaction = Factory.New<CusTempStorageRegLineTransaction>();
		const string noValueErrorMessage = "You have not entered a Goods Value in NOKs.";
		transaction.Validation.ValidateSRT_BondAmount();
		AssertHasMessageErrorContaining(transaction.SRT_BondAmountInfo, noValueErrorMessage);

		transaction.SRT_BondAmount = 100;
		AssertNoMessageErrorContaining(transaction.SRT_BondAmountInfo, noValueErrorMessage);
	});
}
