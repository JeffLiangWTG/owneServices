...
{
	sealed class LicensingCertificateUserControlTest : TestCaseWithFactory
	{
		public void TestCertificateUpload()
		{
			(GlbCompany company, TWGlbCompanyWrapper companyWrapper, GlbCompanyCredential password) = SetUpData();
			password.GP_MailBoxID = "1";
			password.GP_UserID = "1";
			password.CurrentDecryptedCertificatePassphrase = validCertPassword;
			Factory.Save();

			using (var form = new ZForm(companyWrapper))
			{
				var brokerageControl = new CompanyCredentialsUserControl();
				form.Controls.Add(brokerageControl);
				form.Show();

				var licensingCertificateUserControl = brokerageControl.FindSingle<LicensingCertificateUserControl>("LicensingCertificateUserControl");
				var certLoader = licensingCertificateUserControl.FindSingle<Registry.GUI.DigitalCertificateControl_p12>("CertificateLoaderUserControl");
				certLoader.SetFileData(validCert);

				licensingCertificateUserControl.CertificateLoaderUserControl_DataChanged(null, null);
				AssertEquals("Certificate is copied from loader", validCert, password.GP_Certificate);
			}
		}
		...
		public void TestCertificatePassword()
		{
			...
			using (var form = new ZForm(companyWrapper))
			{
				...
				form.Show();
				var licensingCertificateUserControl = brokerageControl.FindSingle<LicensingCertificateUserControl>("LicensingCertificateUserControl");
				AssertEquals("Certificate password", validCertPassword, licensingCertificateUserControl.CertificateLoaderUserControl.CertificatePassword);
			}
		}
		...
		(GlbCompany company, TWGlbCompanyWrapper companyWrapper, GlbCompanyCredential password) SetUpData()
		{
			...
			var companyWrapper = TWGlbCompanyWrapper.GetWrapper<TWGlbCompanyWrapper>(company);

			var password = companyWrapper.LicensingCertificate;
			password.GP_GC = company.PK;
			Factory.Save();
			return (company, companyWrapper, password);
		}
	}
}
