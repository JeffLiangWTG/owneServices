...
{
	sealed class EntryNumberGeneratorCatATest : TestCaseWithFactory
	{
		[ExpectNoExceptions]
		public void TestEntryNumberShouldBeGeneratedCategoryA()
		{
			entryInstruction.CEI_Style = Constants.DeclarationTypes.Import.G1;
			entryInstruction.CEI_DateForDuty = ZDateTime.Empty;
			entryNumberGeneratorForTest = new EntryNumberGeneratorCatAForTest(entryHeader);
		...
		[ExpectNoExceptions]
		public void TestEntryNumberCategoryA()
		{
			entryInstruction.CEI_Style = Constants.DeclarationTypes.Import.G1;
			entryNumberGeneratorForTest = new EntryNumberGeneratorCatAForTest(entryHeader);
			entryInstruction.CEI_CustomsOffice = "AA";
		...
		public void TestKeyFormatting()
		{
			...
			CombineAssertions(() =>
			{
				...
				NUnit.Framework.Assert.That(entryNumberGeneratorForTest.FormatNumberToKeyExposed(209988), NUnit.Framework.Is.EqualTo("K9999"));
				NUnit.Framework.Assert.That(entryNumberGeneratorForTest.FormatNumberToKeyExposed(2971890), NUnit.Framework.Is.EqualTo("BGIA9"));
			});
		}
		...
		{
			var entryNumberGeneratorForTest = new EntryNumberGeneratorCatA(entryHeader);
...
Box Number:You have not entered a Box Number.").Using(CustomComparers.TypeComparison));
		...
		EntryNumberGeneratorCatAForTest entryNumberGeneratorForTest;
