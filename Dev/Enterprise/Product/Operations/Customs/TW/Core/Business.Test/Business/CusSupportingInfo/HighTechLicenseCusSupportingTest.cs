...
{
	[TestedType(typeof(HighTechLicenseCusSupporting))]
	sealed class HighTechLicenseCusSupportingTest : SingleCusSupportingInfoTest<HighTechLicenseCusSupporting>
	{
		[ExpectNoExceptions]
		public void TestSetDefaultValues()
		{
			var supporting = GetBizObjsForCorrectlyTypeDecideTest(Factory).FirstOrDefault();
			NUnit.Framework.Assert.That(supporting.CSI_Type, NUnit.Framework.Is.EqualTo(CusSupportingInfoTypeList.Codes.ShtcImportPermit).Using(CustomComparers.TypeComparison));
		...
		protected override IEnumerable<HighTechLicenseCusSupporting> GetBizObjsForCorrectlyTypeDecideTest(BusinessObjectFactory factory)
		{
			var cusSupportingInfo = factory.NewWithValidTestData<JobDeclaration>().Invoices.AddNew().JobComInvoiceLines.AddNew().HighTechLicenseSupporting;
