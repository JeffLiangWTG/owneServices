...
using CargoWise.Customs.TW.MessageDefinitions.NX302;
using CargoWise.Types;
using Enterprise.Messaging.MessageProcessors;

namespace Enterprise.Customs.TW.Business.MessageProcessors
{
	public class NX302MessageHelper : TWMessageHelper
	{
		public NX302MessageHelper(TWMessage message) : base(message)
		{
			...
			if (response != null)
			{
				...
				if (governmentAgencyGoodsItem != null)
				{
					foreach (var item in governmentAgencyGoodsItem)
					{
						...
						item.Error.ForEach(e => WriteRow(table, Titles.ErrorConditionOverrideCoded, GetCPT_118_302_CodeofErrorConditionOverrideCodeList(e.ValidationCode?.Value ?? ZString.Empty)));
					}
				}
		...
		ZString GetCPT_118_302_CodeofErrorConditionOverrideCodeList(ZString code)
		{
			return GetDescriptionFromCode(code, (itemCode) => Lookups.CPT_118_302_CodeofErrorConditionOverrideCodeList.GetDescriptionFromCode(itemCode));
		}
