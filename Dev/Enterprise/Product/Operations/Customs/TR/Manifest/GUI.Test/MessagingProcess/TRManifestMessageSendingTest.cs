...
namespace Enterprise.Customs.TR.Manifest.GUI.MessagingProcess.Testing
{
	sealed class TRManifestMessageSendingTest : TestCaseWithFactory
	{
...
A valid certificate serial number is required on your staff record on the Brokerage tab", result.Notifications.CreateSummaryNotification(true).Message);
		...
		public void TestSendMessageAwaitingResponse()
		{
			SetupCurrentUser(true);
			var header = SetupHeader();
			...
			{
				AssertEquals("Sending Succeeded", false, result.Success);
				AssertContains("Check last notification", "The status is wait for response message, if you send again, the message will be rejected.", UnitTestUserNotification.Instance.LastMessage.Text);
				AssertEquals("Message Count", 0, result.EDIMessages?.Count ?? 0);
			});
		}
		...
		ActionResult SendMessage(AsycudaManifestHeader header, System.Windows.Forms.DialogResult userAction)
		{
			using (var mainForm = new ZForm(header))
			{
				...
				var providerFactory = new TRCustomsMessagingProviderFactory(TRManifestCustomsMessagingProvider.New, TRMessageTypes.Codes.TRO, TRMessageSignerForTest.New());
