...
{
	sealed class NctsCustomsMessagingProviderTest : TestCaseWithFactory
	{
		public void TestNew()
		{
			var header = Factory.New<NctsHeader>();

			var provider = NctsCustomsMessagingProvider.New(header, string.Empty, null);
			AssertType<NctsCustomsMessagingProvider>(provider);
		}
		...
		public void TestGetMessengers()
		{
			...
			AssertType<NctsMessageGenerator>(generator);
		}

		public void TestITRCustomsMessagingProvider()
		{
			var header = Factory.New<NctsHeader>();
			var trProvider = NctsCustomsMessagingProvider.New(header, string.Empty, null) as ITRCustomsMessagingProvider;
		...
		{
			var header = Factory.New<NctsHeader>();
			var provider = NctsCustomsMessagingProvider.New(header, string.Empty, null) as ISupportPreSendValidation;
