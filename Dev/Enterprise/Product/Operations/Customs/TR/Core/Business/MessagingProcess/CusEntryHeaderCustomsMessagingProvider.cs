...
{
	public sealed class CusEntryHeaderCustomsMessagingProvider : ITRCustomsMessagingProvider, ICustomsMessagingProvider, ISupportPreSendValidation
	{
		public static ICustomsMessagingProvider New(BusinessObject header, ZString messageType, TRMessageSigner signer) => new CusEntryHeaderCustomsMessagingProvider((CusEntryHeader)header, messageType, signer);

		CusEntryHeaderCustomsMessagingProvider(CusEntryHeader cusEntryHeader, ZString messageType, TRMessageSigner signer = null)
		{
			var messengers = CreateMessengers(cusEntryHeader, messageType, signer);
		...
		static IReadOnlyCollection<ITRCustomsMessenger> CreateMessengers(CusEntryHeader cusEntryHeader, ZString messageType, TRMessageSigner signer) => [CusEntryHeaderCustomsMessenger.New(cusEntryHeader, messageType, signer)];
