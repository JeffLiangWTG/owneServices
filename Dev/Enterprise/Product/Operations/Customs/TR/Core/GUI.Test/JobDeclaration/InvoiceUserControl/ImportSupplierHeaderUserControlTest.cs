	...
	class ImportSupplierHeaderUserControlTest : LayoutCustomsSupplierHeaderUserControlAbstractTest<ImportSupplierHeaderUserControl, JobDeclaration>
	{
		public void TestJZ_RelatedIndicatorLocationAndTabIndex()
		{
			using (var control = new ImportSupplierHeaderUserControl())
			{
				...
				AssertEquals("TabIndex of JZ_RelatedIndicator should be 13", 13, rlatedIndicator.TabIndex);
				AssertEquals("Location of JZ_RelatedIndicator should be (118, 299)", CargoWise.Windows.UI.ControlDpiScalingHelper.NewScaledPoint(118, 299, true), rlatedIndicator.Location);
			}
		}
		...
		public void TestReOrderTabPages()
		{
			...
			using (var control = new ImportSupplierHeaderUserControl())
			{
				...
				AssertArrayEqualsByElements(new[] { "ComInvoiceDetailsTabPage", "InvoicePaymentTabPage", "SupportingDocumentsTabPage", "AdditionalInfoTabPage", "PreviousDocumentsTabPage", "CustomFieldsTabPage" }, tabPages.Cast<ZTabPage>().Select(x => x.Name).ToArray());
			}
		}
		...
		class ImportSupplierHeaderUserControlForTest : ImportSupplierHeaderUserControl
		{
