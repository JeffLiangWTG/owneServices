...
{
	sealed class GOVCBR5DPMessageSenderTest : CusEntryHeaderOriginalMessageSenderTest<GOVCBR5DP5DQSender>
	{
		protected override IEnumerable<CusEntryHeader> GetMessageParents() => EntriesToSend;

		protected override MessageSender GetMessageSender() => IsExceptionTest ? new GOVCBR5DPSenderForTest(EntriesToSend, ElectronicDocumentTypeList.Codes._5DP, Factory) : new GOVCBR5DP5DQSender(EntriesToSend, ElectronicDocumentTypeList.Codes._5DP, Factory);
		...
		public void TestRoundDecimalPlaces()
		{
			...
			entry.MergedLines[0].InvoiceLines[0].JI_NetWeight = 1.1233m;
			entry.MergedLines[0].InvoiceLines[0].JI_NetWeightUQ = Core.Constants.Weight.Kilograms;
			entry.MergedLines[0].InvoiceLines[1].JI_NetWeight = 1.0005m;
			entry.MergedLines[0].InvoiceLines[1].JI_NetWeightUQ = Core.Constants.Weight.Kilograms;
			entry.MergedLines[0].InvoiceLines[0].JI_InvoiceQuantity = 0.8765m;
			entry.MergedLines[0].InvoiceLines[1].JI_InvoiceQuantity = 2m;
			entry.MergedLines[0].CL_CustomsValue = 1000.12345m;
			entry.MergedLines[1].CL_CustomsValue = 3000.12m;
			...
			{
				var result = KRXmlObjectSerializer.DeserializeWithoutSchemaValidation<CargoWise.Customs.KR.MessageDefinitions.GOVCBR5DP.Declaration>(textReader);
				...
				AssertEquals(1000m, result.Consignment.ConsignmentItem[0].Commodity.ValueAmount.Value);
				AssertEquals(4000.24m, result.InvoiceAmount.Value);
	...
	class GOVCBR5DPSenderForTest : GOVCBR5DP5DQSender
	{
		public GOVCBR5DPSenderForTest(IEnumerable<CusEntryHeader> entries, string messageType, BusinessObjectFactory factory)
