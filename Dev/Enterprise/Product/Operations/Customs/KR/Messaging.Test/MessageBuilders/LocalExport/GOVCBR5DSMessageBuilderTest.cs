...
{
	sealed class GOVCBR5DSMessageBuilderTest : TestCaseWithFactory
	{
		...
		public void TestGenerateDeclaration()
		{
			...
			var result = new GOVCBR5DSMessageBuilder(localExportHeaderMock.Object, localExportAmendMock.Object, MessageFunctions.MessageFunctionCode.Amendment).GenerateMessage();

			var testFile = File.OpenRead(Path.Combine(MessageBuilderTestHelper.LocalExportOutgoingTestFilePath, "GOVCBR5DS_0.xml"));
			...
			AssertEquals("GOVCBR5DS", result.TypeCode.Value);
			AssertEquals("1", result.TransactionNatureCode.Value);
		...
		public void Testwithdraw()
		{
			...
			var result = new GOVCBR5DSMessageBuilder(localExportHeaderMock.Object, localExportAmendMock.Object, MessageFunctions.MessageFunctionCode.Cancellation).GenerateMessage();

			var testFile = File.OpenRead(Path.Combine(MessageBuilderTestHelper.LocalExportOutgoingTestFilePath, "GOVCBR5DS_1.xml"));
			using (var stream = new CargoWise.IO.Shim.SubStreamableStream(testFile))
			{
				using (var makeStream = KRXmlObjectSerializer.Serialize(result))
				{
		...
		public void TestEmptyData()
		{
			...
			var testFile = File.OpenRead(Path.Combine(MessageBuilderTestHelper.LocalExportOutgoingTestFilePath, "GOVCBR5DS_2.xml"));
			using (var stream = new CargoWise.IO.Shim.SubStreamableStream(testFile))
			{
				using (var makeStream = KRXmlObjectSerializer.Serialize(result))
				{
