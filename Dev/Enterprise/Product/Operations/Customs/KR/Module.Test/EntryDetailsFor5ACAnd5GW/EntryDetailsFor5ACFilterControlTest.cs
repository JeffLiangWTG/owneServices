...
{
	sealed class EntryDetailsFor5ACFilterControlTest : ZFilterStripControlTest
	{
		public void Test5ACFilterControl()
		{
			var filterObject = new EntryDetailsFor5ACFilterStripBusinessObject();
			...
			using (var control = new EntryDetailsFor5ACFilterControl(gridCollection, filterObject))
			{
				...
				AssertEquals(control.Grid.Columns[index++].ColumnName, KREntryHeaderDetailsView.Schema.KEH_ExportPackQty);
				AssertEquals(control.Grid.Columns[index++].ColumnName, KREntryHeaderDetailsView.Schema.KEH_TotalWeightInKG);
				AssertEquals(control.Grid.Columns[index++].ColumnName, nameof(KREntryHeaderDetailsView.Supplier));
				AssertEquals(control.Grid.Columns[index++].ColumnName, KREntryHeaderDetailsView.Schema.KEH_SupplierName);
