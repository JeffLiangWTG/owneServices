...
namespace Enterprise.Customs.KR.Business
{
	...
	[MessageType(ElectronicDocumentTypeList.Codes._RR7)]
	public class GOVCBRRR7Processor : IMessageProcessor
	{
		public void Process(EDIMessage message)
		{
			using (var textReader = message.GetEM_MessageTextReader())
			{
				var messageData = new GOVCBRRR7DataProvider().GetMessageData(textReader);
				var entry = MessageLinkedObjectManager.GetLinkedObject(message.Factory, message.Company, messageData.ImportDeclarationNumber, ElectronicDocumentTypeList.Codes._008);
				if (entry != null)
				{
				...
				message.EM_MessageSubType = ElectronicDocumentTypeList.Codes._008;
			}
		}

		#region SuppressResourceStringsCheckRegion
		void SendNotification(EDIMessage message, CusEntryHeader entry, IGOVCBRRR7MessageData messageData)
		{
			...
			var notificationData = new DeclarationEntryNotificationData()
			{
				...
				OriginalMessageTypes = new ZString[] { ElectronicDocumentTypeList.Codes._008 },
				MessageTypeDescription = $"[{ElectronicDocumentTypeList.Descriptions._RR7}]",
				EntryNumber = messageData.ImportDeclarationNumber
			};
			NotificationSender.SendNotification(notificationData);
		}

		string CreateUserFriendlyMessageInterpretation(EDIMessage message, IGOVCBRRR7MessageData messageData)
		{
			var result = new ZStringBuilder();

			var tableContents = new HtmlTableCreator(new string[] { "항 목", "내 용" });
			tableContents.WriteRow(new string[] { "제출문서", "이사물품신고서" });
			...
			tableContents.WriteRow(new string[] { "접수 세관(과)", "[" + messageData.CustomsOfficeAndDepartmentID + "] " + MessageFunctions.GetCustomsOfficeAndDivision(message.Factory, messageData.CustomsOfficeAndDepartmentID) });
