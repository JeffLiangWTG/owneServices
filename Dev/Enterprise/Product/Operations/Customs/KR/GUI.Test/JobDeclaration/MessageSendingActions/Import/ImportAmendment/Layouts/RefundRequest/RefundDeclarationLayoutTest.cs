...
{
	[TestedType(typeof(RefundDeclarationLayout))]
	sealed class RefundDeclarationLayoutTest : LayoutsAbstractTest
	{
		...
		{
			IPanelLayoutProvider layout = new RefundDeclarationLayout();
			...
			AssertEquals("RefundTypeDropEdit", row1.Parts[1].Name);

			AssertEquals(2, row2.Parts.Count);
			AssertEquals("RefundCauseDropEdit", row2.Parts[1].Name);

			AssertEquals(2, row3.Parts.Count);
			AssertEquals("RefundReasonDropEdit", row3.Parts[1].Name);

			AssertEquals(2, row4.Parts.Count);
			AssertEquals("RefundSentWith5FEDropEdit", row4.Parts[1].Name);

			AssertEquals(2, row5.Parts.Count);
			AssertEquals("TaxOfficeCodeFindBox", row5.Parts[1].Name);
		}
	}
}
