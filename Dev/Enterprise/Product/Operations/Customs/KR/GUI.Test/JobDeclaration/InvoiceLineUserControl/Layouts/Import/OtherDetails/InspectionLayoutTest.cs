...
{
	[TestedType(typeof(InspectionLayout))]
	sealed class InspectionLayoutTest : LayoutsAbstractTest
	{
		...
		{
			IPanelLayoutProvider layout = new InspectionLayout();
			...
			AssertEquals("InspectionDropEdit", row1.Parts[1].Name);

			AssertEquals(2, row2.Parts.Count);
			AssertEquals("DeliveryCompanyDropEdit", row2.Parts[1].Name);
		}
	}
}
