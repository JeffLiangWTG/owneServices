...
namespace Enterprise.Customs.KR.GUI.Testing
{
	...
	sealed class TransportDetailsLayoutTest : LayoutsAbstractTest
	{
		...
		{
			IPanelLayoutProvider layout = new TransportDetailsLayout();
			...
			AssertEquals(nameof(TransportDetailsControlBag.Instance.HouseBillTextBox), row1.Parts[1].Name);

			AssertEquals(2, row2.Parts.Count);
			AssertEquals(nameof(TransportDetailsControlBag.Instance.FreightCalcEdit), row2.Parts[1].Name);

			AssertEquals(4, row3.Parts.Count);
			AssertEquals(nameof(TransportDetailsControlBag.Instance.StartDateDateEdit), row3.Parts[1].Name);
			AssertEquals(nameof(TransportDetailsControlBag.Instance.ArrivalDateDateEdit), row3.Parts[3].Name);

			AssertEquals(2, row4.Parts.Count);
			AssertEquals(nameof(TransportDetailsControlBag.Instance.PortofLoadingCodeFindBox), row4.Parts[1].Name);

			AssertEquals(2, row5.Parts.Count);
			AssertEquals(nameof(TransportDetailsControlBag.Instance.ForeignCityCodeFindBox), row5.Parts[1].Name);
		}
	}
}
