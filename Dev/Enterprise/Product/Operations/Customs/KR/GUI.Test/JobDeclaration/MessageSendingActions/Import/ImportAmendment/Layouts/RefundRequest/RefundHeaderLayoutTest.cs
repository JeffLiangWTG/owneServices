...
{
	[TestedType(typeof(RefundHeaderLayout))]
	sealed class RefundHeaderLayoutTest : LayoutsAbstractTest
	{
		...
		{
			IPanelLayoutProvider layout = new RefundHeaderLayout();
			...
			AssertEquals("CustomsDisbursementBillTextBox", row1.Parts[1].Name);

			AssertEquals(2, row2.Parts.Count);
			AssertEquals("VersionNoCalcEdit", row2.Parts[1].Name);

			AssertEquals(2, row3.Parts.Count);
			AssertEquals("RefundAmountOfValueForVATCalcEdit", row3.Parts[1].Name);

			AssertEquals(2, row4.Parts.Count);
			AssertEquals("RefundAmountOfVATExemptionValueCalcEdit", row4.Parts[1].Name);
		}
	}
}
