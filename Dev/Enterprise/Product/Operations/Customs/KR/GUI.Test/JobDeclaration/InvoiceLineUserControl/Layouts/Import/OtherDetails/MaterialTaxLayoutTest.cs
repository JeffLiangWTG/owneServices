...
{
	[TestedType(typeof(MaterialTaxLayout))]
	sealed class MaterialTaxLayoutTest : LayoutsAbstractTest
	{
		...
		{
			IPanelLayoutProvider layout = new MaterialTaxLayout();
			...
			AssertEquals("ProductDropEdit", row1.Parts[1].Name);

			AssertEquals(2, row2.Parts.Count);
			AssertEquals("LineNoCalcEdit", row2.Parts[1].Name);
		}
	}
}
