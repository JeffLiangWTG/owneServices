...
{
	[TestedType(typeof(OrganizationLayout))]
	sealed class OrganizationLayoutTest : LayoutsAbstractTest
	{
		...
		{
			IPanelLayoutProvider layout = new OrganizationLayout();
			...
			AssertEquals(nameof(OrganizationControlBag.Instance.ForeignCarrierGuidFindBox), row1.Parts[1].Name);

			AssertEquals(2, row2.Parts.Count);
			AssertEquals(nameof(OrganizationControlBag.Instance.DomesticCarrierGuidFindBox), row2.Parts[1].Name);
		}
	}
}
