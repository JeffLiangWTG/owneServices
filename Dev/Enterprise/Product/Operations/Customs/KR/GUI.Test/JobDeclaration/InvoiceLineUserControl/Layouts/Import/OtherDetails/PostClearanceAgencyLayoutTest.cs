...
{
	[TestedType(typeof(PostClearanceAgencyLayout))]
	sealed class PostClearanceAgencyLayoutTest : LayoutsAbstractTest
	{
		...
		{
			IPanelLayoutProvider layout = new PostClearanceAgencyLayout();
			...
			AssertEquals("Agency1CodeFindBox", row1.Parts[1].Name);

			AssertEquals(2, row2.Parts.Count);
			AssertEquals("Agency2CodeFindBox", row2.Parts[1].Name);

			AssertEquals(2, row3.Parts.Count);
			AssertEquals("Agency3CodeFindBox", row3.Parts[1].Name);
		}
	}
}
