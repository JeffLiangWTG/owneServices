...
{
	[TestedType(typeof(PayerLayout))]
	sealed class PayerLayoutTest : LayoutsAbstractTest
	{
		...
		{
			IPanelLayoutProvider layout = new PayerLayout();
			...
			AssertEquals(nameof(RefundDeclarationControlBag.PayerAddressControl), row1.Parts[1].Name);

			AssertEquals(2, row2.Parts.Count);
			AssertEquals(nameof(RefundDeclarationControlBag.RegistrationNumberOneTextBox), row2.Parts[1].Name);

			AssertEquals(2, row3.Parts.Count);
			AssertEquals(nameof(RefundDeclarationControlBag.KoreanRegistrationNumberOfCEOTextBox), row3.Parts[1].Name);

			AssertEquals(2, row4.Parts.Count);
			AssertEquals(nameof(RefundDeclarationControlBag.PayerBankDropEdit), row4.Parts[1].Name);

			AssertEquals(2, row5.Parts.Count);
			AssertEquals(nameof(RefundDeclarationControlBag.BankAccountNumberTextBox), row5.Parts[1].Name);
		}
	}
}
