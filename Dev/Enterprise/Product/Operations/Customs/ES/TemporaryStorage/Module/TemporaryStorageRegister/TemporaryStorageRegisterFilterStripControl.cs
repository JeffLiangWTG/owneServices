...
public partial class TemporaryStorageRegisterFilterStripControl : ZFilterStripControl
{
	public TemporaryStorageRegisterFilterStripControl(IBusinessObjectCollection gridCollection, FilterStripBusinessObject filterBusinessObject)
	...
	protected override void UpdateFilteredGridRefreshWarning()
	{
		...
		void ClearCurrentLinesQuery()
		{
			...
			if (currentItem != null)
			{
				currentItem.CusTempStorageRegLines.AdditionalFilter = new ZDBOnlyQuery(typeof(EU.TemporaryStorage.Business.CusTempStorageRegLine));
			}
		}

	void OnListManagerOnCurrentChanged(object sender, EventArgs args)
	{
		var filter = FilterBusinessObject as TemporaryStorageRegisterFilterBusinessObject;
	...
	{
		var filterBizO = (TemporaryStorageRegisterFilterBusinessObject)FilterBusinessObject;
