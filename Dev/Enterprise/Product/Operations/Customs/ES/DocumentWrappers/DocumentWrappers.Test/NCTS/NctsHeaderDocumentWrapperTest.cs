	...
	sealed class NctsHeaderDocumentWrapperTest : NonPersistentBusinessObjectTestCase
	{
		...
		public void TestBOX5ITEMSFormat()
		{
			...
			CombineAssertions(() =>
			{
				...
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX5ITEMS) + " is 2", "2", wrapper.BOX5ITEMS);

				for (int i = 0; i <= 1000; i++)
				{
					moveHeader.GoodsItems.AddNew();
				}
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX5ITEMS) + " is 1003", "1.003", wrapper.BOX5ITEMS);
			});
		}
		...
		public void TestBOX6PACKAGESFormat()
		{
			...
			{
				var wrapper = NctsHeaderDocumentWrapper.New(header, Factory);
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX6PACKAGES) + " is 2", "2", wrapper.BOX6PACKAGES);

				for (int i = 0; i <= 1000; i++)
				{
					item1.Packages.AddNew();
				}
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX6PACKAGES) + " is 1003", "1.003", wrapper.BOX6PACKAGES);
			});
		}
		...
		public void TestBOX35GROSSMASSFormat()
		{
			...
			CombineAssertions(() =>
			{
							...
							Enterprise.Core.Constants.Customs.Universal.RefDataGrouping.Codes.EuropeanUnionEUN, ZDate.Today, false))
				{
					...
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX35GROSSMASS) + " is 40,123457 when gross mass is declared in moveHeader and final period", "40,123457", wrapper.BOX35GROSSMASS);

					moveHeader.BM_GrossWeight = 0.03m;
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX35GROSSMASS) + " is 0,030000 when gross mass is declared in moveHeader and final period", "0,030000", wrapper.BOX35GROSSMASS);

					moveHeader.BM_GrossWeight = 0m;
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX35GROSSMASS) + " is 0,000000 when gross mass is 0 in moveHeader (even when there is gross weight in items) and final period", "0,000000", wrapper.BOX35GROSSMASS);
				}
				...
				{
					moveHeader.BM_GrossWeight = 40.123456789m;
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX35GROSSMASS) + " is 40,123 when gross mass is declared in moveHeader and transition period", "40,123", wrapper.BOX35GROSSMASS);

					moveHeader.BM_GrossWeight = 0.03m;
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX35GROSSMASS) + " is 0,030 when gross mass is declared in moveHeader and transition period", "0,030", wrapper.BOX35GROSSMASS);

					moveHeader.BM_GrossWeight = 0m;
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX35GROSSMASS) + " is 0,000 when gross mass is 0 in moveHeader (even when there is gross weight in items) and transition period", "0,000", wrapper.BOX35GROSSMASS);
				}
			});
		}
		...
		public void TestBOX38NETTMASSFormat()
		{
			...
			CombineAssertions(() =>
			{
				...
				{
					var wrapper = NctsHeaderDocumentWrapper.New(header, Factory);
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX38NETTMASS) + " is 60,000000 when final period", "60,000000", wrapper.BOX38NETTMASS);

					item1.BY_NetWeight = 3000;
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX38NETTMASS) + " is 3030,000000 when final period", "3.030,000000", wrapper.BOX38NETTMASS);

					item1.BY_NetWeight = 30.1234m;
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX38NETTMASS) + " is 60.123400 when final period", "60,123400", wrapper.BOX38NETTMASS);
				}
							...
							Enterprise.Core.Constants.Customs.Universal.RefDataGrouping.Codes.EuropeanUnionEUN, ZDate.Today, true))
				{
					item1.BY_NetWeight = 30;
					var wrapper = NctsHeaderDocumentWrapper.New(header, Factory);
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX38NETTMASS) + " is 60,000 when transition period", "60,000", wrapper.BOX38NETTMASS);

					item1.BY_NetWeight = 3000;
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX38NETTMASS) + " is 3030,000 when transition period", "3.030,000", wrapper.BOX38NETTMASS);

					item1.BY_NetWeight = 30.1234m;
					AssertEquals(nameof(NctsHeaderDocumentWrapper.BOX38NETTMASS) + " is 60.123 when transition period", "60,123", wrapper.BOX38NETTMASS);
				}
			});
		}
		...
		public void TestBOXDSEALSAFFIXEDNUMBERFormat()
		{
			...
			{
				var wrapper = NctsHeaderDocumentWrapper.New(header, Factory);
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDSEALSAFFIXEDNUMBER) + " is 2", "2", wrapper.BOXDSEALSAFFIXEDNUMBER);
				...
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDSEALSAFFIXEDNUMBER) + " is 1003", "1.003", wrapper.BOXDSEALSAFFIXEDNUMBER);
			});
		...
		public void TestBOXCOFFICEOFDEPARTURE()
		{
			...
			{
				var wrapper = NctsHeaderDocumentWrapper.New(header, Factory);
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXCOFFICEOFDEPARTURE) + " returns empty string when no DEP customs office exists", ZString.Empty, wrapper.BOXCOFFICEOFDEPARTURE);
				...
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXCOFFICEOFDEPARTURE) + " shows office's description and code when DEP customs office exists but mrn doesn't", description + "\n" + officeCode, wrapper.BOXCOFFICEOFDEPARTURE);
				...
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXCOFFICEOFDEPARTURE) + " shows office's description, code and arrival date when DEP customs office and mrn exist and mrn has issue date", description + "\n" + officeCode + "\n16-02-2022", wrapper.BOXCOFFICEOFDEPARTURE);

				header.CustomsOffices.RemoveAndDeleteAll();
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXCOFFICEOFDEPARTURE) + " shows arrival date mrn exists and has issue date but no DEP customs office exists", "\n16-02-2022", wrapper.BOXCOFFICEOFDEPARTURE);
			});
		}
		...
		public void TestBOXCDATE()
		{
			...
			{
				var wrapper = NctsHeaderDocumentWrapper.New(header, Factory);
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXCDATE) + " returns empty string when there is no event", ZString.Empty, wrapper.BOXCDATE);

				SetUpEvent(header);
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXCDATE) + " returns empty string even if there is an event", ZString.Empty, wrapper.BOXCDATE);
			});
		...
		public void TestBOXDRESULT()
		{
			...
			{
				var wrapper = NctsHeaderDocumentWrapper.New(header, Factory);
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDRESULT) + " returns empty string when no clearance criteria is set", ZString.Empty, wrapper.BOXDRESULT);

				header.ClearanceCriteria = "A2";
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDRESULT) + " shows the correct description when clearance criteria is A2", "A2 Normal Procedure", wrapper.BOXDRESULT);

				header.ClearanceCriteria = "A3";
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDRESULT) + " shows the correct description when clearance criteria is A3", "A3 Simplified Procedure", wrapper.BOXDRESULT);

				header.ClearanceCriteria = "A1";
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDRESULT) + " returns empty string when clearance criteria is not A2 or A3", ZString.Empty, wrapper.BOXDRESULT);
			});
		}
		...
		public void TestBOXDTIMELIMITDATE()
		{
			...
			{
				var wrapper = NctsHeaderDocumentWrapper.New(header, Factory);
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDTIMELIMITDATE) + " returns empty string when no csv clearance is set", ZString.Empty, wrapper.BOXDTIMELIMITDATE);
				...
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDTIMELIMITDATE) + " shows the correct date when a csv clearance number is set with expiry date", "16-02-2025", wrapper.BOXDTIMELIMITDATE);

				entryNum.CE_ExpiryDate = ZDateTime.Empty;
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDTIMELIMITDATE) + " returns empty string when  a csv clearance number is set with empty expiry date", ZString.Empty, wrapper.BOXDTIMELIMITDATE);
			});
		}
		...
		public void TestBOXDCLEARANCE()
		{
			...
			{
				var wrapper = NctsHeaderDocumentWrapper.New(header, Factory);
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDCLEARANCE) + " returns only expected Spanish text when no csv clearance is set", "----------------------------------------------------------------------------------------------------\nAutentificación", wrapper.BOXDCLEARANCE);

				var entryNum = CusEntryNumber.LoadOrCreate(header, CusEntryNumberTypes.Spain.ClearanceCSV, header.CountryCode);
				entryNum.CE_EntryNum = "B9026422646FE2AB";
				entryNum.CE_IssueDate = new ZDateTime(2022, 02, 16);
				AssertEquals(nameof(NctsHeaderDocumentWrapper.BOXDCLEARANCE) + " shows the Spanish text and the csv clrarance code when a csv clearance number is set", "----------------------------------------------------------------------------------------------------\nAutentificación: B9026422646FE2AB", wrapper.BOXDCLEARANCE);
			});
		}
