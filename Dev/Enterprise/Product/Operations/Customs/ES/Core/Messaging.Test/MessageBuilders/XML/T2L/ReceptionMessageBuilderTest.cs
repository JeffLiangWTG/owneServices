...
{
	[TestedType(typeof(ReceptionMessageBuilder))]
	public class ReceptionMessageBuilderTest : T2LReceptionCommonMessageBuilderTest<ReceptionMessageBuilder, IReceptionMessageDataProvider, T2LrecepcionV1Ent>
	{
		protected override ZString ExpectedMessageType => DeclarationMessageTypeList.Codes.T2lReception;

		protected override ZString GetSignedMessageTestFileContent() => GetTestFileContents(XMLTestFileConstants.T2LTestFilePath, "TestReceptionMessageSigned.txt");
		protected override ZString GetUnsignedMessageTestFileContent() => GetTestFileContents(XMLTestFileConstants.T2LTestFilePath, "TestReceptionMessage.txt");

		protected override ReceptionMessageBuilder CreateMessageBuilder() => MockRandomGenerator(new ReceptionMessageBuilder(mockProvider.Object, ExpectedMessageType, ExpectedMessageSubType));
		protected override ReceptionMessageBuilder CreateMessageBuilderWithNullProvider() => new ReceptionMessageBuilder(default, ExpectedMessageType, ExpectedMessageSubType);
		...
		protected override void SetUp()
		{
			...
			mockLine1 = SetUpCommonLine(1, new ZString[] { "Container1", "Container2" }, new[] { mockPackage1, mockPackage2 }, Enumerable.Empty<IVehicleCommon>());

			var mockVehicle = BuilderHelperTest.SetUpVehicle("XX", "Ford", "Focus");
			var mockLine2 = SetUpCommonLine(2, Array.Empty<ZString>(), Enumerable.Empty<IPackageCommonNumbers>(), new[] { mockVehicle, mockVehicle });

			mockProvider.Setup(m => m.Lines).Returns(new[] { mockLine1.Object, mockLine2.Object });
		}

		protected Mock<IT2LLineCommon> SetUpCommonLine(ZInt lineNumber, IEnumerable<ZString> containers, IEnumerable<IPackageCommonNumbers> packages, IEnumerable<IVehicleCommon> vehicles)
		{
