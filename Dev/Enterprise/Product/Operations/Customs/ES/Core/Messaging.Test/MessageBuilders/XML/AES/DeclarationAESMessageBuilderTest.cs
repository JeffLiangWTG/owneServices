...
[TestedType(typeof(DeclarationAESMessageBuilder))]
public class DeclarationAESMessageBuilderTest : DeclarationAESCommonMessageBuilderTest<DeclarationAESMessageBuilder, IDeclarationAESMessageDataProvider, Cc515Cv1Ent, IDeclarationAESExportOperation>
{
	#region Tests
	...
	public void TestPopulateRecapitulationDate()
	{
		mockExportOperation.Setup(m => m.RecapitulationDateSpecified).Returns(false);
		AssertNotContains("presentationOfTheGoodsDateAndTime", CreateMessageBuilder().GetSignedMessageText());
	}

	#endregion

	protected override ZString ExpectedMessageType => DeclarationMessageTypeList.Codes.ExportUcc6;

	protected override DeclarationAESMessageBuilder CreateMessageBuilder() => MockRandomGenerator(new DeclarationAESMessageBuilder(mockProvider.Object, ExpectedMessageType, ExpectedMessageSubType));

	protected override DeclarationAESMessageBuilder CreateMessageBuilderWithNullProvider() => new DeclarationAESMessageBuilder(default, ExpectedMessageType, ExpectedMessageSubType);

	protected override ZString GetTestFile() => GetTestFileContents(XMLTestFileConstants.AESTestFilePath, "TestAESDeclaration.txt");
	...
	protected override void SetUp()
	{
		...
		mockExportOperation = SetUpExportOperation();
		mockProvider.Setup(m => m.ExportOperation).Returns(mockExportOperation.Object);
	}

	protected Mock<IDeclarationAESExportOperation> SetUpExportOperation()
	{
		var mockExportOperation = new Mock<IDeclarationAESExportOperation>();
	...
	Mock<IDeclarationAESExportOperation> mockExportOperation;
