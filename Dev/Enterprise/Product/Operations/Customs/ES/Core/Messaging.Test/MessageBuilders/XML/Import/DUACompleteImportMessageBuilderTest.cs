...
{
	[TestedType(typeof(DUACompleteImportMessageBuilder))]
	class DUACompleteImportMessageBuilderTest : ImportDUACommonMessageBuilderTest<DUACompleteImportMessageBuilder, IDUACompleteImportMessageDataProvider, ImportacionCompletaV1Ent, IDUACompleteImportHeader, IDUACompleteImportLine>
	{
		...
		public void TestPopulateExportador()
		{
			mockHeader.Setup(m => m.Exporter).Returns((IDUACompleteImportExporterProvider)null);
			AssertNoExceptionThrown(() => CreateMessageBuilder().GetSignedMessageText());
		}

		public void TestPopulateCondicionesDeEntrega()
		{
			mockHeader.Setup(m => m.DeliveryConditions).Returns((IDUACompleteImportDeliveryConditions)null);
			AssertNoExceptionThrown(() => CreateMessageBuilder().GetSignedMessageText());
		}

		public void TestPopulatePartidas()
		{
			mockProvider.Setup(m => m.Lines).Returns((IReadOnlyCollection<IDUACompleteImportLine>)null);
			AssertNoExceptionThrown(() => CreateMessageBuilder().GetSignedMessageText());
		}

		public void TestPopulateLine()
		{
			mockProvider.Setup(m => m.Lines).Returns(new IDUACompleteImportLine[] { null });
			AssertNoExceptionThrown(() => CreateMessageBuilder().GetSignedMessageText());
		}
		...
		{
			mockLine1.Setup(m => m.DocumentsAndCertificates).Returns(new IImportCommonC44CertificateDocument[] { null });
			mockProvider.Setup(m => m.Lines).Returns(new[] { mockLine1.Object });
			AssertNoExceptionThrown(() => CreateMessageBuilder().GetSignedMessageText());
		}
		...
		protected override ZString ExpectedMessageType => DeclarationMessageTypeList.Codes.ImportCompletePreDeclaration;
		protected override ZString GetTestFile() => GetTestFileContents(XMLTestFileConstants.ImportTestFilePath, "TestDUACompleteImportMessage.txt");

		protected override DUACompleteImportMessageBuilder CreateMessageBuilder() => MockRandomGenerator(new DUACompleteImportMessageBuilder(mockProvider.Object, ExpectedMessageType, ExpectedMessageSubType));
		protected override DUACompleteImportMessageBuilder CreateMessageBuilderWithNullProvider() => new DUACompleteImportMessageBuilder(default, ExpectedMessageType, ExpectedMessageSubType);

		protected Mock<IDUACompleteImportHeader> mockHeader;
		protected Mock<IDUACompleteImportLine> mockLine1;
		...
		protected override void SetUp()
		{
			...
			mockProvider.Setup(m => m.ProcedureDate).Returns(new ZDateTime(2020, 10, 20));
			...
			var mockDeclaredTax3 = SetUpDeclaredTax("CCC", 333.333, 65.654352, ZString.Empty, ZString.Empty, 30.55);
			...
			var mockLine2 = SetUpLine(2, Array.Empty<ZString>(), "40", "00", Enumerable.Empty<IPackageCommonNumbers>(), new[] { mockVehicle, mockVehicle }, Enumerable.Empty<IImportCommonC44CertificateDocument>(), new[] { mockDeclaredTax1, mockDeclaredTax2, mockDeclaredTax3 });

			mockProvider.Setup(m => m.Lines).Returns(new[] { mockLine1.Object, mockLine2.Object });
		}

		protected Mock<IDUACompleteImportHeader> SetUpHeader()
		{
			var mockHeader = new Mock<IDUACompleteImportHeader>();
		...
		protected void SetUpSpecificHeaderFieldsCore(Mock<IDUACompleteImportHeader> mockHeader)
		{
			mockHeader.Setup(m => m.DestinationCountry).Returns("ES");
			mockHeader.Setup(m => m.DestinationState).Returns("28");
			mockHeader.Setup(m => m.ArrivalTransportId).Returns("ArrivalTrID");
			mockHeader.Setup(m => m.FrontierTransportCountry).Returns("ES");
			mockHeader.Setup(m => m.InvoiceAmount).Returns(1256.423145M);
			mockHeader.Setup(m => m.TransactionNature).Returns("IM");
			mockHeader.Setup(m => m.FrontierTransportMode).Returns("3");
			mockHeader.Setup(m => m.InteriorTransportMode).Returns("8");
			mockHeader.Setup(m => m.CustomsOfficeOfEntry).Returns("ES009998");
			mockHeader.Setup(m => m.DepositId).Returns("DepositId");

			var mockExporter = SetUpExporter();
			mockHeader.Setup(m => m.Exporter).Returns(mockExporter.Object);

			var mockDeliveryConditions = SetUpDeliveryConditions();
			mockHeader.Setup(m => m.DeliveryConditions).Returns(mockDeliveryConditions.Object);
		}

		Mock<IDUACompleteImportExporterProvider> SetUpExporter()
		{
			var mockExporter = new Mock<IDUACompleteImportExporterProvider>();
			mockExporter.Setup(m => m.Id).Returns("DE123456789A");
			mockExporter.Setup(m => m.Name).Returns("exporter name ªº aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaabbbb");
			mockExporter.Setup(m => m.Address).Returns("exporter address complete aaaaaaaaabbb");
			mockExporter.Setup(m => m.City).Returns("BERLIN");
			mockExporter.Setup(m => m.PostCode).Returns("54632");
			mockExporter.Setup(m => m.Country).Returns("DE");
			mockExporter.Setup(m => m.SimplifiedProcedureType).Returns("A");
			return mockExporter;
		}

		Mock<IDUACompleteImportDeliveryConditions> SetUpDeliveryConditions()
		{
			var mockDeliveryConditions = new Mock<IDUACompleteImportDeliveryConditions>();
			mockDeliveryConditions.Setup(m => m.Code).Returns("DEL");
			mockDeliveryConditions.Setup(m => m.Place).Returns("Madrid");
			mockDeliveryConditions.Setup(m => m.ZoneIndicator).Returns("1");
			return mockDeliveryConditions;
		}

		protected Mock<IDUACompleteImportLine> SetUpLine(ZInt lineNumber, ZString[] containers, ZString requestedCPC, ZString previousCPC, IEnumerable<IPackageCommonNumbers> packages, IEnumerable<IVehicleCommon> vehicles, IEnumerable<IImportCommonC44CertificateDocument> documents, IEnumerable<IDUAImportDeclaredTax> declaredTaxes)
		{
			var mockLine = new Mock<IDUACompleteImportLine>();
			...
			SetUpSpecificLineFieldsCore(mockLine);

			return mockLine;
		}

		protected void SetUpSpecificLineFieldsCore(Mock<IDUACompleteImportLine> mockLine)
		{
			mockLine.Setup(m => m.REACode).Returns("T001");
			mockLine.Setup(m => m.PositiveAdjustment).Returns(60.99);
			mockLine.Setup(m => m.NegativeAdjustment).Returns(30.44);
			mockLine.Setup(m => m.StatisticalValue).Returns(100.777);
		}
	}
}
