...
{
	[TestedType(typeof(G3PresentGoodMessageBuilder))]
	class G3PresentGoodMessageBuilderTest : G3CommonMessageBuilderTest<G3PresentGoodMessageBuilder, IG3PresentGoodMessageDataProvider, G3PresV1Ent>
	{
		...
		{
			mockProvider.Setup(m => m.Header).Returns((IG3PresentGoodHeader)null);
		...
		{
			mockHeader.Setup(m => m.MasterConsignment).Returns(new IG3PresentGoodMasterConsignment[] { null });
			AssertNoExceptionThrown(() => CreateMessageBuilder().GetSignedMessageText());
		}

		public void TestPopulateHouseConsignment()
		{
			mockMasterConsignment.Setup(m => m.HouseConsignment).Returns(new IG3HouseConsignment[] { null });
			AssertNoExceptionThrown(() => CreateMessageBuilder().GetSignedMessageText());
		}
		...
		protected override void SetUp()
		{
			...
			mockMasterConsignment = new Mock<IG3PresentGoodMasterConsignment>();
			mockHouseConsignment = new Mock<IG3HouseConsignment>();
			...
			mockMasterConsignment.Setup(m => m.HouseConsignment).Returns(new IG3HouseConsignment[] { mockHouseConsignment.Object });

			mockHeader.Setup(m => m.MasterConsignment).Returns(new IG3PresentGoodMasterConsignment[] { mockMasterConsignment.Object });
		}

		protected Mock<IG3PresentGoodHeader> SetupHeader()
		{
			var mockHeader = new Mock<IG3PresentGoodHeader>();
		...
		Mock<IG3PresentGoodHeader> mockHeader;
		Mock<IG3PresentGoodMasterConsignment> mockMasterConsignment;
		Mock<IG3HouseConsignment> mockHouseConsignment;

		protected override ZString ExpectedMessageType => DeclarationMessageTypeList.Codes.G3RevocationOfGoods;

		protected override G3PresentGoodMessageBuilder CreateMessageBuilder() => MockRandomGenerator(new G3PresentGoodMessageBuilder(mockProvider.Object, ExpectedMessageType, ExpectedMessageSubType));

		protected override G3PresentGoodMessageBuilder CreateMessageBuilderWithNullProvider() => new G3PresentGoodMessageBuilder(default, ExpectedMessageType, ExpectedMessageSubType);

		protected override ZString GetTestFile() => GetTestFileContents(XMLTestFileConstants.G3TestFilePath, "TestG3PresentGoodMessage.txt");
	}
}
