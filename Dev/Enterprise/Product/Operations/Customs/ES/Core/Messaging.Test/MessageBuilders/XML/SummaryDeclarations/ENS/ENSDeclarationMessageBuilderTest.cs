...
{
	[TestedType(typeof(ENSDeclarationMessageBuilder))]
	public class ENSDeclarationMessageBuilderTest : ENSCommonMessageBuilderTest<ENSDeclarationMessageBuilder, IENSDeclarationMessageDataProvider, Cc315A>
	{
		...
		{
			mockProvider.Setup(m => m.Header).Returns((IENSDeclarationHeader)null);
			AssertNoExceptionThrown(() => CreateMessageBuilder().GetSignedMessageText());
		}

		public void TestPopulateLodgingCustomsOffice()
		{
			mockProvider.Setup(m => m.LodgingCustomsOffice).Returns(ZString.Empty);
		...
		protected override ZString ExpectedMessageType => DeclarationMessageTypeList.Codes.EntrySummaryDeclaration;
...
#endif
			"TestENSDeclarationMessageSigned.txt");
		protected override ZString GetUnsignedMessageTestFileContent() => GetTestFileContents(XMLTestFileConstants.ENSTestFilePath, "TestENSDeclarationMessage.txt");

		protected override ENSDeclarationMessageBuilder CreateMessageBuilder() => MockRandomGenerator(new ENSDeclarationMessageBuilder(mockProvider.Object, ExpectedMessageType, ExpectedMessageSubType));
		protected override ENSDeclarationMessageBuilder CreateMessageBuilderWithNullProvider() => new ENSDeclarationMessageBuilder(default, ExpectedMessageType, ExpectedMessageSubType);
		...
		protected override void SetUp()
		{
			...
			mockProvider.Setup(m => m.LodgingCustomsOffice).Returns("ES004611");

			var mockHeader = SetUpHeader();
			mockProvider.Setup(m => m.Header).Returns(mockHeader.Object);
		}

		Mock<IENSDeclarationHeader> SetUpHeader()
		{
			var mockHeader = new Mock<IENSDeclarationHeader>();
			...
			mockHeader.Setup(m => m.DeclarationPlaceLanguage).Returns("ES");
			...
			mockHeader.Setup(m => m.DeclarationDate).Returns(new ZDateTime(2020, 03, 12, 14, 55, 00));
