...
[TestedType(typeof(ReceptionT2LMessageBuilder))]
class ReceptionT2LMessageBuilderTest : T2LPOUSCommonMessageBuilderTest<ReceptionT2LMessageBuilder, IReceptionT2LMessageDataProvider, Iep01IndType>
{
	#region Test
	...
	{
		mockProvider.Setup(m => m.ProofOperationInformationForT2LT2LF).Returns((IT2LPOUSReceptionProofOperationInformationForT2LT2LF)null);
		CombineAssertions(() =>
		{
	...
	public void TestPopulateTipoAltaIndirecta()
	{
		mockProvider.Setup(m => m.TipoAltaIndirecta).Returns((ZString)null);
		CombineAssertions(() =>
		{
			AssertNoExceptionThrown(() => CreateMessageBuilder().GetSignedMessageText());
			AssertNotContains($"<{XMLTestFileConstants.XmlElementNamespace}TipoAltaIndirecta>", CreateMessageBuilder().GetSignedMessageText());
		});
	}

	public void TestPopulateEnvioDocumentos()
	{
		mockProvider.Setup(m => m.EnvioDocumentos).Returns((IReadOnlyCollection<IAnnexDocCommon>)null);
		CombineAssertions(() =>
		{
			AssertNoExceptionThrown(() => CreateMessageBuilder().GetSignedMessageText());
			AssertNotContains($"<{XMLTestFileConstants.XmlElementNamespace}EnvioDocumentos>", CreateMessageBuilder().GetSignedMessageText());
		});
	}
	...
	protected override ZString ExpectedMessageType => DeclarationMessageTypeList.Codes.T2lReceptionPous;

	protected override ReceptionT2LMessageBuilder CreateMessageBuilder() => MockRandomGenerator(new ReceptionT2LMessageBuilder(mockProvider.Object, ExpectedMessageType, ExpectedMessageSubType));

	protected override ReceptionT2LMessageBuilder CreateMessageBuilderWithNullProvider() => new ReceptionT2LMessageBuilder(default, ExpectedMessageType, ExpectedMessageSubType);

	protected override ZString GetTestFile() => GetTestFileContents(XMLTestFileConstants.T2LPOUSTestFilePath, "TestT2LPOUSReception.txt");
	...
	protected override void SetUp()
	{
		...
		var mockProofOperationInformation = new Mock<IT2LPOUSReceptionProofOperationInformationForT2LT2LF>();

		mockProofOperationInformation.Setup(m => m.CodigoReferencia).Returns("CodigoReferencia");
		...
		mockProvider.Setup(m => m.TipoAltaIndirecta).Returns("TipoAltaIndirecta");

		var mockReceptionDocumento1 = BuilderHelperTest.SetUpAnnexDocCommon("Descripcion1", "RefDelDoc1", new byte[] { 2, 2 }, "ExtDelDoc1");
		var mockReceptionDocumento2 = BuilderHelperTest.SetUpAnnexDocCommon("Descripcion2", "RefDelDoc2", new byte[] { 2, 2 }, "ExtDelDoc2");
		var mockReceptionEnvioDocumentos = new IAnnexDocCommon[] { mockReceptionDocumento1, mockReceptionDocumento2 };
		mockProvider.Setup(m => m.EnvioDocumentos).Returns(mockReceptionEnvioDocumentos);

		var mockAuthorisation = new Mock<IT2LPOUSAuthorisation>();
		mockAuthorisation = SetUpAuthorisationCommon("C511", "ReferenceNumber", "Holder");
