...
{
	class GoodsNotificationAESMessagePrettyFormatterTest : TestCaseWithFactory
	{
		public void TestConstructor()
		{
			AssertExceptionThrown<ArgumentNullException>("Null response", () => new GoodsNotificationAESMessagePrettyFormatter(null));
		}
		...
		{
			var declarationResponse = new Cc511Cv1Sal();
			declarationResponse.ControlRespuesta = new Cc511Cv1SalControlRespuesta()
			{
				TipoRespuesta = "EF"
			};
			...
			var messagePrettyFormatter = new GoodsNotificationAESMessagePrettyFormatter(declarationResponse);
		...
		Cc511Cv1Sal SetResponseData(DateTime acceptanceDate, ZString mrn, ZString circuitAEAT, ZString circuitATC, DateTime? csvClearanceDate, ZString declarationCSV, ZString csvCode, ZString directFlag, ZString status)
		{
			var response = new Cc511Cv1Sal();
			response.DatosRespuestaCorrecta = new DatosRespuestaCorrectaType511C()
			{
				Mrn = mrn,
		...
		ZString GetAcceptedInterpretationText(Cc511Cv1Sal response)
		{
			var messagePrettyFormatter = new GoodsNotificationAESMessagePrettyFormatter(response);
