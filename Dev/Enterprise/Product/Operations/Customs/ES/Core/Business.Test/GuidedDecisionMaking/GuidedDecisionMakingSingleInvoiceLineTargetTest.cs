...
namespace Enterprise.Customs.ES.Business.Testing
{
	sealed class GuidedDecisionMakingSingleInvoiceLineTargetTest : TestCaseWithFactory
	{
		public void TestTaxOrFeeDetail()
		{
			...
			AssertEquals("JI_TaxOrFeeDetail should be correctly set with tax2", tax2.PK, invoiceLine.JI_TaxOrFeeDetail);
			AssertEquals("JI_ZZF_NKTaxType should be correctly set with tax2", "STD", invoiceLine.JI_ZZF_NKTaxType);

			selectedVAT.Setup(v => v.AdditionalCode).Returns("");
			selectedVAT.Setup(v => v.VATCode).Returns("SRR");
			guidedDecisionMakingTarget.SetVATCode(selectedVAT.Object.VATCode, selectedVAT.Object.AdditionalCode);
			AssertEquals("JI_TaxOrFeeDetail should be correctly set with tax3", tax3.PK, invoiceLine.JI_TaxOrFeeDetail);
			AssertEquals("JI_ZZF_NKTaxType should be correctly set with tax3", "SRR", invoiceLine.JI_ZZF_NKTaxType);
			...
			guidedDecisionMakingTarget.SetVATCode(selectedVAT.Object.VATCode, selectedVAT.Object.AdditionalCode);
			AssertEquals("JI_TaxOrFeeDetail should be left as it was", ZGuid.Empty, invoiceLine.JI_TaxOrFeeDetail);
			AssertEquals("JI_ZZF_NKTaxType should be correctly set with EX", "EX", invoiceLine.JI_ZZF_NKTaxType);
		}
	}
}
