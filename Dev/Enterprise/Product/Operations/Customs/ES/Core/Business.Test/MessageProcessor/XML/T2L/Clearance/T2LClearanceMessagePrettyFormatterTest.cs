...
{
	class T2LClearanceMessagePrettyFormatterTest : T2LCommonMessagePrettyFormatterTest<T2LdatadoV2Sal>
	{
		public void TestConstructor()
		{
			AssertExceptionThrown<ArgumentNullException>("Null response", () => new T2LClearanceMessagePrettyFormatter(null));
		}

		public void TestCreateMessageDetailsAccepted()
		{
			var declarationResponse = SetResponseData("20211004", "103045", "21ES99990012345678", "HHAXSXPQA6NT963Y", "9KJLVLPCWNKUV7N8", "Documento T2L de Datado.", hasCircuitAssigment: true);
			...
			AssertEquals("<H3>Accepted Declaration</H3><H3>CSV Electronic Declaration = HHAXSXPQA6NT963Y</H3>" +
					...
					"<br><table border=\"0\"><tr><td>Description:</td><td>&nbsp;&nbsp;</td><td>Documento T2L de Datado.</td></tr></table>", messageInterpretationText);
		}
		...
		protected override T2LCommonMessagePrettyFormatter<T2LdatadoV2Sal> GetFormatter(T2LdatadoV2Sal response) => new T2LClearanceMessagePrettyFormatter(response);

		protected override T2LdatadoV2Sal GetResponse(string responseCode = "", string responseDesc = "")
		{
			var response = new T2LdatadoV2Sal();
			response.CodigoRespuesta = responseCode;
			response.DescripcionRespuesta = responseDesc;
			return response;
		}

		protected override T2LdatadoV2Sal SetResponseData(ZString date, ZString hour, ZString reference, ZString csvClearance, ZString csvPdfT2l, ZString description, CircuitoTipo circuito = CircuitoTipo.V, bool hasCircuitAssigment = false)
		{
			...
			response.NumeroDeReferenciaDelJec = reference;
			response.CsVdeDeclaracionElectronica = csvClearance;
			response.CsVdelJustificanteDeLevante = csvPdfT2l;
