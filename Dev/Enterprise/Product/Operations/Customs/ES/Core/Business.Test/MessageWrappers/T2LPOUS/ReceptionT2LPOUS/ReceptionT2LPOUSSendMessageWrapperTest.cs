...
public class ReceptionT2LPOUSSendMessageWrapperTest : WrapperHelperTest<ReceptionT2LPOUSSendMessageWrapper>
{
	...
	public void TestTipoAltaIndirecta()
	{
		entryInstruction.ZG_IndirectType = "IND1";
		AssertEquals("Expected filled TipoAltaIndirecta", "IND1", wrapper.TipoAltaIndirecta);
	}

	public void TestEnvioDocumentos()
	{
		CombineAssertions(() =>
		{
			AssertEquals("Expected empty EnvioDocumentos when no anexxes declared", 0, wrapper.EnvioDocumentos.Count);
			...
			var documents = wrapper.EnvioDocumentos;
			AssertEquals("Expected filled EnvioDocumentos with both annexes", 2, documents.Count);
			AssertSame("Cached EnvioDocumentos", wrapper.EnvioDocumentos, documents);
		});
	}
	...
	public void TestRepresentative()
	{
		CombineAssertions(() =>
		{
			...
			declaration.JE_OH_Importer = orgHeader.PK;
			wrapper = GetWrapper(entryHeader);
			AssertNull("Expected null Representative when Declarant is the same as the Importer", wrapper.Representative);
			...
			wrapper = GetWrapper(entryHeader);
			AssertNull("Expected null Representative when Representative is the same as the Importer", wrapper.Representative);
		});
	}
	...
	ReceptionT2LPOUSSendMessageWrapper wrapper;

	ReceptionT2LPOUSSendMessageWrapper GetWrapper(CusEntryHeader entryHeader) => new ReceptionT2LPOUSSendMessageWrapper(entryHeader, Certificate);

	protected override ReceptionT2LPOUSSendMessageWrapper GetProvider() => wrapper;
}
