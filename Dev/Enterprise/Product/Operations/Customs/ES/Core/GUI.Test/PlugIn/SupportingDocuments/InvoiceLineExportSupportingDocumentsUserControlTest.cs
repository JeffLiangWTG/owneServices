...
{
	class InvoiceLineExportSupportingDocumentsUserControlTest : TestCaseWithFactory
	{
		public void TestBindingSourceType()
		{
			SupportingDocumentsControlTestHelper.AssertBindingSource(typeof(InvoiceLineExportSupportingDocumentsUserControl), typeof(JobDeclaration));
		}

		public void TestGridColumns()
		{
			SupportingDocumentsControlTestHelper.AssertGridColumns(Factory, typeof(InvoiceLineExportSupportingDocumentsUserControl), OrderedColumnNamesAndColumnStyleTypes);
		}

		public void TestCaptionRenderingEnabled()
		{
			SupportingDocumentsControlTestHelper.AssertCaptionRenderingEnabledForUserControlAndFieldsControl(typeof(InvoiceLineExportSupportingDocumentsUserControl));
		}
		...
		public void TestGridColumnStyleProperties()
		{
			...
			using (var control = new InvoiceLineExportSupportingDocumentsUserControl())
			{
				...
				CombineAssertions(() =>
				{
					...
					var csi_PackQty = (ZCalcEditColumnStyleInfo)grid.GetColumnStyle(SupportingDocument.Schema.CSI_PackQty);
					AssertEquals("CSI_PackQty: Decimals", 2, csi_PackQty.Decimals);
					var csi_PackType = (ZDropEditColumnStyleInfo)grid.GetColumnStyle(SupportingDocument.Schema.CSI_PackType);
					AssertEquals("CSI_PackType: CharacterCasing", CharacterCasing.Upper, csi_PackType.CharacterCasing);
				});
			}
		}
		...
		IEnumerable<(string ColumnName, Type ColumnType)> OrderedColumnNamesAndColumnStyleTypes => new (string, Type)[]
		{
			...
			(SupportingDocument.Schema.CSI_PackQty, typeof(ZCalcEditColumnStyle)),
			(SupportingDocument.Schema.CSI_PackType, typeof(ZDropEditColumnStyle)),
		};
	}
}
