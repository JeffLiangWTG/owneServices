...
namespace Enterprise.Customs.ES.Module.Testing
{
	...
	{
		public void TestUsesESFilterBusinessObject()
		{
		...
		public void TestGuaranteeCheckAccountingStatusMenuItem_Functionality_ExportDeclaration()
		{
			using (var module = new JobDeclarationModule())
			{
				var actionMenuItem = module.FormActionMenu.FindByText("&Actions");
				...
				var declarationList = new JobDeclarationCollection(Factory, GlbCompany.CurrentCompany.PK);
				declarationList.Add(declaration);

				module.DisplayGrid.SetDataBinding(declarationList, "");
				...
				CombineAssertions(() =>
				{
					...
					AssertEquals("Error message when no entry was selected", "Please select at least one declaration", UnitTestUserNotification.Instance.LastMessage.Text);

					module.DisplayGrid.SelectAllElements();
					checkAccountingStatusMenuItem.PerformClick();
					AssertEquals("Error message when all entries selected are export", "No entries in declarations selected match criteria to check accounting status", UnitTestUserNotification.Instance.LastMessage.Text);
				});
			}
		}
		...
		public void TestGuaranteeCheckAccountingStatusMenuItem_Functionality_ImportEntriesT2L()
		{
			using (var module = new JobDeclarationModule())
			{
				...
				CombineAssertions(() =>
				{
					...
					module.DisplayGrid.SelectAllElements();
					checkAccountingStatusMenuItem.PerformClick();
					AssertEquals("Error message when all entries selected are import but T2L/T2C", "No entries in declarations selected match criteria to check accounting status", UnitTestUserNotification.Instance.LastMessage.Text);
				});
			}
		}
		...
		public void TestGuaranteeCheckAccountingStatusMenuItem_Functionality_ImportEntriesH2()
		{
			using (var module = new JobDeclarationModule())
			{
				...
				CombineAssertions(() =>
				{
					...
					module.DisplayGrid.SelectAllElements();
					checkAccountingStatusMenuItem.PerformClick();
					AssertEquals("Error message when all entries selected are import but H2", "No entries in declarations selected match criteria to check accounting status", UnitTestUserNotification.Instance.LastMessage.Text);
				});
			}
		}
		...
		public void TestGuaranteeCheckAccountingStatusMenuItem_Functionality_ImportEntriesNoMRN()
		{
			using (var module = new JobDeclarationModule())
			{
				...
				CombineAssertions(() =>
				{
					...
					module.DisplayGrid.SelectAllElements();
					checkAccountingStatusMenuItem.PerformClick();
					AssertEquals("Error message when all entries selected are import but without mrn", "No entries in declarations selected match criteria to check accounting status", UnitTestUserNotification.Instance.LastMessage.Text);
				});
			}
		}
		...
		public void TestGuaranteeCheckAccountingStatusMenuItem_Functionality_ImportEntriesNoCSVClearance()
		{
			using (var module = new JobDeclarationModule())
			{
				...
				CombineAssertions(() =>
				{
					...
					module.DisplayGrid.SelectAllElements();
					checkAccountingStatusMenuItem.PerformClick();
					AssertEquals("Error message when all entries selected are import with mrn but without csv clearance", "No entries in declarations selected match criteria to check accounting status", UnitTestUserNotification.Instance.LastMessage.Text);
				});
			}
		}
		...
		public void TestGuaranteeCheckAccountingStatusMenuItem_Functionality_ImportEntriesNoGuarantees()
		{
			using (var module = new JobDeclarationModule())
			{
				...
				CombineAssertions(() =>
				{
					...
					module.DisplayGrid.SelectAllElements();
					checkAccountingStatusMenuItem.PerformClick();
					AssertEquals("Error message when all entries selected are import with mrn and csv clearance but without a correct guarantee associated", "No entries in declarations selected match criteria to check accounting status", UnitTestUserNotification.Instance.LastMessage.Text);
				});
			}
		}
		...
		public void TestGuaranteeCheckAccountingStatusMenuItem_Functionality_MultipleDeclarations()
		{
			...
			using (var module = new JobDeclarationModule())
			{
				...
				var declarationList = new JobDeclarationCollection(Factory, GlbCompany.CurrentCompany.PK);
				declarationList.Add(declarationAEAT);
				declarationList.Add(declarationATC);
				declarationList.Add(declarationEXP);
		...
		protected override BaseJobDeclaration CreateDeclarationForFetchHintTest(BusinessObjectFactory factory, string messageType, int i)
		{
			...
			entryHeader.EntryNumber = "ES000001";
			entryHeader.ZG_AcceptanceDate = ZDateTime.BrettsBirthday;
		...
		{
			var tableNameIsOrgHeader = tableSelect.TableName.Equals(OrgHeader.Schema.TableName);
			var tableNameIsOrgAddressAdditional = tableSelect.TableName.Equals(OrgAddressAdditionalInfoSchema.Constants.TableName);
			var tableNameIsExitReport = tableSelect.TableName.Equals(CusExitReportSchema.Constants.TableName);
			return (!tableNameIsOrgHeader && !tableNameIsExitReport && base.HasFailedFetchHint(tableSelect)) || (tableNameIsOrgHeader && tableSelect.Value > 7) || (tableNameIsOrgAddressAdditional && tableSelect.Value > 8);
		}
