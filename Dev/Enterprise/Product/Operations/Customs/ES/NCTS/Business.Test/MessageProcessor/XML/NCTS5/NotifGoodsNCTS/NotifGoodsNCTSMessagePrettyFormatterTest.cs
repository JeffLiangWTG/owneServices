...
{
	class NotifGoodsNCTSMessagePrettyFormatterTest : TestCaseWithFactory
	{
		public void TestConstructor()
		{
			AssertExceptionThrown<ArgumentNullException>("Null response", () => new NotifGoodsNCTSMessagePrettyFormatter(null));
		}
		...
		public void TestCreateMessageDetailsAcceptedDeclaration()
		{
			...
			var expectedMessageInterpretation = "<H3>Accepted Declaration</H3>" +
				"<br><table border=\"0\"><tr><td>Acceptance:</td><td>&nbsp;&nbsp;</td><td>12-12-2022</td></tr>" +
		...
		{
			var declarationResponse = new Cc170Cv1Sal();
			declarationResponse.ControlRespuesta = new Cc170Cv1SalControlRespuesta()
			{
				TipoRespuesta = "EF"
			};
			...
			var messagePrettyFormatter = new NotifGoodsNCTSMessagePrettyFormatter(declarationResponse);
		...
		public void TestStatusData()
		{
			...
			CombineAssertions(() =>
			{
				var declarationResponse = SetResponseData(DateTime.Today, DateTime.Today, DateTime.Today, status: "PG");
				var messageInterpretationText = GetAcceptedInterpretationText(declarationResponse);
		...
		Cc170Cv1Sal SetResponseData(DateTime acceptanceDate, DateTime limitDate, DateTime clearanceDate, string mrn = "", string circuit = "", string clearance = "", string status = "")
		{
			var response = new Cc170Cv1Sal();
			response.DatosRespuestaCorrecta = new DatosRespuestaCorrectaType170C()
			{
		...
		ZString GetAcceptedInterpretationText(Cc170Cv1Sal response)
		{
			var messagePrettyFormatter = new NotifGoodsNCTSMessagePrettyFormatter(response);
