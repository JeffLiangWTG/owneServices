...
[TestedType(typeof(AmendmentNCTSMessageBuilder))]
public class AmendmentNCTSMessageBuilderTest : NCTSCommonMessageBuilderTest<AmendmentNCTSMessageBuilder, IAmendmentNCTSMessageDataProvider, Cc013Cv1Ent>
{
	#region Tests
	...
	{
		mockProvider.Setup(m => m.TransitOperation).Returns((IAmendmentNCTSTransitOperation)null);
		CombineAssertions(() =>
		{
	...
	protected override ZString ExpectedMessageType => DeclarationMessageTypeList.Codes.Ncts5DepartureAmendment;

	protected override AmendmentNCTSMessageBuilder CreateMessageBuilder() => MockRandomGenerator(new AmendmentNCTSMessageBuilder(mockProvider.Object, ExpectedMessageType, ExpectedMessageSubType));

	protected override AmendmentNCTSMessageBuilder CreateMessageBuilderWithNullProvider() => new AmendmentNCTSMessageBuilder(default, ExpectedMessageType, ExpectedMessageSubType);

	protected override ZString GetTestFile() => GetTestFileContents(NCTSXMLTestFileConstants.NCTSTestFilePath, "TestAmendmentNCTS.txt");
	...
	protected Mock<IAmendmentNCTSTransitOperation> SetUpDepartureTransitOperation()
	{
		var mockTransitOperation = new Mock<IAmendmentNCTSTransitOperation>();
