...
namespace Enterprise.Customs.ES.NCTS.Messaging.MessageBuilders.Testing
{
	...
	public class ArrivalAVIMessageBuilderTest : ArrivalBaseMessageBuilderTest
	{
		protected override ZString ExpectedMessageType => DeclarationMessageTypeList.Codes.NctsArrivalNotification;

		protected override ZBool IsOnlyArrivalNotification => true;
		...
		protected override ZBool IsArrivalWithTNN => false;

		protected override ZString GetTestFile() => GetTestFileContents(NCTSTestFileConstants.TestFilePath, "TestArrivalAVIMessage.txt");
		...
		public override void TestAddCSTSegmentOnSG30_NoGoodsCustomsProcedureCategory1()
		{
			...
			AssertNotContains("CST+1++T1:116:141+M3:ZZZ:148+HB12345678901+12345678'", messageText);
		}
		...
		public override void TestAddCSTSegmentOnSG30_NoGoodsCustomsProcedureCategory2()
		{
			...
			var messageText = CreateMessageBuilder().GetSignedMessageText();
			AssertNotContains("CST+1+88033090:122:148++M3:ZZZ:148+HB12345678901+12345678'", messageText);
		}
		...
		public override void TestAddCSTSegmentOnSG30_NoGoodsCustomsProcedureCategory3()
		{
			...
			var messageText = CreateMessageBuilder().GetSignedMessageText();
			AssertNotContains("CST+1+88033090:122:148+T1:116:141++HB12345678901+12345678'", messageText);
		}
		...
		public override void TestAddCSTSegmentOnSG30_NoGoodsCustomsProcedureCategory4()
		{
			...
			var messageText = CreateMessageBuilder().GetSignedMessageText();
			AssertNotContains("CST+1+88033090:122:148+T1:116:141+M3:ZZZ:148++12345678'", messageText);
		}
		...
		public override void TestAddCSTSegmentOnSG30_NoGoodsCustomsProcedureCategory5()
		{
			...
			var messageText = CreateMessageBuilder().GetSignedMessageText();
			AssertNotContains("CST+1+88033090:122:148+T1:116:141+M3:ZZZ:148+HB12345678901'", messageText);
		}

		public override void TestAddNewSG31Group_NoTags()
		{
			var mockExternalPackages = BuilderHelperTest.SetUpExternalPackages(2, Enumerable.Empty<ZString>());
			mockLine1.Setup(m => m.ExternalPackages).Returns(mockExternalPackages);
			mockProvider.Setup(m => m.Lines).Returns(new[] { mockLine1.Object });

			var messageText = CreateMessageBuilder().GetSignedMessageText();
			AssertNotContains("PAC+2+3+CT'PAC+50+1+BX'", messageText);
		}
