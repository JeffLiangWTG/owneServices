...
{
	[TestedType(typeof(CC043CMessageProcessor))]
	sealed class CC043CMessageProcessorTest : NctsMessageProcessorTestCase<CC043CMessageProcessor, ICC043CDataProvider>
	{
		public void TestCESEvent()
		{
			...
			CombineAssertions(() =>
			{
				...
				var eventLog = movementHeader.Logs.GetAllLogs().Cast<StmALog>().FirstOrDefault(l => l.SL_SE_NKEvent == "CES" && (l.SL_Reference == "UAP"));
				...
				AssertEquals("reference", "UAP", eventLog.SL_Reference);
		...
		public void TestLockEventCreated()
		{
			...
			eventInfo.EventReference = NCTS5ArrivalCustomsStatusList.Codes.UnloadPermissionGranted;
			...
			AssertEquals("EventReference", "The tab 'Arrival Notification' was locked when message 'Unloading Permission' was received.", lckEventlog.SL_Reference);
		}

		NctsHeader ProcessBasicMessage(NctsHeader nctsHeader = null)
		{
			mockProvider.Setup(x => x.MRN).Returns("22BE000000000012J1");
			nctsHeader = nctsHeader ?? Factory.New<NctsHeader>();
		...
		public void TestProcessMessage()
		{
			...
			Assert_Header(header, NCTS5ArrivalCustomsStatusList.Codes.UnloadPermissionGranted);
		...
		public void TestProcessMessageWithoutConsignment()
		{
			mockProvider.Setup(x => x.MRN).Returns("22BE000000000012J1");
			var header = Factory.New<NctsHeader>();
			header.SetMovementType(EU.NCTS.Business.NctsMovementType.Codes.Arrival);
			...
			Assert_Header(header, NCTS5ArrivalCustomsStatusList.Codes.UnloadRemarks);
		}

		public void TestPhaseStatus_UAP()
		{
			Assert_PhaseStatus(NCTS5ArrivalCustomsStatusList.Codes.UnloadPermissionGranted, NctsMovementHeaderTransactionStatusList.Codes.Arrival);
		}

		public void TestPhaseStatus_ULR()
		{
			Assert_PhaseStatus(NCTS5ArrivalCustomsStatusList.Codes.UnloadRemarks, NctsMovementHeaderTransactionStatusList.Codes.UnloadingRemarks);
		}

		void Assert_PhaseStatus(string customsStatus, string expectedPhaseStatus)
		{
			mockProvider.Setup(x => x.MRN).Returns("22BE000000000012J1");
			mockProvider.Setup(x => x.ContinueUnloading).Returns(true);
			var header = Factory.New<NctsHeader>();
			header.SetMovementType(EU.NCTS.Business.NctsMovementType.Codes.Arrival);
			var movementHeader = header.ArrivalMovementHeader;
			...
			AssertEquals("Phase for customs status " + customsStatus, expectedPhaseStatus, movementHeader.BM_Phase);
		}
		...
		public void TestGenerationOfCustomsRegistry()
		{
			...
			using (BECustomsRegistry.Instance.CustomsRegistry.SetTemporaryValue(Guid.Empty, GlbBranch.CurrentBranch.PK.ToGuid(), Guid.Empty, listOfValues))
			{
				...
				CombineAssertions(() =>
				{
					...
					Factory.Save();
					var entryNumber2 = Factory.New<CusEntryNumberForCC043CMessage>();
		...
		void Assert_Header(NctsHeader header, string customsStatus)
		{
			...
			CombineAssertions(() =>
			{
				...
				AssertContains("Incoming Message Interpretation", "Unloading Permission: Continue <br />Status granted on:", incomingMessage.EM_MessageInterpretation);
			});
		}
		...
		void Assert_HouseConsignment(NctsHeader header)
		{
			...
			CombineAssertions(() =>
			{
				AssertEquals("Customs Status", NCTS5ArrivalCustomsStatusList.Codes.UnloadPermissionGranted, header.ArrivalMovementHeader.BM_CustomsStatus);
		...
		protected override Mock<ICC043CDataProvider> MockProvider => mockProvider ?? (mockProvider = new Mock<ICC043CDataProvider>());
		Mock<ICC043CDataProvider> mockProvider;

		protected override Mock<CC043CMessageProcessor> MockProcessor => mockProcessor ?? (mockProcessor = new Mock<CC043CMessageProcessor>(new LoggingInformation()));
		Mock<CC043CMessageProcessor> mockProcessor;

		protected override ZString MessageTypeToInclude => BEIncomingMessageTypes.Codes.CC043C;

		protected override string ExpectedMessageFriendlyName => BEIncomingMessageTypes.Descriptions.CC043C;

		protected override ZString[] PreProcessedOKCustomsStatus => new ZString[] { NctsTransitStatusList.Codes.Unknown, NctsTransitStatusList.Codes.UnloadingPermissionGranted, NCTS5ArrivalCustomsStatusList.Codes.UnloadRemarks };
	...
	class CC043CMessageProcessorForTest : CC043CMessageProcessor
	{
		public CC043CMessageProcessorForTest(LoggingInformation logger) : base(logger) { }

		public BusinessObject FindParentOfMessageExposed(BEMessage message, ICC043CDataProvider messageDataProvider) => base.FindParentOfMessage(message, messageDataProvider);
	}
}
