...
namespace Enterprise.Customs.BE.NCTS.Business.Testing
{
	...
	{
		public void TestEntryTypeList_Cached()
		{
			...
			AssertSame(entryTypeList, lookups.EntryTypeList);
		}
						...
						foreach (var customsStatus in customsStatuses.Distinct().ToList())
						{
							...
							foreach (var phase in phases.Distinct().ToList())
							{
								...
								foreach (var messageStatus in messageStatusses.Distinct().ToList())
								{
									...
									var expected = GetEntryTypesExpected(subApplicationCode, additionalDeclarationType, customsStatus, phase, messageStatus);
									var actual = lookups.EntryTypeList.CodesAsString;
		...
		string GetEntryTypesExpected(ZString subApplicationCode, ZString additionalDeclarationType, ZString customsStatus, ZString phase, ZString messageStatus)
		{
			return entryTypes.TryGetValue((subApplicationCode, additionalDeclarationType, customsStatus, phase, messageStatus), out var messageTypes) ? messageTypes : string.Empty;
		}

		readonly Dictionary<(string subApplicationCode, string additionalDeclarationType, string customsStatus, string phase, string messageStatus), string> entryTypes =
