...
{
	[TestedType(typeof(ACEFDAEditForm))]
	sealed class ACEFDAEditFormTest : ZFormBasherTest
	{
		public void TestScientificDetailsGridNotExist()
		{
			...
			using (var form = new ACEFDAEditForm(fda))
			{
				form.Show();
		...
		public void TestColumnVisibilityForProduct()
		{
			...
			using (var form = new ACEFDAEditForm(fdaOnProduct))
			{
				form.Show();
				var capName = (ZString)form.ConstGrid.GetColumnCaption(("US_PGAQuantityOfConstituentElement"));
				AssertEquals("visible", true, !capName.IsEmpty);
		...
		{
			using (var form = new ACEFDAEditForm())
			{
				form.Show();
		...
		{
			using (var control = new ACEFDAEditForm())
			{
