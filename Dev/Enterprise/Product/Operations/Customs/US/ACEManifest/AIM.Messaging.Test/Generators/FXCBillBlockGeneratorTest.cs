...
{
	[TestedType(typeof(FXCBillBlockGenerator))]
	class FXCBillBlockGeneratorTest : AIMBlockGeneratorTest
	{
		protected override AIMBlockGenerator GetGenerator() => new FXCBillBlockGenerator(GetMessageHeaderMock().Object);

		protected override ZString MessageType => Constants.AIMMessageSubTypes.FXC;
		...
		public void TestPopulateMessage()
		{
			...
			var cbpShipmentDescription = messageBlocks.FirstOrDefault(x => x is AIMCBPShipmentDescription);
			var fdaFreightIndicator = messageBlocks.FirstOrDefault(x => x is AIMFDAFreightIndicator);
			CombineAssertions(() =>
			{
...
", consignee.Serialise());
				AssertEquals(@"-------AIMCBPShipmentDescription--------
DeclaredValue (1-12N)  :123000
ISOCurrencyCode (3-3A) :USD

", cbpShipmentDescription.Serialise());
				AssertEquals(@"---------AIMFDAFreightIndicator---------

", fdaFreightIndicator.Serialise());
			});
		}
	}
}
