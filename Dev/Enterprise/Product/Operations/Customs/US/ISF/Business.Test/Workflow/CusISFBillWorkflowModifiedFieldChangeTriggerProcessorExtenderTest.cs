...
{
	sealed class CusISFBillWorkflowModifiedFieldChangeTriggerProcessorExtenderTest : TestCaseWithFactory
	{
		[TestDate(2005, 1, 1)]
		public void TestFireTriggerOnFieldChange()
		{
			...
			header.BF_OwnerReference = "OWNREF";
			var bill = header.ReferenceDatas.AddNew();
			bill.BB_BillType = BillTypeList.Codes.HouseBillOfLading;
			bill.BB_BillNum = "BM1231221";
			DummyWorkflowDescriptor.Instance.AddToWorkflowTriggerFieldColumnList(CusISFBillSchema.BB_CustomsStatus);
			var fieldTrigger = header.WorkflowItems.Triggers.AddNew();
			fieldTrigger.TriggerConditions.TriggerFieldName = CusISFBillSchema.BB_CustomsStatus.Name;
