...
{
	sealed class ACEApplicationControlGeneratorTest : TestCaseWithFactory
	{
		[TestDate(1971, 9, 18)]
		public void TestNew()
		{
			...
			applicationControlGenerator = ApplicationControlGenerator.New(CBPEDIInterchange.ApplicationCodes.USCustomsImport, ACEApplicationIdentifierCodeList.Codes.EntrySummary, GlbBranch.CurrentBranch);
			AssertEquals(typeof(ACEApplicationControlGenerator), applicationControlGenerator.GetType());
			AssertEquals(typeof(AABIInputA), applicationControlGenerator.A.GetType());
			AssertEquals(typeof(AABIInputZ), applicationControlGenerator.Z.GetType());
			applicationControlGenerator.AddMessage(message);
