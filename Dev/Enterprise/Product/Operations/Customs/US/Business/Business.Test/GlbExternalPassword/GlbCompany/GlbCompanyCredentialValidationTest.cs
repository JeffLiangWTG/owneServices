...
namespace Enterprise.Customs.US.Business.Testing
{
	...
	sealed class GlbCompanyCredentialValidationTest : MasterFiles.Business.Testing.GlbExternalPasswordValidationTest<GlbCompanyCredential, GlbCompanyCredentialValidation>
	{
		public void TestCheckGP_MailBoxID()
		{
			...
			var collection = new GlbCompanyCredentialCollection(Factory.New<GlbCompany>());
		...
		{
			var collection = GlbCompanyWrapper.GetWrapper<GlbCompanyWrapper>(Factory.New<GlbCompany>()).PasswordCollection;
			var credential = collection.AddNew();
		...
		public void TestCheckCurrentDecryptedCertificatePassphrase()
		{
			...
			Assert(credential.CurrentDecryptedCertificatePassphraseInfo.HasMessageError("You have not entered a value."));

			credential.CurrentDecryptedCertificatePassphrase = "123";
			Assert(credential.CurrentDecryptedCertificatePassphraseInfo.HasMessageError("Password must greater than 3 characters."));
