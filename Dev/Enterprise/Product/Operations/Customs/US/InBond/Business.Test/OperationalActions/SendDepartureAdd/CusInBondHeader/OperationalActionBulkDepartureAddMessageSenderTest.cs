...
{
	sealed class OperationalActionBulkDepartureAddMessageSenderTest : TestCaseWithFactory
	{
		public void TestSendWhenThereAreErrorsOnInBond()
		{
			...
			var sender = new OperationalActionBulkDepartureAddMessageSender(inBondHeader);
			...
			sender = new OperationalActionBulkDepartureAddMessageSender(inBondHeader);
			sender.OperationalActionSendMessage(true, log);
		...
		public void TestSendWhenThereIsNoValidMoveHeaderOnInBond()
		{
			...
			var sender = new OperationalActionBulkDepartureAddMessageSender(inBondHeader);
			sender.OperationalActionSendMessage(true, log);
			AssertContains("There is no In-Bond Movement available for sending a 'Departure Add' message to Customs.", log.MessagesString());
			moveHeader.BM_CustomsStatus = ZString.Empty;
			Factory.Save();
			log = new DummyOperationalActionSectionLog();
			sender = new OperationalActionBulkDepartureAddMessageSender(inBondHeader);
			sender.OperationalActionSendMessage(true, log);
			AssertNotContains("There is no In-Bond Movement available for sending a 'Departure Add' message to Customs.", log.MessagesString());
			moveHeader.Messages.Reload(true);
			AssertEquals(1, moveHeader.Messages.Count);
			Assert("Mutex is unlocked", !inBondHeader.IsSendCustomsMessageMutexLocked);
		}
