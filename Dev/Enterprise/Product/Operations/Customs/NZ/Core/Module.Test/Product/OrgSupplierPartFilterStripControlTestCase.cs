	...
	sealed class OrgSupplierPartFilterStripControlTestCase : TestCaseWithFactory
	{
		public void TestShowTariffsOnGrid()
		{
			...
			using (var stripControl = new OrgSupplierPartFilterStripControl(products, new OrgSupplierPartFilterStripBusinessObject()))
			{
				...
				form.Show();
				Assert("NZ should be showing Tariff column", stripControl.FilteredGrid.Columns.Any(l => l.ColumnName == "Tariffs"));
				Assert("NZ should be showing Lookup columns", stripControl.FilteredGrid.Columns.Any(l => l.ColumnName == "ExportClassificationLookup"));
				Assert("NZ should be showing Lookup columns", stripControl.FilteredGrid.Columns.Any(l => l.ColumnName == "ImportClassificationLookup"));
			}
		}
	}
}
