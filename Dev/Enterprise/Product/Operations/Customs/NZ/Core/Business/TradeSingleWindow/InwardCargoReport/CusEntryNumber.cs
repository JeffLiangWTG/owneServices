...
namespace Enterprise.Customs.NZ.Business.TradeSingleWindow
{
	public class CusEntryNumber : Common.CusEntryNumber
	{
		...
		public static ZQuery GetEntryNumberFilter(ZGuid parentId, ZString entrytype)
		{
			ZQuery filter = new ZQuery(CusEntryNumSchema.CE_ParentID, SQLComparisonOperator.Equal, parentId);
			...
			filter.AddToFilter(CusEntryNumSchema.CE_EntryType, SQLComparisonOperator.Equal, entrytype);
			return filter;
		}

		public static ZQuery GetEntryNumberFilter(ForwardingConsol consol, ZString entrytype)
		{
			return GetEntryNumberFilter(consol.PK, entrytype);
		}
		...
		public static ZQuery GetEntryNumberFilter(ForwardingShipment shipment)
		{
			...
			subFilter.AddToFilter(JoinCondition.Or, CusEntryNumSchema.CE_EntryType, SQLComparisonOperator.Equal, CusEntryNumberTypeList.Codes.ICRNumber);

			filter.AddToFilter(subFilter, JoinCondition.And);
			return filter;
		}

		public static ZQuery GetEntryNumberFilter(JobDeclaration declaration, ZString entrytype)
		{
			ZQuery filter = new ZQuery(CusEntryNumSchema.CE_ParentID, SQLComparisonOperator.Equal, declaration.PK);
			filter.AddToFilter(CusEntryNumSchema.CE_RN_NKCountryCode, SQLComparisonOperator.Equal, GlbCompany.CurrentCompany.GC_RN_NKCountryCode);
			filter.AddToFilter(CusEntryNumSchema.CE_EntryType, SQLComparisonOperator.Equal, entrytype);
			return filter;
		}
		...
		{
			get { return LowValueManifestStatusList.GetDescriptionFromCode(CE_EntryStatus); }
		}
