...
{
	sealed class TSWEX1MessageBuilderTest : XmlMessageBuilderTest
	{
		public override void TestGenerateTestMessage()
		{
			...
			messageBuilder = new TSWEX1MessageBuilder(entryHeader, null, Enterprise.Customs.NZ.Business.MessageBuilders.FormalEntry.MessageBuilder.MessageTypes.CancelEntry);
			messageBuilder.GenerateMessage();
			var messageGenerated = messageBuilder.GetMessageText();
			AssertEquals("EX1 message generated", expectedMessage, messageGenerated);

			var message = Declaration.CusEntryHeader.Messages[0];
			AssertEquals("EDIMessage generated MUST have TSW message type from MappedTSWMessageSubType", MessageTypeList.Codes.E40, message.EM_MessageType);
		}
		...
		public void TestGenerateQueuedMessage()
		{
			using (GlbCompany.TemporaryLoginInNewCompanyForCountry(Core.Constants.CountryCodes.NewZealand))
			{
				...
				messageBuilder = new TSWEX1MessageBuilder((CusEntryHeader)Declaration.CusEntryHeader, null, MessageBuilder.MessageTypes.Original);
				messageBuilder.GenerateMessage();
		...
		public void TestSetParentMessagingStatusAfterMessagePosting()
		{
			...
			CusEntryHeader entryHeader = (CusEntryHeader)Declaration.CusEntryHeader;
			messageBuilder = new TSWEX1MessageBuilder(entryHeader, null, FormalEntry.MessageBuilder.MessageTypes.CancelEntry);
			messageBuilder.GenerateMessage();
			var messageGenerated = messageBuilder.GetMessageText();
		...
		public void TestReplacementSendsPDO()
		{
			...
			messageBuilder = new TSWEX1MessageBuilder(entryHeader, null, FormalEntry.MessageBuilder.MessageTypes.Original);
			Assert("'PDO' Other info should not be included on this entry", !Declaration.JE_PDOOtherInfoValue);

			messageBuilder = new TSWEX1MessageBuilder(entryHeader, null, FormalEntry.MessageBuilder.MessageTypes.Replacement);
