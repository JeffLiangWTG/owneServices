...
{
	public class IE801Provider : IIE801
	{
		...
		public ZString JourneyTime
		{
			get
			{
				var time = message.Body.EadesadContainer.HeaderEadEsad.JourneyTime.Substring(1, 2);
				var timeFormat = message.Body.EadesadContainer.HeaderEadEsad.JourneyTime.Substring(0, 1);
		...
		public ZDateTime DispatchTime
		{
			get
			{
				var eadEsad = message.Body.EadesadContainer.EadEsad;
				var dateOfDispatch = eadEsad.DateOfDispatch;
				var timeOfDispatch = eadEsad.TimeOfDispatch;
				var timeOfDispatchHour = 0;
				var timeOfDispatchMinute = 0;
				if (!string.IsNullOrEmpty(timeOfDispatch))
				{
					timeOfDispatchHour = int.Parse(timeOfDispatch.Substring(0, 2));
					timeOfDispatchMinute = int.Parse(timeOfDispatch.Substring(3, 2));
				}

				return new ZDateTime(dateOfDispatch.Year, dateOfDispatch.Month, dateOfDispatch.Day, timeOfDispatchHour, timeOfDispatchMinute, 0);
			}
		}
		...
		public IEMCSPartyGuarantor Guarantor => guarantor ?? (GuarantorTypeCode == CargoWise.Customs.IE.MessageDefinitions.EMCSPhase4_1.tcl.GuarantorTypeCode.Item3.XmlEnumToString() ? guarantor = IE801PartyGuarantorProvider.NewOrNull(message.Body.EadesadContainer.MovementGuarantee.GuarantorTrader?.FirstOrDefault()) : null);
	...
	class IE801EventProvider : IEMCSEvent
	{
