using CargoWise.Customs.IE.MessageDefinitions.EMCSPhase4_1.IE810;
using CargoWise.Types;
using NUnit.Framework;

namespace Enterprise.Customs.IE.EMCS.Business.Testing.Phase4_1
{
	[TestedType(typeof(IE810MessageProcessor))]
	sealed class IE810MessageProcessorTest : IE810MessageProcessorAbstractTest<Ie810Type>
	{
		protected override ZString MessageText => EMCSMessageProcessorTestHelper.GetStandardIE810Text();

		protected override string MessageSampleNameSpace => "Enterprise.Customs.IE.EMCS.Business.Testing.Message.MessageProcessors.Phase4_1.TestFiles.IE810MessageSample.txt";
	}
}
