...
{
	public class EMCSJobDeclarationUpdateHelperTest : TestCaseWithFactory
	{
		...
		public void TestCreateOrUpdateGuarantor_MatchesExistingTEN()
		{
			...
			cusCode.ModifyOrgCusCode(orgHeader.PK, OrgCusCode.EuropeanUnionSharedCodeTypes.TraderExciseNumber, Core.Constants.CountryCodes.Ireland, "IETEN001", orgAddress.PK);
			Factory.Save();

			var guarantorTrader = new GuarantorTraderType { TraderExciseNumber = "IETEN001" };
		...
		public void TestCreateOrUpdateDeliveryPlace_MatchesExistingTID()
		{
			...
			var cusCode = Factory.NewWithValidTestData<OrgCusCode>();
			cusCode.ModifyOrgCusCode(orgHeader.PK, OrgCusCode.EuropeanUnionSharedCodeTypes.TraderID, Core.Constants.CountryCodes.Ireland, "IETID001", orgAddress.PK);
			Factory.Save();

			var deliveryPlace = new DeliveryPlaceTraderType { Traderid = "IETID001" };
		...
		public void TestUpdatePartyAddress()
		{
			var partyAddressMock = new Mock<IEMCSPartyAddress>();
			partyAddressMock.Setup(x => x.Language).Returns("IE");
			partyAddressMock.Setup(x => x.City).Returns("Dublin");
			partyAddressMock.Setup(x => x.StreetAndNumber).Returns("Street A 9");
			partyAddressMock.Setup(x => x.Postcode).Returns("2000");
			partyAddressMock.Setup(x => x.Name).Returns("MyComp");
			var inputPartyAddress = Factory.New<JobDocAddress>();
			inputPartyAddress.UpdatePartyAddress(partyAddressMock.Object);

			CombineAssertions("JobDocAddress fields by UpdatePartyAddress", () =>
			{
				AssertEquals("E2_RN_NKCountryCode", "IE", inputPartyAddress.E2_RN_NKCountryCode);
				AssertEquals("E2_City", "Dublin", inputPartyAddress.E2_City);
				AssertEquals("E2_Address1", "Street A 9", inputPartyAddress.E2_Address1);
				AssertEquals("E2_Postcode", "2000", inputPartyAddress.E2_Postcode);
				AssertEquals("E2_CompanyName", "MyComp", inputPartyAddress.E2_CompanyName);
			});
		}
