...
namespace Enterprise.Customs.IE.EMCS.Business.Testing
{
	...
	abstract public class HeaderProviderAbstractTest<T> : Customs.Business.Testing.DataProviderTestCase<T>
		where T : HeaderProvider
	{
		...
		protected OrgHeader GetPartyGuarantorOrg(ZString exciseNumber, ZString vatNumber)
		{
			...
			orgHeader.CustomsCodes.AddNew(OrgCusCode.CodeTypes.VATCode, vatNumber, Core.Constants.CountryCodes.Ireland);
			orgHeader.MainAddress.OA_RN_NKCountryCode = Core.Constants.CountryCodes.Ireland;
			return orgHeader;
		}
