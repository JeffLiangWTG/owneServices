...
namespace Enterprise.Customs.IE.EMCS.Business.Testing
{
	sealed class PartyDeliveryPlaceProviderTest : Customs.Business.Testing.DataProviderTestCase<PartyDeliveryPlaceProvider>
	{
		...
		public void TestTraderId()
		{
			...
			organisation.CustomsCodes.AddNew(OrgCusCode.EuropeanUnionSharedCodeTypes.TraderID, "TID123", Core.Constants.CountryCodes.Ireland);
			orgAddress.Address.CustomsCodes.AddNew(OrgCusCode.EuropeanUnionSharedCodeTypes.TraderID, "TID234", Core.Constants.CountryCodes.Ireland);
		...
		public void TestTraderId_JE_MessageSubType4Or11()
		{
			organisation.CustomsCodes.AddNew(OrgCusCode.FranceCodeTypes.TVA, "TVA123", Core.Constants.CountryCodes.France);

			jobDeclaration.JE_MessageSubType = EMCSDestinationTypeList.Codes.DestinationDirectDelivery;
			var deliveryPlaceTrader = PartyDeliveryPlaceProvider.NewOrNull(orgAddress);
			AssertEquals("VatId", string.Empty, deliveryPlaceTrader.TraderId);

			jobDeclaration.JE_MessageSubType = EMCSDestinationTypeList.Codes.DestinationReturnPlaceOfDispatchConsignor;
			deliveryPlaceTrader = PartyDeliveryPlaceProvider.NewOrNull(orgAddress);
			AssertEquals("VatId", string.Empty, deliveryPlaceTrader.TraderId);
		}
		...
		public void TestStreetAndNumber()
		{
			...
			var deliveryPlaceTrader = PartyDeliveryPlaceProvider.NewOrNull(jobDocAddress);
			AssertEquals("Address", "23 CROWN ST", deliveryPlaceTrader.StreetAndNumber);
		}

		public void TestStreetAndNumber_Empty()
		{
			jobDeclaration.JE_MessageSubType = EMCSDestinationTypeList.Codes.DestinationTaxWarehouse;

			var deliveryPlaceTrader = PartyDeliveryPlaceProvider.NewOrNull(orgAddress);
			AssertEquals("Address", ZString.Empty, deliveryPlaceTrader.StreetAndNumber);
		}
