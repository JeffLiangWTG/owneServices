	...
	public static class EMCSJobDeclarationUpdateHelper
	{
		public static void CreateOrUpdateGuarantor(this EMCSJobDeclaration emcsDeclaration, IEMCSPartyGuarantor guarantorInMessage)
		{
			...
			if (guarantorInMessage != null)
			{
				...
				var orgHeader = factory.GetOrgHeaderByCustomsRegNo(countryCode, OrgCusCode.EuropeanUnionSharedCodeTypes.TraderExciseNumber, traderExciseNumber)
					?? factory.GetOrgHeaderByCustomsRegNo(countryCode, vatCodeType, vatNumber);
				if (orgHeader != null)
				{
		...
		public static void CreateOrUpdateTransportDetails(this EMCSJobDeclaration emcsDeclaration, IReadOnlyCollection<IEMCSTransportDetails> transportDetails)
		{
			...
			foreach (var transportDetail in transportDetails)
			{
				...
				transport.CO_ContainerNumber = transportDetail.IdentityOfUnit;
				transport.CO_Seal = transportDetail.CommercialSealIdentification;
		...
		public static void UpdatePartyAddress(this JobDocAddress jobDocAddress, IEMCSPartyAddress addressInMessage)
		{
			jobDocAddress.E2_RN_NKCountryCode = addressInMessage.Language;
			jobDocAddress.E2_City = addressInMessage.City;
			jobDocAddress.E2_Address1 = addressInMessage.StreetAndNumber;
