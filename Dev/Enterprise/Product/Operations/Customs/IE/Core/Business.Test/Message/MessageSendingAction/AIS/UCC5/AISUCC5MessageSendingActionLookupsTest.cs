...
{
	sealed class AISUCC5MessageSendingActionLookupsTest : TestCaseWithFactory
	{
		public void TestSendingActionTypeList()
		{
			(_, _, _, var lookups) = GetNewLookups(Factory);
			AssertEquals("SendingActionTypeList.Code", "413, 414, 415, 432", lookups.SendingActionTypeList.CodesAsString);

			var anotherJob = Factory.New<JobDeclaration>();
			anotherJob.JE_ApplicationCode = "V1";
			var anotherLookups = new AISUCC5MessageSendingAction((CusEntryHeader)anotherJob.ActiveEntryHeaders.AddNew()).Lookups;
		...
		public void TestSendingActionTypeListForDisplay()
		{
			...
			AssertEquals("SendingActionTypeList.Code", "IM413, IM414, IM415, IM432", list.CodesAsString);
		...
		static (JobDeclaration declaration, CusEntryHeader entry, AISUCC5MessageSendingAction sendingAction, AISUCC5MessageSendingActionLookups lookups) GetNewLookups(BusinessObjectFactory factory)
		{
			...
			var lookups = (AISUCC5MessageSendingActionLookups)sendingAction.Lookups;
