...
namespace Enterprise.Customs.IE.Business.CusTempStorage.Testing
{
	...
	sealed class TemporaryStorageSupportingDocumentConfigurationTest : TemporaryStorageSupportingDocumentConfigurationAbstractTest<TemporaryStorageSupportingDocumentConfiguration>
	{
		public override void TestGetCodeList()
		{
			...
			var supportingDocument = Factory.NewWithValidTestData<TemporaryStorageHeader>().Bills.AddNew().SupportingDocuments.AddNew();
			var codeList = configuration.GetCodeList(supportingDocument) as ZZRefCusCodeListCombinedCollection;
			...
			AssertSame("Cached", codeList, (ZZRefCusCodeListCombinedCollection)supportingDocument.Lookups.CodeList);
		}
		...
		void GenerateCodeList(out RefCusCodeList codeLists1, out RefCusCodeList codeLists2, out RefCusCodeList codeLists3, out RefCusCodeList codeLists4, out RefCusCodeList codeLists5)
		{
			...
			helper.CreateCusCodeType(Core.Constants.Customs.Universal.RefCusCodeListTypes.Codes.SupportingDocumentOfImportDirection,
				"Document Type (EU Box 44 Imports)");
			helper.CreateCusCodeType(Core.Constants.Customs.Universal.RefCusCodeListTypes.Codes.SupportingDocumentsTemporaryStorageUCC6,
				"Supporting Document of PNTS");
			...
			codeLists1 = helper.CreateNewOrGetExistingCusCodeList(Core.Constants.CountryCodes.Ireland,
				Core.Constants.Customs.Universal.RefCusCodeListTypes.Codes.SupportingDocumentOfImportDirection, "A", "Test 1",
				yesterday, tomorrow);

			codeLists2 = helper.CreateNewOrGetExistingCusCodeList(Core.Constants.CountryCodes.Ireland,
				Core.Constants.Customs.Universal.RefCusCodeListTypes.Codes.SupportingDocumentOfImportDirection, "B", "Test 1",
				yesterday, tomorrow);

			codeLists3 = helper.CreateNewOrGetExistingCusCodeList(Core.Constants.CountryCodes.France,
				Core.Constants.Customs.Universal.RefCusCodeListTypes.Codes.SupportingDocumentOfImportDirection, "C", "Test 1",
				yesterday, tomorrow);

			codeLists4 = helper.CreateNewOrGetExistingCusCodeList(Core.Constants.CountryCodes.Ireland,
				Core.Constants.Customs.Universal.RefCusCodeListTypes.Codes.SupportingDocumentOfImportDirection, "D", "Test 1",
				yesterday, tomorrow);
			codeLists5 = helper.CreateNewOrGetExistingCusCodeList(Core.Constants.CountryCodes.Ireland,
				Core.Constants.Customs.Universal.RefCusCodeListTypes.Codes.SupportingDocumentsTemporaryStorageUCC6, "E", "Test 2",
				yesterday, tomorrow);
			Factory.Save();
		}
	}
