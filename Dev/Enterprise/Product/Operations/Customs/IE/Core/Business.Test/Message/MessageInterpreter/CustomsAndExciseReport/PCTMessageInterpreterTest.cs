...
{
	[TestedType(typeof(PCTMessageInterpreter))]
	sealed class PCTMessageInterpreterTest : CustomsAndExciseReportInboundMessageInterpreterTest<PCTMessageInterpreter, PCTProvider>
	{
		protected override ZString MessageType => CustomsAndExciseReportTypeList.Codes.PCT;

		protected override PCTProvider GetProvider(TextReader reader) => new PCTProvider(CustomsAndExciseReportInterchangeProcessorTestHelper.CreatePCTMessage());

		protected override ZString ExpectedInterpretation => GetExpectedInterpretation();

		internal static ZString GetExpectedInterpretation() => @"Period (monthly) details – combined taxes report for Payers<br />
			<br />
