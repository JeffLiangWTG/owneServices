...
namespace Enterprise.Customs.IE.GUI.Testing
{
	sealed class OwnerOfGoodsUserControlTest : TestCaseWithFactory
	{
		public void TestGroupBoxCaption()
		{
			...
			using (var control = new OwnerOfGoodsUserControlForTest())
			{
				control.SetJobDeclarationForTest(declaration);

				var groupBox = control.FindSingleOrDefault<ZGroupBox>("OwnersOfGoodsGroupBox");
				CombineAssertions(() =>
				{
					AssertEquals("Caption", "[Art. 163 3/8] Owner of Goods", groupBox.CaptionResourceString.Caption);
					AssertEquals("Full Description", "[Article 163 3/8] Owner of Goods", groupBox.CaptionResourceString.FullDescription);
				});
			}
		}
	}

	class OwnerOfGoodsUserControlForTest : OwnerOfGoodsUserControl
	{
