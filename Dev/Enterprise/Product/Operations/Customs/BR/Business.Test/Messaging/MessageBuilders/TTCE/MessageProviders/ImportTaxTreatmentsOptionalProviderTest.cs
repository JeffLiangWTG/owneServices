...
{
	public class ImportTaxTreatmentsOptionalProviderTest : TestCaseWithFactory
	{
		public void TestConstructor()
		{
			AssertExceptionThrown<ArgumentException>("RespostaObterTratamentosTributariosImportacaoDTO is null", () => new ImportTaxTreatmentsOptionalProvider(null));
		}

		public void TestProperties()
		{
			var ttce = BRMessageHelper.DeserializeObject<RespostaObterTratamentosTributariosImportacaoDTO>(ResponseMessageMTT);
			var provider = new ImportTaxTreatmentsOptionalProvider(ttce);
			CombineAssertions(() =>
			{
				AssertEquals("NCM should be", "01010101", provider.Ncm);
				AssertEquals("CountryCodeshould be", 160, provider.CountryCode);
				AssertEquals("TaxEventDateshould be", new DateTime(2024, 12, 26), provider.TaxEventDate);
				AssertEquals("OperationType should be", Constants.OperationType.Import, provider.OperationType);
				AssertEquals("OptionalLegalBasisList count should be", 4, provider.OptionalLegalBasisList.Count());
			});
		}
