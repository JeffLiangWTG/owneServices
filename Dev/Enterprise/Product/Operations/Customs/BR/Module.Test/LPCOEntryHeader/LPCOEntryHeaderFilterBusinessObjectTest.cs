...
{
	[TestedType(typeof(LPCOEntryHeaderFilterBusinessObject))]
	class LPCOEntryHeaderFilterBusinessObjectTest : Customs.Module.Testing.EntryHeaderFilterBusinessObjectAbstractTest
	{
		public void TestFilterForMessageType()
		{
			...
			var entry2Dec1 = dec1.CustomsEntryHeaders.AddNew();
			entry2Dec1.CH_MessageType = BRJobMessageTypeList.Codes.LPCO;
			...
			var entry2Dec2 = dec2.CustomsEntryHeaders.AddNew();
			entry2Dec2.CH_MessageType = MessageTypeList.Codes.CDE;
			...
			Assert(entry2Dec1.MatchesFilter(filterBO.Filter));
			Assert(!entry1Dec2.MatchesFilter(filterBO.Filter));
			Assert(!entry2Dec2.MatchesFilter(filterBO.Filter));
		...
		{
			AssertEquals(LPCODeclarationFilterStripBusinessObject.LPCONumberText, filterBO[Customs.Module.EntryHeaderFilterBusinessObject.Constants.EntryNumber].MultilingualDescription);
		}

		public override void TestEntryStatusFilterDescription()
		{
			AssertEquals(LPCODeclarationFilterStripBusinessObject.LPCOStatusText, filterBO[Customs.Module.EntryHeaderFilterBusinessObject.Constants.EntryStatus].MultilingualDescription);
		}
		...
		protected override FilterStripBusinessObject GetNewFilterStripBusinessObject() => new LPCOEntryHeaderFilterBusinessObject();

		LPCOEntryHeaderFilterBusinessObject filterBO;

		protected override void SetUp()
		{
			base.SetUp();
			filterBO = (LPCOEntryHeaderFilterBusinessObject)GetNewFilterStripBusinessObject();
		}
	}
}
