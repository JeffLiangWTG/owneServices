...
{
	class AsycudaBillScreeningLookupsTest : BusinessObjectLookupsTestCase
	{
		public void TestScreeningResultList()
		{
			var header = Factory.New<AsycudaManifestHeader>();
			var bill = header.Bills.AddNew();
			var billScreening = bill.BillScreenings.AddNew();
			var list = billScreening.Lookups.ScreeningResultList;

			AssertSame("Accessing the list twice should get the exact same object as the list is cached", list, billScreening.Lookups.ScreeningResultList);
		...
		public void TestScreeningAuthorizedPersonTypes()
		{
			...
			var bill = header.Bills.AddNew();
			var billScreening = bill.BillScreenings.AddNew();
			var list = billScreening.Lookups.ScreeningAuthorizedPersonTypes;

			AssertSame("Accessing the list twice should get the exact same object as the list is cached", list, billScreening.Lookups.ScreeningAuthorizedPersonTypes);
			AssertEquals(true, list.ContainsCode("1"));
			AssertEquals(true, list.ContainsCode("2"));
		...
		public void TestTransportDocumentTypes()
		{
			...
			var list = billScreening.Lookups.TransportDocumentTypeList;
