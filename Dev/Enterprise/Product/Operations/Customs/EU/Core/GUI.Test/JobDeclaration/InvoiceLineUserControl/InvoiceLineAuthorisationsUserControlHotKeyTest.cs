...
{
	class InvoiceLineAuthorisationsUserControlHotKeyTest : TestCaseWithFactory
	{
		[RequiresSTA]
		public void TestAuthorisationUsageUpdate()
		{
			...
			using (var control = new InvoiceLineAuthorisationsUserControlForAdditionalTest())
			{
				...
				{
					AssertEquals("Authorizations count must be 1", 1, invoiceLine.CusAuthorizationUsages.Count);

					var authorisationUsage = invoiceLine.CusAuthorizationUsages[0];
		...
		class InvoiceLineAuthorisationsUserControlForAdditionalTest : InvoiceLineAuthorisationsUserControlForTest
		{
