...
namespace Enterprise.Customs.EU.Business.Testing
{
	class VatFeeRefresherTest : TestCaseWithFactory
	{
		...
		{
			var vatRefresherWithUniversalFeeCalculation = ((CusEntryLineFeeCollection<CusEntryLineFee, CusEntryLine>)VatCalculationTestHelper.SetUpEntryLine(Factory).Fees).VatRefresher;
			...
			{
				var vatRefresherWithoutUniversalFeeCalculation = ((CusEntryLineFeeCollection<CusEntryLineFee, CusEntryLine>)VatCalculationTestHelper.SetUpEntryLine(Factory).Fees).VatRefresher;
		...
		public void TestDummyVatRefresher()
		{
			using (ConfigurationTestHelper.TemporarilySetUseUniversalFeeCalculationConfiguration(Factory, false))
			{
				...
				var vatRefresher = ((CusEntryLineFeeCollection<CusEntryLineFee, CusEntryLine>)entryLine.Fees).VatRefresher;
		...
		public void TestUpdateSystemAddedVatFeeIfAllowed()
		{
			using (ConfigurationTestHelper.TemporarilySetUseUniversalFeeCalculationConfiguration(Factory, true))
			{
				...
				using (((CusEntryLineFeeCollection<CusEntryLineFee, CusEntryLine>)entryLine.Fees).SuspendSystemAddedVatFeeRecalculation())
				{
					...
					systemDefinedVatFee = VatCalculationTestHelper.CreateEntryLineFee(entryLine, UniversalReferenceConstants.RefCusRateCodes.Vat, "%", 22m, 1m, 1m, "", false);
				...
				CombineAssertions("Updated VAT", () =>
				{
					AssertNotNull(systemDefinedVatFee);
					AssertEquals("CF_Rate", 22m, systemDefinedVatFee.CF_Rate);
					AssertEquals("CF_BaseValue", 825.96m, systemDefinedVatFee.CF_BaseValue);
					AssertEquals("CF_ChargeAmount", 181.7112m, systemDefinedVatFee.CF_ChargeAmount);
				});
			}
		}
		...
		public void TestUpdateSystemAddedVatFeeIfAllowedWithoutUniversalFeeCalculation()
		{
			using (ConfigurationTestHelper.TemporarilySetUseUniversalFeeCalculationConfiguration(Factory, false))
			{
				...
				var entryLine = VatCalculationTestHelper.SetUpEntryLine(Factory);
				AssertNotNull("VatRefresher", ((CusEntryLineFeeCollection<CusEntryLineFee, CusEntryLine>)entryLine.Fees).VatRefresher);
		...
		public void TestSystemVatFeeReCalculated_OnItemRemoved()
		{
			using (ConfigurationTestHelper.TemporarilySetUseUniversalFeeCalculationConfiguration(Factory, true))
			{
				...
				using (((CusEntryLineFeeCollection<CusEntryLineFee, CusEntryLine>)entryLine.Fees).SuspendSystemAddedVatFeeRecalculation())
				{
					...
					systemDefinedVatFee = VatCalculationTestHelper.CreateEntryLineFee(entryLine, UniversalReferenceConstants.RefCusRateCodes.Vat, "%", 11m, 320m, 35.2m, "", false);
				}
		...
		public void TestSystemVatFeeReCalculated_OnCusEntryLineFeeChange()
		{
			using (ConfigurationTestHelper.TemporarilySetUseUniversalFeeCalculationConfiguration(Factory, true))
			{
				...
				{
					systemDefinedDtyFee = VatCalculationTestHelper.CreateEntryLineFee(entryLine, UniversalReferenceConstants.RefCusRateCodes.CustomsDutyOnIndustrialProducts, "%", 10m, 1000m, 100m, "", false);
					systemDefinedVatFee = VatCalculationTestHelper.CreateEntryLineFee(entryLine, UniversalReferenceConstants.RefCusRateCodes.Vat, "%", 11m, 100m, 11m, "", false);
				}

				userDefinedVatFee = VatCalculationTestHelper.CreateEntryLineFee(entryLine, UniversalReferenceConstants.RefCusRateCodes.Vat, "%", 5, 50m, 2.5m, "ADD", false);
				...
				{
					AssertEquals("CF_BaseValue", 220m, systemDefinedVatFee.CF_BaseValue);
					AssertEquals("CF_ChargeAmount", 24.2m, systemDefinedVatFee.CF_ChargeAmount);
				});
		...
		public void TestEntryFeeCollectionVatRefreshingEventsAreHookedWithoutExplicitlyReferencingVatRefresher()
		{
			using (ConfigurationTestHelper.TemporarilySetUseUniversalFeeCalculationConfiguration(Factory, true))
			{
				...
				VatCalculationTestHelper.CreateEntryLineFee(entryLine, UniversalReferenceConstants.RefCusRateCodes.Vat, Customs.Business.UniversalReferenceConstants.MethodOfCalculation.Percentage, 22m, 1m, 1m, "", false);
				...
				CombineAssertions("[PRE-CONDITION] Pre-Existing VAT Fee", () =>
				{
					...
					AssertEquals("CF_ChargeType", UniversalReferenceConstants.RefCusRateCodes.Vat, fee.CF_ChargeType);
				...
				VatCalculationTestHelper.CreateEntryLineFee(entryLine, UniversalReferenceConstants.RefCusRateCodes.CustomsDutyOnIndustrialProducts, "%", 25m, 1212m, 456m, RateOverrideReasonList.Codes.Additional, false);
				...
				CombineAssertions("Updated VAT fee", () =>
				{
					...
					AssertEquals("CF_BaseValue", 456m, vatFee.CF_BaseValue);
					AssertEquals("CF_ChargeAmount", 100.32m, vatFee.CF_ChargeAmount);
		...
		CusEntryLineFee GetSystemDefinedVatFee(CusEntryLine entryLine) => entryLine.Fees.Cast<CusEntryLineFee>().SingleOrDefault(x => x.CF_ChargeType == UniversalReferenceConstants.RefCusRateCodes.Vat && x.CF_RateOverrideReasonCode.IsEmpty);
	}
}
