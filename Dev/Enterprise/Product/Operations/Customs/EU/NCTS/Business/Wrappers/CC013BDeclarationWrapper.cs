...
{
	public class CC013BDeclarationWrapper : DeclarationWrapper, ICC013BDeclaration
	{
		public CC013BDeclarationWrapper(NctsHeader nctsHeader) : base(nctsHeader)
		{
		...
		public ZBool ContainerisedIndicator => departureMovementHeader.IsContainerised;

		public ZString DialogLanguageIndicatorAtDeparture => ZString.Empty;

		public ZString NctsAccompanyingDocumentLanguageCode
		{
			get
			{
		...
		public ZLong SealsNumber => SealsSelected.Length;

		public IReadOnlyCollection<ISealID> Seals
		{
			get
			{
				if (seals == null)
				{
					seals = Array.Empty<ISealID>();
					switch (departureMovementHeader.BM_SealType)
					{
						case SealTypeList.Codes.ContainerSeal:
							seals = SealsSelected.Select(x => new SealWrapper(x)).Cast<ISealID>().ToArray();
							break;
						case SealTypeList.Codes.PackageSeal:
							seals = nctsHeader.Seals.Cast<Seal>().Select(x => new SealWrapper(x.CY_Data)).Cast<ISealID>().ToArray();
							break;
					}
				}
				return seals;
		...
		public IReadOnlyCollection<IDepartureGoodsItem> GoodsItems
			=> goodsItems ??= EffectiveDepartureGoodsItems.Cast<NctsDepartureCargoDesc>().Select(line => new DepartureGoodsItemWrapper(line)).OrderBy(line => line.ItemNumber).ToArray<IDepartureGoodsItem>();
		IReadOnlyCollection<IDepartureGoodsItem> goodsItems;

		public ITrader Carrier => null;

		ZString[] SealsSelected => sealsSelected ?? (sealsSelected = nctsHeader.GetSealsSelected());
