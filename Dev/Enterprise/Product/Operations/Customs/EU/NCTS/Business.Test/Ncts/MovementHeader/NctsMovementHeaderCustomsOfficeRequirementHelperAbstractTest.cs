...
{
	[TestsSubclassesOf(typeof(NctsMovementHeaderCustomsOfficeRequirementHelper))]
	public abstract class NctsMovementHeaderCustomsOfficeRequirementHelperAbstractTest<T> : TestCaseWithFactory
		where T : NctsMovementHeaderCustomsOfficeRequirementHelper
	{
		...
		{
			foreach (var role in CustomsOfficeRequirementWithRoles())
			{
		...
		protected virtual IEnumerable<CustomsOfficeRequirement> ExpectedOtherRequirements
		{
			get
			{
				...
				{
					new CustomsOfficeRequirement(OfficeCodes_NCTS.Codes.NCTSOfficeOfDeparture, true, false, "NCTS Office of departure"),
					new CustomsOfficeRequirement(OfficeCodes_NCTS.Codes.NCTSOfficeOfDestination, true, false, "NCTS Office of destination"),
					new CustomsOfficeRequirement(OfficeCodes_NCTS.Codes.NCTSOfficeOfTransit, false, false, "NCTS Office of transit") { MaxOfficeCountLimit = 9 },
					new CustomsOfficeRequirement(OfficeCodes_NCTS.Codes.NCTSOfficeOfExitForTransit, false, false, "NCTS Office of exit for transit") { MaxOfficeCountLimit = 9 }
				};
				return list;
			}
		}

		protected virtual IEnumerable<ZString> CustomsOfficeRequirementWithRoles() => new ZString[]
		{
