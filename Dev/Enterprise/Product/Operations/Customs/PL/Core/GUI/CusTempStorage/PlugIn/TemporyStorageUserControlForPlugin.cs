...
public partial class TemporyStorageUserControlForPlugin : ZUserControl
{
	public TemporyStorageUserControlForPlugin()
	{
		InitializeComponent();
	...
	void InitializeComponentExtend()
	{
		...
		CargoWise.ComponentModel.Design.CompileTimeCheckBindingMember.Check(((Messaging.Business.IEDIMessageCollectionProvider)(((CusTempStorageJobHeader)(null)).CusTempStorageDecs)));
	...
	#region Messages
	string NotToCreateJobText => Res.GetString("PLCusTempStorageDec|NotToCreateJobText", "You have chosen not to create a Declaration now.\r\nPlease change to another tab. You can click back to this tab when/if you need to create a Declaration.");
	#endregion
