...
sealed class ImportJobDeclarationLookupsTest : BusinessObjectLookupsTestCase
{
	public void TestGoodsOrigin()
	{
		...
		helper.CreateCusCodeList(Core.Constants.CountryCodes.Poland, EURefCusCodeListTypes.Code_IM15, "AA", "Test AA", ZDateTime.BrettsBirthday, ZDateTime.Now.AddMonths(2));
	...
	public void TestGoodsDestination()
	{
		...
		helper.CreateCusCodeList(Core.Constants.CountryCodes.Poland, EURefCusCodeListTypes.Code_IM17, "AA", "Test AA", ZDateTime.BrettsBirthday, ZDateTime.Now.AddMonths(2));
		...
		{
			jobDeclaration.JE_EntryStyle = EntryStyleListImport.Codes.ImportNormal;
			AssertEquals("IM Entry Style", "AA", ((CodeDescriptionPairList)lookups.GoodsDestination).CodesAsString);

			jobDeclaration.JE_EntryStyle = EntryStyleListImport.Codes.ImportFromSpecialTerritory;
			AssertEquals("CO Entry Style", "CC", ((CodeDescriptionPairList)lookups.GoodsDestination).CodesAsString);

			jobDeclaration.JE_EntryStyle = EntryStyleListImport.Codes.ImportFromEFTAMember;
			AssertEquals("EU Entry Style", "EE", ((CodeDescriptionPairList)lookups.GoodsDestination).CodesAsString);
		});
	}
