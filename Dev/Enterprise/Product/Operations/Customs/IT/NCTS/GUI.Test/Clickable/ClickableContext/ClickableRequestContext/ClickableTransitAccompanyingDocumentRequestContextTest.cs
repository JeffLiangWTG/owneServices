...
[TestedType(typeof(ClickableTransitAccompanyingDocumentRequestContext))]
sealed class ClickableTransitAccompanyingDocumentRequestContextTest : ClickableRequestContextAbstractTest
{
	...
	public void TestExecute_CreateTadRequestMessage()
	{
		nctsHeader.MovementReferenceEntryNumber.CE_EntryNum = "24ITQ0B8TEK17951J4";
		SetupCryptokiCertificate();

		var clickableContextWithPin = new ClickableTransitAccompanyingDocumentRequestContextForTest(nctsHeader, "PQE123");
		ZFormModaliser.ResultToReturnFromShowDialog = DialogResult.OK;

		clickableContextWithPin.Execute(GetClickableItem(clickableContextWithPin));
		AssertRequestCreation(nctsHeader, EDIMessageTypeList.Codes.TadRequest, "TAD Request has been sent to customs.");
	}

	public void TestExecute_CreateTadRequestMessage_FormPreSaved()
	{
	...
	public void TestExecute_CreateTadRequestMessage_AskForPinIfNotFoundInMemory()
	{
		SetupCryptokiCertificate();
		clickableContext.Execute(GetClickableItem());
	...
	public void TestExecute_CreateTadRequestMessage_DoesNotAskPinWhenUserHasAutomaticSignature()
	{
		SetupCryptokiCertificate();
	...
	protected override void SetUp()
	{
		...
		clickableContext = new ClickableTransitAccompanyingDocumentRequestContextForTest(nctsHeader);
	}
	...
	protected override string GetExpectedName() => "TadRequest";
	protected override string GetExpectedCaption() => "TAD Request";
}

sealed class ClickableTransitAccompanyingDocumentRequestContextForTest : ClickableTransitAccompanyingDocumentRequestContext
{
	public ClickableTransitAccompanyingDocumentRequestContextForTest(NctsHeader header, string certificatePin = null) : base(header)
	{
	...
	protected override IDocumentManagementServiceRequestContext<NctsHeader, NctsDepartureMovementHeader> CreateRequestContext(NctsHeader nctsHeader)
	{
		...
		_ = contextMock.Setup(ctx => ctx.MessageParent).Returns(nctsHeader.MovementHeader);
		_ = contextMock.Setup(ctx => ctx.BusinessObject).Returns(nctsHeader);
		_ = contextMock.Setup(ctx => ctx.CryptokiCertificate).Returns(cryptokeiCertificate);
		_ = contextMock.Setup(ctx => ctx.MauCertificate).Returns(mauCertificate);
		_ = contextMock.Setup(ctx => ctx.ServiceId).Returns("SERVICE_ID_TAD");
		_ = contextMock.Setup(ctx => ctx.XmlSigner).Returns(xmlSignerMock.Object);
