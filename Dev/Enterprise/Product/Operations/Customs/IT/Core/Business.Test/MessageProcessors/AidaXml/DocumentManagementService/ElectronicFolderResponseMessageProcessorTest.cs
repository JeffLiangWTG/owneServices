using System.Collections.Generic;
using CargoWise.Types;
using Enterprise.BatchProcessor;

namespace Enterprise.Customs.IT.Business.Testing;

sealed class ElectronicFolderResponseMessageProcessorTest : XmlIncomingMessageProcessorTest<ElectronicFolderResponseMessageProcessor>
{
	public void TestMessage_WithCodeZero()
	{
		var (_, entryHeader, _, receivedMessage) = PrepareTestData(messageText: responseWithCodeZero, messageType: MessageProcessorConstants.InterchangeTypes.ElectronicFolderResponseType);
		var responseMessageProcessor = GetMessageProcessor(logger);
		responseMessageProcessor.ProcessMessage(receivedMessage);
		AssertEquals(nameof(entryHeader.CustomsChannel), "VM", entryHeader.CustomsChannel);
	}

	public void TestMessage_WithCode_D_024()
	{
		var (_, entryHeader, _, receivedMessage) = PrepareTestData(messageText: responseWithCodeDZero24, messageType: MessageProcessorConstants.InterchangeTypes.ElectronicFolderResponseType);
		var processor = GetMessageProcessor(logger);
		processor.ProcessMessage(receivedMessage);
		AssertEquals(nameof(entryHeader.CustomsChannel), "CA", entryHeader.CustomsChannel);
	}

	protected override IReadOnlyList<ZString> ExpectedMessageTypesToInclude => new ZString[] { MessageProcessorConstants.InterchangeTypes.ElectronicFolderResponseType };

	protected override ElectronicFolderResponseMessageProcessor GetMessageProcessor(LoggingInformation logger)
	{
		return new ElectronicFolderResponseMessageProcessor(logger);
	}

	const string responseWithCodeZero = @"<soapenv:Envelope xmlns:soapenv=""http://schemas.xmlsoap.org/soap/envelope/"">
   <soapenv:Body>
      <ns2:Output xmlns:ns2=""http://ws.sogei.it/output/"" xmlns=""http://ponimport.ssi.sogei.it/type/"">
         <ns2:IUT>20220530D12000451314</ns2:IUT>
         <ns2:esito>
            <ns2:codice>199</ns2:codice>
            <ns2:messaggio>Elaborazione OK: completata senza esito finale</ns2:messaggio>
         </ns2:esito>
         <ns2:data></ns2:data>
         <ns2:dataRegistrazione>2022-05-30+02:00</ns2:dataRegistrazione>
      </ns2:Output>
   </soapenv:Body>
</soapenv:Envelope>";

	const string responseWithCodeDZero24 = @"<soapenv:Envelope xmlns:soapenv=""http://schemas.xmlsoap.org/soap/envelope/"">
   <soapenv:Body>
      <ns2:Output xmlns:ns2=""http://ws.sogei.it/output/"" xmlns=""http://ponimport.ssi.sogei.it/type/"">
         <ns2:IUT>20220905D12000630339</ns2:IUT>
         <ns2:esito>
            <ns2:codice>197</ns2:codice>
            <ns2:messaggio>Elaborazione KO: senza esito</ns2:messaggio>
         </ns2:esito>
         <ns2:data></ns2:data>
         <ns2:dataRegistrazione>2022-09-05+02:00</ns2:dataRegistrazione>
      </ns2:Output>
   </soapenv:Body>
</soapenv:Envelope>";
}
