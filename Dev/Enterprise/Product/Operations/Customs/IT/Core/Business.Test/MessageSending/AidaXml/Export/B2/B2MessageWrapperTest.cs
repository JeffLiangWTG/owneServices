...
sealed class B2MessageWrapperTest : B2MessageWrapperTestBase
{
	public override void TestConstructor()
	{
		AssertExceptionThrown<ArgumentNullException>("When EntryHeader is null", () => new B2MessageWrapper(entryHeader: null, exportMessageSendingWrapperFactory));
		AssertExceptionThrown<ArgumentNullException>("When ExportMessagingSendingWrapperFactory is null", () => new B2MessageWrapper(EntryHeader, exportMessageSendingWrapperFactory: null));
	}

	public override void TestMessageHeader()
	{
		var messageWrapper = CreateWrapper();
		AssertType<B2HeaderWrapper>("MessageHeader Type", messageWrapper.MessageHeader);
	}
	...
	public override void TestItems()
	{
		...
		AssertType<B2ItemWrapper>("Item Type", items.Single());
	}
	...
	public void TestAmendment()
	{
		...
		messageWrapper = new B2MessageWrapper(EntryHeader, exportMessageSendingWrapperFactory, amendmentMock.Object);
		AssertNotNull("Amendment", messageWrapper.MessageHeader?.Amendment);
	}

	protected override IB2Message CreateWrapper() => new B2MessageWrapper(EntryHeader, exportMessageSendingWrapperFactory);
