...
sealed class ElectronicFolderResponseMessageTest : TestCase
{
	public void TestResponseBodyStatus()
	{
		var status = responseMessage.ResponseStatus;
		AssertEquals("Status", "199", status);
	}
	...
	{
		var wrapper = responseMessage.GetResponseWrapper();
		AssertNotNull("Wrapper", wrapper);
		AssertType<ElectronicFolderResponseMessageWrapper>("wrapper type", wrapper);
	}

	protected override void SetUp()
	{
		base.SetUp();
		responseMessage = new ElectronicFolderResponseMessage(soapMessage);
	}

	ElectronicFolderResponseMessage responseMessage;
