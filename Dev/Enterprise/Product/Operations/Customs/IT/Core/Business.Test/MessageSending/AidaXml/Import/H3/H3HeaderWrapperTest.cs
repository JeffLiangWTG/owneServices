...
sealed class H3HeaderWrapperTest : H3HeaderWrapperTestBase
{
	public override void TestConstructor()
	{
		AssertExceptionThrown<ArgumentNullException>(() => new H3HeaderWrapper(entryHeader: null, messageSendingWrapperFactory));
		...
		AssertExceptionThrown<ArgumentNullException>("Expected exception when parent declaration is null", () => new H3HeaderWrapper(entryHeaderWithNoParentDeclaration, messageSendingWrapperFactory));

		var entryHeaderWithNoEntryInstruction = Factory.New<JobDeclaration>().CustomsEntryHeaders.AddNew();
		AssertExceptionThrown<ArgumentNullException>("Expected exception when related Entry Instruction is null", () => new H3HeaderWrapper(entryHeaderWithNoEntryInstruction, messageSendingWrapperFactory));
	}
	...
	public override void TestAmendment()
	{
		...
		wrapper = new H3HeaderWrapper(EntryHeader, messageSendingWrapperFactory, amendmentMock.Object);
		AssertNotNull(nameof(IH3Header.Amendment), wrapper.Amendment);
	}

	public override void TestDeclarationCustomsOffice()
	{
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.DeclarationCustomsOffice), wrapper.DeclarationCustomsOffice);

		declarationWrapperMock.Setup(d => d.DeclarationCustomsOffice).Returns("000000");
		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.DeclarationCustomsOffice), "000000", wrapper.DeclarationCustomsOffice);
	}
	...
	public override void TestAdditionalDeclarationType()
	{
		...
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.AdditionalDeclarationType), wrapper.AdditionalDeclarationType);

		entryInstructionWrapperMock.Setup(e => e.AdditionalDeclarationType).Returns("D");
		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.AdditionalDeclarationType), "D", wrapper.AdditionalDeclarationType);
	}

	public override void TestSignerFiscalCode()
	{
		var wrapper = CreateWrapper();
		AssertNullOrEmpty($"When the current user does not have a COD certificate, {nameof(IH3Header.SignerFiscalCode)}", wrapper.SignerFiscalCode);
		...
		AssertNullOrEmpty($"When the current user has a COD certificate with empty number, {nameof(IH3Header.SignerFiscalCode)}", wrapper.SignerFiscalCode);

		codCertificate.XZ_RefNumber = "ITCOD";
		wrapper = CreateWrapper();
		AssertEquals($"When the current user has a COD certificate with filled number, {nameof(IH3Header.SignerFiscalCode)}", "ITCOD", wrapper.SignerFiscalCode);
	...
	{
		var wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.Lrn), ITEDIMessage.ITMessageNumberPlaceholder, wrapper.Lrn);
	}
	...
	public override void TestUcr()
	{
		...
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.Ucr), wrapper.Ucr);

		declarationWrapperMock.Setup(d => d.Ucr).Returns("UCR");
		AssertEquals(nameof(IH3Header.Ucr), "UCR", wrapper.Ucr);
	}
	...
	public override void TestDeferredPayment()
	{
		...
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.DeferredPayment), wrapper.DeferredPayment);

		entryHeaderWrapperMock.Setup(d => d.DeferredPayment).Returns("123456A");
		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.DeferredPayment), "123456A", wrapper.DeferredPayment);
	}

	public override void TestWarehouse()
	{
		var wrapper = CreateWrapper();
		AssertNull(nameof(IH3Header.Warehouse), wrapper.Warehouse);

		var warehouseMock = new Mock<IWarehouse>();
		entryInstructionWrapperMock.Setup(e => e.Warehouse).Returns(warehouseMock.Object);

		wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.Warehouse), wrapper.Warehouse);
	}

	public override void TestPreviousDocuments()
	{
		var wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.PreviousDocuments), wrapper.PreviousDocuments);
		AssertEquals($"{nameof(IH3Header.PreviousDocuments)} count", 0, wrapper.PreviousDocuments.Count);
		...
		AssertEquals($"{nameof(IH3Header.PreviousDocuments)} count", 1, previousDocumentWrappers.Count);
	}

	public override void TestSupportingDocuments()
	{
		var wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.SupportingDocuments), wrapper.SupportingDocuments);
		AssertEquals($"{nameof(IH3Header.SupportingDocuments)} count", 0, wrapper.SupportingDocuments.Count);
	...
	{
		var wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.AdditionalInformation), wrapper.AdditionalInformation);
		...
		wrapper = CreateWrapper();
		AssertEquals($"{nameof(IH3Header.AdditionalInformation)} count", 1, wrapper.AdditionalInformation.Count);
	}

	public override void TestSupplier()
	{
		var wrapper = CreateWrapper();
		AssertNull(nameof(IH3Header.Supplier), wrapper.Supplier);

		var supplier = TraderWrapperTest.SetupTrader("SUPPLIER");
		declarationWrapperMock.Setup(d => d.Supplier).Returns(supplier.Object);

		wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.Supplier), wrapper.Supplier);
	}

	public override void TestImporter()
	{
		var wrapper = CreateWrapper();
		AssertNull(nameof(IH3Header.Importer), wrapper.Importer);

		var importer = TraderWrapperTest.SetupTrader("IMPORTER");
		declarationWrapperMock.Setup(d => d.Importer).Returns(importer.Object);

		wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.Importer), wrapper.Importer);
	}

	public override void TestDeclarant()
	{
		var wrapper = CreateWrapper();
		AssertNull(nameof(IH3Header.Declarant), wrapper.Declarant);
		...
		wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.Declarant), wrapper.Declarant);
	}

	public override void TestRepresentative()
	{
		var wrapper = CreateWrapper();
		AssertNull(nameof(IH3Header.Representative), wrapper.Representative);
		...
		wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.Representative), wrapper.Representative);
	}

	public override void TestAdditionalSupplyChainActors()
	{
		var wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.AdditionalSupplyChainActors), wrapper.AdditionalSupplyChainActors);
		AssertEquals(nameof(IH3Header.AdditionalSupplyChainActors), 0, wrapper.AdditionalSupplyChainActors.Count);
		...
		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.AdditionalSupplyChainActors), 1, wrapper.AdditionalSupplyChainActors.Count);
	}

	public override void TestGuaranteeHolderIdentificationNumber()
	{
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.GuaranteeHolderIdentificationNumber), wrapper.GuaranteeHolderIdentificationNumber);

		entryInstructionWrapperMock.Setup(e => e.GetGuaranteeHolderIdentificationNumber(It.IsAny<IEoriTrader>())).Returns("IT1234766767");
		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.GuaranteeHolderIdentificationNumber), "IT1234766767", wrapper.GuaranteeHolderIdentificationNumber);
	}

	public override void TestAuthorizations()
	{
		var wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.Authorizations), wrapper.Authorizations);
		AssertEquals($"{nameof(IH3Header.Authorizations)} count", 0, wrapper.Authorizations.Count);
		...
		AssertEquals($"{nameof(IH3Header.Authorizations)} count", 1, authorizations.Count);
	}

	public override void TestTermsOfDelivery()
	{
		var wrapper = CreateWrapper();
		AssertNull(nameof(IH3Header.TermsOfDelivery), wrapper.TermsOfDelivery);
		...
		wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.TermsOfDelivery), wrapper.TermsOfDelivery);
	}

	public override void TestInvoiceCurrencyCode()
	{
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.InvoiceCurrencyCode), wrapper.InvoiceCurrencyCode);

		entryHeaderWrapperMock.Setup(e => e.InvoiceCurrencyCode).Returns("AUD");

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.InvoiceCurrencyCode), "AUD", wrapper.InvoiceCurrencyCode);
	}

	public override void TestInvoiceTotalAmount()
	{
		var wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.InvoiceTotalAmount), 0m, wrapper.InvoiceTotalAmount);

		entryHeaderWrapperMock.Setup(e => e.InvoiceTotalAmount).Returns(334m);

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.InvoiceTotalAmount), 334m, wrapper.InvoiceTotalAmount);
	}
	...
	public override void TestInternalCurrencyUnit()
	{
		...
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.InternalCurrencyUnit), wrapper.InternalCurrencyUnit);

		GlbCompany.CurrentCompany.GC_RX_NKLocalCurrency = "CHF";
		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.InternalCurrencyUnit), "CHF", wrapper.InternalCurrencyUnit);
	}

	public override void TestExchangeRate()
	{
		var wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.ExchangeRate), 0m, wrapper.ExchangeRate);

		entryHeaderWrapperMock.Setup(e => e.ExchangeRate).Returns(1.1239m);

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.ExchangeRate), 1.1239m, wrapper.ExchangeRate);
	}

	public override void TestCountryOfDestination()
	{
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.CountryOfDestination), wrapper.CountryOfDestination);

		declarationWrapperMock.Setup(d => d.GoodsCountryOfDestination).Returns("TR");

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.CountryOfDestination), "TR", wrapper.CountryOfDestination);
	}

	public override void TestRegionOfDestination()
	{
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.RegionOfDestination), wrapper.RegionOfDestination);

		declarationWrapperMock.Setup(d => d.RegionOfDestination).Returns("XX");

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.RegionOfDestination), "XX", wrapper.RegionOfDestination);
	}

	public override void TestCountryOfDispatch()
	{
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.CountryOfDispatch), wrapper.CountryOfDispatch);

		declarationWrapperMock.Setup(d => d.GoodsCountryOfOrigin).Returns("DE");

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.CountryOfDispatch), "DE", wrapper.CountryOfDispatch);
	}

	public override void TestLocationOfGoods()
	{
		var wrapper = CreateWrapper();
		AssertNull(nameof(IH3Header.LocationOfGoods), wrapper.LocationOfGoods);
		...
		wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.LocationOfGoods), wrapper.LocationOfGoods);
	}

	public override void TestPresentationCustomsOffice()
	{
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.PresentationCustomsOffice), wrapper.PresentationCustomsOffice);

		declarationWrapperMock.Setup(d => d.PresentationCustomsOffice).Returns("DE123123");

		wrapper = CreateWrapper();

		AssertEquals(nameof(IH3Header.PresentationCustomsOffice),
			"DE123123",
	...
	public override void TestSupervisingCustomsOffice()
	{
		...
		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.SupervisingCustomsOffice), "LV002000", wrapper.SupervisingCustomsOffice);
	}

	public override void TestAcceptanceDate()
	{
		var wrapper = CreateWrapper();
		AssertNull(nameof(IH3Header.AcceptanceDate), wrapper.AcceptanceDate);
		...
		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.AcceptanceDate), testSimpleDecAcceptanceDate, wrapper.AcceptanceDate);
	}

	public override void TestGrossMass()
	{
		var wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.GrossMass), 0m, wrapper.GrossMass);

		entryHeaderWrapperMock.Setup(e => e.GrossMass).Returns(23.0563m);

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.GrossMass), 23.0563m, wrapper.GrossMass);
	}

	public override void TestNumberOfPackages()
	{
		var wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.NumberOfPackages), 0, wrapper.NumberOfPackages);

		entryHeaderWrapperMock.Setup(e => e.NumberOfPackages).Returns(22);

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.NumberOfPackages), 22, wrapper.NumberOfPackages);
	}

	public override void TestContainerMode()
	{
		declarationWrapperMock.Setup(d => d.ContainerModeForImportMessage).Returns(1);
		var h1HeaderWrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.ContainerMode), 1, h1HeaderWrapper.ContainerMode);
	}

	public override void TestBorderMeansOfTransportMode()
	{
		var wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.BorderMeansOfTransportMode), 0, wrapper.BorderMeansOfTransportMode);

		declarationWrapperMock.Setup(d => d.BorderTransportMode).Returns(4);

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.BorderMeansOfTransportMode), 4, wrapper.BorderMeansOfTransportMode);
	}

	public override void TestInlandTransportMode()
	{
		var wrapper = CreateWrapper();
		AssertNull(nameof(IH3Header.InlandTransportMode), wrapper.InlandTransportMode);

		declarationWrapperMock.Setup(d => d.InlandTransportMode).Returns(1);

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.InlandTransportMode), 1, wrapper.InlandTransportMode);
	}

	public override void TestBorderMeansOfTransportNationality()
	{
		var wrapper = CreateWrapper();
		AssertNullOrEmpty(nameof(IH3Header.BorderMeansOfTransportNationality), wrapper.BorderMeansOfTransportNationality);

		declarationWrapperMock.Setup(d => d.BorderMeansOfTransportNationality).Returns("IT");

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.BorderMeansOfTransportNationality), "IT", wrapper.BorderMeansOfTransportNationality);
	}

	public override void TestArrivalMeansOfTransport()
	{
		var wrapper = CreateWrapper();
		AssertNull(nameof(IH3Header.ArrivalMeansOfTransport), wrapper.ArrivalMeansOfTransport);
	}

	public override void TestGuarantees()
	{
		var wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.Guarantees), wrapper.Guarantees);
		AssertEquals($"{nameof(IH3Header.Guarantees)} count", 0, wrapper.Guarantees.Count);
		...
		AssertEquals($"{nameof(IH3Header.Guarantees)} count", 1, guaranteesWrappers.Count);
	}

	public override void TestGuaranteeTypes()
	{
		var wrapper = CreateWrapper();
		AssertNotNull(nameof(IH3Header.GuaranteeTypes), wrapper.GuaranteeTypes);
		AssertEquals($"{nameof(IH3Header.GuaranteeTypes)} count", 0, wrapper.GuaranteeTypes.Count);
	...
	{
		var wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.NatureOfTransaction), 0, wrapper.NatureOfTransaction);

		entryHeaderWrapperMock.Setup(e => e.NatureOfTransaction).Returns(11);

		wrapper = CreateWrapper();
		AssertEquals(nameof(IH3Header.NatureOfTransaction), 11, wrapper.NatureOfTransaction);
	}

	protected override IH3Header CreateWrapper() => new H3HeaderWrapper(EntryHeader, messageSendingWrapperFactory);
