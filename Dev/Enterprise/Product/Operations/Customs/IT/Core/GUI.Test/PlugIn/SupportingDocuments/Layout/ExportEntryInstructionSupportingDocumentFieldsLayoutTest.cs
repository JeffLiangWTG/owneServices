...
[TestedType(typeof(ExportEntryInstructionSupportingDocumentFieldsLayout))]
sealed class ExportEntryInstructionSupportingDocumentFieldsLayoutTest : LayoutsAbstractTest
{
	public void TestReferenceNumberCodeVisibility()
	{
		Factory.CreateSupportingDocumentCodeLists(
			new TestSupportingDocumentCodeList("2800", isImport: false, hasPermitAttribute: true),
			new TestSupportingDocumentCodeList("3200", isImport: false, hasPermitAttribute: false));
		Factory.Save();
		...
		using (var userControl = new ExportEntryInstructionLayoutSupportingDocumentsFieldsControl(declaration))
		{
