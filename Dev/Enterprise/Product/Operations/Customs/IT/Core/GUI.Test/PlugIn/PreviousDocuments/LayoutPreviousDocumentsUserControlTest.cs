...
sealed class LayoutPreviousDocumentsUserControlTest : EU.GUI.PlugIn.Testing.PreviousDocumentUserControlAbstractTest<LayoutPreviousDocumentsUserControl, JobDeclaration>
{
	public void TestPreviousDocumentsFields_InvoiceHeader() => TestPreviousDocumentsFields(nameof(JobDeclaration.Invoices));
	...
	IEnumerable<(string, Type)> GetFieldWithTypes()
	{
		return new[]
		{
			...
			("Quantity3CalcDropEdit", typeof(ZCalcDropEdit)),
	...
	protected override IEnumerable<(string, Type)> GetOrderedGridColumns()
	{
		return new[]
		{
			...
			(PreviousDocument.Schema.CSI_Quantity3, typeof(ZCalcEditColumnStyle)),
			(PreviousDocument.Schema.CSI_UnitOfQuantity3, typeof(ZDropEditColumnStyle)),
