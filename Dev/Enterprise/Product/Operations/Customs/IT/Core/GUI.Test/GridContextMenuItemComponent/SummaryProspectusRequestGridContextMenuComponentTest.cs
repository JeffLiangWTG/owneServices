...
[TestedType(typeof(SummaryProspectusRequestGridContextMenuComponent))]
sealed class SummaryProspectusRequestGridContextMenuComponentTest : GridContextMenuItemComponentAbstractTest<CusEntryHeader>
{
	public void TestConstructor_ParentMenuItem()
	{
		using var entriesGrid = new EntriesGridForTest(declaration);
		AssertExceptionThrown<ArgumentNullException>("parentMenuItem parameter is required", () => new SummaryProspectusRequestGridContextMenuComponent(entriesGrid, null));
	}

	public void TestSummaryProspectusRequestContextMenuItemVisibility()
	{
		...
		new SummaryProspectusRequestGridContextMenuComponentForTest(entriesGrid, parentMenuItem: parentMenuItem).Initialize();
	...
	public void TestSummaryProspectusRequestContextMenuItemEnabled()
	{
		using var entriesGrid = new EntriesGridForTest(declaration);
		var parentMenuItem = entriesGrid.ContextMenu.MenuItems.Add("Request to Customs");
		new SummaryProspectusRequestGridContextMenuComponentForTest(entriesGrid, parentMenuItem: parentMenuItem).Initialize();
	...
	public void TestCreateSummaryProspectusRequestMessage()
	{
		entryHeader.MovementReferenceNumberSetter("MRN123434");
		...
		using var entriesGrid = new EntriesGridForTest(declaration);
		var parentMenuItem = entriesGrid.ContextMenu.MenuItems.Add("Request to Customs");
		new SummaryProspectusRequestGridContextMenuComponentForTest(entriesGrid, "PQE123", parentMenuItem).Initialize();
		ZFormModaliser.ResultToReturnFromShowDialog = DialogResult.OK;

		var menuItem = parentMenuItem.MenuItems.FindByText(MenuItemText);
		menuItem.PerformClick();
		AssertEquals("Summary Prospectus Request has been sent to customs.", UnitTestUserNotification.Instance.LastMessage.Text);

		entryHeader.Messages.Reload(reLoadExistingRows: false);
		var summaryProspectusMessage = entryHeader.Messages.GetLastMessageByType("SPR");
		AssertNotNull("Summary Prospectus Message", summaryProspectusMessage);
	}

	public void TestCreateSummaryProspectusRequestMessage_AskForPinIfNotFoundInMemory()
	{
		entryHeader.MovementReferenceNumberSetter("MRN123434");
		SetupCryptokiCertificate();
	...
	public void TestCreateSummaryProspectusRequestMessage_DeclarationPendingChanges()
	{
		entryHeader.MovementReferenceNumberSetter("MRN123434");
		SetupCryptokiCertificate();
	...
	public void TestCreateSummaryProspectusRequestMessage_DoesNotAskPinWhenUserHasAutomaticSignature()
	{
		entryHeader.MovementReferenceNumberSetter("MRN123434");
	...
	protected override string MenuItemText => "Summary Prospectus Request";

	protected override GridContextMenuItemComponent<CusEntryHeader> GetNewContextMenuItemComponent(ZGrid grid)
		=> new SummaryProspectusRequestGridContextMenuComponent(grid, new MenuItem("Request to Customs"));
...
sealed class SummaryProspectusRequestGridContextMenuComponentForTest : SummaryProspectusRequestGridContextMenuComponent
{
	public SummaryProspectusRequestGridContextMenuComponentForTest(ZGrid grid, string certificatePin = null, MenuItem parentMenuItem = null) : base(grid, parentMenuItem)
	{
	...
	protected override IDocumentManagementServiceRequestContext<CusEntryHeader, CusEntryHeader> CreateSummaryProspectusRequestContext(CusEntryHeader entryHeader)
	{
