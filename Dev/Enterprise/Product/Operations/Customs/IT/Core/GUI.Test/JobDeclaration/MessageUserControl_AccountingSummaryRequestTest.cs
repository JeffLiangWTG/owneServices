...
sealed class MessageUserControl_AccountingSummaryRequestTest : TestCaseWithFactory
{
	public void TestContextMenuIsAvailable()
	{
		...
		var accountingSummaryRequestMenuItem = requestToCustomsMenuItem.MenuItems.FindByText("Accounting summary request");

		entriesBoundGrid.Select(0);
		requestToCustomsMenuItem.ShowPopupMenu();
		AssertEquals("Visible", expected: true, accountingSummaryRequestMenuItem.Visible);
		AssertEquals("Enabled", expected: true, accountingSummaryRequestMenuItem.Enabled);
	}
}
