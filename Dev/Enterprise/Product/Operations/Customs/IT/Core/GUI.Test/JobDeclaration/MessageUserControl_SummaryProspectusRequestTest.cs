...
sealed class MessageUserControl_SummaryProspectusRequestTest : TestCaseWithFactory
{
	public void TestContextMenuIsAvailable()
	{
		...
		var summaryProspectusRequestMenuItem = requestToCustomsMenuItem.MenuItems.FindByText("Summary Prospectus Request");

		entriesBoundGrid.Select(0);
		requestToCustomsMenuItem.ShowPopupMenu();
		AssertEquals("Visible", expected: true, summaryProspectusRequestMenuItem.Visible);
		AssertEquals("Enabled", expected: true, summaryProspectusRequestMenuItem.Enabled);
	}
}
