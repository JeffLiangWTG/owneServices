...
{
	class ClearExpiredStockOperationalActionUserControlTest : TestCaseWithFactory
	{
		public void TestGridColumns()
		{
			var context = new ClearExpiredStockApplicator(Factory, () => ZDate.Today);
			context.Build(Array.Empty<ZGuid>());
			using (var form = new ZForm(context) { CaptionRenderingEnabled = true })
			using (var control = new ClearExpiredStockOperationalActionUserControl())
			{
				...
				AssertColumn(grid, "CustomsEntryNumber", "Customs Entry Number", 150);
				AssertColumn(grid, "OwnerReference", "Owner Reference", 100);
				AssertColumn(grid, "IntoBondDate", "Into Bond Date", 90);
			}
		}

		public void TestLayoutForLockedApplicator()
		{
			var lockedContext = new ClearExpiredStockApplicator(Factory, () => ZDate.Today);
			lockedContext.Build(Array.Empty<ZGuid>());

			var blockedContext = new ClearExpiredStockApplicator(Factory, () => ZDate.Today);
