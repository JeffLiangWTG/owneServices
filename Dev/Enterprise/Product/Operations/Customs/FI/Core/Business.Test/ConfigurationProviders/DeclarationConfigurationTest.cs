...
sealed class DeclarationConfigurationTest : EU.Business.Testing.DeclarationConfigurationAbstractTest<DeclarationConfiguration, InstructionConfiguration, InvoiceHeaderConfiguration, InvoiceLineConfiguration, EntryHeaderConfiguration, EntryLineConfiguration>
{
	...
	{
		AssertEquals(expected: false, configuration.IsUCC5(declaration));
	}
	...
	{
		AssertEquals("UCC6 is disabled when business object is not JobDeclaration", expected: false, configuration.IsUCC6(Factory.New<DummyBusinessObject>()));
		declaration.JE_MessageType = Common.Shared.SharedJobMessageTypeList.Codes.Export;
		AssertEquals("UCC6 is enabled for export", expected: true, configuration.IsUCC6(declaration));
		declaration.JE_MessageType = Common.Shared.SharedJobMessageTypeList.Codes.Import;
		AssertEquals("UCC6 is enabled for import", expected: true, configuration.IsUCC6(declaration));
	});
