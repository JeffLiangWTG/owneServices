...
{
	public class InvoiceLineApportionedChargeValidation : JobComInvHeaderChargeValidation
	{
		public InvoiceLineApportionedChargeValidation(InvoiceLineApportionedCharge parent)
			: base(parent)
		{
		}
		...
		protected override void CheckJ7_Percentage()
		{
			...
			else
			{
				if (Parent.J7_ChargeType.In((ZString)CustomsChargeTypeList.Codes.OverseasFreight, (ZString)CustomsChargeTypeList.Codes.OverseasInsurance))
				{
					...
					var charges = (Parent.Parent as JobComInvoiceLine).ApportionedCharges;
					decimal totalPercent = 0;
					foreach (InvoiceLineApportionedCharge item in charges)
					{
