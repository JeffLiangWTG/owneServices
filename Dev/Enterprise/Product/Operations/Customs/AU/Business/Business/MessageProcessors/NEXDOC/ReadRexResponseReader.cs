...
using Enterprise.Customs.AU.Declaration.Business.ReadRexSoap;
...
{
	public class ReadRexResponseReader : DataObjectReader
	{
		public ReadRexResponseReader(BusinessObjectFactory factory, LoggingInformation logger)
		...
		void ProcessProductionProcesses(JobComInvoiceHeader invoice, QuarantineExDocLine quarantineLine, ProductLineTypeProductionProcesses processes)
		{
			foreach (var process in processes.Items.Cast<ProductionProcessType>())
			{
				...
				using (processBO.GetValidationSuspender())
				{
					...
					processBO.EE_ProcessingType = process.processGroup;
					processBO.EE_EstablishmentIndicator = process.establishmentIndicator;

					if (process.Item is string processingEstablishmentNumber)
					{
						processBO.EE_AuthorisationEstablishmentID = processingEstablishmentNumber;
					}
					else if (process.Item is EstablishmentType processingEstablishment)
					{
						var docAddress = MatchAQISEstablishment(processingEstablishment.establishmentName, processingEstablishment.establishmentAddress, invoice.JobDeclaration);
						if (docAddress != null)
						{
							processBO.EE_E2_Address = docAddress.PK;
						}
					}
				}
			}
		...
		JobDocAddress MatchAQISEstablishment(ZString orgName, AddressType address, BusinessObject docAddressParent)
		{
			...
			return factory.GetCachedValue(ZString.Format("NEXDOCEstablishment+{0}+{1}+{2}+{3}", orgName, city, state, postCode), () =>
			{
				JobDocAddress docAddress = null;

				if (docAddressParent != null)
				{
					var orgPk = MatchOrg(orgName, address, OrganisationTypes.None | OrganisationTypes.Consignee);
					if (orgPk.IsValid)
					{
						var org = factory.Load<OrgHeader>(orgPk);
						var aqisAddress = org.Addresses.Cast<OrgAddress>().FirstOrDefault(a => checkCityFunc(a) && checkStateFunc(a) && checkPostcodeFunc(a))
							?? org.MainAddress;

						docAddress = JobDocAddress.New(docAddressParent, DocAddressType.AQISProcessingEstablishment);
						docAddress.E2_OA_Address = aqisAddress.PK;
					}
				}
				return docAddress;
			});
