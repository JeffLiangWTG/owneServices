...
{
	public class ReadRexResponseReaderRC5 : DataObjectReader
	{
		public ReadRexResponseReaderRC5(BusinessObjectFactory factory, LoggingInformation logger)
		...
		QuarantineExDocHeader PopulateQuarantineHeader(JobComInvoiceHeader invoice, ReadRexResponse readRexResponse)
		{
			...
			using (new CopySuspender(quarantineHeader))
			{
				...
				if (exportDetails != null)
				{
					...
					var exportDetailsItem = exportDetails.Item;
					switch (exportDetailsItem)
					{
						case WoolExportDetails woolExportDetails:
							PopulateQuarantineHeader_WoolExportDetails(quarantineHeader, woolExportDetails);
							break;
						case SkinsAndHidesExportDetails skinsAndHidesExportDetails:
							PopulateQuarantineHeader_SkinsAndHidesExportDetails(invoice, quarantineHeader, skinsAndHidesExportDetails);
							break;
						default:
							PopulateQuarantineHeader_ExportDetails(quarantineHeader, transportDetails, exportDetailsItem as FishExportDetails);
							break;
					}
				}
		...
		void PopulateQuarantineHeader_ExportDetails(QuarantineExDocHeader quarantineHeader, TransportDetailsType transportDetails, FishExportDetails fishExportDetails)
		{
		...
		void PopulateQuarantineHeader_WoolExportDetails(QuarantineExDocHeader quarantineHeader, WoolExportDetails woolExportDetails)
		{
			if (woolExportDetails.packDateSpecified)
			{
				quarantineHeader.QH_PackDate = woolExportDetails.packDate;
			}
		}

		void PopulateQuarantineHeader_SkinsAndHidesExportDetails(JobComInvoiceHeader invoice, QuarantineExDocHeader quarantineHeader, SkinsAndHidesExportDetails skinsAndHidesExportDetails)
		{
			if (skinsAndHidesExportDetails.packDateSpecified)
			{
				quarantineHeader.QH_PackDate = skinsAndHidesExportDetails.packDate;
			}
			if (skinsAndHidesExportDetails.loadingDateSpecified)
			{
				quarantineHeader.QH_LoadingDate = new ZDateTime(skinsAndHidesExportDetails.loadingDate).Date;
			}
			if (!string.IsNullOrEmpty(skinsAndHidesExportDetails.loadingEstablishment))
			{
				invoice.AQISLoadingEstablishmentLocation.EXDOCEstablishmentNumber = skinsAndHidesExportDetails.loadingEstablishment;
			}
		}
		...
		JobComInvoiceLine PopulateLine(JobComInvoiceHeader invoice, ProductLineType productLine, ImportPermitType permit, IEnumerable<CertificateLineType> certificateLines, IEnumerable<ManufacturerLineType> manufacturerLines)
		{
			...
			using (new CopySuspender(quarantineLine))
			{
				...
				if (GetItemSafe(productLineItems, ItemsChoiceType2.skinsAndHidesProductLineDetails) is SkinsAndHidesProductLineDetails skinsAndHidesProductLineDetails)
				{
					ProcessSkinsAndHidesProductLineDetails(quarantineLine, skinsAndHidesProductLineDetails);
				}
			}
		...
		internal void ProcessSkinsAndHidesProductLineDetails(QuarantineExDocLine quarantineLine, SkinsAndHidesProductLineDetails skinsAndHidesProductLineDetails)
		{
			if (skinsAndHidesProductLineDetails.saltingDateSpecified)
			{
				quarantineLine.QL_SaltingDate = skinsAndHidesProductLineDetails.saltingDate;
			}
		}
