...
{
	sealed class WARRELMessageBuilderTest : CMRMessageBuilderAbstractTest
	{
		public void TestOriginalWarehouseReleaseMessage()
		{
			var expectedMessage = CMRExportMessagesTestData.WARRELOriginal;
			var builder = new WARRELMessageBuilder(declaration);
		...
		public void TestReplacelWarehouseReleaseMessage()
		{
			var expectedMessage = CMRExportMessagesTestData.WARRELReplace;
			var builder = new WARRELMessageBuilder(declaration);
			builder.MessageSubType = Common.MessageBuilders.MessageSubTypes.Replace;
			AssertEquals("MessageString", expectedMessage, builder.MessageText);
		}

		public void TestWithdrawWarehouseReleaseMessage()
		{
			var expectedMessage = CMRExportMessagesTestData.WARRELWithdrawl;
			var builder = new WARRELMessageBuilder(declaration);
		...
		public void TestUsingEntryLineToBuildMessage()
		{
			...
			var expectedMessage = CMRExportMessagesTestData.WARRELOriginal;
			var builder = new WARRELMessageBuilder(declaration.ActiveEntryHeaders.Cast<CusEntryHeader>().FirstOrDefault());
			builder.MessageSubType = Common.MessageBuilders.MessageSubTypes.Create;
			AssertEquals("Message should work like before", expectedMessage, builder.MessageText);
		}

		protected override CMRMessageBuilder GetMessageBuilderToTest() => new WARRELMessageBuilder(declaration);
