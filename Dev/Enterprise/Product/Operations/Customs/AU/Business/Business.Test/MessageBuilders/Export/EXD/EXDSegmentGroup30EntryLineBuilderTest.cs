...
{
	public class EXDSegmentGroup30EntryLineBuilderTest : TestCaseWithFactory
	{
		...
		void TestValues(short lineNumber, string tariffRefString, string tempImportNumber, string description, int totalWeight, int invoiceQuantity, string unitOfQuantityString, string permitNumber, string countryOfOriginString, string aUState, ZDecimal invoiceLineAmount, string expectedFOBAmount)
		{
			...
			var entryLine = GetTestCusEntryLine(
				lineNumber,
				tariffRefString,
				tempImportNumber,
				description,
			...
			var myBuilder = new EXDSegmentGroup30EntryLineBuilder(entryLine, group30, 0);
		...
		CusEntryLine GetTestCusEntryLine(short lineNumber, string tariff, string tempImportNumber, string description, int totalWeight, int invoiceQuantity, string unitOfQuantity, string permitNumber, ZString countryOfOrigin, string aUState, ZDecimal invoiceLineAmount)
		{
			var factory = new BusinessObjectFactory();

			var jobDec = factory.NewWithValidTestData<JobDeclaration>();
			var sender = new SendsMessagesToCustomsShutterUpperer();
			jobDec.MessageInitiator = sender;
			...
			jobDec.DoMerge();
			var entryHeader = jobDec.EntryHeader;
