...
{
	sealed class COLSSwitchAepLodgementMessageBuilderTest : COLSMessageBuilderTest
	{
		public override void TestCreateNewMessage()
		{
			...
			var messageBuilder = new COLSSwitchAepLodgementMessageBuilder(colsHeader);
			var message = messageBuilder.CreateNewMessage();

			CombineAssertions(() =>
			{
				AssertEquals("Message count", 1, colsHeader.Messages.Count);
				AssertEquals("Message type", AUCOLSMessageTypeList.Codes.SwitchAepLodgement, message.EM_MessageType);
