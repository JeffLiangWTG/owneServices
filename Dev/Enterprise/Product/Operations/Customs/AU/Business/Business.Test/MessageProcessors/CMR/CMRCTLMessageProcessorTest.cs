...
{
	sealed class CMRCTLMessageProcessorTest : TestCaseWithFactory
	{
		public void TestMessageFilter()
		{
			...
			var processor = new CMRCTLMessageProcessor(new LoggingInformation());
			var messageFilter = processor.MessageFilter;
			CombineAssertions(() =>
			{
				AssertEquals("Contains message1", true, message1.MatchesFilter(messageFilter));
				AssertEquals("Not Contains message2", false, message2.MatchesFilter(messageFilter));
				AssertEquals("Not Contains message3", false, message3.MatchesFilter(messageFilter));
			});
		}
