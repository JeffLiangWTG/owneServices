...
namespace Enterprise.Customs.AU.GUI
{
	...
	public partial class AUExportTariffBulkChangeStartForm : ZChildForm
	{
		public AUExportTariffBulkChangeStartForm(AUExportTariffBulkChange businessEntity)
			: base(businessEntity)
		{
		}
		...
		void TariffBulkChangeFile(object sender, EventArgs e)
		{
			...
			{
				dialog.Title = "Select AHECC Bulk Change Concordance File";
				...
				if (ZFormModaliser.ShowCommonDialogWithoutDispose(dialog) == DialogResult.OK)
				{
					try
					{
						using (var stream = dialog.OpenFile())
						{
							...
							ZFormModaliser.ShowDialogAndDispose(new AUExportTariffBulkChangeForm(BusinessEntity));
						}
					}
