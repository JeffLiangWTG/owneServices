...
{
	public interface ICusEntryLineFeeCollection<out TFee, out TLine> : Customs.Business.ICusEntryLineFeeCollection<TFee, TLine>
	...
	public class CusEntryLineFeeCollection<TFee, TLine> : Customs.Business.CusEntryLineFeeCollection<TFee, TLine>, ICusEntryLineFeeCollection<TFee, TLine>
		where TFee : CusEntryLineFee
		where TLine : CusEntryLine
	{
		...
		{
			return Elements.Cast<CusEntryLineFee>().Any(f => f.CF_ChargeType == feeType && f.CF_RateOverrideReasonCode == ILRateOverrideReasonList.Codes.Override);
		}
