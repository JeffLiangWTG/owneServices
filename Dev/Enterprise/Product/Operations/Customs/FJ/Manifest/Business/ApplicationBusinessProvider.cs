...
namespace Enterprise.Customs.FJ.Manifest.Business
{
	public class ApplicationBusinessProvider : ASYCUDAManifest.Business.ApplicationBusinessProvider
	{
		...
		protected override IReadOnlyList<ZString> ApplicableCountryCodesCore(Directions direction, ZString transportType, string manifestStyle = null)
			=> CountryHelper.SupportedAsycudaCountryCodesList(factory).Where(x => x.ZZD_Code == Core.Constants.CountryCodes.Fiji && x.HasAttribute(manifestStyle)).Select(x => x.ZZD_Code).ToList();

		protected override IReadOnlyList<ZString> CreateCountryCodes() => new[] { (ZString)Core.Constants.CountryCodes.Fiji };
