...
namespace Enterprise.Customs.MX.Manifest.GUI
{
	public sealed class MXManifestlLayouts : IPanelLayoutProvider
	{
		...
		{
			var builder = new ManifestLayoutBuilder<Business.AsycudaManifestHeader>();
			var common = builder.CommonBag;
			var mx = MXManifestControlBag.Instance;
			builder.AddControlBag(mx);
			...
			builder.Add(mx.LastForeignPortCodeFindBox, ControlWidthClass.Auto);
			...
			builder.SetVisibility(common.LloydsNumberTextBox, h => h.IsSea, h => h.AMA_TransportModeInfo);
			builder.SetVisibility(common.CustomsLoadPortCodeFindBox, h => h.IsSea, h => h.AMA_TransportModeInfo);
			builder.SetVisibility(common.CustomsDischargePortCodeFindBox, h => h.IsSea, h => h.AMA_TransportModeInfo);
			builder.SetVisibility
			(
				mx.LastForeignPortCodeFindBox,
				h => h.IsSea && h.IsImport,
				h => h.AMA_TransportModeInfo,
				h => h.AMA_NatureInfo
			);
			builder.SetVisibility(common.AgentTypeDropEdit, h => !h.IsAir, h => h.AMA_TransportModeInfo);
			builder.SetVisibility(common.CustomsOfficeDropEdit, h => h.IsSea, h => h.AMA_TransportModeInfo);
			builder.SetVisibility(common.RegistrationDateEdit, h => h.IsSea, h => h.AMA_TransportModeInfo);
			builder.SetVisibility(common.RegistrationNumberTextBox, h => h.IsSea, h => h.AMA_TransportModeInfo);
			builder.SetVisibility(common.CustomsStatusDropEdit, h => h.IsSea, h => h.AMA_TransportModeInfo);
