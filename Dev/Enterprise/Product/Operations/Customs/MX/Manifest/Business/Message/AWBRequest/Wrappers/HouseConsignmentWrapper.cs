...
using ILocation = CargoWise.Customs.MX.MessageContracts.ILocation;
...
namespace Enterprise.Customs.MX.Manifest.Business
{
	internal class HouseConsignmentWrapper : IHouseConsignment
	{
		...
		string IHouseConsignment.InsuranceValueAmountCurrencyID => bill.ABL_RX_NKInsuranceValueCurrency;
		...
		decimal IHouseConsignment.IncludedTareGrossWeightMeasure => AWBRequestHelper.WeightConvertion(bill.ABL_GrossWeightUQ, bill.ABL_GrossWeight);

		string IHouseConsignment.IncludedTareGrossWeightMeasureUnitCode => AWBRequestHelper.WeightUnitCodeCalculator(bill.ABL_GrossWeightUQ);

		decimal IHouseConsignment.GrossVolumeMeasure => AWBRequestHelper.VolumeConvertion(bill.ABL_VolumeUQ, bill.ABL_Volume);

		string IHouseConsignment.GrossVolumeMeasureUnitCode => AWBRequestHelper.VolumeUnitCodeCalculator(bill.ABL_VolumeUQ);
		...
		IParty IHouseConsignment.FreightForwarderParty => freightForwarderParty ?? (freightForwarderParty = new FreightForwarderPartyWrapper(GlbCompany.CurrentCompany));
		...
		ILocation IHouseConsignment.OriginLocation => originLocation ?? (originLocation = new LocationWrapper(bill.Factory, header.AMA_RL_NKPortOfLoading));
		ILocation originLocation;

		ILocation IHouseConsignment.FinalDestinationLocation => finalDestinationLocation ?? (finalDestinationLocation = new LocationWrapper(bill.Factory, header.AMA_RL_NKPortOfDischarge));
