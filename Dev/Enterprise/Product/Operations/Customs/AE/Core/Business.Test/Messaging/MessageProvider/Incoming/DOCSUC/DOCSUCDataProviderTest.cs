...
sealed class DOCSUCDataProviderTest : DataProviderTestCase<DOCSUCDataProvider, IDOCSUCDataProvider>
{
	public void TestConstructor()
	{
		AssertExceptionThrown<ArgumentNullException>("The input parameter cannot be null.", () => new DOCSUCDataProvider(null));

		var message = Factory.New<EDIMessage>();
		message.EM_Status = EDIMessageStatusList.Codes.PreProcessedOK;

		AssertNoExceptionThrown(() => new DOCSUCDataProvider(message));
	}

	public void TestOutgoingAccessReference()
	{
		var (inboundMessage, outboundInterchange) = CreatePreparedData();
		var provider = new DOCSUCDataProvider(inboundMessage);

		AssertEquals(outboundInterchange.EI_InterchangeNum, provider.OutgoingAccessReference);
	}

	protected override DOCSUCDataProvider GetProvider()
	{
		var (inboundMessage, _) = CreatePreparedData();
		return new DOCSUCDataProvider(inboundMessage);
	}
