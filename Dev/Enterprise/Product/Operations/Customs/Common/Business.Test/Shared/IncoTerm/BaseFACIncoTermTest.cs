...
{
	public class BaseFCAIncoTermTest : IncoTermTest
	{
		[ExpectNoExceptions]
		...
		public override void TestITOTIncoterm()
		{
			...
			NUnit.Framework.Assert.That(IncoTermAndChargeFactory.ITOTIncoTerm(IncotermToTest, invoice), Is.EqualTo(Core.Constants.IncoTerms.ExWorks).Using(CustomComparers.TypeComparison), "With PAC, ITOT Incoterm should be EXW");

			pAC.J7_IsIncludedInITOT = true;
			NUnit.Framework.Assert.That(IncoTermAndChargeFactory.ITOTIncoTerm(IncotermToTest, invoice), Is.EqualTo(IncotermToTest).Using(CustomComparers.TypeComparison), "With PAC in lines, ITOT Incoterm should be FCA");
		}
		...
		{
			get { return Core.Constants.IncoTerms.FreeCarrier; }
		}
