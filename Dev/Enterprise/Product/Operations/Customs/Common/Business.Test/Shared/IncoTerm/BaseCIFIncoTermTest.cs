...
{
	public class BaseCIFIncoTermTest : IncoTermTest
	{
		[ExpectNoExceptions]
		...
		public override void TestITOTIncoterm()
		{
			...
			NUnit.Framework.Assert.That(IncoTermAndChargeFactory.ITOTIncoTerm(IncotermToTest, invoice), Is.EqualTo(Core.Constants.IncoTerms.FreeOnBoard).Using(CustomComparers.TypeComparison), "With ONS and OFT, ITOT Incoterm should be FOB");

			oFT.J7_IsIncludedInITOT = true;
			NUnit.Framework.Assert.That(IncoTermAndChargeFactory.ITOTIncoTerm(IncotermToTest, invoice), Is.EqualTo(Core.Constants.IncoTerms.CostAndFreight).Using(CustomComparers.TypeComparison), "With OFT in lines, ITOT Incoterm should be CFR");

			oNS.J7_IsIncludedInITOT = true;
			NUnit.Framework.Assert.That(IncoTermAndChargeFactory.ITOTIncoTerm(IncotermToTest, invoice), Is.EqualTo(IncotermToTest).Using(CustomComparers.TypeComparison), "With OFT & ONS in lines, ITOT Incoterm should be CIF");

			oFT.J7_IsIncludedInITOT = false;
			oNS.J7_IsIncludedInITOT = true;
			NUnit.Framework.Assert.That(IncoTermAndChargeFactory.ITOTIncoTerm(IncotermToTest, invoice), Is.EqualTo(IncoTermAndCustomsChargeFactory.ErrorIncoTermCode).Using(CustomComparers.TypeComparison), "With ONS in lines, but not OFT, ITOT Incoterm should be an error");
		}
		...
		{
			get { return Core.Constants.IncoTerms.CostInsuranceAndFreight; }
		}
