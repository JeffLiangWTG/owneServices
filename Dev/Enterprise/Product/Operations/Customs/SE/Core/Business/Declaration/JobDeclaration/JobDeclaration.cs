...
namespace Enterprise.Customs.SE.Business.Declaration
{
	public class JobDeclaration : EU.Business.Declaration.JobDeclaration
		, Integration.Customs.SE.IJobDeclaration
	{
		...
		protected override ZString LocalCurrencyCodeCore => Core.Constants.CurrencyCodes.Sweden;

		protected override bool HasSplitEntriesCore
		{
			get
			{
				if (!GetType().FullName.Contains("SE"))
				{
