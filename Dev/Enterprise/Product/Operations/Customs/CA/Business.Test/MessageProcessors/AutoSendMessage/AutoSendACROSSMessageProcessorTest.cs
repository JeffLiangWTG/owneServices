...
{
	[TestedType(typeof(AutoSendACROSSMessageProcessor))]
	sealed class AutoSendACROSSMessageProcessorTest : CAAutoSendCustomsMessageProcessorTest
	{
		protected override void PrepareDeclaration(BaseJobDeclaration declaration)
		{
			...
			caDeclaration.CA_ServiceOption = ACROSSServiceOptions.Codes.PARS;
		...
		{
			return new AutoSendACROSSMessageProcessor((JobDeclaration)declaration);
		}

		protected override ZString ExpectedMessageDescription => JobDeclaration.Constants.MessageNames.Release;

		protected override void AssertEntryAndMessageResultForEndToEndTest(Customs.Business.CusEntryHeader entry)
		{
			AssertEquals("Original ACROSS message should be generated.", 1, entry.Messages.Count);
			AssertEquals(MessageStatusList.Codes.AwaitingOriginal, entry.CH_Status);
			var originalMessage = entry.Messages[0];
			AssertEquals(MessageTypeList.Codes.EDIRelease, originalMessage.EM_MessageType);
