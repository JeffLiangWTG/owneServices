...
{
	class AWBCancelMessageProcessor : CLMessageProcessor
	{
		internal AWBCancelMessageProcessor(LoggingInformation logger) : base(logger) { }

		internal override string ProcessCustomsManifestMessage(CLMessage message, string status)
		{
			var response = MessageBuilderHelper.GetAWBCancelResponse(message.EM_MessageText);
			if (response != null)
			{
