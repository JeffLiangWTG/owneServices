...
{
	internal class BLMessageProcessor : CLMessageProcessor
	{
		internal BLMessageProcessor(LoggingInformation logger) : base(logger) { }

		internal override string ProcessCustomsManifestMessage(CLMessage message, string status)
		{
			var response = MessageBuilderHelper.GetBLResponse(message.EM_MessageText);
			if (response != null)
			{
