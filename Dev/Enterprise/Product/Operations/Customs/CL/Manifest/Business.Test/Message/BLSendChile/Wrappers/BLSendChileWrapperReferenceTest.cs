...
{
	class BLSendChileWrapperReferenceTest : TestCaseWithFactory
	{
		AsycudaManifestHeader header;

		public void TestBLSendChileWrapperReference()
		{
			header = Factory.NewWithValidTestData<AsycudaManifestHeader>();
			header.RegistrationNumber = "194914";
			...
			entryNum.CE_IssueDate = new ZDate(2020, 03, 11);
			...
			IReference reference1 = wrapper.References.ElementAt(0);
			IReference reference2 = wrapper.References.ElementAt(1);

			Factory.Save();

			CombineAssertions(() =>
			{
				AssertEquals(2, wrapper.References.Count);

				AssertEquals(WrappersConstants.ReferenceType.Ref, reference1.ReferenceType);
				AssertEquals(WrappersConstants.DocumentType.Mfto, reference1.DocumentType);
				AssertEquals("194914", reference1.Number);
				AssertEquals("11-03-2020", reference1.Date);

				AssertEquals(WrappersConstants.ReferenceType.Madre, reference2.ReferenceType);
				AssertEquals(WrappersConstants.DocumentType.Bl, reference2.DocumentType);
