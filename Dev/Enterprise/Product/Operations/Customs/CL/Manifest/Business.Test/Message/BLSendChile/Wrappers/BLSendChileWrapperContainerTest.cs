...
{
	class BLSendChileWrapperContainerTest : TestCaseWithFactory
	{
		AsycudaManifestHeader header;

		public void TestBLSendChileWrapperContainer()
		{
			...
			CombineAssertions(() =>
			{
				AssertEquals("CAIU", container.Initials);
				AssertEquals("305178", container.Number);
				AssertEquals("6", container.CheckDigit);
				AssertEquals("22G0", container.IsoCode);
				AssertEquals("170.000", container.GoodsWeight);
				AssertEquals("MEDITERRANEAN SHIPPING COMPANY CHILE S.A.", container.ShipmentOwner);
				AssertEquals(BLSendChileConstants.StatusContainer.FclFcl, container.Status);
			});
		}
