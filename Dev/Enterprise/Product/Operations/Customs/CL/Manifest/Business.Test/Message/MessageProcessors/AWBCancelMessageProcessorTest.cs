...
{
	sealed class AWBCancelMessageProcessorTest : TestCaseWithFactory
	{
		...
		public void TestProcessAcceptedMessage()
		{
			...
			var expectedresult = @"Bill Number (numero-referencia): 729-75818610
Customs ID (id-documento-servidor): 68630860

Status: Aprobado

Datetime sent: 2022-01-10 15:15:29
Datetime response: 2022-01-10 15:15:44
...
";
		...
		public void TestProcessRejectedMessage()
		{
			...
			var expectedresult = @"Bill Number (numero-referencia): 729-75818610
Customs ID (id-documento-servidor): 0
...
La Guia Aerea N 729-75818610 no se encuentra en los registros de sistema
";
		...
		CLMessage CreateMessage(ZString messageText, ZGuid billPK)
		{
			...
			message.EM_MessageType = MessageTypes.Codes.CHF;
		...
		readonly ZString acceptedResponse = CLInboundInterchangeProcessorTest.GetExpectedMessageXML(Path.Combine(BaseSourcePath, CLMessagingConstants.AirCancelAcceptedResponse));
		readonly ZString rejectedResponse = CLInboundInterchangeProcessorTest.GetExpectedMessageXML(Path.Combine(BaseSourcePath, CLMessagingConstants.AirCancelRejectedResponse));
	}
}
