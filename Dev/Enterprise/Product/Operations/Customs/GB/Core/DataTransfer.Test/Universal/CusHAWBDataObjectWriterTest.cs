...
{
	[TestedType(typeof(CusHAWBDataObjectWriter))]
	sealed class CusHAWBDataObjectWriterTest : DataObjectWriterBaseTest
	{
		public void Test_PopulateCountrySpecificDetails()
		{
			...
			hawb.CS_CM = mawb.PK;
			hawb.CS_PiecesLanded = 10000;
			hawb.CS_FolioReference = "FOLIO";
			hawb.AirportOfOrigin = "GBXX";
			hawb.AirportOfArrival = "LHR";
			hawb.CargoTerminalOperatorAirportAndShed = "LHRBAC";
			hawb.CS_CustomsStatus = "XO";
			hawb.PresenceOnNetworkStatus = "COM";
			hawb.LatestCustomsActionText = "CUSTOMS ACTION";
			hawb.TemporaryStorageEndDate = storageDate;
			hawb.Status1Date = status1Date;
			hawb.Status2Granted = true;
			hawb.ShipmentDescriptionCode = "C";
			hawb.ConsignmentOrEntryType = "23";
			CusHAWB.LogCustomsActionCodeEvent("XO", customsActionDate, hawb.Logs, hawb.PK);

			var manager = (IShipmentDataContextManager)hawb.GetUniversalDataContextManager();
			var writer = manager.GetShipmentDataObjectWriter(new DataWritingManager(new ActionInfo(RecipientRoleType.ORP, hawb)));
			var hawbData = (Shipment)writer.GetDataObject(hawb);
			var shipment = hawbData.SubShipmentCollection[0];
			...
			AssertCodeDescriptionPair("Shipment Code", shipment.ShipmentSubType, "C23", null);
		...
		public void TestPopulateSplits()
		{
			...
			split.PresenceOnNetworkStatus = PresenceOnNetworkList.Codes.ArchivedOnCcsuk;
			...
			AssertEquals("Additional Bills Count", 1, hawbData.SubShipmentCollection[0].AdditionalBillCollection.Count);
			var additionalBill = hawbData.SubShipmentCollection[0].AdditionalBillCollection[0];
