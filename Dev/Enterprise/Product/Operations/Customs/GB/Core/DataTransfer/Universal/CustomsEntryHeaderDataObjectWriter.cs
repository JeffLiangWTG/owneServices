...
namespace Enterprise.Customs.GB.DataTransfer.Universal
{
	public class CustomsEntryHeaderDataObjectWriter : Customs.DataTransfer.Universal.CustomsEntryHeaderDataObjectWriter
	{
		...
		protected override void PopulateCusEntrySupportingInfoData(CusEntryHeader entryHeaderBO, EntryHeader entryHeaderData)
		{
			base.PopulateCusEntrySupportingInfoData(entryHeaderBO, entryHeaderData);
			entryHeaderData.CustomsSupportingInformationCollection = CreateCollection((Business.Declaration.CusEntryHeader)entryHeaderBO, writeManager);
		}
		...
		protected List<CustomsSupportingInformation> CreateCollection(Business.Declaration.CusEntryHeader entryHeader, IDataWritingManager writeManager)
		{
			var result = new List<CustomsSupportingInformation>();
			if (entryHeader != null)
			{
				var typeList = CusSupportingInfoTypeListProvider.GetListForList();
				entryHeader.SupportingDocuments.ToList().ForEach(x => result.Add(Create(x, typeList, writeManager)));
				entryHeader.AdditionalInfos.ToList().ForEach(x => result.Add(Create(x, typeList, writeManager)));
			}
