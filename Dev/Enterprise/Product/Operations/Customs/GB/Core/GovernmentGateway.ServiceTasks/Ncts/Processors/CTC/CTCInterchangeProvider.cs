...
{
	public class CTCInterchangeProvider : CDSInterchangeProvider
	{
		public CTCInterchangeProvider(LoggingInformation logger, NonDependentEDIMessageCollection messages) : base(logger, messages)
		{
		}
		...
		{
			var ctcRequestMessage = messages.Cast<EDIMessage>().Single();
			SetInterchangeValuesForTransmit(interchange, messages, GetInterchangeType(ctcRequestMessage), CDS.Constants.EDIInterchange.GBCustoms, ctcRequestMessage.EM_MessageOwner);
			...
			{
				interchange.EI_HeaderText = GetHeaderText(ctcRequestMessage);
			}
		}

		protected virtual ZString GetInterchangeType(EDIMessage message) => message.EM_MessageSubType;

		protected virtual ZString GetHeaderText(EDIMessage message)
		{
			return GovernmentGatewayExtensions.NewGBCustomsRequest(message)?.Serialize() ?? ZString.Empty;
		}
		...
		protected override void MarkMessagesInThisInterchangeAsFailed(NonDependentEDIMessageCollection messages, EDIInterchange interchange)
		{
			foreach (EDIMessage message in messages.ToArray())
			{
				...
				logger.LogError(string.Format(CultureInfo.InvariantCulture, "Failed to create interchange for Government Gateway Message #" + message.EM_MessageNum + " : " + errorMessage.ToString()));
