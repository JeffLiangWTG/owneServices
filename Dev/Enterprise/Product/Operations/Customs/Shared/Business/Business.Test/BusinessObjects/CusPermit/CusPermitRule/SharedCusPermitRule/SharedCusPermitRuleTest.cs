...
{
	public abstract class SharedCusPermitRuleTest : EnterpriseBusinessObjectTestCase
	{
		public void TestCPR_ValueTo_FieldType()
		{
		...
		public void TestValueFromFieldIsTextDropEdit()
		{
			var permit = Factory.New<BaseCusPermitHeader_ForTest>();
			var rule = permit.CusPermitRules.AddNew();
			...
			rule.CPR_RuleCode = ((int)FieldType.Boolean).ToString();
			AssertEquals(nameof(FieldType.Boolean), rule.CPR_ValueFrom_FieldType);
			AssertEquals(false, rule.ValueFromFieldIsTextDropEdit);

			rule.CPR_RuleCode = ((int)FieldType.TextDropEdit).ToString();
			AssertEquals(nameof(FieldType.TextDropEdit), rule.CPR_ValueFrom_FieldType);
			AssertEquals(true, rule.ValueFromFieldIsTextDropEdit);
		}
		...
		#region  Bindings for Visibility of ValueTo Edit
		public void TestValueToFieldIsTextDropEdit()
		{
			var permit = Factory.New<BaseCusPermitHeader_ForTest>();
			var rule = permit.CusPermitRules.AddNew();

			rule.CPR_RuleCode = ((int)FieldType.Boolean).ToString();
			AssertEquals(nameof(FieldType.Boolean), rule.CPR_ValueTo_FieldType);
			AssertEquals(false, rule.ValueToFieldIsTextDropEdit);

			rule.CPR_RuleCode = ((int)FieldType.TextDropEdit).ToString();
			AssertEquals(nameof(FieldType.TextDropEdit), rule.CPR_ValueTo_FieldType);
			AssertEquals(true, rule.ValueToFieldIsTextDropEdit);
		}

		public void TestValueToFieldIsOthers()
		{
			var permit = Factory.New<BaseCusPermitHeader_ForTest>();
			var rule = permit.CusPermitRules.AddNew();
			...
			rule.CPR_RuleCode = ((int)FieldType.Boolean).ToString();
			AssertEquals(nameof(FieldType.Boolean), rule.CPR_ValueTo_FieldType);
			AssertEquals(true, rule.ValueToFieldIsOthers);

			rule.CPR_RuleCode = ((int)FieldType.TextCodeFindBox).ToString();
			AssertEquals(nameof(FieldType.TextCodeFindBox), rule.CPR_ValueTo_FieldType);
			AssertEquals(false, rule.ValueToFieldIsOthers);
		}
		#endregion
