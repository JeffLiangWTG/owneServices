...
namespace Enterprise.Customs.Business.Testing
{
	...
	sealed class CusEntryLineBaseOnlyTest : CusEntryLineTest<CusEntryLine, BaseJobComInvoiceLine>
	{
		public void TestGetInvoicedDocumentaryAmountAndCurrency()
		{
			...
			AssertEquals(0m, entryLine.GetInvoicedDocumentaryAmount());
			AssertEquals("", entryLine.GetInvoicedDocumentaryAmountCurrency());
		}

		public void TestCL_DataModel_SetOnSaving()
		{
			var declaration = Factory.New<BaseJobDeclaration>();
			var entry = declaration.CustomsEntryHeaders.AddNew();
			var entryLine = entry.AllEntryLines.AddNew();
			AssertEquals("Not set", ZString.Empty, entryLine.CL_DataModel);
			Factory.Save();
			AssertEquals("set", "ER", entryLine.CL_DataModel);
		}

		public void TestCL_DataModel_ReportErrorWhenUpdated() =>
			DataModelTestHelper.RunDataModelTest_ReportErrorWhenUpdated<CusEntryLine>(Factory);

		public void TestCL_DataModel_CanSaveTwice() =>
			DataModelTestHelper.RunDataModelTest_CanSaveTwice<CusEntryLine>(Factory);

		public void TestCL_DataModel_ReportErrorWhenSetToEmpty() =>
			DataModelTestHelper.RunDataModelTest_ReportErrorWhenSetToEmpty<CusEntryLine>(Factory);

		public void TestCL_DataModel_PopulateParent()
		{
			var declaration = Factory.New<BaseJobDeclaration>();
			var entry = declaration.CustomsEntryHeaders.AddNew();
			var entryLine = entry.AllEntryLines.AddNew();
		...
		public void TestIDataModelSupporter()
		{
			...
			var entry = declaration.CustomsEntryHeaders.AddNew();
			var entryLine = entry.AllEntryLines.AddNew();
			var cusSupportingInfoParent = (IDataModelSupporter)entryLine;
		...
		public void TestAssignDescriptionToCL_Description()
		{
			...
			entryLineMock
				.Protected()
				.Setup<ZString>("DescriptionInternal")
				.Returns(new ZString("A".PadLeft(CusEntryLineSchema.CL_Description.MaxLength + 1)));
		...
		public void TestEffectiveDescription()
		{
			...
			entryLineMock
				.Protected()
				.Setup<ZString>("DescriptionInternal")
				.Returns(new ZString("CalculatedDescription"));
			var entryLine = entryLineMock.Object;
...
CusEntryLine", handler.ReportInformationMessage.Trim());
		...
		public void TestDutyAmountByConfirmedFee()
		{
			var declaration = Factory.New<BaseJobDeclaration>();
			var entry = declaration.ActiveEntryHeaders.AddNew();
			var entryLine = entry.MergedLines.AddNew();
			var fee1 = entryLine.Fees.AddOrUpdate(Core.Constants.Customs.CusEntryFeeTypes.DutyAmount, 100m);
			AssertEquals("entryLine.DutyAmountByConfirmedFee", 0m, entryLine.DutyAmountByConfirmedFee);
			var fee2 = entryLine.ConfirmedFees.AddOrUpdate(Core.Constants.Customs.CusEntryFeeTypes.DutyAmount, 200m);
			AssertEquals("entryLine.DutyAmountByConfirmedFee", 200m, entryLine.DutyAmountByConfirmedFee);
		}

		public void TestConfirmedFeesAreDeletedWhenCusEntryLineDelete()
		{
			var declaration = Factory.New<BaseJobDeclaration>();
			var entry = declaration.ActiveEntryHeaders.AddNew();
			var entryLine = entry.MergedLines.AddNew();
			...
			{
				entryLine.Delete();
				AssertEquals("fee1.IsDeleted", true, fee1.IsDeleted);
				AssertEquals("fee2.IsDeleted", true, fee2.IsDeleted);
		...
		public void TestTotalInvoiceLinesGrossWeightInKG()
		{
			...
			invoice1Line1.JI_CL = entry1Line.PK;
			invoice1Line2.JI_CL = entry1Line.PK;
			invoice1Line3.JI_CL = entry1Line.PK;
			...
			CombineAssertions(() =>
			{
				AssertEquals("entry1Line.TotalInvoiceLinesGrossWeightInKG", 1560.2m, entry1Line.TotalInvoiceLinesGrossWeightInKG);
				AssertEquals("entry2Line1.TotalInvoiceLinesGrossWeightInKG", 300m, entry2Line1.TotalInvoiceLinesGrossWeightInKG);
			});
		}
		...
		public void TestHasAnyProcedureWithSuspendedVat()
		{
			...
			GenerateProcedure("No", YesNoList.Codes.No, YesNoList.Codes.No, "No description", false);
			Factory.Save();
		...
		public void TestUNDGs()
		{
			...
			CombineAssertions(() =>
			{
				var uNDGs = entryLine.UNDGs;
				AssertContainsExactElementsInAnyOrder("UNDGs", new[] { undg1, undg2, undg3 }, uNDGs);
				AssertSame("UNDGs object instance", uNDGs, entryLine.UNDGs);
			});
		}
		...
		public void TestITypeDeciderContext()
		{
			...
			CombineAssertions(() =>
			{
				AssertEquals("From CurrentCompany", "ER", (Factory.New<CusEntryLine>() as ITypeDeciderContext).Country);

				var declaration = (BaseJobDeclaration)Factory.New<Integration.Customs.NZ.IJobDeclaration>();
				declaration.JE_GB = nzBranch.PK;
				var entry = declaration.ActiveEntryHeaders.AddNew();
				var entryLine = entry.MergedLines.AddNew();
				AssertEquals("From EntryHeader", "NZ", (entryLine as ITypeDeciderContext).Country);
			});
		}
