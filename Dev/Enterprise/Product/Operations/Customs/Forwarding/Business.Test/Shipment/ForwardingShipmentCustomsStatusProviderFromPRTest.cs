...
{
	[CountrySpecificTest(Core.Constants.CountryCodes.PuertoRico)]
	class ForwardingShipmentCustomsStatusProviderFromPRTest : TestCaseWithFactory
	{
		public void TestGetsTheRightForwardingShipmentCustomsStatusProvider()
		{
			...
			NUnit.Framework.Assert.That(statusProvider.GetType(), Is.EqualTo(ObjectFactory.GetType<Integration.Customs.US.IForwardingShipmentCustomsStatusProvider>()), "ForwardingShipmentCustomsStatusProvider.New(shipment).GetType()");

			AssertNoExceptionThrown(delegate
			{
				statusProvider.CustomsCargoStatus();
				statusProvider.CustomsMessageStatus();
				statusProvider.CRLStatus();
				statusProvider.ENSStatus();
				statusProvider.EXPStatus();
				statusProvider.ITStatus();
			});
		}
	}
}
