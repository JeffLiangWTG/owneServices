...
{
	public abstract class CusTempStorageDec : EU.Business.CusTempStorage.CusTempStorageDec,
		Integration.Customs.FR.ICusTempStorageDec
	...
	{
		protected CusTempStorageDec(BusinessObjectFactory factory, DataRow row) : base(factory, row)
		{
		}
		...
		#region STH_Calc_StorageLimitRemaining
		public ZString STH_Calc_StorageLimitRemaining
		{
			get
			{
				...
				{
					var endDate = StorageHeader.SJH_TempStorageEndDateUtc;
					var createDate = STH_SystemCreateTimeUtc;
					if (endDate.IsValid && createDate.IsValid)
					{
						var remainingDate = endDate - createDate;

						if (endDate < ZDateTime.Now)
						{
							result = ZString.Empty;
						}
						else if (remainingDate.TotalHours < 48)
						{
							result = ((int)remainingDate.TotalHours).ToString() + (NoResString)"h";
						}
						else if (remainingDate.TotalHours >= 48)
						{
							result = ((int)remainingDate.TotalDays).ToString() + (NoResString)"j";
						}
					}
				}
				return result;
		...
		protected virtual Dictionary<ZString, Type> GetCusCodeDataTypesCore()
		{
			var result = new Dictionary<ZString, Type>
			{
				{ CusCodeDataTypeList.Codes.TempStorageContainer, typeof(CusTempStorageContainer) }
			};
			return result;
		}
		...
		public const string DeclarationStatusForCorrectionMessage = "TBA";
