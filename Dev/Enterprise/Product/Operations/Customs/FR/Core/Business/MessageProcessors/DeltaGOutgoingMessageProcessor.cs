...
{
	public class DeltaGOutgoingMessageProcessor : OutgoingMessageProcessor
	{
		public DeltaGOutgoingMessageProcessor(LoggingInformation logger)
			: base(logger)
		{
		}
				...
				if (fMessageFilter == null)
				{
					fMessageFilter = new ZQuery(EDIMessageSchema.EM_ApplicationCode, ApplicationCodeList.Codes.FRCustomsMessage)
						.AddToFilter(EDIMessageSchema.EM_MessageType, new string[]
						{
							...
							MessageTypeList.Codes.DCG,
							MessageTypeList.Codes.IMC,
							MessageTypeList.Codes.EXC,
							MessageTypeList.Codes.IMD,
							MessageTypeList.Codes.EXD
						});
				}
		...
		{
			return new DeltaGInterchangeProvider(readyMessages);
		}
	}
}
