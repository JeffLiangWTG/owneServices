...
{
	public class DeltaIEAutoSendCustomsMessageProcessor : AutoSendCustomsMessageProcessor
	{
		public DeltaIEAutoSendCustomsMessageProcessor(JobDeclaration declaration) : base(declaration)
		{
		}
		...
		protected override ZBool SendCustomsMessageCore(INotifications notifications, Customs.Business.CusEntryHeader entryHeader)
		{
			...
			var messageSendingObject = new DeltaIEJobDeclarationMessageSendingObject((CusEntryHeader)entryHeader);
			...
			if (messageSendingObject.ShouldSend)
			{
				...
				if (rule != null)
				{
					var sender = new DeltaIEMessageSender(messageSendingObject, errorCollector)
					{
		...
		protected override IEnumerable<IAutoSendCustomsMessageRule> AutoSendCustomsMessages
		{
			get
			{
				yield return new Send415MessageWhenStatusIsEmpty();
			}
		}
	}
}
