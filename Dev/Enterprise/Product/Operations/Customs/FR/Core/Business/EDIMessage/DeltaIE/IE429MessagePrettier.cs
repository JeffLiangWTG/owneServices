...
{
	public class IE429MessagePrettier : DeltaIEMessagePrettier<CC429BType>
	{
		public IE429MessagePrettier(IE429MessageDataObject messageDataObject)
		...
		protected override ZString GetMessageInterpretationCore(CC429BType messageObject)
		{
			...
			if (messageObject.ControlResult != null)
			{
				...
				content = ToTd(ToStrongIfNotEmpty(ToLargerPIfNotEmpty("Control Result")), 4, 1);
			...
			if (messageObject.ControlResults != null && messageObject.ControlResults.Count > 0)
			{
				...
				foreach (var controlResult in messageObject.ControlResults)
				{
					...
					if (controlResult.ResultsOfControl != null)
					{
						...
						foreach (var resultOfControl in resultsOfControl)
						{
							...
							{
								totalRows += 6 * resultOfControl.ControlDetails.Count;
							}
						}
					}
				}

				content = ToTd(ToStrongIfNotEmpty(ToLargerPIfNotEmpty("Control Results")), totalRows, 1);
				...
				foreach (var controlResult in messageObject.ControlResults)
				{
					...
					if (controlResult.ResultsOfControl != null && controlResult.ResultsOfControl.Count > 0)
					{
						...
						content = ToTd(ToStrongIfNotEmpty(ToLargerPIfNotEmpty("Results of Control")), totalRows, 1);
						...
						foreach (var resultOfControl in resultsOfControl)
						{
							...
							if (resultOfControl.ControlDetails != null && resultOfControl.ControlDetails.Count > 0)
							{
								...
								content = ToTd(ToStrongIfNotEmpty(ToLargerPIfNotEmpty("Control Details")), controlDetails.Count * 6, 1);
