...
{
	class HolderOfTheTransitProcedureWrapperTest : Customs.Business.Testing.DataProviderTestCase<HolderOfTheTransitProcedureWrapper>
	{
		public void TestTIRHolderIdentificationNumber()
		{
			AssertEquals("TIRHolderIdentificationNumber should be mapped to OrgCus code of type 'TIR'.", "FR987654321987654" , Provider.TIRHolderIdentificationNumber);
		}

		public void TestContactPerson()
		{
			var contact = Provider.ContactPerson;
			AssertEquals("ContactPerson should be mapped to organisation first contact of type CUS.", "Contact1, +33 1 01 01 01 01, a@a.com", $"{contact.Name}, {Provider.ContactPerson.PhoneNumber}, {contact.EmailAddress}");
		}
		...
		{
			AssertEquals("Not Implemented", null, Provider.Role);
		}

		protected override HolderOfTheTransitProcedureWrapper GetProvider()
		{
			{
				var orgHeader = Factory.New<OrgHeader>();
				orgHeader.OH_FullName = "BN CORP";
				orgHeader.CustomsCodes.AddNew("TIR", "98765432198765432111", Core.Constants.CountryCodes.France);
				...
				var representative = Factory.New<JobDocAddress>();
				representative.OrganisationPK = orgHeader.PK;

				return HolderOfTheTransitProcedureWrapper.New(orgHeader);
			}
		}
	}
}
