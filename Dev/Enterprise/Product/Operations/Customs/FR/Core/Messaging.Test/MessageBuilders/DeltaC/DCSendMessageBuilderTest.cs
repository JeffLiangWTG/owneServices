...
{
	public abstract class DCSendMessageBuilderTest<TMessageBuilder> : TestCaseWithFactory
		where TMessageBuilder : IMessageBuilderBase
	{
		...
		public void TestSend()
		{
			...
			var entry = deltaHelper.CreateEntryDeclarationForTest(true, IsImport);
		...
		protected TMessageBuilder CreateMessageBuilder(string actionCode, bool isImport = true)
		{
			...
			if (isImport)
			{
				...
				{
					EntryActionCodeList.Codes.REC => new DCRECSendImpMessageBuilder(declarationObject, new ErrorCollector(), TransactionTypes.Original, 999),
					EntryActionCodeList.Codes.EAV => new DCEAVSendImpMessageBuilder(declarationObject, new ErrorCollector(), TransactionTypes.Original, 999),
					EntryActionCodeList.Codes.ANT => new DCANTSendImpMessageBuilder(declarationObject, new ErrorCollector(), TransactionTypes.Original, 999),
					EntryActionCodeList.Codes.MAP => new DCMAPSendImpMessageBuilder(declarationObject, new ErrorCollector(), TransactionTypes.Original, 999),
			...
			else
			{
				messageBuilder = actionCode switch
				{
					EntryActionCodeList.Codes.REC => new DCRECSendExpMessageBuilder(declarationObject, new ErrorCollector(), TransactionTypes.Original, 999),
					EntryActionCodeList.Codes.EAV => new DCEAVSendExpMessageBuilder(declarationObject, new ErrorCollector(), TransactionTypes.Original, 999),
					EntryActionCodeList.Codes.ANT => new DCANTSendExpMessageBuilder(declarationObject, new ErrorCollector(), TransactionTypes.Original, 999),
					EntryActionCodeList.Codes.MAP => new DCMAPSendExpMessageBuilder(declarationObject, new ErrorCollector(), TransactionTypes.Original, 999),
					_ => Activator.CreateInstance(typeof(TMessageBuilder), declarationObject, new ErrorCollector(), TransactionTypes.Original, 999)
				};
			}
		...
		protected void CreateDeclarationMock(string actionCode, bool import = true)
		{
			...
			myMetaDatas.Setup(m => m.Application).Returns("DELTAC");
			...
			myMessageEnvelop.Setup(m => m.SchemaID).Returns(import ? "MessageCDecImp" : "MessageCDecExp");
			...
			myDeclGen.Setup(m => m.ProcedureType).Returns("C");
			...
			myDeclGen.Setup(m => m.ArrivalState).Returns("FR");
			...
			myDeclImportSupplier.Setup(m => m.OrganisationNumberEoriOnly).Returns("ETRANGEREoriOnly");
			...
			myDeclImportImporter.Setup(m => m.OrganisationNumberEoriOnly).Returns("FR31501335900156");
			...
			myDeclRepresentative.Setup(m => m.OrganisationNumberEoriOnly).Returns("FR32582075100081");
			...
			myDeclGen.Setup(m => m.EntryGoodsPriceCurrencyRate).Returns(new ZDecimal(0.8));
			...
			myDeclGen.Setup(m => m.DOMCostsAndInsurance).Returns<ICostsAndInsurance>(null);
			...
			myDeclGenArticle1.Setup(m => m.AlternateCalcValue).Returns<IAlternateCalcValue>(null);
			...
			FinancialDatasMock(myDeclGenArticle1, 1101.01m, "EUR");
			...
			declarationMock.Setup(m => m.IsOfficeOfLodgementDifferentFromOfficeOfExit).Returns(false);
		...
		protected void CreateDeclarationMockDNM(string actionCode, bool import = true)
		{
			...
			myDeclGen = new Mock<ICusProcedure>();
			...
			myDeclGenArticle1DNM.Setup(m => m.AlternateCalcValue).Returns((IAlternateCalcValue)null);
			...
			myDeclGenArticle1DNM.Setup(m => m.PartDispos).Returns(
				new List<ITariffAdditionalCode>() { myDispoPart1.Object, myDispoPart2.Object, myDispoPart3.Object });

			#endregion

			myDeclGenArticle1DNM
				.Setup(m => m.EntryLineDescription)
				.Returns(
					"Tableaux, panneaux, consoles, pupitres, armoires et autres supports comportant plusieurs appareils des n° 8535 ou 8536"
			...
			myDeclGenArticle1DNM.Setup(m => m.SupportingDocuments).Returns(new List<ISupportingDocumentOnly>()
					{ mySupportingDoc1Art1, mySupportingDoc2Art1, mySupportingDoc3Art1,mySupportingDoc4Art1 });
			#endregion

			FinancialDatasMockDNM(myDeclGenArticle1DNM, 1101.01m, "EUR");
			...
			myDeclGenArticle2
				.Setup(m => m.SupportingDocuments)
				.Returns(new List<ISupportingDocumentOnly>()
				{
					mySupportingDoc1Art2,
					mySupportingDoc2Art2,
					mySupportingDoc3Art2,
					mySupportingDoc4Art2
				});
			#endregion
			...
			var myPreCalcEntryLine4 = PreCalcEntryLineMock("A455", "0", 20m, 12m, 0m, "", "", 0m, true);
			myDeclGenArticle2.Setup(m => m.PreCalcEntryLines)
				.Returns(new List<IPreCalcEntryLine>() { myPreCalcEntryLine4 });
		...
		void SpecificsMock(Mock<IArticle> articleMock, ZString code, ZString qualif, ZDecimal qty)
		{
			...
			articleMock.Setup(m => m.ApplicantDescription).Returns("ApplicantDescription");
			articleMock.Setup(m => m.ApplicantConditions).Returns("ApplicantConditions");
			articleMock.Setup(m => m.ApplicantPurOffice).Returns("ApplicantPurOffice");
			articleMock.Setup(m => m.ApplicantInwardLocation).Returns("ApplicantInwardLocation");
			articleMock.Setup(m => m.ApplicantTransFormality).Returns("ApplicantTransFormality");
			articleMock.Setup(m => m.IsPlacingGoodsUnderBW).Returns(false);
			articleMock.Setup(m => m.HasSpecificRegimeAuthorisation).Returns(false);
			#endregion
		...
		void ContainerAndPackingMockDNM(Mock<IArticle> articleMock, ZString count)
		{
			...
			myPacking.Setup(m => m.ItemsCount).Returns(new ZInt(ZString.Empty));
			myPacking.Setup(m => m.Type).Returns("CT");
			myPacking.Setup(m => m.MarksAndNos).Returns("Test0123456789abcdefghijkl11223344556677889900");
			articleMock.Setup(m => m.Packing).Returns(myPacking.Object);
		}
		...
		{
			articleMock.Setup(m => m.ProcedureCode).Returns(new ZString("40"));
			articleMock.Setup(m => m.PreviousCode).Returns(new ZString("00"));
			articleMock.Setup(m => m.Concession).Returns(new ZString("000"));
		}
		...
		{
			var mySupUnit = new Mock<ISupplementaryUnit>();
		...
		{
			var ecoRegimeDeclDatas1 = new Mock<IEcoRegimeDeclDatas>();
			...
			var ecoRegimeDeclDatas2 = new Mock<IEcoRegimeDeclDatas>();
			...
			var ecoRegimeDatas = new Mock<IEcoRegimeDatas>();
			ecoRegimeDatas.Setup(m => m.GuaranteeAmount).Returns(23.5m);
			ecoRegimeDatas.Setup(m => m.NumberDaysOfDischarge).Returns(12);
			ecoRegimeDatas.Setup(m => m.DecEcos).Returns(new[] { ecoRegimeDeclDatas1.Object, ecoRegimeDeclDatas2.Object });
		...
		void FinancialDatasMock(Mock<IArticle> articleMock, ZDecimal invoiceLinePrice, ZString currencyCode)
		{
			...
			articleMock.Setup(m => m.StatisticalAmount).Returns(50.2m);
			articleMock.Setup(m => m.CustomsValue).Returns(60.5m);
			articleMock.Setup(m => m.TVAAssessedAmount).Returns(70.8);
		...
		{
			var myCostsAndInsurance = new Mock<ICostsAndInsurance>();
		...
		{
			var myIAmountAndCurrencyMock = new Mock<IAmountAndCurrency>();
		...
		{
			var mySupportingDocumentMock = new Mock<ISupportingDocument>();
			if (type.IsEmpty)
			{
		...
		Mock<ISupportingDocumentOnly> SupportingDocumentMock_ISupportingDocumentOnly(ZString type, ZString refNumber, ZString dateIssue, ZString code)
		{
			var mySupportingDocumentMock = new Mock<ISupportingDocumentOnly>();
			if (type.IsEmpty)
			{
				mySupportingDocumentMock.Setup(m => m.Type).Returns(new ZString(null));
			}
		...
		{
			var mySupportingDocumentMock = SupportingDocumentMock_ISupportingDocumentOnly(type, refNumber, dateIssue, code);

			var myImputationsSheetMock = new Mock<IImputationSheet>();
			...
			var myWeightMock = new Mock<IWeight>();
		...
		{
			var myPreCalcEntryLine = new Mock<IPreCalcEntryLine>();
			myPreCalcEntryLine.Setup(m => m.IsVAT).Returns(isVAT);

			var myPreCalcEntryLineSuppUnit = new Mock<ISupplementaryUnit>();
			...
			var myPreCalcEntryLineTax = new Mock<ITax>();
		...
		{
			var myEntryHeaderCharge = new Mock<ITax>();
		...
		{
			var myEcoRegimeDatas = new Mock<IEcoRegimeDatas>();
			var myEcoRegimeDeclDatas = new Mock<IEcoRegimeDeclDatas>();
		...
		{
			var myEcoRegimeAuthorization = new Mock<IEcoRegimeAuthorization>();
			myEcoRegimeAuthorization.Setup(m => m.EcoRegimeAuthorizationNumber).Returns(new ZString(null));
			myEcoRegimeAuthorization.Setup(m => m.EcoRegimeCountryCode).Returns(new ZString(null));
		...
		{
			var myLiquidationItem = new Mock<ILiquidationItem>();
			myLiquidationItem.Setup(m => m.ArticleNumber).Returns(new ZShort(articleNumber));

			var myTaxationDetail = new Mock<ITaxationDetail>();
			...
			var myTaxationDetailTax = new Mock<ITax>();
