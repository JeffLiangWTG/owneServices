...
namespace Enterprise.Customs.FR.Business.CusTempStorage.Testing
{
	...
	sealed class TemporaryStorageBillValidationDeciderTest : TemporaryStorageBillValidationDeciderAbstractTest<TemporaryStorageBillValidationDecider>
	{
		...
		{
			var validationDecider = new TemporaryStorageBillValidationDecider();
			AssertEquals("AllowDuplicateTypeAndNumber should be false when TemporaryStorageBill is null", expected: false, validationDecider.AllowDuplicateTypeAndNumber.Invoke(null));

			var header = Factory.New<TemporaryStorageHeader>();
			header.IsENSReuse = false;
			var bill = header.Bills.AddNew();
			AssertEquals("AllowDuplicateTypeAndNumber should be false when IsENSReuse is false", expected: false, validationDecider.AllowDuplicateTypeAndNumber.Invoke(bill));

			header.IsENSReuse = true;
			AssertEquals("AllowDuplicateTypeAndNumber should be true when IsENSReuse is true", expected: true, validationDecider.AllowDuplicateTypeAndNumber.Invoke(bill));
		}
