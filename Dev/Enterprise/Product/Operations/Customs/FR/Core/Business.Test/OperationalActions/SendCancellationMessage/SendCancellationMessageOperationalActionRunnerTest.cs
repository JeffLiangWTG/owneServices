...
{
	public class SendCancellationMessageOperationalActionRunnerTest : TestCaseWithFactory
	{
		[TestDate(2023, 9, 5, 10, 0, 0)]
		public void TestMessageSentForEachEntry()
		{
			...
			entryHeader1_1.CH_BGMReference = "3B00001000";
			...
			entryHeader1_2.CH_BGMReference = "3B00001000/1";
			...
			entryHeader2_1.CH_BGMReference = "3B00001001";
		...
		[TestDate(2023, 9, 5, 10, 0, 0)]
		public void TestMessageSentForEachEntryOfEachDeclaration()
		{
			var declaration1 = Factory.New<JobDeclaration>();
			declaration1.JE_ApplicationCode = DeclarationApplicationCodeList.Codes.DeltaIE;
		...
		public void TestActionAbortedIfDataObjectHasErrors()
		{
			var declaration1 = Factory.New<JobDeclaration>();
			declaration1.JE_ApplicationCode = DeclarationApplicationCodeList.Codes.DeltaIE;
			var entryHeader1_1 = declaration1.CustomsEntryHeaders.AddNew();
			...
			cancellationMessage.InvalidationReason = "BBB";
			cancellationMessage.Targets = new BusinessObject[] { declaration1 };
			...
			var ediMessages = Factory.Load<EDIMessage>(new ZQuery()).ToArray();
			AssertEquals("No message should be sent.", 0, ediMessages.Length);

			var logMessages = new List<string>()
			{
				"ERROR: Error - InvalidationMotivation: Please enter a Motivation."
			};
			AssertContainsExactElementsInAnyOrder("Log messages must be as expected", logMessages, log.messages);
		}
		...
		void ValidateMessage(List<CusEntryHeader> headers, EDIMessage message)
		{
			...
			AssertEquals("every message should be of the same batch and then have the same EM_ApplicationReference", "0000000000000001", message.EM_ApplicationReference);
