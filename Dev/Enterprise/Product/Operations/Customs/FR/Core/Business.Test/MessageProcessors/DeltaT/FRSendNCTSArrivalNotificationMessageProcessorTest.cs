...
{
	[TestedType(typeof(FRSendNCTSArrivalNotificationMessageProcessor))]
	public class FRSendNCTSArrivalNotificationMessageProcessorTest : AutoSendNCTSMessageProcessorTest<FRSendNCTSArrivalNotificationMessageProcessor>
	{
		public void TestMSNEventLoggedIfMessageSent()
		{
			...
			CombineAssertions(() =>
			{
				...
				AssertEquals("MSN event should have the reference as message type", "007", nctsHeader.Logs.GetAllLogs().Where(l => l.SL_SE_NKEvent == "MSN").First().SL_Reference);
		...
		public void TestMGFEventLoggedIfSendMessageFailed()
		{
			NctsHeader nctsHeader = base.Factory.New<NctsHeader>();
			PrepareNctsHeader(nctsHeader);
			var processor = new FRSendNCTSArrivalNotificationMessageProcessorForTest(nctsHeader) as IProcessor;
			NotificationBuffer notificationBuffer = new NotificationBuffer();
			processor.Process(notificationBuffer);
			CombineAssertions(() =>
			{
				...
				AssertEquals("MGF event should have the reference as message type", "007", nctsHeader.Logs.GetAllLogs().Where(l => l.SL_SE_NKEvent == "MGF").First().SL_Reference);
				...
				AssertEquals("FR AutoNCTSMessage failed to send for header: NCT00000001, Message Type: 007.\r\n", notificationBuffer.AsString);
			});
		}
		...
		protected override void AssertEntryAndMessageResultForEndToEndTest(EU.NCTS.Business.NctsHeader nctsHeader)
		{
			...
			AssertEquals("007", message.EM_MessageType);
		}

		protected override IProcessor CreateProcessor(EU.NCTS.Business.NctsHeader nctsHeader)
		{
			return new FRSendNCTSArrivalNotificationMessageProcessor((NctsHeader)nctsHeader);
		}
...
Error - LocalReferenceNumber: [7] Customer Reference Number only accepts Western European languages characters.";
		...
		public class FRSendNCTSArrivalNotificationMessageProcessorForTest : FRSendNCTSArrivalNotificationMessageProcessor
		{
			public FRSendNCTSArrivalNotificationMessageProcessorForTest(NctsHeader header) : base(header)
			{
			}
