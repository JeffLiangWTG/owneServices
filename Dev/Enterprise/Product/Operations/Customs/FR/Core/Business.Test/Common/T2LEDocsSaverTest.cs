...
{
	sealed class T2LEDocsSaverTest : TestCaseWithFactory
	{
		public void TestT2LIsRenderedAndSavedToEDocs()
		{
			...
			new T2LEDocsSaver(entry).RenderDocumentAndSaveInEDocs();
			Factory.Save();
			...
			AssertContains("T2L - BGMReference", printJobs[0].SP_DocumentName);
			AssertEquals("T2L - BGMReference - T2L.XLSX", printJobs[0].SP_EmailAttachments);
		}
	}
}
