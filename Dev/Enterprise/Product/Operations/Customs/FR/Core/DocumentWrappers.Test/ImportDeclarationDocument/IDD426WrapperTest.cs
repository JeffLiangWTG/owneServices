...
class IDD426WrapperTest : Enterprise.DocumentWrappers.Testing.DocBaseWrapperTest
{
	protected override DocBaseWrapper GetNewDocumentWrapper()
	{
		...
		return IDD426Wrapper.New(message);
	}

	[TestDate(2024, 01, 02, 03, 04, 05)]
	public void TestPrintDateTime()
	{
		var wrapper = new IDD426Wrapper(message, Factory);
	...
	public void TestCRN()
	{
		messageObject.ImportOperation = new MCciOperationType10FR();
	...
	public void TestItems()
	{
		...
		AssertContainsExactElementsInExactOrder(new[] { "1", "2" }, wrapper.Items.Select(item => ((IDD426ItemWrapper)item).ItemNumber));
	}
	...
	public void TestItemsType()
	{
		var wrapper = GetNewDocumentWrapper() as IDD426Wrapper;
		AssertEquals(typeof(IDDGoodsShipmentWrapper<IDD426ItemWrapper>), wrapper.Items.GetType());
	}

	DeltaIEFREDIMessage GetMessage()
	{
		var message = Factory.New<DeltaIEFREDIMessage>();
		message.EM_MessageSubType = "426";
		...
		message.EM_MessageText = resourceRetriever.Value.GetString("Enterprise.Customs.FR.DocumentWrappers.Testing.TestFiles.DeltaIE_IE426ResponseMessage.json");
