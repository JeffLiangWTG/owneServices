...
using REPREPType = CargoWise.Customs.FR.MessageDefinitions.DeltaT.CC013B.ReprepType;

namespace Enterprise.Customs.FR.NCTS.Messaging
{
	public class CC013BMessageBuilder : NctsXmlMessageBuilder<ICC013BDeclaration, NctsMessageFunctionSet.DeclarationAmendmentMessage, Cc013BType>
	{
		public CC013BMessageBuilder(ICC013BDeclaration wrapper, NctsMessageFunctionSet.DeclarationAmendmentMessage messageFunction, ErrorCollector errorCollector) : base(wrapper, messageFunction, errorCollector)
		{
		}

		protected override void PopulateMessageHeader(Cc013BType message)
		{
			base.PopulateMessageHeader(message);
			message.MesTypMes20 = MessageTypes.Cc013B;
		}

		protected override void PopulateMessageBody(Cc013BType message)
		{
		...
		HeaheaType PopulateHeaheaType()
		{
			...
			if (wrapper.ContainerisedIndicator)
			{
				data.ConIndHea96 = Flag.Item1;
			}
			data.DiaLanIndAtDepHea254 = wrapper.DialogLanguageIndicatorAtDeparture;
			data.NctsAccDocHea601Lng = wrapper.NctsAccompanyingDocumentLanguageCode;
			...
			data.AmdPlaHea598 = wrapper.DeclarationPlace;
			data.AmdPlaHea598Lng = wrapper.DeclarationPlaceLanguage;
			data.AmdDatHea599 = wrapper.AmendmentDate;
			data.MotHea1016 = messageFunction.UserReasonForCancellation;
			data.JusRegHea1017 = JustificationReglementaire.Item1;
			data.ComHea1018 = !messageFunction.CommentOnCancellation.IsEmpty ? messageFunction.CommentOnCancellation : null;
			if (wrapper.SafetyAndSecurityData)
			{
			...
			var isPrelodge = wrapper.PrelodgeDeclarationIndicator;
		...
		Trapripc1Type PopulateTrapripc1()
		{
			...
			{
				errorCollector.AddError(Res.GetString("9CEB19C8-D1D0-48E2-AF1C-EBEC2BABDCBA", "Principal is empty"));
			}
			return result;
		}
		...
		Traconce1Type PopulateTraconce1()
		{
			...
			if (consignee != null)
			{
				...
				{
					result.NamCe17 = consignee.CompanyName.TrimEnd();
					result.StrAndNumCe122 = consignee.StreetAndNumber.TrimEnd();
					result.PosCodCe123 = consignee.PostalCode;
					result.CouCe125 = consignee.CountryCode;
					result.CitCe124 = consignee.City;
					result.Nadlngce = consignee.NameAndAddressLanguage.Left(2);
				}
			}
			return result;
		}
		...
		Ctlcl1Type PopulateCtlcl1()
		{
			return new Ctlcl1Type
			{
				AmeTypFlaCl628 = Flag.Item1
			};
		}
