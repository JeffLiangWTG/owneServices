...
public class IDD428Wrapper : DocBaseWrapper, IIDD<IDD428ItemWrapper, IDD428LiquidationWrapper, IDD428DutiesAndTaxWrapper>
{
	public IDD428Wrapper(DeltaIEFREDIMessage message, BusinessObjectFactory factory) : base(message, factory)
	{
		this.message = Argument.NotNull(message, nameof(message));
		this.messageObject = (message.MessageDataObject as IE428MessageDataObject)?.ResponseMessage;
	}

	public static IDD428Wrapper New(DeltaIEFREDIMessage message)
	{
		var messageObejct  = (message.MessageDataObject as IE428MessageDataObject)?.ResponseMessage;

		return messageObejct != null && message.EM_LinkedObject is CusEntryHeader entryHeader
			? new IDD428Wrapper(message, message.Factory)
			: null;
	}

	readonly CC428BType messageObject;
	...
	public ZString CRN => messageObject.ImportOperation.CustomsRegistrationNumber;

	public ZString MRN => messageObject.ImportOperation.MRN;

	public ZString LRN => messageObject.ImportOperation.LRN;

	public DocBaseWrapperCollection<IDD428ItemWrapper> Items => items ??= GetItems();
	DocBaseWrapperCollection<IDD428ItemWrapper> items;

	DocBaseWrapperCollection<IDD428ItemWrapper> GetItems()
	{
		int i = 0;
		var result = new IDDGoodsShipmentWrapper<IDD428ItemWrapper>(Factory);
		GoodsShipment?.GoodsShipmentItem?.ForEach(item => result.Add(IDD428ItemWrapper.New(++i, item, messageObject.DetailedTaxation?.GoodsShipment?.GoodsShipmentItem?.FirstOrDefault(itemFromDetailedTaxation => itemFromDetailedTaxation.DeclarationGoodsItemNumber.Equals(item.DeclarationGoodsItemNumber)), messageObject.GeneralTaxation?.DutiesAndTaxesSummaries, Factory)));
		return result;
	}
