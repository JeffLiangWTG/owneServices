...
[TestedType(typeof(ConsignmentType06Provider))]
sealed class ConsignmentType06ProviderTest : Customs.Business.Testing.DataProviderTestCase<ConsignmentType06Provider>
{
	...
	public void TestGrossMass()
	{
		...
		goodsItem3.BY_GrossWeight = 0.6715m;
		goodsItem3.BY_GrossWeightUnit = Core.Constants.Weight.Tonnes;
	...
	public void TestTransportEquipments()
	{
		...
		AssertArrayEqualsByElements(new[] { 2, 3, 4, 5 }, Provider.TransportEquipments.Select(x => x.SequenceNumeric).ToArray());
	}
	...
	public void TestDepartureTransportMeans()
	{
		...
		AssertArrayEqualsByElements(new[] { 2, 3 }, Provider.DepartureTransportMeans.Select(x => x.SequenceNumeric).ToArray());
	}
	...
	{
		AssertEquals(Array.Empty<INCTSSupportingDocument>(), Provider.SupportingDocuments);
	}

	public void TestTransportDocuments()
	{
		AssertEquals(Array.Empty<IDocument>(), Provider.TransportDocuments);
	}

	public void TestAdditionalReferences()
	{
		AssertEquals(Array.Empty<IDocument>(), Provider.AdditionalReferences);
	}
	...
	protected override void SetUp()
	{
		...
		provider = new ConsignmentType06Provider(header);
	}

	NctsHeader header;
	ConsignmentType06Provider provider;

	protected override ConsignmentType06Provider GetProvider() => provider;
}
