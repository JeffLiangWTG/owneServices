...
sealed class CusGoodsLocationLayoutTest : LayoutsAbstractTest
{
	...
	public void TestTypeVisibility_Phase5ArrivalIncident()
	{
		...
		var location = nctsHeader.EnRouteIncidents.AddNew().GoodsLocation;
		var layout = ((IPanelLayoutProvider)new CusGoodsLocationLayout()).Layout;

		AssertEquals("Type should not NL visible", false, layout.IsVisible(CusGoodsLocationControlBag.Instance.TypeDropEdit, location));
	}
	...
	public void TestTypeOfPanelLayout()
	{
		CombineAssertions("Layout should not have Extensions, no field coloring for address-fields", () =>
		{
			AssertEquals("Should be an IPanelLayout", true, typeof(IPanelLayoutProvider).IsAssignableFrom(typeof(CusGoodsLocationLayout)));
			AssertEquals("Should not be an IPanelLayoutWithExtensions", false, typeof(IPanelLayoutProviderWithExtensions).IsAssignableFrom(typeof(CusGoodsLocationLayout)));
		});
	}
