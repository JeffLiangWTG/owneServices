...
public sealed class ImportInvoiceLineDetailsLayout : IPanelLayoutProvider
{
	...
	static PanelLayout CreateLayout()
	{
		...
		builder.Add(euBag.ValuationAdjustmentPercentageCalcEdit, ControlWidthClass.Auto);
		...
		builder.SetVisibility(euBag.DestinationUsingZZRefCusCodeListCodeFindBox, x =>
		{
			var invoiceLineConfiguration = (Business.InvoiceLineConfiguration)x.Declaration?.Configuration.InvoiceLineConfiguration;
			return invoiceLineConfiguration?.CountryOfDestinationVisibleOnImportControl(x.Declaration) ?? new Business.InvoiceLineConfiguration().CountryOfDestinationVisibleOnImportControl(x);
		}, x => x.ZG_CountryOfDestinationInfo);
