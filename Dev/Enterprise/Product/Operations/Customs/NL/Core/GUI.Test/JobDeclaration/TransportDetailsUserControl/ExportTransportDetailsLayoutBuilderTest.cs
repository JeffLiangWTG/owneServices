...
[TestedType(typeof(ExportTransportDetailsLayoutBuilder))]
sealed class ExportTransportDetailsLayoutBuilderTest : ColumnLayoutBuilderAbstractTest<ExportTransportDetailsLayoutBuilder, JobDeclaration, Customs.GUI.TransportDetailsControlBag>
{
	...
	{
		AssertControlVisibility(Customs.GUI.TransportDetailsControlBag.Instance.TransportInlandRoadUserControl, x => x.JE_TransportModeInland == TransportModes.Road, messageType: MessageTypeList.Codes.Export);
	}

	public void TestTransportInlandSeaUserControlVisible()
	{
		AssertControlVisibility(Customs.GUI.TransportDetailsControlBag.Instance.TransportInlandSeaUserControl, x => x.JE_TransportModeInland == TransportModes.Sea, messageType: MessageTypeList.Codes.Export);
	}

	public void TestTransportInlandIDAndNationalityUserControlVisible()
	{
		AssertControlVisibility(Customs.GUI.TransportDetailsControlBag.Instance.TransportInlandIDAndNationalityUserControl, x => (x.JE_TransportModeInland.IsEmpty || (!x.IsRoadInland && !x.IsSeaInland && !x.IsMailInland && !x.IsFixedInstallationInland)), messageType: MessageTypeList.Codes.Export);
	}
	...
	public void TestFlightAndNationalityUserControlVisibility()
	{
		...
		CombineAssertions(() =>
		{
			...
			AssertEquals("IsVisible = false, JE_MessageType = EXP, JE_TransportMode = ''", expected: false, Layout.IsVisible(NLTransportDetailsControlBag.Instance.FlightAndNationalityUserControl, declaration));

			declaration.JE_TransportMode = TransportTypeList.Codes.Sea;
			AssertEquals("IsVisible = false, JE_MessageType = EXP, JE_TransportMode = SEA", expected: false, Layout.IsVisible(NLTransportDetailsControlBag.Instance.FlightAndNationalityUserControl, declaration));

			declaration.JE_TransportMode = TransportTypeList.Codes.Air;
			AssertEquals("IsVisible = true, JE_MessageType = EXP, JE_TransportMode = AIR", expected: true, Layout.IsVisible(NLTransportDetailsControlBag.Instance.FlightAndNationalityUserControl, declaration));
		});
	}
	...
	protected override ExportTransportDetailsLayoutBuilder GetColumnLayoutBuilderForTesting()
	{
	...
	void AssertControlVisibility(ControlReference controlReference, Func<JobDeclaration, bool> isVisible, string messageType = MessageTypeList.Codes.Export)
	{
		var declaration = Factory.New<JobDeclaration>();
	...
	PanelLayout Layout => layout ?? (layout = new ExportTransportDetailsLayout().Layout);
	PanelLayout layout;
}
