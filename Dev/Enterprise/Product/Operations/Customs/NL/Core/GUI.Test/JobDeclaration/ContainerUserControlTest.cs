...
{ 
	...
	{
		public static void TestSecondSealNumber()
		{
			using (var userControl = new ContainerUserControl())
			{
				AssertNotNull(userControl);

				var controls = userControl.Controls.Find("edSecondSealNum", true);
		...
		public static void TestSealParties()
		{
			using (var userControl = new ContainerUserControl())
			{
				AssertNotNull(userControl);

				var controls = userControl.Controls.Find("SealPartyDropEdit", true);
				...
				controls = userControl.Controls.Find("AdditionalSealPartyDropEdit", true);
		...
		public static void TestSealNumber()
		{
			using (var userControl = new ContainerUserControl())
			{
				AssertNotNull(userControl);

				var controls = userControl.Controls.Find("edSealNum", true);
		...
		public static void TestContainerDetailUserControlTabIndex()
		{
			using (var userControl = new ContainerUserControl())
			{
				foreach (var (controlName, tabIndex) in ContainerUserControlTest.FieldsDetails)
				{
					var actualfield = userControl.Controls.Find(controlName, true)[0];
					if (actualfield == null)
					{
		...
		static IEnumerable<(string ControlName, int TabIndex)> FieldsDetails => new (string, int)[]
		{
			("edContainerNum", 0),
