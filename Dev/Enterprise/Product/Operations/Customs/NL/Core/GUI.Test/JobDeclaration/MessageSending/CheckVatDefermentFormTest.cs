...
[TestedType(typeof(CheckVatDefermentForm))]
class CheckVatDefermentFormTest : ZArchitecture.GUI.Testing.ZFormBasherTest
{
	public void TestFields()
	{
		...
		{
			form.Refresh();
			var eoriNumberPaymentParty = form.FindSingle<ZTextBox>("PaymentPartyEoriNumberTextBox");
			var vatNumberDefermentParty = form.FindSingle<ZTextBox>("VATPartyTaxNumberTextBox");
			CombineAssertions(() =>
			{
				AssertEquals("EORI Number should be ReadOnly", true, eoriNumberPaymentParty.ReadOnly);
				AssertEquals("VAT Number should be ReadOnly", true, vatNumberDefermentParty.ReadOnly);
			});
		}
	}
	...
	{
		using (var form = new CheckVatDefermentForm(declarationNOK))
		{
			form.Show();
			UnitTestUserNotification.Instance.AddAnswer(DialogResult.OK);
			form.FindSingle<Button>("ButtonSubmit").PerformClick();
			AssertEquals("'Payment Party EORI Number' AND 'VAT Party Tax Number' should be filled when sending the message.", UnitTestUserNotification.Instance.LastMessage.Text);
		}
	}
	...
	{
		return new CheckVatDefermentForm(declarationOK);
	}
