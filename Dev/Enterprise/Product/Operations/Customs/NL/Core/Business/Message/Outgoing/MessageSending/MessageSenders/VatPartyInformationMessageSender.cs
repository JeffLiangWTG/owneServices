...
public class VatPartyInformationMessageSender
{
	public VatPartyInformationMessageSender(JobDeclarationMessageSendingObjectParent declarationWrapper)
	{
		declaration = declarationWrapper.ParentDeclaration;
		var wrapper = new RequestForVatPartyInformationWrapper(declaration);
		messageBuilder = new VatPartyInformationMessageBuilder(wrapper);
	}
	...
	protected virtual NLEDIMessage CreateEDIMessage()
	{
		...
		message.EM_MessageSubType = NLConstants.EdiMessageSubTypes.VATPartyInformation;
		message.EM_IsTestMessage = NLCustomsRegistry.Instance.IsNLTestingSystem.Value;
		message.EM_MessageText = messageBuilder.GenerateXmlMessage().GetSerializedString();
