...
{
	[TestedType(typeof(PackageWrapperFromTransitPackageHeader))]
	sealed class PackageWrapperFromTransitPackageHeaderTest : PackageWrapperTest
	{
		...
		public override void TestWrapperMappingFull()
		{
			...
			var wrapperFull = new PackageWrapperFromTransitPackageHeader(packageJob, packageHeader, Factory);
		...
		public void TestOutterCount()
		{
			...
			packageHeader.KPH_PackageID = "LooseID1";
			var wrapper = new PackageWrapperFromTransitPackageHeader(job, packageHeader, Factory);
		...
		public void TestPackageState()
		{
			...
			var packageState = helper.CreatePackageState(rcn, 10, "BOX", null, TransitWarehouseStatuses.Codes.Arrived, rtu);
			Factory.Save();
			...
			var wrapper = new PackageWrapperFromTransitPackageHeader(job, packageHeader, Factory);
			AssertEquals(true, (wrapper.PackageState.WrappedObject as WhsItemPackageState).IsNull);

			wrapper = new PackageWrapperFromTransitPackageHeader(job, packageHeader, Factory);
			wrapper.SetPKGPackage(packageState.WPS_KP_Package);
			AssertEquals(packageState.PK, (wrapper.PackageState.WrappedObject as WhsItemPackageState).PK);
		}
		...
		public void TestHandlingUnit()
		{
			...
			helper.PackPackageIntoHandlingUnit(handlingUnit, packageState, ZDateTimeOffset.Now, "XXX", handlingUnit);
			Factory.Save();
			...
			var wrapper = new PackageWrapperFromTransitPackageHeader(job, packageHeader, Factory);
			AssertEquals(true, (wrapper.HandlingUnit.WrappedObject as PkgPackage).IsNull);

			wrapper = new PackageWrapperFromTransitPackageHeader(job, packageHeader, Factory);
			wrapper.SetPKGPackage(packageState.WPS_KP_Package);
			AssertEquals("HU", wrapper.HandlingUnit.RefNumber);
		}

		#endregion

		#region TestTopLevelHandlingUnit

		public void TestTopLevelHandlingUnit()
		{
			var helper = new WhsTransitTestHelper(Factory);
			var warehouse = helper.CreateTRWWarehouse();
			var location = helper.CreateLocation(warehouse);
			...
			var topHandlingUnitPackage = helper.PackingHelper.CreatePkgHandlingUnit(branch.PK, "TWH");
			var topHandlingUnit = helper.CreateHandlingUnitPackage("TOPHU", topHandlingUnitPackage, rtu);

			helper.PackPackageIntoHandlingUnit(topHandlingUnit, handlingUnit, ZDateTimeOffset.Now, "XXX", topHandlingUnit);
			helper.PackPackageIntoHandlingUnit(handlingUnit, packageState, ZDateTimeOffset.Now, "XXX", topHandlingUnit);
			Factory.Save();
			...
			wrapper = new PackageWrapperFromTransitPackageHeader(job, packageHeader, Factory);
			wrapper.SetPKGPackage(packageState.WPS_KP_Package);
			AssertEquals("TOPHU", wrapper.TopLevelHandlingUnit.RefNumber);
		}
		...
		public void TestParent()
		{
			...
			var dcnWrapper = new FreightWrapperFromWhsItemDispatchConsignment(dcn, Factory);
			var dcnJob = PkgPackageJob.LoadOrCreatePackageJob(dcn);
			...
			packageHeader.CurrentPackageJob = job;
			var wrapper = new PackageWrapperFromTransitPackageHeader(job, packageHeader, Factory);
			AssertEquals("RCN1", wrapper.Parent.JobNumber);

			wrapper = new PackageWrapperFromTransitPackageHeader(dcnJob, packageHeader, Factory);
			wrapper.SetPKGPackage(packageState.WPS_KP_Package);
			AssertEquals("RCN1", wrapper.Parent.JobNumber);
		}
		...
		public void TestStarTrack_QRCodeText_UsingProtectedConstructor()
		{
			...
			var wrapperFull = new PackageWrapperFromTransitPackageHeaderForTest(package, Factory);
			var expectedQRCodeText = $"SYDNEY                        2000            123                                    1   0    0    {ZDateTime.Today.ToString("yyyyMMdd")}CNE SYDNEY                                                                      BOX    #1                                                                                            NN                                            ";
			AssertEquals(expectedQRCodeText, wrapperFull.StarTrack_QRCodeText);
		}
		...
		{
			return new PackageWrapperFromTransitPackageHeader(null, null, Factory);
		}
...
";
		...
		protected override GenericWrapper GetSetupWrapperForDefaultFormatting()
		{
			...
			return new PackageWrapperFromTransitPackageHeader(null, header, Factory);
		}

		#endregion

		class PackageWrapperFromTransitPackageHeaderForTest : PackageWrapperFromTransitPackageHeader
		{
			public PackageWrapperFromTransitPackageHeaderForTest(PkgPackage package, BusinessObjectFactory factory) : base(package, factory)
			{ }
		}
	}
}
