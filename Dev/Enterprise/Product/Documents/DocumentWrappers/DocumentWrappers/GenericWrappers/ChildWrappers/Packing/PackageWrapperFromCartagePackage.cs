...
{
	public class PackageWrapperFromCartagePackage : PackageWrapper
	{
		public PackageWrapperFromCartagePackage(CommonBookedCtgMove moveBO, BusinessObjectFactory factory)
			: base(moveBO, factory)
		{
			MoveBO = moveBO ?? factory.GetNull<CommonBookedCtgMove>();
		}
		readonly CommonBookedCtgMove MoveBO;
		...
		protected override UNDGSubstanceWrapperCollection GetUNDGSubstances()
		{
			...
			foreach (UNDGDataItem dgItem in MoveBO.UNDGs)
			{
		...
		{
			return new PackQTYWrapper(MoveBO.EW_BookedPackCount, MoveBO.EW_F3_NKPackType, BindToLists.GetCachedLists(Factory).OuterPackTypes.GetAsCodeDescriptionPair(), Factory);
		}
		...
		{
			return new PackQTYWrapper(0, MoveBO.EW_F3_NKPackType, BindToLists.GetCachedLists(Factory).OuterPackTypes.GetAsCodeDescriptionPair(), Factory);
		}
		...
		{
			return new WeightWrapper(MoveBO.EW_BookedWeight, MoveBO.EW_WeightUQ, WeightWrapper.StandardDecimalPlaces, BindToLists.GetCachedLists(Factory).WeightUnits, Factory);
		}
		...
		{
			return new VolumeWrapper(MoveBO.EW_BookedVolume, MoveBO.EW_VolumeUQ, BindToLists.GetCachedLists(Factory).VolumeUnits, Factory);
		}
		...
		{
			return new DimensionsWrapper(MoveBO.EW_BookedLength, MoveBO.EW_BookedWidth, MoveBO.EW_BookedHeight, MoveBO.EW_DimUnit, BindToLists.GetCachedLists(Factory).DimensionUnits, Factory);
		}
		...
		{
			return new ContainerWrapperFromCartage(new FreightWrapperFromCartage(MoveBO.Cartage, Factory), null, Factory);
		}
		...
		{
			FreightWrapper[] parents = FreightWrapper.New(MoveBO.Cartage, Factory);
