...
{
	[TestedType(typeof(OrganizationMatchingMessageProcessor))]
	public class OrganizationMatchingMessageProcessorTest : TestCaseWithFactory
	{
		...
		public void TestProcessMessages_EnsureCorrectFilterAndBatchingParameters_ForSuccess()
		{
			...
			using (GlowRegistry.Instance.GlowServiceUriRegistryItem.SetTemporaryValue(Guid.Empty, Guid.Empty, Guid.Empty, "https://localhost/Glow"))
			{
				...
				var ediMessage = CreateEdiMessage(WTG.Shared.Dash.Common.Constants.DataProcessingType.Code.OrganizationMatching, dashDocument.PK.ToGuid());
				...
				dashGlowService.Setup(service => service.MatchOrganisations(It.IsAny<Guid>())).Returns(new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent("Success") });
				
				var dashMessageProcessor = new OrganizationMatchingMessageProcessor(factoryMock.Object, loggerMock.Object, dashGlowService.Object, shipamaxService.Object, dashErrorReporter.Object);
		...
		public void TestProcessMessages_EnsureCorrectFilterAndBatchingParameters_ForFailuer()
		{
			...
			using (GlowRegistry.Instance.GlowServiceUriRegistryItem.SetTemporaryValue(Guid.Empty, Guid.Empty, Guid.Empty, "https://localhost/Glow"))
			{
				...
				var dashGlowService = new Mock<IDashGlowService>();
				var dashErrorReporter = new Mock<IDashErrorReporter>();
				dashGlowService.Setup(service => service.MatchOrganisations(It.IsAny<Guid>())).Returns(new HttpResponseMessage(statusCode) { Content = new StringContent(errorMessageText) });
		...
		static readonly Expression<Func<ZQuery, bool>> ExpectedQueryFilter = x => x.MaximumRows == BatchSize && x.FilterPartsHashKey.Contains("and EM_MessageType = 'ORM'");
	}
}
