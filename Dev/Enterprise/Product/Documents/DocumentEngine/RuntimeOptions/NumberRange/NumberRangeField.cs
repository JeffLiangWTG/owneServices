...
{
	public class NumberRangeField : FilterFieldWithUTSupport, IJsonSerializable
	{
		public NumberRangeField(BusinessObjectFactory factory)
			: base(factory)
		{
			CreateParameters();
		}
		...
		internal NumberRangeField(NumberRangeFieldJsonData data)
			: base(data)
		{
			CreateParameters();
		...
		{
			fFromParam = new SqlParameter(SqlParameterNameGenerator.Next(), SqlDbType.Decimal);
			fToParam = new SqlParameter(SqlParameterNameGenerator.Next(), SqlDbType.Decimal);
		...
		protected override string NonEmptyWhereClause()
		{
			...
			{
				return String.Format((NoResString)@"{0} BETWEEN {1} AND {2}", FieldName, fFromParam, fToParam);
			}
		...
		{
			get { return FilterFieldSuggestedUserControlType.NumberRangeUserControl; }
		}
		...
		public override object ValueAsObject
		{
			get
			{
				return Res.GetString("1a7912e9-cda7-4395-9868-3762dc95b7e7", "Between {0} and {1}", fFromParam.Value.ToString(), fToParam.Value.ToString());
			}
		}
		...
		{
			if (source is NumberRangeField)
			{
				this.To = ((NumberRangeField)source).To;
				this.From = ((NumberRangeField)source).From;
			}
		}
		...
		{
			var filterData = new NumberRangeFilter();
			SetBaseFilterData(filterData);

			filterData.From = From;
			filterData.To = To;

			reportFilterData.NumberRangeFilterCollection.Add(filterData);
		}

		public override void SetFilterValue(ReportFilterData reportFilterData)
		{
			var selectedValue = reportFilterData.NumberRangeFilterCollection.FirstOrDefault(a => a.DisplayName == DisplayName);
		...
		{
			var result = new NumberRangeFieldJsonData()
			{
				From = From,
				To = To
			};
