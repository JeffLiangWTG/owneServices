...
namespace Enterprise.DocumentEngine.GUI.RuntimeOptions.Testing
{
	...
	sealed class DateTimeOffsetRangeFieldUserControlFormBasherTest : ZFormBasherTest
	{
		public void TestSetFilter()
		{
			using (var form = new ZForm())
			using (var control = new DateTimeOffsetRangeFieldUserControl())
			{
				form.Controls.Add(control);
				form.Show();
				var filter = new DateTimeOffsetRangeField(Factory);
		...
		protected override Form GetFormToBashCore()
		{
			...
			result.Width = 700;
			result.Controls.Add(new DateTimeOffsetRangeFieldUserControl());
			return result;
		}
	}
}
