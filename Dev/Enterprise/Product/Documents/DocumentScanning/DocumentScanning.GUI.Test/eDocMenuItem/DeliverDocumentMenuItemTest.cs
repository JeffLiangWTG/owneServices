...
{
	sealed class DeliverDocumentMenuItemTest : eDocMenuItemTestCase
	{
		...
		void RunAssertionsForStorageDocs(eDocMenuItem menuItem, StorageDocsBase bizO)
		{
			...
			AssertEquals("Case: BizO selected under mouse is deleted, Multiple rows selected - multiple rows get precedence", true, menuItem.GetEnabledStatus(bizO, true));
		}
		...
		{
			return new DeliverDocumentMenuItem();
		}
	}
}
