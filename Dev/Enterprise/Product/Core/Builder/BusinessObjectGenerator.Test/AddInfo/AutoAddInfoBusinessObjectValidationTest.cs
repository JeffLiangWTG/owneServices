...
{
	sealed class AutoAddInfoBusinessObjectValidationTest : AutoAddInfoCodeTestCase
	{
		[DatCapabilityRequirement("SOURCE_CODE")]
		public void TestGeneratedSourceCode()
		{
			var autoBusinessObject = new AutoAddInfoBusinessObject(CreateInfo(CreateTestDataTable()));
			var testObject = new AutoAddInfoBusinessObjectValidation(CreateInfo(CreateTestDataTable()), autoBusinessObject.AutoProperties);
			var resourceRetriever = new EmbeddedResourceRetriever(GetType().Assembly);
			var str = resourceRetriever.GetString("Enterprise.BusinessObjectGenerator.Test.Testing.AddInfo.TestGeneratedAutoAddInfoBusinessObjectValidation.cs", System.Text.Encoding.UTF8);
