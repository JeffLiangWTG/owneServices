...
{
	sealed class AutoAddInfoBusinessObjectWithParentSchemaTest : AutoAddInfoCodeTestCase
	{
		public void TestGeneratedSourceCode()
		{
			var testObject = new AutoAddInfoBusinessObject(CreateInfo(CreateTestDataTable(), Array.Empty<string>(), Array.Empty<string>(), Array.Empty<string>(), "Enterprise.Customs.BusinessObject", "Enterprise.Customs.Business", parentView: "EUBusinessObject", parentSchema: "EUAddInfoSchema"));
			var resourceRetriever = new EmbeddedResourceRetriever(GetType().Assembly);
			var str = resourceRetriever.GetString("Enterprise.BusinessObjectGenerator.Test.Testing.AddInfo.TestGeneratedAutoAddInfoBusinessObjectWithParentSchema.cs", System.Text.Encoding.UTF8);
