...
{
	[TestedType(typeof(eTailImportAirFreightManifestConsignments))]
	sealed class eTailImportAirFreightManifestConsignmentsTest : RefStlScriptWithDefaultsTest
	{
		...
		protected override void PrepareTestData()
		{
			...
			INSERT dbo.JobShipment (JS_PK, JS_ShipmentType, JS_TransportMode, JS_RL_NKDestination, JS_IsForwardRegistered, JS_UniqueConsignRef) VALUES 
				(@JsPk01, 'HVL', 'AIR', 'AUSYD', 1, 'Consign1'),
				(@JsPk02, 'STD', 'AIR', 'AUSYD', 1, 'Consign2'),
				(@JsPk03, 'HVL', 'AIR', 'USAAA', 1, 'Consign3'),
				(@JsPk04, 'HVL', 'AIR', 'AUSYD', 1, 'Consign4'),
				(@JsPk05, 'STD', 'AIR', 'AUSYD', 1, 'Consign5'),
				(@JsPk06, 'HVL', 'AIR', 'USAAA', 1, 'Consign6'),
				(@JsPk07, 'HVL', 'AIR', 'AUSYD', 1, 'Consign7'),
				(@JsPk08, 'STD', 'AIR', 'AUSYD', 1, 'Consign8'),
				(@JsPk09, 'HVL', 'AIR', 'USAAA', 1, 'Consign9'),
				(@JsPk10, 'HVL', 'AIR', 'AUSYD', 1, 'Consign10'),
				(@JsPk11, 'STD', 'AIR', 'AUSYD', 1, 'Consign11'),
				(@JsPk12, 'HVL', 'AIR', 'USAAA', 1, 'Consign12');
			...
			INSERT dbo.CusEntryNum (CE_PK, CE_ParentTable, CE_ParentID, CE_EntryType, CE_SystemCreateTimeUtc, CE_SystemCreateUser, CE_SystemLastEditTimeUtc, CE_SystemLastEditUser) VALUES 
				(@CePk01, 'JobConsol', @JkPk01, 'CRN', '2013-10-01', '~BP', getutcdate(), '~BP'),
				(@CePk02, 'JobConsol', @JkPk02, 'CRN', '2013-11-02', '~BP', getutcdate(), '~BP'),
				(@CePk03, 'JobConsol', @JkPk03, 'AAA', '2013-10-03', '~BP', getutcdate(), '~BP'),
				(@CePk04, 'JobConsol', @JkPk04, 'CRN', '2013-09-01', '~BP', getutcdate(), '~BP');
			...
			INSERT dbo.HVLVBookingHeader (HVH_PK, HVH_BookingReference, HVH_OA_BillToParty, HVH_SystemCreateTimeUtc, HVH_SystemLastEditTimeUtc, HVH_SystemCreateUser, HVH_SystemLastEditUser, HVH_ClusterKey) VALUES
				(@HVBPk01, 'M00001', @OaPk, '2013-10-01', '2013-10-01', 'US1', 'US1', @Cluster_Key);

			INSERT dbo.HVLVConsignment (HVC_PK, HVC_HVH_BookingHeader, HVC_ConsignmentId, HVC_ItemCount, HVC_SystemCreateTimeUtc, HVC_SystemLastEditTimeUtc, HVC_Status, HVC_SystemCreateUser, HVC_SystemLastEditUser, HVC_ClusterKey) VALUES
				(@HVCPk01, @HVBPk01, 'Consignee1', '1', '2013-10-01', '2013-10-01', 'CNF', 'US1', 'US1', @Cluster_Key);
			...
			INSERT dbo.HVLVItem (HVI_PK, HVI_HVC_Consignment, HVI_JS_LoadedOnShipment, HVI_ItemId, HVI_ClusterKey, HVI_SystemCreateTimeUtc, HVI_SystemLastEditTimeUtc, HVI_SystemCreateUser, HVI_SystemLastEditUser) VALUES
				(@HVIPk01, @HVCPk01, @JsPk01, '1234', @Cluster_Key, '2013-10-01', '2013-10-01', 'US1', 'US1');
";

			var query = string.Format(sqlText, DLPk01, DLPk02, DLPk03, DLPk04, DLPk05, DLPk06, DLPk07, DLPk08, DLPk09, DLPk10, DLPk11, DLPk12, HVCPk01);
		...
		protected override void AssertResultSet(IEnumerable<IStlTransaction> transactions)
		{
			...
			AssertEquals("[T1] TransactionReference01", "Consignee1", transaction1.Reference1);
