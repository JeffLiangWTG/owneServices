...
{
	[TestedType(typeof(OtherPartiesForwarderSeaCargoExport))]
	sealed class OtherPartiesForwarderSeaCargoExportTest : RefStlScriptWithDefaultsTest
	{
		protected override void PrepareTestData()
		{
			string sqlText = @"
				...
				INSERT dbo.CusEntryNum (CE_PK, CE_ParentID, CE_ParentTable, CE_EntryNum, CE_RN_NKCountryCode, CE_EntryType, CE_Category, CE_SystemCreateTimeUtc, CE_SystemCreateUser, CE_SystemLastEditTimeUtc, CE_SystemLastEditUser) VALUES
					...
					(newid(), @JkPk01, 'JobConsol', 'CPG06', 'AU', 'CRN', 'CUS', '2015-09-26', 'US6', getutcdate(), '~BP');
				...
				INSERT dbo.EDIMessage (EM_PK, EM_GB, EM_GE, EM_ApplicationCode, EM_LinkUniqueID, EM_LinkTable, EM_SystemCreateTimeUtc, EM_ReceiveTransmit, EM_MessageType, EM_MessageSubType, EM_Status, EM_SystemCreateUser, EM_SystemLastEditTimeUtc, EM_SystemLastEditUser) VALUES
					(newid(), @GbPk, @CePkGe, 'CMR', @JkPk02, 'JobConsol', '2014-09-25', 'TRX', 'ESM', 'ORG', 'SNT', 'US5', '2014-09-25', 'US5');";
		...
		protected override void AssertResultSet(IEnumerable<IStlTransaction> transactions)
		{
			...
			AssertEquals("[T1] TransactionDateUtc", new DateTime(2014, 9, 25), transaction1.ServiceOccuredUTC);
			AssertEquals("[T1] UserCode", "US5", transaction1.ClientStaffCode);
			AssertEquals("[T1] ItemCount", 1, transaction1.BillableCount);
			AssertEquals("[T1] TransactionReference01", "CPG05", transaction1.Reference1);
			AssertEquals("[T1] TransactionReference02", "JKSEA", transaction1.Reference2);
		}
