...
{
	[TestedType(typeof(BookingRequestReceivedByNVOCC))]
	sealed class BookingRequestReceivedByNVOCCTest : RefStlScriptWithDefaultsTest
	{
		...
		protected override void AssertResultSet(IEnumerable<IStlTransaction> transactions)
		{
			...
			var row2 = transactions.Single(x => x.Reference4.StartsWith("RES007"));
			...
			CombineAssertions(() =>
			{
				...
				AssertEquals("[Row-2] TransactionDateUtc", new DateTime(2022, 11, 22, 12, 02, 00), row2.ServiceOccuredUTC);
				AssertEquals("[Row-2] ItemCount", 1, row2.BillableCount);
				AssertEquals("[Row-2] TransactionReference01", "S00001007", row2.Reference1);
				AssertEquals("[Row-2] TransactionReference02", "BRC007", row2.Reference2);
				AssertNull("[Row-2] TransactionReference03", row2.Reference3);
				AssertEquals("[Row-2] TransactionReference04", "RES007 - Original", row2.Reference4);
			});
		}
...
DECLARE @jobDocAddr5PK UNIQUEIDENTIFIER = newid();
...
INSERT dbo.StmALog (SL_PK, SL_Table, SL_EventTime, SL_PostedTimeUtc, SL_Parent, SL_SE_NKEvent, SL_Reference, SL_GB_NKBranch, SL_IsCancelled) VALUES
	(newid(), 'JobShipment', getdate(), '2022-11-02 12:01:00', @JsPK01, 'STU', 'Original|RES=RES001|NEW=EBK|TYP=Shipment Status', 'GB1', 'N'), -- 01
	(newid(), 'JobShipment', getdate(), '2022-11-12 12:02:00', @JsPK02, 'STU', 'Amendent|RES=RES002|NEW=EBK|TYP=Shipment Status', 'GB3', 'N'),  -- Amendent
	(newid(), 'JobShipment', getdate(), '2022-10-01 12:03:00', @JsPK03, 'STU', 'Original|RES=RES002|NEW=EBK|TYP=Shipment Status', 'GB0', 'N'), -- Before date range
	(newid(), 'JobShipment', getdate(), '2022-12-01 12:04:00', @JsPK04, 'STU', 'Original|RES=RES003|NEW=EBK|TYP=Shipment Status', 'GB0', 'N'), -- After date range
	(newid(), 'JobShipment', getdate(), '2022-11-01 12:05:00', @JsPK05, 'XXX', 'Original|RES=RES004|NEW=EBK|TYP=Shipment Status', 'GB0', 'N'), -- SL_SE_NKEvent is not STU
	(newid(), 'JobShipment', getdate(), '2022-11-01 12:05:00', @JsPK05, 'STU', 'Original|RES=RES004|NEW=AAA|TYP=Shipment Status', 'GB0', 'N'), -- NEW is not EBK
	(newid(), 'ViewQuotedBooking', getdate(), '2022-11-22 12:02:00', @JsPK06, 'STU', 'Original|RES=RES006|NEW=EBK|TYP=Shipment Status', 'GB3', 'N'),  -- 06
	(newid(), 'ViewQuotedBooking', getdate(), '2022-11-22 12:02:00', @ThPk01, 'STU', 'Original|RES=RES007|NEW=EBK|TYP=Shipment Status', 'GB3', 'N')  -- 07
...
INSERT dbo.JobShipment (JS_PK, JS_UniqueConsignRef, JS_IsBooking, JS_IsShipping, JS_BookingReference, JS_RL_NKOrigin, JS_TH_OneTimeQuote) VALUES
	(@JsPk01, 'S00001001', 0, 1, 'BRC001', 'BGSOF', null),
	(@JsPk02, 'S00001002', 1, 0, 'BRC002', 'PGPOM', null),
	(@JsPk03, 'S00001003', 0, 0, 'BRC003', 'NENIM', null),
	(@JsPk04, 'S00001004', 1, 1, 'BRC004', 'PAPTY', null),
	(@JsPk05, 'S00001005', 1, 1, 'BRC005', 'AUSYD', null),
	(@JsPk06, 'S00001006', 1, 0, 'BRC006', 'PGPOM', null),
	(@JsPk07, 'S00001007', 1, 0, 'BRC007', 'PGPOM', @ThPk01)
...
INSERT dbo.jobDocAddress(E2_PK, E2_ParentID, E2_ParentTableCode, E2_AddressType, E2_City, E2_State, E2_RN_NKCountryCode, E2_GeoLocation, E2_OA_Address,E2_AddressOverride,E2_ValidationStatus, E2_CompanyName, E2_SystemCreateTimeUtc,E2_SystemCreateUser,E2_SystemLastEditTimeUtc,E2_SystemLastEditUser) VALUES
	...
	(@jobDocAddr5PK, @JsPK07, 'JS', 'BKD','','','',geography::Point(47.636, -122.360, 4326), NULL, 0, 'NRQ','', GETUTCDATE(),'TST',GETUTCDATE(),'TST')
";
