...
{
	[TestedType(typeof(eTailImportSeaFreightManifestConsignments))]
	sealed class eTailImportSeaFreightManifestConsignmentsTest : RefStlScriptWithDefaultsTest
	{
		...
		readonly Guid HVCPk01 = Guid.NewGuid();
		readonly Guid HVCPk02 = Guid.NewGuid();
		...
		protected override void PrepareTestData()
		{
			...
			DECLARE @HVBPk01 UNIQUEIDENTIFIER = newid();
			DECLARE @HVCPk01 UNIQUEIDENTIFIER = '{12}';
			DECLARE @HVIPk01 UNIQUEIDENTIFIER = newid();
			DECLARE @HVBPk02 UNIQUEIDENTIFIER = newid();
			DECLARE @HVCPk02 UNIQUEIDENTIFIER = '{13}';
			DECLARE @HVIPk02 UNIQUEIDENTIFIER = newid();

			DECLARE @Cluster_Key1 int = 888;
			DECLARE @Cluster_Key2 int = 999;
			...
			INSERT dbo.JobShipment (JS_PK, JS_ShipmentType, JS_RL_NKDestination, JS_IsForwardRegistered, JS_UniqueConsignRef) VALUES 
				(@JsPk01, 'HVL', 'AUSYD', 1, 'Consign1'),
				(@JsPk02, 'STD', 'AUSYD', 1, 'Consign2'),
				(@JsPk03, 'HVL', 'USAAA', 1, 'Consign3'),
				(@JsPk04, 'HVL', 'AUSYD', 1, 'Consign4'),
				(@JsPk05, 'STD', 'AUSYD', 1, 'Consign5'),
				(@JsPk06, 'HVL', 'USAAA', 1, 'Consign6'),
				(@JsPk07, 'HVL', 'AUSYD', 1, 'Consign7'),
				(@JsPk08, 'STD', 'AUSYD', 1, 'Consign8'),
				(@JsPk09, 'HVL', 'USAAA', 1, 'Consign9'),
				(@JsPk10, 'HVL', 'AUSYD', 1, 'Consign10'),
				(@JsPk11, 'STD', 'AUSYD', 1, 'Consign11'),
				(@JsPk12, 'HVL', 'USAAA', 1, 'Consign12');
			...
			INSERT dbo.CusSCAOceanBill (CB_PK, CB_GB, CB_ApplicationCode, CB_SystemCreateTimeUtc, CB_ParentId, CB_ParentTableCode, CB_SystemCreateUser, CB_SystemLastEditTimeUtc, CB_SystemLastEditUser) VALUES 
				(@CmPk01, @GbPk01, 'CMR', '2013-10-01', @JkPk01, 'JK', '~BP', GetUtcDate(), '~BP'),
				(@CmPk02, @GbPk01, 'AAA', '2013-10-02', @JkPk02, 'JK', '~BP', GetUtcDate(), '~BP'),
				(@CmPk03, @GbPk01, 'CMR', '2013-10-03', @JkPk03, 'JK', '~BP', GetUtcDate(), '~BP'),
				(@CmPk04, @GbPk01, 'CMR', '2013-09-02', @JkPk04, 'JK', '~BP', GetUtcDate(), '~BP');
			...
			INSERT dbo.HVLVBookingHeader (HVH_PK, HVH_ClusterKey, HVH_BookingReference, HVH_OA_BillToParty, HVH_SystemCreateTimeUtc, HVH_SystemLastEditTimeUtc, HVH_SystemCreateUser, HVH_SystemLastEditUser) VALUES
				(@HVBPk01, @Cluster_Key1, 'M00001', @OaPk, '2013-10-01', '2013-10-01', 'US1', 'US1'),
				(@HVBPk02, @Cluster_Key2, 'M00007', @OaPk, '2013-10-01', '2013-10-01', 'US1', 'US1');

			INSERT dbo.HVLVConsignment(HVC_PK, HVC_HVH_BookingHeader, HVC_ConsignmentId, HVC_WaybillNumber, HVC_ItemCount, HVC_SystemCreateTimeUtc, HVC_SystemLastEditTimeUtc, HVC_Status, HVC_SystemCreateUser, HVC_SystemLastEditUser, HVC_ClusterKey) VALUES
				(@HVCPk01, @HVBPk01, 'Consignee1', 'Waybill1', '1', '2013-10-01', '2013-10-01', 'CNF', 'US1', 'US1', @Cluster_Key1),
				(@HVCPk02, @HVBPk02, 'Consignee7', 'Waybill7', '1', '2013-10-01', '2013-10-01', 'CNF', 'US7', 'US7', @Cluster_Key2);

			INSERT dbo.HVLVItem (HVI_PK, HVI_HVC_Consignment, HVI_JS_LoadedOnShipment, HVI_ItemId, HVI_ClusterKey, HVI_SystemCreateTimeUtc, HVI_SystemLastEditTimeUtc, HVI_SystemCreateUser, HVI_SystemLastEditUser) VALUES
				(@HVIPk01, @HVCPk01, @JsPk01, '1234', @Cluster_Key1, '2013-10-01', '2013-10-01', 'US1', 'US1'),
				(@HVIPk02, @HVCPk02, @JsPk07, '777', @Cluster_Key2, '2013-10-01', '2013-10-01', 'US7', 'US7');
";

			var query = string.Format(sqlText, DLPk01, DLPk02, DLPk03, DLPk04, DLPk05, DLPk06, DLPk07, DLPk08, DLPk09, DLPk10, DLPk11, DLPk12, HVCPk01, HVCPk02);
		...
		protected override void AssertResultSet(IEnumerable<IStlTransaction> transactions)
		{
			...
			AssertEquals("[T1] TransactionReference02", HVCPk01.ToString().ToUpper(), transaction1.Reference2);
			AssertEquals("[T1] TransactionReference03", "con1", transaction1.Reference3);
			AssertEquals("[T1] TransactionReference04", "Consign1", transaction1.Reference4);
			AssertEquals("[T1] TransactionGuidReference", HVCPk01, Guid.Parse(transaction1.Reference5));
			...
			var transaction2 = FindRowByRef1(transactions, "Consignee7");
			...
			AssertEquals("[T2] TransactionReference02", HVCPk02.ToString().ToUpper(), transaction2.Reference2);
			AssertEquals("[T2] TransactionReference03", "con3", transaction2.Reference3);
			AssertEquals("[T2] TransactionReference04", "Consign7", transaction2.Reference4);
			AssertEquals("[T2] TransactionGuidReference", HVCPk02, Guid.Parse(transaction2.Reference5));
			AssertEquals("[T2] BranchCode", "BNE", transaction2.GetBranchCode());
			AssertEquals("[T2] UserCode", "US7", transaction2.ClientStaffCode);
