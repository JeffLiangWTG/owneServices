...
{
	[TestedType(typeof(RaisedExceptions))]
	[TestDate(2022, 12, 08, 10, 0, 0)]
	class RaisedExceptionsTest : RefStlScriptWithDefaultsTest
	{
		protected override bool IsMandatoryForMilestones => false;

		const int exceptionsCreatedByStaff_GGG_InCompany1_AAA_WithinMonth = 7;
		const int exceptionsCreatedByStaff_FFF_InCompany1_BBB_WithinMonth = 4;
		const int exceptionsCreatedByStaff_FFF_InCompany1_BBB_OutsideMonth = 6;
		const int exceptionsCreatedByStaff_EEE_InCompany1_AAA_WithinMonth = 2;
		...
		protected override void PrepareTestData()
		{
			...
			using (EnvProxy.Instance.SetTemporaryUserContext(staff3.GS_LoginName, branch1.PK.ToGuid(), department.PK.ToGuid()))
			{
				...
				for (int i = 0; i < exceptionsCreatedByStaff_GGG_InCompany1_AAA_WithinMonth; i++)
				{
			...
			{
				for (int i = 0; i < exceptionsCreatedByStaff_EEE_InCompany1_AAA_WithinMonth; i++)
				{
					((IWorkflowProvider)jobHeader1.Parent).WorkflowItems.Exceptions.AddNew();
				}
				factory.Save();
			...
			using (EnvProxy.Instance.SetTemporaryUserContext(staff2.GS_LoginName, branch2.PK.ToGuid(), department.PK.ToGuid()))
			{
				...
				for (int i = 0; i < exceptionsCreatedByStaff_FFF_InCompany1_BBB_WithinMonth; i++)
				{
					((IWorkflowProvider)jobHeader1.Parent).WorkflowItems.Exceptions.AddNew();
				}
				factory.Save();
				...
				for (int i = 0; i < exceptionsCreatedByStaff_FFF_InCompany1_BBB_OutsideMonth; i++)
				{
					((IWorkflowProvider)jobHeader1.Parent).WorkflowItems.Milestones.AddNew();
				}
				factory.Save();
		...
		protected override void AssertResultSet(IEnumerable<IStlTransaction> transactions)
		{
			...
			AssertRow(transactions, 0, "AAA", "CCC", "GGG", exceptionsCreatedByStaff_GGG_InCompany1_AAA_WithinMonth);
			AssertRow(transactions, 1, "AAA", "CCC", "EEE", exceptionsCreatedByStaff_EEE_InCompany1_AAA_WithinMonth);
			AssertRow(transactions, 2, "BBB", "DDD", "FFF", exceptionsCreatedByStaff_FFF_InCompany1_BBB_WithinMonth);
		}
