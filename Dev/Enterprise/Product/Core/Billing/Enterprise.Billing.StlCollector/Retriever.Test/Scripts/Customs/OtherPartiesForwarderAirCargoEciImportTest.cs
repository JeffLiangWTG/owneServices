...
{
	[TestedType(typeof(OtherPartiesForwarderAirCargoEciImport))]
	sealed class OtherPartiesForwarderAirCargoEciImportTest : RefStlScriptWithDefaultsTest
	{
		protected override void PrepareTestData()
		{
			string sqlText = @"
				...
				INSERT dbo.JobDeclaration (JE_PK, JE_DataModel, JE_DeclarationReference, JE_MessageType, JE_MessageSubType, JE_GB, JE_GC, JE_SystemCreateTimeUtc, JE_SystemCreateUser, JE_ClusterKey) VALUES
					(newid(), 'NZ', 'DEC01', 'IMP', 'ECI', @GbPkNz, @GcPkNz, '2014-10-01', 'US1', 1),
					(newid(), 'SG', 'DEC02', 'IMP', 'ECI', @GbPkSg, @GcPkSg, '2014-09-02', 'US2', 2),
					(newid(), 'NZ', 'DEC03', 'IMP', 'ECI', @GbPkNz, @GcPkNz, '2014-09-03', 'US3', 3),
					(newid(), 'NZ', 'DEC04', 'XXX', 'ECI', @GbPkNz, @GcPkNz, '2014-09-04', 'US4', 4),
					(newid(), 'NZ', 'DEC05', 'IMP', 'ECI', @GbPkNz, @GcPkNz, '2014-09-05', 'US5', 5),
					(newid(), 'NZ', 'DEC06', 'IMP', 'XXX', @GbPkNz, @GcPkNz, '2014-09-06', 'US6', 6);";
		...
		protected override void AssertResultSet(IEnumerable<IStlTransaction> transactions)
		{
			...
			var transaction1 = FindRowByRef1(transactions, "DEC03");
			AssertEquals("[T1] CompanyCode", "DEM", transaction1.GetCompanyCode());
			AssertEquals("[T1] BranchCode", "DEM", transaction1.GetBranchCode());
			AssertEquals("[T1] TransactionDateUtc", new DateTime(2014, 9, 3), transaction1.ServiceOccuredUTC);
			...
			var transaction2 = FindRowByRef1(transactions, "DEC05");
			AssertEquals("[T2] CompanyCode", "DEM", transaction2.GetCompanyCode());
			AssertEquals("[T2] BranchCode", "DEM", transaction2.GetBranchCode());
			AssertEquals("[T2] TransactionDateUtc", new DateTime(2014, 9, 5), transaction2.ServiceOccuredUTC);
