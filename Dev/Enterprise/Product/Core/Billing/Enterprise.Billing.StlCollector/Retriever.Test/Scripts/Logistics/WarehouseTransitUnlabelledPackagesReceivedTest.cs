...
{
	[TestedType(typeof(WarehouseTransitUnlabelledPackagesReceived))]
	sealed class WarehouseTransitUnlabelledPackagesReceivedTest : RefStlScriptWithDefaultsTest
	{
		...
		protected override void PrepareTestData()
		{
			string sqlText = @"
				...
				INSERT INTO dbo.PkgPackageHeader(KPH_PK, KPH_PackageID, KPH_SystemCreateTimeUtc, KPH_SystemCreateUser, KPH_SystemLastEditTimeUtc, KPH_SystemLastEditUser) VALUES
					...
					(@KP03OuterInRCN2IDPK, 'KP04', GetUtcDate(), '~BP', GetUtcDate(), '~BP'),
					(@KP04OuterInRCN2IDPK, 'KP05', GetUtcDate(), '~BP', GetUtcDate(), '~BP'),
					(@KP05PackageIDInRCN1PK, 'KP07', GetUtcDate(), '~BP', GetUtcDate(), '~BP'),
		...
		protected override void AssertResultSet(IEnumerable<IStlTransaction> transactions)
		{
			...
			WarehouseTransitPackagesCommonTest.AssertRowSet(transactions, 0, "DEM", "DEM", "US2", 3, "KP07", "WRH02", new DateTimeOffset(2014, 10, 31, 21, 0, 0, TimeSpan.FromHours(8)));
			WarehouseTransitPackagesCommonTest.AssertRowSet(transactions, 1, "DEM", "DEM", "US1", 5, "KP01", "RC001", new DateTimeOffset(2014, 11, 01, 1, 1, 1, TimeSpan.FromHours(8)));
			WarehouseTransitPackagesCommonTest.AssertRowSet(transactions, 2, "DEM", "DEM", "US2", 3, "KP02", "WRH02", new DateTimeOffset(2014, 11, 02, 2, 2, 2, TimeSpan.FromHours(8)));
			WarehouseTransitPackagesCommonTest.AssertRowSet(transactions, 3, "DEM", "DEM", "US1", 8, "KP04", "RC002", new DateTimeOffset(2014, 11, 03, 3, 3, 3, TimeSpan.FromHours(11)));
		}
		...
		{
			WarehouseTransitPackagesCommonTest.AssertScriptShouldUseDateTimeOffsetParameters(new RefStlScriptRetriever(new WarehouseTransitUnlabelledPackagesReceived()));
		}
	}
}
