...
{
	[TestedType(typeof(CompletedWorkflows))]
	[TestDate(2023, 02, 14, 10, 0, 0)]
	class CompletedWorkflowsTest : RefStlScriptWithDefaultsTest
	{
		protected override bool IsMandatoryForMilestones => false;

		const int WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedLastMonth_Closed = 3;

		const int WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedCurrentMonth_Open = 5;
		const int WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedCurrentMonth_Closed = 2;
		const int WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedCurrentMonth_ClosedAndReopened2Times = 8;

		const int WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedNextMonth_Closed = 6;

		const int WorkflowsCreatedByStaff_FFF_InCompany_BBB_CreatedCurrentMonth_Open = 1;
		const int WorkflowsCreatedByStaff_FFF_InCompany_BBB_CreatedCurrentMonth_Closed = 4;

		const int WorkflowsCreatedByStaff_GGG_InCompany_AAA_CreatedCurrentMonth_Closed = 3;
		...
		protected override void PrepareTestData()
		{
			...
			using (EnvProxy.Instance.SetTemporaryUserContext(staff1.GS_LoginName, branch1.PK.ToGuid(), department.PK.ToGuid()))
			{
				...
				for (int i = 0; i < WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedLastMonth_Closed; i++)
				{
				...
				for (int i = 0; i < WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedCurrentMonth_Open; i++)
				{
				...
				for (int i = 0; i < WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedCurrentMonth_Closed; i++)
				{
					var workflow = helper.CreateWorkflow(jobHeader1, $"C{i}");
					var task = (ProcessTask)helper.CreateTask(workflow, GlbStaff.CurrentUser.GS_Code, 1, taskStatus: ProcessTaskStatusCodeList.Codes.Closed);
				}
				factory.Save();
				...
				for (int i = 0; i < WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedNextMonth_Closed; i++)
				{
			...
			using (EnvProxy.Instance.SetTemporaryUserContext(staff2.GS_LoginName, branch2.PK.ToGuid(), department.PK.ToGuid()))
			{
				...
				for (int i = 0; i < WorkflowsCreatedByStaff_FFF_InCompany_BBB_CreatedCurrentMonth_Open; i++)
				{
				...
				for (int i = 0; i < WorkflowsCreatedByStaff_FFF_InCompany_BBB_CreatedCurrentMonth_Closed; i++)
				{
			...
			using (EnvProxy.Instance.SetTemporaryUserContext(staff3.GS_LoginName, branch1.PK.ToGuid(), department.PK.ToGuid()))
			{
				...
				for (int i = 0; i < WorkflowsCreatedByStaff_GGG_InCompany_AAA_CreatedCurrentMonth_Closed; i++)
				{
					var workflow = helper.CreateWorkflow(jobHeader1, $"C{i}");
					var task = (ProcessTask)helper.CreateTask(workflow, GlbStaff.CurrentUser.GS_Code, 1, taskStatus: ProcessTaskStatusCodeList.Codes.Closed);
				}
		...
		protected override void AssertResultSet(IEnumerable<IStlTransaction> transactions)
		{
			...
			AssertRow(transactions, 0, "AAA", "CCC", "EEE", WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedCurrentMonth_Closed + 2 * WorkflowsCreatedByStaff_EEE_InCompany_AAA_CreatedCurrentMonth_ClosedAndReopened2Times);
			AssertRow(transactions, 1, "BBB", "DDD", "FFF", WorkflowsCreatedByStaff_FFF_InCompany_BBB_CreatedCurrentMonth_Closed);
			AssertRow(transactions, 2, "AAA", "CCC", "GGG", WorkflowsCreatedByStaff_GGG_InCompany_AAA_CreatedCurrentMonth_Closed);
		}
		...
		{
			var script = new CompletedWorkflows();
