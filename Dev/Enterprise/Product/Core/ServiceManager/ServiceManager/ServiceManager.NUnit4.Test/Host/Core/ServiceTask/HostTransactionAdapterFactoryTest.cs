...
class HostTransactionAdapterFactoryTest
{
	[Test]
	public void TestWrongConstructorParamsCall()
	{
		var exception = Assert.Throws<ArgumentNullException>(() => new HostTransactionAdapterFactory(null));
		Assert.That(exception?.ParamName, Is.EqualTo("serviceProvider"));
	}

	[Test]
	public void TestCreateNativeServiceTaskTransaction()
	{
		// Arrange
		...
		var factory = new HostTransactionAdapterFactory(serviceProvider);
