...
namespace Enterprise.eHubMessaging.Business
{
	internal sealed class XmlCharType
	{
		...
		static readonly ImmutableArray<byte> s_CharProperties = ImmutableArray.Create(CreateCharPropertiesArray());

		static byte[] CreateCharPropertiesArray()
		{
			var result = new byte[65536];
			SetProperties(result, s_Whitespace, FWHITESPACE);
			SetProperties(result, s_Letter, FLETTER);
			SetProperties(result, s_StartName, FSTARTNAME);
			SetProperties(result, s_Name, FNAME);
			SetProperties(result, s_CharData, FCHARDATA);
			SetProperties(result, s_PublicID, FPUBLICID);
			return result;
		}

		static void SetProperties(byte[] charProperties, string ranges, byte value)
		{
