...
{
	class TRCustomsMessageHandlerForGlobalManifestTest : TestCaseWithFactory
	{
		public void TestCreateInterchange()
		{
			...
			message.Setup(m => m.SchemaName).Returns(EDIInterchangeTypeList.Descriptions.TRCustomsGlobalManifest);
			...
			var handler = new TRCustomsMessageHandlerForGlobalManifest();
			...
			CombineAssertions(() =>
			{
				...
				AssertEquals("EI_InterchangeType", EDIInterchangeTypeList.Codes.TRCustomsGlobalManifest, interchange.EI_InterchangeType);
