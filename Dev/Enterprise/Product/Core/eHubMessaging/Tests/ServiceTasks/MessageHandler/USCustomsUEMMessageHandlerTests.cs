...
{
	class USCustomsUEMMessageHandlerTests : TestCaseWithFactory
	{
		public void TestCreateInterchangeCore()
		{
			...
			message.Setup(m => m.ApplicationCode).Returns(ApplicationCodeList.Codes.USExportManifest);
			...
			var handler = new USCustomsUEMMessageHandler();
			...
			AssertEquals(EDIInterchangeTypeList.Codes.USCustomsExportManifest, interchange.EI_InterchangeType);
