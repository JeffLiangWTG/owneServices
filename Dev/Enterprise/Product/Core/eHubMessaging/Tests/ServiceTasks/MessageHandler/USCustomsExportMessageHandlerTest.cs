...
{
	class USCustomsExportMessageHandlerTest : TestCaseWithFactory
	{
		public void TestCreateInterchangeCore()
		{
			...
			message.Setup(m => m.ApplicationCode).Returns(EDIInterchange.ApplicationCodes.USCustomsExport);
			...
			var handler = new USCustomsExportMessageHandler();
			...
			AssertEquals(EDIInterchange.ApplicationCodes.USCustomsExport, interchange.EI_ApplicationCode);
