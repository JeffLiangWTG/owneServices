...
{
	class IsWeightUnitMetricEDWIntegrationTest : TransactionedTestCase
	{
		public void TestIsWeightUnitMetric()
		{
			...
			foreach (FieldInfo info in typeof(Constants.Weight).GetFields(BindingFlags.Public | BindingFlags.Static))
			{
				...
				string actualEDW = IsWeightUnitMetricEDW(code);

				if (actualEDW != expected)
				{
					errors.Add(string.Format("{0} ({1}) - In code: {2}, but in sql: {3}", code, Constants.Weight.GetDescription(code, Constants.PluralState.Plural), expected, actualEDW));
				}
			}
		...
		string IsWeightUnitMetricEDW(string unitOfWeight)
		{
			var sql = string.Format(CultureInfo.InvariantCulture, @"select value from [{0}].dbo.IsWeightUnitMetric(@unitOfWeight)", Db.EdwDatabaseName);
