using System;
using System.Linq;
using System.Security.Cryptography.X509Certificates;
using CargoWise.Windows.UI;
using Enterprise.ZArchitecture;
using Enterprise.ZArchitecture.Environment;
using Enterprise.ZArchitecture.GUI;
using NUnit.Framework;

namespace Enterprise.Registry.GUI.Testing
{
	public class DigitalCertificateControl_p12Test : TestCase
	{
		public void TestSetFileData()
		{
			using (var control = GetNewControl())
			{
				control.SetFileData(new byte[] { 1, 2 });
				AssertEquals("State", DataLoadState.DataExists, control.State);
				AssertEquals("FileDataAsBinary", new byte[] { 1, 2 }, control.FileDataAsBinary());

				control.SetFileData(Array.Empty<byte>());
				AssertEquals("State", DataLoadState.NoData, control.State);
				AssertEquals("FileDataAsBinary", Array.Empty<byte>(), control.FileDataAsBinary());
			}
		}

		[TestDate(2019, 01, 01)]
		public void TestShowCertificateDetails()
		{
			UnitTestUserNotification.Instance.ClearMessagesAndAnswers();
			string expectedMessage;

			using (var certificate = new X509Certificate2(validCert, validCertPassword, X509KeyStorageFlags.MachineKeySet))
			{
				DateTime validFromDate = certificate.NotBefore;
				DateTime validToDate = certificate.NotAfter;

				expectedMessage = string.Format(
					"Issued To: CN=13149600150-003, OU=Servizi Autenticazione, O=Agenzia delle Dogane, C=IT\n" +
					"Issued By: CN=CA Agenzia delle Dogane, OU=Servizio Telematico, O=Agenzia delle Dogane, C=IT\n\n" +
					"Valid From: {0}\n" +
					"Valid To: {1}\n\n" +
					"Serial Number: 482E902A4DA913C9\n\n" +
					"Errors: None\n" +
					"Warnings: None",
					validFromDate, validToDate);
			}

			AssertEquals("Precondition: LastMessage.Text should be null.", null, UnitTestUserNotification.Instance.LastMessage.Text);

			using (var control = GetNewControl())
			{
				control.Show();
				control.SetFileData(validCert);
				control.CertificatePassword = validCertPassword;
				control.FindSingle<ZButton>("ViewButton").PerformClick();
				AssertEquals("LastMessage.Text", expectedMessage, UnitTestUserNotification.Instance.LastMessage.Text);
			}
		}

		[TestDate(2000, 1, 1)]
		public void TestShowSACertificateDetails()
		{
			UnitTestUserNotification.Instance.ClearMessagesAndAnswers();
			string expectedMessage;

			using (var certificate = new X509Certificate2(validCertSA, "", X509KeyStorageFlags.MachineKeySet))
			{
				DateTime validFromDate = certificate.NotBefore;
				DateTime validToDate = certificate.NotAfter;
				TestDateAttribute.Date = validFromDate;

				expectedMessage = string.Format(
					"Issued To: CN=127.0.0.1, OU=haya yaghmour, O=agile, C=SA\n" +
					"Issued By: CN=TSZEINVOICE-SubCA-1, DC=extgazt, DC=gov, DC=local\n\n" +
					"Valid From: {0}\n" +
					"Valid To: {1}\n\n" +
					"Serial Number: 6F00007B7500615537E08FFEE5000100007B75\n\n" +
					"Subject Alternative Names: \n" +
					"     Directory Address:\n" +
					"     OID.2.5.4.15=Food Bussiness3\r\n" +
					"     OID.2.5.4.26=Zatca 12\r\n" +
					"     T=1100\r\n" +
					"     OID.0.9.2342.19200300.100.1.1=300075588700003\r\n" +
					"     SN=1-haya|2-234|3-112\r\n\n\n" +
					"Errors: None\n" +
					"Warnings: None",
					validFromDate, validToDate);
			}

			AssertEquals("Precondition: LastMessage.Text should be null.", null, UnitTestUserNotification.Instance.LastMessage.Text);

			using (var control = GetNewControl())
			{
				control.Show();
				control.SetFileData(validCertSA);
				control.CertificatePassword = "";
				control.FindSingle<ZButton>("ViewButton").PerformClick();
				AssertEquals("LastMessage.Text", expectedMessage, UnitTestUserNotification.Instance.LastMessage.Text);
			}
		}

		[TestDate(2020, 7, 15)]
		public void TestWarnings()
		{
			using (var certificate = new X509Certificate2(validCert, validCertPassword, X509KeyStorageFlags.MachineKeySet))
			using (var control = GetNewControl())
			{
				control.Show();
				control.SetFileData(validCert);
				control.CertificatePassword = validCertPassword;
				control.UpdateCertificate();

				var actualWarnings = control.Warnings();
				AssertEquals("Warning count", 1, actualWarnings.Length);
				AssertEquals("Warning message", ExpectedCertificateAboutToExpireMessage(certificate.NotAfter), actualWarnings.Single());
			}
		}

		[TestDate(2020, 05, 01)]
		public virtual void TestWarnings_WithDaysBeforeExpiryWarningMessage()
		{
			using (var certificate = new X509Certificate2(validCert, validCertPassword, X509KeyStorageFlags.MachineKeySet))
			using (var control = GetNewControl())
			{
				control.DaysBeforeExpiryWarningMessage = 90;
				control.Show();
				control.SetFileData(validCert);
				control.CertificatePassword = validCertPassword;
				control.UpdateCertificate();

				var actualWarnings = control.Warnings();
				AssertEquals("Warning count", 1, actualWarnings.Length);
				AssertEquals("Warning message", ExpectedCertificateAboutToExpireMessage(certificate.NotAfter), actualWarnings.Single());
			}
		}

		public void TestShowCertificateDetailsIfDataIsEmpty()
		{
			using (var control = GetNewControl())
			{
				AssertEquals("Precondition: FileDataAsBinary should be null.", null, control.FileDataAsBinary());

				control.Show();
				control.FindSingle<ZButton>("ViewButton").PerformClick();
				AssertEquals("LastMessage.Text", "There is currently no Certificate loaded.", UnitTestUserNotification.Instance.LastMessage.Text);
			}
		}

		public void TestSupportingFileExtensionList()
		{
			using (var control = new DigitalCertificateControl_p12TestClass())
			{
				AssertContainsExactElementsInAnyOrder(control.SupportingFileExtensionList, new[] { ".P12", ".PFX" });
			}
		}

		public void TestUserFeedbackLabel_Visible()
		{
			using (var control = GetNewControl())
			{
				control.ShowUserFeedbackLabel = true;
				AssertEquals(true, control.FindSingle<ZLabel>("UserFeedbackLabel").Visible);
			}
		}

		public void TestUserFeedbackLabel_Invisible()
		{
			using (var control = GetNewControl())
			{
				control.ShowUserFeedbackLabel = false;
				AssertEquals(false, control.FindSingle<ZLabel>("UserFeedbackLabel").Visible);
			}
		}

		public void TestLoadButtonText_Default()
		{
			using (var control = GetNewControl())
			{
				control.LoadButtonText = DigitalCertificateLoadButtonText.Default;
				AssertEquals("LoadButtonText(Default) => Load", "Load", control.FindSingle<ZButton>("LoadButton").CaptionResourceString.Caption);
			}
		}

		public void TestLoadButtonText_Load()
		{
			using (var control = GetNewControl())
			{
				control.LoadButtonText = DigitalCertificateLoadButtonText.Load;
				AssertEquals("LoadButtonText(Load) => Load", "Load", control.FindSingle<ZButton>("LoadButton").CaptionResourceString.Caption);
			}
		}

		public void TestLoadButtonText_Add()
		{
			using (var control = GetNewControl())
			{
				control.LoadButtonText = DigitalCertificateLoadButtonText.Add;
				AssertEquals("LoadButtonText(Add) => Add", "Add", control.FindSingle<ZButton>("LoadButton").CaptionResourceString.Caption);
			}
		}

		public void TestLoadButtonText_Invalid()
		{
			using (var control = GetNewControl())
			{
				control.LoadButtonText = (DigitalCertificateLoadButtonText)1234;
				AssertEquals("LoadButtonText(???) => Load", "Load", control.FindSingle<ZButton>("LoadButton").CaptionResourceString.Caption);
			}
		}

		public void TestIExtendedControl()
		{
			CombineAssertions(() =>
			{
				using (var control = GetNewControl())
				{
					var provider = (IExtendedControl)control;
					AssertSame("Host", control, provider.Host);
					AssertType<DefaultControlExtensionCollection>("Extensions", control.Extensions);
				}
			});
		}

		protected virtual DigitalCertificateControl_p12 GetNewControl() => new DigitalCertificateControl_p12();

		protected virtual string ExpectedCertificateAboutToExpireMessage(DateTime expiryDate) => $"The certificate will shortly expire.  Please replace this certificate by {expiryDate}.";

		const string validCertPassword = "CaRgOw1sE";

		readonly byte[] validCert = new byte[]
		{
				48, 130, 8, 50, 2, 1, 3, 48, 130, 7, 236, 6, 9, 42, 134, 72, 134, 247, 13, 1, 7, 1, 160, 130, 7, 221, 4, 130, 7, 217, 48, 130, 7, 213, 48,
				130, 3, 26, 6, 9, 42, 134, 72, 134, 247, 13, 1, 7, 1, 160, 130, 3, 11, 4, 130, 3, 7, 48, 130, 3, 3, 48, 130, 2, 255, 6, 11, 42, 134, 72,
				134, 247, 13, 1, 12, 10, 1, 2, 160, 130, 2, 178, 48, 130, 2, 174, 48, 40, 6, 10, 42, 134, 72, 134, 247, 13, 1, 12, 1, 3, 48, 26, 4, 20, 3,
				201, 53, 132, 254, 125, 104, 210, 117, 214, 94, 59, 92, 119, 146, 241, 28, 192, 223, 218, 2, 2, 4, 0, 4, 130, 2, 128, 193, 8, 229,
				243, 255, 114, 100, 3, 49, 196, 12, 225, 34, 75, 208, 74, 147, 224, 211, 120, 110, 214, 57, 117, 191, 242, 23, 218, 199, 80, 207, 10,
				107, 209, 177, 156, 176, 248, 43, 130, 127, 83, 149, 50, 161, 6, 133, 70, 236, 228, 69, 59, 2, 9, 176, 223, 127, 233, 66, 217, 27, 217,
				74, 203, 86, 0, 233, 240, 40, 57, 16, 75, 40, 47, 121, 60, 239, 32, 234, 137, 160, 30, 197, 86, 203, 216, 181, 140, 9, 50, 21, 143, 5,
				59, 218, 110, 137, 45, 138, 235, 121, 40, 96, 234, 4, 165, 76, 167, 91, 205, 82, 167, 49, 95, 251, 24, 134, 34, 7, 251, 212, 216, 238,
				213, 71, 65, 29, 129, 201, 224, 146, 8, 135, 185, 217, 58, 232, 87, 208, 74, 224, 148, 150, 170, 100, 188, 99, 42, 167, 167, 82, 170, 23,
				92, 144, 203, 90, 218, 241, 134, 238, 92, 122, 79, 51, 97, 186, 192, 9, 18, 207, 7, 66, 231, 87, 117, 30, 212, 186, 98, 174, 112, 240, 84,
				154, 224, 65, 208, 247, 185, 70, 5, 151, 151, 55, 181, 208, 96, 50, 146, 52, 254, 50, 131, 246, 102, 103, 207, 250, 170, 163, 71, 18, 81,
				227, 232, 85, 65, 229, 35, 90, 16, 23, 44, 237, 222, 74, 198, 213, 158, 207, 50, 8, 197, 205, 254, 133, 147, 36, 1, 240, 200, 49, 186,
				187, 81, 177, 128, 154, 220, 114, 248, 219, 123, 255, 120, 145, 173, 117, 193, 45, 62, 252, 4, 253, 45, 63, 130, 151, 182, 148, 128, 144,
				197, 106, 114, 139, 237, 85, 110, 129, 239, 180, 29, 69, 238, 249, 4, 226, 21, 48, 91, 122, 204, 86, 200, 189, 254, 227, 60, 244, 50, 125,
				202, 94, 37, 110, 24, 242, 245, 1, 74, 16, 38, 25, 247, 53, 251, 40, 45, 118, 0, 13, 158, 197, 62, 159, 195, 163, 235, 71, 132, 47, 200,
				183, 195, 230, 204, 33, 136, 204, 103, 11, 83, 68, 171, 93, 51, 147, 240, 1, 21, 94, 183, 43, 200, 208, 135, 29, 248, 184, 43, 62, 249, 241,
				188, 74, 46, 161, 80, 29, 15, 31, 95, 43, 140, 108, 163, 129, 55, 144, 133, 225, 57, 24, 123, 95, 169, 120, 250, 236, 34, 44, 151, 162, 31,
				115, 12, 103, 74, 104, 215, 246, 116, 106, 3, 87, 200, 201, 152, 144, 67, 151, 154, 36, 104, 139, 91, 8, 43, 156, 13, 187, 87, 50, 98, 184, 41,
				235, 39, 167, 18, 64, 250, 107, 141, 158, 192, 176, 237, 221, 191, 67, 29, 105, 13, 91, 182, 133, 21, 221, 213, 51, 24, 30, 2, 12, 109, 82, 56,
				233, 137, 172, 165, 191, 86, 80, 38, 37, 182, 230, 100, 124, 86, 55, 65, 55, 220, 110, 66, 244, 174, 182, 16, 63, 138, 110, 90, 184, 91, 31, 10,
				238, 71, 61, 125, 198, 244, 231, 122, 179, 134, 172, 62, 216, 4, 62, 238, 234, 49, 52, 255, 4, 241, 93, 62, 178, 154, 170, 99, 76, 41, 72, 196,
				144, 160, 3, 186, 197, 221, 92, 72, 154, 164, 118, 192, 20, 105, 10, 0, 159, 83, 80, 238, 70, 120, 240, 9, 21, 238, 143, 74, 83, 41, 204, 84, 112,
				231, 75, 49, 241, 78, 56, 58, 108, 196, 95, 83, 148, 78, 149, 194, 39, 99, 162, 45, 217, 32, 208, 108, 12, 0, 71, 38, 27, 226, 183, 55, 154, 205,
				102, 86, 225, 45, 9, 109, 42, 164, 125, 107, 168, 152, 37, 206, 189, 141, 178, 154, 26, 30, 240, 63, 223, 161, 98, 69, 94, 55, 61, 153, 219, 60,
				107, 141, 229, 253, 233, 61, 144, 221, 129, 5, 217, 230, 142, 114, 152, 238, 214, 192, 49, 58, 48, 21, 6, 9, 42, 134, 72, 134, 247, 13, 1, 9, 20,
				49, 8, 30, 6, 0, 115, 0, 115, 0, 108, 48, 33, 6, 9, 42, 134, 72, 134, 247, 13, 1, 9, 21, 49, 20, 4, 18, 84, 105, 109, 101, 32, 49, 53, 48, 48, 54,
				50, 53, 53, 54, 57, 53, 56, 49, 48, 130, 4, 179, 6, 9, 42, 134, 72, 134, 247, 13, 1, 7, 6, 160, 130, 4, 164, 48, 130, 4, 160, 2, 1, 0, 48, 130, 4,
				153, 6, 9, 42, 134, 72, 134, 247, 13, 1, 7, 1, 48, 40, 6, 10, 42, 134, 72, 134, 247, 13, 1, 12, 1, 6, 48, 26, 4, 20, 234, 207, 97, 241, 214, 134,
				128, 105, 23, 115, 99, 96, 119, 67, 69, 148, 76, 159, 190, 28, 2, 2, 4, 0, 128, 130, 4, 96, 153, 80, 254, 57, 80, 169, 35, 66, 207, 142, 31, 56,
				40, 132, 110, 177, 214, 116, 131, 219, 73, 122, 115, 40, 201, 21, 98, 192, 249, 135, 45, 20, 224, 8, 250, 82, 88, 173, 208, 2, 182, 227, 100, 8,
				203, 4, 49, 233, 77, 182, 11, 173, 161, 0, 115, 172, 77, 159, 119, 139, 107, 103, 208, 28, 211, 24, 234, 139, 85, 85, 247, 128, 75, 25, 13, 180,
				120, 38, 166, 53, 96, 194, 230, 27, 86, 5, 175, 22, 153, 102, 123, 2, 90, 62, 29, 113, 203, 63, 229, 21, 135, 242, 242, 233, 65, 182, 126, 52, 64,
				158, 146, 175, 43, 70, 66, 134, 109, 175, 144, 117, 121, 34, 181, 4, 49, 201, 127, 70, 154, 95, 8, 160, 135, 151, 211, 46, 12, 138, 129, 40, 169,
				87, 189, 34, 116, 183, 216, 228, 140, 17, 27, 73, 202, 200, 146, 210, 109, 67, 162, 1, 54, 58, 99, 44, 115, 34, 41, 7, 37, 255, 191, 180, 77, 170,
				86, 72, 236, 63, 95, 113, 73, 95, 97, 203, 239, 100, 170, 130, 182, 9, 79, 240, 72, 81, 46, 226, 72, 126, 191, 194, 199, 188, 110, 161, 32, 106,
				39, 217, 198, 79, 22, 195, 185, 126, 75, 240, 93, 177, 124, 127, 136, 98, 124, 2, 82, 98, 125, 57, 185, 131, 213, 208, 106, 54, 191, 103, 21, 33,
				153, 150, 254, 80, 201, 224, 243, 67, 49, 77, 239, 106, 43, 110, 209, 104, 245, 6, 243, 51, 82, 181, 239, 212, 228, 145, 45, 14, 83, 59, 246, 191,
				42, 65, 89, 123, 179, 251, 179, 10, 103, 238, 119, 96, 164, 187, 172, 150, 202, 159, 245, 78, 33, 51, 60, 105, 96, 119, 28, 168, 231, 169, 91, 109,
				5, 212, 204, 9, 154, 109, 84, 203, 198, 51, 102, 117, 119, 15, 105, 253, 74, 143, 27, 156, 4, 85, 190, 123, 110, 183, 100, 193, 136, 8, 23, 200, 72,
				48, 48, 65, 31, 30, 14, 164, 191, 15, 250, 1, 202, 120, 44, 210, 129, 127, 131, 197, 148, 229, 103, 178, 147, 184, 39, 0, 53, 64, 172, 233, 232, 31,
				119, 53, 104, 241, 101, 4, 216, 8, 195, 25, 29, 49, 51, 168, 85, 111, 47, 181, 219, 232, 157, 221, 250, 17, 244, 183, 201, 140, 99, 229, 91, 141,
				247, 43, 143, 241, 146, 163, 111, 31, 157, 159, 218, 17, 14, 223, 42, 18, 75, 172, 223, 174, 37, 1, 39, 228, 25, 250, 240, 136, 39, 79, 253, 167,
				209, 254, 39, 118, 207, 246, 181, 68, 32, 255, 151, 147, 197, 195, 45, 73, 56, 181, 56, 122, 131, 78, 13, 105, 136, 137, 97, 212, 7, 13, 198, 90, 52,
				66, 64, 126, 24, 120, 225, 63, 176, 27, 104, 34, 67, 176, 133, 48, 59, 164, 79, 67, 139, 152, 143, 15, 45, 97, 105, 109, 50, 245, 91, 207, 110, 206,
				56, 96, 201, 195, 38, 118, 185, 44, 52, 230, 215, 161, 101, 88, 41, 113, 218, 245, 169, 47, 111, 3, 204, 185, 196, 166, 171, 178, 39, 240, 90, 232,
				90, 23, 39, 60, 16, 255, 10, 93, 174, 248, 171, 51, 88, 152, 15, 52, 245, 110, 243, 103, 7, 80, 112, 128, 51, 40, 154, 125, 224, 159, 54, 222, 21, 222,
				121, 227, 159, 6, 170, 3, 19, 159, 174, 86, 38, 149, 84, 250, 21, 89, 173, 61, 192, 152, 106, 197, 233, 178, 158, 175, 131, 231, 238, 82, 85, 178, 204,
				100, 43, 40, 204, 18, 72, 129, 81, 125, 51, 238, 170, 100, 251, 39, 164, 75, 83, 100, 229, 27, 53, 130, 212, 179, 6, 138, 32, 196, 108, 102, 154, 114,
				59, 18, 72, 192, 77, 29, 246, 201, 169, 223, 45, 60, 160, 105, 228, 201, 27, 249, 252, 160, 10, 231, 249, 115, 203, 106, 249, 147, 108, 81, 85, 113, 121,
				165, 157, 92, 84, 15, 122, 166, 94, 212, 182, 86, 239, 6, 217, 238, 148, 49, 36, 116, 177, 209, 155, 16, 186, 192, 59, 232, 64, 18, 171, 50, 245, 170, 14,
				104, 33, 87, 43, 115, 236, 66, 126, 226, 203, 131, 223, 160, 121, 190, 58, 15, 179, 185, 160, 8, 191, 214, 16, 108, 12, 90, 58, 33, 116, 137, 145, 170, 91,
				243, 12, 229, 178, 59, 71, 95, 199, 175, 152, 233, 81, 30, 54, 59, 184, 125, 202, 107, 63, 134, 73, 96, 255, 234, 85, 241, 13, 255, 174, 38, 20, 187, 180,
				43, 213, 134, 180, 119, 184, 89, 183, 205, 98, 141, 238, 219, 13, 201, 99, 64, 250, 202, 234, 231, 92, 121, 72, 195, 16, 142, 252, 175, 25, 223, 19, 137,
				221, 59, 157, 124, 0, 218, 144, 221, 174, 41, 122, 146, 169, 147, 94, 230, 175, 143, 245, 203, 205, 204, 96, 122, 254, 91, 202, 60, 136, 218, 171, 172, 96,
				8, 161, 142, 140, 198, 148, 75, 62, 177, 174, 233, 81, 65, 35, 20, 92, 62, 10, 191, 207, 174, 44, 51, 245, 247, 59, 175, 178, 89, 77, 223, 58, 169, 145, 202,
				126, 241, 93, 211, 120, 161, 216, 51, 249, 16, 118, 158, 228, 224, 161, 85, 193, 236, 199, 189, 115, 11, 83, 186, 167, 15, 179, 172, 233, 173, 139, 214,
				220, 168, 232, 149, 91, 218, 244, 30, 127, 25, 100, 122, 138, 248, 81, 116, 4, 140, 235, 70, 143, 18, 191, 83, 134, 137, 39, 66, 220, 116, 243, 126, 60, 205,
				193, 150, 71, 38, 177, 219, 130, 98, 187, 54, 115, 44, 27, 37, 4, 246, 53, 44, 208, 33, 91, 67, 50, 174, 131, 172, 6, 199, 120, 145, 154, 246, 66, 213, 237,
				36, 254, 205, 117, 160, 226, 100, 106, 202, 84, 59, 190, 193, 92, 146, 41, 134, 84, 233, 237, 139, 24, 16, 171, 38, 194, 103, 222, 25, 134, 183, 28, 17, 100,
				189, 46, 71, 137, 220, 207, 187, 39, 156, 133, 91, 250, 66, 17, 41, 146, 41, 200, 173, 101, 44, 204, 47, 237, 46, 58, 75, 213, 223, 226, 252, 213, 226, 117,
				89, 205, 234, 179, 179, 136, 187, 205, 132, 254, 126, 232, 166, 247, 55, 71, 172, 182, 78, 83, 78, 157, 59, 209, 46, 48, 123, 192, 20, 90, 127, 7, 242, 179,
				123, 46, 131, 65, 211, 159, 242, 189, 97, 189, 114, 148, 77, 58, 77, 191, 127, 218, 17, 16, 27, 3, 71, 120, 255, 180, 65, 196, 169, 237, 7, 52, 76, 243, 247,
				213, 83, 237, 117, 38, 223, 105, 226, 246, 53, 51, 212, 50, 235, 150, 185, 48, 61, 48, 33, 48, 9, 6, 5, 43, 14, 3, 2, 26, 5, 0, 4, 20, 165, 236, 46, 49, 120,
				101, 103, 122, 101, 135, 254, 168, 124, 35, 161, 176, 197, 228, 148, 112, 4, 20, 94, 152, 178, 64, 157, 196, 49, 13, 135, 220, 30, 60, 140, 118, 42, 41, 144,
				244, 15, 196, 2, 2, 4, 0
		};

		readonly byte[] validCertSA = new byte[]
		{
				77, 73, 73, 68, 49, 68, 67, 67, 65, 51, 109, 103, 65, 119, 73, 66, 65, 103, 73, 84, 98, 119, 65, 65, 101, 51, 85, 65, 89, 86, 85, 51, 52, 73, 47, 43, 53, 81, 65,
				66, 65, 65, 66, 55, 100, 84, 65, 75, 66, 103, 103, 113, 104, 107, 106, 79, 80, 81, 81, 68, 65, 106, 66, 106, 77, 82, 85, 119, 69, 119, 89, 75, 67, 90, 73, 109,
				105, 90, 80, 121, 76, 71, 81, 66, 71, 82, 89, 70, 98, 71, 57, 106, 89, 87, 119, 120, 69, 122, 65, 82, 66, 103, 111, 74, 107, 105, 97, 74, 107, 47, 73, 115, 90,
				65, 69, 90, 70, 103, 78, 110, 98, 51, 89, 120, 70, 122, 65, 86, 66, 103, 111, 74, 107, 105, 97, 74, 107, 47, 73, 115, 90, 65, 69, 90, 70, 103, 100, 108, 101, 72,
				82, 110, 89, 88, 112, 48, 77, 82, 119, 119, 71, 103, 89, 68, 86, 81, 81, 68, 69, 120, 78, 85, 85, 49, 112, 70, 83, 85, 53, 87, 84, 48, 108, 68, 82, 83, 49, 84, 100,
				87, 74, 68, 81, 83, 48, 120, 77, 66, 52, 88, 68, 84, 73, 121, 77, 68, 89, 120, 77, 106, 69, 51, 78, 68, 65, 49, 77, 108, 111, 88, 68, 84, 73, 48, 77, 68, 89, 120, 77,
				84, 69, 51, 78, 68, 65, 49, 77, 108, 111, 119, 83, 84, 69, 76, 77, 65, 107, 71, 65, 49, 85, 69, 66, 104, 77, 67, 85, 48, 69, 120, 68, 106, 65, 77, 66, 103, 78, 86, 66,
				65, 111, 84, 66, 87, 70, 110, 97, 87, 120, 108, 77, 82, 89, 119, 70, 65, 89, 68, 86, 81, 81, 76, 69, 119, 49, 111, 89, 88, 108, 104, 73, 72, 108, 104, 90, 50, 104,
				116, 98, 51, 86, 121, 77, 82, 73, 119, 69, 65, 89, 68, 86, 81, 81, 68, 69, 119, 107, 120, 77, 106, 99, 117, 77, 67, 52, 119, 76, 106, 69, 119, 86, 106, 65, 81, 66,
				103, 99, 113, 104, 107, 106, 79, 80, 81, 73, 66, 66, 103, 85, 114, 103, 81, 81, 65, 67, 103, 78, 67, 65, 65, 84, 84, 65, 75, 57, 108, 114, 84, 86, 107, 111, 57, 114,
				107, 113, 54, 90, 89, 99, 99, 57, 72, 68, 82, 90, 80, 52, 98, 57, 83, 52, 122, 65, 52, 75, 109, 55, 89, 88, 74, 43, 115, 110, 84, 86, 104, 76, 107, 122, 85, 48, 72, 115,
				109, 83, 88, 57, 85, 110, 56, 106, 68, 104, 82, 84, 79, 72, 68, 75, 97, 102, 116, 56, 67, 47, 117, 117, 85, 89, 57, 51, 52, 118, 117, 77, 78, 111, 52, 73, 67, 74, 122,
				67, 67, 65, 105, 77, 119, 103, 89, 103, 71, 65, 49, 85, 100, 69, 81, 83, 66, 103, 68, 66, 43, 112, 72, 119, 119, 101, 106, 69, 98, 77, 66, 107, 71, 65, 49, 85, 69, 66,
				65, 119, 83, 77, 83, 49, 111, 89, 88, 108, 104, 102, 68, 73, 116, 77, 106, 77, 48, 102, 68, 77, 116, 77, 84, 69, 121, 77, 82, 56, 119, 72, 81, 89, 75, 67, 90, 73, 109,
				105, 90, 80, 121, 76, 71, 81, 66, 65, 81, 119, 80, 77, 122, 65, 119, 77, 68, 99, 49, 78, 84, 103, 52, 78, 122, 65, 119, 77, 68, 65, 122, 77, 81, 48, 119, 67, 119, 89, 68,
				86, 81, 81, 77, 68, 65, 81, 120, 77, 84, 65, 119, 77, 82, 69, 119, 68, 119, 89, 68, 86, 81, 81, 97, 68, 65, 104, 97, 89, 88, 82, 106, 89, 83, 65, 120, 77, 106, 69, 89, 77,
				66, 89, 71, 65, 49, 85, 69, 68, 119, 119, 80, 82, 109, 57, 118, 90, 67, 66, 67, 100, 88, 78, 122, 97, 87, 53, 108, 99, 51, 77, 122, 77, 66, 48, 71, 65, 49, 85, 100, 68,
				103, 81, 87, 66, 66, 83, 103, 109, 73, 87, 68, 54, 98, 80, 102, 98, 98, 75, 107, 109, 84, 119, 79, 74, 82, 88, 118, 73, 98, 72, 57, 72, 106, 65, 102, 66, 103, 78, 86, 72,
				83, 77, 69, 71, 68, 65, 87, 103, 66, 82, 50, 89, 73, 122, 55, 66, 113, 67, 115, 90, 49, 99, 49, 110, 99, 43, 97, 114, 75, 99, 114, 109, 84, 87, 49, 76, 122, 66, 79, 66,
				103, 78, 86, 72, 82, 56, 69, 82, 122, 66, 70, 77, 69, 79, 103, 81, 97, 65, 47, 104, 106, 49, 111, 100, 72, 82, 119, 79, 105, 56, 118, 100, 72, 78, 48, 89, 51, 74, 115, 76,
				110, 112, 104, 100, 71, 78, 104, 76, 109, 100, 118, 100, 105, 53, 122, 89, 83, 57, 68, 90, 88, 74, 48, 82, 87, 53, 121, 98, 50, 120, 115, 76, 49, 82, 84, 87, 107, 86, 74,
				84, 108, 90, 80, 83, 85, 78, 70, 76, 86, 78, 49, 89, 107, 78, 66, 76, 84, 69, 117, 89, 51, 74, 115, 77, 73, 71, 116, 66, 103, 103, 114, 66, 103, 69, 70, 66, 81, 99, 66, 65,
				81, 83, 66, 111, 68, 67, 66, 110, 84, 66, 117, 66, 103, 103, 114, 66, 103, 69, 70, 66, 81, 99, 119, 65, 89, 90, 105, 97, 72, 82, 48, 99, 68, 111, 118, 76, 51, 82, 122, 100,
				71, 78, 121, 98, 67, 53, 54, 89, 88, 82, 106, 89, 83, 53, 110, 98, 51, 89, 117, 99, 50, 69, 118, 81, 50, 86, 121, 100, 69, 86, 117, 99, 109, 57, 115, 98, 67, 57, 85, 85, 49,
				112, 70, 97, 87, 53, 50, 98, 50, 108, 106, 90, 86, 78, 68, 81, 84, 69, 117, 90, 88, 104, 48, 90, 50, 70, 54, 100, 67, 53, 110, 98, 51, 89, 117, 98, 71, 57, 106, 89, 87, 120,
				102, 86, 70, 78, 97, 82, 85, 108, 79, 86, 107, 57, 74, 81, 48, 85, 116, 85, 51, 86, 105, 81, 48, 69, 116, 77, 83, 103, 120, 75, 83, 53, 106, 99, 110, 81, 119, 75, 119, 89,
				73, 75, 119, 89, 66, 66, 81, 85, 72, 77, 65, 71, 71, 72, 50, 104, 48, 100, 72, 65, 54, 76, 121, 57, 48, 99, 51, 82, 106, 99, 109, 119, 117, 101, 109, 70, 48, 89, 50, 69, 117,
				90, 50, 57, 50, 76, 110, 78, 104, 76, 50, 57, 106, 99, 51, 65, 119, 68, 103, 89, 68, 86, 82, 48, 80, 65, 81, 72, 47, 66, 65, 81, 68, 65, 103, 101, 65, 77, 66, 48, 71, 65, 49,
				85, 100, 74, 81, 81, 87, 77, 66, 81, 71, 67, 67, 115, 71, 65, 81, 85, 70, 66, 119, 77, 67, 66, 103, 103, 114, 66, 103, 69, 70, 66, 81, 99, 68, 65, 122, 65, 110, 66, 103, 107,
				114, 66, 103, 69, 69, 65, 89, 73, 51, 70, 81, 111, 69, 71, 106, 65, 89, 77, 65, 111, 71, 67, 67, 115, 71, 65, 81, 85, 70, 66, 119, 77, 67, 77, 65, 111, 71, 67, 67, 115, 71, 65,
				81, 85, 70, 66, 119, 77, 68, 77, 65, 111, 71, 67, 67, 113, 71, 83, 77, 52, 57, 66, 65, 77, 67, 65, 48, 107, 65, 77, 69, 89, 67, 73, 81, 67, 86, 119, 68, 77, 99, 113, 54, 80, 79,
				43, 77, 99, 109, 115, 66, 88, 85, 122, 47, 118, 49, 71, 100, 104, 71, 112, 55, 114, 113, 83, 97, 50, 65, 120, 84, 75, 83, 118, 56, 51, 56, 73, 65, 73, 104, 65, 79, 66, 78, 68,
				66, 116, 57, 43, 51, 68, 83, 108, 105, 106, 111, 86, 102, 120, 122, 114, 100, 68, 104, 53, 50, 56, 87, 67, 51, 55, 115, 109, 69, 100, 111, 71, 87, 86, 114, 83, 112, 71, 49
		};
	}
}
