...
{
	[System.Diagnostics.CodeAnalysis.SuppressMessage("CargoWiseOne", "CW1161:Res.GetString Analyzer", Justification = "IATA Legal Text")]
	public static class AWBTermsAndConditionsText
	{
		public const string Heading = @"NOTICE CONCERNING CARRIERS' LIMITATION OF LIABILITY";

		public const string Introduction = @"If the carriage involves an ultimate destination or stop in a country other than the country of departure, the Montreal Convention or the Warsaw Convention may be applicable to the liability of the Carrier in respect of loss of, damage or delay to cargo. Carrier's limitation of liability in accordance with those Conventions shall be as set forth in subparagraph 4 unless a higher value is declared.";

		public const string MiddleHeading = @"CONDITIONS OF CONTRACT";

		public static string Body1 => $@"1. In this contract and the Notices appearing hereon:
...
5.2 When no part of the consignment is delivered, a claim with respect to such consignment will be considered even though transportation charges thereon are unpaid.";
		...
		static int NumOfSDR
			=> ZDateTime.UtcNow >= new ZDateTime(2024, 12, 28) ? 26
				: ZDateTime.UtcNow >= new ZDateTime(2019, 12, 29) ? 22
				: 19;

		public const string Body2 = @"6./6.1 For cargo accepted for carriage, the Warsaw Convention and the Montreal Convention permit shipper to increase the limitation of liability by declaring a higher value for carriage and paying a supplemental charge if required.
