...
{
	class STACusIntraStatGroupArchiveStageDescriptorTest : STAArchiveDescriptorTest
	{
		...
		protected override SchemaColumn ExpectedMainArchiveDateFilterColumn { get; set; } = CusIntrastatGroupSchema.CIG_SystemCreateTimeUtc;
		...
		protected override List<ArchiveRelationshipForSTATest> ExpectedRelationships
			=> new()
			{
				new ArchiveRelationshipForSTATest(CusIntrastatGroupSchema.Constants.TableName, CusIntrastatGroupSchema.PK, CusIntrastatHeaderSchema.Constants.TableName, CusIntrastatHeaderSchema.CIH_CIG_MergedGroup, isReversed: false),
				new ArchiveRelationshipForSTATest(CusIntrastatGroupSchema.Constants.TableName, CusIntrastatGroupSchema.PK, CusIntrastatMergedLineSchema.Constants.TableName, CusIntrastatMergedLineSchema.CIM_CIG_Group, isReversed: false),
		...
		public void TestRunArchiveSystem_WhenArchivingCusIntrastatGroup()
		{
			...
			{
				AssertEquals("Expected Customs Intrastat Group to exist.", 1, Factory.GetDatabaseCount(ObjectFactory.GetType<ICusIntrastatGroup>(), new ZQuery(CusIntrastatGroupSchema.PK, group.PK)));
			...
			{
				AssertEquals("Expected Customs Intrastat Group to be deleted.", 0, Factory.GetDatabaseCount(ObjectFactory.GetType<ICusIntrastatGroup>(), new ZQuery(CusIntrastatGroupSchema.PK, group.PK)));
