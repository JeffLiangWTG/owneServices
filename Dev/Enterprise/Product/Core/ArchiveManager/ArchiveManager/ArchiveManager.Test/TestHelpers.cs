...
namespace Enterprise.ArchiveManager.Test
{
	public static class TestHelpers
	{
		...
		public static IeDoc GetArchiveReportFromSchedule(ArchiveScheduleTask archiveSchedule, string reportNameExcludingDateTime)
			=> GetAllReportsFromSchedule(archiveSchedule)
				.FirstOrDefault(eDoc => eDoc.FileNameOnly.Contains(reportNameExcludingDateTime));
		...
		public static void CreateRelatedDocuments(List<ZGuid> parentPKs, DocumentFactory documentFactory, NumberedBusinessObjectFactory documentFactoryForDB333, int dbNumber)
		{
			foreach (var pk in parentPKs)
			{
				...
				storageDoc.SC_FileName = "sample.pdf";
				storageDoc.SC_ImageData = TestFileHelper.SamplePDF;
			}
		}

		public static JobHeader CreateJobHeaderWithRelatedDocuments(string jobStatus, DocumentFactory documentFactory, BusinessObjectFactory factory, NumberedBusinessObjectFactory factoryForDB333, IForwardingTestDataCreator testDataCreator, int dbNumber)
		{
			...
			jobHeader.JH_Status = jobStatus;

			CreateRelatedDocuments(new List<ZGuid> { shipmentPK }, documentFactory, factoryForDB333, dbNumber);
