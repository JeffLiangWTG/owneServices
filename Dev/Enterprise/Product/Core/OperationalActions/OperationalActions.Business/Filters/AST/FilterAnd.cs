...
namespace Enterprise.Services.OperationalActions.Business.AST
{
	...
	/// </summary>
	sealed class FilterAnd : IFilterExpression
	{
		public FilterAnd(IFilterExpression left, IFilterExpression right)
		{
			if (left == null)
			{
		...
		{
			get { return FilterPrecedence.And; }
		}
		...
		{
			if (left.Precidence > FilterPrecedence.And)
			{
				builder.Append("(");
				left.Append(builder);
				builder.Append(")");
			...
			if (right.Precidence > FilterPrecedence.And)
			{
				builder.Append("(");
		...
		{
			return new FilterAnd(Left.AddConstraintPrefix(prefix), Right.AddConstraintPrefix(prefix));
		}

		public IFilterExpression RemoveConstraintPrefix(string prefix)
		{
			return new FilterAnd(Left.RemoveConstraintPrefix(prefix), Right.RemoveConstraintPrefix(prefix));
		}
