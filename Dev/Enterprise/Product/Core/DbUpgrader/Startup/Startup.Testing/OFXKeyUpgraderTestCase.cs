...
namespace Enterprise.DbUpgrader.Startup.Testing
{
	...
	sealed class OFXKeyUpgraderTestCase : TestCase
	{
		public void TestName()
		{
			AssertEquals("OFX Key creation", upgrader.Name);
		}
		...
		{
			DeleteKey("OFXEncryptionKey");
			upgrader.RunUpgrade();
			AssertKeyExists("OFXEncryptionKey");
		}

		public void TestWhenKeyExists()
		{
			SetKey("OFXEncryptionKey", new byte[] { 0x12, 0x34, 0x56, 0x78 });
			upgrader.RunUpgrade();
			AssertKeyExists("OFXEncryptionKey");
			AssertKeyEquals("OFXEncryptionKey", new byte[] { 0x12, 0x34, 0x56, 0x78 });
		}

		OFXKeyUpgrader upgrader;

		protected override void SetUp()
		{
			base.SetUp();

			upgrader = new OFXKeyUpgrader(new DummyUpgradeManager(), Db.Connection);
		}
