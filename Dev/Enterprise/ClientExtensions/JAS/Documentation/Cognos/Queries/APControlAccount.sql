DECLARE @APControlGuid uniqueidentifier
...
(
	...
	FROM	dbo.AccTransactionHeader
			...
			INNER JOIN #CognosAccounts ON @APControlGuid = T1_AG
	...
	WHERE	GB_GC = @CompanyPK
			AND	AH_Ledger = 'AP'
			AND	AH_TransactionType IN ('JNL', 'PAY', 'REC', 'CTR', 'DSC', 'OVP', 'INV', 'ADJ', 'CRD', 'EXX', 'TRF')	
) DerivedTbl
