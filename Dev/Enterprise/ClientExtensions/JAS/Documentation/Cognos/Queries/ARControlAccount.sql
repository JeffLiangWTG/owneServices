DECLARE @ARControlGuid uniqueidentifier
...
(
	SELECT	T1_Name AS AA_AccountName,
			...
			END AS AA_Business,
			CASE
				WHEN AH_TransactionType IN ('INV', 'ADJ', 'CRD') THEN AH_InvoiceAmount + AH_GSTAmount
			...
			CASE
				WHEN T4_Currency = 'Y' AND AH_TransactionType IN ('INV', 'ADJ', 'CRD') THEN AH_OSTotal				
	...
	FROM	dbo.AccTransactionHeader
			...
			INNER JOIN #CognosAccounts ON @ARControlGuid = T1_AG
	...
	WHERE	AH_Ledger NOT IN ('AP', 'JC')
			AND	
			(
				AH_TransactionType IN ('JNL', 'PAY', 'REC', 'CTR', 'DSC', 'OVP', 'INV', 'ADJ', 'CRD')
				OR (AH_TransactionType =  'EXX' AND AH_Ledger != 'CB')
				OR (AH_TransactionType = 'TRF' AND AH_Ledger = 'AR')
			)
) DerivedTbl
