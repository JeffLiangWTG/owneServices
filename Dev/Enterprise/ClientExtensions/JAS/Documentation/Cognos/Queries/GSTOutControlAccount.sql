DECLARE @GSTOutControlGuid uniqueidentifier
...
(
	SELECT	T1_Name AS AA_AccountName,
			...
			CASE
				...
				ELSE ''
			END AS AA_Business,
			AL_GSTVAT AS AA_Amount,
			CASE
				WHEN T4_Currency = 'Y' THEN RX_Code
				ELSE ''
			END AS AA_TransactionCurrency,
			CASE				
				WHEN T4_Currency = 'Y' THEN dbo.ClientConvertLocalToForeignAmount(AL_GSTVAT, AL_ExchangeRate, @CompanyPK, AL_RX)
				ELSE 0
	...
	FROM	dbo.AccTransactionHeader
			...
			INNER JOIN #CognosAccounts ON @GSTOutControlGuid = T1_AG
	...
	WHERE	(AH_TransactionType = 'DRC' AND AH_Ledger = 'CB')
			OR (AH_TransactionType IN ('INV', 'ADJ', 'CRD') AND AH_Ledger != 'AP')
			AND GB_GC = @CompanyPK
) DerivedTbl
