...
(
	SELECT	T1_Name AS AA_AccountName,
			...
			CASE
				WHEN T4_Currency = 'Y' THEN dbo.ClientConvertLocalToForeignAmount(AL_LineAmount, AH_ExchangeRate, @CompanyPK, AH_RX)			
				ELSE 0
	...
	FROM	dbo.AccTransactionHeader
			...
			INNER JOIN #CognosAccounts ON AC_AG_RevenueAccount = T1_AG
