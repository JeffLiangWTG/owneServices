...
(
	SELECT	T1_Name AS AA_AccountName,
			...
			CASE
				...
				ELSE ''
			END AS AA_Business,
			dbo.ClientGetGLAutoJournalAmount(AH_PostDate, AH_DueDate, GETDATE(), AL_LineAmount, @CompanyPK) AS AA_Amount,
			CASE
				WHEN T4_Currency = 'Y' THEN RX_Code
				ELSE ''
			END AS AA_TransactionCurrency,
			CASE
				WHEN T4_Currency = 'Y' THEN dbo.ClientConvertLocalToForeignAmount(dbo.ClientGetGLAutoJournalAmount(AH_PostDate, AH_DueDate, GETDATE(), AL_LineAmount, @CompanyPK), AL_ExchangeRate, @CompanyPK, AL_RX)
				ELSE 0
