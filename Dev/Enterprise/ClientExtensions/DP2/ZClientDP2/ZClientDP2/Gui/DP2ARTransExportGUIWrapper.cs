...
{
	public class DP2ARTransExportGUIWrapper : FlatFileXmlExportGUIWrapper
	{
		public DP2ARTransExportGUIWrapper(BusinessObjectFactory factory)
			: base(factory)
		{
		}
		...
		{
			get { return DP2DataRegistry.Instance.ARTransExportDirectory; }
		}
		...
		protected override AccountingTransactionsDataExporter DataExporter
		{
			get
			{
				if (fExporter == null)
				{
					fExporter = new DP2ARInvoiceDataExporter(Factory);
				}
				return fExporter;
			}
		}
		DP2ARInvoiceDataExporter fExporter;

		public override string FormCaption
		{
			get { return "Export Transactions To DP2 File"; }
		}
		...
		protected override void CreateFileAndExport(Stream stream)
		{
			...
			using (ProgressForm = new ProgressForm())
			{
					...
					&& fExporter.DeptWithoutCodeMap.Count == 0)
				{
					...
					string finalPath = Path.Combine(DP2DataRegistry.Instance.ARTransExportDirectory, finalFileName);
		...
		void NotifyUserThatPropertiesAreNotSet()
		{
			...
			{
				builder.Append(GetMessageBody(LastBatchNumber, fExporter.BranchWithoutCodeMap, "Branches", "DP2 Branch Code"));
				builder.AppendLine();
			}

			if (fExporter.DeptWithoutCodeMap.Count > 0)
			{
				builder.Append(GetMessageBody(LastBatchNumber, fExporter.DeptWithoutCodeMap, "Departments", "DP2 Department Code"));
				builder.AppendLine();
			}
		...
		string GetMessageBody(int batchNumber, StringCollection codeMapList, ZString objName, ZString message)
		{
			...
			result.AppendLine(DP2Constants.DashLine);
