...
{
	public class DP2ARTransactionConverterTest : TestCaseWithFactory
	{
		public void TestMapExport()
		{
			NotificationBuffer buffer = new NotificationBuffer();
			DP2ARTransactionConverterForTest converter = new DP2ARTransactionConverterForTest(buffer, Factory);
			...
			AssertEquals("ClientAccount", "12345", dataRow[DP2FlatFileDataRow.Schema.ClientAccount.Name]);
			AssertEquals("GLAccount", "ABC1111111", dataRow[DP2FlatFileDataRow.Schema.GLAccount.Name]);
			AssertEquals("Date", "20070101", dataRow[DP2FlatFileDataRow.Schema.Date.Name]);
			AssertEquals("Desc", "093 charge code  1 2 3 4", dataRow[DP2FlatFileDataRow.Schema.Desc.Name]);
			AssertEquals("Amount", "92387423", dataRow[DP2FlatFileDataRow.Schema.Amount.Name]);
		...
		public void TestExportThrowLSCCodeNotSetException()
		{
			...
			header.DebtorOrCreditor = organisation;
			AssertThrowCorrectException(DP2Exception.DP2ExceptionType.LegacySystemCodeNotSet, header);
		}
		...
		public void TestExportThrowBranchNotSetException()
		{
			...
			Xsd.Organisation organisation = CreateOrganisation(true);
			header.DebtorOrCreditor = organisation;
			AssertThrowCorrectException(DP2Exception.DP2ExceptionType.BranchCodeMappingNotSet, header);
		}
		...
		public void TestExportThrowDeptNotSetException()
		{
			...
			Xsd.Organisation organisation = CreateOrganisation(true);
			header.DebtorOrCreditor = organisation;
			AssertThrowCorrectException(DP2Exception.DP2ExceptionType.DeptCodeMappingNotSet, header);
		}

		void AssertThrowCorrectException(DP2Exception.DP2ExceptionType expectedType, Xsd.TxnHeader header)
		{
			NotificationBuffer buffer = new NotificationBuffer();
			DP2ARTransactionConverterForTest converter = new DP2ARTransactionConverterForTest(buffer, Factory);
			try
			...
			{
				AssertEquals("Exception is an DP2Exception", typeof(DP2Exception), e.GetType());
				DP2Exception ex = (DP2Exception)e;
...
#region Setup
		...
		class DP2ARTransactionConverterForTest : DP2ARTransactionConverter
		{
			public DP2ARTransactionConverterForTest(INotifications notify, BusinessObjectFactory factory) : base(notify, factory)
			{
			}
