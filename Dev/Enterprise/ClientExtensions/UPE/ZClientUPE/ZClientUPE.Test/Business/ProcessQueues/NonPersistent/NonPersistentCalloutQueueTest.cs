...
{
	[TestedType(typeof(NonPersistentCalloutQueue))]
	internal class NonPersistentCalloutQueueTest : NonPersistentProcessQueueTestCase
	{
		...
		protected override Type ExpectedUPEProcessQueueValidationHelperType
		{
			get
			{
				return typeof(UPECommercialQueueValidationHelper);
			}
		}

		public override void TestQueueName()
		{
			var mockQueue = new Mock<NonPersistentCalloutQueue>(new object[] { Factory });
			var queue = (NonPersistentProcessQueue)mockQueue.Object;
			var mockValidationHelper = new Mock<UPECommercialQueueValidationHelper>(new object[] { queue });
			mockQueue.Protected()
