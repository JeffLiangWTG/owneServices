...
namespace Enterprise.ZClientWebCargoWiseEDI.Testing
{
	...
	public class LicenceRegistrationBaseControllerTest : TestCaseWithFactory
	{
...
DatabaseNumber:", notes.ST_NoteDataAsText);
		...
		public void TestRegisterTenant_DefaultLicenceType()
		{
			var logger = new NLogWrapperForTest(GetType());
			...
			AssertNotNull(db);
			AssertEquals("Licence type should fallback to PRD default", DatabaseTypes.Codes.Production, db.LD_LicenceType);
		}
		...
		public void TestRegisterTenant_ExistingTenant()
		{
			...
			AssertEquals(ErrorCodes.Codes.Validation_InvalidValue, context.Messages.Messages[0].Code);
		...
		public void TestRegisterTenant_DuplicateDatabase()
		{
			...
			AssertEquals("Should be updated to match incoming tenant id", "DEMO", db1InNewFactory.LD_TenantID);
...
DatabaseNumber:", notes.ST_NoteDataAsText);
		...
		LicenceRegistrationBaseControllerForTest CreateController(NLogWrapper logger)
		{
			var requestMessage = new HttpRequestMessage(HttpMethod.Post, "http://unit-testing/api/registration");
			requestMessage.Content = new StringContent("{ }", Encoding.UTF8, "application/json");
			var controller = new LicenceRegistrationBaseControllerForTest(logger);
		...
		public class LicenceRegistrationBaseControllerForTest : LicenceRegistrationBaseController<TenantRegistrationInfo>
		{
			public LicenceRegistrationBaseControllerForTest() : base()
			{
			}

			public LicenceRegistrationBaseControllerForTest(NLogWrapper logger) : base(logger)
			{
			}
