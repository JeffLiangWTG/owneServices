...
{
	[TestedType(typeof(MultiSectionStlRawUsage))]
	internal class MultiSectionStlRawUsageTest : NonPersistentBusinessObjectTestCase
	{
		public void TestGetRawUsageSummarySections()
		{
			var context = new BillingLoadRawUsageContext(Factory, EdiDateTest.MonthToday, ZGuid.NewZGuid(), ZGuid.Empty, ZGuid.Empty);
			var rawUsage = new MultiSectionStlRawUsage(context, BillingConstants.BillingSystem.AirlineMessaging);
		...
		{
			var context = new BillingLoadRawUsageContext(Factory, EdiDateTest.MonthToday, ZGuid.NewZGuid(), ZGuid.Empty, ZGuid.Empty);
			return new MultiSectionStlRawUsage(context, BillingConstants.BillingSystem.AirlineMessaging);
		}
