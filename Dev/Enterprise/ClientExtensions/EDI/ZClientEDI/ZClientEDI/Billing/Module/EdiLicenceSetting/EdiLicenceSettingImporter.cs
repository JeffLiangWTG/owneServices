...
{
	public class EdiLicenceSettingImporter
	{
		public EdiLicenceSettingImporter()
		{
		}

		public void Import()
		{
			var importInfo = new EdiLicenceSettingImportInfo(new EdiLicenceSettingFlattenedCollection(new BusinessObjectFactory()));
			var processor = new EdiLicenceSettingFlattenedDataTransferProcessor(new EdiLicenceSettingCollectionNonDependent(new BusinessObjectFactory()), importInfo);
			const string contextKey = "{fbd8537b-2ebf-4297-9843-9ed6640475fc}";
			var isCancelled = false;
			var form = new MultistepDataImportWizardForm(importInfo, contextKey, new DataTransferProcessor[] { processor }, "Licence Setting");
			form.Cancelled += (s, e) => { processor.Rollback(); isCancelled = true; };
			form.Imported += (s, e) => { DisplayResult(processor, isCancelled); };
			form.Show();
		}

		void DisplayResult(EdiLicenceSettingFlattenedDataTransferProcessor processor, bool isCancelled)
		{
			string message;
			string caption;
			if (isCancelled)
