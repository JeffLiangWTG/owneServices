	...
	public partial class TreeViewAdv
	{
		...
		private void SetDropPosition(Point pt)
		{
			...
			if (node != null)
			{
				...
				if (pos < TopEdgeSensivity && AllowDropPositionBefore(node))
					_dropPosition.Position = NodePosition.Before;
				else if (pos > (1 - BottomEdgeSensivity) && AllowDropPositionAfter(node))
		...
		protected virtual bool AllowDropPositionBefore(TreeNodeAdv node)
		{
			return true;
		}

		protected virtual bool AllowDropPositionAfter(TreeNodeAdv node)
		{
			return true;
		}
		...
		protected override void OnDragEnter(DragEventArgs dragEvent)
		{
			_search.EndSearch();
			DragMode = true;
			CreateDragBitmap(dragEvent.Data);
			base.OnDragEnter(dragEvent);
		}

#else

		protected override void OnDragOver(DragEventArgs dragEvent) => base.OnDragOver(dragEvent);

		protected override void OnDragEnter(DragEventArgs dragEvent) => base.OnDragEnter(dragEvent);
