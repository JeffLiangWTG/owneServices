...
namespace Aga.Controls.Tree
{
	public partial class TreeViewAdv
	{
		...
		private void CreateLinePen()
		{
			...
			_linePen.DashStyle = _lineDashStyle;
		}
		...
		protected virtual void DrawLines(Graphics gr, TreeNodeAdv node, Rectangle rowRect)
		{
			...
			while (curNode != _root && curNode != null)
			{
				...
				var lineColourDecidingEventArgs = new LineColourDecidingEventArgs(curNode, _linePen);
				OnLineColourDeciding(lineColourDecidingEventArgs);
				var penToUse = lineColourDecidingEventArgs.Pen;

				if (curNode == node)
				{
					int midy = y + rowRect.Height / 2;
					gr.DrawLine(penToUse, x, midy, x + width, midy);
		...
		protected void OnLineColourDeciding(LineColourDecidingEventArgs args)
		{
			if (LineColourDeciding != null)
			{
				LineColourDeciding(this, args);
			}
		}
		public event EventHandler<LineColourDecidingEventArgs> LineColourDeciding;

		public class LineColourDecidingEventArgs : EventArgs
		{
			public LineColourDecidingEventArgs(TreeNodeAdv node, Pen pen)
			{
				Node = node;
				Pen = pen;
			}

			public readonly TreeNodeAdv Node;
			public Pen Pen;
		}
