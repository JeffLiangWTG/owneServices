	...
	public partial class TreeViewAdv : Control
	{
		...
		public TreeViewAdv()
		{
			...
			if (Application.RenderWithVisualStyles)
				_columnHeaderHeight = ScaleToCurrentDpiY(20);
			else
				_columnHeaderHeight = ScaleToCurrentDpiY(17);
...
#else
			_measureContext.Graphics = new BGraphics();
#endif
		...
		{
			return (int)Math.Ceiling(ScaleFactorY * valueUnscaled - 0.5);
		}

#if WINZOR

		const float ScaleFactorY = 1;

#else

		static readonly float ScaleFactorY = GetScaleFactorY();

		static float GetScaleFactorY()
		{
		...
		internal void SetIsExpanded(TreeNodeAdv node, bool value, bool ignoreChildren)
		{
			...
			{
				WaitCallback wc = delegate (object argument)
