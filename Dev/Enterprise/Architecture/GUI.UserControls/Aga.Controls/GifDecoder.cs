		...
		private void SetPixels()
		{
			...
			if (lastDispose > 0)
			{
				...
				if (lastImage != null)
				{
					...
					using (var bitmap = new Bitmap(lastImage))
					{
						int[] prev = GetPixels(bitmap);
					...
					if (lastDispose == 2)
					{
						...
						using (Brush brush = new SolidBrush(transparency ?
							Color.FromArgb(0, 0, 0, 0) :        // assume background is transparent
							Color.FromArgb(lastBgColor)))       // use given background color
						{
