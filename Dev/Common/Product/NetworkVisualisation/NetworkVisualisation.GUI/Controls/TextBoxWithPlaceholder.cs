...
namespace CargoWise.NetworkVisualisation.GUI
{
	public class TextBoxWithPlaceholder : TextBox
	{
		...
		public static readonly DependencyProperty PlaceholderTextProperty = DependencyProperty.Register("PlaceholderText", typeof(string), typeof(TextBoxWithPlaceholder), new FrameworkPropertyMetadata(string.Empty, FrameworkPropertyMetadataOptions.AffectsRender));

		// Shadowed BackgroundProperty to disassociate base.Background and this.Background. This causes the effect that OnRender can just show the PlaceholderText rather than the usual textbox background.
		...
		public new static readonly DependencyProperty BackgroundProperty = DependencyProperty.Register("Background", typeof(Brush), typeof(TextBoxWithPlaceholder), new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.AffectsRender));
		...
		protected override void OnRender(DrawingContext drawingContext)
		{
			...
			if (!IsFocused && string.IsNullOrEmpty(Text) && !string.IsNullOrEmpty(PlaceholderText))
			{
				...
				Typeface typeface = new Typeface(FontFamily, this.FontStyle, FontWeight, FontStretch);

#pragma warning disable CS0618 // Type or member is obsolete (in .NET 4.6.2)
				var formattedText = new FormattedText(PlaceholderText, CultureInfo.CurrentCulture, FlowDirection, typeface, FontSize, foreground);
#pragma warning restore CS0618 // Type or member is obsolete
				...
				var proposedWidth = RenderSize.Width - BorderThickness.Left - BorderThickness.Right - Padding.Left - Padding.Right - 4.0;

				formattedText.MaxTextWidth = Math.Max(proposedWidth, 0);
