...
namespace Enterprise.Upgrades.Postinstall.Testing
{
	class ProgramTest
	{
		...
		public void TestErrorsAreShownOnInstallationResultsForm()
		{
			...
			Assert.DoesNotThrow(() =>
			{
				exitCode = program.StartApplication(new[] { @"TempPath\20.8.20.0", "-NoUI" });
			});
		...
		public void TestInitializeParseCommandLineArgs()
		{
			...
			TestInitializeParseCommandLineArgsCore(new[] { @"TempPath\20.8.20.0" }, Environment.UserInteractive ? UILevel.Normal : UILevel.AutomatedWithNoUI);
			TestInitializeParseCommandLineArgsCore(new[] { @"TempPath\20.8.20.0", "-NoUI" }, UILevel.AutomatedWithNoUI);
		}
		...
		public void TestInitializeWrongParamsCall()
		{
			...
			Assert.That(
				argumentException,
				Has.Message.StartsWith("Enterprise.Upgrades.Postinstall command line args must contain at least the installation path."));
		}
	}
