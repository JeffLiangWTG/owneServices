...
namespace CargoWise.Windows.UI
{
	...
	public class MouseHook : LocalWindowsHook, IDisposable
	{
		...
		protected int MouseProc(int code, UIntPtr wParam, IntPtr lParam)
		{
			// by convention a code < 0 means skip
			if (code < 0)
			{
				return (int)NativeMethods.CallNextHookEx(m_hhook, code, wParam, lParam);
			}

			// we're gonna ignore peek messages
			if (code == Win32.HC_ACTION)
			{
				MouseHookEventArgs e = CrackHookMsg(wParam, (Win32.MOUSEHOOKSTRUCT)Marshal.PtrToStructure(lParam, typeof(Win32.MOUSEHOOKSTRUCT)));
				RaiseMouseHookEvent(wParam, e);
				if (e.DisableMessage)
				{
					return 1;
				}
...
#pragma warning disable CA1502
		...
		void RaiseMouseHookEvent(UIntPtr wParam, MouseHookEventArgs args)
		{
			unchecked
			{
...
#pragma warning restore CA1502
		...
		/// <param name="hookStruct">Mouse message arguments</param>
		/// <returns>A repacked set of MouseProc arguments</returns>
		MouseHookEventArgs CrackHookMsg(UIntPtr wParam, Win32.MOUSEHOOKSTRUCT hookStruct)
		{
		...
		public struct Win32
		{
			...
			public static ushort HIWORD(int l) { return (ushort)((l >> 16) & 0xFFFF); }
