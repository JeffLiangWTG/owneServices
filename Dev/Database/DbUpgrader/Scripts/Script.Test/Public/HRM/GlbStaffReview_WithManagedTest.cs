...
{
	[TestedType(typeof(GlbStaffReview_WithManaged))]
	class GlbStaffReview_WithManagedTest : ManagedTypeTest<GlbStaffReviewSchema>
	{
		protected override string StaffFKColumnName => "GSV_GS_Staff";
...
INSERT INTO hrm.GlbStaffReview
	(GSV_PK, GSV_GS_Staff, GSV_EffectiveDate, GSV_GS_NKReviewer, GSV_Score, GSV_Comments, GSV_SystemCreateTimeUtc, GSV_SystemCreateUser, GSV_SystemLastEditTimeUtc, GSV_SystemLastEditUser, GSV_AutoEffectiveEndDate)
VALUES
	(NEWID(), @staff0, '2021-02-09 00:00:00', 'AA1', 1.1, 'SampleText 1', GETDATE(), 'E', GETDATE(), 'E', NULL),
	(NEWID(), @staff1, '2021-06-19 00:00:00', 'BB2', 2.2, 'SampleText 2', GETDATE(), 'E', GETDATE(), 'E', NULL),
	(NEWID(), @staff2, '2021-07-15 00:00:00', 'CC3', 3.3, 'SampleText 3', GETDATE(), 'E', GETDATE(), 'E', NULL),
	(NEWID(), @staff3, '2021-02-23 00:00:00', 'DD4', 4.4, 'SampleText 4', GETDATE(), 'E', GETDATE(), 'E', NULL);
",
		p =>
		{
			...
			expectedColumns.AddRange(["GSV_AutoVersion", "IsSelf", "IsManaged1", "IsManaged2", "IsManaged3"]);
		...
		public void TestCannotExecuteIfLoginHasNoPermission()
		{
			...
			AssertExceptionThrown(typeof(SqlException), "The SELECT permission was denied on the object 'GlbStaffReview_WithManaged', database '" + TestConnection.CurrentDatabase + "', schema 'hrm'.",
