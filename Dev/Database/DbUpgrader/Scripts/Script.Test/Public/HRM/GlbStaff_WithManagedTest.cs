...
{
	[TestedType(typeof(GlbStaff_WithManaged))]
	class GlbStaff_WithManagedTest : ManagedTypeTest<GlbStaffSchema>
	{
		protected override string StaffFKColumnName => "GS_PK";
...
",
		p =>
		{
			...
			var dataRows = results.Select().OrderBy(row => row["GS_PK"]).ToArray();

			DataTable originalDataTable;
			using (var command = TestConnection.Command($"SELECT * FROM {Schema.SqlSchemaName}.{Schema.TableName} WHERE GS_Code IN ('S00', 'S01', 'S02', 'S03')"))
			{
				originalDataTable = DataUtils.GetDataTableFromCommand(command);

				originalDataTable.DefaultView.Sort = "GS_PK";
				originalDataTable = originalDataTable.DefaultView.ToTable();
			}

			var tableColumns = Schema.All.Select(c => c.Name).ToList();
			var expectedColumns = new List<string>();
			expectedColumns.AddRange(tableColumns);
			expectedColumns.AddRange(["GS_AutoVersion", "IsSelf", "IsManaged1", "IsManaged2", "IsManaged3"]);
