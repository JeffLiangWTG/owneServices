...
{
	[TestedType(typeof(HRMRemHistoryCompact_WithManaged))]
	class HRMRemHistoryCompact_WithManagedTest : DbCreateScriptTest
	{
...
VALUES
	...
	(NEWID(), @staff1, '2022-01-13 08:50:00 -04:00', 'WB2', 0, 'E', 'E', GETUTCDATE(), GETUTCDATE());
			...
			for (int i = 0; i < baseview.Rows.Count; i++)
			{
				...
				var expectedIsSelf = (Guid)resultRows[i]["GMR_GS_Staff"] == staff0;
				var expectedIsManaged1 = (Guid)resultRows[i]["GMR_GS_Staff"] == staff2;
				var expectedIsManaged2 = (Guid)resultRows[i]["GMR_GS_Staff"] == staff1;
				var expectedIsManaged3 = (Guid)resultRows[i]["GMR_GS_Staff"] == staff3;
		...
		{
			using (var command = TestConnection.Command($"SELECT * FROM hrm.HRMRemHistoryCompact_WithManaged(@loggedInStaff, @managerTypes, @effectiveAsAt)"))
			{
