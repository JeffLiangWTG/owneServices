...
{
	[TestedType(typeof(HRMStaffHistoryCompact_WithManaged))]
	class HRMStaffHistoryCompact_WithManagedTest : DbCreateScriptTest
	{
...
VALUES
	...
	(NEWID(), @staff1, '2015-06-20 10:05:23', 'WFO', @benBranch3PK, '73 ORiordan Street', 'AlexandriTwo', 'Sydney', 'NSW', '2016', 'AU', 'MAN', GETUTCDATE(), 'E', GETUTCDATE(), 'E');
			...
			DataTable baseview;
			using (var command = TestConnection.Command("SELECT EHV_PK, EHV_ParentID FROM hrm.HRMStaffHistoryBase"))
			{
			...
			AssertEquals(2 + hrmColumns.Length, results.Columns.Count);
			Assert(results.Columns.Contains("EHV_PK"));
			Assert(results.Columns.Contains("EHV_ParentID"));
			...
			{
				var matchedRow = resultRows.FirstOrDefault(r => r["EHV_PK"].Equals(baseview.Rows[i]["EHV_PK"]));
				AssertNotNull(matchedRow);

				AssertEquals("EHV_ParentID", baseview.Rows[i]["EHV_ParentID"], matchedRow["EHV_ParentID"]);

				var expectedIsSelf = (Guid)resultRows[i]["EHV_ParentID"] == staff0;
				var expectedIsManaged1 = (Guid)resultRows[i]["EHV_ParentID"] == staff2;
				var expectedIsManaged2 = (Guid)resultRows[i]["EHV_ParentID"] == staff1;
				var expectedIsManaged3 = (Guid)resultRows[i]["EHV_ParentID"] == staff3;

				AssertEquals(expectedIsSelf, resultRows[i]["IsSelf"]);
				AssertEquals(expectedIsManaged1, resultRows[i]["IsManaged1"]);
				AssertEquals(expectedIsManaged2, resultRows[i]["IsManaged2"]);
				AssertEquals(expectedIsManaged3, resultRows[i]["IsManaged3"]);
			}
		}

		DataTable Execute(Guid loggedInStaff, string managerTypes, DateTimeOffset effectiveAsAt)
		{
			using (var command = TestConnection.Command($"SELECT * FROM hrm.HRMStaffHistoryCompact_WithManaged(@loggedInStaff, @managerTypes, @effectiveAsAt)"))
			{
