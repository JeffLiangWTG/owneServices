...
{
	[TestedType(typeof(WhsStockOnHandReport))]
	class WhsStockOnHandReportTest : DbCreateScriptTest
	{
		...
		public void TestNoArithmeticOverflowExceptionThrown_WhenStockOnHandIsMassive()
		{
			...
			AssertEquals("Total Client Units should return 0, since Stock on Hand is too large to calculate Conversion.", 0m,
				TestConnection.ExecuteScalar($"SELECT TotalClientUnits FROM WhsStockOnHandReport({client.PK.AsSQL()}, {warehouse.PK.AsSQL()}, {product.PK.AsSQL()}, null)"));
		}
