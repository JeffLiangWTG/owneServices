...
{
	[TestedType(typeof(GetWorkflowDescendentHierarchy))]
	sealed class GetWorkflowDescendentHierarchyTest : DbCreateScriptTest
	{
		...
		{
			var sql = string.Format("SELECT FH_PK from dbo.GetWorkflowDescendentHierarchy('{0}', {1})", startingPK, recursionLimit);
		...
		public void TestChildWorkflowFromDifferentJob()
		{
			...
			var hasChildFromDifferentJob = $@"SELECT DISTINCT 1 
FROM dbo.ProcessHeaderLink AS link 
INNER JOIN dbo.ProcessHeader AS child ON child.FH_PK = link.FP_FH_HeaderFrom
INNER JOIN dbo.ProcessHeader AS parent ON parent.FH_PK	= link.FP_FH_HeaderTo
WHERE parent.FH_PK='{workflow1ParentPK}'
	AND parent.FH_ParentId <> child.FH_ParentId
	AND FP_LinkType = 'PCH'";

			var result = ExecuteScalar(TestConnection, hasChildFromDifferentJob);
			AssertNotNullOrEmpty("GIVEN has child-workflow from different job", result);

			var command = TestConnection.Command($"SELECT FH_PK FROM GetWorkflowDescendentHierarchy('{workflow1ParentPK}', 100)");
			var descendentsPK = DataUtils.GetDataTableFromCommand(command);

			CombineAssertions("WHEN executing GetWorkflowDescendentHierarchy, THEN should return child workflow from different job", () =>
			{
				AssertEquals("count", 2, descendentsPK.Rows.Count);
				AssertContainsExactElementsInAnyOrder("FH_PK", new[] { workflow1ParentPK, workflow2ChildPK }, descendentsPK.Rows.Cast<DataRow>().Select(x => x["FH_PK"]));
			});
		}

		static string ExecuteScalar(DbConnection connection, string sql)
		{
