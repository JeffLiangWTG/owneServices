...
{
	[TestedType(typeof(TG_INS_RatingCalcUnit))]
	class TG_INS_RatingCalcUnitTest : DbCreateScriptTest
	{
		...
		public void TestSetsTypeToUnit()
		{
			var item = InsertItem();
			AssertEquals("UNT", item.Item2["TM_Type"]);
		}
		...
		Tuple<Guid, DataRow> InsertItem()
		{
			...
			var commandText = pks.Fill("INSERT dbo.RatingCalcUnit (TM_PK, TM_TL, TM_Value, TM_SystemLastEditTimeUtc, TM_SystemLastEditUser, TM_SystemCreateTimeUtc, TM_SystemCreateUser) VALUES ('@item', '@rateLine', 990.123, GetUtcDate(), '~BP', GetUtcDate(), '~BP')");
