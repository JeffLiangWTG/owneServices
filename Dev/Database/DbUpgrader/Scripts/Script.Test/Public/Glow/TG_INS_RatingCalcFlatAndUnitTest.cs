...
{
	[TestedType(typeof(TG_INS_RatingCalcFlatAndUnit))]
	class TG_INS_RatingCalcFlatAndUnitTest : DbCreateScriptTest
	{
		...
		Tuple<GuidHelper, DataRow> InsertItem()
		{
			...
			var commandText = pks.Fill("INSERT dbo.RatingCalcFlatAndUnit (TM_PK, TM_TL, TM_Flat, TM_Unit, TM_SystemLastEditTimeUtc, TM_SystemLastEditUser, TM_SystemCreateTimeUtc, TM_SystemCreateUser) VALUES ('@item', '@rateLine', 11.22, 33.44, GetUtcDate(), '~BP', GetUtcDate(), '~BP')");
			using (var command = TestConnection.Command(commandText))
			{
				command.ExecuteNonQuery();
			}

			return Tuple.Create(pks, generator.GetRateLineItem(pks["item"], "RatingCalcFlatAndUnit"));
		}
	}
}
