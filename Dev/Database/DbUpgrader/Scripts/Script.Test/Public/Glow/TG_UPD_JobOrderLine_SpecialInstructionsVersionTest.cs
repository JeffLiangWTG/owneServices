...
{
	[TestedType(typeof(TG_UPD_JobOrderLine_SpecialInstructionsVersion))]
	class TG_UPD_JobOrderLine_SpecialInstructionsVersionTest : DbCreateScriptTest
	{
		public void TestSpecialInstructionsVersion()
		{
...
INSERT INTO dbo.JobOrderLine (JO_PK, JO_JD) VALUES ('{orderLinePk}','{orderHeaderPk}');");
			AssertEquals((short)0, TestConnection.ExecuteScalar<short>($"SELECT JO_SpecialInstructionsVersion FROM dbo.JobOrderLine WHERE JO_PK='{orderLinePk}'"));
...
WHERE
	JO_PK='{orderLinePk}'");
		...
		{
			using (var command = TestConnection.Command("SELECT JO_SpecialInstructionsVersion, JO_SystemLastEditTimeUtc, JO_SystemLastEditUser FROM dbo.JobOrderLine WHERE JO_PK= @orderLinePk"))
			{
