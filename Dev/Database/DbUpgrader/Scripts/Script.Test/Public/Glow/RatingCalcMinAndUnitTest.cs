...
{
	[TestedType(typeof(RatingCalcMinAndUnit))]
	class RatingCalcMinAndUnitTest : DbCreateScriptTest
	{
		public void TestLineWithMinAndUnit_SelectsPKFromMinItem()
		{
			var item = GetItem("rateLine1");
			var key = item.Item1.Key(item.Item2["TM_PK"]);
			AssertEquals("minItem1", key);
		}

		public void TestLineWithMinAndUnit_SelectsTM_UnitFromUnitItem()
		{
			var item = GetItem("rateLine1");
		...
		public void TestLineWithMinAndUnit_SelectsMinFromMinItem()
		{
			var item = GetItem("rateLine1");
			AssertEquals(11.22m, item.Item2["TM_Min"]);
		}

		public void TestLineWithMinAndUnit_SelectsUnitFromUnitItem()
		{
			var item = GetItem("rateLine1");
			AssertEquals(22.33m, item.Item2["TM_Unit"]);
		}

		public void TestLineWithMinOnly_SelectsPKFromMinItem()
		{
			var item = GetItem("rateLine2");
			var key = item.Item1.Key(item.Item2["TM_PK"]);
			AssertEquals("minItem2", key);
		}

		public void TestLineWithMinOnly_SelectsTM_UnitAsNull()
		{
			var item = GetItem("rateLine2");
			AssertEquals(DBNull.Value, item.Item2["TM_TM_Unit"]);
		}

		public void TestLineWithMinOnly_SelectsMinFromMinItem()
		{
			var item = GetItem("rateLine2");
			AssertEquals(33.44m, item.Item2["TM_Min"]);
		}

		public void TestLineWithMinOnly_SelectsUnitAsZero()
		{
			var item = GetItem("rateLine2");
		...
		public void TestLineWithUnitOnly_SelectsMinAsZero()
		{
			var item = GetItem("rateLine3");
			AssertEquals(0m, item.Item2["TM_Min"]);
		}
		...
		Tuple<GuidHelper, DataRow> GetItem(string lineKey)
		{
			...
			pks["minItem1"] = generator.NewRateLineItem(pks["rateLine1"], "MIN", value: 11.22m);
			pks["unitItem1"] = generator.NewRateLineItem(pks["rateLine1"], "UNT", value: 22.33m);

			pks["rateLine2"] = generator.NewRateLine();
			pks["minItem2"] = generator.NewRateLineItem(pks["rateLine2"], "MIN", value: 33.44m);

			pks["rateLine3"] = generator.NewRateLine();
			pks["unitItem3"] = generator.NewRateLineItem(pks["rateLine3"], "UNT", value: 44.55m);

			var commandText = string.Format("SELECT * FROM dbo.RatingCalcMinAndUnit WHERE TM_TL = ('{0}')", pks[lineKey]);
