...
{
	[TestedType(typeof(ViewLatestHighestLevelAccreditationAttempt))]
	class ViewLatestHighestLevelAccreditationAttemptTest : DbCreateScriptTest
	{
...
SELECT HLA_HAA, HLA_IsCompleted, HLA_PER, HLA_HAG
FROM dbo.ViewLatestHighestLevelAccreditationAttempt
JOIN dbo.GlbAccreditationAttempt ON HAA_PK = HLA_HAA
JOIN dbo.GlbAccreditation ON HAC_PK = HAA_HAC
WHERE 
	HLA_HAG = @HAG_PK
	AND HLA_IsCompleted = @IsCompleted
	AND HLA_PER = @PersonPK
ORDER BY HAC_Code");
