...
{
	[TestedType(typeof(vw_AGG__DocAddressExtended))]
	class vw_AGG__DocAddressExtendedTest : BiCreateScriptTest
	{
		[ExpectNoExceptions]
		public void TestRun()
		{
		...
		HashSet<string> GetColumns()
		{
			...
			columns.Add("DocAddressAddressOverride");
			columns.Add("DocAddressAddressSequence");
			columns.Add("DocAddressAddressType");
			columns.Add("DocAddressCompanyName");
			columns.Add("DocAddressKey");
			columns.Add("DocAddressOrganizationAddressKey");
			columns.Add("DocAddressParentID");
			columns.Add("DocAddressParentTableCode");
		...
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT * FROM [{0}].[Organization].[vw_AGG__DocAddressExtended]",
					ScriptDbName
			);
