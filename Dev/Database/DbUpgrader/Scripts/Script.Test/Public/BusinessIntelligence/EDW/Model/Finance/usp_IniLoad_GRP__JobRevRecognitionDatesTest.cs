...
namespace Enterprise.Build.Database.Script.Public.BusinessIntelligence.EDW.Model.Finance.Testing
{
	...
	internal class usp_IniLoad_GRP__JobRevRecognitionDatesTest : BiCreateScriptTest
	{
		/// <summary>
		...
		{
			string sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT [InitialLoadQuery] FROM [{0}].[biAdmin].[CustomTableConfiguration] WHERE  [ModelSchemaName] = 'Finance' AND [ModelTableName] = 'GRP__JobRevRecognitionDates'",
					ScriptDbName
			);
		...
		void Execute()
		{
			string iniLoadSQLText = GetIniLoadSQLText();
			string sqlText1 = "USE " + ScriptDbName + " " + iniLoadSQLText;
