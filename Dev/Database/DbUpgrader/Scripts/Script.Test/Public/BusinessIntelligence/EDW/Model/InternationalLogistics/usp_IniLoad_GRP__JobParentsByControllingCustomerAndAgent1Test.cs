...
namespace Enterprise.Build.Database.Script.Public.BusinessIntelligence.EDW.Model.InternationalLogistics.Testing
{
	...
	internal class usp_IniLoad_GRP__JobParentsByControllingCustomerAndAgent1Test : BiCreateScriptTest
	{
		[ExpectNoExceptions]
		public void TestRun()
		{
		...
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT [InitialLoadQuery] FROM [{0}].[biAdmin].[CustomTableConfiguration] WHERE  [ModelSchemaName] = 'InternationalLogistics' AND [ModelTableName] = 'GRP__JobParentsByControllingCustomerAndAgent1'",
					ScriptDbName
			);
