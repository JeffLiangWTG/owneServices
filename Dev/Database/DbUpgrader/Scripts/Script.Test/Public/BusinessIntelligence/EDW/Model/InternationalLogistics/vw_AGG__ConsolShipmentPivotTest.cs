...
{
	[TestedType(typeof(vw_AGG__ConsolShipmentPivot))]
	internal class vw_AGG__ConsolShipmentPivotTest : BiCreateScriptTest
	{
		/// <summary>
		...
		HashSet<string> GetColumns()
		{
			...
			columns.Add("ConsolidationShipmentPivotKey");
			columns.Add("JobNumber");
			columns.Add("ShipmentID");
			columns.Add("ShipmentKey");
		...
		DataTable SelectRows()
		{
			string sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT * FROM [{0}].[InternationalLogistics].[vw_AGG__ConsolShipmentPivot]",
					ScriptDbName
			);
