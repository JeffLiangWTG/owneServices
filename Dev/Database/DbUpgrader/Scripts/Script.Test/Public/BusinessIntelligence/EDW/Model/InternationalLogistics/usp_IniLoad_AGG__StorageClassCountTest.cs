...
{
	[TestedType(typeof(usp_IniLoad_AGG__StorageClassCount))]
	internal class usp_IniLoad_AGG__StorageClassCountTest : BiCreateScriptTest
	{
		[ExpectNoExceptions]
		public void TestRun()
		{
		...
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT * FROM [{0}].[InternationalLogistics].[AGG__StorageClassCount]",
					ScriptDbName
			);
