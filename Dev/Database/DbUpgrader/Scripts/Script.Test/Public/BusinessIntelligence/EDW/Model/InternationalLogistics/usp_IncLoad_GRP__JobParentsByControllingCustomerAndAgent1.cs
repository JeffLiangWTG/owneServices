...
namespace Enterprise.Build.Database.Script.Public.BusinessIntelligence.EDW.Model.InternationalLogistics.Testing
{
	...
	internal class usp_IncLoad_GRP__JobParentsByControllingCustomerAndAgent1Test : BiCreateScriptTest
	{
		[ExpectNoExceptions]
		public void TestRun()
		{
		...
		void PrepareTestData()
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture, @"
				...
				INSERT INTO[{0}].[biadmin].[TransformedRow]
					...
					SELECT 'InternationalLogistics', 'BAS__ControllingCustomerAndAgentAddress', ControllingCustomerAndAgentAddressKey 
					FROM [{0}].[InternationalLogistics].[BAS__ControllingCustomerAndAgentAddress]",
		...
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT * FROM [{0}].[InternationalLogistics].[GRP__JobParentsByControllingCustomerAndAgent1]",
					ScriptDbName
			);
		...
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT [IncrementalLoadQuery] FROM [{0}].[biAdmin].[CustomTableConfiguration] WHERE  [ModelSchemaName] = 'InternationalLogistics' AND [ModelTableName] = 'GRP__JobParentsByControllingCustomerAndAgent1'",
					ScriptDbName
			);
