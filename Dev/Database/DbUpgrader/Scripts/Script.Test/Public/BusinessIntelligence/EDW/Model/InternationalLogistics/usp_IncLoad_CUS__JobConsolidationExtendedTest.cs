...
namespace Enterprise.Build.Database.Script.Public.BusinessIntelligence.EDW.Model.InternationalLogistics.Testing
{
	...
	internal class usp_IncLoad_CUS__JobConsolidationExtendedTest : BiCreateScriptTest
	{
		/// <summary>
		...
		string GetIniLoadSQLText()
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT [IncrementalLoadQuery] FROM [{0}].[biAdmin].[CustomTableConfiguration] WHERE  [ModelSchemaName] = 'InternationalLogistics' AND [ModelTableName] = 'CUS__JobConsolidationExtended'",
					ScriptDbName
			);
