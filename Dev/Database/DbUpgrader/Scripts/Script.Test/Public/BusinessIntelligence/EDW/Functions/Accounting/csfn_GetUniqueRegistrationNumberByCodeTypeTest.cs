...
namespace Enterprise.Build.Database.Script.Public.BusinessIntelligence.EDW.Functions.Accounting.Testing
{
	...
	class csfn_GetUniqueRegistrationNumberByCodeTypeTest : BiCreateScriptTest
	{
		public void Testcsfn_GetUniqueRegistrationNumber()
		{
			var insertSql = $@"
				...
				INSERT INTO {Db.EdwDatabaseName}.Organization.BAS__Organization (OrganizationID, OrganizationKey, Code) VALUES ('4CD7C9F7-9EC9-4CDF-B595-21B90BB9C392', 1, 'TESTORG1');
				INSERT INTO {Db.EdwDatabaseName}.Organization.BAS__Organization (OrganizationID, OrganizationKey, Code) VALUES ('BEA9CDC0-C0CC-44EC-BFBE-D9B7DF04D9D6', 2, 'TESTORG1');

				INSERT INTO {Db.EdwDatabaseName}.Organization.BAS__OrgCusCode (OrgCusCodeID, OrgCusCodeKey, CustomsRegNo, CodeType, CodeCountry, OrganizationKey) VALUES (newid(), 1, 'ABN1234', 'ABN', 'AU', 1)
				INSERT INTO {Db.EdwDatabaseName}.Organization.BAS__OrgCusCode (OrgCusCodeID, OrgCusCodeKey, CustomsRegNo, CodeType, CodeCountry, OrganizationKey) VALUES (newid(), 2, 'ABN1234', 'ABN', 'NZ', 1)

				INSERT INTO {Db.EdwDatabaseName}.Organization.BAS__OrgCusCode (OrgCusCodeID, OrgCusCodeKey, CustomsRegNo, CodeType, CodeCountry, OrganizationKey) VALUES (newid(), 3, 'GST1234', 'GST', 'NZ', 1)
				";

			TestConnection.ExecuteNonQuery(insertSql);

			var sqlQuery = $@"SELECT * FROM {Db.EdwDatabaseName}.dbo.csfn_GetUniqueRegistrationNumberByCodeType(@OrgKey, @CodeType, @CountryCode)";

			var command = Db.Connection.Command(sqlQuery);
			command.AddParameter("@OrgKey", SqlDbType.BigInt, 2); //Org2
			...
			command.AddParameter("@OrgKey", SqlDbType.BigInt, 1); //Org1
			command.AddParameter("@CodeType", SqlDbType.Char, "ABN");
			command.AddParameter("@CountryCode", SqlDbType.VarChar, "");
