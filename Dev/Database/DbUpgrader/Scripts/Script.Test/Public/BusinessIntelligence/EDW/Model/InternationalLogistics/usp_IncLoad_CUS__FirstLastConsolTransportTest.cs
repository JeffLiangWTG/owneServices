...
namespace Enterprise.Build.Database.Script.Public.BusinessIntelligence.EDW.Model.InternationalLogistics.Testing
{
	...
	internal class usp_IncLoad_CUS__FirstLastConsolTransportTest : BiCreateScriptTest
	{
		[ExpectNoExceptions]
		public void TestRun()
		{
		...
		void PrepareTestData()
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture, @"
				...
				INSERT INTO[{0}].[biadmin].[TransformedRow]
					...
					SELECT 'InternationalLogistics', 'BAS__ConsolAndShipmentTransport', ConsolAndShipmentTransportKey 
					FROM [{0}].[InternationalLogistics].[BAS__ConsolAndShipmentTransport]",
		...
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT [IncrementalLoadQuery] FROM [{0}].[biAdmin].[CustomTableConfiguration] WHERE  [ModelSchemaName] = 'InternationalLogistics' AND [ModelTableName] = 'CUS__FirstLastConsolTransport'",
					ScriptDbName
			);
