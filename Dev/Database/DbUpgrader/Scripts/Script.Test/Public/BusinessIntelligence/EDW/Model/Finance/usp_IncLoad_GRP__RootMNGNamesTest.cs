...
namespace Enterprise.Build.Database.Script.Public.BusinessIntelligence.EDW.Model.Finance.Testing
{
	...
	internal class usp_IncLoad_GRP__RootMNGNamesTest : BiCreateScriptTest
	{
		[ExpectNoExceptions]
		public void TestRun()
		{
		...
		void PrepareTestData()
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture, @"
				...
				INSERT INTO[{0}].[biadmin].[TransformedRow]
					...
					SELECT 'Organization', 'BAS__OrganizationRelatedParty', OrganizationRelatedPartyKey 
		...
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT [IncrementalLoadQuery] FROM [{0}].[biAdmin].[CustomTableConfiguration] WHERE  [ModelSchemaName] = 'Finance' AND [ModelTableName] = 'GRP__RootMNGNames'",
					ScriptDbName
			);
