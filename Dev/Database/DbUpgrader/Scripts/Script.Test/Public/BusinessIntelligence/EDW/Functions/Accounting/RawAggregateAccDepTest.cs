...
namespace Enterprise.Build.Database.Script.Testing.Public.BusinessIntelligence.EDW.Functions.Accounting
{
	...
	class RawAggregateAccDepTest : BiCreateScriptTest
	{
		...
		public void TestTransactionCategoryGroup()
		{
			...
			var selectFromProcedure = $"SELECT * FROM [{ScriptDbName}].[dbo].RawAggregateAccDep(202001,'CBH',202002,1,'{"{0}"}')";

			var result = DataUtils.GetDataTableFromQuery(TestConnection, string.Format(selectFromProcedure, "A00"));
			AssertEquals("Result should have one row", 1, result.Rows.Count);
			AssertEquals("singe category", 100m, result.Rows[0]["CurrentAmount"]);

			result = DataUtils.GetDataTableFromQuery(TestConnection, string.Format(selectFromProcedure, categoryGroupBuilder.ToString().TrimEnd(',')));
		...
		public void TestCanGetAllAggregateData()
		{
			...
			var selectFromProcedure = $"SELECT * FROM [{ScriptDbName}].[dbo].RawAggregateAccDep(201901,'CBH',202002,1,'{"{0}"}')";
