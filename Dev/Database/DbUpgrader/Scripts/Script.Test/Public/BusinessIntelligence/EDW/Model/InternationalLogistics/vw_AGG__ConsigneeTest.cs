...
{
	[TestedType(typeof(vw_AGG__Consignee))]
	internal class vw_AGG__ConsigneeTest : BiCreateScriptTest
	{
		/// <summary>
		...
		HashSet<string> GetColumns()
		{
			...
			columns.Add("OrganizationAddressOrganizationKey");
		...
		DataTable SelectRows()
		{
			string sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT * FROM [{0}].[InternationalLogistics].[vw_AGG__Consignee]",
					ScriptDbName
			);
