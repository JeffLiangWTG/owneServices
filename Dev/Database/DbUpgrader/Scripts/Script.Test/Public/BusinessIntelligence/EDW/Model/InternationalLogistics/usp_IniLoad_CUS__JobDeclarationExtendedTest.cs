...
namespace Enterprise.Build.Database.Script.Public.BusinessIntelligence.EDW.Model.InternationalLogistics.Testing
{
	...
	internal class usp_IniLoad_CUS__JobDeclarationExtendedTest : BiCreateScriptTest
	{
		/// <summary>
	 ...
	 string sendingForwarderCode, int? sendingForwarderKey, Guid? sendingForwarderPK, string shippingLineCode, int? shippingLineKey, Guid? shippingLinePK)
		...
		string GetIniLoadSQLText()
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT [InitialLoadQuery] FROM [{0}].[biAdmin].[CustomTableConfiguration] WHERE  [ModelSchemaName] = 'InternationalLogistics' AND [ModelTableName] = 'CUS__JobDeclarationExtended'",
					ScriptDbName
			);
