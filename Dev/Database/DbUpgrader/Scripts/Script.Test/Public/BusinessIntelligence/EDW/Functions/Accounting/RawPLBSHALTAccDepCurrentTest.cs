...
namespace Enterprise.Build.Database.Script.Testing.Public.BusinessIntelligence.EDW.Functions.Accounting
{
	...
	class RawPLBSHALTAccDepCurrentTest : BiCreateScriptTest
	{
		...
		public void TestRawPLBSHALTAccDepCurrent()
		{
			PrepareData();

			Helper.InsertGLAggregate(100, 201009, 1);
			Helper.InsertGLAggregate(200, 201009, 2);
			Helper.InsertGLAggregate(300, 201009, 3);
			Helper.InsertGLAggregate(400, 201009, 4);

			DataTable result = DataUtils.GetDataTableFromQuery(TestConnection, $@"SELECT * FROM {ScriptDbName}.dbo.RawPLBSHALTAccDepCurrent (201009, '878D7ACA-FFC3-49FC-9710-969CA0C0F2AC', 'CBH', '', '')");
			AssertEquals("Row count == 2", 2, result.Rows.Count);
			AssertEquals("The GLAccountKey of row 1", 1L, result.Rows[0]["GLAccountKey"]);
			AssertEquals("The AmountCurrentRevenue of row 1", 0m, result.Rows[0]["AmountCurrentRevenue"]);
			AssertEquals("The AmountCurrentCost of row 1", -100m, result.Rows[0]["AmountCurrentCost"]);
			AssertEquals("The GLAccountKey of row 2", 2L, result.Rows[1]["GLAccountKey"]);
			AssertEquals("The AmountCurrentRevenue of row 2", -200m, result.Rows[1]["AmountCurrentRevenue"]);
			AssertEquals("The AmountCurrentCost of row 2", 0m, result.Rows[1]["AmountCurrentCost"]);
		}
