...
namespace Enterprise.Build.Database.Script.Testing.Public.BusinessIntelligence.EDW.Model.InternationalLogistics
{
	...
	internal class usp_IncLoad_CUS__JobDocAddressTest : BiCreateScriptTest
	{
		[ExpectNoExceptions]
		public void TestRun()
		{
...
",
		...
		string GetIncLoadSQLText()
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT [IncrementalLoadQuery] FROM [{0}].[biAdmin].[CustomTableConfiguration] WHERE  [ModelSchemaName] = 'InternationalLogistics' AND [ModelTableName] = 'CUS__JobDocAddress'",
					ScriptDbName
			);
