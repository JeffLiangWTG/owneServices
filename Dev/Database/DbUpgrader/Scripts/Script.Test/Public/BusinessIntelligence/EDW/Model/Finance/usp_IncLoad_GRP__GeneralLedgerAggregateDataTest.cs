...
namespace Enterprise.Build.Database.Script.Public.BusinessIntelligence.EDW.Model.Finance.Testing
{
	...
	internal class usp_IncLoad_GRP__GeneralLedgerAggregateDataTest : BiCreateScriptTest
	{
		[ExpectNoExceptions]
		public void TestRun()
		{
			...
			PrepareTestDateForDeleteAndAddNewData();
			ExecuteInc();
			...
			CombineAssertions(() =>
			{
				AssertRowValues(resultTable, 202308, 3, 1, 1, 1, "", 1, "AUD", 300.00, 600.00, 300.00, 300.00, 600.00, 300.00);
				AssertRowValues(resultTable, 202308, 2, 1, 1, 1, "TS3", 1, "AUD", 700.00, 1400.00, 700.00, 700.00, 1400.00, 700.00);
			});
		}
		...
		void PrepareTestDateForDeleteAndAddNewData()
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture, @"
				DELETE [{0}].[biadmin].[TransformedRow]
				DELETE [{0}].[Finance].[BAS__GeneralLedgerData] WHERE GeneralLedgerDataKey in (1, 2)
				...
				INSERT [{0}].[Finance].[BAS__GeneralLedgerData]
					...
					[Type], [GLTransactionHeaderKey], [TaxBranchKey], [TaxGLMovementKey])
					VALUES
						(1, 1, 1, 1, newid(), 5, 3, 'AUD', 1, 'ARC', 100.00, 200.00, 100.00, 100.00, 100.00, 200.00, '2023-08-10',202308, '2023-08-21', '~BP', '2023-08-21', '~BP', 'PST', 1, 1, 1),
						(1, 1, 1, 1, newid(), 6, 3, 'AUD', 1, 'ARC', 200.00, 400.00, 200.00, 200.00, 200.00, 400.00, '2023-08-10',202308, '2023-08-21', '~BP', '2023-08-21', '~BP', 'PST', 2, 1, 1)

				INSERT INTO [{0}].[biadmin].[TransformedRow] (SchemaName, TableName, KeyValue, RefValue1, RefValue2, RefValue3, RefValue4, RefValue5)
				VALUES
					('Finance', 'BAS__GeneralLedgerData', 1, 1, 202308, 1, 1, 1),
					('Finance', 'BAS__GeneralLedgerData', 2, 1, 202308, 1, 1, 1),
					('Finance', 'BAS__GeneralLedgerData', 5, null, null, null, null, null),
					('Finance', 'BAS__GeneralLedgerData', 6, null, null, null, null, null)
			",
		...
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT [IncrementalLoadQuery] FROM [{0}].[biAdmin].[CustomTableConfiguration] WHERE  [ModelSchemaName] = 'Finance' AND [ModelTableName] = 'GRP__GeneralLedgerAggregateData'",
					ScriptDbName
			);
