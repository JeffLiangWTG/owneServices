...
{
	[TestedType(typeof(vw_AGG__JobDeclartionExtended))]
	internal class vw_AGG__JobDeclartionExtendedTest : BiCreateScriptTest
	{
		/// <summary>
	 ...
	 string sendingForwarderCode, int? sendingForwarderKey, Guid? sendingForwarderPK, string shippingLineCode, int? shippingLineKey, Guid? shippingLinePK)
		...
		DataTable SelectRows()
		{
			string sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT * FROM [{0}].[InternationalLogistics].[vw_AGG__JobDeclartionExtended]",
					ScriptDbName
			);
