...
namespace Enterprise.Build.Database.Script.Public.BusinessIntelligence.EDW.Model.Organization.Testing
{
	...
	class usp_IniLoad_GRP__OrgRelatedPartyTest : BiCreateScriptTest
	{
		[ExpectNoExceptions]
		public void TestRun()
		{
		...
		{
			var sqlText = string.Format(CultureInfo.InvariantCulture,
					"SELECT [InitialLoadQuery] FROM [{0}].[biadmin].[CustomTableConfiguration] WHERE [ModelSchemaName] = 'Organization' AND [ModelTableName] = 'GRP__OrgRelatedParty'",
					ScriptDbName
			);
