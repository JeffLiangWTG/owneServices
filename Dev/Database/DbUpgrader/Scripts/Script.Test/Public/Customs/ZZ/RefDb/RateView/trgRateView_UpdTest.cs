...
{
	[TestedType(typeof(trgRateView_Upd))]
	class trgRateView_Upd_Test : DbCreateScriptTest
	{
		public void TestRateView_Upd()
		{
			var createRecordsSql = @"
				...
				VALUES (NEWID(), @tariffPK1, @preferencePK, 'ZA', 'ZZRate')
				...
				VALUES (NEWID(), @tariffPK2, @rateCodePK, 'CusRate', '2020-01-01 00:00:00.000', GetUtcDate(), '~BP', GetUtcDate(), '~BP')
			";

			var connection = Db.Connection;
			connection.ExecuteNonQuery(createRecordsSql);

			var updateSql = "UPDATE dbo.RateView SET ZZ2_DataSet = 'Z' WHERE ZZ2_RateFormula = 'CusRate'";
			var exceptionSql = AssertExceptionThrown<SqlException>(() => connection.ExecuteNonQuery(updateSql));
			AssertContains("Cannot update the system-defined Rate record with ZZ2_DataSet = 'Z'.", exceptionSql.Message);

			updateSql = "UPDATE dbo.RateView SET ZZ2_RateFormula = 'Changed ZZ RateFormula' WHERE ZZ2_RateFormula = 'ZZRate'";
			exceptionSql = AssertExceptionThrown<SqlException>(() => connection.ExecuteNonQuery(updateSql));
			AssertContains("Cannot update the system-defined Rate record with ZZ2_DataSet = 'Z'.", exceptionSql.Message);

			updateSql = "UPDATE dbo.RateView SET ZZ2_DataSet = 'O' WHERE ZZ2_RateFormula = 'ZZRate'";
			exceptionSql = AssertExceptionThrown<SqlException>(() => connection.ExecuteNonQuery(updateSql));
			AssertContains("Cannot update the system-defined Rate record with ZZ2_DataSet = 'Z'.", exceptionSql.Message);

			updateSql = "UPDATE dbo.RateView SET ZZ2_DataSet = 'O', ZZ2_RateFormula = 'CusRate2', ZZ2_SystemLastEditTimeUtc='2020-07-03', ZZ2_SystemLastEditUser='NEW' WHERE ZZ2_RateFormula = 'CusRate'";
			AssertNoExceptionThrown(() =>
			{
				AssertEquals(1, connection.ExecuteNonQuery(updateSql));
			});

			var selectSql = "SELECT * FROM dbo.RateView WHERE ZZ2_RateFormula = 'CusRate2'";
			var results = new List<string>();
			connection.ExecuteReader(selectSql, reader =>
			{
				results.Add($"{reader["ZZ2_IsSystem"]}, {reader["ZZ2_DataSet"]}, {reader["ZZ2_SystemLastEditTimeUtc"]}, {reader["ZZ2_SystemLastEditUser"]}");
			});
