...
{
	[TestedType(typeof(CargoWise.DbUpgrader.Scripts.Definitions.Customs.ZZ.RefDb.CusRefTariffLanguageView.CusRefTariffLanguageView))]
	class CusRefTariffLanguageViewTest : DbCreateScriptTest
	{
		public void TestCusRefTariffLanguageView()
		{
			var createRecordsSql = @"
				...
				VALUES (NEWID(), 'FRN', @tariffPK2, 'French', '2020-07-01', '~E1', '2021-07-01', '~F1')
			";

			var connection = Db.Connection;
			connection.ExecuteNonQuery(createRecordsSql);
			AssertSelectResult("ZZ RefCusTariffLanguage", connection, "EN", new[] { "True, Z, English, , ~BP, , ~BP" });
			AssertSelectResult("Cus CusRefTariffLanguage", connection, "FRN", new[] { "False, O, French, 1/07/2020 12:00:00 AM, ~E1, 1/07/2021 12:00:00 AM, ~F1" });
		}

		public void TestViewColumnsMatchUnderlyingTable()
		{
			TestDbViewHelper.AssertViewColumnsMatchUnderlyingTable(Db.Connection, "CusRefTariffLanguageView", "RefDatabase_RefCusTariffLanguage", new[] {
				new TestDbViewHelper.DbColumn("ZX7_IsSystem", "bit", -1),
				new TestDbViewHelper.DbColumn("ZX7_DataSet", "varchar", 1),
				new TestDbViewHelper.DbColumn("ZX7_SystemCreateTimeUtc", "smalldatetime", -1),
				new TestDbViewHelper.DbColumn("ZX7_SystemCreateUser", "varchar", 3),
				new TestDbViewHelper.DbColumn("ZX7_SystemLastEditTimeUtc", "smalldatetime", -1),
				new TestDbViewHelper.DbColumn("ZX7_SystemLastEditUser", "varchar", 3),
			});
		}

		protected override bool RequiresSchemaBinding => false;

		void AssertSelectResult(string message, DbConnection connection, string language, string[] expectedResults)
		{
			var selectSql = $"SELECT * FROM dbo.CusRefTariffLanguageView WHERE ZX7_ZX6_NKLanguage = '{language}'";
			var results = new List<string>();
			connection.ExecuteReader(selectSql, reader =>
			{
				results.Add($@"{reader["ZX7_IsSystem"]}, {reader["ZX7_DataSet"]}, {reader["ZX7_Description"]}, {reader["ZX7_SystemCreateTimeUtc"]}, {reader["ZX7_SystemCreateUser"]}, {reader["ZX7_SystemLastEditTimeUtc"]}, {reader["ZX7_SystemLastEditUser"]}");
			});
