...
{
	[TestedType(typeof(trgCusRefApplicabilityView_Del))]
	class trgCusRefApplicabilityView_Del_Test : DbCreateScriptTest
	{
...
VALUES ('CEB001F0-2BB6-4A27-AFB8-88237E7519BE','4C9D1634-332A-4296-B22C-5C275481B8DB','1900-01-01','2079-06-06','ADD','2')"
			...
			var delExp = AssertExceptionThrown<SqlException>(() => connection.ExecuteNonQuery("DELETE FROM dbo.CusRefApplicabilityView WHERE ZZT_PK='CEB001F0-2BB6-4A27-AFB8-88237E7519BE'"));
			AssertEquals("Should have thrown a correct exception with correct message", "Cannot deleted a system-defined Applicability.", delExp.Message);

			AssertNoExceptionThrown("Should be able to delete item from CusDB.", () => connection.ExecuteNonQuery("DELETE FROM dbo.CusRefApplicabilityView WHERE ZZT_PK='C9B3A235-E397-486B-9B18-DD705176D1B0'"));
			connection.ExecuteReader("SELECT ZZT_PK FROM dbo.CusRefApplicabilityView",
			reader =>
			{
				AssertEquals("Should only be RefDb item left.", "CEB001F0-2BB6-4A27-AFB8-88237E7519BE", reader["ZZT_PK"].ToString().ToUpper());
			});
		}
	}
}
