...
{
	[TestedType(typeof(trgCusRefRateUOMView_Del))]
	class trgCusRefRateUOMView_Del_Test : DbCreateScriptTest
	{
		public void TestCusRefRateUOMView_Del()
		{
			var rateUomPK2 = Guid.NewGuid();

			var createRecordsSql = @"
				DECLARE @ratePK UNIQUEIDENTIFIER = NEWID()
				...
				VALUES (@ratePK, @tariffPK2, @rateCodePK, 'CusRate', '2020-01-01 00:00:00.000', GetUtcDate(), '~BP', GetUtcDate(), '~BP')
				...
				VALUES (NEWID(), @ratePK, 'CusUOM', 0, GetUtcDate(), '~BP', GetUtcDate(), '~BP')
			";

			var connection = Db.Connection;
			connection.ExecuteNonQuery(createRecordsSql);

			var deleteSql = "DELETE dbo.CusRefRateUOMView WHERE ZXG_DataSet = 'Z'";
			var exceptionSql = AssertExceptionThrown<SqlException>(() => connection.ExecuteNonQuery(deleteSql));
			AssertContains("Cannot delete the system-defined Rate UOM record with ZXG_DataSet = 'Z'.", exceptionSql.Message);

			deleteSql = "DELETE dbo.CusRefRateUOMView WHERE ZXG_DataSet = 'O'";
