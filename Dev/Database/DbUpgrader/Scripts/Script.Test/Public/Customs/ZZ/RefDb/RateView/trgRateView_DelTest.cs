...
{
	[TestedType(typeof(trgRateView_Del))]
	class trgRateView_Del_Test : DbCreateScriptTest
	{
		public void TestRateView_Del()
		{
			...
			var deleteSql = "DELETE dbo.RateView WHERE ZZ2_DataSet = 'Z'";
			var exceptionSql = AssertExceptionThrown<SqlException>(() => connection.ExecuteNonQuery(deleteSql));
			AssertContains("Cannot delete the system-defined Rate record with ZZ2_DataSet = 'Z'.", exceptionSql.Message);

			deleteSql = "DELETE dbo.RateView WHERE ZZ2_DataSet = 'O'";
