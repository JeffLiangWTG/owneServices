...
{
	[TestedType(typeof(USExportInvoiceLines))]
	class USExportInvoiceLinesTest : DbCreateScriptTest
	{
		class Address
		{
		...
		public void TestOrgCusCode()
		{
			...
			var fPPIOrgPK = TestDataCreator.CreateOrganisation("ORG6", "Company Name 6");
			...
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK1, 1, dataModel: "US");
			...
			var reportSql = $"SELECT UltimateConsigneeIDNumber, IntermConsigneeIDNumber, MainSupplierIDNumber, ForwarderIDNumber, USPPIIDNumber, fPPIIDNumber FROM USExportInvoiceLines('{companyPK}', '', '', '', '', '', '', '', '', 'N', '', '', '', '', '', '')";
		...
		public void TestSupplierPickupAddress()
		{
			...
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK2, 2, dataModel: "US");
			...
			var invoiceHeaderPK3 = TestDataCreator.CreateJobComInvoiceHeader(declarationPK3, false, 3, dataModel: "US");
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK3, 3, dataModel: "US");
			...
			var invoiceHeaderPK4 = TestDataCreator.CreateJobComInvoiceHeader(declarationPK4, false, 4, dataModel: "US");
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK4, 4, dataModel: "US");
			TestDataCreator.CreateDocAddress(addressPK1, "", declarationPK4, "JE", "SUG");

			var sql = @"select JE_DeclarationReference, USPPIPK, USPPICode, USPPIFullName from dbo.USExportInvoiceLines(@companyPK, '', '', '', '', '', '', '', '', 'N','', '', '', '', '', '')";
		...
		public void TestLicenseNo()
		{
			...
			var declarationPK = TestDataCreator.CreateJobDeclaration(branchPK, companyPK, "B0000001", "EXP", 1, null, dataModel: "US");
			var invoicePK = TestDataCreator.CreateJobComInvoiceHeader(declarationPK, false, 1, invoiceNumber: "INV000001", dataModel: "US");
			TestDataCreator.CreateJobComInvoiceLine(invoicePK, 1, addInfo: "LicenseValue=7.65*LicenseNo=CIV", dataModel: "US");

			var declarationPK2 = TestDataCreator.CreateJobDeclaration(branchPK, companyPK, "B0000002", "EXP", 2, null, dataModel: "US");
			var invoicePK2 = TestDataCreator.CreateJobComInvoiceHeader(declarationPK2, false, 2, invoiceNumber: "INV000001", dataModel: "US", addInfo: "LicenseNo=INV");
			TestDataCreator.CreateJobComInvoiceLine(invoicePK2, 2, addInfo: "LicenseValue=7.65", dataModel: "US");

			var declarationPK3 = TestDataCreator.CreateJobDeclaration(branchPK, companyPK, "B0000003", "EXP", 3, null, dataModel: "US", addInfo: "LicenseNo=DEC");
			var invoicePK3 = TestDataCreator.CreateJobComInvoiceHeader(declarationPK3, false, 3, invoiceNumber: "INV000001", dataModel: "US");
			TestDataCreator.CreateJobComInvoiceLine(invoicePK3, 3, addInfo: "LicenseValue=7.65", dataModel: "US");

			var result = new Dictionary<string, string>();
			var script = $@"SELECT JE_DeclarationReference, US_LicenseNo FROM USExportInvoiceLines('{companyPK}','','','','', '', '', '', '', '', '', '', '', '', '', '')";
			using (var command = TestConnection.Command(script))
			using (var reader = command.ExecuteReader())
			{
				while (reader.Read())
				{
					var decNo = reader.GetString(0);
					var licenseNo = reader.GetString(1);
					result.Add(decNo, licenseNo);
				}
			}

			CombineAssertions(() =>
			{
				AssertEquals("US_LicenseNo from invoice line level", "CIV", result["B0000001"]);
				AssertEquals("US_LicenseNo from invoice level", "INV", result["B0000002"]);
				AssertEquals("US_LicenseNo from declaration level", "DEC", result["B0000003"]);
			});
		}

		public void TestLineNoNoDefault()
		{
			var companyPK = TestDataCreator.CreateCompany("TC1", "US", "USD");
			var branchPK = TestDataCreator.CreateBranch(companyPK, "TB1", "USCHI");
			...
			var addressPK1 = TestDataCreator.CreateAddress(organisationPK1, "Delivery Address", "Address 1");
			var shipmentPK1 = TestDataCreator.CreateShipment("S001");
			var declarationPK1 = TestDataCreator.CreateJobDeclaration(branchPK, companyPK, "S001", "EXP", 1, shipmentPK1, organisationPK1, addInfo: "LicenseNo=NLR*LicenseType=C33", dataModel: "US");
			var invoiceHeaderPK1 = TestDataCreator.CreateJobComInvoiceHeader(declarationPK1, false, 1, addInfo: "LicenseNo=LVS*LicenseType=C35", dataModel: "US");
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK1, 1, addInfo: "LicenseNo=LV7*LicenseType=C37", dataModel: "US");
			TestDataCreator.CreateDocAddress(addressPK1, "", shipmentPK1, "JS", "CRG");

			var organisationPK2 = TestDataCreator.CreateOrganisation("OrgCode2", "Company Name 2");
			var addressPK2 = TestDataCreator.CreateAddress(organisationPK2, "Delivery Address", "Address 1");
			var declarationPK2 = TestDataCreator.CreateJobDeclaration(branchPK, companyPK, "B002", "EXP", 2, null, organisationPK2, addInfo: "LicenseNo=NNN*LicenseType=C88", dataModel: "US");
			var invoiceHeaderPK2 = TestDataCreator.CreateJobComInvoiceHeader(declarationPK2, false, 2, addInfo: "LicenseNo=LL5*LicenseType=C55", dataModel: "US");
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK2, 2, dataModel: "US");
			TestDataCreator.CreateDocAddress(addressPK2, "", declarationPK2, "JE", "SUG");

			var result = new Dictionary<string, Tuple<string, string>>();
			var sql = @"select JE_DeclarationReference, US_LicenseNo, US_LicenseType from dbo.USExportInvoiceLines(@companyPK, '', '', '', '', '', '', '', '', 'N','', '', '', '', '', '')";
			using (var command = Db.Connection.Command(sql))
			{
				command.AddParameter("@companyPK", SqlDbType.UniqueIdentifier, companyPK);
				using (var reader = command.ExecuteReader())
				{
					while (reader.Read())
					{
						var decNo = reader.GetString(0);
						var licenseNo = reader.GetString(1);
						var licenseType = reader.GetString(2);
						result.Add(decNo, new Tuple<string, string>(licenseNo, licenseType));
					}
				}
			}
			CombineAssertions(() =>
			{
				AssertEquals("LV7", result["S001"].Item1);
				AssertEquals("C37", result["S001"].Item2);
				AssertEquals("LL5", result["B002"].Item1);
				AssertEquals("C55", result["B002"].Item2);
			});
		}

		public void TestProductCodeStartWith()
		{
			var companyPK = TestDataCreator.CreateCompany("TC1", "US", "USD");
			var branchPK = TestDataCreator.CreateBranch(companyPK, "TB1", "USCHI");

			var shipmentPK1 = TestDataCreator.CreateShipment("S001");
			var declarationPK1 = TestDataCreator.CreateJobDeclaration(branchPK, companyPK, "S001", "EXP", 1, shipmentPK1, dataModel: "US");
			var invoiceHeaderPK1 = TestDataCreator.CreateJobComInvoiceHeader(declarationPK1, false, 1, dataModel: "US");
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK1, 1, dataModel: "US", partNo: "112233");

			var declarationPK2 = TestDataCreator.CreateJobDeclaration(branchPK, companyPK, "B002", "EXP", 2, dataModel: "US");
			var invoiceHeaderPK2 = TestDataCreator.CreateJobComInvoiceHeader(declarationPK2, false, 2, dataModel: "US");
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK2, 2, dataModel: "US", partNo: "223344");
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK2, 2, dataModel: "US", partNo: "334455");
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK2, 2, dataModel: "US", partNo: "445566");
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK2, 2, dataModel: "US", partNo: "556677");
			TestDataCreator.CreateJobComInvoiceLine(invoiceHeaderPK2, 2, dataModel: "US", partNo: "667788");

			var result1 = new Dictionary<string, string>();
			var sql1 = @"select JE_DeclarationReference, JI_PartNo from dbo.USExportInvoiceLines(@companyPK, '', '', '', '', '', '', '', '', 'N', '', @ProductCodeStartsWith1, '', '', '', '')";
			using (var command = Db.Connection.Command(sql1))
			{
				command.AddParameter("@companyPK", SqlDbType.UniqueIdentifier, companyPK);
				command.AddParameter("@ProductCodeStartsWith1", SqlDbType.VarChar, "11");
				...
				using (var reader = command.ExecuteReader())
				{
					...
					{
						var decNo = reader.GetString(0);
						var partNo = reader.GetString(1);
						result1.Add(decNo, partNo);
					}
				}
			}
			AssertEquals("partNo", "112233", result1["S001"]);

			var result2 = new List<String>();
			var sql2 = @"select JI_PartNo from dbo.USExportInvoiceLines(@companyPK, '', '', '', '', '', '', '', '', 'N', '', @ProductCodeStartsWith1, @ProductCodeStartsWith2, @ProductCodeStartsWith3, @ProductCodeStartsWith4, @ProductCodeStartsWith5)";
			using (var command = Db.Connection.Command(sql2))
			{
				command.AddParameter("@companyPK", SqlDbType.UniqueIdentifier, companyPK);
				command.AddParameter("@ProductCodeStartsWith1", SqlDbType.VarChar, "11");
				command.AddParameter("@ProductCodeStartsWith2", SqlDbType.VarChar, "22");
				command.AddParameter("@ProductCodeStartsWith3", SqlDbType.VarChar, "33");
				command.AddParameter("@ProductCodeStartsWith4", SqlDbType.VarChar, "44");
				command.AddParameter("@ProductCodeStartsWith5", SqlDbType.VarChar, "55");

				using (var reader = command.ExecuteReader())
				{
					while (reader.Read())
					{
						result2.Add(reader.GetString(0));
					}
				}
			}
			AssertContainsExactElementsInAnyOrder(new[] { "112233", "223344", "334455", "445566", "556677" }, result2);
		}
		...
		public void TestUSPPIUltimateConsigneeIntermConsigneeAddress()
		{
			...
			var sql = @"select JE_DeclarationReference, USPPIPK, USPPICode, USPPIFullName, USPPIAddress1, USPPIAddress2, USPPICity, USPPIState, USPPIPostCode, USPPIContact, USPPIPhone, USPPIIDNumber from dbo.USExportInvoiceLines(@companyPK, '', '', '', '', '', '', '', '', 'N','', '', '', '', '', '')";
		...
		public void TestMainSupplierIDNumber()
		{
			...
			var invoicePK = TestDataCreator.CreateJobComInvoiceHeader(declarationPK, false, 1, invoiceNumber: "INV000001", dataModel: "US");
			TestDataCreator.CreateJobComInvoiceLine(invoicePK, 1, addInfo: "MarksAndNumbers=1234", dataModel: "US");

			var script = $@"SELECT * FROM USExportInvoiceLines('{companyPK}','','','','', '', '', '', '', '', '', '', '', '', '', '')";
			using (var command = TestConnection.Command(script))
			using (var reader = command.ExecuteReader())
			{
				reader.Read();
				CombineAssertions(() =>
				{
					AssertEquals("JE_DeclarationReference", "B0000001", reader["JE_DeclarationReference"].ToString());
					AssertEquals("JZ_InvoiceNumber", "INV000001", reader["JZ_InvoiceNumber"].ToString());
					AssertEquals("US_MarksAndNumbers", "1234", reader["US_MarksAndNumbers"].ToString());
					AssertEquals("MainSupplierIDNumber", "DUN: 758516744", reader["MainSupplierIDNumber"].ToString());
				});
			}
		}

		public void TestUS_DDTCQuantityAndUS_LicenseValue()
		{
			var companyPK = TestDataCreator.CreateCompany("DUS", "US", "USD");
			var branchPK = TestDataCreator.CreateBranch(companyPK, "PHL", "USPHL");
			var declarationPK = TestDataCreator.CreateJobDeclaration(branchPK, companyPK, "B0000001", "EXP", 1, null, dataModel: "US");
			var invoicePK = TestDataCreator.CreateJobComInvoiceHeader(declarationPK, false, 1, invoiceNumber: "INV000001", dataModel: "US");
			TestDataCreator.CreateJobComInvoiceLine(invoicePK, 1, addInfo: "LicenseValue=7.65*DDTCQuantity=10", dataModel: "US");
			...
			using (var reader = command.ExecuteReader())
			{
				...
				CombineAssertions(() =>
				{
					AssertEquals("JE_DeclarationReference", "B0000001", reader["JE_DeclarationReference"].ToString());
					AssertEquals("JZ_InvoiceNumber", "INV000001", reader["JZ_InvoiceNumber"].ToString());
					AssertEquals("US_LicenseValue", "7.65", reader["US_LicenseValue"].ToString());
					AssertEquals("US_DDTCQuantity", "10", reader["US_DDTCQuantity"].ToString());
				});
			}
		}
