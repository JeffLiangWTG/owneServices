...
{
	[TestedType(typeof(USImportInvoiceLinePGALaceyAct))]
	class USImportInvoiceLinePGALaceyActTest : DbCreateScriptTest
	{
		public void TestGetCorrectData()
		{
			...
			var laceyID1 = Guid.NewGuid();
			var laceyID2 = Guid.NewGuid();
			...
			var cusAddInfoSql = @"
				...
				VALUES(@laceyID1, 'PGA', 'PGACommercialDescription=123456789*InvCurrPGAValue=22334455*NameOfConstituentElement=nameBlah*ProgramCode=PC1*ProcessingCode=ProC1*ProductCode=BLAH*RefusedCountry=CN*Qty1=11*Qty2=22*Qty3=33*Qty4=44*Qty5=55*UQ1=A*UQ2=B*UQ3=C*UQ4=D*UQ5=E*UQ6=F', 'JI', @invoiceLinePK1, 1)
				INSERT INTO dbo.CusAddInfo (B7_PK, B7_Type, B7_AddInfoData, B7_ParentTableCode, B7_ParentID, B7_IsValid)
				VALUES(@laceyID2, 'ATF', '', 'JI', @invoiceLinePK2, 1)
				INSERT INTO dbo.CusAddInfo (B7_PK, B7_Type, B7_AddInfoData, B7_ParentTableCode, B7_ParentID, B7_IsValid)
				VALUES(NEWID(), 'USP', 'PGANameOfTheConstituentElement=LACEYACTName*PGACommercialDescription=123456789*InvCurrPGAValue=22334455*NameOfConstituentElement=nameBlah', 'B7', @laceyID1, 1)
				INSERT INTO dbo.CusAddInfo (B7_PK, B7_Type, B7_AddInfoData, B7_ParentTableCode, B7_ParentID, B7_IsValid)
				VALUES(NEWID(), 'USP', 'PGACommercialDescription=123456789*InvCurrPGAValue=22334455*NameOfConstituentElement=nameBlah', 'B7', @laceyID2, 1)";
			using (var command = Db.Connection.Command(cusAddInfoSql))
			{
				...
				command.AddParameter("@laceyID1", SqlDbType.UniqueIdentifier, laceyID1);
				command.AddParameter("@laceyID2", SqlDbType.UniqueIdentifier, laceyID2);
				command.ExecuteNonQuery();
			}
			...
			var reportSql = @"SELECT * FROM USImportInvoiceLinePGALaceyAct(@companyPK, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)";
			using (var command = Db.Connection.Command(reportSql))
			{
				...
				using (var reader = command.ExecuteReader())
				{
					...
					CombineAssertions(() =>
					{
						...
						AssertEquals("LACYCommercialDescription", "123456789", reader["LACYCommercialDescription"].ToString());
						AssertEquals("LACYInvoiceCurrencyValue", "22334455", reader["LACYInvoiceCurrencyValue"].ToString());
						AssertEquals("LACYNameOfConstituentElement", "nameBlah", reader["LACYNameOfConstituentElement"].ToString());
						AssertEquals("RowRole", "HEADER", reader["RowRole"].ToString());
						AssertEquals("LACYElemName", "LACEYACTName", reader["LACYElemName"].ToString());
