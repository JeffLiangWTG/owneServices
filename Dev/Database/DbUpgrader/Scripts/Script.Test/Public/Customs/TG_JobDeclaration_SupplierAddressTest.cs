...
{
	[TestedType(typeof(TG_JobDeclaration_SupplierAddress))]
	class TG_JobDeclaration_SupplierAddressTest : DbCreateScriptTest
	{
		const string TriggerMessage = "JE_OA_SupplierAddress should match JE_OH_Supplier: ";
		...
		{
			TestConnection.ExecuteNonQuery($@"
				insert into dbo.JobDeclaration(JE_PK, JE_DataModel, JE_DeclarationReference, JE_OH_Supplier, JE_OA_SupplierAddress, JE_GB, JE_GC, JE_ClusterKey, JE_SystemCreateTimeUtc, JE_SystemCreateUser, JE_SystemLastEditTimeUtc, JE_SystemLastEditUser)
				values
		...
		public void TestUpdate_Supplier()
		{
			...
			AssertContains(TriggerMessage, AssertExceptionThrown<SqlException>(() => TestConnection.ExecuteNonQuery(
				$"update dbo.JobDeclaration set JE_OH_Supplier = '{org2}', JE_SystemLastEditTimeUtc = GetUtcDate(), JE_SystemLastEditUser = '~BP' where JE_PK = '{decPK}'"
			)).Message);
		}
		...
		public void TestUpdate_SupplierAddress()
		{
			...
			AssertContains(TriggerMessage, AssertExceptionThrown<SqlException>(() => TestConnection.ExecuteNonQuery(
				$"update dbo.JobDeclaration set JE_OA_SupplierAddress = '{addr2}', JE_SystemLastEditTimeUtc = GetUtcDate(), JE_SystemLastEditUser = '~BP' where JE_PK = '{decPK}'"
			)).Message);
		}
		...
		public void TestUpdateExistingInvalid()
		{
			...
			TestConnection.ExecuteNonQuery($@"
				DISABLE TRIGGER TG_JobDeclaration_SupplierAddress ON JobDeclaration;
				insert into dbo.JobDeclaration(JE_PK, JE_DataModel, JE_DeclarationReference, JE_OH_Supplier, JE_OA_SupplierAddress, JE_GB, JE_GC, JE_ClusterKey, JE_SystemCreateTimeUtc, JE_SystemCreateUser, JE_SystemLastEditTimeUtc, JE_SystemLastEditUser)
				values
				...
				;
				ENABLE TRIGGER TG_JobDeclaration_SupplierAddress ON JobDeclaration;
			");

			TestConnection.ExecuteNonQuery(@$"
UPDATE dbo.JobDeclaration
SET
	JE_OA_SupplierAddress = '{addr1}',
