...
{
	[TestedType(typeof(AccCFXUpliftConfigurationView))]
	class AccCFXUpliftConfigurationViewTest : DbCreateScriptTest
	{
		public void TestQueriesWithoutGCDoNotDoTableScan()
		{
			...
			for (int i = 0; i < 1000; i++)
			{
				...
				helper.InsertAccJobConfig("CFX", parentTableCode: "OH", parentId: orgPK, ledger: "AR");
			}
			...
			{
				var totalRows = 0;
				TestConnection.ExecuteReader($"SELECT * FROM dbo.AccCFXUpliftConfigurationView WHERE JCF_ParentId = '{orgPK}' AND JCF_ParentTableCode = 'OH'",
					_ => totalRows++);
				AssertEquals(1, totalRows);
				var queryPlan = TestConnection.ExecutedCommandsAndQueryPlans.First(t => t.Item1.Contains("AccCFXUpliftConfigurationView"));
