...
{
	[TestedType(typeof(CreatePeriodCompanyPartitionKeys))]
	class CreatePeriodCompanyPartitionKeysTest : DbCreateScriptTest
	{
		public void TestSampleCall()
		{
			...
			if (dt != null)
			{
				...
				{
					actualPartitionKeys.Add(Convert.ToString(row[0], CultureInfo.InvariantCulture).ToUpperInvariant());
				}
			}
		...
		void CreatePartitionData(Guid companyPK)
		{
			...
			for (int year = currentYear; year <= currentYear + 11; year++)
			{
				for (int month = 1; month <= 12; month++)
				{
					...
					AddAccountingPeriod(dateTime, dateTime.AddDays(DateTime.DaysInMonth(dateTime.Year, dateTime.Month)), year * 100 + month, year, companyPK);
					partitionKeys.Add(string.Format("{0}{1}", year * 100 + month, companyPK).ToUpperInvariant());
				}
			}
		}
