...
namespace Enterprise.Build.Database.Script.Public.Accounting
{
	...
	class ProfitAndLossRetainedEarningsTest : DbCreateScriptTest
	{
		public void TestTransactionCategoryGroup()
		{
			...
			var sqlText = $"SELECT * FROM ProfitAndLossRetainedEarnings(202002,'{TestDbHelper.DefaultCompanyPK}','{branchCode}','{"{0}"}')";
			...
			AssertEquals("singe category", 100m, result.Rows[0]["YearToPeriod"]);

			result = DataUtils.GetDataTableFromQuery(TestConnection, string.Format(sqlText, categotyGroups));
			AssertEquals("Result should have one row", 1, result.Rows.Count);
			AssertEquals("maximum group is 25", 2500m, result.Rows[0]["YearToPeriod"]);
		}
	}
}
