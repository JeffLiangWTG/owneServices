...
{
	[TestedType(typeof(AccountingTransactionExportGetTaxBranchAddressesSingle))]
	class AccountingTransactionExportGetTaxBranchAddressesSingleTest : DbCreateScriptTest
	{
		public void TestResultColumnsAreIdenticalToBatchVersion()
		{
			var singleResult = DataUtils.GetDataTableFromQuery(TestConnection, $"EXEC AccountingTransactionExportGetTaxBranchAddressesSingle '{Guid.NewGuid()}'");
			var batchResult = DataUtils.GetDataTableFromQuery(TestConnection, $"EXEC AccountingTransactionExportGetTaxBranchAddresses 'EDI', -1, NULL");
		...
		public void TestCoreQueryLogicIsSameAsBatchVersion()
		{
			...
			var batchScriptCoreLinesToCompare = BatchAndSingleTestHelper.GetScriptLinesToCompare(new AccountingTransactionExportGetTaxBranchAddresses().Text, excludeLinesForBatch);
