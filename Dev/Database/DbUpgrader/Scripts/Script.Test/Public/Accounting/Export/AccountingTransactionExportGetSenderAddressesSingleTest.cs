...
{
	[TestedType(typeof(AccountingTransactionExportGetSenderAddressesSingle))]
	class AccountingTransactionExportGetSenderAddressesSingleTest : DbCreateScriptTest
	{
		public void TestResultColumnsAreIdenticalToBatchVersion()
		{
			var singleResult = DataUtils.GetDataTableFromQuery(TestConnection, $"EXEC AccountingTransactionExportGetSenderAddressesSingle '{Guid.NewGuid()}'");
			var batchResult = DataUtils.GetDataTableFromQuery(TestConnection, $"EXEC AccountingTransactionExportGetSenderAddresses 'EDI', -1, NULL");
		...
		public void TestCoreQueryLogicIsSameAsBatchVersion()
		{
			...
			var batchScriptCoreLinesToCompare = BatchAndSingleTestHelper.GetScriptLinesToCompare(new AccountingTransactionExportGetSenderAddresses().Text, excludeLinesForBatch);
