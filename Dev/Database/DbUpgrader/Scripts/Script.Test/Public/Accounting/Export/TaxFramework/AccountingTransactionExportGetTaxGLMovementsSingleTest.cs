...
{
	[TestedType(typeof(AccountingTransactionExportGetTaxGLMovementsSingle))]
	class AccountingTransactionExportGetTaxGLMovementsSingleTest : DbCreateScriptTest
	{
		public void TestResultColumnsAreIdenticalToBatchVersion()
		{
			var singleResult = DataUtils.GetDataTableFromQuery(TestConnection, $"EXEC AccountingTransactionExportGetTaxGLMovementsSingle '{Guid.NewGuid()}'");
			var batchResult = DataUtils.GetDataTableFromQuery(TestConnection, $"EXEC AccountingTransactionExportGetTaxGLMovements 'EDI', -1");
