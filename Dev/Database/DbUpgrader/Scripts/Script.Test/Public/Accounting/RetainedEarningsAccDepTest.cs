...
namespace Enterprise.Build.Database.Script.Public.Accounting
{
	...
	class RetainedEarningsAccDepTest : DbCreateScriptTest
	{
		public void TestTransactionCategoryGroup()
		{
			...
			var sqlText = $"SELECT dbo.RetainedEarningsAccDep(202001,'{branchCode}',202003,'{TestDbHelper.DefaultCompanyPK}','{"{0}"}') as RetainedEarnings";
			...
			AssertEquals("singe category", 100m, result.Rows[0]["RetainedEarnings"]);

			result = DataUtils.GetDataTableFromQuery(TestConnection, string.Format(sqlText, categotyGroups));
			AssertEquals("Result should have one row", 1, result.Rows.Count);
			AssertEquals("maximum group is 25", 2500m, result.Rows[0]["RetainedEarnings"]);
		}
	}
}
