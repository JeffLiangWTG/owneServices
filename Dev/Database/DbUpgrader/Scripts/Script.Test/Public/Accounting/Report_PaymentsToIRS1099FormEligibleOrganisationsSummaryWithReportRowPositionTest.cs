...
{
	[TestedType(typeof(Report_PaymentsToIRS1099FormEligibleOrganisationsSummaryWithReportRowPosition))]
	class Report_PaymentsToIRS1099FormEligibleOrganisationsSummaryWithReportRowPositionTest : DbCreateScriptTest
	{
		const string ReportQuery = "SELECT * FROM Report_PaymentsToIRS1099FormEligibleOrganisationsSummaryWithReportRowPosition('878D7ACA-FFC3-49FC-9710-969CA0C0F2AC', 2005, 'ALL', '', '', 2)";

		string GetReportQueryWithOrgFilter(string onlyIncludeOrgs, string excludeOrgs)
		{
			return $"SELECT * FROM Report_PaymentsToIRS1099FormEligibleOrganisationsSummaryWithReportRowPosition('878D7ACA-FFC3-49FC-9710-969CA0C0F2AC', 2005, 'ALL', '{onlyIncludeOrgs}', '{excludeOrgs}', 2)";
		}
		...
		public void TestOnlyIncludeOrgFilter()
		{
			...
			CombineAssertions(() =>
			{
				...
				AssertEquals("Result should have rows", "KINYIN", ((string)report.Rows[0]["Org_OH_Code"]).Trim());
				AssertEquals("Result should have rows", "AB-C123456", ((string)report.Rows[0]["Org_OK_CustomsRegNoEIN"]).Trim());
				AssertEquals("Result should have rows", "SSNABC123456", ((string)report.Rows[0]["Org_OK_CustomsRegNoSSN"]).Trim());
				AssertEquals("Result should have rows", "LG-N123456", ((string)report.Rows[0]["Org_OK_LoginCompanyEIN"]).Trim());
				AssertEquals("Result should have rows", 1, (int)report.Rows[0].Field<long>("ReportRowPosition"));
			});
		}
		...
		public void TestExcludeOrgFilter()
		{
			...
			CombineAssertions(() =>
			{
				...
				AssertEquals("Result should have rows", "JANSEW", ((string)report.Rows[0]["Org_OH_Code"]).Trim());
				AssertEquals("Result should have rows", "12-3456789", ((string)report.Rows[0]["Org_OK_CustomsRegNoEIN"]).Trim());
				AssertEquals("Result should have rows", "SSN123456789", ((string)report.Rows[0]["Org_OK_CustomsRegNoSSN"]).Trim());
				AssertEquals("Result should have rows", "LG-N123456", ((string)report.Rows[0]["Org_OK_LoginCompanyEIN"]).Trim());
				AssertEquals("Result should have rows", 1, report.Rows[0].Field<long>("ReportRowPosition"));

				AssertEquals("Result should have rows", "PALVAL", ((string)report.Rows[1]["Org_OH_Code"]).Trim());
				AssertEquals("Result should have rows", "AB-CD12345", ((string)report.Rows[1]["Org_OK_CustomsRegNoEIN"]).Trim());
				AssertEquals("Result should have rows", "SSNABCD12345", ((string)report.Rows[1]["Org_OK_CustomsRegNoSSN"]).Trim());
				AssertEquals("Result should have rows", "LG-N123456", ((string)report.Rows[1]["Org_OK_LoginCompanyEIN"]).Trim());
				AssertEquals("Result should have rows", 2, (int)report.Rows[1].Field<long>("ReportRowPosition"));
			});
		}
		...
		public void TestQueryOutputAllOrgs()
		{
			SetUpData();
			...
			CombineAssertions(() =>
			{
				AssertEquals("Result should have rows", 3, report.Rows.Count);
				...
				AssertEquals("Result should have rows", "KINYIN", ((string)report.Rows[1]["Org_OH_Code"]).Trim());
				AssertEquals("Result should have rows", "AB-C123456", ((string)report.Rows[1]["Org_OK_CustomsRegNoEIN"]).Trim());
				AssertEquals("Result should have rows", "SSNABC123456", ((string)report.Rows[1]["Org_OK_CustomsRegNoSSN"]).Trim());
				AssertEquals("Result should have rows", "LG-N123456", ((string)report.Rows[1]["Org_OK_LoginCompanyEIN"]).Trim());
				AssertEquals("Result should have rows", 2, (int)report.Rows[1].Field<long>("ReportRowPosition"));

				AssertEquals("Result should have rows", "PALVAL", ((string)report.Rows[2]["Org_OH_Code"]).Trim());
				AssertEquals("Result should have rows", "AB-CD12345", ((string)report.Rows[2]["Org_OK_CustomsRegNoEIN"]).Trim());
				AssertEquals("Result should have rows", "SSNABCD12345", ((string)report.Rows[2]["Org_OK_CustomsRegNoSSN"]).Trim());
				AssertEquals("Result should have rows", "LG-N123456", ((string)report.Rows[2]["Org_OK_LoginCompanyEIN"]).Trim());
				AssertEquals("Result should have rows", 1, (int)report.Rows[2].Field<long>("ReportRowPosition"));
			});
		}
