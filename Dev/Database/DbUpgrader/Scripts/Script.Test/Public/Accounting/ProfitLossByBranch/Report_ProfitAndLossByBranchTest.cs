...
namespace Enterprise.Build.Database.Script.Public.Accounting.ProfitLossByBranch
{
	...
	class Report_ProfitAndLossByBranchTest : DbCreateScriptTest
	{
		public void TestTransactionCategoryGroup()
		{
			...
			var sqlText = $"SELECT * FROM Report_ProfitAndLossByBranch(202001,'{TestDbHelper.DefaultCompanyPK}','{departmentCode}','{"{0}"}')";
			...
			AssertEquals("singe category", -100m, result.Rows[0]["CurrentPeriodCost"]);

			result = DataUtils.GetDataTableFromQuery(TestConnection, string.Format(sqlText, categotyGroups));
			AssertEquals("Result should have one row", 1, result.Rows.Count);
			AssertEquals("maximum group is 25", -2500m, result.Rows[0]["CurrentPeriodCost"]);
		}
	}
}
