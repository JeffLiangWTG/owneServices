...
{
	[TestedType(typeof(fn_NVPMSummary))]
	class fn_NVPMSummaryTest : DbCreateScriptTest
	{
		public void Testfn_NVPMSummary()
		{
			var insertSql = @"
...
";
			...
			var sqlQuery = "SELECT * FROM fn_NVPMSummary(@CompanyPKs, @StartDate, @EndDate, @NettingCompanyPK, @IsReciprocal)";
			...
			var expectedResult = @"
ParticipantCode NettingType TotalARInvoices TotalAPInvoices GrossValueOfAR                          GrossValueOfAP                          NetValue
--------------- ----------- --------------- --------------- --------------------------------------- --------------------------------------- ---------------------------------------
BNE             NULL        2               2               1500.00                                 1500.00                                 0.00
MEL             NULL        3               3               1500.00                                 1500.00                                 0.00
SYD             NULL        3               3               600.00                                  600.00                                  0.00
";
			var testHelper = new TestDbHelperBase(TestConnection);
			testHelper.AssertTableAsTextFromSQLServerManagenentStudio("fn_NVPMSummary", result, expectedResult, new List<string>() { });
		}
	}
}
