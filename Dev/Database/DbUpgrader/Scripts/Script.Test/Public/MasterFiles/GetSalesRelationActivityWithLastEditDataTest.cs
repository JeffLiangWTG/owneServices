...
{
	[TestedType(typeof(GetSalesRelationActivityWithLastEditDataV2))]
	class GetSalesRelationActivityWithLastEditDataTest : DbCreateScriptTest
	{
		public void TestGeneralUsage()
		{
			InsertTestData();

			var activities = GetAllActivities(null, null);
		...
		public void TestGeneralUsage_WithSpecificRelatedActivityType()
		{
			...
			var activities = GetAllActivities(null, "INQ");
			var activity = activities[Guid.Parse("fb1b3bd0-6bdc-48fc-9ca5-175d94e9c626")];
			AssertEquals("RelatedActivityCount", 1, activity.RelatedActivityCount);
			AssertEquals("RecentActivityDate", new DateTime(2014, 3, 26), activity.RecentActivityDate);
		...
		public void TestGeneralUsage_WithSelectedActivityType()
		{
			...
			var activities = GetAllActivities("COM", "INQ");
...
";
		...
		Dictionary<Guid, Activity> GetAllActivities(string selectedActivityType, string includeRelatedActivityType)
		{
			var selectSql = @"SELECT * FROM GetSalesRelationActivityWithLastEditDataV2(@SelectedActivityType, @IncludeRelatedActivityType, @IncludeRelatedActivityType)";
			using (var command = TestConnection.Command(selectSql))
			{
				command.AddParameter("@SelectedActivityType", SqlDbType.VarChar, (object)selectedActivityType ?? DBNull.Value);
