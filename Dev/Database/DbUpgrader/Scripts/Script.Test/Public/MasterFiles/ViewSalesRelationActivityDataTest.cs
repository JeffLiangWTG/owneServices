...
namespace Enterprise.Build.Database.Script.Public.MasterFiles.Testing
{
	...
	class ViewSalesRelationActivityDataTest : DbCreateScriptTest
	{
		public void TestGeneralUsage()
		{
			...
			CombineAssertions(() =>
			{
				...
				AssertEquals("RelatedActivityCount", 3, activity.RelatedActivityCount);
			});
...
VALUES ('fb1b3bd0-6bdc-48fc-9ca5-175d94e9c626', @CompanyPk, '~name', '2134', '2014-2-21', 'E', '2014-2-21', 'E')
...
";
		...
		Dictionary<Guid, Activity> GetAllActivities()
		{
			var selectSql = @"SELECT * FROM dbo.ViewSalesRelationActivityData";
			using (var command = TestConnection.Command(selectSql))
			{
				using (var reader = command.ExecuteReader())
				{
					...
					while (reader.Read())
					{
						var activityId = (Guid)reader["VSR_PK"];
						var activityRecentActivityDate = reader["VSR_RecentActivityDate"] == DBNull.Value ? null : (DateTime?)reader["VSR_RecentActivityDate"];
						var activityRelatedActivityCount = reader["VSR_RelatedActivityCount"] == DBNull.Value ? null : (int?)reader["VSR_RelatedActivityCount"];

						result.Add(activityId, new Activity(activityId, activityRecentActivityDate, activityRelatedActivityCount));
					}
		...
		class Activity
		{
			public Activity(Guid id, DateTime? recentActivityDate, int? relatedActivityCount)
			{
				ID = id;
