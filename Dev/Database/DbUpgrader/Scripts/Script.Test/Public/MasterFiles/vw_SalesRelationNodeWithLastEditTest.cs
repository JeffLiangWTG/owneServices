...
{
	[TestedType(typeof(vw_SalesRelationNodeWithLastEdit))]
	class vw_SalesRelationNodeWithLastEditTest : DbCreateScriptTest
	{
...
DECLARE @OrgAddressPk UNIQUEIDENTIFIER = '00000001-ff34-40e8-8dfa-790ad5de0e52';
DECLARE @OrgContactPk UNIQUEIDENTIFIER = '00000002-ff34-40e8-8dfa-790ad5de0e52';
...
INSERT INTO dbo.OrgAddress (OA_PK, OA_OH, OA_City, OA_Address1) VALUES (@OrgAddressPk, @OrgPk, 'Sydney', 'Alexandria')
INSERT INTO dbo.OrgContact (OC_PK, OC_OH) VALUES (@OrgContactPk, @OrgPk)
...
VALUES ('fb1b3bd0-6bdc-48fc-9ca5-175d94e9c626', @CompanyPk, 'name333', '324', '2014-2-21', 'E', '2014-2-21', 'E')
...
INSERT INTO dbo.OrgOpportunity (P8_PK, P8_OH, P8_OpportunityID, P8_GC, P8_SystemCreateTimeUtc, P8_SystemCreateUser, P8_SystemLastEditTimeUtc, P8_SystemLastEditUser)
VALUES ('00000001-0001-43c4-bc3b-2cf82e6e3a0c', @OrgPk, 'OppId2', @CompanyPk, '2014-6-19', 'E', '2014-6-19', 'E')

INSERT INTO dbo.WorkProject (WKP_PK, WKP_SystemCreateTimeUtc, WKP_SystemLastEditTimeUtc, WKP_OA_ClientAddress, WKP_OC_Contact, WKP_ProjectNumber, WKP_SystemCreateUser, WKP_SystemLastEditUser) VALUES ('00000001-0002-43c4-bc3b-2cf82e6e3a0c', '2014-7-12', '2014-7-12', @OrgAddressPk, @OrgContactPk, 'WKP00001', 'E', 'E')
...
VALUES
	...
	(newid(), 'P8', '00000001-0001-43c4-bc3b-2cf82e6e3a0c', 'WKP', '00000001-0002-43c4-bc3b-2cf82e6e3a0c', '00000001-0001-43c4-bc3b-2cf82e6e3a0c', GetUtcDate(), 'E', GetUtcDate(), 'E')
";
			/*
			 * G0
			 *  |\
			...
			AssertContainsExactElementsInAnyOrder(
				new[]
				{
					"G0 fb1b3bd0-6bdc-48fc-9ca5-175d94e9c626 2014-02-21",
					"G8 4e33c461-ed10-465c-a064-f8e5a52054cc 2014-02-21",
				},
				g8Tree.Activities.Select(x => string.Format("{0} {1} {2}", x.TableCode, x.ID, x.SystemLastEditTime.HasValue ? x.SystemLastEditTime.Value.ToString("yyyy-MM-dd") : "")));
			...
			AssertContainsExactElementsInAnyOrder(
				...
				{
					"G0 fb1b3bd0-6bdc-48fc-9ca5-175d94e9c626 2014-02-21",
					"O1 3e1fc5e3-b779-4177-b273-1d36461089e1 2014-03-26",
					"OQ ce6e6b1e-81b2-4750-9d0a-a97e556bb2a8 2014-01-21",
				},
				o1Tree.Activities.Select(x => string.Format("{0} {1} {2}", x.TableCode, x.ID, x.SystemLastEditTime.HasValue ? x.SystemLastEditTime.Value.ToString("yyyy-MM-dd") : "")));
			...
			AssertContainsExactElementsInAnyOrder(
				...
				{
					"OQ ce6e6b1e-81b2-4750-9d0a-a97e556bb2a8 2014-01-21",
					"P8 eef57e46-f43e-43c4-bc3b-2cf82e6e3a0c 2014-04-21",
				},
				p8Tree.Activities.Select(x => string.Format("{0} {1} {2}", x.TableCode, x.ID, x.SystemLastEditTime.HasValue ? x.SystemLastEditTime.Value.ToString("yyyy-MM-dd") : "")));
			...
			AssertContainsExactElementsInAnyOrder(
				...
				{
					"OQ ce6e6b1e-81b2-4750-9d0a-a97e556bb2a8 2014-01-21",
					"TH bd41879b-62ea-406f-8549-9c48c21da95b 2014-01-21",
				},
				thTree.Activities.Select(x => string.Format("{0} {1} {2}", x.TableCode, x.ID, x.SystemLastEditTime.HasValue ? x.SystemLastEditTime.Value.ToString("yyyy-MM-dd") : "")));

			var vbTree = trees[Guid.Parse("eecba5d5-2390-4361-b982-cd38dad9d2bd")];
			AssertContainsExactElementsInAnyOrder(
				new[]
				{
					"VB eecba5d5-2390-4361-b982-cd38dad9d2bd 2014-06-16",
					"OQ ce6e6b1e-81b2-4750-9d0a-a97e556bb2a8 2014-01-21",
				},
				vbTree.Activities.Select(x => string.Format("{0} {1} {2}", x.TableCode, x.ID, x.SystemLastEditTime.HasValue ? x.SystemLastEditTime.Value.ToString("yyyy-MM-dd") : "")));

			var workProjectTree = trees[Guid.Parse("00000001-0001-43c4-bc3b-2cf82e6e3a0c")];
			AssertContainsExactElementsInAnyOrder(
				new[]
				{
					"P8 00000001-0001-43c4-bc3b-2cf82e6e3a0c 2014-06-19",
					"WKP 00000001-0002-43c4-bc3b-2cf82e6e3a0c 2014-07-12",
				},
				workProjectTree.Activities.Select(x => string.Format("{0} {1} {2}", x.TableCode, x.ID, x.SystemLastEditTime.HasValue ? x.SystemLastEditTime.Value.ToString("yyyy-MM-dd") : "")));
		...
		{
			var selectSql = @"SELECT * FROM dbo.vw_SalesRelationNodeWithLastEdit";
			using (var command = TestConnection.Command(selectSql))
			{
				using (var reader = command.ExecuteReader())
				{
					...
					while (reader.Read())
					{
						...
						var activityLastEditTime = reader["ActivitySystemLastEditTime"] == DBNull.Value ? null : (DateTime?)reader["ActivitySystemLastEditTime"];

						tree.Activities.Add(new Activity(activityTableCode, activityId, activityType, activityLastEditTime));
					}
		...
		class Activity
		{
			public Activity(string tableCode, Guid id, string type, DateTime? systemLastEditTime)
			{
				TableCode = tableCode;
				ID = id;
				Type = type;
				SystemLastEditTime = systemLastEditTime;
			}

			public readonly string TableCode;
			public readonly Guid ID;
			public readonly string Type;
			public readonly DateTime? SystemLastEditTime;
		}
		#endregion
	}
}
