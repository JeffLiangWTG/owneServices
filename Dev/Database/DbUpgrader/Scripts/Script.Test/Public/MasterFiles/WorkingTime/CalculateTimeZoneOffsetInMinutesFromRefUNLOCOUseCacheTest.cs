...
{
	class TimeZoneInMinutesFunctionUseCacheTests : TransactionedTestCase
	{
...
";
			...
			AssertTimeZoneOffset("BRSAO", "2019-01-14", 720, new DateTime(2019, 1, 13, 15, 0, 0), new DateTime(2019, 11, 10, 15, 0, 0));
			AssertTimeZoneOffset("BRSAO", "2019-11-11", 780, new DateTime(2019, 11, 10, 15, 0, 0), new DateTime(2020, 1, 12, 15, 0, 0));
			AssertTimeZoneOffset("BRSAO", "2020-01-13", 720, new DateTime(2020, 1, 12, 15, 0, 0), new DateTime(2020, 12, 20, 15, 0, 0));
			AssertTimeZoneOffset("BRSAO", "2020-12-21", 780, new DateTime(2020, 12, 20, 15, 0, 0), new DateTime(2021, 01, 17, 15, 0, 0));
			AssertTimeZoneOffset("BRSAO", "2021-01-18", 720, new DateTime(2021, 1, 17, 15, 0, 0), new DateTime(2021, 11, 14, 15, 0, 0));
		}
...
";
			TestConnection.Command(sql).ExecuteScalar();
			AssertTimeZoneOffset("BRSAO", "2008-10-20", -120, new DateTime(2008, 10, 18, 21, 0, 0), new DateTime(2009, 2, 14, 22, 0, 0));
			AssertTimeZoneOffset("BRSAO", "2009-02-16", -180, new DateTime(2009, 2, 14, 22, 0, 0), new DateTime(2009, 10, 17, 21, 0, 0));
			AssertTimeZoneOffset("BRSAO", "2018-02-17", -120, new DateTime(2017, 10, 14, 21, 0, 0), new DateTime(2018, 2, 17, 22, 0, 0));
			AssertTimeZoneOffset("BRSAO", "2018-02-19", -180, new DateTime(2018, 2, 17, 22, 0, 0), new DateTime(2018, 11, 3, 21, 0, 0));
			AssertTimeZoneOffset("BRSAO", "2018-11-5", -120, new DateTime(2018, 11, 3, 21, 0, 0), new DateTime(2019, 2, 16, 22, 0, 0));
			AssertTimeZoneOffset("BRSAO", "2019-2-18", -180, new DateTime(2019, 2, 16, 22, 0, 0), new DateTime(2019, 11, 9, 21, 0, 0));
		...
		{
			var sql = $"SELECT DstStart, DstEnd, Offset FROM CalculateTimeZoneOffsetInMinutesFromRefUNLOCO('{unloco}', '{utctime}', 1);";
...
";
		...
		void AssertTimeZoneOffset(string unloco, short expectedOffset, string expectedMethod = "Calculate", DateTime? referenceDate = null)
		{
			...
			{
				sql = $"SELECT Offset FROM dbo.{FunctionName}('{unloco}', '{(referenceDate ?? DateTime.Now).ToSqlFormat()}', 1)"; // test only so we don't care if Datetime.Now is accurate or not
			}
	...
	sealed class CalculateTimeZoneOffsetInMinutesFromRefUNLOCOUseCacheTest : DbCreateScriptTest
	{
	}
}
