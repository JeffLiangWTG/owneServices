...
{
	[TestedType(typeof(HolidaySourceHoliday))]
	class HolidaySourceHolidayTest : DbCreateScriptTest
	{
		readonly IEqualityComparer<DataRow> comparator = DataRowComparer.Default;

		void SetUpHolidaysOfAllTypes(Guid holidaySourcePK)
		{
			var insertions = $@"
INSERT INTO dbo.GlbHoliday (GH_PK, GH_HolidayName, GH_Date, GH_ParentID, GH_ParentTableCode)
VALUES
	(NEWID(), 'Date Holiday 0', '2022-02-10', @source, 'GHS'),
	(NEWID(), 'Date Holiday 1', '2022-08-16', @source, 'GHS'),
	(NEWID(), 'Date Holiday 2', '2022-08-30', @source, 'GHS');
...
VALUES
	(NEWID(), 'Recurring Holiday 0', 1, '1ST', 'MAR', 'WED', @source, 'GHS'),
	(NEWID(), 'Recurring Holiday 1', 1, '2ND', 'AUG', 'MON', @source, 'GHS'),
	(NEWID(), 'Recurring Holiday 2', 1, '3RD', 'SEP', 'FRI', @source, 'GHS');

INSERT INTO dbo.GlbHoliday (GH_PK, GH_HolidayName, GH_Recurring, GH_RecurrType, GH_ParentID, GH_ParentTableCode)
VALUES
	(NEWID(), 'Good Friday', 1, 'EFR', @source, 'GHS'),
	(NEWID(), 'Easter Monday', 1, 'EMN', @source, 'GHS');

INSERT INTO dbo.GlbHoliday (GH_PK, GH_HolidayName, GH_Recurring, GH_Date, GH_ParentID, GH_ParentTableCode)
VALUES
	(NEWID(), 'Recurring Date Holiday 0', 1, '2022-11-26', @source, 'GHS');
";

			TestConnection.ExecuteNonQuery(insertions, p => p.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK));
		}

		void SetUpRecurringDateHolidays(Guid holidaySourcePK)
		{
			var insertions = $@"
INSERT INTO dbo.GlbHoliday (GH_PK, GH_HolidayName, GH_Recurring, GH_Date, GH_ParentID, GH_ParentTableCode)
VALUES
	(NEWID(), 'Recurring Date Holiday 0', 1, '2022-11-26', @source, 'GHS'),
	(NEWID(), 'Recurring Date Holiday 1', 1, '2022-10-20', @source, 'GHS'),
	(NEWID(), 'Recurring Date Holiday 2', 1, '2022-08-11', @source, 'GHS');
";
			TestConnection.ExecuteNonQuery(insertions, p => p.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK));
		}
		...
		{
			var holidaySourcePK = Guid.NewGuid();
			SetUpHolidaysOfAllTypes(holidaySourcePK);

			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2022-02-01', '2022-02-20')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
		...
		public void TestRecurringHoliday()
		{
			...
			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2022-02-20', '2022-03-10')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
		...
		public void TestRecurringFixedDateHoliday()
		{
			...
			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2022-11-01', '2022-11-30')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
			...
			query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2012-11-01', '2012-11-30')";
		...
		public void TestEasterHoliday()
		{
			...
			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2022-04-10', '2022-04-17')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
		...
		public void TestAllHolidayTypes()
		{
			...
			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2022-04-17', '2022-08-17')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
		...
		public void TestMixedHolidayTypes()
		{
			...
			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2022-08-01', '2022-08-31')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
		...
		public void TestBothEasterHolidays()
		{
			...
			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2021-04-01', '2021-04-10')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
		...
		public void TestRecurringHolidayIsCalculatedCorrectlyEachYear()
		{
			...
			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2022-09-01', '2022-09-17')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
			...
			query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2021-09-01', '2021-09-17')";
			command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
		...
		{
			var holidaySourcePK = Guid.NewGuid();
			SetUpRecurringDateHolidays(holidaySourcePK);

			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2018-01-01', '2022-12-31')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
		...
		public void TestExactlyFiftyYearSpanReturnsAllHolidays()
		{
			...
			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2013-01-01', '2062-12-31')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
		...
		public void TestMoreThanFiftyYearSpanReturnsHolidaysInFirstFiftyYears()
		{
			var holidaySourcePK = Guid.NewGuid();
			SetUpRecurringDateHolidays(holidaySourcePK);

			var query = "SELECT Holiday, HolidayName FROM dbo.HolidaySourceHoliday(@source, '2012-01-01', '2062-12-31')";
			var command = TestConnection.Command(query);
			command.AddParameter("@source", SqlDbType.UniqueIdentifier, holidaySourcePK);
