...
{
	[TestedType(typeof(trgRefExchangeRate_Upd))]
	class trgRefExchangeRate_Upd_Test : DbCreateScriptTest
	{
		public void TestUpdate()
		{
			...
			using (var command = TestConnection.Command(@"UPDATE dbo.RefExchangeRate SET RE_SellRate=@rate WHERE RE_PK=@PK"))
			{
				command.AddParameterBasedOnDbColumn("@PK", exchangeRatePK, RefExchangeRateSchema.PK);
				command.AddParameterBasedOnDbColumn("@rate", 0.7m, RefExchangeRateSchema.RE_SellRate);
				command.ExecuteNonQuery();
			}
			using (var command = TestConnection.Command(@"SELECT RE_SellRate FROM dbo.ZZRefExchangeRate WHERE RE_PK=@PK"))
			{
				command.AddParameterBasedOnDbColumn("@PK", exchangeRatePK, ZZRefExchangeRateSchema.PK);
				AssertEquals("ZZRefExchangeRate should be updated", 0.7m, (decimal)command.ExecuteScalar());
			}
			using (var command = TestConnection.Command(@"SELECT ZZN_Rate FROM RefDatabase_RefExchangeRateZZ WHERE ZZN_PK=@PK"))
			{
				command.AddParameterBasedOnDbColumn("@PK", exchangeRatePK, RefExchangeRateZZSchema.PK);
				AssertEquals("RefExchangeRateZZ should NOT be updated", 0.6m, (decimal)command.ExecuteScalar());
			}
		}
	}
}
