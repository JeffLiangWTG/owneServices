...
{
	[TestedType(typeof(Report_Client_eTailShipmentItemDetails))]
	internal class Report_Client_eTailShipmentItemDetailsTest : DbCreateScriptTest
	{
		public void TestReport_eTailShipmentItem()
		{
			using (CultureUtils.WithCulture(System.Globalization.CultureInfo.CreateSpecificCulture("en-AU")))
			{
				...
				var result = DataUtils.GetDataTableFromQuery(TestConnection, $"SELECT * FROM Report_Client_eTailShipmentItemDetails(null, '{countryCode}', '{companyPK}', '', '', 'ALL', '1')");
		...
		public void TestReport_eTailShipmentItemWithoutItemLine()
		{
			using (CultureUtils.WithCulture(System.Globalization.CultureInfo.CreateSpecificCulture("en-AU")))
			{
				...
				var result = DataUtils.GetDataTableFromQuery(TestConnection, $"SELECT * FROM Report_Client_eTailShipmentItemDetails(null, '{countryCode}', '{companyPK}', '', '', 'ALL', '0')");
				AssertShipmentItemResults(result);
		...
		public void TestReport_eTailShipmentItem_CustomsStatusDescription_AU()
		{
			...
			DataTable result = DataUtils.GetDataTableFromQuery(TestConnection,
				$"SELECT * FROM Report_Client_eTailShipmentItemDetails(null, '{countryCode}', '{companyPK}', '', '', 'ALL', '1')");
		...
		public void TestReport_eTailShipmentItem_FilterClient()
		{
			using (CultureUtils.WithCulture(System.Globalization.CultureInfo.CreateSpecificCulture("en-AU")))
			{
				...
				_ = TestDataCreator.CreateDocAddress(addressPk1, "", shipmentPK3, "JS", "CRD");
				_ = TestDataCreator.CreateDocAddress(addressPk2, "", shipmentPK3, "JS", "CED");

				var result1 = DataUtils.GetDataTableFromQuery(TestConnection,
					$"SELECT * FROM Report_Client_eTailShipmentItemDetails(null, '{countryCode}', '{companyPK}', '', '', 'ALL', '0')");
				AssertEquals("Result should have three row when Client is null", 3, result1.Rows.Count);

				var result2 = DataUtils.GetDataTableFromQuery(TestConnection,
					$"SELECT * FROM Report_Client_eTailShipmentItemDetails('{organisationPK1}', '{countryCode}', '{companyPK}', '', '', 'ALL', '0')");
				...
				var result3 = DataUtils.GetDataTableFromQuery(TestConnection,
					$"SELECT * FROM Report_Client_eTailShipmentItemDetails('{organisationPK2}', '{countryCode}', '{companyPK}', '', '', 'ALL', '0')");
			...
			Guid PrepTestReport_eTailShipmentItem(TestDbHelper testDbHelper, string shipmentNumber, string orgCode, int clusterKey, string bookingReference)
			{
				...
				_ = CreateHVLVItem(hvlvConsignment, clusterKey, shipmentPK, itemId);
				return shipmentPK;
			}
			...
			Guid CreateHVLVItem(Guid hvlvConsignmentPK, int clusterKey, Guid shipmentPK, string itemId)
			{
				var hvlvItem = new ActiveRowWrapper(HVLVItemSchema.Instance)
				{
					...
					[HVLVItemSchema.HVI_Status] = DispatchedToLastMileCarrier,
				};
		...
		Guid CreateHVLVBookingHeader(Guid billToPartyAddressPK, int clusterKey, string bookingReference, string userCode)
		{
		...
		void PrepTestReport_eTailShipmentItem()
		{
			...
			var hvlvItem = CreateHVLVItem(hvlvConsignment.PK, clusterKey, shipmentPK, itemId);
		...
		void PrepTestReport_eTailShipmentItem_WithCustomsStatus_AU()
		{
			...
			CreateHVLVItem(hvlvConsignment.PK, clusterKey,shipmentPK, itemId);
