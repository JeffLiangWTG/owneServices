...
namespace Enterprise.DbUpgrader.Transformations.Test.Transforms.Customs.Shared
{
	...
	sealed class LandedCostHistory_ParentTableCodeTest : ConstraintBase_ParentTableCodeTest<LandedCostTables_ParentTableCode>
	{
		protected override string TableName => "LandedCostHistory";

		protected override string TablePrefix => "LH";

		protected override string[] SupportedParentPrefixes => ["JI", "JO"];

		protected override bool UseNoCheck => false;

		protected override bool AllowEmptyParentTableCode => false;

		protected override string[] ExpectedIndexIncludeColumns => ["[LH_SystemCreateTimeUtc]", "[LH_SystemLastEditTimeUtc]"];
		...
		{
			DBTransformationTestHelper.DropConstraintIfExists(TableName, "Constraint_LH_ClusterKey");
