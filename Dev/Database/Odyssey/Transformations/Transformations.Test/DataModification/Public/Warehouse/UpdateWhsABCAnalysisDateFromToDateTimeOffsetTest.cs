...
{
	[TestedType(typeof(UpdateWhsABCAnalysisDateFromToDateTimeOffset))]
	class UpdateWhsABCAnalysisDateFromToDateTimeOffsetTest : ConvertDateTimeToDateTimeOffsetTransformTestCase
	{
		protected override void PrepareTestData()
		{
			...
			}.InsertAsDateTimeForColumn(d => d.WJ_AnalysisDateFrom).AppendToInsert(sql);
		...
		protected override void AssertTransformationResults()
		{
			...
			abcCategory.BuildAssertion(TestConnection)
				.ExpectEquals(nameof(WhsABCCategory.WJ_AnalysisDateFrom), d => d.WJ_AnalysisDateFrom,
		...
		public void TestTwoBranchs()
		{
			...
			var branch1 = new GlbBranch("BR1") { GB_RL_NKHomePort = "AUSYD" }.InsertAndReturnObject(TestConnection);
			var whs1 = new WhsWarehouse("WH1", "PRW", branch1.PK).WithDockDoor(TestConnection);
			...
			{
				WJ_AnalysisDateFrom = new DateTimeOffset(2024, 6, 6, 10, 0, 0, TimeSpan.FromHours(0)),
			}.InsertAsDateTimeForColumn(d => d.WJ_AnalysisDateFrom).AppendToInsert(sql);

			var abcCategory3 = new WhsABCCategory("key003", whs2.PK, client.PK, product.PK)
			{
				WJ_AnalysisDateFrom = new DateTimeOffset(2024, 6, 6, 11, 0, 0, TimeSpan.FromHours(0)),
			}.InsertAsDateTimeForColumn(d => d.WJ_AnalysisDateFrom).AppendToInsert(sql);
			...
			WhsABCCategory.AssertFromDB(TestConnection, abcCategory1.PK)
				.ExpectEquals("WJ_AnalysisDateFrom should be equal.", d => d.WJ_AnalysisDateFrom,
					new DateTimeOffset(2024, 6, 6, 0, 0, 0, TimeSpan.FromHours(10)))
				.VerifyAll();
		...
		public void TestNoNeedToUpdate()
		{
			...
			WhsABCCategory.AssertFromDB(TestConnection, abcCategory.PK)
				.ExpectEquals("WJ_AnalysisDateFrom not changed.", d => d.WJ_AnalysisDateFrom,
					new DateTimeOffset(2024, 6, 6, 0, 0, 0, TimeSpan.FromHours(0)))
				.VerifyAll();
		}
		...
		{
			AssertEquals("Update Whs ABC Category AnalysisDateFrom from DateTime to DateTimeOffset.",
...
");
		...
		protected override DataTransformation GetNewTestTransformationInstance() =>
			new UpdateWhsABCAnalysisDateFromToDateTimeOffset();

		protected override SchemaDateTimeOffsetColumn GetDateTimeOffsetColumnToConvert() => WhsABCCategorySchema.WJ_AnalysisDateFrom;
