...
{
	[TestedType(typeof(DeduplicateCartageLegByJobAndSplitDeliverySuffixV2))]
	class DeduplicateCartageLegByJobAndSplitDeliverySuffixV2Test : DataTransformationTestCase
	{
		protected override DataTransformation GetNewTestTransformationInstance()
		{
			return new DeduplicateCartageLegByJobAndSplitDeliverySuffixV2();
		}
...
");
		...
		protected override void AssertTransformationResults()
		{
			...
			CombineAssertions("Checks that transform ran correctly", () =>
			{
				...
				AssertDuplicateLegSuffixPairHaveBeenDeduped(JobT2Move2Leg1PK, JobT2Move3Leg1PK, "C", "E", CartageJobT2ConsignmentID);
				AssertDuplicateLegSuffixPairHaveBeenDeduped(JobT2Move2Leg2PK, JobT2Move3Leg2PK, "D", "F", CartageJobT2ConsignmentID);
			});
		}
		...
		void AssertDuplicateLegSuffixPairHaveBeenDeduped(Guid leg1PK, Guid leg2PK, string unchangedLegSuffix, string expectedDedupedLegSuffix, string cartageJobConsignmentID)
		{
			var commandLeg1 = TestConnection.Command(CheckLegSuffixSql);
			commandLeg1.AddParameter("@legPK", SqlDbType.UniqueIdentifier, leg1PK);
			var currentLeg1Suffix = (string)commandLeg1.ExecuteScalar();

			var commandLeg2 = TestConnection.Command(CheckLegSuffixSql);
			commandLeg2.AddParameter("@legPK", SqlDbType.UniqueIdentifier, leg2PK);
			var currentLeg2Suffix = (string)commandLeg2.ExecuteScalar();
			Assert(FormattableString.Invariant($"Transform should have de-duped legs on cartage job {cartageJobConsignmentID} with the same suffix '{unchangedLegSuffix}'. Instead after transfer these legs now have suffixes '{currentLeg1Suffix}' and '{currentLeg2Suffix}'"),
				(currentLeg1Suffix == unchangedLegSuffix && currentLeg2Suffix == expectedDedupedLegSuffix) ||
				(currentLeg1Suffix == expectedDedupedLegSuffix && currentLeg2Suffix == unchangedLegSuffix));
		}
