...
{
	public class UpdateIncidentDispositionsToClosedTransformV3 : DataTransformation
	{
		public override string UserDescription => "Update resolution details for Incidents in ediprod for IM_ClosureResolution refactor Version 3";
...
IF NOT EXISTS (SELECT Name FROM sys.indexes WHERE name='NR_RX__IM_SystemCreateTimeUtc_Closed_NRC' AND object_id = OBJECT_ID('dbo.IncidentMain'))
	CREATE INDEX NR_RX__IM_SystemCreateTimeUtc_Closed_NRC ON dbo.IncidentMain (IM_SystemCreateTimeUtc) INCLUDE (IM_PK, IM_ResolutionCode, IM_SystemLastEditTimeUtc)
	WHERE
		IM_Status = 'CLS'
		AND IM_ResolutionCode IN ('NRC')
...
";
		...
		{
			var dropIndexQuery = "DROP INDEX IF EXISTS NR_RX__IM_SystemCreateTimeUtc_Closed_NRC ON dbo.IncidentMain";
