namespace CargoWise.Billing.Collectors.Logistics
{
	...
	sealed public class ForwarderExportConsolByContainerTEU : RefStlScriptWithDefaults
	{
...
(
	...
	COALESCE(si.SL_PostedTimeUtc, @MaxDateTime) < COALESCE(jw.TransactionDate, @MaxDateTime) OR -- we're taking the DEP event we need to check that the event time of the DEP is not too old
	(
		jw.EventTime >= DATEADD(HOUR, -24, DATEADD(MONTH, -1, @StartDateTimeInclusive))
		AND jw.EventTime < DATEADD(HOUR, 24, @EndDateTimeExclusive)
	)
)
AND rc.RC_PK IS NOT NULL
AND Containers.JC_ContainerNum != ''
AND Company.GC_IsActive = 1
AND COALESCE(BranchFromSI.GB_IsActive, BranchFromSF.GB_IsActive) = 1
AND jk.JK_AgentType != 'CLD'
...
AND (rsl.RSL_BookingRequestAvailable = 1 OR rsl.RSL_ShippingInstructionAvailable = 1 OR (rsl.RSL_ShippingOrderAvailable = 1 AND COALESCE(BranchFromSF.GB_RN_NKCountryCode, Company.GC_RN_NKCountryCode) = 'CN'))";
