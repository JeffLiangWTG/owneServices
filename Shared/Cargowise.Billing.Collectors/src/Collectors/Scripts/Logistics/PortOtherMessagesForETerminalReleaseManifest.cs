namespace CargoWise.Billing.Collectors.Logistics
{
	...
	sealed public class PortOtherMessagesForETerminalReleaseManifest : RefStlScriptWithDefaults
	{
		public override string FeatureCode => "PMC";
		public override string RoleName => "Terminal Release Manifest Submission to CN Ports";
		...
		public override string FeatureName => "Port Other Messages(PMC)";
...
when COUNT(1)=1 THEN (SELECT TOP 1 CONCAT(jct1.JW_Vessel, ' - ', jct1.JW_VoyageFlight)  FROM dbo.JobConsolTransport jct1 WHERE jct1.JW_ParentGUID = JobConsol.JK_PK)
when COUNT(CASE when jct.JW_TransportMode=jc.JK_TransportMode THEN 1 ELSE NULL END)=0 THEN (SELECT TOP 1 CONCAT(jct1.JW_Vessel, ' - ', jct1.JW_VoyageFlight)  FROM dbo.JobConsolTransport jct1 WHERE jct1.JW_ParentGUID = JobConsol.JK_PK ORDER BY jct1.JW_LegOrder ASC)
when COUNT(CASE when jct.JW_TransportMode=jc.JK_TransportMode THEN 1 ELSE NULL END)>=1 THEN (SELECT TOP 1 CONCAT(jct1.JW_Vessel, ' - ', jct1.JW_VoyageFlight)  FROM dbo.JobConsolTransport jct1 WHERE jct1.JW_ParentGUID = JobConsol.JK_PK AND jct1.JW_TransportMode=JobConsol.JK_TransportMode ORDER BY jct1.JW_LegOrder ASC)
ELSE NULL
END
FROM
dbo.JobConsolTransport jct
JOIN dbo.JobConsol jc ON jc.JK_PK = jct.JW_ParentGUID AND jc.JK_PK = JobConsol.JK_PK
)
";
...
(
	...
	WHERE
		...
		AND SL_Reference LIKE '%MST=eTerminal Release Manifest%'
	...
	SELECT
		...
		(
			...
			WHERE
				...
				AND MSN.SL_Reference LIKE '%MST=eTerminal Release Manifest%'
