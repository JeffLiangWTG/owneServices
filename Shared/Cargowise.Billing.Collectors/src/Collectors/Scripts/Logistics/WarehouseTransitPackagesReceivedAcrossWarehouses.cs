namespace CargoWise.Billing.Collectors.Logistics
{
	...
	public class WarehouseTransitPackagesReceivedAcrossWarehouses : RefStlScriptWithDefaults
	{
		...
		public override string FromClause => $@"
			(
				...
				(
					...
					WHERE WPS_UnloadedTime IS NOT NULL
					AND WPS_UnloadedTime >= {Constants.StartDateTimeInclusiveParamName} AND WPS_UnloadedTime < {Constants.EndDateTimeExclusiveParamName} AND WPS_WL_LastLocation IS NOT NULL
					UNION ALL
					SELECT KPH_PackageID 
					FROM dbo.WhsItemPackageState
					JOIN dbo.PkgPackage ON KP_PK = WPS_KP_Package 
					JOIN dbo.PkgPackageHeader ON KP_KPH_PackageHeader = KPH_PK 
					WHERE WPS_UnloadedTime IS NULL
					AND CAST(WPS_SystemCreateTimeUtc AS DATETIMEOFFSET) >= {Constants.StartDateTimeInclusiveParamName} AND CAST(WPS_SystemCreateTimeUtc AS DATETIMEOFFSET) < {Constants.EndDateTimeExclusiveParamName} AND WPS_WL_LastLocation IS NOT NULL
				)
