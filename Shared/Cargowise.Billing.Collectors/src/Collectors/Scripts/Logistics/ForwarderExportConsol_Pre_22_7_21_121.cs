namespace CargoWise.Billing.Collectors.Logistics
{
	...
	public class ForwarderExportConsol_Pre_22_7_21_121 : RefStlScriptWithDefaults
	{
...
(
	...
	WHERE
		...
		public override string WhereClause => @"(   si.JK_PK IS NOT NULL OR   (    jw.EventTime >= DATEADD(HOUR, -24, DATEADD(MONTH, -1, @StartDateTimeInclusive))    AND jw.EventTime < DATEADD(HOUR, 24, @EndDateTimeExclusive)   )  )  AND Company.GC_IsActive = 1  AND COALESCE(BranchFromSI.GB_IsActive, BranchFromSF.GB_IsActive) = 1  AND jk.JK_IsForwarding = 1  AND  (   (    jk.JK_AgentType = 'CLD'    AND Creditor.OH_IsSeaWholesaler = 1    AND rslCoLoadWith.RSL_IsNVO = 1    AND rslCoLoadWith.RSL_OceanCarrierMessagingAvailable = 1   )   OR   (    jk.JK_AgentType != 'CLD'    AND ShippingLine.OH_IsSeaWholesaler = 1    AND rslShippingLine.RSL_IsNVO = 1    AND rslShippingLine.RSL_OceanCarrierMessagingAvailable = 1   )  )";
