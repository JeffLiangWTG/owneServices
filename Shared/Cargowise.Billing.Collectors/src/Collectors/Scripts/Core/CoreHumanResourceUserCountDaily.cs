namespace CargoWise.Billing.Collectors.Core
{
	...
	public class CoreHumanResourceUserCountDaily : RefStlScriptWithDefaults
	{
		public override bool UsedInBilling => false;
		public override string FeatureCode => "HR2";
		...
		public override string FeatureName => "HR User Count Daily";
...
INSERT @ActiveStaffPerDay
	...
	SELECT @DayAsDate, (DATEPART(year, @DayAsDate) * 100) + DATEPART(month, @DayAsDate), staff.GS_Code 
		...
		AND ustLog.SL_SE_NKEvent = @UstEvent
		AND ustLog.SL_Reference like '%HRU%'
	UNION
	...
	SELECT @DayAsDate, (DATEPART(year, @DayAsDate) * 100) + DATEPART(month, @DayAsDate), staff.GS_Code 
		...
		AND rankedUst.SL_Reference like 'HRU%'
	UNION
