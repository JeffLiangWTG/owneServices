namespace CargoWise.Billing.Collectors.Core
{
	...
	public class CoreDeviceOnlyUserCount : RefStlScriptWithDefaults
	{
		public override bool UsedInBilling => false;
		public override string FeatureCode => "DOU";
		...
		public override string FeatureName => "Device-Only User Count";
...
FROM StmALog ustLog
INNER JOIN CurrentStaffCte staff ON staff.GS_PK = ustLog.SL_Parent
WHERE ustLog.SL_PostedTimeUtc >= @MonthRangeStart AND ustLog.SL_PostedTimeUtc < @MonthRangeEnd AND ustLog.SL_SE_NKEvent = @UstEvent AND ustLog.SL_Reference like '%DOU%'
UNION
...
WHERE rankedUst.LogSequence = 1 AND rankedUst.SL_Reference like 'DOU%'
UNION
...
WHERE rankedUst.LogSequence is null AND staff.GS_IsActive = 1 AND staff.GS_IsDevice = 1 AND staff.GS_IsRobot = 0 AND staff.GS_CanLogin = 1;";
