namespace CargoWise.Billing.Collectors.Core
{
	...
	public class CoreRobotUserCount : RefStlScriptWithDefaults
	{
		public override bool UsedInBilling => true;
		public override string FeatureCode => "RBU";
		...
		public override string FeatureName => "Robot User Count";
...
IF (DATEDIFF(dd, @MonthRangeStart, @MonthRangeEnd) > 31)  BEGIN   RAISERROR ('Date range cannot be greater than one month.', 16, 1);
	RETURN;
END;
...
SELECT @MonthAsDate, staff.GS_Code    FROM StmALog ustLog    INNER JOIN CurrentStaffCte staff ON staff.GS_PK = ustLog.SL_Parent
WHERE ustLog.SL_PostedTimeUtc >= @MonthRangeStart AND ustLog.SL_PostedTimeUtc < @MonthRangeEnd AND ustLog.SL_SE_NKEvent = @UstEvent AND ustLog.SL_Reference like '%RBU%'
UNION
...
WHERE rankedUst.LogSequence = 1 AND rankedUst.SL_Reference like 'RBU%'
UNION
...
WHERE rankedUst.LogSequence is null AND staff.GS_IsActive = 1 AND staff.GS_IsRobot = 1 AND staff.GS_CanLogin = 1;";
