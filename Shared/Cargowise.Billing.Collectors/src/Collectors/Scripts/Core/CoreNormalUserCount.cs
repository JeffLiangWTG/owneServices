namespace CargoWise.Billing.Collectors.Core
{
	...
	public class CoreNormalUserCount : RefStlScriptWithDefaults
	{
		public override bool UsedInBilling => true;
		public override string FeatureCode => "USR";
		...
		public override string FeatureName => "User Count";
...
WITH
	...
	RankedUstLogCte AS (
		...
		WHERE
			ustLog.SL_PostedTimeUtc >= @MonthRangeEnd
...
SELECT @MonthAsDate, staff.GS_Code    FROM StmALog ustLog
INNER JOIN CurrentStaffCte staff ON staff.GS_PK = ustLog.SL_Parent
WHERE ustLog.SL_PostedTimeUtc >= @MonthRangeStart AND ustLog.SL_PostedTimeUtc < @MonthRangeEnd AND ustLog.SL_SE_NKEvent = @UstEvent AND ustLog.SL_Reference like '%USR%'

UNION   -- Add staff who first UST event after date range has a Reference value like '___>>%'
...
WHERE rankedUst.LogSequence = 1 AND rankedUst.SL_Reference like 'USR%'

UNION   -- Add staff with no UST events after date range and whose current flags match the staff status (IsActive=1, IsDevice=?, CanLogin=?)
SELECT @MonthAsDate, staff.GS_Code
FROM CurrentStaffCte staff
LEFT JOIN RankedUstLogCte rankedUst ON staff.GS_Code = rankedUst.GS_Code
WHERE rankedUst.LogSequence is null AND staff.GS_IsActive = 1 AND staff.GS_IsDevice = 0 AND staff.GS_IsRobot = 0 AND staff.GS_CanLogin = 1;";
