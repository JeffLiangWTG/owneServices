namespace CargoWise.Billing.Collectors.Core
{
	...
	public class CoreNormalUserCountDaily : RefStlScriptWithDefaults
	{
		public override bool UsedInBilling => true;
		public override string FeatureCode => "US2";
		...
		public override string FeatureName => "User Count Daily";
...
INSERT @ActiveStaffPerDay
	...
	SELECT @DayAsDate, (DATEPART(year, @DayAsDate) * 100) + DATEPART(month, @DayAsDate), staff.GS_Code 
		...
		AND ustLog.SL_SE_NKEvent = @UstEvent
		AND ustLog.SL_Reference like '%USR%'
	UNION
	...
	SELECT @DayAsDate, (DATEPART(year, @DayAsDate) * 100) + DATEPART(month, @DayAsDate), staff.GS_Code 
		...
		AND rankedUst.SL_Reference like 'USR%'
	UNION
