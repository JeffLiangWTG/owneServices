namespace CargoWise.Billing.Collectors.Customs
{
	...
	public class CommunicationImportClearanceBLT : RefStlScriptWithDefaults
	{
		public override bool UsedInBilling => true;
		public override string FeatureCode => "IF2";
		...
		public override string FeatureName => "Import Clearance Full / Detailed Formal Entry / Fiscal Report (BLT)";
		...
		public override string BillingReference4 => "jech.Ref4";
...
(
	SELECT
		...
		GC_RN_NKCountryCode,
		GB_Code,
		Ref4 = IIF(jh.JH_IsDisbursement = 1, CONVERT(VARCHAR(36), jh.JH_PK) , '')
	...
	WHERE
		...
		AND (je.JE_ApplicationCode = 'BLT' OR (gc.GC_RN_NKCountryCode = 'FR' AND je.JE_ApplicationCode IN ('DG', 'DI')) OR (gc.GC_RN_NKCountryCode = 'IE' AND je.JE_ApplicationCode IN ('V1','V2')))
		AND ((gc.GC_RN_NKCountryCode = 'ZA' AND je.JE_MessageType IN ('IMP', 'EXW')) OR (gc.GC_RN_NKCountryCode <> 'ZA' AND je.JE_MessageType <> 'EXP'))
		AND je.JE_TransportMode NOT IN ('RAI', 'ROA', 'TRK')
	UNION ALL
	SELECT
		je.JE_PK,
		je.JE_DeclarationReference,
		...
		(	
			...
			(
				(
					...
					OR (gc.GC_RN_NKCountryCode = 'FR' AND je.JE_ApplicationCode IN ('DG', 'DI'))
					OR (gc.GC_RN_NKCountryCode = 'IE' AND je.JE_ApplicationCode IN ('V1','V2'))
				)
				AND 
				(	
