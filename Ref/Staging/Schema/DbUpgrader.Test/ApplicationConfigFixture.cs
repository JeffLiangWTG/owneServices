...
namespace CargoWise.RefDbRepo.Staging.DbUpgrader.Test
{
	...
	class ApplicationConfigFixture
	{
		...
		{
			var configFile = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "CargoWise.RefDbRepo.Staging.DbUpgrader.config.json");
			Assert.True(File.Exists(configFile));

			var folderPath = Path.Combine(TestSourcePathHelper.DATTestSupplementaryContentPath, @"Staging\Schema\DbUpgrader");
			...
			foreach (var jsonFile in deployConfigJsonFiles)
			{
				...
				var connectionString = jObject["ConnectionStrings"]["staging"].Value<string>();
