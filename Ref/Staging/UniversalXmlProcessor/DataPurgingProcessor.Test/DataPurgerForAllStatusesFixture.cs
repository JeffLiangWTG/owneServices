...
namespace CargoWise.RefDbRepo.Staging.DataPurgingProcessor.Test;
	...
	[TestFixture]
public class DataPurgerForAllStatusesFixture
{
	[Test]
	[TransactionedTestCase]
	[Property("DAT:CapabilityRequirements", "SQL2019+")]
	public void PurgeDataForAllStatuses()
	{
...
<Property Name=""ZX3_ZX4_ZZZ_NKDataGrouping"" Type=""varchar"" DefaultValue=""CN"" MaxLength=""3""/>
		...
		using (var repo = new StagingRepository(builderString))
		{
			...
			var info1 = new DataProcessingInformation
			{
				...
				DPI_Status = StatusProvider.GetQUEStatus()
			};
			...
			repo.ExecuteSqlCommand(
				...
				new SqlParameter("@CreatedTime", System.Data.SqlDbType.DateTime2) { Value = new DateTime(2021, 1, 1) },
				new SqlParameter("@SourceID", System.Data.SqlDbType.UniqueIdentifier) { Value = source1ID });
		}

		using (var repo = new StagingRepository(builderString))
		{
			var purger = new DataPurgerForAllStatuses(repo);
			purger.Purge();
			Assert.That(repo.Get<RefCusTariff>().ToArray().Length, Is.EqualTo(1));
			Assert.That(repo.Get<RefCusCondition>().ToArray().Length, Is.EqualTo(0));
			Assert.That(repo.Get<RefCusConditionValue>().ToArray().Length, Is.EqualTo(0));
			Assert.That(repo.Get<DataProcessingInformation>().Where(x => x.DPI_SourceId == source1ID).ToArray().Length, Is.EqualTo(0));
			Assert.That(repo.Get<DataProcessingInformation>().Where(x => x.DPI_SourceId == source2ID).ToArray().Length, Is.EqualTo(1));
			Assert.That(repo.Get<SourceData>().FirstOrDefault(x => x.SDA_PK == source1ID)?.SDA_Status, Is.EqualTo(StatusProvider.GetPURStatus()));
		}
	}
	...
	{
		var builderString = GetEntityConnectionBuilderString();
		var sdaPk = Guid.NewGuid();
		var source1 = new SourceData
		{
			SDA_PK = sdaPk,
			SDA_ContentType = DataSourceConstants.ContentType.ZA_ProDat,
			SDA_ContentText = "any",
			SDA_Status = StatusProvider.GetFIEStatus(),
			SDA_Source = DataSourceConstants.Source.InternalWebsite,
		...
		using (var repo = new StagingRepository(builderString))
		{
			...
			repo.ExecuteSqlCommand(
				$"UPDATE {nameof(SourceData)} SET {nameof(SourceData.SDA_CreatedTime)} = @CreatedTime WHERE {nameof(SourceData.SDA_PK)} = @SourceID",
				new SqlParameter("@CreatedTime", System.Data.SqlDbType.DateTime2) { Value = new DateTime(2021, 1, 1) },
				new SqlParameter("@SourceID", System.Data.SqlDbType.UniqueIdentifier) { Value = sdaPk });
		}
		...
		using (var repo = new StagingRepository(builderString))
		{
			var purger = new DataPurgerForAllStatuses(repo);
			purger.Purge();
			Assert.AreEqual(0, repo.Get<DataProcessingInformation>().Where(x => x.DPI_SourceId == sdaPk).ToArray().Length);
			Assert.AreEqual(0, repo.Get<RefCusTariff>().Where(x => x.ZZ1_PK == tariff1.ZZ1_PK).ToArray().Length);
		}
	}
