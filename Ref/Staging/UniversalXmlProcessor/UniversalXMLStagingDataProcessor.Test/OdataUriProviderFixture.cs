...
{
	[TestFixture]
	class OdataUriProviderFixture
	{
		[Test]
		public void GetUriTest_Safe_MutilTables()
		{
			var metadataProvider = new MetadataProvider(xmlContentWithMutilTables, cacheProvider, cacheProviderForOriginal);
			var odataUriProvider = new OdataUriProvider(metadataProvider);
			var parentPK = Guid.NewGuid();
			var datetimeUTC = DateTime.UtcNow;
			var uri = odataUriProvider.GetUri(DbSource.RefDbRepoSafe, typeof(Safe.RefCusTariff), parentPK, datetimeUTC);

			Assert.NotNull(uri);
			Assert.AreEqual($"{Constants.SafeUpdateServiceUri}RefCusTariffUpdate/Default.GetWithOptimizedExpand()?$filter=ZZ1_PK eq {parentPK}&SystemVersionUTC={datetimeUTC:yyyy-MM-ddTHH:mm:ss.fffffffZ}&$expand=RefCusRates($expand=RefCusApplicabilities($expand=RefCusExcludedTradeGroups)),RefCusTariffAttributes,RefCusTariffNationalCodes($expand=RefCusRates($expand=RefCusApplicabilities($expand=RefCusExcludedTradeGroups)),RefCusVATApplicabilities,RefCusTariffAttributes,RefCusTariffUOMs),RefCusTariffRelationships,RefCusConditions($expand=RefCusConditionValues,RefCusApplicabilities($expand=RefCusExcludedTradeGroups)),RefCusTariffUOMs,RefCusVATApplicabilities", uri.ToString());
		}

		[Test]
		public void GetUriTest_Safe_SingleTable()
		{
			var metadataProvider = new MetadataProvider(xmlContentWithSingleTable, cacheProvider, cacheProviderForOriginal);
			var odataUriProvider = new OdataUriProvider(metadataProvider);
			var parentPK = Guid.NewGuid();
			var datetimeUTC = DateTime.UtcNow;
			var uri = odataUriProvider.GetUri(DbSource.RefDbRepoSafe, typeof(Safe.RefCusTariff), parentPK, datetimeUTC);
			...
			Assert.NotNull(uri);
			Assert.AreEqual($"{Constants.SafeUpdateServiceUri}RefCusTariffUpdate/Default.GetWithOptimizedExpand()?$filter=ZZ1_PK eq {parentPK}&SystemVersionUTC={datetimeUTC:yyyy-MM-ddTHH:mm:ss.fffffffZ}", uri.ToString());
		}

		[Test]
		public void GetUriTest_Staging_MutilTables()
		{
			var metadataProvider = new MetadataProvider(xmlContentWithMutilTables, cacheProvider, cacheProviderForOriginal);
			var odataUriProvider = new OdataUriProvider(metadataProvider);
			var parentPK = Guid.NewGuid();
			var uri = odataUriProvider.GetUri(DbSource.RefDbRepoStaging, typeof(Stage.RefCusTariff), parentPK);

			Assert.NotNull(uri);
			Assert.AreEqual($"{Constants.StagingServiceUri}RefCusTariffUpdate?$filter=ZZ1_PK eq {parentPK}&$expand=RefCusRates($expand=RefCusApplicabilities($expand=RefCusExcludedTradeGroups)),RefCusTariffAttributes,RefCusTariffNationalCodes($expand=RefCusRates($expand=RefCusApplicabilities($expand=RefCusExcludedTradeGroups)),RefCusVATApplicabilities,RefCusTariffAttributes,RefCusTariffUOMs),RefCusTariffRelationships,RefCusConditions($expand=RefCusConditionValues,RefCusApplicabilities($expand=RefCusExcludedTradeGroups)),RefCusTariffUOMs,RefCusVATApplicabilities", uri.ToString());
		}

		[Test]
		public void GetUriTest_Staging_SingleTable()
		{
			var metadataProvider = new MetadataProvider(xmlContentWithSingleTable, cacheProvider, cacheProviderForOriginal);
			var odataUriProvider = new OdataUriProvider(metadataProvider);
			var parentPK = Guid.NewGuid();
			var datetimeUTC = DateTime.UtcNow;
			var uri = odataUriProvider.GetUri(DbSource.RefDbRepoStaging, typeof(Stage.RefCusTariff), parentPK, datetimeUTC);

			Assert.NotNull(uri);
			Assert.AreEqual($"{Constants.StagingServiceUri}RefCusTariffUpdate?$filter=ZZ1_PK eq {parentPK}", uri.ToString());
		}
