...
{
	class Test_Condition_MultiApps : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Condition_MultiApps.xml"];

		public XmlProcessIntegrationTestAssertResultHandler[] AssertResults => [AssertResult_AfterProcessingXml1];

		public string TestDescription => "Test will create duplicate Condition intermittently with more than 3 apps";
...
values (newid(), 'GB', 'Great Britain', null);
...
values ('D2E07963-932F-4BB4-A2E1-9C90F8848C23','DTY', 'Duty',1, 'GB', '', 'CV',0);
...
values ('FC6DB681-F5F4-4B2F-9020-84AFF4F8BE98', 'IMP', 'Import Tariff', '', 'GB', 'D2E07963-932F-4BB4-A2E1-9C90F8848C23', 0);

insert into [dbo].[RefCusTradeGroup] (ZZA_PK, ZZA_TradeGroup, ZZA_Description, ZZA_StartDate, ZZA_EndDate, ZZA_ZZZ_NKDataGrouping)
values ('73DD6854-4168-4D35-B9B9-3F8F99DB0108', 'LY','LY Desc', '1958-01-01 00:00:00', '2079-06-06 23:59:00','GB'),
(newid(), 'RU','RU Desc', '1958-01-01 00:00:00', '2079-06-06 23:59:00','GB'),
(newid(), 'KP','KP Desc', '1958-01-01 00:00:00', '2079-06-06 23:59:00','GB');

insert into [dbo].[RefCusConditionType] (ZX2_PK, ZX2_ConditionClass, ZX2_ConditionType, ZX2_Description, ZX2_ZZZ_NKDataGrouping)
values (newid(), 'CTRL', '711', 'Import control on luxury goods', 'GB');

insert into [dbo].[RefCusConditionValueType] (ZX4_PK, ZX4_ValueType, ZX4_Description, ZX4_IsFormula, ZX4_ZZZ_NKDataGrouping)
values(newid(), 'SUP', 'Presentation of an endorsed certificate/licence', 0, 'GB');
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml1(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "ZZ1", "ZX1", "ZX3", "ZZT" }, dataProcessingResults.Select(x => x.DPR_ParentTableCode).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "Y920" }, vals.Select(x => x.ZX3_Value).Distinct());
		}
	}
}
