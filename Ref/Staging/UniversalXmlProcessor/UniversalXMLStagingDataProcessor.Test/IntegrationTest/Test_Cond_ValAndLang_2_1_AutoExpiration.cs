...
{
	class Test_Cond_ValAndLang_2_1_AutoExpiration : IXmlProcessIntegrationTest
	{
		...
		public string TestDescription => "Test consilidating RefCusCondition(single, identical)/RefCusApplicability with a single new ConditionValue/ConditionLanguage instead of 2 existing ones when auto expiration is on";
...
";
		...
		void AssertResult_SafeDb_RefCusConditionValue_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(vals.Select(x => x.ZX3_Value).Distinct(), Is.EquivalentTo(new string[] { "C003" }));
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(langs.Select(x => x.ZXJ_ZX6_NKLanguage).Distinct(), Is.EquivalentTo(new string[] { "EN" }));
		}
	}
}
