...
{
	class Test_SafeFutureUnderDifferentKeyAndWrapperFutureConflicts : IXmlProcessIntegrationTest
	{
		public string[] FileNames => new string[] { "TestFiles\\Test_SafeFutureUnderDifferentKeyAndWrapperFutureConflicts_1.xml", "TestFiles\\Test_SafeFutureUnderDifferentKeyAndWrapperFutureConflicts_2.xml" };
		...
		public string TestDescription => "Test both SafeDb and XML contains one future record and besides one mentioned, Safe has another future records but under different key";
...
";
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "2001-01-01 00:00:00", "2062-01-01 00:00:00", "2065-01-01 00:00:00" }, apps.Select(x => $"{x.ZZT_StartDate:yyyy-MM-dd HH:mm:ss}").Distinct());
			CollectionAssert.AreEquivalent(new string[] { "2020-01-01 00:00:00", "2064-01-01 00:00:00", "2068-01-01 00:00:00" }, apps.Select(x => $"{x.ZZT_EndDate:yyyy-MM-dd HH:mm:ss}").Distinct());
			Assert.That(apps.Select(x => x.ZZT_AdditionalCode).Distinct(), Is.EquivalentTo(new string[] { "a", "c", "f" }));
		}
		...
		void AssertResult_StagingDb_RefCusApplicability_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(apps.Select(x => x.ZZT_AdditionalCode).Distinct(), Is.EquivalentTo(new[] { "a", "b", "c", "f" }));
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusCondition[] conds, IDbCommand safeCommand)
		{
			...
			Assert.That(apps, Has.Count.EqualTo(3));
			var app_a = apps.FirstOrDefault(x => x.ZZT_AdditionalCode == "a");
			Assert.That(app_a, Is.Not.Null);
			Assert.That(app_a.ZZT_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2001-01-01 00:00:00");
			Assert.That(app_a.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2023-01-01 00:00:00");

			var app_b = apps.FirstOrDefault(x => x.ZZT_AdditionalCode == "b");
			Assert.That(app_b, Is.Not.Null);
			Assert.That(app_b.ZZT_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2061-01-01 00:00:00");
			Assert.That(app_b.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2070-01-01 00:00:00");

			var app_f = apps.FirstOrDefault(x => x.ZZT_AdditionalCode == "f");
			Assert.That(app_f, Is.Not.Null);
			Assert.That(app_f.ZZT_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2065-01-01 00:00:00");
			Assert.That(app_f.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2068-01-01 00:00:00");
		}
	}
}
