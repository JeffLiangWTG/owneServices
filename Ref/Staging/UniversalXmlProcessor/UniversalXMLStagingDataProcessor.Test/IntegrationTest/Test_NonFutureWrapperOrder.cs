...
{
	class Test_NonFutureWrapperOrder : IXmlProcessIntegrationTest
	{
		public string[] FileNames => new string[] { "TestFiles\\Test_NonFutureWrapperOrder_1.xml", "TestFiles\\Test_NonFutureWrapperOrder_2.xml" };
		...
		public string TestDescription => "Test non future wrapper should be processed order by startdate asc";
...
";
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "2001-01-01 00:00:00"}, apps.Select(x => $"{x.ZZT_StartDate:yyyy-MM-dd HH:mm:ss}").Distinct());
			CollectionAssert.AreEquivalent(new string[] { "2020-01-01 00:00:00"}, apps.Select(x => $"{x.ZZT_EndDate:yyyy-MM-dd HH:mm:ss}").Distinct());
			Assert.That(apps.Select(x => x.ZZT_AdditionalCode).Distinct(), Is.EquivalentTo(new string[] { "a" }));
		}
		...
		void AssertResult_StagingDb_RefCusApplicability_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(apps.Select(x => x.ZZT_AdditionalCode).Distinct(), Is.EquivalentTo(new[] { "a", "b" }));
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusCondition[] conds, IDbCommand safeCommand)
		{
			...
			Assert.That(app_a.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2010-01-01 00:00:00");

			var app_b = apps.FirstOrDefault(x => x.ZZT_AdditionalCode == "b");
			Assert.That(app_b, Is.Not.Null);
			Assert.That(app_b.ZZT_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2015-01-01 00:00:00");
			Assert.That(app_b.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2020-01-01 00:00:00");
		}
	}
}
