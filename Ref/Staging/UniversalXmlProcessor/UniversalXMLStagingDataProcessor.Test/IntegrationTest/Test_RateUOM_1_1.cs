...
{
	class Test_RateUOM_1_1 : IXmlProcessIntegrationTest
	{
		...
		public string TestDescription => "Test consilidating RefCusRate(single, haschange)/RefCusApplicability with a single new UOM";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "ZXG_PRS", "ZZ1_QUE", "ZZT_QUE", "ZZ2_QUE", "ZXG_QUE" }, dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}"));
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "MIL", "UNB" }, uoms.Select(x => x.ZXG_UOM).Distinct());
		}
	}
}
