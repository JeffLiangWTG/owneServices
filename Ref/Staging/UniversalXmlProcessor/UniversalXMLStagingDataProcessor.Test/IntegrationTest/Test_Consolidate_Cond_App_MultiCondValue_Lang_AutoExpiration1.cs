...
{
	class Test_Consolidate_Cond_App_MultiCondValue_Lang_AutoExpiration1 : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Cond_App_WithMultiValAndLang7.xml", "TestFiles\\Test_Cond_App_WithMultiValAndLang8.xml"];
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml1(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "ZZ1", "ZX1", "ZZT", "ZX3", "ZXJ" }, dataProcessingResults.Select(x => x.DPR_ParentTableCode).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusCondition_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "Cond1", "Cond2" }, conds.Select(x => x.ZX1_Comment).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusConditionValue_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "C001", "C002" }, vals.Select(x => x.ZX3_Value).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "ITL", "GRM" }, langs.Select(x => x.ZXJ_ZX6_NKLanguage).Distinct());
		}
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults, Has.Count.EqualTo(9));
			CollectionAssert.AreEquivalent(new string[] {"ZZ1_QUE", "ZZT_QUE", "ZX1_QUE", "ZX3_QUE", "ZXJ_QUE" }, dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}").Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusCondition_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(conds, Has.Count.EqualTo(2));
			CollectionAssert.AreEquivalent(new string[] { "Cond3" }, conds.Select(x => $"{x.ZX1_Comment}").Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(apps, Has.Count.EqualTo(2));
			CollectionAssert.AreEquivalent(new string[] { "U313", "U314" }, apps.Select(x => $"{x.ZZT_AdditionalCode}"));
		}
		...
		void AssertResult_SafeDb_RefCusConditionValue_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(vals, Has.Count.EqualTo(2));
			CollectionAssert.AreEquivalent(new string[] { "C003", "C004" }, vals.Select(x => x.ZX3_Value).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(langs, Has.Count.EqualTo(2));
			CollectionAssert.AreEquivalent(new string[] { "FRN", "EN" }, langs.Select(x => x.ZXJ_ZX6_NKLanguage).Distinct());
		}
	}
}
