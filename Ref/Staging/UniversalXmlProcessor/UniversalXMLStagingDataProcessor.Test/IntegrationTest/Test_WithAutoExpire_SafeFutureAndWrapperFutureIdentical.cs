...
{
	class Test_WithAutoExpire_SafeFutureAndWrapperFutureIdentical : IXmlProcessIntegrationTest
	{
		public string[] FileNames => new string[] { "TestFiles\\Test_SafeFutureAndWrapperFutureIdentical_1.xml", "TestFiles\\Test_SafeFutureAndWrapperFutureIdentical_2.xml" };
		...
		public string TestDescription => "Test both SafeDb and XML contains the same future record will neither delete nor recreate records with auto expiration on";
...
";
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			using (var reader = safeCommand.ExecuteReader())
			{
				while (reader.Read())
				{
					...
					app.ZZT_PK = (Guid)reader[nameof(Safe.RefCusApplicability.ZZT_PK)];
			...
			CollectionAssert.AreEquivalent(new string[] { "2060-01-01 00:00:00" }, apps.Select(x => $"{x.ZZT_StartDate:yyyy-MM-dd HH:mm:ss}").Distinct());
			CollectionAssert.AreEquivalent(new string[] { "2070-01-01 00:00:00" }, apps.Select(x => $"{x.ZZT_EndDate:yyyy-MM-dd HH:mm:ss}").Distinct());
			Assert.That(apps.Select(x => x.ZZT_AdditionalCode).Distinct(), Is.EquivalentTo(new string[] { "c" }));
			appPK = apps.First().ZZT_PK;
		}
		...
		void AssertResult_StagingDb_RefCusApplicability_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(apps.Select(x => x.ZZT_AdditionalCode).Distinct(), Is.EquivalentTo(new[] { "c" }));
		}
		...
		void AssertResult_SafeDb_RefCusTariff_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(tariffs, Has.Count.EqualTo(1));
			Assert.That(tariffs.Select(x => x.ZZ1_TariffCode).Distinct(), Is.EquivalentTo(new string[] { "22030" }));
			Assert.That(tariffs.Select(x => x.ZZ1_Description).Distinct(), Is.EquivalentTo(new string[] { "Hand-made1" }));
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusCondition[] conds, IDbCommand safeCommand)
		{
			...
			Assert.That(apps, Has.Count.EqualTo(1));
			var app_c = apps.FirstOrDefault(x => x.ZZT_AdditionalCode == "c");
			Assert.That(app_c, Is.Not.Null);
			Assert.That(app_c.ZZT_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2060-01-01 00:00:00");
			Assert.That(app_c.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2070-01-01 00:00:00");
			Assert.That(appPK == app_c.ZZT_PK);
		}
	}
}
