...
{
	class Test_Cond_ValAndLang_1_2_AutoExpiration : IXmlProcessIntegrationTest
	{
		...
		public string TestDescription => "Test consilidating RefCusCondition(single, identical)/RefCusApplicability with multiple new ConditionValues/ConditionLanguages when auto expiration is on";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}"), Is.EquivalentTo(new string[] {"ZZ1_QUE", "ZZT_QUE", "ZX1_QUE", "ZX3_QUE", "ZX3_QUE", "ZXJ_QUE", "ZXJ_QUE" }));
		}
		...
		void AssertResult_SafeDb_RefCusConditionValue_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(vals.Select(x => x.ZX3_Value).Distinct(), Is.EquivalentTo(new string[] { "C002", "C003" }));
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(langs.Select(x => x.ZXJ_ZX6_NKLanguage).Distinct(), Is.EquivalentTo(new string[] { "EN", "FRN" }));
		}
	}
}
