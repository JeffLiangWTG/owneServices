...
{
	class Test_WithAutoExpire_SafeFutureAndWrapperNonFutureConflicts : IXmlProcessIntegrationTest
	{
		public string[] FileNames => new string[] { "TestFiles\\Test_SafeFutureAndWrapperNonFutureConflicts_1.xml", "TestFiles\\Test_SafeFutureAndWrapperNonFutureConflicts_2.xml" };
		...
		public string TestDescription => "Test SafeDb contains one future record, wrapper record not future but conflicts with future record in SafeDb with auto expiration on";
...
";
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusCondition[] conds, IDbCommand safeCommand)
		{
			...
			Assert.That(app_b.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2063-01-01 00:00:00");
		}
	}
}
