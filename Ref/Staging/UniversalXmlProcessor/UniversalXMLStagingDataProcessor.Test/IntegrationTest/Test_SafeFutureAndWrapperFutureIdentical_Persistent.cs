...
{
	class Test_SafeFutureAndWrapperFutureIdentical_Persistent : IXmlProcessIntegrationTest
	{
		public string[] FileNames => new string[] { "TestFiles\\Test_SafeFutureAndWrapperFutureIdentical_Persistent_1.xml", "TestFiles\\Test_SafeFutureAndWrapperFutureIdentical_Persistent_2.xml" };
		...
		public string TestDescription => "Test both SafeDb and XML contains the same future persistent record will neither delete nor recreate records";
...
";
		...
		void AssertResult_AfterProcessingXml1(IDbCommand stagingCommand, IDbCommand safeCommand)
		{
			Assert.Multiple(() =>
			{
				...
				AssertResult_SafeDb_RefCusTariffNationalCode_AfterProcessingXml1(safeCommand);
			});
		}
		...
		void AssertResult_AfterProcessingXml2(IDbCommand stagingCommand, IDbCommand safeCommand)
		{
			Assert.Multiple(() =>
			{
				...
				AssertResult_SafeDb_RefCusTariffNationalCode_AfterProcessingXml2(safeCommand);
			});
		}
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml1(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults.Select(x => x.DPR_ParentTableCode).Distinct(), Is.EquivalentTo(new string[] { "ZZ1", "ZZW" }));
		}
		...
		void AssertResult_SafeDb_RefCusTariff_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			Assert.That(tariffs.Select(x => x.ZZ1_Description).Distinct(), Is.EquivalentTo(new string[] { "Hand-made" }));
		}

		void AssertResult_SafeDb_RefCusTariffNationalCode_AfterProcessingXml1(IDbCommand safeCommand)
		{
			var nationalCodes = new List<Safe.RefCusTariffNationalCode>();
			safeCommand.CommandText = "SELECT * FROM dbo.RefCusTariffNationalCode";
			using (var reader = safeCommand.ExecuteReader())
			{
				while (reader.Read())
				{
					var nationalCode = new Safe.RefCusTariffNationalCode();
					nationalCode.ZZW_ZZF_NKTaxOrFeeCode = reader[nameof(Safe.RefCusTariffNationalCode.ZZW_ZZF_NKTaxOrFeeCode)].ToString();
					nationalCode.ZZW_StartDate = (DateTime)reader[nameof(Safe.RefCusTariffNationalCode.ZZW_StartDate)];
					nationalCode.ZZW_EndDate = (DateTime)reader[nameof(Safe.RefCusTariffNationalCode.ZZW_EndDate)];
					nationalCodes.Add(nationalCode);
				}
			}
			Assert.That(nationalCodes, Has.Count.EqualTo(1));
			Assert.That(nationalCodes[0].ZZW_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture), Is.EqualTo("2070-06-28 00:00:00"));
			Assert.That(nationalCodes[0].ZZW_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture), Is.EqualTo("2079-06-06 00:00:00"));
			Assert.That(nationalCodes[0].ZZW_ZZF_NKTaxOrFeeCode, Is.EqualTo("ERM"));
		}
		...
		void AssertResult_SafeDb_RefCusTariffNationalCode_AfterProcessingXml2(IDbCommand safeCommand)
		{
			var nationalCodes = new List<Safe.RefCusTariffNationalCode>();
			safeCommand.CommandText = "SELECT * FROM dbo.RefCusTariffNationalCode";
			using (var reader = safeCommand.ExecuteReader())
			...
			safeCommand.CommandText = "SELECT COUNT(*) FROM dbo.RefCusTariffNationalCodeHistory";
			var count = (int)safeCommand.ExecuteScalar();
