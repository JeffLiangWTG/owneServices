...
{
	class Test_Rate_MultiApps : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Rate_MultiApps.xml"];

		public XmlProcessIntegrationTestAssertResultHandler[] AssertResults => [AssertResult_AfterProcessingXml1];

		public string TestDescription => "Test will create duplicate Rate intermittently with more than 3 apps";
...
";
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "U313", "U314", "U315" }, apps.Select(x => x.ZZT_AdditionalCode).Distinct());
		}
