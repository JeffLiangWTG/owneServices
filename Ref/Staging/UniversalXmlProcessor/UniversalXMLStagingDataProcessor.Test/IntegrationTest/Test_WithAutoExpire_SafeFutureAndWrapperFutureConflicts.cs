...
{
	class Test_WithAutoExpire_SafeFutureAndWrapperFutureConflicts : IXmlProcessIntegrationTest
	{
		public string[] FileNames => new string[] { "TestFiles\\Test_SafeFutureAndWrapperFutureConflicts_1.xml", "TestFiles\\Test_SafeFutureAndWrapperFutureConflicts_2.xml" };
		...
		public string TestDescription => "Test both SafeDb and XML contains one future record and future records are conflicting with auto expiration on";
...
";
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusCondition[] conds, IDbCommand safeCommand)
		{
			...
			Assert.That(app_a.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2063-01-01 00:00:00");
