...
{
	class Test_Cond_ValAndLang_1_1 : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Cond_App_WithMultiValAndLang1.xml", "TestFiles\\Test_Cond_App_WithMultiValAndLang2.xml"];
		...
		public string TestDescription => "Test consilidating RefCusCondition(single, haschange)/RefCusApplicability with a single new ConditionValue and ConditionLanguage";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}"), Is.EquivalentTo(new string[] { "ZXJ_PRS", "ZX3_PRS", "ZZ1_QUE", "ZZT_QUE", "ZX1_QUE", "ZX3_QUE", "ZXJ_QUE" }));
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(langs.Select(x => x.ZXJ_ZX6_NKLanguage).Distinct(), Is.EquivalentTo(new string[] { "ITL", "EN" }));
		}
	}
}
