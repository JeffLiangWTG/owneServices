...
{
	class Test_RateUOM_2_1 : IXmlProcessIntegrationTest
	{
		...
		public string TestDescription => "Test consilidating RefCusRate(single, identical)/RefCusApplicability with a single new UOM instead of 2 existing UOM";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "ZXG_PRS", "ZXG_PRS", "ZZ1_QUE", "ZZT_QUE", "ZZ2_QUE", "ZXG_QUE" }, dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}"));
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "MIL", "HLT", "UNB" }, uoms.Select(x => x.ZXG_UOM).Distinct());
		}
	}
}
