...
{
	class Test_Consolidate_Cond_App_MultiCondValue_Lang1 : IXmlProcessIntegrationTest
	{
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "ZX3_PRS", "ZXJ_PRS", "ZZ1_QUE", "ZZT_QUE", "ZX1_QUE", "ZX3_QUE", "ZXJ_QUE" }, dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}").Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusConditionValue_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(vals, Has.Count.EqualTo(4));
			Assert.That(vals.Select(x => x.ZX3_Value).Distinct(), Is.EquivalentTo(new string[] { "C001", "C002", "C003", "C004" }));
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(langs, Has.Count.EqualTo(4));
			Assert.That(langs.Select(x => x.ZXJ_ZX6_NKLanguage).Distinct(), Is.EquivalentTo(new string[] { "FRN", "EN", "ITL", "GRM" }));
		}
	}
}
