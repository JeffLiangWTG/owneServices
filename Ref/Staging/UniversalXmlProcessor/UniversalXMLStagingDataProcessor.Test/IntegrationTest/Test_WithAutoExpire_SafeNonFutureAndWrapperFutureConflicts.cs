...
{
	class Test_WithAutoExpire_SafeNonFutureAndWrapperFutureConflicts : IXmlProcessIntegrationTest
	{
		public string[] FileNames => new string[] { "TestFiles\\Test_SafeNonFutureAndWrapperFutureConflicts_1.xml", "TestFiles\\Test_SafeNonFutureAndWrapperFutureConflicts_2.xml" };
		...
		public string TestDescription => "Test SafeDb has one non-future records that conflicts with XML contains one future record with auto expiration on";
...
";
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusCondition[] conds, IDbCommand safeCommand)
		{
			...
			Assert.That(app_a.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2064-12-31 23:59:00");

			var app_b = apps.FirstOrDefault(x => x.ZZT_AdditionalCode == "b");
			Assert.That(app_b, Is.Not.Null);
			Assert.That(app_b.ZZT_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2065-01-01 00:00:00");
			Assert.That(app_b.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2068-01-01 00:00:00");
		}
	}
}
