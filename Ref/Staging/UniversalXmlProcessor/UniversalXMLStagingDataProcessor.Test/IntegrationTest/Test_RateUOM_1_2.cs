...
{
	class Test_RateUOM_1_2 : IXmlProcessIntegrationTest
	{
		...
		public string TestDescription => "Test consilidating RefCusRate(single, identical)/RefCusApplicability with multiple new UOMs";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "ZXG_QUE", "ZXG_QUE", "ZXG_PRS", "ZZ1_QUE", "ZZ2_QUE", "ZZT_QUE", }, dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}"));
		}
