...
{
	class Test_CondWithAppSeparateMerged : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_CondWithAppSeparateMerged1.xml", "TestFiles\\Test_CondWithAppSeparateMerged2.xml"];
		...
		public string TestDescription => "Test RefCusCondition with multiple RefCusApplicability can be separated or merged";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml1(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults.Select(x => x.DPR_ParentTableCode).Distinct(), Is.EquivalentTo(new string[] { "ZZ1", "ZX1", "ZZT" }));
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			Assert.That(apps.Select(x => $"{x.ZZT_StartDate:yyyy-MM-dd HH:mm:ss}").Distinct(), Is.EquivalentTo(new string[] { "2014-01-01 00:00:00" }));
		...
		void AssertResult_StagingDb_RefCusCondition_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(conds.Select(x => x.ZX1_Comment).Distinct(), Is.EquivalentTo(new[] { "Cond1", "Cond2" }));
			Assert.That(conds.Select(x => x.ZX1_StartDate).Distinct(), Is.EquivalentTo(new[] { new DateTime(2014, 1, 1, 0, 0, 0, DateTimeKind.Utc) }));
			Assert.That(conds.Select(x => x.ZX1_EndDate).Distinct(), Is.EquivalentTo(new[] { new DateTime(2079, 1, 1, 0, 0, 0, DateTimeKind.Utc) }));
		}
		...
		void AssertResult_StagingDb_RefCusApplicability_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(apps.Select(x => x.ZZT_AdditionalCode).Distinct(), Is.EquivalentTo(new[] { "U313", "U314" }));
			Assert.That(apps.Select(x => x.ZZT_EndDate).Distinct(), Is.EquivalentTo(new[] { new DateTime(2029, 6, 6, 23, 59, 0, DateTimeKind.Utc) }));
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusCondition[] conds, IDbCommand safeCommand)
		{
			...
			var app_U313 = apps.FirstOrDefault(x =>
				x.ZZT_ZX1_Conditions == conds.First(y => y.ZX1_Comment == "Cond1").ZX1_PK);
			Assert.That(app_U313, Is.Not.Null);
			Assert.That(app_U313.ZZT_AdditionalCode, Is.EqualTo("U313"));
			Assert.That(app_U313.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2029-06-06 23:59:00");

			var app_U314 = apps.FirstOrDefault(x =>
				x.ZZT_ZX1_Conditions == conds.First(y => y.ZX1_Comment == "Cond2").ZX1_PK);
			Assert.That(app_U314, Is.Not.Null);
			Assert.That(app_U314.ZZT_AdditionalCode, Is.EqualTo("U314"));
			Assert.That(app_U314.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2029-06-06 23:59:00");
		}
	}
}
