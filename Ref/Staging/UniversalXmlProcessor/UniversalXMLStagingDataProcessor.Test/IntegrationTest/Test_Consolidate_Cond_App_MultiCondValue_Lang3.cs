...
{
	class Test_Consolidate_Cond_App_MultiCondValue_Lang3 : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Cond_App_WithMultiValAndLang11.xml", "TestFiles\\Test_Cond_App_WithMultiValAndLang12.xml"];
		...
		public string TestDescription => "Test consilidating RefCusCondition(single)/RefCusApplicability(multiple) with single ConditionValue and single ConditionLanguage";
...
";
		...
		void AssertResult_SafeDb_RefCusConditionValue_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			Assert.That(vals, Has.Count.EqualTo(0));
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			Assert.That(langs, Has.Count.EqualTo(0));
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(langs.Select(x => x.ZXJ_ZX6_NKLanguage).Distinct(), Is.EquivalentTo(new string[] { "FRN" }));
		}
	}
}
