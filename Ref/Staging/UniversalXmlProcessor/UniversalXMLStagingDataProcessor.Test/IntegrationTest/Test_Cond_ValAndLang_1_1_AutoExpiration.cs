...
{
	class Test_Cond_ValAndLang_1_1_AutoExpiration : IXmlProcessIntegrationTest
	{
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults, Has.Count.EqualTo(5));
			Assert.That(dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}"), Is.EquivalentTo(new string[] { "ZZ1_QUE", "ZZT_QUE", "ZX1_QUE", "ZX3_QUE", "ZXJ_QUE" }));
		}
