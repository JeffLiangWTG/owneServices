...
{
	class Test_Cond_App_StartAndEndDate : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Cond_App_StartAndEndDate.xml"];

		public XmlProcessIntegrationTestAssertResultHandler[] AssertResults => [AssertResult_AfterProcessingXml1];

		public string TestDescription => "Test process RefCusConditionApplicabiity with different startDate and endDate from RefCusCondition and RefCusApplicability";
...
";
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "U313", "U314", "U315", "U316" }, apps.Select(x => x.ZZT_AdditionalCode).Distinct());
			...
			var app_U315 = apps.FirstOrDefault(x => x.ZZT_AdditionalCode == "U315");
			Assert.That(app_U315.ZZT_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture), Is.EqualTo("2013-01-01 00:00:00"));
			Assert.That(app_U315.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture), Is.EqualTo("2027-06-06 23:59:00"));

			var app_U316 = apps.FirstOrDefault(x => x.ZZT_AdditionalCode == "U316");
			Assert.That(app_U316.ZZT_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture), Is.EqualTo("2014-01-01 00:00:00"));
			Assert.That(app_U316.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture), Is.EqualTo("2028-06-06 23:59:00"));
		}
	}
}
