...
{
	class Test_DataProcessingClone : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_DataProcessingClone1.xml", "TestFiles\\Test_DataProcessingClone2.xml", "TestFiles\\Test_DataProcessingClone3.xml"];
		...
		public string TestDescription => "Test DataProcessingClone";
...
values (newid(), 'DataProcessingClone A', 1);

";
			stagingCommand.CommandText = stageSql;
		...
		void AssertResult_AfterProcessingXml1(IDbCommand stagingCommand, IDbCommand safeCommand)
		{
			Assert.Multiple(() =>
			{
				...
				AssertResult_StagingDb_RefCusTariff_AfterProcessingXml1(stagingCommand);
				AssertResult_StagingDb_DataProcessingClone_AfterProcessingXml1(stagingCommand);
		...
		void AssertResult_AfterProcessingXml2(IDbCommand stagingCommand, IDbCommand safeCommand)
		{
			Assert.Multiple(() =>
			{
				var sourceDatas = AssertResult_StagingDb_SourceData_SDA_Status_AfterProcessingXml2(stagingCommand);
				AssertResult_StagingDb_RefCusTariff_AfterProcessingXml2(stagingCommand);
				var dprs = AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(stagingCommand);
				AssertResult_StagingDb_DataProcessingClone_AfterProcessingXml2(dprs, sourceDatas, stagingCommand);
		...
		void AssertResult_AfterProcessingXml3(IDbCommand stagingCommand, IDbCommand safeCommand)
		{
			Assert.Multiple(() =>
			{
				var sourceDatas = AssertResult_StagingDb_SourceData_SDA_Status_AfterProcessingXml3(stagingCommand);
				AssertResult_StagingDb_RefCusTariff_AfterProcessingXml3(stagingCommand);
				var dprs = AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml3(stagingCommand);
				AssertResult_StagingDb_DataProcessingClone_AfterProcessingXml3(dprs, sourceDatas, stagingCommand);

				var tariffs = AssertResult_SafeDb_RefCusTariff_AfterProcessingXml3(safeCommand);
				AssertResult_SafeDb_RefCusRate_AfterProcessingXml3(tariffs, safeCommand);
				AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml3(tariffs, safeCommand);
			});
		}
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml1(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults.All(x => x.DPR_SubSource == "DataProcessingClone A"));
		...
		void AssertResult_StagingDb_RefCusTariff_AfterProcessingXml1(IDbCommand stagingCommand)
		{
			...
			using (var reader = stagingCommand.ExecuteReader())
			{
				while (reader.Read())
				{
					...
					tariff.ZZ1_PK = (Guid)reader[nameof(RefCusTariff.ZZ1_PK)];
					tariff.ZZ1_TariffCode = reader[nameof(RefCusTariff.ZZ1_TariffCode)].ToString();
					tariffs.Add(tariff);
				}
			}
		...
		void AssertResult_StagingDb_DataProcessingClone_AfterProcessingXml1(IDbCommand stagingCommand)
		{
			stagingCommand.CommandText = "SELECT count(*) FROM dbo.DataProcessingClone";
			Assert.That((int)stagingCommand.ExecuteScalar(), Is.EqualTo(0));
		}
		...
		SourceData[] AssertResult_StagingDb_SourceData_SDA_Status_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			using (var reader = stagingCommand.ExecuteReader())
			{
				while (reader.Read())
				{
					...
					sourceData.SDA_PK = (Guid)reader[nameof(SourceData.SDA_PK)];
					sourceData.SDA_Status = reader[nameof(SourceData.SDA_Status)].ToString();
					sourceData.SDA_CreatedTime = (DateTime)reader[nameof(SourceData.SDA_CreatedTime)];
		...
		void AssertResult_StagingDb_RefCusTariff_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			var tariffs = new List<RefCusTariff>();
			stagingCommand.CommandText = "SELECT * FROM dbo.RefCusTariff";
			using (var reader = stagingCommand.ExecuteReader())
			{
				while (reader.Read())
				{
					...
					tariff.ZZ1_PK = (Guid)reader[nameof(RefCusTariff.ZZ1_PK)];
					tariff.ZZ1_TariffCode = reader[nameof(RefCusTariff.ZZ1_TariffCode)].ToString();
					tariff.ZZ1_Description = reader[nameof(RefCusTariff.ZZ1_Description)].ToString();
					tariffs.Add(tariff);
				}
			}
			Assert.That(tariffs, Has.Count.EqualTo(2));
			CollectionAssert.AreEquivalent(new string[] { "22030" }, tariffs.Select(x => x.ZZ1_TariffCode).Distinct());
			CollectionAssert.AreEquivalent(new string[] { "Test1" }, tariffs.Select(x => x.ZZ1_Description).Distinct());
		}

		DataProcessingResult[] AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			var dataProcessingResults = new List<DataProcessingResult>();
			stagingCommand.CommandText = "SELECT * FROM dbo.DataProcessingResult";
			using (var reader = stagingCommand.ExecuteReader())
			{
				while (reader.Read())
				{
					...
					dataProcessingResult.DPR_ParentTableCode = reader[nameof(DataProcessingResult.DPR_ParentTableCode)].ToString();
					dataProcessingResult.DPR_Status = reader[nameof(DataProcessingResult.DPR_Status)].ToString();
					dataProcessingResult.DPR_ParentPK = (Guid)reader[nameof(DataProcessingResult.DPR_ParentPK)];
					dataProcessingResults.Add(dataProcessingResult);
				}
			}
			...
			Assert.That(dataProcessingResults.All(x => x.DPR_Status == "QUE"));
			CollectionAssert.AreEquivalent(new string[] { "ZZ1", "ZZ2", "ZZT" }, dataProcessingResults.Select(x => x.DPR_ParentTableCode).Distinct());
			return dataProcessingResults.ToArray();
		}

		void AssertResult_StagingDb_DataProcessingClone_AfterProcessingXml2(DataProcessingResult[] dprs, SourceData[] sourceDatas, IDbCommand stagingCommand)
		{
			var dataProcessingClones = new List<DataProcessingClone>();
			stagingCommand.CommandText = "SELECT * FROM dbo.DataProcessingClone";
			using (var reader = stagingCommand.ExecuteReader())
			{
				while (reader.Read())
				{
					var dataProcessingClone = new DataProcessingClone();
					dataProcessingClone.DPC_NewRecordPK = (Guid)reader[nameof(DataProcessingClone.DPC_NewRecordPK)];
					dataProcessingClone.DPC_ExpiredRecordPK = (Guid)reader[nameof(DataProcessingClone.DPC_ExpiredRecordPK)];
					dataProcessingClone.DPC_TableCode = reader[nameof(DataProcessingClone.DPC_TableCode)].ToString();
					dataProcessingClone.DPC_Status = reader[nameof(DataProcessingClone.DPC_Status)].ToString();
					dataProcessingClone.DPC_SourceId = (Guid)reader[nameof(DataProcessingClone.DPC_SourceId)];
					dataProcessingClones.Add(dataProcessingClone);
				}
			}
			Assert.That(dataProcessingClones, Has.Count.EqualTo(0));
		}
		...
		Safe.RefCusTariff[] AssertResult_SafeDb_RefCusTariff_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			using (var reader = safeCommand.ExecuteReader())
			{
				while (reader.Read())
				{
					...
					tariff.ZZ1_TariffCode = reader[nameof(Safe.RefCusTariff.ZZ1_TariffCode)].ToString();
					tariff.ZZ1_Description = reader[nameof(Safe.RefCusTariff.ZZ1_Description)].ToString();
					tariff.ZZ1_EndDate = (DateTime)reader[nameof(Safe.RefCusTariff.ZZ1_EndDate)];
					tariff.ZZ1_PublishedDate = (DateTime)reader[nameof(Safe.RefCusTariff.ZZ1_PublishedDate)];
					tariffs.Add(tariff);
				}
			}
			Assert.That(tariffs, Has.Count.EqualTo(1));
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusTariff[] tariffs, IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "U414", "U313" }, apps.Select(x => x.ZZT_AdditionalCode).Distinct());
		}

		SourceData[] AssertResult_StagingDb_SourceData_SDA_Status_AfterProcessingXml3(IDbCommand stagingCommand)
		{
			var sourceDatas = new List<SourceData>();
			stagingCommand.CommandText = "SELECT * FROM dbo.SourceData";
			using (var reader = stagingCommand.ExecuteReader())
		...
		void AssertResult_StagingDb_RefCusTariff_AfterProcessingXml3(IDbCommand stagingCommand)
		{
			var tariffs = new List<RefCusTariff>();
			stagingCommand.CommandText = "SELECT * FROM dbo.RefCusTariff";
			using (var reader = stagingCommand.ExecuteReader())
		...
		DataProcessingResult[] AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml3(IDbCommand stagingCommand)
		{
			var dataProcessingResults = new List<DataProcessingResult>();
			stagingCommand.CommandText = "SELECT * FROM dbo.DataProcessingResult";
			using (var reader = stagingCommand.ExecuteReader())
		...
		void AssertResult_StagingDb_DataProcessingClone_AfterProcessingXml3(DataProcessingResult[] dprs, SourceData[] sourceDatas, IDbCommand stagingCommand)
		{
			var dataProcessingClones = new List<DataProcessingClone>();
			stagingCommand.CommandText = "SELECT * FROM dbo.DataProcessingClone";
			using (var reader = stagingCommand.ExecuteReader())
			...
			Assert.That(dataProcessingClones, Has.Count.EqualTo(1));
			Assert.That(dataProcessingClones[0].DPC_TableCode, Is.EqualTo("ZZ1"));
			Assert.That(dataProcessingClones[0].DPC_Status, Is.EqualTo("PRS"));
			Assert.That(dprs.Any(x => x.DPR_ParentPK == dataProcessingClones[0].DPC_NewRecordPK && x.DPR_Status == "QUE"));
			Assert.That(dprs.Any(x => x.DPR_ParentPK == dataProcessingClones[0].DPC_ExpiredRecordPK && x.DPR_Status == "PRS"));
		}

		Safe.RefCusTariff[] AssertResult_SafeDb_RefCusTariff_AfterProcessingXml3(IDbCommand safeCommand)
		{
			var tariffs = new List<Safe.RefCusTariff>();
			safeCommand.CommandText = "SELECT * FROM dbo.RefCusTariff";
			using (var reader = safeCommand.ExecuteReader())
		...
		void AssertResult_SafeDb_RefCusRate_AfterProcessingXml3(Safe.RefCusTariff[] tariffs, IDbCommand safeCommand)
		{
			var rates = new List<Safe.RefCusRate>();
			safeCommand.CommandText = "SELECT * FROM dbo.RefCusRate";
			using (var reader = safeCommand.ExecuteReader())
			...
			CollectionAssert.AreEquivalent(new string[] { "43.92 * [HLT]", "47.48 * [HLT]" }, rates.Select(x => x.ZZ2_RateFormula).Distinct());
			CollectionAssert.AreEquivalent(new DateTime[] { new DateTime(1, 1, 1, 0, 0, 0) }, rates.Select(x => x.ZZ2_StartDate.DateTime).Distinct());
		}

		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml3(Safe.RefCusTariff[] tariffs, IDbCommand safeCommand)
		{
			var apps = new List<Safe.RefCusApplicability>();
			safeCommand.CommandText = "SELECT * FROM dbo.RefCusApplicability";
			using (var reader = safeCommand.ExecuteReader())
			...
			CollectionAssert.AreEquivalent(new string[] { "U414", "U313" }, apps.Select(x => x.ZZT_AdditionalCode).Distinct());
			CollectionAssert.AreEquivalent(new DateTime[] { new DateTime(2029, 7, 6, 23, 59, 0), new DateTime(2029, 6, 6, 23, 59, 0), new DateTime(2022, 3, 17, 23, 59, 0) }, apps.Select(x => x.ZZT_EndDate.DateTime).Distinct());
		}
	}
}
