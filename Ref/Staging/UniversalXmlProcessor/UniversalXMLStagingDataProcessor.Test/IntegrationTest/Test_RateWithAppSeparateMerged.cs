...
{
	class Test_RateWithAppSeparateMerged : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_RateWithAppSeparateMerged1.xml", "TestFiles\\Test_RateWithAppSeparateMerged2.xml"];
		...
		public string TestDescription => "Test RefCusRate with multiple RefCusApplicability can be separated or merged";
...
values (newid(), '1011',' ERGA OMNES1', '1958-01-01 00:00:00', '2079-06-06 23:59:00','EUN');
...
";
		...
		void AssertResult_AfterProcessingXml2(IDbCommand stagingCommand, IDbCommand safeCommand)
		{
			Assert.Multiple(() =>
			{
				...
				var rates = AssertResult_SafeDb_RefCusRate_AfterProcessingXml2(safeCommand);
				AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(rates, safeCommand);
			});
		}
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml1(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults.All(x => x.DPR_SubSource == "RateWithAppSeparateMerged"));
		...
		void AssertResult_SafeDb_RefCusRate_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			Assert.That(rates[0].ZZ2_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture), Is.EqualTo("1900-01-01 00:00:00"));
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "2014-01-01 00:00:00" }, apps.Select(x => $"{x.ZZT_StartDate:yyyy-MM-dd HH:mm:ss}").Distinct());
			CollectionAssert.AreEquivalent(new string[] { "2029-06-06 23:59:00" }, apps.Select(x => $"{x.ZZT_EndDate:yyyy-MM-dd HH:mm:ss}").Distinct());
			CollectionAssert.AreEquivalent(new string[] { "U313", "U319" }, apps.Select(x => x.ZZT_AdditionalCode).Distinct());
		}
		...
		void AssertResult_StagingDb_SourceData_SDA_Status_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { StatusProvider.GetMERStatus() }, sourceDatas.Select(x => x.SDA_Status).Distinct());
		}
		...
		void AssertResult_StagingDb_RefCusRate_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			using (var reader = stagingCommand.ExecuteReader())
			{
				while (reader.Read())
				{
					...
					rate.ZZ2_StartDate = (DateTime)reader[nameof(RefCusRate.ZZ2_StartDate)];
					rate.ZZ2_EndDate = (DateTime)reader[nameof(RefCusRate.ZZ2_EndDate)];
					rates.Add(rate);
				}
			}
			Assert.That(rates, Has.Count.EqualTo(3));
			CollectionAssert.AreEquivalent(new[] { "43.92 * [HLT]", "47.48 * [HLT]" }, rates.Select(x => x.ZZ2_RateFormula).Distinct());
			CollectionAssert.AreEquivalent(new[] { new DateTime(1900, 1, 1, 0, 0, 0, DateTimeKind.Utc), new DateTime(2014, 1, 1, 0, 0, 0, DateTimeKind.Utc) }, rates.Select(x => x.ZZ2_StartDate).Distinct());
			CollectionAssert.AreEquivalent(new[] { new DateTime(2039, 6, 6, 23, 59, 0, DateTimeKind.Utc), new DateTime(2079, 6, 6, 0, 0, 0, DateTimeKind.Utc) }, rates.Select(x => x.ZZ2_EndDate).Distinct());
		}
		...
		void AssertResult_StagingDb_RefCusApplicability_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new[] { "U313", "U319" }, apps.Select(x => x.ZZT_AdditionalCode).Distinct());
			CollectionAssert.AreEquivalent(new[] { new DateTime(2029, 6, 6, 23, 59, 0, DateTimeKind.Utc), new DateTime(2039, 6, 6, 23, 59, 0, DateTimeKind.Utc) }, apps.Select(x => x.ZZT_EndDate).Distinct());
		}
		...
		Safe.RefCusRate[] AssertResult_SafeDb_RefCusRate_AfterProcessingXml2(IDbCommand safeCommand)
		{
			var rates = new List<Safe.RefCusRate>();
			safeCommand.CommandText = "SELECT * FROM dbo.RefCusRate";
			using (var reader = safeCommand.ExecuteReader())
			...
			CollectionAssert.AreEquivalent(new string[] { "1900-01-01 00:00:00" }, rates.Select(x => x.ZZ2_StartDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture)).Distinct());
			CollectionAssert.AreEquivalent(new string[] { "2079-06-06 00:00:00" }, rates.Select(x => x.ZZ2_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture)).Distinct());
			return rates.ToArray();
		}

		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusRate[] rates, IDbCommand safeCommand)
		{
			var apps = new List<Safe.RefCusApplicability>();
			safeCommand.CommandText = "SELECT * FROM dbo.RefCusApplicability";
			using (var reader = safeCommand.ExecuteReader())
			{
				while (reader.Read())
				{
					var app = new Safe.RefCusApplicability();
					app.ZZT_AdditionalCode = reader[nameof(Safe.RefCusApplicability.ZZT_AdditionalCode)].ToString();
					app.ZZT_ZZ2_Rate = (Guid)reader[nameof(Safe.RefCusApplicability.ZZT_ZZ2_Rate)];
					app.ZZT_EndDate = (DateTime)reader[nameof(RefCusApplicability.ZZT_EndDate)];
			...
			Assert.That(apps, Has.Count.EqualTo(2));
			var app_U313 = apps.FirstOrDefault(x =>
				x.ZZT_ZZ2_Rate == rates.First(y => y.ZZ2_RateFormula == "43.92 * [HLT]").ZZ2_PK);
			Assert.That(app_U313, Is.Not.Null);
			Assert.That(app_U313.ZZT_AdditionalCode, Is.EqualTo("U313"));
			Assert.That(app_U313.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2039-06-06 23:59:00");

			var app_U319 = apps.FirstOrDefault(x =>
				x.ZZT_ZZ2_Rate == rates.First(y => y.ZZ2_RateFormula == "47.48 * [HLT]").ZZ2_PK);
			Assert.That(app_U319, Is.Not.Null);
			Assert.That(app_U319.ZZT_AdditionalCode, Is.EqualTo("U319"));
			Assert.That(app_U319.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) == "2029-06-06 23:59:00");
		}
	}
}
