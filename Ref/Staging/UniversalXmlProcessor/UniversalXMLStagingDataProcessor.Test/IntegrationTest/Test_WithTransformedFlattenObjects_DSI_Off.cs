...
{
	class Test_WithTransformedFlattenObjects_DSI_Off : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\TestWithTransformedFlattenObjects1.xml", "TestFiles\\TestWithTransformedFlattenObjects2.xml"];
		...
		public string TestDescription => "Test RefCusRate/RefCusApplicability with NonPersistentObjectTransformer feature";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml1(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults.All(x => x.DPR_SubSource == "IntegrationTest"));
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults, Has.Count.EqualTo(10));
			CollectionAssert.AreEquivalent(new string[] { "ZXG_PRS", "ZXG_QUE", "ZZ1_PRS", "ZZ1_QUE", "ZZ2_PRS", "ZZ2_QUE", "ZZT_PRS", "ZZT_QUE", },
		...
		void AssertResult_SafeDb_RefCusRate_AfterProcessingXml2(Safe.RefCusTariff[] tariffs, IDbCommand safeCommand)
		{
			...
			Assert.That(rateRelatedTariff_22031.ZZ2_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture), Is.EqualTo("2079-06-06 00:00:00"));
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusTariff[] tariffs, IDbCommand safeCommand)
		{
			...
			Assert.That(apps, Has.Count.EqualTo(4));
			CollectionAssert.AreEquivalent(new string[] { "U313", "U314", "U319", "U320" }, apps.Select(x => x.ZZT_AdditionalCode).Distinct());
			var appRelatedTariff22030 = apps.FirstOrDefault(x => x.ZZT_DataSetPK == tariffs.First(y => y.ZZ1_TariffCode == "22030").ZZ1_PK);
			Assert.That(appRelatedTariff22030, Is.Not.Null);
			Assert.That(appRelatedTariff22030.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture), Is.EqualTo("2039-06-06 23:59:00"));

			var appRelatedTariff22031 = apps.FirstOrDefault(x => x.ZZT_DataSetPK == tariffs.First(y => y.ZZ1_TariffCode == "22031").ZZ1_PK);
			Assert.That(appRelatedTariff22031, Is.Not.Null);
			Assert.That(appRelatedTariff22031.ZZT_EndDate.ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture), Is.EqualTo("2029-06-06 23:59:00"));
		}
	}
}
