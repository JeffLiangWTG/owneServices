...
{
	class Test_Cond_ValAndLang_1_2 : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Cond_App_WithMultiValAndLang3.xml", "TestFiles\\Test_Cond_App_WithMultiValAndLang4.xml"];
		...
		public string TestDescription => "Test consilidating RefCusCondition(single, identical)/RefCusApplicability with multiple new ConditionValues/ConditionLanguages";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}"), Is.EquivalentTo(new string[] { "ZX3_PRS", "ZXJ_PRS", "ZZ1_QUE", "ZZT_QUE", "ZX1_QUE", "ZX3_QUE", "ZX3_QUE", "ZXJ_QUE", "ZXJ_QUE" }));
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(langs.Select(x => x.ZXJ_ZX6_NKLanguage).Distinct(), Is.EquivalentTo(new string[] { "ITL", "EN", "FRN" }));
		}
	}
}
