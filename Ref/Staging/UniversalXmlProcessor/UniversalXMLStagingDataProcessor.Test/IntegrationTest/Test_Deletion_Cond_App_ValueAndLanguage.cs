...
{
	class Test_Deletion_Cond_App_ValueAndLanguage : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Deletion_Cond_App_ValueAndLanguage1.xml", "TestFiles\\Test_Deletion_Cond_App_ValueAndLanguage2.xml"];
		...
		public string TestDescription => "Test Deletion on RefCusConditionValue and RefCusConditionLanguage";
...
";
		...
		void AssertResult_SafeDb_RefCusCondition_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(rates.Select(x => x.ZX1_Comment).Distinct(), Is.EquivalentTo(new string[] { "Cond2" }));
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(apps.Select(x => $"{x.ZZT_EndDate:yyyy-MM-dd HH:mm:ss}").Distinct(), Is.EquivalentTo(new string[] { "2029-06-06 23:59:00" }));
			Assert.That(apps.Select(x => x.ZZT_AdditionalCode).Distinct(), Is.EquivalentTo(new string[] { "U314" }));
		}
