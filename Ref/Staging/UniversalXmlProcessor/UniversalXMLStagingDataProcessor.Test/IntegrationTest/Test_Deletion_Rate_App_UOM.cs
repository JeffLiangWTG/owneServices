...
{
	class Test_Deletion_Rate_App_UOM : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Deletion1_Rate_App_UOM.xml", "TestFiles\\Test_Deletion2_Rate_App_UOM.xml"];
		...
		public string TestDescription => "Test Deletion on RefCusRate, RefCusApplicability and RefCusRateUOM";
...
";
		...
		void AssertResult_SafeDb_RefCusRate_AfterProcessingXml2(Safe.RefCusTariff[] tariffs, IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "47.48 * [HLT]" }, rates.Select(x => x.ZZ2_RateFormula).Distinct());
			var rateRelatedToTariff_22030 = rates.FirstOrDefault(x => x.ZZ2_ZZ1_Tariff == tariffs.First(y => y.ZZ1_TariffCode == "22030").ZZ1_PK);
			Assert.That(rateRelatedToTariff_22030, Is.Null);

			var rateRelatedToTariff_22031 = rates.FirstOrDefault(x => x.ZZ2_ZZ1_Tariff == tariffs.First(y => y.ZZ1_TariffCode == "22031").ZZ1_PK);
			Assert.That(rateRelatedToTariff_22031, Is.Not.Null);
			Assert.That(rateRelatedToTariff_22031.ZZ2_EndDate.Date, Is.EqualTo(new DateTime(2079, 6, 6, 0, 0, 0)));
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(Safe.RefCusTariff[] tariffs, IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "U319" }, apps.Select(x => x.ZZT_AdditionalCode).Distinct());
			var appRelatedToTariff_22030 = apps.FirstOrDefault(x => x.ZZT_DataSetPK == tariffs.First(y => y.ZZ1_TariffCode == "22030").ZZ1_PK);
			Assert.That(appRelatedToTariff_22030, Is.Null);

			var appRelatedToTariff_22031 = apps.FirstOrDefault(x => x.ZZT_DataSetPK == tariffs.First(y => y.ZZ1_TariffCode == "22031").ZZ1_PK);
			Assert.That(appRelatedToTariff_22031, Is.Not.Null);
			Assert.That(appRelatedToTariff_22031.ZZT_EndDate.Date, Is.EqualTo(new DateTime(2029, 6, 6, 0, 0, 0)));
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml2(Safe.RefCusTariff[] tariffs, IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "HLT" }, uoms.Select(x => x.ZXG_UOM).Distinct());
			Assert.That(uoms[0].ZXG_DataSetPK == tariffs.First(y => y.ZZ1_TariffCode == "22031").ZZ1_PK);
		}
	}
}
