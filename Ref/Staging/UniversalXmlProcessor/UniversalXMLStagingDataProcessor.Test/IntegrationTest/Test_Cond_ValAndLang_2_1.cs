...
{
	class Test_Cond_ValAndLang_2_1 : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Cond_App_WithMultiValAndLang5.xml", "TestFiles\\Test_Cond_App_WithMultiValAndLang6.xml"];
		...
		public string TestDescription => "Test consilidating RefCusCondition(single, identical)/RefCusApplicability with a single new ConditionValue/ConditionLanguage instead of 2 existing ones";
...
";
		...
		void AssertResult_SafeDb_RefCusCondition_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "Cond1" }, conds.Select(x => x.ZX1_Comment).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			Assert.That(langs.Select(x => x.ZXJ_ZX6_NKLanguage).Distinct(), Is.EquivalentTo(new string[] { "ITL", "FRN" }));
		}
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			Assert.That(dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}").Distinct(), Is.EquivalentTo(new string[] { "ZX3_PRS", "ZXJ_PRS", "ZZ1_QUE", "ZZT_QUE", "ZX1_QUE", "ZX3_QUE", "ZXJ_QUE" }));
		}
		...
		void AssertResult_SafeDb_RefCusConditionValue_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(vals.Select(x => x.ZX3_Value).Distinct(), Is.EquivalentTo(new string[] { "C001", "C002", "C003" }));
		}
		...
		void AssertResult_SafeDb_RefCusConditionLanguage_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(langs, Has.Count.EqualTo(3));
			Assert.That(langs.Select(x => x.ZXJ_ZX6_NKLanguage).Distinct(), Is.EquivalentTo(new string[] {"ITL", "FRN", "EN" }));
		}
	}
}
