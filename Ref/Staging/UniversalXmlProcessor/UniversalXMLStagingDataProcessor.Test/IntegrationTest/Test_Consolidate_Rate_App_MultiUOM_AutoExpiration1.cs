...
{
	class Test_Consolidate_Rate_App_MultiUOM_AutoExpiration1 : IXmlProcessIntegrationTest
	{
		...
		public string TestDescription => "Test consilidating RefCusRate(multiple, identical)/RefCusApplicability with multiple UOM when auto expiration is on";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] {"ZZ1_QUE", "ZZT_QUE", "ZZ2_QUE", "ZXG_QUE" }, dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}").Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(uoms, Has.Count.EqualTo(2));
			CollectionAssert.AreEquivalent(new string[] { "TONE", "LITER" }, uoms.Select(x => x.ZXG_UOM).Distinct());
		}
	}
}
