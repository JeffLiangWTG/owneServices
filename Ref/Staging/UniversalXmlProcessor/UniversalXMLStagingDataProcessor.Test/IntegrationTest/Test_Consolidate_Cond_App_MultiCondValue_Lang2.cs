...
{
	class Test_Consolidate_Cond_App_MultiCondValue_Lang2 : IXmlProcessIntegrationTest
	{
		...
		public string TestDescription => "Test consilidating RefCusCondition(multiple, identical)/RefCusApplicability with multiple ConditionValue and ConditionLanguage";
...
";
		...
		void AssertResult_SafeDb_RefCusConditionValue_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(vals.Select(x => x.ZX3_Value).Distinct(), Is.EquivalentTo(new string[] { "C001", "C003", "C004" }));
		}
