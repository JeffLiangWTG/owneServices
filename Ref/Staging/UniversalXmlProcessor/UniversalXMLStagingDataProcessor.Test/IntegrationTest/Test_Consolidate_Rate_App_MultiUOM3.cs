...
{
	class Test_Consolidate_Rate_App_MultiUOM3 : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Rate_App_WithMultiUOM11.xml", "TestFiles\\Test_Rate_App_WithMultiUOM12.xml"];
		...
		public string TestDescription => "Test consilidating RefCusRate(single)/RefCusApplicability(multiple) with single UOM";
...
";
		...
		void AssertResult_AfterProcessingXml2(IDbCommand stagingCommand, IDbCommand safeCommand)
		{
			Assert.Multiple(() =>
			{
				...
				AssertResult_SafeDb_RefCusExcludedTradeGroup_AfterProcessingXml2(safeCommand);
			});
		}
		...
		void AssertResult_SafeDb_RefCusRate_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "10 * VFD" }, rates.Select(x => x.ZZ2_RateFormula).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "AU", "CN" }, apps.Select(x => x.ZZT_AdditionalCode).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			Assert.That(uoms, Has.Count.EqualTo(0));
		}
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "ZZ1", "ZZ2", "ZZT", "ZXG", "ZZC" }, dataProcessingResults.Select(x => x.DPR_ParentTableCode).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusRate_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(rates, Has.Count.EqualTo(1));
			Assert.That(rates[0].ZZ2_RateFormula, Is.EqualTo("15 * VFD"));
		}
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(apps, Has.Count.EqualTo(2));
			CollectionAssert.AreEquivalent(new string[] { "AU", "CN" }, apps.Select(x => $"{x.ZZT_AdditionalCode}"));
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "TONE" }, uoms.Select(x => x.ZXG_UOM).Distinct());
		}

		void AssertResult_SafeDb_RefCusExcludedTradeGroup_AfterProcessingXml2(IDbCommand safeCommand)
		{
			var exes = new List<Safe.RefCusExcludedTradeGroup>();
			...
			Assert.That(exes, Has.Count.EqualTo(2));
		}
	}
}
