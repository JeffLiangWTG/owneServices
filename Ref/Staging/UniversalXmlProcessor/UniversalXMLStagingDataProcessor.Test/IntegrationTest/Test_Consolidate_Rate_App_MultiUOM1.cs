...
{
	class Test_Consolidate_Rate_App_MultiUOM1 : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Rate_App_WithMultiUOM7.xml", "TestFiles\\Test_Rate_App_WithMultiUOM8.xml"];
		...
		public string TestDescription => "Test consilidating RefCusRate(multiple, identical)/RefCusApplicability with multiple UOM";
...
";
		...
		void AssertResult_SafeDb_RefCusRate_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "10 * VFD", "5 * VFD" }, rates.Select(x => x.ZZ2_RateFormula).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "KG", "G" }, uoms.Select(x => x.ZXG_UOM).Distinct());
		}
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "ZXG_PRS", "ZZ1_QUE", "ZZ2_QUE", "ZZT_QUE", "ZXG_QUE" }, dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}").Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusRate_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "15 * VFD" }, rates.Select(x => $"{x.ZZ2_RateFormula}").Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			Assert.That(uoms, Has.Count.EqualTo(4));
			CollectionAssert.AreEquivalent(new string[] { "KG", "G", "TONE", "LITER" }, uoms.Select(x => x.ZXG_UOM).Distinct());
		}
	}
}
