...
{
	class Test_Consolidate_Rate_App_MultiUOM_AutoExpiration2 : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Rate_App_WithMultiUOM9.xml", "TestFiles\\Test_Rate_App_WithMultiUOM10.xml"];
		...
		public string TestDescription => "Test consilidating RefCusRate(single, identical)/RefCusApplicability with multiple UOM when auto expiration is on";
...
";
		...
		void AssertResult_SafeDb_RefCusApplicability_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "AU" }, apps.Select(x => x.ZZT_AdditionalCode).Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "KG" }, uoms.Select(x => x.ZXG_UOM).Distinct());
		}
