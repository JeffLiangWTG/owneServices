...
{
	class Test_RefCusTariffUOM_WithoutStartEndDate : IXmlProcessIntegrationTest
	{
		...
		public string[] FileNames => ["TestFiles\\RefCusTariffUOM_WithoutStartEndDate.xml"];

		public string TestDescription => "Test RefCusTariffUOM without EnableExpirable='true' can be merged successfully when existing record has start/end date";
...
values (newid(), '{tariffPK}', 'CU1', 'NAR', '{tradeGroupPK1}', 'FR', '2017-01-01 00:00:00.000', '2025-12-31 23:59:00.000');
";
...
WHERE ZZ1_TariffCode='0101210000' AND ZZ1_ZZZ_NKDataGrouping='GB'";
			...
			Assert.AreEqual(2, tariffUOMs.Count);
			Assert.True(string.IsNullOrEmpty(tariffUOMs.First(x => x.ZZ8_Type == "CU1").ZZ8_ZZA_NKTradeGroup));
			Assert.AreEqual("1011", tariffUOMs.First(x => x.ZZ8_Type == "CU2").ZZ8_ZZA_NKTradeGroup);
		}
		...
		void AssertResult_SafeDb_RefCusTariff_AfterProcessingXml1(IDbCommand safeCommand)
		{
			...
			Assert.AreEqual("0101210000", tariffCode);
			Assert.AreEqual(tariffTypePK, tariffType);
		}
...
WHERE ZZ1_TariffCode IN ('0101210000','8523520000') AND ZZ1_ZZZ_NKDataGrouping='GB'";
			...
			Assert.AreEqual(2, tariffUOMs_0101210000.Count);
			Assert.IsNull(tariffUOMs_0101210000.First(x => x.ZZ8_Type == "CU1").ZZ8_ZZA_TradeGroup);
			Assert.AreEqual(tradeGroupPK1, tariffUOMs_0101210000.First(x => x.ZZ8_Type == "CU2").ZZ8_ZZA_TradeGroup);

			var tariffUOMs_8523520000 = allTariffUOMs.Where(x => x.ZZ8_ZZ1_Tariff == tariffPK).ToList();
			Assert.AreEqual(2, tariffUOMs_0101210000.Count);
			Assert.That(tariffUOMs_8523520000.First(x => x.ZZ8_Type == "CU1").ZZ8_StartDate.Value.DateTime, Is.EqualTo(new DateTime(2017, 1, 1)));
			Assert.That(tariffUOMs_8523520000.First(x => x.ZZ8_Type == "CU1").ZZ8_EndDate.Value.DateTime, Is.EqualTo(new DateTime(2025, 12, 31, 23, 59, 0)));
			Assert.That(tariffUOMs_8523520000.First(x => x.ZZ8_Type == "CU2").ZZ8_StartDate, Is.Null);
			Assert.That(tariffUOMs_8523520000.First(x => x.ZZ8_Type == "CU2").ZZ8_EndDate, Is.Null);
		}
	}
}
