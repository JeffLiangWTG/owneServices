...
{
	class Test_RateUOM_2_1_AutoExpiration : IXmlProcessIntegrationTest
	{
		public string[] FileNames => ["TestFiles\\Test_Rate_App_WithMultiUOM5.xml", "TestFiles\\Test_Rate_App_WithMultiUOM6.xml"];
		...
		public string TestDescription => "Test consilidating RefCusRate(single, identical)/RefCusApplicability with a single new UOM instead of 2 existing UOM when auto expiration is on";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] {"ZZ1_QUE", "ZZT_QUE", "ZZ2_QUE", "ZXG_QUE" }, dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}"));
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "UNB" }, uoms.Select(x => x.ZXG_UOM).Distinct());
		}
	}
}
