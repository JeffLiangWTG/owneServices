...
{
	class Test_Consolidate_Rate_App_MultiUOM2 : IXmlProcessIntegrationTest
	{
		...
		public string TestDescription => "Test consilidating RefCusRate(single, identical)/RefCusApplicability with multiple UOM";
...
";
		...
		void AssertResult_StagingDb_DataProcessingResult_AfterProcessingXml2(IDbCommand stagingCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "ZXG_PRS", "ZZ1_QUE", "ZZT_QUE", "ZZ2_QUE", "ZXG_QUE" }, dataProcessingResults.Select(x => $"{x.DPR_ParentTableCode}_{x.DPR_Status}").Distinct());
		}
		...
		void AssertResult_SafeDb_RefCusRateUOM_AfterProcessingXml2(IDbCommand safeCommand)
		{
			...
			CollectionAssert.AreEquivalent(new string[] { "KG", "TONE", "LITER" }, uoms.Select(x => x.ZXG_UOM).Distinct());
		}
	}
}
