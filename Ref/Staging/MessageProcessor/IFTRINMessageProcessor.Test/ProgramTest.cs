...
namespace CargoWise.RefDbRepo.Staging.IFTRINMessageProcessor.Test
{
	...
	class ProgramTest
	{
		...
		{
			AssertArgumentError("X!", "There is no IFTRIN Processor support for 'X!'.");
		}
		...
		void AssertArgumentError(string parameters, string expectedErrorMessage)
		{
			...
			var processStartInfo = new ProcessStartInfo(Path.Combine(BinFilesPath, "CargoWise.RefDbRepo.Staging.IFTRINMessageProcessor.exe"), parameters)
			{
