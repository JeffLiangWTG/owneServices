...
{
	[TestFixture]
	public class RefComplianceCommodityAlertFixture
	{
		static short DataSetId => Helper.GetDataSetId(DataSet.RefComplianceCommodityAlert);
		static string TblPrefix => "RRC";

		[Test]
		public void GetLatest_RefComplianceCommodityAlert_Data()
		{
			var dataSet1 = repo.Create(() => new RefComplianceCommodityAlert
			{
				RCR_PK = new Guid("58E9ADF1-400C-4E5F-9439-D57C0C83063F"),
				RCR_AlertCode = "AA",
				RCR_CountryRegion = "AA",
				RCR_TradeDirection = "EXP"
			});
			repo.Create(() => new RefDbVersionControl
			{
				RVC_LastUpdatedUTC = Now,
				RVC_DataSetId = DataSetId,
				RVC_ParentPK = dataSet1.RCR_PK,
				RVC_ParentCode = TblPrefix,
				RVC_IsPublished = true
			});
			var dataSets = GetDataSets(null, 0, null, null, DataSetId);
			var result = dataSets.ElementAt(0);
			Assert.That(result.RCR_AlertCode, Is.EqualTo(dataSet1.RCR_AlertCode));
		}
		...
		public void GetLatest_FilterCheckpoint_Data(string checkpointPK, string[] expected)
		{
			...
			Assert.That(dataSets.Select(x => x.RCR_AlertCode).ToArray(), Is.EqualTo(expected));
		}
		...
		RefComplianceCommodityAlert CreateRepairCode(string code, DateTime dateTime, Guid? codePK = null)
		{
			var result = repo.Create(() => new RefComplianceCommodityAlert
			{
				RCR_PK = codePK ?? Guid.NewGuid(),
				RCR_AlertCode = code
			});
			repo.Create(() => new RefDbVersionControl
			{
		...
		IEnumerable<Models.RefComplianceCommodityAlert> GetDataSets(int? lowerTimestampOffset, int upperTimestampOffset, ICheckpoint checkpoint, int? chunkSize, short datasetId)
		{
			...
			var service = new RefComplianceCommodityAlertService(repo);
			return service.GetData(lowerTimestamp, upperTimestamp, checkpoint, chunkSize, DataSetId).OrderBy(x => x.RCR_AlertCode);
		}
