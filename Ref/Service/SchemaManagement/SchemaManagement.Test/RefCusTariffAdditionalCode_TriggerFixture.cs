...
class RefCusTariffAdditionalCode_TriggerFixtures
{
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Insert_ThrowException_1()
	{
...
";
	...
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Insert_ThrowException_2()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
...
";
	...
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Insert_ThrowException_3()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
...
";
	...
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Insert_DoesNot_ThrowException_1()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
...
";
	...
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Update_ThrowException_1()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
...
";
	...
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Update_ThrowException_2()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
...
";
	...
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Update_ThrowException_3()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
...
";
	...
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Update_ThrowException_4()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
...
";
	...
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Update_DoesNot_ThrowException_1()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
...
";
	...
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Insert_DoesNot_ThrowException_2()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
...
";
	...
	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Insert_DoesNot_ThrowException_3()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
		DBHelper.ExecuteNonQuery(conn, PrepareDataScript);
		var sql = @"
UPDATE RefCusTariffAdditionalCode SET ZY2_ZZ1_Tariff = 'FDCA07B0-8E98-4A44-A609-D441FCF0E585' WHERE ZY2_PK = '18D35FB8-30A0-48F8-8A9F-45404BF7927B'
";
		DBHelper.AssertDoesNotThrowsSqlException(conn, sql);
	}

	[Test]
	public void RefCusTariffAdditionalCode_Trigger_Insert_DoesNot_ThrowException_4()
	{
		var dbName = TransactionedTestCaseAttribute.GetDbName(DbSchema.RefDbRepoSafe);
		using var conn = new SqlConnection(TestConnectionString.GetAdmin(dbName));
		conn.Open();
		DBHelper.ExecuteNonQuery(conn, PrepareDataScript);
		var sql = @"
UPDATE RefCusTariffAdditionalCode SET ZY2_ZZ1_Tariff = 'FDCA07B0-8E98-4A44-A609-D441FCF0E585' WHERE ZY2_PK = '7558A41B-F5AE-46A0-AB6D-510117207DFF'
";
		DBHelper.AssertDoesNotThrowsSqlException(conn, sql);
	}

	const string ExceptionMessage = "A RefCusTariffAdditionalCode was inserted/updated with invalid values in ZY2_ParentAdditionalCode or ZY2_ZY3_NKParentCategory which are not found in RefCusTariffAdditionalCode.ZY2_ZZ1_Tariff or RefCusTariffAdditionalCode.ZY2_ZZW_NationalCode having matched ZY2_AdditionalCode and ZY2_ZY3_NKCategory.";
