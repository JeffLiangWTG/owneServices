...
{
	[TestFixture]
	class RefAirlineCommodityCodeUpdaterInfo_3Fixture
	{
		[TransactionedTestCase]
		[TestCaseSource(typeof(TransactionedTestCaseAttribute), nameof(TransactionedTestCaseAttribute.RemoteDbTestCases))]
		public void RefAirlineCommodityCodeUpdaterInfo_3(DbSchema dbSchema)
		{
	...
	INSERT RefAirlineCommodityCode (RAC_PK,RAC_AirlineID,RAC_Code,RAC_Description,RAC_SpecialHandlingCodes)
	VALUES ('8B78EF0A-B8D2-4083-871D-CBAE3C055FC7', 'A', 'AAA', 'AAAAAA', 'AAAAAA'),
	('8B78EF0A-B8D2-4083-871D-CBAE3C055FC0', 'B', 'BB', 'BBBBB', 'BBBBB')
	...
	", trans);
					...
					Assert.AreEqual(1,
						TestDBHelper.ExecuteScalar(conn,
							@"SELECT COUNT(*) FROM RefAirlineCommodityCode WHERE RAC_AirlineID = 'A' AND RAC_SpecialHandlingCodes = 'AAAAAA'",
							trans));
					Assert.AreEqual(1,
						TestDBHelper.ExecuteScalar(conn,
					...
					var info = new RefAirlineCommodityCodeUpdaterInfo_3();
					TestDBHelper.ExecuteNonQuery(conn, info.PrepareTemporaryTablesScripts.FirstOrDefault().Value,
						trans);
					TestDBHelper.ExecuteNonQuery(conn, $@"
	INSERT {info.PrepareTemporaryTablesScripts.FirstOrDefault().Key} (RAC_PK,RAC_AirlineID,RAC_Code,RAC_Description,RAC_SpecialHandlingCodes,Deleted)
	VALUES (NEWID(), 'A', 'AAA', '111', '111', 0),
	(NEWID(), 'B', 'BB', 'BBBBB', 'BBBBB', 1),
	(NEWID(), 'C', 'CC', 'CCCCC', 'CCCCC', 0)
	", trans);
					...
					Assert.AreEqual(1,
						TestDBHelper.ExecuteScalar(conn,
							@"SELECT COUNT(*) FROM RefAirlineCommodityCode WHERE RAC_AirlineID = 'A' AND RAC_SpecialHandlingCodes = '111'",
							trans));
					Assert.AreEqual(1,
						TestDBHelper.ExecuteScalar(conn,
							@"SELECT COUNT(*) FROM RefAirlineProductCodeCommodityCodePivot JOIN RefAirlineCommodityCode ON RPC_RAC = RAC_PK WHERE RAC_AirlineID = 'A'",
