...
{
	[TestFixture]
	class RefCusProfileUpdaterInfo_1Fixture
	{
		[TestCase(DbSchema.RemoteDbCollationCS)]
		[TestCase(DbSchema.RemoteDb)]
		[TransactionedTestCase]
		public void RefCusProfileUpdaterInfo_1(DbSchema dbSchema)
		{
...
VALUES ('9EC9F58A-0DE4-48AA-8081-18A45E5E216F','1799FE44-08BE-42CF-B130-E89921BC3E85','1001','QC1','2024-01-01 00:00:00.000','2079-06-06 23:59:00.000','ZA'), --keep
('6EDA47B3-F6E3-4FAE-819E-4CD5BC107E98','D272AC2C-C3CB-4981-851E-764899C55D73','1002','QC2','2024-01-01 00:00:00.000','2079-06-06 23:59:00.000','ZA'),           --update
('53404306-8565-4655-BFFC-1C96DAAB754E','D272AC2C-C3CB-4981-851E-764899C55D73','1003','QC3','2024-01-01 00:00:00.000','2079-06-06 23:59:00.000','ZA');           --delete
...
", trans);
					...
					var info = new RefCusProfileUpdaterInfo_1();
					foreach (var script in info.PrepareTemporaryTablesScripts)
					{
						TestDBHelper.ExecuteNonQuery(conn, script.Value, trans);
					}
					TestDBHelper.ExecuteNonQuery(conn, $@"
INSERT INTO #TempRefCusProfile (XX0_PK,XX0_XXX_ProfileType,XX0_TariffCode,XX0_QuestionCode,XX0_StartDate,XX0_EndDate,XX0_ZZZ_NKDataGrouping,Deleted)
VALUES ('62319CF2-095E-4FEF-BB63-090D7303155C','1799FE44-08BE-42CF-B130-E89921BC3E85','2001','QC21','2024-01-01 00:00:00.000','2079-06-06 23:59:00.000','ZA',0),                --insert
('1B540903-4156-4AA7-AB4F-B376866F640E','D272AC2C-C3CB-4981-851E-764899C55D73','1002 Update','QC2 Update','2024-01-01 00:00:00.000','2079-06-06 23:59:00.000','ZA',0),   --update
('B487B019-E207-4ADA-8839-2B1691E53FA9','D272AC2C-C3CB-4981-851E-764899C55D73','1003','QC3','2024-01-01 00:00:00.000','2079-06-06 23:59:00.000','ZA',1);                           --delete
...
", trans);
					...
					Assert.AreEqual(1, TestDBHelper.ExecuteScalar(conn, "SELECT COUNT(1) FROM RefCusProfile WHERE XX0_XXX_ProfileType = '1799FE44-08BE-42CF-B130-E89921BC3E85' AND XX0_TariffCode = '2001' AND XX0_QuestionCode = 'QC21'", trans));
					...
					Assert.AreEqual(1, TestDBHelper.ExecuteScalar(conn, "SELECT COUNT(1) FROM RefCusProfile WHERE XX0_TariffCode = '1002 Update' AND XX0_QuestionCode = 'QC2 Update'", trans));
					Assert.AreEqual(1, TestDBHelper.ExecuteScalar(conn, "SELECT COUNT(1) FROM RefCusProfileAttribute JOIN RefCusProfile ON XXY_XX0_Profile = XX0_PK WHERE XX0_TariffCode = '1002 Update' AND XX0_QuestionCode = 'QC2 Update'", trans));
					Assert.AreEqual(0, TestDBHelper.ExecuteScalar(conn, "SELECT COUNT(1) FROM RefCusProfile WHERE XX0_TariffCode = '1003' AND XX0_QuestionCode = 'QC3'", trans));
				}
			}
		}
	}
