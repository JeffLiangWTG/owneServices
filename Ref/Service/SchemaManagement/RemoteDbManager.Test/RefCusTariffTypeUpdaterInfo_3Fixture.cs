...
", trans);
					var info = new RefCusTariffTypeUpdaterInfo_3();
...
", trans);
					...
					Assert.AreEqual(0, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffTypeLanguage WHERE ZXK_Description = 'AAA'", trans));
					Assert.AreEqual(0, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffRelationship WHERE ZZH_TariffCode = '1111111'", trans));

					Assert.AreEqual(0, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffBRCharacteristic WHERE ZB1_CharacteristicType = 'NVE' AND ZB1_Code = 'AA'", trans));
					Assert.AreEqual(0, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffBRCharacteristicValue WHERE ZB2_Value = 'AAA' AND ZB2_Description = 'AA'", trans));
					Assert.AreEqual(0, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffBRCharacteristicAttribute WHERE ZB3_Name = 'AAA' AND ZB3_Code = 'AA'", trans));

					Assert.AreEqual(1, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffType WHERE ZZI_TariffType = 'BBB' AND ZZI_Description = 'XBB'", trans));
					Assert.AreEqual(1, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffTypeLanguage WHERE ZXK_Description = 'XBB'", trans));
					Assert.AreEqual(1, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffType WHERE ZZI_TariffType = 'CCC'", trans));
					Assert.AreEqual(1, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffTypeLanguage WHERE ZXK_Description = 'CCC'", trans));
					Assert.AreEqual(1, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffType WHERE ZZI_TariffType = 'DDD'", trans));
					Assert.AreEqual(0, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefCusTariffTypeLanguage WHERE ZXK_Description = 'DDD'", trans));
				}
			}
		}
	}
