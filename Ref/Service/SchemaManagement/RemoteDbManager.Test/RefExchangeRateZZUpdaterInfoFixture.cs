...
{
	[TestFixture]
	class RefExchangeRateZZUpdaterInfoFixture
	{
...
VALUES (NEWID(), 'CUS', '2017/1/1', '2017/12/12', 3, 'A', 'A', 1),
(NEWID(), 'CUS', '2017/1/1', '2017/12/12', 3, 'B', 'B', 1)", trans);
					var info = new RefExchangeRateZZUpdaterInfo_2();
...
VALUES (NEWID(), 'CUS', '2017/1/1', '2017/12/12', 9, 'A', 'A', 1, 0),
(NEWID(), 'CUS', '2017/1/1', '2017/12/12', 3, 'B', 'B', 1, 1),
(NEWID(), 'CUS', '2017/1/1', '2017/12/12', 3, 'C', 'C', 1, 0)", trans);
					TestDBHelper.ExecuteNonQuery(conn, info.MergeScript, trans);
					Assert.AreEqual(1, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefExchangeRateZZ WHERE ZZN_RX_NKExCurrency = 'C'", trans));
					Assert.AreEqual(0, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefExchangeRateZZ WHERE ZZN_RX_NKExCurrency = 'B'", trans));
					Assert.AreEqual(1, TestDBHelper.ExecuteScalar(conn, @"SELECT COUNT(*) FROM RefExchangeRateZZ WHERE ZZN_RX_NKExCurrency = 'A' AND ZZN_Rate = 9", trans));
				}
			}
		}
	}
