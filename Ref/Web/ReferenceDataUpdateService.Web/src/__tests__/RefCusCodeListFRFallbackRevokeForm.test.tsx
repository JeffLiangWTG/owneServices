...
describe("<RefCusCodeListFRFallbackRevokeForm />", () => {
	...
	it("render", async () => {
		let wrapper = shallow<RefCusCodeListFRFallbackRevokeForm>(<RefCusCodeListFRFallbackRevokeForm entityManager={entityManager.object} />);
		expect(wrapper.find(DateTimeInput).at(0).props().propertyName).toEqual("revokingAllDate");
	});

	it("render data", async () => {
		var wrapper = shallow<RefCusCodeListFRFallbackRevokeForm>(<RefCusCodeListFRFallbackRevokeForm entityManager={entityManager.object} />);
		await wrapper.instance().loadListRefCusCode();
		wrapper.instance().state.views.forEach(element => {
			element.ZZD_Edit = false;
		});
		expect(wrapper.find("tbody > tr")).toHaveLength(2);
		expect(wrapper.find("tbody > tr").at(0).html()).toContain("<td>BLABLAT</td><td>DELTAT</td><td><button type=\"button\" class=\"btn btn-info btn-sm\">Edit</button></td><td><button type=\"button\" class=\"btn btn-info btn-sm\" disabled=\"\">Save</button></td></tr>");
	});
});
