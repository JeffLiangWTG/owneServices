...
{
	class ThetaEnvParameterCreationStrategy : IParameterCreationStrategy
	{
		public List<(string paramName, object paramValue)> CreateParameters(TestRig testRig)
		{
			var stagingDeployPassword = DATCrypto.Decrypt(Passwords.Theta_StagingDeployPassword, true);
			var deliveryServiceDeployPassword = DATCrypto.Decrypt(Passwords.Theta_DeliveryServiceDeployPassword, true);
			return new List<(string, object)>
			{
				("xmlProducersDestination", @"XMLProducersDeployTheta\"),
				("stagingDestination", @"StagingDeployTheta\"),
				...
				("quartzServiceName", "RefDbRepoTheta Quartz Server"),
				("refDbRepoSafeDbName", "RefDbRepoSafeTheta"),
				("refDbRepoStagingDbName", "RefDbRepoStagingTheta"),
