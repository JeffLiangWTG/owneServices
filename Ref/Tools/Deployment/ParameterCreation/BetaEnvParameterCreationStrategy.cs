...
{
	class BetaEnvParameterCreationStrategy : IParameterCreationStrategy
	{
		public List<(string paramName, object paramValue)> CreateParameters(TestRig testRig)
		{
			var stagingDeployPassword = DATCrypto.Decrypt(Passwords.Beta_StagingDeployPassword, true);
			var deliveryServiceDeployPassword = DATCrypto.Decrypt(Passwords.Beta_DeliveryServiceDeployPassword, true);
			return new List<(string, object)>
			{
				("xmlProducersDestination", @"XMLProducersDeployBeta\"),
				("stagingDestination", @"StagingDeployBeta\"),
				...
				("quartzServiceName", "RefDbRepoBeta Quartz Server"),
				("refDbRepoSafeDbName", "RefDbRepoSafeBeta"),
				("refDbRepoStagingDbName", "RefDbRepoStagingBeta"),
