...
{
	class AlphaEnvParameterCreationStrategy : IParameterCreationStrategy
	{
		public List<(string paramName, object paramValue)> CreateParameters(TestRig testRig)
		{
			var stagingDeployPassword = DATCrypto.Decrypt(Passwords.Alpha_StagingDeployPassword, true);
			var deliveryServiceDeployPassword = DATCrypto.Decrypt(Passwords.Alpha_DeliveryServiceDeployPassword, true);
			return new List<(string, object)>
			{
				("xmlProducersDestination", @"XMLProducersDeployAlpha\"),
				("stagingDestination", @"StagingDeployAlpha\"),
				...
				("quartzServiceName", "RefDbRepoAlpha Quartz Server"),
				("refDbRepoSafeDbName", "RefDbRepoSafeAlpha"),
				("refDbRepoStagingDbName", "RefDbRepoStagingAlpha"),
