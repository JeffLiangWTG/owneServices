...
{
	class EpsilonEnvParameterCreationStrategy : IParameterCreationStrategy
	{
		public List<(string paramName, object paramValue)> CreateParameters(TestRig testRig)
		{
			var stagingDeployPassword = DATCrypto.Decrypt(Passwords.Epsilon_StagingDeployPassword, true);
			var deliveryServiceDeployPassword = DATCrypto.Decrypt(Passwords.Epsilon_DeliveryServiceDeployPassword, true);
			return new List<(string, object)>
			{
				("xmlProducersDestination", @"XMLProducersDeployEpsilon\"),
				("stagingDestination", @"StagingDeployEpsilon\"),
				...
				("quartzServiceName", "RefDbRepoEpsilon Quartz Server"),
				("refDbRepoSafeDbName", "RefDbRepoSafeEpsilon"),
				("refDbRepoStagingDbName", "RefDbRepoStagingEpsilon"),
