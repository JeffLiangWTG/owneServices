...
{
	[TestFixture]
	public class RefCusProcedureCodesLoaderTest
	{
		[Test]
		[TestCase(
		...
		{
			var refCusProcedureList = RefCusProcedureCodesLoader.LoadData(DataFileName);
			Assert.IsNotNull(refCusProcedureList, "refCusProcedureList should not be null after loading data.");
			...
			var refCusProcedureAttribute = refCusProcedure.RefCusProcedureAttributes.FirstOrDefault();
			Assert.IsNotNull(refCusProcedureAttribute, "Attribute entry is missing");
