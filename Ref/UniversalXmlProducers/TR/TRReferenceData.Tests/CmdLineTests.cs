...
namespace CargoWise.RefDbRepo.TRReferenceData.Tests.CmdLine
{
	[TestFixture]
	class CmdLineTests
	{
		...
		internal static void AssertArgumentError(string parameters, string expectedErrorMessage)
		{
			...
			var processStartInfo = new ProcessStartInfo(Path.Combine(binPath, "CargoWise.RefDbRepo.TRReferenceData.CmdLine.exe"), parameters)
			{
