...
{
	public sealed class TariffDataRepo : ITopLevelDataRepo<RefCusTariff>
	{
		public TariffDataRepo()
		{
		...
		public void Sort()
		{
			...
			{
				tariff.RefCusRates = tariff.RefCusRates?
					.OrderBy(r => r.ZZ2_ZY1_ZZR_NKRateType)
					.ThenBy(r => r.ZZ2_ZZS_NKPreference ?? string.Empty)
					.ThenBy(r => r.RefCusApplicabilities?.FirstOrDefault()?.ZZT_EndDate ?? Constants.MaxSmallDateTime)
					.ToArray();
			}
		}
