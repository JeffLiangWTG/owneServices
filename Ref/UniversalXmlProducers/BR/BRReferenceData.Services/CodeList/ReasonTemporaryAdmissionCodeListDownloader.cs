...
{
	public class ReasonTemporaryAdmissionCodeListDownloader
	{
		public byte[] DownloadXML(HttpClient client)
		{
			int downloadRetries = 0;
			do
			{
				var bXMl = Download(client);
				var xml = System.Text.Encoding.UTF8.GetString(bXMl, 0, bXMl.Length);
				if (xml.Contains("<MotivoAdmissaoTemporariaList>"))
				{
					return bXMl;
				}
				downloadRetries++;
			} while (downloadRetries < 5);

			return null;
		}

		byte[] Download(HttpClient client)
		{
			using (var loginResponse = CaptchaSolvingUtils.Instance.GetLoginResponse(client))
			{
				var sectorsPageResponse = client.GetAsyncEx(URL_ADMISSAO_TEMPORARIA)?.Result;
				Contract.Assume(sectorsPageResponse.IsSuccessStatusCode);

				var viewId = CaptchaSolvingUtils.GetViewId(sectorsPageResponse.Content.ReadAsStringAsync()?.Result);
		...
		HttpResponseMessage DoPrePostDownload(HttpClient client, string viewId)
		{
			...
			prePost.Add("j_id111:setorLotacaoMB_tipoConsulta", "0");
			...
			{
				var prePostResponse = client.PostAsyncEx(URL_ADMISSAO_TEMPORARIA, prePostBody)?.Result;
		...
		HttpResponseMessage DoPrePostDownloadPart2(HttpClient client, string viewId)
		{
			...
			prePost2.Add("subviewAjaxCarregando:formAjaxCarregando:j_id5", "subviewAjaxCarregando:formAjaxCarregando:j_id5");
			...
			{
				var prePostResponse2 = client.PostAsyncEx(URL_ADMISSAO_TEMPORARIA, prePostBody2)?.Result;
		...
		HttpResponseMessage DoPostDownloadXML(HttpClient client, string viewId)
		{
			...
			p2.Add("j_id111:setorLotacaoMB_tipoConsulta", "0");
			...
			{
				var downloadResponse = client.PostAsyncEx(URL_ADMISSAO_TEMPORARIA, postBody2).Result;
		...
		readonly string URL_ADMISSAO_TEMPORARIA = ConfigurationProvider.Configuration.GetSection("URL_TABELAS_ADUANEIRAS_DOWNLOAD_MOTIVO_ADMISSAO_TEMPORARIA").Value;
	}
}
