...
{
	public static class AgreementsLAIACodeListDownloader
	{
		public static byte[] Download(HttpClient client)
		{
			...
			{
				var alaiaResponse = client.GetAsyncEx(URL_ALADI)?.Result;
				Contract.Assume(alaiaResponse.IsSuccessStatusCode);

				var viewId = CaptchaSolvingUtils.GetViewId(alaiaResponse.Content.ReadAsStringAsync()?.Result);
		...
		{
			var url = ConfigurationProvider.Configuration.GetSection("URL_TABELA_ACORDOS_TARIFARIOS").Value;
			using (var httpResponseMessage = client.GetAsyncEx(url)?.Result)
			{
		...
		static HttpResponseMessage DoPrePostDownload(HttpClient client, string viewId)
		{
			...
			prePost.Add("j_id111:acordoAladiMB_tipoConsulta", "0");
			...
			{
				var prePostResponse = client.PostAsyncEx(URL_ALADI, prePostBody)?.Result;
		...
		static HttpResponseMessage DoPrePostDownloadPart2(HttpClient client, string viewId)
		{
			...
			{
				var prePostResponse2 = client.PostAsyncEx(URL_ALADI, prePostBody2)?.Result;
		...
		static HttpResponseMessage DoPostDownloadXML(HttpClient client, string viewId)
		{
			...
			p2.Add("j_id111:acordoAladiMB_tipoConsulta", "0");
			...
			{
				var downloadResponse = client.PostAsyncEx(URL_ALADI, postBody2).Result;
		...
		static string URL_ALADI => ConfigurationProvider.Configuration.GetSection("URL_TABELAS_ADUANEIRAS_DOWNLOAD_ACORDO_ALADI").Value;
	}
}
