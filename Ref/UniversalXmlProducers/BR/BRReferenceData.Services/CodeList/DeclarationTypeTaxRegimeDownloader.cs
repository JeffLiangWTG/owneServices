...
{
	public class DeclarationTypeTaxRegimeDownloader
	{
		public byte[] Download(HttpClient client)
		{
			using (var loginResponse = CaptchaSolvingUtils.Instance.GetLoginResponse(client))
			{
				var lRTIIPageResponse = client.GetAsyncEx(URL_REGIME_TRIBUTARIO)?.Result;
		...
		HttpResponseMessage DoPrePostDownload(HttpClient client, string viewId)
		{
			...
			{
				var prePostResponse = client.PostAsyncEx(URL_REGIME_TRIBUTARIO, prePostBody)?.Result;
		...
		HttpResponseMessage DoPostDownloadXML(HttpClient client, string viewId)
		{
			...
			{
				var downloadResponse = client.PostAsyncEx(URL_REGIME_TRIBUTARIO, postBody2).Result;
		...
		readonly string URL_REGIME_TRIBUTARIO = ConfigurationProvider.Configuration.GetSection("URL_TABELAS_TIPO_DECLARACAO_REGIME_TRIBUTARIO").Value;
	}
}
