...
{
	public class PisCofinsLegalBasisCodeListDownloader
	{
		public byte[] Download(HttpClient client)
		{
			using (var loginResponse = CaptchaSolvingUtils.Instance.GetLoginResponse(client))
			{
				var LegalBasisTaxationRegimePisCofins = client.GetAsyncEx(URL_REGIME_PIS_COFINS)?.Result;
				Contract.Assume(LegalBasisTaxationRegimePisCofins.IsSuccessStatusCode);

				var viewId = CaptchaSolvingUtils.GetViewId(LegalBasisTaxationRegimePisCofins.Content.ReadAsStringAsync()?.Result);
		...
		HttpResponseMessage DoPrePostDownload(HttpClient client, string viewId)
		{
			...
			prePost.Add("j_id111:fundamentoLegalRegimeTributacaoPisCofinsMB_tipoConsulta", "0");
			...
			{
				var prePostResponse = client.PostAsyncEx(URL_REGIME_PIS_COFINS, prePostBody)?.Result;
		...
		HttpResponseMessage DoPrePostDownloadPart2(HttpClient client, string viewId)
		{
			...
			{
				var prePostResponse2 = client.PostAsyncEx(URL_REGIME_PIS_COFINS, prePostBody2)?.Result;
		...
		HttpResponseMessage DoPostDownloadXML(HttpClient client, string viewId)
		{
			...
			p2.Add("j_id111:fundamentoLegalRegimeTributacaoPisCofinsMB_tipoConsulta", "0");
			...
			{
				var downloadResponse = client.PostAsyncEx(URL_REGIME_PIS_COFINS, postBody2).Result;
		...
		readonly string URL_REGIME_PIS_COFINS = ConfigurationProvider.Configuration.GetSection("URL_TABELAS_ADUANEIRAS_DOWNLOAD_LEGAL_BASIS_TAXATION_REGIME_PIS_COFINS").Value;
	}
}
