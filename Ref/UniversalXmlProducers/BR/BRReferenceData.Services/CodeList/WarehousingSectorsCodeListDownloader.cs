...
{
	public class WarehousingSectorsCodeListDownloader
	{
		public byte[] Download(HttpClient client)
		{
			using (var loginResponse = CaptchaSolvingUtils.Instance.GetLoginResponse(client))
			{
				var sectorsPageResponse = client.GetAsyncEx(URL_SETOR_LOTACAO)?.Result;
		...
		HttpResponseMessage DoPrePostDownload(HttpClient client, string viewId)
		{
			...
			prePost.Add("j_id111:j_id203", "j_id111:j_id203");
			using (var prePostBody = new FormUrlEncodedContent(prePost))
			{
				var prePostResponse = client.PostAsyncEx(URL_SETOR_LOTACAO, prePostBody)?.Result;
		...
		HttpResponseMessage DoPrePostDownloadPart2(HttpClient client, string viewId)
		{
			...
			{
				var prePostResponse2 = client.PostAsyncEx(URL_SETOR_LOTACAO, prePostBody2)?.Result;
		...
		HttpResponseMessage DoPostDownloadXML(HttpClient client, string viewId)
		{
			...
			p2.Add("j_id111:j_id214", "j_id111:j_id214");
			using (var postBody2 = new FormUrlEncodedContent(p2))
			{
				var downloadResponse = client.PostAsyncEx(URL_SETOR_LOTACAO, postBody2).Result;
		...
		readonly string URL_SETOR_LOTACAO = ConfigurationProvider.Configuration.GetSection("URL_TABELAS_ADUANEIRAS_DOWNLOAD_SETOR_LOTACAO").Value;
	}
}
