...
{
	public static class NaladiNccaDownloader
	{
		public static byte[] Download(HttpClient client)
		{
			...
			{
				var customsNaladiNccaPageResponse = client.GetAsyncEx(ConfigurationProvider.Configuration.GetSection("URL_TABELAS_ADUANEIRAS_DOWNLOAD_NALADI_NCCA").Value)?.Result;
		...
		static HttpResponseMessage DoPrePostDownload(HttpClient client, string viewId)
		{
			...
			prePost.Add("j_id111:naladiNccaMB_tipoConsulta", "0");
			...
			prePost.Add("j_id111:j_id190", "j_id111:j_id190");
			using (var prePostBody = new FormUrlEncodedContent(prePost))
			{
				var prePostResponse = client.PostAsyncEx(ConfigurationProvider.Configuration.GetSection("URL_TABELAS_ADUANEIRAS_DOWNLOAD_NALADI_NCCA").Value, prePostBody)?.Result;
		...
		static HttpResponseMessage DoPrePostDownloadPart2(HttpClient client, string viewId)
		{
			...
			{
				var prePostResponse2 = client.PostAsyncEx(ConfigurationProvider.Configuration.GetSection("URL_TABELAS_ADUANEIRAS_DOWNLOAD_NALADI_NCCA").Value, prePostBody2)?.Result;
		...
		static HttpResponseMessage DoPostDownloadXML(HttpClient client, string viewId)
		{
			...
			p2.Add("j_id111:naladiNccaMB_tipoConsulta", "0");
			...
			p2.Add("j_id111:j_id201", "j_id111:j_id201");
			using (var postBody2 = new FormUrlEncodedContent(p2))
			{
				var downloadResponse = client.PostAsyncEx(ConfigurationProvider.Configuration.GetSection("URL_TABELAS_ADUANEIRAS_DOWNLOAD_NALADI_NCCA").Value, postBody2).Result;
