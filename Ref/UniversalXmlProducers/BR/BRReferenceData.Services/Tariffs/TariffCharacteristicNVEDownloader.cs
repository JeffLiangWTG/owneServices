...
{
	public static class TariffCharacteristicNVEDownloader
	{
		public static byte[] Download(HttpClient client)
		{
			...
			{
				var nvePageResponse = client.GetAsyncEx(ConfigurationProvider.Configuration.GetSection("URL_TARIFF_NVE_ATTRIBUTE").Value)?.Result;
				Contract.Assume(nvePageResponse.IsSuccessStatusCode);

				var viewId = CaptchaSolvingUtils.GetViewId(nvePageResponse.Content.ReadAsStringAsync()?.Result);
		...
		static HttpResponseMessage DoPrePostDownload(HttpClient client, string viewId)
		{
			...
			prePost.Add("j_id111:nveMB_tipoConsulta", "0");
			...
			prePost.Add("j_id111:j_id254", "j_id111:j_id254");
			using (var prePostBody = new FormUrlEncodedContent(prePost))
			{
				var prePostResponse = client.PostAsyncEx(ConfigurationProvider.Configuration.GetSection("URL_TARIFF_NVE_ATTRIBUTE").Value, prePostBody)?.Result;
		...
		static HttpResponseMessage DoPrePostDownloadPart2(HttpClient client, string viewId)
		{
			...
			{
				var prePostResponse2 = client.PostAsyncEx(ConfigurationProvider.Configuration.GetSection("URL_TARIFF_NVE_ATTRIBUTE").Value, prePostBody2)?.Result;
		...
		static HttpResponseMessage DoPostDownloadXML(HttpClient client, string viewId)
		{
			...
			p2.Add("j_id111:j_id265", "j_id111:j_id265");
			using (var postBody2 = new FormUrlEncodedContent(p2))
			{
				var downloadResponse = client.PostAsyncEx(ConfigurationProvider.Configuration.GetSection("URL_TARIFF_NVE_ATTRIBUTE").Value, postBody2).Result;
