...
namespace CargoWise.RefDbRepo.GBReferenceData.Tests.Tariff
{
	...
	class ProcessManagerTests
	{
		...
		public void GetFileManager()
		{
			...
			Assert.That(fileManager.GetType(), Is.EqualTo(typeof(Services.Tariff.WebClientFileManager)));
		}
		...
		{
			var processManager = new ProcessManagerForTest(new IProcessor[] { }, new ILoader[] { });
			Assert.That(processManager.SourceDataProvider, Is.EqualTo("HMRC"));
		}
		...
		{
			var processManager = new ProcessManagerForTest(new IProcessor[] { }, new ILoader[] { });
			Assert.That(processManager.Team, Is.EqualTo("CUK"));
		}
	}
}
