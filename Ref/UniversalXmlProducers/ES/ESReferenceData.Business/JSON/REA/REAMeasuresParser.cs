...
public class REAMeasuresParser(IDateTimeProvider dateTimeProvider, string refDbServiceURI) : CommonParser(dateTimeProvider)
{
	...
	public string ConvertToXMLFile(string uomJson, string measuresJsonContent, string outPutFileWithPath)
	{
		...
		if (UOMMapCodesList.Count > 0)
		{
			...
			if (result.Count > 0)
			{
				Helper.ExportToXMLFile(Constants.DataSources.REAMeasures, outPutFileWithPath, XMLWriterConfiguration, DateTimeProvider.CurrentLocalDate, result);
			}
			else
			{
				ErrorBuilder.AppendLine("Unable to locate any records for REA Measure Codes.");
			}
		}
		else
		{
	...
	{
		var resultREA = new List<RefDataRepoModelEntityType>();

		var measures = JsonHelper.GetListItemsFromJsonl<MeasuresSchema>(measuresJsonContent, ErrorBuilder, handleDates: true, x => x.measure_type.Equals(Constants.REA.MeasureTypeCode, StringComparison.Ordinal));
		...
		foreach (var measure in measures)
		{
			...
			{
				var isTobacco = tariff.StartsWith("24", StringComparison.InvariantCulture);
				...
				var rate = new RefCusRate() {
					...
					ZZ2_RateFormula = FormulaFormatter.ParseJsonFormula(duty, UOMMapCodesList),
					ZZ2_ZZS_NKPreference = GetPreference(isTobacco),
					RefCusApplicabilities = applicabilities
				};

				var conditionsToApply = GetConditions(measure, conditionDescriptionsList, applicabilities, isTobacco);
	...
	static string GetPreference(bool isTobacco) => isTobacco ? Constants.Preference.TobaccoProductCoveredByREA : Constants.Preference.GoodsCoveredByREA;

	RefCusCondition[] GetConditions(MeasuresSchema measure, Dictionary<string, string> conditionDescriptionsList, RefCusApplicability[] applicabilities, bool isTobacco)
	{
		List<RefCusCondition> conditions = [];
		var conditionsToProcess = measure.conditions.Replace("Cond:", string.Empty).Split(';');
		var preference = GetPreference(isTobacco);
		var logicalAndWithinGroup = (byte)(isTobacco ? 1 : 0);
		...
		foreach (var conditionString in conditionsToProcess)
		{
			if (conditionString.Contains("cert:"))
			{
				...
				if (lastIndex > 0)
				{
					var certificateCode = conditionString.Substring(startIndex, lastIndex - startIndex).Trim();
					...
					if (conditionDescriptionsList.TryGetValue(conditionID, out string description))
					{
						conditions.Add(new RefCusCondition()
						{
							...
							ZX1_LogicalANDWithinGroup = logicalAndWithinGroup,
							RefCusConditionValues = [new RefCusConditionValue() { ZX3_Value = certificateCode }],
							RefCusApplicabilities = applicabilities
						});
					}
					else
					{
						ErrorBuilder.AppendLine(CultureInfo.InvariantCulture, $"Unable to import REA Measure 'condition', description not found. Details:");
				...
				{
					ErrorBuilder.AppendLine(CultureInfo.InvariantCulture, $"Unable to import REA Measure 'condition' as invalid format. Details:");
					ErrorBuilder.AppendLine(CultureInfo.InvariantCulture, $"Tariff: {measure.goods_id}");
					ErrorBuilder.AppendLine(CultureInfo.InvariantCulture, $"Condition: {conditionString}");
				}
			}
	...
	bool CheckTariffDataIsValid(string code, string duty)
	{
		...
		{
			ErrorBuilder.AppendLine(CultureInfo.InvariantCulture, $"Unable to import REA Measure as missing or invalid 'tariff' or 'duty'. Details:");
	...
	protected override XmlWriterConfiguration XMLWriterConfiguration
	{
		get
		{
			...
			refCusRateConfiguration.IncludeColumn(x => x.ZZ2_ZZS_NKPreference, false);
			...
			refCusConditionConfiguration.IncludeColumn(x => x.ZX1_ZZS_NKPreference, false);
			refCusConditionConfiguration.IncludeColumn(x => x.ZX1_LogicalANDWithinGroup, false);
			refCusConditionConfiguration.IncludeColumnWithConstantValue(x => x.ZX1_ZX2_NKConditionType, true, Constants.ConditionType.REASuspensions);
