...
{
	public class HSExtensionCodesSubCategoryParser : BaseEntityConfigurationParser
	{
		public HSExtensionCodesSubCategoryParser(string configFilePath, string dataFilePath) : base(configFilePath, dataFilePath)
		{
		}
		...
		protected override void ExportToXmlFilesCore(string outputFilePath, DateTime publicationDate, EntityConfiguration configuration, IWorkbook workbook, string suffix)
		{
			...
			var tariffs = new ExcelEntityUpdater<RefCusTariff>(configuration, new HSExtensionCodesAdditionalDataUpdater(), new HSExtensionCodesTopEntityLookupManager()).Update(workbook);
			...
			for (int i = 0; i < 10; i++)
			{
				...
				Helper.ExportToXMLFile($"{Constants.DataSources.HSExtensionCodesSub} {i}", outputFile, writerConfiguration, publicationDate, entities);
			}
		}
	}
}
