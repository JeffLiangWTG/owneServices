...
{
	[TestFixture]
	sealed class AdditionalInformationBuilderTest
	{
		[Test]
		public void ConvertXmlFileToRefCusCodeList()
		{
			...
			var builder = new AdditionalInformationBuilderForTest(errorCollector, dateTimeProvider.Object);
			...
			Assert.AreEqual("AI44I", resultList[0].ZZD_ZZK_NKCodeType, "RefCusCodeList is generated with wrong Code Type ");
			Assert.AreEqual("AI44E", resultList[1].ZZD_ZZK_NKCodeType, "RefCusCodeList is generated with wrong Code Type ");
			Assert.AreEqual("NL", resultList[0].ZZD_ZZZ_NKDataGrouping, "RefCusCodeList should have 'NL' as Data Grouping value");
			Assert.AreEqual(new DateTime(1900, 01, 01, 00, 00, 00), resultList[0].ZZD_StartDate, "Start date should be equal to '2024-07-09T15:58:07'");
			...
			Assert.AreEqual("Description", resultList[2].RefCusCodeListAttributes[2].ZZE_ZXE_NKName, "Third attribute should have the name 'Description'");
		...
		public void DuplicateDataInXml()
		{
			...
			var errorCollector = new StringBuilder();
			var builder = new AdditionalInformationBuilderForTest(errorCollector);
			var refDataCollection = builder.ConvertTableElementsToRefDataExposed(duplicateData);

			Assert.That(errorCollector.ToString().StartsWith("RefCusCodeList duplicate exists. Key: '001_AI44I' Description: Description 1", StringComparison.InvariantCulture), "Duplicate data (001_AI44I) is not detected");
		...
		public void InvalidDataInXml()
		{
			...
			Assert.That(errorCollector.ToString().Contains("RefCusCodeList validation error. Key '_AI44I' Errors: ZZD_Code is required. '"), "Invalid data (missing code) is not detected.");
			Assert.That(errorCollector.ToString().Contains("RefCusCodeList validation error. Key '003_AI44I' Errors: ZZD_Description is required. '"), "Invalid data (missing description) is not detected.");
		...
		public void GenerateUniversalReferenceDataXml()
		{
			...
			var builder = new AdditionalInformationBuilder(errorCollector, dateTimeProvider.Object);

			string expectedXml, generatedXml = string.Empty;

			builder.BuildXml(new DateTime(2020, 12, 15, 14, 15, 30), ReferenceData, TempFolder);

			var expectedFileName = Path.Combine(TempFolder, "NL CodeList - Additional Information_141530000.xml");
			if (File.Exists(expectedFileName))
			{
				generatedXml = File.ReadAllText(expectedFileName);
			}

			expectedXml = TestHelper.ReadManifestResourceContent("CargoWise.RefDbRepo.NLReferenceData.Tests.TestFiles.AdditionalInformation.Output.RefCusCodeList_AdditionalInformation.xml");
	...
	public class AdditionalInformationBuilderForTest : AdditionalInformationBuilder
	{
		public AdditionalInformationBuilderForTest(StringBuilder errorCollector, IDateTimeProvider dateTimeProvider = null) : base(errorCollector, dateTimeProvider)
		{
		}
