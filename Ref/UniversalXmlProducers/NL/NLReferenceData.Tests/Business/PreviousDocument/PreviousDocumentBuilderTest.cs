...
{
	[TestFixture]
	sealed class PreviousDocumentBuilderTest
	{
		[Test]
		public void ConvertXmlFileToRefCusCodeList()
		{
			...
			var builder = new PreviousDocumentBuilderForTest(errorCollector, dateTimeProvider.Object);
			...
			Assert.AreEqual("DC40I", resultList[0].ZZD_ZZK_NKCodeType, "RefCusCodeList is generated with wrong Code Type ");
			Assert.AreEqual("DC40E", resultList[1].ZZD_ZZK_NKCodeType, "RefCusCodeList is generated with wrong Code Type ");
		...
		public void DuplicateDataInXml()
		{
			...
			var errorCollector = new StringBuilder();
			var builder = new PreviousDocumentBuilderForTest(errorCollector);
			var refDataCollection = builder.ConvertTableElementsToRefDataExposed(duplicateData);

			Assert.That(errorCollector.ToString().StartsWith("RefCusCodeList duplicate exists. Key: '001_DC40I' Description: Description 1", StringComparison.InvariantCulture), "Duplicate data (001_DC40I) is not detected");
		...
		public void InvalidDataInXml()
		{
			...
			Assert.That(errorCollector.ToString().Contains("RefCusCodeList validation error. Key '_DC40I' Errors: ZZD_Code is required. '"), "Invalid data (missing code) is not detected.");
			Assert.That(errorCollector.ToString().Contains("RefCusCodeList validation error. Key '003_DC40I' Errors: ZZD_Description is required. '"), "Invalid data (missing description) is not detected.");
		...
		public void GenerateUniversalReferenceDataXml()
		{
			...
			var builder = new PreviousDocumentBuilder(errorCollector, dateTimeProvider.Object);
			string expectedXml, generatedXml = string.Empty;

			builder.BuildXml(new DateTime(2024, 06, 14, 15, 25, 06), ReferenceData, TempFolder);

			var expectedFileName = Path.Combine(TempFolder, "NL CodeList - Previous Document_152506000.xml");
			if (File.Exists(expectedFileName))
			{
				generatedXml = File.ReadAllText(expectedFileName);
			}

			expectedXml = TestHelper.ReadManifestResourceContent("CargoWise.RefDbRepo.NLReferenceData.Tests.TestFiles.PreviousDocument.Output.RefCusCodeList_PreviousDocument.xml");
	...
	public class PreviousDocumentBuilderForTest : PreviousDocumentBuilder
	{
		public PreviousDocumentBuilderForTest(StringBuilder errorCollector, IDateTimeProvider dateTimeProvider = null) : base(errorCollector, dateTimeProvider)
		{
		}
