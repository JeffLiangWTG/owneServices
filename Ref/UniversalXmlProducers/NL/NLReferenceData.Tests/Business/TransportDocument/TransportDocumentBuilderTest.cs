...
{
	sealed class TransportDocumentBuilderTest
	{
		[Test]
		public void ConvertXmlFileToRefCusCodeList()
		{
			...
			var builder = new TransportDocumentBuilderForTest(errorCollector, dateTimeProvider.Object);

			var resultList = builder.ConvertTableElementsToRefDataExposed(ReferenceData);

			// Change to new mapping, based on new codelist
			...
			Assert.AreEqual("TD44I", resultList[0].ZZD_ZZK_NKCodeType, "RefCusCodeList is generated with wrong Code Type ");
			Assert.AreEqual("TD44E", resultList[1].ZZD_ZZK_NKCodeType, "RefCusCodeList is generated with wrong Code Type ");
			...
			Assert.AreEqual("Reference", resultList[2].RefCusCodeListAttributes[2].ZZE_ZXE_NKName, "Third attribute should have the name 'Description'");
		...
		public void DuplicateDataInXml()
		{
			...
			var errorCollector = new StringBuilder();
			var builder = new TransportDocumentBuilderForTest(errorCollector);
			var refDataCollection = builder.ConvertTableElementsToRefDataExposed(duplicateData);

			Assert.That(errorCollector.ToString().StartsWith("RefCusCodeList duplicate exists. Key: '001_TD44I' Description: Description 1", StringComparison.InvariantCulture), "Duplicate data (001_TD44I) is not detected");
		...
		public void InvalidDataInXml()
		{
			...
			Assert.That(errorCollector.ToString().Contains("RefCusCodeList validation error. Key '_TD44I' Errors: ZZD_Code is required. '"), "Invalid data (missing code) is not detected.");
			Assert.That(errorCollector.ToString().Contains("RefCusCodeList validation error. Key '003_TD44I' Errors: ZZD_Description is required. '"), "Invalid data (missing description) is not detected.");
		...
		public void GenerateUniversalReferenceDataXml()
		{
			...
			var builder = new TransportDocumentBuilder(errorCollector, dateTimeProvider.Object);

			string expectedXml, generatedXml = string.Empty;

			builder.BuildXml(new DateTime(2020, 12, 15, 14, 15, 30), ReferenceData, TempFolder);

			var expectedFileName = Path.Combine(TempFolder, "NL CodeList - Transport Document_141530000.xml");
			if (File.Exists(expectedFileName))
			{
				generatedXml = File.ReadAllText(expectedFileName);
			}

			expectedXml = TestHelper.ReadManifestResourceContent("CargoWise.RefDbRepo.NLReferenceData.Tests.TestFiles.TransportDocument.Output.RefCusCodeList_TransportDocument.xml");
		...
		public class TransportDocumentBuilderForTest : TransportDocumentBuilder
		{
			public TransportDocumentBuilderForTest(StringBuilder errorCollector, IDateTimeProvider dateTimeProvider = null) : base(errorCollector, dateTimeProvider)
			{
			}
