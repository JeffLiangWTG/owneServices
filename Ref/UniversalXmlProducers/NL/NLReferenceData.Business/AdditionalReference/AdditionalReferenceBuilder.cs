...
{
	public class AdditionalReferenceBuilder : RefCusCodeListBuilder<TableElement>, ICodebookBuilder
	{
		public AdditionalReferenceBuilder(StringBuilder errorCollector, IDateTimeProvider dateTimeProvider = null) : base(errorCollector)
		{
		...
		protected override IEnumerable<RefCusCodeList> ConvertToRefModels(IEnumerable<TableElement> data)
		{
			...
			var headerLevel = CreateAttribute(Constants.AdditionalReference.AttributeNames.Level, Constants.AttributeValues.HeaderLevel);
			var itemLevel = CreateAttribute(Constants.AdditionalReference.AttributeNames.Level, Constants.AttributeValues.ItemLevel);
			var description = CreateAttribute(Constants.AdditionalReference.AttributeNames.Reference, Constants.AttributeValues.Yes);

			foreach (var elem in data)
			{
				results.Add(CreateRefCusCodeList(elem, Constants.AdditionalReference.ImportCodeType, headerLevel, itemLevel, description));
				results.Add(CreateRefCusCodeList(elem, Constants.AdditionalReference.ExportCodeType, headerLevel, itemLevel, description));
			}
		...
		protected override string XMLWriterDataSource => "Additional Reference";
