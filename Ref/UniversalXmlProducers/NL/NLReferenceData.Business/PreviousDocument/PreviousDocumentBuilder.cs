...
{
	public class PreviousDocumentBuilder : RefCusCodeListBuilder<TableElement>, ICodebookBuilder
	{
		public PreviousDocumentBuilder(StringBuilder errorCollector, IDateTimeProvider dateTimeProvider = null) : base(errorCollector)
		{
		...
		protected override IEnumerable<RefCusCodeList> ConvertToRefModels(IEnumerable<TableElement> data)
		{
			...
			var headerLevel = CreateAttribute(Constants.PreviousDocument.AttributeNames.Level, Constants.AttributeValues.HeaderLevel);
			var itemLevel = CreateAttribute(Constants.PreviousDocument.AttributeNames.Level, Constants.AttributeValues.ItemLevel);
			var description = CreateAttribute(Constants.PreviousDocument.AttributeNames.Reference, Constants.AttributeValues.Yes);

			foreach (var elem in data)
			{
				results.Add(CreateRefCusCodeList(elem, Constants.PreviousDocument.ImportCodeType, headerLevel, itemLevel, description));
				results.Add(CreateRefCusCodeList(elem, Constants.PreviousDocument.ExportCodeType, headerLevel, itemLevel, description));
			}
		...
		protected override string XMLWriterDataSource => "Previous Document";
