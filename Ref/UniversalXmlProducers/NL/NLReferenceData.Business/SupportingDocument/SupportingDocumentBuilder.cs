...
{
	public class SupportingDocumentBuilder : RefCusCodeListBuilder<TableElement>, ICodebookBuilder
	{
		public SupportingDocumentBuilder(StringBuilder errorCollector, IDateTimeProvider dateTimeProvider = null) : base(errorCollector)
		{
		...
		protected override IEnumerable<RefCusCodeList> ConvertToRefModels(IEnumerable<TableElement> data)
		{
			...
			var headerLevel = CreateAttribute(Constants.SupportingDocument.AttributeNames.Level, Constants.AttributeValues.HeaderLevel);
			var itemLevel = CreateAttribute(Constants.SupportingDocument.AttributeNames.Level, Constants.AttributeValues.ItemLevel);
			var description = CreateAttribute(Constants.SupportingDocument.AttributeNames.Reference, Constants.AttributeValues.Yes);

			foreach (var elem in data)
			{
				results.Add(CreateRefCusCodeList(elem, Constants.SupportingDocument.ImportSupportingDocument, headerLevel, itemLevel, description));
				results.Add(CreateRefCusCodeList(elem, Constants.SupportingDocument.ExportSupportingDocument, headerLevel, itemLevel, description));
			}
		...
		protected override string XMLWriterDataSource => "Supporting Document";
