...
{
	[TestFixture]
	public class CARMRateDataProducerTest : TariffProducerTest
	{
		Mock<IWebServiceCaller> callerMock;
		string WorkingDirectory;

		protected override string FunctionCode => Constants.ProgramFunctions.CARate;

		protected override string NothingNewPublishedMessage => $"CA rate data: Nothing new published since last process. Skip processing this time.";
		...
		{
			return new CARMRateDataProducer(callerMock.Object, LogBuilder);
		}

		protected override void CreateDownloaderMockData()
		{
			callerMock = new Mock<IWebServiceCaller>();
			callerMock.Setup(x => x.GetLatestUpdateOnDate(CARMAPIQueryTypes.ExciseTaxCodesQueryType)).Returns(new System.DateTime(2021, 12, 08));
