...
namespace CargoWise.RefDbRepo.UniversalXMLProducers.EUNDailyImportTariffPopulator.Test
{
	...
	internal class ImportTariffWriterFixture
	{
		...
		public void Write()
		{
			...
			tariffWriter.Write(new[] { tariff1, tariff2, tariff3 }, new DateTime(2022, 05, 04), @"C:\Dev\out.xml");
			...
			writer.Verify(x => x.SetDataSource($"Import Daily EUN Tariffs chapters 10-19"), Times.Once());
			writer.Verify(x => x.SetDataSource($"Import Daily EUN Tariffs chapters 40-49"), Times.Once());
			writer.Verify(x => x.SaveXml(@"C:\Dev\out_20220504_1.xml", true), Times.Once());
			writer.Verify(x => x.SaveXml(@"C:\Dev\out_20220504_4.xml", true), Times.Once());
		}
	}
}
