...
namespace CargoWise.RefDbRepo.UniversalXMLProducers.EUNMonthlyImportTariffPopulator
{
	public class ImportTariffWriter
	{
		...
		public void Write(IEnumerable<RefCusTariff> tariffs, DateTime publicationTime, bool isPartial, string output)
		{
			foreach (var tariffsPerChapter in tariffs.GroupBy(x => x.ZZ1_TariffCode.Substring(0, 1)))
			{
				...
				writer.SetDataSource($"Import EUN Tariffs chapters {tariffsPerChapter.Key}0-{tariffsPerChapter.Key}9");
				foreach (var tariff in tariffsPerChapter.OrderBy(x => x.ZZ1_TariffCode))
				{
					writer.PopulateData(tariff);
				}
				writer.SaveXml(output.Replace(".xml", $"_{tariffsPerChapter.Key}.xml"));
			}
		}
