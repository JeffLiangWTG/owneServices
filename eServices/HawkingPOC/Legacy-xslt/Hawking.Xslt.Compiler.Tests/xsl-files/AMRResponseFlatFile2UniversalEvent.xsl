...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		<xsl:apply-templates select="/s0:AMROutput" />
	</xsl:template>

	<xsl:template match="s0:AMROutput">
					...
					<ns0:Event>
						...
						<ns0:ContextCollection>
							...
							<xsl:call-template name="ContextKeyValuePair">
								...
								<xsl:with-param name="value" select="s0:BillOfLadingNumber" />
							</xsl:call-template>
							<xsl:call-template name="ContextKeyValuePair">
							...
							<xsl:element name="ns0:Context">
								...
								<xsl:element name="ns0:Value">
									<xsl:variable name="BillNumber" select="s0:BillOfLadingNumber" />
									...
									<xsl:call-template name="HTMLTR">
										<xsl:with-param name="name" select="'Bill Of Lading Number'" />
										<xsl:with-param name="value" select="$BillNumber" />
									</xsl:call-template>
									<xsl:call-template name="HTMLTR">
										...
										<xsl:with-param name="value">
											...
											<xsl:if test="$DiscrepancyCode != ''">
												...
												<xsl:value-of select="' - '"/>
												<xsl:choose>
													<xsl:when test="$DiscrepancyCode = '1'"> The Bill is marked as Master Bill but no Forwarder has manifested associated house Bill</xsl:when>
													<xsl:when test="$DiscrepancyCode = '2'"> The Bill is not marked as Master Bill but Forwarder has manifested associated house Bill</xsl:when>
									...
									<xsl:for-each select="s0:ProcessResultCodes/s0:ProcessResultCode">
										<xsl:if test="s0:Code and s0:Code != '00000' and s0:Code != ''">
											...
											<xsl:variable name="ErrorFieldID" select="s0:ErrorFieldID"/>
											<xsl:if test="$ErrorFieldID != '0000'">
												<xsl:call-template name="HTMLTR">
													<xsl:with-param name="name" select="concat('Process Result Field - ', $position)"/>
													<xsl:with-param name="value">
														<xsl:value-of select="'Error occured on Field:&quot;'" />
														<xsl:value-of select="$ErrorFieldID" />
														<xsl:value-of select="' - '" />
														<xsl:value-of select="ScriptNS0:GetRecipientCode('JPCustoms' , 'JPCustoms' , 'JP AFR - Import SAHR' , 'Field' , 'FieldName', $ErrorFieldID, $ProcedureCode)"/>
														<xsl:value-of select="'&quot;'" />
											...
											<xsl:call-template name="HTMLTR">
												...
												<xsl:with-param name="value">
													<xsl:value-of select="ScriptNS0:GetRecipientCode('JPCustoms' , 'JPCustoms' , 'JP AFR - Import SAMR' , 'ErrorDetail' , 'ErrorDescription', s0:Code, $ProcedureCode)" disable-output-escaping="no"/>
												</xsl:with-param>
											</xsl:call-template>
											<xsl:call-template name="HTMLTR">
												...
												<xsl:with-param name="value">
													<xsl:value-of select="concat(
																				ScriptNS0:GetRecipientCode('JPCustoms' , 'JPCustoms' , 'JP AFR - Import SAMR' , 'ErrorDetail' , 'ErrorSolutionPartA', s0:Code, $ProcedureCode),
																				ScriptNS0:GetRecipientCode('JPCustoms' , 'JPCustoms' , 'JP AFR - Import SAMR' , 'ErrorDetail' , 'ErrorSolutionPartB', s0:Code, $ProcedureCode)
