...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="msxsl s0 ScriptNS0 userCSharp" version="1.0"
  ...
  <xsl:template match="/">
    ...
    <LPCOImages xsi:noNamespaceSchemaLocation="LPCO_Image_TCP.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xop="http://www.w3.org/2004/08/xop/include">
      ...
      <Image>
        <ImageHeader>
          <Identifier><xsl:value-of select="$EventParameters/s0:RequestNumber/text()"/></Identifier>
          <FunctionCode><xsl:value-of select="$EventParameters/s0:MessageSubType/text()"/></FunctionCode>
          <LPCOImageTypeCode><xsl:value-of select="$ContextCollection/s0:Context[s0:Type/text()='CustomsDocumentType']/s0:Value/text()"/></LPCOImageTypeCode>
          <xsl:call-template name="NodeIfNotEmpty">
          ...
          <LPCOImageEffectiveDate><xsl:value-of select="substring-before($ContextCollection/s0:Context[s0:Type/text()='EffectiveDate']/s0:Value/text(),'T')"/></LPCOImageEffectiveDate>
          <LPCOImageExpiryDate><xsl:value-of select="substring-before($ContextCollection/s0:Context[s0:Type/text()='ExpiryDate']/s0:Value/text(),'T')"/></LPCOImageExpiryDate>
          ...
          <LPCOImageFormat><xsl:value-of select="userCSharp:GetFileFormat($FileName)"/></LPCOImageFormat>
        </ImageHeader>
