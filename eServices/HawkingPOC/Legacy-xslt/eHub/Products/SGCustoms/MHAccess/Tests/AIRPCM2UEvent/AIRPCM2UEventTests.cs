...
namespace CargoWise.eHub.Products.SGCustoms.MHAccess.BizTalk.Tests
{
    ...
    {
        const string xsl = "AIRPCM2UEvent.MapFiles.AIRPCM2UEvent.xsl";
        const string filePath = "AIRPCM2UEvent.TestFiles.";

        [Fact]
        public void TestAIRPCM2UEvent1_failure()
        {
            ...
            mockDataModelAccessor.Setup(x => x.InsertSubscriptionValue(
                            ...
                            It.IsAny<string>(),
                            It.IsAny<string>()));
            mockIContextAccessor.Setup(x => x.GetContextProperty("OverrideFilename", "http://cargowise.com/ehub/processing/2010/06")).Returns("ErrorCode: 1325;LinkedOutboxMessageTrackingID: 3DF09E8A-C79A-4F6A-95BC-E790A38FA7FB. ErrorMessage: Invalid User ID / Password");
        ...
        [Fact]
        public void TestAIRPCM2UEvent1_success()
        {
            var input = filePath + "Test1_input.xml";
