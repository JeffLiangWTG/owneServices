...
using CargoWise.eHub.Clients.AAJ.Transforms.PackingScheduleCsv_2_UniShip;
using CargoWise.eHub.DataAccess.Models.CodeMapsTesting;
using CargoWise.eHub.DataAccess.Sql;
using Microsoft.VisualStudio.TestTools.UnitTesting;

namespace CargoWise.eHub.Clients.AAJ.Tests
{
    [TestClass]
    public class PackingScheduleCsv_2_UniShipTests
    {
        [TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
        public void TestPackingScheduleCsv_2_UniShip()
        {
            InitialiseCodeMapsTestingContext();
            MapTester mapTester = new MapTester(Assembly.GetExecutingAssembly());

            string sourceFile = "PackingScheduleCsv_2_UniShip.TestFiles.Input.xml";
            string expectedFile = "PackingScheduleCsv_2_UniShip.TestFiles.Output.xml";
            mapTester.ExecuteCompiled<PackingScheduleCsv_2_UniShip>(sourceFile, expectedFile);
        }

        static void InitialiseCodeMapsTestingContext()
        {
            CodeMapsTestingContext ctx = new CodeMapsTestingContext();
            ctx.eHubClients.Add(new eHubClient { CC_ID = "AAJMELPRD_PAC" });
            ctx.eHubClients.Add(new eHubClient { CC_ID = "AAJMELPRD" });
            ctx.eHubTransformationSets.Add(new eHubTransformationSet { TS_Name = "Packing CSV - Add Containers & Pack Lines", eHubClient_Sender = ctx.eHubClients[0], eHubClient_Recipient = ctx.eHubClients[1] });
            ...
            ctx.eHubCodeMapValues.Add(new eHubCodeMapValue { eHubCodeMapKey = ctx.eHubCodeMapKeys.Last(), eHubCodeSetResult = ctx.eHubCodeSetResults.Last(), CV_OutputCode = "AWE" });
            ...
            ctx.eHubCodeSets.Add(new eHubCodeSet { CS_Name = "PRA Status", eHubClient_Sender = ctx.eHubClients[0], eHubClient_Recipient = ctx.eHubClients[1], eHubTransformationSet = ctx.eHubTransformationSets[0] });
            ctx.eHubCodeSetResults.Add(new eHubCodeSetResult { eHubCodeSet = ctx.eHubCodeSets.Last(), CR_Order = 1, CR_Name = "Output" });
            ...
            ctx.eHubCodeMapKeys.Add(new eHubCodeMapKey { eHubCodeSet = ctx.eHubCodeSets.Last(), CK_Order = 2, CK_Key1Value = "SENT" });
            ctx.eHubCodeMapValues.Add(new eHubCodeMapValue { eHubCodeMapKey = ctx.eHubCodeMapKeys.Last(), eHubCodeSetResult = ctx.eHubCodeSetResults.Last(), CV_OutputCode = "" });
            ctx.eHubCodeMapKeys.Add(new eHubCodeMapKey { eHubCodeSet = ctx.eHubCodeSets.Last(), CK_Order = 3, CK_Key1Value = "NOT SENT" });
