...
using CargoWise.eHub.Clients.TRX.Transforms.UniversalTransaction_2_X12_210;
...
{
	[TestClass]
	public class UniversalTransaction_2_X12_210Tests
	{
		[TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
		public void TestUniversalTransaction_2_X12_210()
		{
			InitialiseCodeMapsTestingContext();

			List<string> exclusionXpaths = new List<string>();
			exclusionXpaths.Add("//*[local-name()='B3']/*[local-name()='B312']");
			...
			string sourceFile = "UniversalTransaction_2_X12_210.TestFiles.UniversalTransaction_2_X12_210_input.xml";
			string expectedFile = "UniversalTransaction_2_X12_210.TestFiles.UniversalTransaction_2_X12_210_output.xml";
			mapTester.Execute<UniversalTransaction_2_X12_210>(sourceFile, expectedFile);
		}
		...
		private static void InitialiseCodeMapsTestingContext()
		{
			...
			ctx.eHubClients.Add(new eHubClient { CC_ID = "TRXELPELP_BIR" });
			ctx.eHubTransformationSets.Add(new eHubTransformationSet { TS_Name = "Omnicell 210 - Export A/R Invoices", eHubClient_Sender = ctx.eHubClients[0], eHubClient_Recipient = ctx.eHubClients[1] });
			...
			ctx.eHubCodeSets.Add(new eHubCodeSet { CS_Name = "Special Charge Code", eHubClient_Sender = ctx.eHubClients[0], eHubClient_Recipient = ctx.eHubClients[1], eHubTransformationSet = ctx.eHubTransformationSets[0] });
			...
			ctx.eHubCodeMapValues.Add(new eHubCodeMapValue { eHubCodeMapKey = ctx.eHubCodeMapKeys.Last(), eHubCodeSetResult = ctx.eHubCodeSetResults.Last(), CV_OutputCode = "080" });
			...
			ctx.eHubCodeSetResults.Add(new eHubCodeSetResult { eHubCodeSet = ctx.eHubCodeSets.Last(), CR_Order = 1, CR_Name = "ISO 3166 Code" });
			ctx.eHubCodeMapKeys.Add(new eHubCodeMapKey { eHubCodeSet = ctx.eHubCodeSets.Last(), CK_Order = 1, CK_Key1Value = "AU" });
			ctx.eHubCodeMapValues.Add(new eHubCodeMapValue { eHubCodeMapKey = ctx.eHubCodeMapKeys.Last(), eHubCodeSetResult = ctx.eHubCodeSetResults.Last(), CV_OutputCode = "AUS" });
			...
			ctx.eHubCodeSetResults.Add(new eHubCodeSetResult { eHubCodeSet = ctx.eHubCodeSets.Last(), CR_Order = 1, CR_Name = "X12 Code" });
			ctx.eHubCodeMapKeys.Add(new eHubCodeMapKey { eHubCodeSet = ctx.eHubCodeSets.Last(), CK_Order = 1, CK_Key1Value = "PKG" });
