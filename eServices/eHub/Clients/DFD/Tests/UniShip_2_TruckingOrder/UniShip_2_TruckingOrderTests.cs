...
using CargoWise.eHub.Clients.DFD.Transforms.UniShip_2_TruckingOrder;
...
namespace CargoWise.eHub.Clients.DFD.Tests
{
	...
	public class UniShip_2_TruckingOrderTests
	{
		...
		public void TestUniShip_2_TruckingOrder()
		{
            ...
            Assert.IsTrue(Regex.IsMatch((string)ctx.Read("OverrideFilename", "http://cargowise.com/ehub/processing/2010/06"), "DFDQ_EXW_ORDER_........_0000666.xml"));
		...
		static void InitialiseCodeMapsTestingContext()
		{
			...
			ctx.eHubClients.Add(new eHubClient { CC_ID = "DFDEWRPRD_EXW" });
			ctx.eHubClients.Add(new eHubClient { CC_ID = "DFDEWRPRD_EXW" });
			ctx.eHubTransformationSets.Add(new eHubTransformationSet { TS_Name = "ExWorks XML - Send Transport Booking", eHubClient_Sender = ctx.eHubClients[0], eHubClient_Recipient = ctx.eHubClients[1] });
			...
			ctx.ActionProcedures.Add(new ActionProcedure
			{
				...
				InputParms = new List<string> { 
					"@name", "CargoWise.eHub.Clients.DFD.Transforms.UniShip_2_TruckingOrder.transactionId",
					"@maxlength", "7",
				},
				Result = "666"
			});
