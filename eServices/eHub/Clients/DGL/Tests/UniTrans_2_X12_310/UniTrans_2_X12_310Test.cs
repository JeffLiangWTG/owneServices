...
using CargoWise.eHub.Clients.DGL.Transforms.UniTrans_2_X12_310;
...
{
	[TestClass]
	public class UniTrans_2_X12_310Test
	{
        MapTester mapTester;
        TestingMessageContext testContext;

        [TestInitialize]
        public void TestSetup()
        {
            testContext = InitialiseTestingMessageContext();
        ...
        public void TestUniTrans_2_X12_310_1()
        {
            string sourceFile = "UniTrans_2_X12_310.TestFiles.1_Input.xml";
            string expectedFile = "UniTrans_2_X12_310.TestFiles.1_Output.xml";
            mapTester.ExecuteCompiled<UniTrans_2_X12_310>(sourceFile, expectedFile);

            Assert.IsTrue(Regex.Match(testContext.Read("OverrideFilename", "http://cargowise.com/ehub/processing/2010/06").ToString(), @"JAGJAGDGU............06\.310").Success);
        }

        [TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
        public void TestUniTrans_2_X12_310_2()
        {
            string sourceFile = "UniTrans_2_X12_310.TestFiles.2_Input.xml";
            string expectedFile = "UniTrans_2_X12_310.TestFiles.2_Output.xml";
            mapTester.ExecuteCompiled<UniTrans_2_X12_310>(sourceFile, expectedFile);
        ...
        [TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
        public void TestUniTrans_2_X12_310_Air()
        {
            string sourceFile = "UniTrans_2_X12_310.TestFiles.Air_Input.xml";
            string expectedFile = "UniTrans_2_X12_310.TestFiles.Air_Output.xml";
            mapTester.ExecuteCompiled<UniTrans_2_X12_310>(sourceFile, expectedFile);
        ...
        static void InitialiseCodeMapsTestingContext()
		{
			...
			ctx.eHubClients.Add(new eHubClient { CC_ID = "DGLNYCCMB_N10" });
			ctx.eHubClients.Add(new eHubClient { CC_ID = "DGLNYCCMB_N10" });
			ctx.eHubTransformationSets.Add(new eHubTransformationSet { TS_Name = "Nine West 310 - Send A/R Invoices", eHubClient_Sender = ctx.eHubClients[0], eHubClient_Recipient = ctx.eHubClients[1] });
			...
			ctx.eHubCodeMapValues.Add(new eHubCodeMapValue { eHubCodeMapKey = ctx.eHubCodeMapKeys.Last(), eHubCodeSetResult = ctx.eHubCodeSetResults.Last(), CV_OutputCode = "DNYC" });
			ctx.eHubCodeSetResults.Add(new eHubCodeSetResult { eHubCodeSet = ctx.eHubCodeSets.Last(), CR_Order = 1, CR_Name = "GDX Version - N903" });
			ctx.eHubCodeMapValues.Add(new eHubCodeMapValue { eHubCodeMapKey = ctx.eHubCodeMapKeys.Last(), eHubCodeSetResult = ctx.eHubCodeSetResults.Last(), CV_OutputCode = "1.14" });
			ctx.eHubCodeSetResults.Add(new eHubCodeSetResult { eHubCodeSet = ctx.eHubCodeSets.Last(), CR_Order = 1, CR_Name = "GDX Action - N903" });
			...
			ctx.eHubCodeSetResults.Add(new eHubCodeSetResult { eHubCodeSet = ctx.eHubCodeSets.Last(), CR_Order = 1, CR_Name = "N722 - AIR" });
			ctx.eHubCodeMapValues.Add(new eHubCodeMapValue { eHubCodeMapKey = ctx.eHubCodeMapKeys.Last(), eHubCodeSetResult = ctx.eHubCodeSetResults.Last(), CV_OutputCode = "AFRT" });
			...
			ctx.eHubCodeSets.Add(new eHubCodeSet { CS_Name = "Equipment Service Code", eHubClient_Sender = ctx.eHubClients[0], eHubClient_Recipient = ctx.eHubClients[1], eHubTransformationSet = ctx.eHubTransformationSets[0] });
			ctx.eHubCodeSetResults.Add(new eHubCodeSetResult { eHubCodeSet = ctx.eHubCodeSets.Last(), CR_Order = 1, CR_Name = "N719" });
			ctx.eHubCodeMapKeys.Add(new eHubCodeMapKey { eHubCodeSet = ctx.eHubCodeSets.Last(), CK_Order = 1, CK_Key1Value = "%" });
			ctx.eHubCodeMapValues.Add(new eHubCodeMapValue { eHubCodeMapKey = ctx.eHubCodeMapKeys.Last(), eHubCodeSetResult = ctx.eHubCodeSetResults.Last(), CV_OutputCode = "HH" });
			...
			ctx.eHubCodeSetResults.Add(new eHubCodeSetResult { eHubCodeSet = ctx.eHubCodeSets.Last(), CR_Order = 1, CR_Name = "L009" });
			...
			ctx.ActionProcedures.Add(new ActionProcedure
			{
				...
				InputParms = new List<string> { 
			        "@TransformatonSetName", "Nine West 310 - Send A/R Invoices",
