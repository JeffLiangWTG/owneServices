...
using CargoWise.eHub.Clients.OSP.Transforms.EDIConsolsInternalToEDIConsolsInternal;
...
{
	[TestClass]
	public class EDIConsolsInternalToEDIConsolsInternalTests
	{
		[TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
		public void TestEDIConsolsInternalToEDIConsolsInternal()
		{
			InitialiseCodeMapsTestingContext();

			MapTester mapTester = new MapTester(Assembly.GetExecutingAssembly());

			string sourceFile = "EDIConsolsInternalToEDIConsolsInternal.TestFiles.EDIConsolsInternal_Input.xml";
			string expectedFile = "EDIConsolsInternalToEDIConsolsInternal.TestFiles.EDIConsolsInternal_Output.xml";
			mapTester.Execute<EDIConsolsInternalToEDIConsolsInternal>(sourceFile, expectedFile);
		}

		static void InitialiseCodeMapsTestingContext()
		{
			CodeMapsTestingContext ctx = new CodeMapsTestingContext();
			ctx.eHubClients.Add(new eHubClient { CC_ID = "GPLORDORD" });
			ctx.eHubClients.Add(new eHubClient { CC_ID = "OSPAKLAKL" });
			ctx.eHubTransformationSets.Add(new eHubTransformationSet { TS_Name = "GP Logistics xml-File - Import  Consols+Shipments", eHubClient_Sender = ctx.eHubClients[0], eHubClient_Recipient = ctx.eHubClients[1] });
			...
			ctx.eHubCodeMapKeys.Add(new eHubCodeMapKey { eHubCodeSet = ctx.eHubCodeSets.Last(), CK_Order = 3, CK_Key1Value = "IN" });
			...
			ctx.eHubCodeMapKeys.Add(new eHubCodeMapKey { eHubCodeSet = ctx.eHubCodeSets.Last(), CK_Order = 4, CK_Key1Value = "CF" });
			...
			ctx.eHubCodeSetResults.Add(new eHubCodeSetResult { eHubCodeSet = ctx.eHubCodeSets.Last(), CR_Order = 1, CR_Name = "Oceanbridge Code" });
			ctx.eHubCodeMapKeys.Add(new eHubCodeMapKey { eHubCodeSet = ctx.eHubCodeSets.Last(), CK_Order = 1, CK_Key1Value = "CFS" });
