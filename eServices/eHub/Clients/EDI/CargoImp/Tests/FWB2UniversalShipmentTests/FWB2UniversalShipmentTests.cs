...
namespace CargoWise.eHub.Clients.EDI.CargoImp.Tests.FWB2UniversalShipmentTests
{
	[TestClass]
	public class FWB2UniversalShipmentTests
	{
		[TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
		public void TestFWB2UniversalShipment()
		{
			InitialiseCodeMapsTestingContext();

			MapTester mapTester = new MapTester(Assembly.GetExecutingAssembly());

			string sourceFile = "FWB2UniversalShipmentTests.TestFiles.FWB_Inut.xml";
			string expectedFile = "FWB2UniversalShipmentTests.TestFiles.UniversalShipment_Output.xml";
			mapTester.Execute<FWB2UniversalShipment>(sourceFile, expectedFile);
		}
		...
		private static void InitialiseCodeMapsTestingContext()
		{
			...
			ctx.eHubTransformationSets.Add(new eHubTransformationSet { TS_Name = "Champ FWB - Import of Shipments", eHubClient_Sender = ctx.eHubClients[0], eHubClient_Recipient = ctx.eHubClients[1] });
			...
			ctx.eHubCodeMapValues.Add(new eHubCodeMapValue { eHubCodeMapKey = ctx.eHubCodeMapKeys.Last(), eHubCodeSetResult = ctx.eHubCodeSetResults.Last(), CV_OutputCode = "AYTestDP" });
			ctx.eHubCodeSetResults.Add(new eHubCodeSetResult { eHubCodeSet = ctx.eHubCodeSets.Last(), CR_Order = 2, CR_Name = "Package Type" });
			...
			ctx.eHubCodeMapKeys.Add(new eHubCodeMapKey { eHubCodeSet = ctx.eHubCodeSets.Last(), CK_Order = 1, CK_Key1Value = "K" });
