...
{
	public abstract class ComponentBase : IBaseComponent, IPersistPropertyBag, IComponentUI
	{
		...
		{
			return propertyInfos.GetOrAdd(this.GetType(), this.GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public).Where(p => { var b = p.GetCustomAttributes(true).OfType<BrowsableAttribute>().FirstOrDefault(); return b == null ? true : b.Browsable; }).ToArray());
		}
