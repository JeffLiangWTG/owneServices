...
namespace CargoWise.eHub.Products.NZCustoms.Tests
{
	class TestLogger : ILog
	{
		...
		{
			log.Append("Debug - ").AppendFormat(format, args).AppendLine();
		}
		...
		{
			log.Append("Info - ").AppendFormat(format, args).AppendLine();
		}
		...
		{
			log.Append("Warn - ").Append(message).Append(" Exception message: ").AppendLine(exception.Message);
		}
		...
		{
			log.Append("Warn - ").AppendFormat(format, args).AppendLine();
		}
	}
}
