<xsl:stylesheet exclude-result-prefixes="msxsl var s0" version="1.0" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
  ...
  <xsl:template name="HeaderItems">
    ...
    <xsl:element name="ns0:DecHead">
      ...
      <xsl:element name="ns0:GrossWet">
        <xsl:variable name="grossWeightOrg" select="floor(sum(msxsl:node-set($weightsOnInvoiceLines)/GrossWeightInKG/text()) * 100000) div 100000"/>
        <xsl:call-template name="RoundUpIfLessThanOne">
          <xsl:with-param name="OriginValue" select="$grossWeightOrg"/>
        </xsl:call-template>
      </xsl:element>
      <!-- 毛重 -->
      ...
      <xsl:element name="ns0:NetWt">
        <xsl:variable name="NetWtOrg" select="floor(sum(msxsl:node-set($weightsOnInvoiceLines)/NetWeightInKG/text()) * 100000) div 100000"/>
        <xsl:call-template name="RoundUpIfLessThanOne">
          <xsl:with-param name="OriginValue" select="$NetWtOrg"/>
        </xsl:call-template>
      </xsl:element>
      <!-- 净重 -->
      ...
      <xsl:element name="ns0:DataSource">
        <xsl:value-of select="concat('   1', $LinkedEntryInstruction/s0:SubStyle/s0:Code)" />
      </xsl:element>
      <!-- 扩展字段 -->
      ...
      <!-- B/L号 -->
      <xsl:element name="ns0:InspOrgCode">
        <xsl:if test="$NeedToSumbitCIQData">
          <xsl:value-of select="/s0:UniversalShipment/s0:Shipment/s0:CustomsReferenceCollection/s0:CustomsReference[s0:Type/s0:Code='COF' and s0:SubType/s0:Code='ENT']/s0:Reference"/>
        </xsl:if>
      </xsl:element>
      <!-- 口岸检验检疫机关 -->
      ...
      <!-- 关联理由 -->
      <xsl:element name="ns0:VsaOrgCode">
        <xsl:if test="$NeedToSumbitCIQData">
          <xsl:value-of select="/s0:UniversalShipment/s0:Shipment/s0:CustomsReferenceCollection/s0:CustomsReference[s0:Type/s0:Code='COF' and s0:SubType/s0:Code='VIS']/s0:Reference"/>
        </xsl:if>
      </xsl:element>
      <!-- 领证机关 -->
      ...
      <!-- 无其他包装 -->
      <xsl:element name="ns0:OrgCode">
        <xsl:if test="$NeedToSumbitCIQData">
          <xsl:value-of select="/s0:UniversalShipment/s0:Shipment/s0:CustomsReferenceCollection/s0:CustomsReference[s0:Type/s0:Code='COF' and s0:SubType/s0:Code='ACC']/s0:Reference"/>
        </xsl:if>
      </xsl:element>
      <!-- 检验检疫受理机关 -->
  ...
  <xsl:template name="EntryLine2DecList">
    ...
    <xsl:element name="ns0:DecList">
      ...
      <xsl:element name="ns0:CiqName">
        <xsl:if test="$NeedToSumbitCIQData">
          <xsl:value-of select="$FirstLinkedInvoiceLine/s0:AddInfoCollection/s0:AddInfo[s0:Key='CIQTariffDescription']/s0:Value"/>
        </xsl:if>
      </xsl:element>
      <!-- TODO: check with CSY if we need it  -->
      <!-- 检验检疫名称 -->
      <xsl:if test="$NeedToSumbitCIQData">
  ...
  <xsl:template name="RoundUpIfLessThanOne">
    <xsl:param name="OriginValue" />
    <xsl:choose>
      <xsl:when test="$OriginValue &lt; 1">1</xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="$OriginValue"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
