<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		<xsl:apply-templates select="/s0:SAS135Output" />
	</xsl:template>

	<xsl:template match="s0:SAS135Output">
					...
					<ns0:Event>
						<ns0:DataContext>
							<ns0:ActionPurpose>
								<ns0:Code>
									<xsl:choose>
										<xsl:when test="normalize-space(s0:OutputCommonField/s0:OutputMessageCode) = 'SAS1350'">NRC</xsl:when>
						...
						<ns0:ContextCollection>
							...
							<xsl:call-template name="ContextKeyValuePair">
								...
								<xsl:with-param name="value">
									...
									<xsl:if test="s0:MasterBillIdentifier='M'">
										<xsl:call-template name="HTMLTR">
											<xsl:with-param name="name" select="'Master Bill Identifier'" />
											<xsl:with-param name="value" select="'M - House Bill Advance Filling is to be implemented by consigned freight forwarding business operator (NVOCC) et al.'" />
										</xsl:call-template>
									</xsl:if>
									<xsl:if test="s0:DiscrepancyIdentifier='*'">
										<xsl:call-template name="HTMLTR">
											<xsl:with-param name="name" select="'Master Bill Discrepancy Identifier'" />
											<xsl:with-param name="value" select="'* - The Master Bill Number is not registered via AMR procedure'" />
										</xsl:call-template>
									</xsl:if>
									<xsl:if test="s0:HouseBillRegisterCompletionIdentifier='E'">
										<xsl:call-template name="HTMLTR">
											<xsl:with-param name="name" select="'House Bill Register Completion Identifier'" />
											<xsl:with-param name="value" select="'E - Register of House Bill relevant to the Master Bill is Complete'" />
										</xsl:call-template>
									</xsl:if>
									<xsl:call-template name="HTMLTR">
