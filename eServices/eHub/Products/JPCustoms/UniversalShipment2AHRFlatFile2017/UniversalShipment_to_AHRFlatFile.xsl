              ...
              <xsl:value-of select="ScriptNS3:GetSPPassword()" />
					...
					<xsl:if test="$AllHouseBillRegistered!='Y' or $HouseBillOfLadingNumber">
						<xsl:element name="ns0:HouseBillInfo">
							...
							<xsl:element name="ns0:VesselInformationChangeIdentifier">
								<xsl:variable name="VesselInformationChangeIdentifier" select="../../*[local-name()='AddInfoCollection']/*[local-name()='AddInfo' and *[local-name()='Key']/text()='JPVesselDetailsChanged'][1]/*[local-name()='Value']" />
								<xsl:value-of select="userCSharp:StringMaxAllowed(normalize-space($VesselInformationChangeIdentifier),1)"/>
							</xsl:element>
