...
{
  [TestClass]
  public class UShipment2DTI_Tests
  {
    const string filePath = "APPLUS.DTI.TestFiles.";

    [TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
    public void TestUShipment2DTI()
    {
    ...
    public void AssertSchema(string expectedOutputFile)
    {
      ...
      var schemaValidator = XMLValidator.Validate<dti>(expectedOutput);
      Assert.AreEqual(string.Empty, schemaValidator, "UShipment2DTI is not valid: \r\n " + schemaValidator);
    }
    ...
    void AssertMapping(string inputFile, string expectedOutputFile, string serviceProvider, string routingPartyValue = "")
    {
      ...
      var destinationParty = serviceProvider + "_DTI1";
      ...
      mockContextAccessor.Expect(x => x.SetContextProperty("OverrideFilename", "http://cargowise.com/ehub/processing/2010/06", "DTI_TESTSENDER_1"));
      ...
      mockCodeMapper.Expect(x => x.GetRecipientCode("FORWARDING_PORT_MESSAGE", "FORWARDING_PORT_MESSAGE", "FPM System Configuration", "Port Settings", "Name", serviceProvider + "_DTI1")).Return(serviceProvider).Repeat.Any();
      ...
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue(serviceProviderMSGID, serviceProvider, "TESTSENDER", formattedInterchangeNumber, "Underbond Movement Request - Import (DTI)", "DocumentName"));
      ...
      mapTester.ExecuteCompiled<UShipment2DTI>(input, expectedOutput);
