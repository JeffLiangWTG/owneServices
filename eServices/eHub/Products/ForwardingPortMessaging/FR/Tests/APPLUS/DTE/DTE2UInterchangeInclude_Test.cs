...
{
  [TestClass]
  public class DTE2UInterchangeInclude_Test
  {
    const string filePath = "APPLUS.DTE.TestFiles_DTE_NOTIF.";

    [TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
    public void TestDTE2UInterchangeInclude()
    {
    ...
    void AssertMapping(string inputFile, string expectedOutputFile, string eHubID = "")
    {
      ...
      mockCodeMapper.Expect(x => x.CallActionProcedureHelper("SelectSubscribedReference", "@reference", "@senderId", senderID, "@recipientId", "", "@ST_ID", serviceProviderMSGID, "@value", "MGI0000001", "@referenceType", "DocumentName")).Return("DTE Notification").Repeat.Any();

      mockCodeMapper.Expect(x => x.CallActionProcedureHelper("SelectSubscribedReference", "@reference", "@senderId", senderID, "@recipientId", "", "@ST_ID", serviceProviderSICID, "@value", "DTE_LDE60736636")).Return("MGI0000001").Repeat.Any();
      mockCodeMapper.Expect(x => x.CallActionProcedureHelper("SelectSubscribedReference", "@reference", "@senderId", senderID, "@recipientId", "", "@ST_ID", serviceProviderSICID, "@value", "DTE_DTE60912320")).Return("MGI0000001").Repeat.Any();
      ...
      mockCodeMapper.Expect(x => x.GetRecipientCode(senderID, senderID, serviceProvider + " System Configuration", "ContainerTypeToISOCode", "MGI Code", "45R1")).Return("ISO45R1").Repeat.Any();
      mockCodeMapper.Expect(x => x.GetRecipientCode("FORWARDING_PORT_MESSAGE", "FORWARDING_PORT_MESSAGE", "FPM System Configuration", "SCAC (Inbound)", "SCAC", senderID, "CMFR")).Return("SCACCMFR").Repeat.Any();
      mockCodeMapper.Expect(x => x.GetRecipientCode("MGI", "MGI", "MGI System Configuration", "Acknowledgment Status", "Output Code", "ADO")).Return("OK").Repeat.Any();
      ...
      mapTester.ExecuteCompiled<DTE2UInterchangeInclude>(input, expectedOutput);
