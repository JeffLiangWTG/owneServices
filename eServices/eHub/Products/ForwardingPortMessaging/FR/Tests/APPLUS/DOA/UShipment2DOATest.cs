...
using Rhino.Mocks;
using CargoWise.eHub.Products.ForwardingPortMessaging.FR.APPLUS.DOA;

namespace CargoWise.eHub.Products.ForwardingPortMessaging.FR.Tests
{
  [TestClass]
  public class UShipment2DOATest
  {
    const string filePath = "APPLUS.DOA.TestFiles.";

    [TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
    public void TestUShipment2DOA()
    {
      AssertMapping("Test1_input.xml", "Test1_SOGET_output.xml", "SOGET", true);
      AssertMapping("Test1_input.xml", "Test1_MGI_output.xml", "MGI", true);
      AssertMapping("Test1_input.xml", "Test1_MGI_output.xml", "MGI", true, routingPartyValue: "SOGET");
      AssertMapping("Test2_input.xml", "Test2_SOGET_output.xml", "SOGET", false);
      AssertMapping("Test2_input.xml", "Test2_MGI_output.xml", "MGI", false);
    }

    void AssertMapping(string inputFile, string expectedOutputFile, string serviceProvider, bool isExport, string routingPartyValue = "")
    {
      ...
      var destinationParty = serviceProvider + "_CAED1";
      ...
      mockContextAccessor.Expect(x => x.SetContextProperty("OverrideFilename", "http://cargowise.com/ehub/processing/2010/06", "DOA_TESTSENDER_1"));
      ...
      mockCodeMapper.Expect(x => x.GetRecipientCode("FORWARDING_PORT_MESSAGE", "FORWARDING_PORT_MESSAGE", "FPM System Configuration", "Port Settings", "Name", serviceProvider + "_CAED1")).Return(serviceProvider).Repeat.Any();
      ...
      mockCodeMapper.Expect(x => x.GetRecipientCode("FORWARDING_PORT_MESSAGE", "FORWARDING_PORT_MESSAGE", "FPM System Configuration", "Service Provider Settings", "DestinationUser", serviceProvider, "FRMRS", routingParty)).Return(serviceProvider + "_U");
      mockCodeMapper.Expect(x => x.GetRecipientCode("FORWARDING_PORT_MESSAGE", "FORWARDING_PORT_MESSAGE", "FPM System Configuration", "Service Provider Settings", "DestinationParty", serviceProvider, "FRMRS", routingParty)).Return(serviceProvider + "_P");

      mockDataModelAccessor.Expect(x => x.GetClientRegistrationCode("TESTSENDER", "FRMRS", serviceProvider)).Return("APPLUS001,DHLTR");
      ...
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue(serviceProviderMSGID, serviceProvider, "TESTSENDER", formattedInterchangeNumber, "Customs Pre-Check (DOA) - " + (isExport ? "Export" : "Import"), "DocumentName"));
      ...
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue(serviceProviderID, serviceProvider, "TESTSENDER", "APPLUS001,DHLTR"));
      ...
      mapTester.ExecuteCompiled<UShipment2DOA>(input, expectedOutput);
