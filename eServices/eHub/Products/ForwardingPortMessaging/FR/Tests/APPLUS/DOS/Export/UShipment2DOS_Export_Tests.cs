...
{
  [TestClass]
  public class UShipment2DOS_Export_Tests
  {
    const string filePath = "APPLUS.DOS.Export.TestFiles.";
    ...
    public void AssertSchema(string expectedOutputFile)
    {
      ...
      var schemaValidator = XMLValidator.Validate<dos_export>(expectedOutput);
    ...
    void AssertMapping(string inputFile, string expectedOutputFile, string serviceProvider, string routingPartyValue = "", string previousMessageIdentifier = "")
    {
      ...
      mockCodeMapper.Expect(x => x.CallActionProcedureHelper("GetCounterValue", "@value", "@name", "CargoWise.eHub.Products.ForwardingPortMessaging.FR.APPLUS", "@maxlength", "14")).Return("99").Repeat.Any();
			...
			mockCodeMapper.Expect(x => x.CallActionProcedureHelper("SelectSubscribedReference", "@reference", "@senderId", serviceProvider, "@recipientId", "TESTSENDER", "@ST_ID", serviceProviderMSGID, "@value", "C00001266_DOS_Export", "@referenceType", "MessageReference")).Return(previousMessageIdentifier).Repeat.Any();
      ...
      mockDataModelAccessor.Expect(x => x.GetClientRegistrationCode("TESTSENDER", "FRMRS", serviceProvider)).Return("APPLUS001,DHLLTR");
			...
			{
				mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue(serviceProviderMSGID, serviceProvider, "TESTSENDER", "C00001266_DOS_Export", messageIdentifier, "MessageReference")).Repeat.Any();
			}
