...
{
  [TestClass]
  public class UShipment2DOS_Import_Tests
  {
    const string filePath = "APPLUS.DOS.Import.TestFiles.";

    [TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
    public void TestUShipment2DOS()
    {
      ...
      AssertMapping("Test2_noThirdPartyAddress_input.xml", "Test2_noThirdPartyAddress_SOGET_output.xml", "SOGET");
      AssertMapping("Test3_noCarrierBookingAgent_input.xml", "Test3_noCarrierBookingAgent_MGI_output.xml", "MGI");

      //Test optional fields : tiers-dos/booking-dos/billOfLading-dos 
      AssertMapping("Test4_input.xml", "Test4_output.xml", "MGI");

      //Test optional fields : Export
      AssertMapping("Test5_input.xml", "Test5_output.xml", "MGI");

      AssertMapping("Test6_input.xml", "Test6_SOGET_output.xml", "SOGET");
      AssertMapping("Test6_input.xml", "Test6_MGI_output.xml", "MGI");

      AssertSchema("Test1_SOGET_output.xml");
      AssertSchema("Test1_MGI_output.xml");
      AssertSchema("Test2_noThirdPartyAddress_SOGET_output.xml");
      AssertSchema("Test3_noCarrierBookingAgent_MGI_output.xml");
      AssertSchema("Test4_output.xml");
      AssertSchema("Test5_output.xml");
      AssertSchema("Test6_SOGET_output.xml");
      AssertSchema("Test6_MGI_output.xml");
    }

    public void AssertSchema(string expectedOutputFile)
    {
      var expectedOutput = filePath + expectedOutputFile;
      var schemaValidator = XMLValidator.Validate<dos_import>(expectedOutput);
      Assert.AreEqual(string.Empty, schemaValidator, "UShipment2DOS is not valid: \r\n " + schemaValidator);
    }
    ...
    void AssertMapping(string inputFile, string expectedOutputFile, string serviceProvider, string routingPartyValue = "", string previousMessageIdentifier = "")
    {
      ...
      var destinationParty = serviceProvider + "_DOS1";
      ...
      mockContextAccessor.Expect(x => x.SetContextProperty("OverrideFilename", "http://cargowise.com/ehub/processing/2010/06", "DOS_TESTSENDER_1"));
      ...
      mockCodeMapper.Expect(x => x.CallActionProcedureHelper("SelectSubscribedReference", "@reference", "@senderId", serviceProvider, "@recipientId", "TESTSENDER", "@ST_ID", serviceProviderMSGID, "@value", "C00001266_DOS_Import", "@referenceType", "MessageReference")).Return(previousMessageIdentifier).Repeat.Any();

      mockCodeMapper.Expect(x => x.GetRecipientCode("FORWARDING_PORT_MESSAGE", "FORWARDING_PORT_MESSAGE", "FPM System Configuration", "Port Settings", "Name", serviceProvider + "_DOS1")).Return(serviceProvider).Repeat.Any();
      ...
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue(serviceProviderMSGID, serviceProvider, "TESTSENDER", messageIdentifier, "File Creation Request (DOS) - Import", "DocumentName")).Repeat.Any();
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue(serviceProviderMSGID, serviceProvider, "TESTSENDER", messageIdentifier, "File Creation Request (DOS) - Export", "DocumentName")).Repeat.Any();
      ...
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue(serviceProviderMSGID, serviceProvider, "TESTSENDER", messageIdentifier, "C00001266", "RequestID")).Repeat.Any();
      ...
      {
        mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue(serviceProviderMSGID, serviceProvider, "TESTSENDER", "C00001266_DOS_Import", messageIdentifier, "MessageReference")).Repeat.Any();
      }
      ...
      mapTester.ExecuteCompiled<UShipment2DOS>(input, expectedOutput);
