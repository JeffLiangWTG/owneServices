  ...
  <xsl:template name="CreateUniversalEvent">
    ...
    <xsl:variable name="sicSubscribedMessageReference">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="CodeMapper:CallActionProcedureHelper('SelectSubscribedReference', '@reference', '@senderId', $SenderID, '@recipientId', '', '@ST_ID', $serviceProviderSTID, '@value',  concat('LDE_', $messageReference))"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
        ...
        <Event>
          ...
          <ContextCollection>
            ...
            <xsl:call-template name="CreateContext">
              ...
              <xsl:with-param name="value" select="$lmarchandise-dec/@extcbk"/>
            </xsl:call-template>
            <xsl:call-template name="CreateContext">
              <xsl:with-param name="type" select="'CBKReference'"/>
              <xsl:with-param name="value" select="$lmarchandise-dec/@cbk"/>
            </xsl:call-template>
            <xsl:call-template name="CreateContext">
              <xsl:with-param name="type" select="'LDEReference'"/>
              <xsl:with-param name="value" select="$messageReference"/>
            </xsl:call-template>
            <xsl:call-template name="CreateContext">
              <xsl:with-param name="type" select="'VesselName'"/>
              <xsl:with-param name="value" select="$lmarchandise-dec/@nav"/>
            </xsl:call-template>
            <xsl:call-template name="CreateContext">
              <xsl:with-param name="type" select="'ECTReference'"/>
              <xsl:with-param name="value" select="$lmarchandise-dec/@sic"/>
            </xsl:call-template>
            <xsl:call-template name="CreateContext">
              <xsl:with-param name="type" select="'ATPReference'"/>
              <xsl:with-param name="value" select="$lmarchandise-dec/@atp"/>
            </xsl:call-template>

            <xsl:variable name="carrierCode">
              <xsl:choose>
                <xsl:when test="$lmarchandise-dec/@afret !=''">
                  <xsl:value-of select="CodeMapper:GetRecipientCode('FORWARDING_PORT_MESSAGE', 'FORWARDING_PORT_MESSAGE', 'FPM System Configuration', 'SCAC (Inbound)', 'SCAC', $SenderID, $lmarchandise-dec/@afret)"/>
                </xsl:when>
                <xsl:otherwise></xsl:otherwise>
              </xsl:choose>
            </xsl:variable>
            ...
            <xsl:call-template name="CreateContext">
              ...
              <xsl:with-param name="value" select="$lieu-dec_2/@manut"/>
            </xsl:call-template>
            <xsl:call-template name="CreateContext">
