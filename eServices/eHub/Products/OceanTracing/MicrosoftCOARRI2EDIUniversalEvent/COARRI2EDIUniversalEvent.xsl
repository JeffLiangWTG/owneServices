            ...
            <ns0:Event>
              ...
              <xsl:variable name="location">
                <xsl:choose>
                  ...
                  <xsl:when test="$eventType='FLO' and $LOC9!=''">
                    <xsl:value-of select="$LOC9"/>
                  </xsl:when>
                  <xsl:when test="$eventType='FUL' and $LOC11!=''">
              ...
              <ns0:EventParameters>
                ...
                <ns0:Location>
                  <xsl:call-template name="GetValue">
                    <xsl:with-param name="value1" select="$location" />
                    <xsl:with-param name="fallbackValue" select="s0:LOC_2[LOC01/text()='165']/s0:C517_2/C51701/text()" />
                  </xsl:call-template>
                </ns0:Location>
              </ns0:EventParameters>
              <ns0:EventReference>
              ...
              <xsl:element name="ns0:ContextCollection">
                ...
                <xsl:for-each select="s0:LOC_2">
                  <xsl:variable name="unlocoContextName">
                    <xsl:choose>
                      ...
                      <xsl:when test="LOC01/text()='164'">ContainerFinalDestinationUNLOCO</xsl:when>
                    </xsl:choose>
                  </xsl:variable>
                ...
                <xsl:variable name="containerWeightMEA" select="s0:MEA[MEA01/text()='AAE' and s0:C502/C50201/text()='G']" />
                <xsl:variable name="containerWeight" select="$containerWeightMEA/s0:C174/C17402/text()" />
                <xsl:if test="$containerWeight!=''">
                  <xsl:variable name="measurementUnit" select="CodeMapper:GetRecipientCode('OCEAN_CONTAINER_TRACING', 'OCEAN_CONTAINER_TRACING', 'OCT System Configuration', 'Unit of Measurement', 'ediEnterprise Code', $codeMappingPortCode, $containerWeightMEA/s0:C174/C17401/text())" />
                  <xsl:call-template name="GenerateContext">
                ...
                <xsl:variable name="containerTemperatureTMP" select="s0:TMP[TMP01/text() = '2']" />
                <xsl:variable name="containerTemperature" select="$containerTemperatureTMP/s0:C239/C23901/text()" />
                <xsl:if test="$containerTemperature != ''">
                  <xsl:call-template name="GenerateContext">
                    <xsl:with-param name="Type" select="'ContainerTemperatureSetting'" />
                    <xsl:with-param name="Value" select="concat($containerTemperature, $containerTemperatureTMP/s0:C239/C23902/text())" />
                  </xsl:call-template>
                </xsl:if>
                ...
                <xsl:variable name="containerGoodsDescription" select="s0:FTX_2[FTX01/text()='AAA']/s0:C108_2/C10801/text()" />
                <xsl:call-template name="GenerateContext">
