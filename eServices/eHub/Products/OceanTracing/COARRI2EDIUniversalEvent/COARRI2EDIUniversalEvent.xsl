...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <xsl:apply-templates select="/s0:EFACT_D95B_COARRI"/>
  </xsl:template>
  
  <xsl:template match="/s0:EFACT_D95B_COARRI">
        ...
        <xsl:for-each select="//s0:EQDLoop1">
          <ns0:UniversalEvent>
            <ns0:Event>
              ...
              <ns0:EventType>
                <xsl:value-of select="CodeMapper:GetRecipientCode('POROFAAKL', 'POROFAAKL', 'Ocean Containers - COARRI to UniversalEvent', 'Event Type', 'ediEnterprise code', //s0:BGM/s0:C002/C00201/text())"/>
              </ns0:EventType>
              ...
              <xsl:element name="ns0:ContextCollection">
                ...
                <xsl:call-template name="GenerateContext">
                  ...
                  <xsl:with-param name="Value" select="s0:EQD/s0:C237/C23701/text()"/>
                </xsl:call-template>
                <xsl:call-template name="GenerateContext">
                ...
                <xsl:call-template name="GenerateContext">
                  ...
                  <xsl:with-param name="Value" select="CodeMapper:GetRecipientCode('POROFAAKL', 'POROFAAKL', 'Ocean Containers - COARRI to UniversalEvent', 'Container Ownership', 'ediEnterprise code', s0:EQD/EQD04/text())"/>
                </xsl:call-template>
                <xsl:call-template name="GenerateContext">
                  <xsl:with-param name="Type" select="'ContainerMovementType'"/>
                  <xsl:with-param name="Value" select="CodeMapper:GetRecipientCode('POROFAAKL', 'POROFAAKL', 'Ocean Containers - COARRI to UniversalEvent', 'Container Movement Type', 'ediEnterprise code', s0:EQD/EQD05/text())"/>
                </xsl:call-template>
                <xsl:call-template name="GenerateContext">
                ...
                <xsl:call-template name="GenerateContext">
                  ...
                  <xsl:with-param name="Value" select="CodeMapper:GetRecipientCode('POROFAAKL', 'POROFAAKL', 'Ocean Containers - COARRI to UniversalEvent', 'Container Mode', 'ediEnterprise code', s0:TMD/s0:C219/C21901/text())"/>
                </xsl:call-template>
                ...
                <xsl:for-each select="s0:LOC_2">
                  ...
                  <xsl:variable name="LocationName">
                    <xsl:choose>
                      ...
                      <xsl:when test="$LOC01 = 164">ContainerFinalDestinationUNLOCO'</xsl:when>
                ...
                <xsl:variable name="Weight" select="s0:MEA/s0:C174/C17402/text()"/>
                <xsl:if test="$Weight != '' and s0:MEA/MEA01/text() = 'AAE' and s0:MEA/s0:C502/C50201/text() = 'G'">
                  <xsl:variable name="Unit" select="CodeMapper:GetRecipientCode('POROFAAKL', 'POROFAAKL', 'Ocean Containers - COARRI to UniversalEvent', 'Unit of Measurement', 'ediEnterprise code', s0:MEA/s0:C174/C17401/text())"/>
                  <xsl:call-template name="GenerateContext">
                    <xsl:with-param name="Type" select="'ContainerGrossWeight'"/>
                    <xsl:with-param name="Value" select="normalize-space(concat($Weight, ' ', $Unit))"/>
                  </xsl:call-template>
                </xsl:if>
                <xsl:variable name="ContainerTemperatureSetting" select="s0:TMP[TMP01/text() = '2']/s0:C239/C23901/text()"/>
                <xsl:if test="$ContainerTemperatureSetting != ''">
                  <xsl:variable name="ContainerTemperatureUnit">
                    <xsl:if test="s0:TMP[TMP01/text() = '2']/s0:C239/C23902/text() != ''">
                      <xsl:value-of select="concat(' ', s0:TMP[TMP01/text() = '2']/s0:C239/C23902/text())"/>
                    </xsl:if>
                  </xsl:variable>
                  <xsl:call-template name="GenerateContext">
                    <xsl:with-param name="Type" select="'ContainerTemperatureSetting'"/>
                    <xsl:with-param name="Value" select="concat($ContainerTemperatureSetting, $ContainerTemperatureUnit)"/>
                  </xsl:call-template>
                </xsl:if>
