...
{
	[TestClass]
	public class AnswerSchemaTests
	{
		...
		{
			var ff_source = "AnswerSchemaTests.TestFiles.Text." + testcase + ".txt";
			var xml_source = "AnswerSchemaTests.TestFiles.Text." + testcase + ".xml";

			var xml = GetResourceAsString(xml_source);
			var generated_xml = XDocument.Load(SchemaTester<AnswerSchema>.ParseFF(GetEmbeddedResource(ff_source))).ToString(); // XDocument is used for formatting
			Assert.AreEqual(xml, generated_xml);
		}

		[TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
		public void AnswerSchemaTest()
		{
			AssertTestCase("test01");
			AssertTestCase("test02");
			AssertTestCase("test03");
		}
	}
}
