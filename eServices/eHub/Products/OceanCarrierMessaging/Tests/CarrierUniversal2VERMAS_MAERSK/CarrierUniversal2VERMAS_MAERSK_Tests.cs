...
using CargoWise.eHub.Products.OceanCarrierMessaging.Transforms.CarrierUniversal2VERMAS_MAERSK;
...
{
  [TestClass]
  public class CarrierUniversal2VERMAS_MAERSK_Tests
  {
    const string filePath = "CarrierUniversal2VERMAS_MAERSK.TestFiles.";

    [TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
    public void TestCarrierUniversal2Vermas_MAERSK()
    {
    ...
    void AssertMapping(string inputFile, string expectedOutputFile)
    {
      ...
      mockCodeMapper.Expect(x => x.CallActionProcedureHelper("GetCounterValue", "@value", "@name", "CargoWise.eHub.Products.OceanCarrierMessaging.Transforms.MAERSK.UNH1", "@maxlength", "14")).Return("88");
      mockCodeMapper.Expect(x => x.CallActionProcedureHelper("GetCounterValue", "@value", "@name", "CargoWise.eHub.Products.OceanCarrierMessaging.Transforms.MAERSK.BGM", "@maxlength", "14")).Return("1");
      mockDataModelAccessor.Expect(x => x.GetClientRegistrationCode("TESTSENDER__1", "BN1", "MAERSK")).Return("CGWS");
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue("MAEID", "MAERSK", "TESTSENDER__1", "CGWS"));
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue("MAEMSG", "MAERSK", "TESTSENDER__1", "88", "C00678281"));
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue("MAEMSG", "MAERSK", "TESTSENDER__1", "MAE0000000001", "C00678281"));
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue("MAEMSG", "MAERSK", "TESTSENDER__1", "C00678281", "MAE0000000001"));
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue("MAEMSG", "MAERSK", "TESTSENDER__1", "36ACCF3C-893F-40CE-BA01-1D8CBE2DF678", "88"));
      mockCodeMapper.Expect(x => x.CallActionProcedureHelper("SelectSubscribedReference", "@reference", "@senderId", "MAERSK", "@recipientId", "TESTSENDER__1", "@ST_ID", "MAEMSG", "@value", "C00678281")).Return("");
      ...
      mockContextAccessor.Expect(x => x.SetContextProperty("DestinationPartyReceiverIdentifier", "http://schemas.microsoft.com/Edi/PropertySchema", "MAEU"));
      ...
      mapTester.Execute<CarrierUniversal2VERMAS_MAERSK>(input, expectedOutput);
