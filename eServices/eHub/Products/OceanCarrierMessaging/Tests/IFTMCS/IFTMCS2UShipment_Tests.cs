...
{
  [TestClass]
  public class IFTMCS2UShipment_Tests
  {
    const string filePath = "IFTMCS.IFTMCS2UShipment.";

    [TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
    public void Test_IFTMCS2UShipment()
    {
      AssertMapping("Test1_input.xml", "Test1_output.xml", "CARGOSMART", "CGSMSG", "CGS000000100001", SubscriberConsolReferenceType.JobNumber, "MAA", "Bill of Lading Withdrawn", subscriber1: "subscriber1");
      AssertMapping("Test2_input.xml", "Test2_output.xml", "CARGOSMART", "CGSMSG", "CGS000000100001", SubscriberConsolReferenceType.JobNumber, "MWA", "Bill of Lading Withdrawn", subscriber2: "subscriber2");
      AssertMapping("Test3_input.xml", "Test3_output.xml", "CARGOSMART", "CGSMSG", "CGS000000100001", SubscriberConsolReferenceType.JobNumber, "MWA", "Bill of Lading Withdrawn", subscriber3: "subscriber3");
      ...
      AssertMapping("Test5_input.xml", "Test5_output.xml", "CARGOSMART", "CGSMSG", "C00001005", SubscriberConsolReferenceType.JobNumber, "MWA", "Bill of Lading Withdrawn", "N", "", shipmentType: "CLD");
      ...
      AssertMapping("Test10_include_all_node_input.xml", "Test10_include_all_node_output.xml", "CARGOSMART", "CGSMSG", "CGS000000100001", SubscriberConsolReferenceType.JobNumber, "MAA", "Bill of Lading Confirmed");
    ...
    void AssertMapping(string inputFile, string expectedOutputFile, string senderID, string ST_ID, string messageReferenceRFFSI, SubscriberConsolReferenceType subscriberConsolReferenceType,
                       string eventCode, string eventRef, string isDirectValue = "N", string expectedUNB2 = "UNB_SCAC", string rffValue = "SI", string rffFallbackValue = "ZZZ", string shipmentType = "AGT", bool isCTS = false, string subscriber1 = "", string subscriber2 = "", string subscriber3 = "")
    {
      ...
      mockContextAccessor.Stub(x => x.GetContextProperty("DestinationParty", "http://schemas.microsoft.com/BizTalk/2003/system-properties")).Return(recipientID);
      ...
      if (isCTS)
      {
        mockContextAccessor.Expect(x => x.SetContextProperty("DestinationParty", "http://schemas.microsoft.com/BizTalk/2003/system-properties", ""));
      }
      ...
      mockCodeMapper.Expect(x => x.GetRecipientCode(Arg<string>.Is.Equal("SHIPPING_INSTRUCTION"), Arg<string>.Is.Equal("SHIPPING_INSTRUCTION"), Arg<string>.Is.Equal("OCM System Configuration"),
                                              Arg<string>.Is.Equal("Package Type ISO"), Arg<string>.Is.Equal("Package Type"), Arg<string>.Is.Anything)).Return("").Repeat.Any();

      mockCodeMapper.Expect(x => x.CallActionProcedureHelper(Arg<string>.Is.Equal("SelectSubscribedReference"), Arg<string>.Is.Equal("@reference"), Arg<string>.Is.Equal("@senderId"), Arg<string>.Is.Anything, Arg<string>.Is.Equal("@recipientId"), Arg<string>.Is.Equal(""), Arg<string>.Is.Equal("@ST_ID"), Arg<string>.Is.Anything, Arg<string>.Is.Equal("@value"), Arg<string>.Is.Anything, Arg<string>.Is.Equal("@referenceType"), Arg<string>.Is.Equal("ForwardingType"))).Return("ForwardingConsol").Repeat.Any();
      ...
      mockCodeMapper.Expect(x => x.GetRecipientCode("OCMCARGOWISE", "OCMCARGOWISE", "OCM Cargowise System Configuration", "SCAC (Inbound)", "SCAC", senderID, "424280015")).Return("MAEU424280015").Repeat.Any();
      ...
      mockCodeMapper.Expect(x => x.CallActionProcedureHelper("SelectSubscribedClients", "", "@senderId", senderID, "@ST_ID", ST_ID, "@value", messageReferenceRFFSI)).Return("DirectSubscriber").Repeat.Any();
      ...
      mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue("OCMCNF", recipientID, senderID, $"KKLUTW0350519|{UNB5}", $"NUMNUMNUM|C00001005|MAEU424280015", "IFTMCS")).Repeat.Any();
      mockDataModelAccessor.Expect(x => x.GeteHubIDByCode("C08201_HI", senderID, false)).Return("C08201_HI").Repeat.Any();
      mockDataModelAccessor.Expect(x => x.GeteHubIDByCode("11300121", senderID, false)).Return("11300121").Repeat.Any();
      mockDataModelAccessor.Expect(x => x.GeteHubIDByCode("825595", senderID, false)).Return("11300121").Repeat.Any();

      mockSubscriptionHelper.Expect(x => x.SelectSubscriptionsByValue(ST_ID, messageReferenceRFFSI, "JobNumber")).Repeat.Any();
      mockSubscriptionHelper.Expect(x => x.SelectSubscriptionsByValue(ST_ID, messageReferenceRFFSI, "IFTMIN")).Repeat.Any();
      mockSubscriptionHelper.Expect(x => x.SelectSubscriptionsByValue(ST_ID, messageReferenceRFFSI)).Repeat.Any();
      if (subscriber1 != string.Empty)
      {
        mockSubscriptionHelper.Expect(x => x.GetSubscriber()).Return(subscriber1).Repeat.Any();
      }
      else if (subscriber2 != string.Empty)
      {
        mockSubscriptionHelper.Expect(x => x.GetSubscriber()).Return(subscriber2).Repeat.Any();
      }
      else
      {
        mockSubscriptionHelper.Expect(x => x.GetSubscriber()).Return(subscriber3 == string.Empty ? "TestSubscriber" : subscriber3).Repeat.Any();
      }
      ...
      mapTester.Execute<IFTMCS2UShipment_D99B>(input, expectedOutput);
