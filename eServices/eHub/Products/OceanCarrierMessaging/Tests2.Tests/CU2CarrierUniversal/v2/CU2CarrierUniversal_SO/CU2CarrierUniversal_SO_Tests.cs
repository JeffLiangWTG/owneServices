...
using CargoWise.eHub.Products.OceanCarrierMessaging.Transforms.CU2CarrierUniversal.v2;
...
{
	[TestClass]
	public class CU2CarrierUniversal_SO_Tests
	{
		const string filePath = "CU2CarrierUniversal.v2.CU2CarrierUniversal_SO.TestFiles.";

		[TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
		public void TestCU2CarrierUniversal_SO()
		{
		...
		void AssertMapping(string inputFile, string expectedOutputFile, string carrierCode, string carrierName)
		{
			...
			var destinationParty = carrierName + "_SO";

			var serviceProvider = destinationParty;

			mockDataModelAccessor.Expect(x => x.GetClientRegistrationCode("TESTSENDER__1", "BNE", mappedCarrierName)).Return("ECU").Repeat.Any();
			mockContextAccessor.Expect(x => x.SetContextProperty("OverrideFilename", "http://cargowise.com/ehub/processing/2010/06", "ECU_ECULINE_SO_12345")).Repeat.Any();
			...
			mapTester.ExecuteCompiled<CU2CarrierUniversal_SO>(input, expectedOutput);
