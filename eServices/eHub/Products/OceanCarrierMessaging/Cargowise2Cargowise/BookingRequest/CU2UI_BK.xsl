...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="msxsl ns0 CodeMapper ContextAccessor DataModelAccessor OCMHelper ListHelper userCSharp DateMapper SubscriptionHelper" version="1.0"
  ...
  <xsl:variable name="SubscribeDocumentName" select="DataModelAccessor:InsertSubscriptionValue('CW1MSG', $eHubCargowiseClientID, $SenderID, $SubscriberConsolReference, 'Booking Request', 'DocumentName')" />
    ...
    <ns0:UniversalInterchange>
        ...
        <ns0:Acknowledgement>
          ...
          <ns0:ContextCollection>
            ...
            <ns0:Context>
              ...
              <ns0:Value>Booking Request</ns0:Value>
            </ns0:Context>
          </ns0:ContextCollection>
        </ns0:Acknowledgement>
      </ns0:Header>
      <ns0:Body>
        <ns0:UniversalShipment>
          <xsl:choose>
            <xsl:when test="$purposeCode='WTH'">
              <xsl:apply-templates mode="WTH"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:apply-templates/>
            </xsl:otherwise>
  ...
  <xsl:template match="@*|node()" mode="WTH">
    <xsl:copy>
      <xsl:apply-templates select="@*|node()" mode="WTH"/>
    </xsl:copy>
  </xsl:template>

  <xsl:template match="ns0:DataTargetCollection|ns0:DataSourceCollection|ns0:ActionPurpose|ns0:Company|ns0:DataProvider|ns0:EnterpriseID|ns0:EventBranch|ns0:ServerID|ns0:AddInfoCollection|ns0:RegistrationNumberCollection|ns0:Shipment/ns0:PackingLineCollection|ns0:PaymentHandlingInstructionCollection|ns0:ShipmentIncoTerm|ns0:WayBillType" mode="WTH"/>

  <xsl:template match="/ns0:UniversalShipment/ns0:Shipment" mode="WTH">
    <ns0:Shipment>
      <xsl:apply-templates select="ns0:DataContext|ns0:AgentsReference" mode="WTH" />
      <xsl:apply-templates select="ns0:BookingConfirmationReference|ns0:CoLoadBookingConfirmationReference|ns0:ShipmentType" />
      ...
      <xsl:variable name="orgCurrentUser" select="ns0:OrganizationAddressCollection/ns0:OrganizationAddress[ns0:AddressType/text()='CurrentUser']" />
      <xsl:if test="$orgCurrentUser">
        <ns0:OrganizationAddressCollection>
          <xsl:call-template name="PopulateOrganisationAddressCore">
            <xsl:with-param name="org" select="$orgCurrentUser" />
  ...
  <xsl:template match="ns0:DataTargetCollection|ns0:DataSourceCollection|ns0:ActionPurpose|ns0:Company|ns0:DataProvider|ns0:EnterpriseID|ns0:EventBranch|ns0:ServerID|ns0:AddInfoCollection|ns0:RegistrationNumberCollection|ns0:Shipment/ns0:PackingLineCollection|ns0:PaymentHandlingInstructionCollection|ns0:ShipmentIncoTerm|ns0:WayBillType|ns0:RecipientRoleCollection" />
  ...
  <xsl:template match="ns0:RecipientRoleCollection">
    <xsl:call-template name="CreateRecipientRoleCollection" />
  </xsl:template>

  <xsl:template match="ns0:RecipientRoleCollection" mode="WTH">
    <xsl:call-template name="CreateRecipientRoleCollection" />
  </xsl:template>

  <xsl:template name="CreateRecipientRoleCollection">
  ...
  <xsl:template match="/ns0:UniversalShipment/ns0:Shipment/ns0:ShipmentType">
    <xsl:if test="$purposeCode!='WTH'">
      <xsl:variable name="freightCharge" select="$shipment/ns0:PaymentHandlingInstructionCollection/ns0:PaymentHandlingInstruction[ns0:Category/ns0:Code/text()='FRT']" />
      <ns0:ShipmentIncoTerm>
        <xsl:choose>
          <xsl:when test="$eHubPartyType='NVOCC' and $freightCharge">
            <xsl:variable name="freightChargePaymentMethod" select="$freightCharge/ns0:PaymentMethod/ns0:Code/text()" />
            <ns0:Code>
              <xsl:choose>
                <xsl:when test="$freightChargePaymentMethod='PPD'">CFR</xsl:when>
                <xsl:otherwise>FOB</xsl:otherwise>
              </xsl:choose>
            </ns0:Code>
            <ns0:Description>
              <xsl:choose>
                <xsl:when test="$freightChargePaymentMethod='PPD'">Cost And Freight</xsl:when>
  ...
  <xsl:template match="ns0:LocalProcessing">
    <xsl:if test="$eHubPartyType='NVOCC'">
      <xsl:variable name="pickupDate" select="$shipment/ns0:DateCollection/ns0:Date[ns0:Type/text()='Pickup']/ns0:Value/text()" />
      ...
      <ns0:LocalProcessing>
        ...
        <ns0:PickupRequiredBy>
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:value-of select="$shipment/ns0:ContainerCollection/ns0:Container/ns0:DepartureEstimatedPickup/text()" />
            </xsl:otherwise>
          </xsl:choose>
        </ns0:PickupRequiredBy>
      </ns0:LocalProcessing>
  ...
  <xsl:template match="ns0:DateCollection">
    <ns0:DateCollection>
      ...
      <xsl:call-template name="CreateDateElement">
        <xsl:with-param name="type" select="'ClientRequestedETA'"/>
        <xsl:with-param name="value" select="$latestDeliveryDate"/>
      </xsl:call-template>
      ...
      <xsl:call-template name="CreateDateElement">
        <xsl:with-param name="type" select="'Arrival'"/>
        <xsl:with-param name="value" select="$arrivalDate"/>
      </xsl:call-template>
      ...
      <xsl:call-template name="CreateDateElement">
        <xsl:with-param name="type" select="'Departure'"/>
        <xsl:with-param name="value" select="$departureDate"/>
      </xsl:call-template>
    </ns0:DateCollection>
  </xsl:template>
  ...
  <xsl:template match="/ns0:UniversalShipment/ns0:Shipment/ns0:NoteCollection">
    ...
    <xsl:for-each select="$shipment/ns0:PaymentHandlingInstructionCollection/ns0:PaymentHandlingInstruction">
      <xsl:variable name="addToList" select="ListHelper:AddToListIfNotExists(concat(ns0:Category/ns0:Description/text(), ' - ', ns0:PaymentMethod/ns0:Description/text()))" />
    </xsl:for-each>
    ...
    <ns0:NoteCollection>
      ...
      <xsl:variable name="mainLeg" select="$shipment/ns0:TransportLegCollection/ns0:TransportLeg[ns0:LegType/text()='Main']"/>
      <xsl:if test="$mainLeg">

        <xsl:variable name="clearList1" select="ListHelper:ClearList()"/>
        <xsl:variable name="vesselNote" select="userCSharp:GetBillingNoteDescription('Vessel (Lloyds)', $mainLeg/ns0:VesselName/text(), $mainLeg/ns0:VesselLloydsIMO/text())" />
        <xsl:variable name="voyageNote" select="userCSharp:GetBillingNoteDescription('Voyage Number', $mainLeg/ns0:VoyageFlightNo/text(), '')" />
        <xsl:variable name="portofLoadingNote" select="userCSharp:GetBillingNoteDescription('Port Of Loading (ETD)', $mainLeg/ns0:PortOfLoading/ns0:Code/text(),DateMapper:ConvertToDate($mainLeg/ns0:EstimatedDeparture/text(),'yyyy-MM-dd'))" />
        <xsl:variable name="portOfDischargeNote" select="userCSharp:GetBillingNoteDescription('Port Of Discharge (ETA)', $mainLeg/ns0:PortOfDischarge/ns0:Code/text(), DateMapper:ConvertToDate($mainLeg/ns0:EstimatedArrival/text(),'yyyy-MM-dd'))" />

        <xsl:variable name="addToList1" select="ListHelper:AddToListIfNotExists('Requested Departure Vessel:')" />
        <xsl:variable name="addToList2" select="ListHelper:AddToListIfNotExists($vesselNote)" />
        <xsl:variable name="addToList3" select="ListHelper:AddToListIfNotExists($voyageNote)" />
        <xsl:variable name="addToList4" select="ListHelper:AddToListIfNotExists($portofLoadingNote)" />
        <xsl:variable name="addToList5" select="ListHelper:AddToListIfNotExists($portOfDischargeNote)" />
        <xsl:variable name="billingNote" select="ListHelper:ToStringWithNewLine()"/>

        <ns0:Note>
          <ns0:Description>Booking Notes</ns0:Description>
          <ns0:NoteText>
            <xsl:value-of select="$billingNote"/>
          </ns0:NoteText>
          <ns0:IsCustomDescription>false</ns0:IsCustomDescription>
        </ns0:Note>
      </xsl:if>
  ...
  <xsl:template match="/ns0:UniversalShipment/ns0:Shipment/ns0:PackingLineCollection">

    <xsl:if test="not(/ns0:UniversalShipment/ns0:Shipment/ns0:SubShipmentCollection/ns0:SubShipment)">
      <ns0:PackingLineCollection>
        <xsl:for-each select="./ns0:PackingLine">
          <xsl:variable name="referenceNumber" select="ns0:ReferenceNumber/text()" />

          <ns0:PackingLine>
            <xsl:copy-of select ="ns0:ContainerLink"/>
            <xsl:copy-of select ="ns0:ContainerNumber"/>
            <xsl:copy-of select ="ns0:DetailedDescription"/>
            <xsl:copy-of select ="ns0:ExportReferenceNumber"/>
            <xsl:copy-of select ="ns0:GoodsDescription"/>
            <xsl:copy-of select ="ns0:HarmonisedCode"/>
            <xsl:copy-of select ="ns0:MarksAndNos"/>
            <xsl:copy-of select ="ns0:PackQty"/>
            <xsl:copy-of select ="ns0:PackType"/>
            <ns0:ReferenceNumber>
              <xsl:value-of select="$referenceNumber"/>
            </ns0:ReferenceNumber>
            <xsl:copy-of select ="ns0:Vehicle"/>
            <xsl:copy-of select ="ns0:Volume"/>
            <xsl:copy-of select ="ns0:VolumeUnit"/>
            <xsl:copy-of select ="ns0:Weight"/>
            <xsl:copy-of select ="ns0:WeightUnit"/>
            <xsl:copy-of select ="ns0:ClassificationCollection"/>
            <xsl:copy-of select ="ns0:UNDGCollection"/>
          </ns0:PackingLine>
        </xsl:for-each>
      </ns0:PackingLineCollection>
    </xsl:if>
  ...
  <xsl:template name="CreatePackingLine">
    ...
    <xsl:variable name="referenceNumber" select="$packingLine/ns0:ReferenceNumber/text()" />
    <ns0:PackingLine>
  ...
  <msxsl:script language="C#" implements-prefix="userCSharp">
    ...
    public string GetBillingNoteDescription(string keyName, string keyValue, string optionalValue = "")
    {
      var optionalText = !string.IsNullOrEmpty(optionalValue) ? "(" + optionalValue + ")" : "";

      return !string.IsNullOrEmpty(keyValue) ? string.Format("{0}: {1} {2}", keyName, keyValue, optionalText).Trim() : string.Empty;
    }

    public string GetNoteIsCustomDescription(string noteType)
    {
        var isTrueNoteTypes = new System.Collections.Generic.List<string>() { "ChargesFreighted" };
        var isFalseNoteTypes = new System.Collections.Generic.List<string>() { "Goods Handling Instructions", "Forwarding Instruction Notes", "Special Instructions" };
    ...
    public bool ShouldCreateDate(string dateType)
    {
      ...
      {
        var disallowDateTypes = new System.Collections.Generic.List<string>() {"EarliestDeparture", "LatestDelivery", "Arrival", "Departure", "Pickup"};
