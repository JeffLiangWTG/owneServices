...
{
	[TestClass]
	public class EDI756_2UInterchangeInclude_Tests
	{
		const string filePath = "EDI756_2UInterchangeInclude.TestFiles.";

		[TestMethod, TestProperty("DAT:CapabilityRequirements", "BIZTALK2020")]
		public void Test_XML756_2UInterchangeInclude()
		{
			AssertMapping("Test1_input.xml", "Test1_output.xml", "20230243684");
			AssertMapping("Test2_input.xml", "Test2_output.xml", "20230243684");
			AssertMapping("Test3_input.xml", "Test3_output.xml", "20230243684");
			AssertMapping("Test4_input.xml", "Test4_output.xml", "20240243684");
		...
		void AssertMapping(string inputFile, string expectedOutputFile, string reference)
		{
			...
			var subscribedJobNumber = reference == "20230243684" ? "SAPAR00000016" : string.Empty;
			...
			mockCodeMapper.Expect(x => x.CallActionProcedureHelper("SelectSubscribedReference", "@reference", "@senderId", senderId, "@recipientId", "", "@ST_ID", serviceProviderMsgId, "@value", reference, "@referenceType", "JobNumber")).Return(subscribedJobNumber).Repeat.Once();
			...
			mockCodeMapper.Expect(x => x.CallActionProcedureHelper("SelectSubscribedReference", "@reference", "@senderId", senderId, "@recipientId", "", "@ST_ID", serviceProviderMsgId, "@value", reference, "@referenceType", "OperationPort")).Return("NLAMS").Repeat.Any();
			...
			mockDataModelAccessor.Expect(x => x.InsertSubscriptionValue("CGNTRC", recipientID, senderId, subscribedJobNumber + "_" + reference + "_0000099", "MRJ", "STU-756")).Repeat.Any();
			...
			mapTester.Execute<EDI756_2UInterchangeInclude>(input, expectedOutput);
