...
namespace Renci.SshNet.Sftp
{
	internal class SftpSession : SubsystemSession, ISftpSession
	{
		...
		/// Added for logging.
		/// </customization>
		private TraceListener _traceLog;
		...
		/// <customization>
		/// Added 'traceLog' parameter to constructor and initialized local '_traceLog' field.
		/// </customization>
		public SftpSession(ISession session, TimeSpan operationTimeout, Encoding encoding, TraceListener traceLog)
		...
		private void Log(string message, string prefix)
		{
			if (this._traceLog != null)
				if (String.IsNullOrWhiteSpace(prefix))
					this._traceLog.WriteLine(message);
				else
					foreach (var line in message.TrimEnd().Split(new[] { Environment.NewLine }, StringSplitOptions.None))
						this._traceLog.WriteLine(String.Format("{0} {1}", prefix, line));
		}
	}
}
