...
namespace CargoWise.eHub.BizTalkAdapters.Loopback.Admin
{
	public class LoopbackAdapterManagement : 
		IAdapterConfig,
		...
		IAdapterConfigValidation
	{
		...
		public string GetConfigSchema(ConfigType configType)
		{
			...
			{
				using (var xsd = Assembly.GetExecutingAssembly().GetManifestResourceStream("CargoWise.eHub.BizTalkAdapters.Loopback.Admin.LoopbackTransmitLocation.xsd"))
		...
		public string ValidateConfiguration(ConfigType configType, string configuration)
		{
			...
			var uriValue = new Uri("loopback://" + Uri.EscapeUriString(name.Value));
