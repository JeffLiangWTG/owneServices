...
{
    internal class GetOrchStatusCommand : AbstractCommand
    {
        internal GetOrchStatusCommand(string[] args) : base(args) { }

        /// <summary>
        /// Validate the arguments and then execute the orchestration status query
        /// </summary>
                    ...
                    foreach (Application application in catalogue.Applications)
                    {
                        foreach (BtsOrchestration orchestration in application.Orchestrations)
                        {
                            ...
                            if (matcher.Success)
                            {
                                ...
                                OrchestrationStatus status = orchestration.Status;
                                _output.Add(String.Format("Application: {0}; Orchestration: {1}; Status: {2}",
                                    application.Name, orchestration.FullName, status));
                            }
                        }
                    }
                }
        ...
        protected override void ReportUsage()
        {
            ...
            usage.Add(base.GetExeName() + " " + CommandType.GetOrchStatus.ToString() + 
                " -TypeName:<Orchestration Type Name RegEx>");
