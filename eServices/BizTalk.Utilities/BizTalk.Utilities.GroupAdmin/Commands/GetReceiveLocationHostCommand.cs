...
{
    internal class GetReceiveLocationHostCommand: AbstractCommand
    {
        internal GetReceiveLocationHostCommand(string[] args) : base(args) { }

        /// <summary>
        /// Validate the arguments and then query the receive location host properties
        /// </summary>
                ...
                using (BtsCatalogExplorer catalogue = new BtsCatalogExplorer())
                {
                    ...
                    foreach (ReceivePort receivePort in catalogue.ReceivePorts)
                    {
                        foreach (ReceiveLocation receiveLocation in receivePort.ReceiveLocations)
                        {
                            ...
                            {
                                locationFound = true;
                                _output.Add(String.Format("ReceiveLocation: {0}; Host: {1}",
                                    receiveLocation.Name, receiveLocation.ReceiveHandler.Host.Name));
                            }
                        }
                    }
                }
        ...
        protected override void ReportUsage()
        {
            ...
            usage.Add(base.GetExeName() + " " + CommandType.GetReceiveLocationHost.ToString() +
            ...
            usage.Add(string.Empty);
            usage.Add(base.GetExeName() + " " + CommandType.GetReceiveLocationHost.ToString() + " -ReceiveLocation:^pControl_Services_AckNack$");
            usage.Add(base.GetExeName() + " " + CommandType.GetReceiveLocationHost.ToString() + " -ReceiveLocation:[a-zA-Z]");
