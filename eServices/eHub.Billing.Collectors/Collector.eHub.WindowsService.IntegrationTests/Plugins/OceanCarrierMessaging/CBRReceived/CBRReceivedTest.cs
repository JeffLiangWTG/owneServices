...
using CargoWise.eServices.Billing.Collector.eHub.WindowsService.Plugins.OceanCarrierMessaging.CBRReceived;
using NUnit.Framework;

namespace CargoWise.eServices.Billing.Collector.eHub.WindowsService.IntegrationTests.Plugins.OceanCarrierMessaging.CBRReceived
{
	class CBRReceivedTest : PluginTest<Plugin>
	{
		[Test]
		public void TestReceiveMessageNVO()
		{
			...
			var messageToShippingInstruction = new eHubArchiveMessage
			{
				...
				AM_RecipientMessageXML = GetMessageXml("ReceivedMessageNVO"),
		...
		[Test]
		public void TestReceiveMessageNotNVO()
		{
			var RT_PK = Guid.NewGuid();
			var eHubRegistrationType = new eHubRegistrationType
			{
			...
			var messageToShippingInstruction = new eHubArchiveMessage
			{
				...
				AM_CC_RecipientOutbox = client.CC_PK,
				AM_CC_RecipientInbox = shippingInstruction.CC_PK,
				AM_RecipientMessageXML = GetMessageXml("ReceivedMessageNotNVO"),
				AM_ReceivedFromSenderUTC = messageReceivedTime,
				AM_OutboxMessageTrackingID = new Guid("{790BE798-2F09-4BA2-BC9A-458648472B6B}"),
			};
			AddArchiveMessage(messageToShippingInstruction);
			var expectedTransactions = new[]
			{
				new TimeStampedTransaction(messageArchivedTime, new BillingTransaction
				{
					...
					ClientID = "TSTCLIENT",
					Reference1 = "790be798-2f09-4ba2-bc9a-458648472b6b",
					Reference2 = "CBR2DDEE10S000688850",
					Reference3 = "BookingConfirmReference",
					Reference4 = "WayBillNumber",
					Reference5 = "1STOPTEST",
					ServiceOccuredUTC = messageReceivedTime,
					Category = "SHI",
					PriceItemCode = "CBR",
