...
namespace CargoWise.eServices.Billing.Collector.eHub.WindowsService.Plugins.OceanTracing
{
	public class Plugin : SqlBillingTransactionsPlugin
	{
		protected override sealed string QueryName
		{
			get { return "CargoWise.eServices.Billing.Collector.eHub.WindowsService.Plugins.OceanTracing.Query.sql"; }
		}
		...
		protected override IEnumerable<TimeStampedTransaction> CreateTransactions(IDataRecord record)
		{
			...
			yield return new TimeStampedTransaction(GetDateTime(record, "AM_ArchivedUTC"), new BillingTransaction
			{
				BillableCount = 1,
				ReportingSource = "HUB",
				ClientID = GetString(record, "ClientID").Substring(0, 9),
				Reference1 = messageTrackingID,
				Reference2 = GetString(record, "EventType"),
				Reference3 = GetString(record, "EventTime"),
				Reference4 = GetString(record, "ContainerNumber"),
