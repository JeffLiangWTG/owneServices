...
public class WiseCloudReadOnlyPluginTest
{
    ...
    {
        var referencePath = Path.Combine(TestContext.CurrentContext.TestDirectory,
            "WiseCloudReadOnlyPluginTest", "TestData", "test_reference.csv");
        ...
        var prodsettings = new PluginSettings
        {
            Parameters = new PluginParameter[]
            {
                ...
                new PluginParameter("ReferenceFilePath", referencePath)
            }
        };
        ...
        Assert.That(plugin.ReferenceFilePath, Is.EqualTo(referencePath));
