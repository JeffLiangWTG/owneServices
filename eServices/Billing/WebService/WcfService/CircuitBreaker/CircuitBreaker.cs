using System;

namespace CargoWise.eServices.Billing.WcfService.CircuitBreaker
{
    public class CircuitBreaker
	{
        private readonly object monitor = new object();
        private CircuitBreakerState state;

        public CircuitBreaker(int threshold, int maxRetries, TimeSpan timeout)
        {
            if (threshold < 1)
			{
                throw new ArgumentOutOfRangeException("Threshold should be greater than 0");
            }

            if (maxRetries < 1)
            {
	            throw new ArgumentOutOfRangeException("MaxRetries should be greater than 0");
            }

            if (timeout.TotalMilliseconds < 1)
			{
                throw new ArgumentOutOfRangeException("Timeout should be greater than 0");
            }

            Threshold = threshold;
            MaxRetries = maxRetries;
            Timeout = timeout;
            MoveToClosedState();
        }

        public int Failures { get; private set; }
        public int RetryCount { get; private set; }
        public int Threshold { get; }
        public int MaxRetries { get; }
        public TimeSpan Timeout { get; }
        public bool IsClosed
        {
            get { return state.Update() is ClosedState; }
        }

        public bool IsOpen
        {
            get { return state.Update() is OpenState; }
        }

        public bool IsHalfOpen
        {
            get { return state.Update() is HalfOpenState; }
        }

        internal CircuitBreakerState MoveToClosedState()
        {
            state = new ClosedState(this);
            return state;
        }

        internal CircuitBreakerState MoveToOpenState()
        {
            state = new OpenState(this);
            return state;
        }

        internal CircuitBreakerState MoveToHalfOpenState()
        {
            state = new HalfOpenState(this);
            return state;
        }

        internal void IncreaseFailureCount()
        {
            Failures++;
        }

        internal void IncreaseRetryCount()
        {
	        RetryCount++;
        }

        internal void ResetFailureCount()
        {
            Failures = 0;
            RetryCount = 0;
        }

        public bool IsThresholdReached()
        {
            return Failures >= Threshold;
        }

        public bool IsMaxRetriesReached()
        {
	        return RetryCount >= MaxRetries;
        }

        private Exception exceptionFromLastAttemptCall;

        public Exception GetExceptionFromLastAttemptCall()
        {
            return exceptionFromLastAttemptCall;
        }

        public CircuitBreaker AttemptCall(Action protectedCode)
        {
            this.exceptionFromLastAttemptCall = null;
            lock (monitor)
            {
                state.ProtectedCodeIsAboutToBeCalled();
                if (state is OpenState)
                {
                    return this;
                }
            }

            try
            {
                protectedCode();
            }
            catch (Exception e)
            {
                this.exceptionFromLastAttemptCall = e;
                lock (monitor)
                {
                    state.ActUponException(e);
                }
                return this;
            }

            lock (monitor)
            {
                state.ProtectedCodeHasBeenCalled();
            }
            return this;
        }

        public void Close()
        {
            lock (monitor)
            {
                MoveToClosedState();
            }
        }

        public void Open()
        {
            lock (monitor)
            {
                MoveToOpenState();
            }
        }
    }
}