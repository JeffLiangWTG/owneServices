USE [Billing2017]

DECLARE @expectedCount bigint = 0
DECLARE @actualCount bigint = 0
DECLARE @partitionFromPeriod int = 201701
DECLARE @partitionToPeriod int = 201801

--Note: [edi].[ClientCompany], [edi].[ClientCompanyCodeHistory], [edi].[LicenceDatabaseCodeHistory] should be verified immediately after copy as the souce data might change quickly.
--[edi].[ClientCompany]
SELECT @expectedCount = COUNT(*)
  FROM [Billing].[edi].[ClientCompany]

SELECT @actualCount = COUNT(*)
  FROM [edi].[ClientCompany]

IF @expectedCount = @actualCount
BEGIN
  PRINT 'The verification process on [edi].[ClientCompany] succeeded: ' + Concat('@expectedCount: ', @expectedCount, ', @actualCount: ', @actualCount);
END
ELSE 
BEGIN
  PRINT 'The verification process on [edi].[ClientCompany] failed: ' + Concat('@expectedCount: ', @expectedCount, ', @actualCount: ', @actualCount);
END

--[edi].[ClientCompanyCodeHistory]
SELECT @expectedCount = COUNT(*)
  FROM [Billing].[edi].[ClientCompanyCodeHistory]

SELECT @actualCount = COUNT(*)
  FROM [edi].[ClientCompanyCodeHistory]

IF @expectedCount = @actualCount
BEGIN
  PRINT 'The verification process on [edi].[ClientCompanyCodeHistory] succeeded: ' + Concat('@expectedCount: ', @expectedCount, ', @actualCount: ', @actualCount);
END
ELSE 
BEGIN
  PRINT 'The verification process on [edi].[ClientCompanyCodeHistory] failed: ' + Concat('@expectedCount: ', @expectedCount, ', @actualCount: ', @actualCount);
END

--[edi].[LicenceDatabaseCodeHistory]
SELECT @expectedCount = COUNT(*)
  FROM [Billing].[edi].[LicenceDatabaseCodeHistory]

SELECT @actualCount = COUNT(*)
  FROM [edi].[LicenceDatabaseCodeHistory]

IF @expectedCount = @actualCount
BEGIN
  PRINT 'The verification process on [edi].[LicenceDatabaseCodeHistory] succeeded: ' + Concat('@expectedCount: ', @expectedCount, ', @actualCount: ', @actualCount);
END
ELSE 
BEGIN
  PRINT 'The verification process on [edi].[LicenceDatabaseCodeHistory] failed: ' + Concat('@expectedCount: ', @expectedCount, ', @actualCount: ', @actualCount);
END

--[edi].[Usage]
SELECT @expectedCount = COUNT_BIG(*)
  FROM [Billing].[edi].[Usage]
  WHERE US_Period < @partitionToPeriod

SELECT @actualCount = COUNT_BIG(*)
  FROM [edi].[Usage]
  WHERE US_Period < @partitionToPeriod

IF @expectedCount = @actualCount
BEGIN
  PRINT 'The verification process on [edi].[Usage] succeeded: ' + Concat('@expectedCount: ', @expectedCount, ', @actualCount: ', @actualCount);
END
ELSE 
BEGIN
  PRINT 'The verification process on [edi].[Usage] failed: ' + Concat('@expectedCount: ', @expectedCount, ', @actualCount: ', @actualCount);
END

--[edi].[Chargeable]
SELECT @expectedCount = COUNT_BIG(*)
  FROM [Billing].[edi].[Chargeable]
  WHERE (@partitionFromPeriod IS NULL OR @partitionFromPeriod <= CH_Period)
	AND CH_Period < @partitionToPeriod

SELECT @actualCount = COUNT_BIG(*)
  FROM [edi].[Chargeable]

IF @expectedCount = @actualCount
BEGIN
  PRINT 'The verification process on [edi].[Chargeable] succeeded: ' + Concat('@expectedCount: ', @expectedCount, ', @actualCount: ', @actualCount);
END
ELSE 
BEGIN
  PRINT 'The verification process on [edi].[Chargeable] failed: ' + Concat('@expectedCount: ', @expectedCount, ', @actualCount: ', @actualCount);
END


